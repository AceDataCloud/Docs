...
- **Multimodal användarinmatning**: Genom det strukturerade `message`-fältet kan text + bild + filblock skickas direkt utan att först använda `references` för att indirekt bifoga.
...
Specifika avgiftsregler kan ses på tjänstens prissättningskort.

## Fleromgångsdialog

Som med v1, skicka `stateful: true` för att aktivera sessionslagring, API:t kommer att returnera ett `id`; för efterföljande begärningar, ta med `id` för att fortsätta dialogen, utan att behöva hantera meddelandehistorik själv.
...
| `tool_result`       | Resultatet av verktyget, parat med föregående `tool_use` via `tool_id`, `is_error` indikerar om det misslyckades.                                       |
...
| `ask_user_question` | Modellen skickar ut en begäran om att användaren ska komplettera information, konversationen går in i `awaiting_user_input`-status, se nedan [återuppta pausad konversation](#återuppta-pausad-konversation).                           |
...
För klienter som endast är intresserade av det slutgiltiga svaret, att sammanfoga alla `text_delta`- `content` är likvärdigt med `answer` i `application/json`-läget.
...
`max_turns` kan begränsa hur många gånger modellen kan anropa verktyg i denna begäran, standardgränsen bestäms av plattformen. Att sätta den låg (till exempel `max_turns: 1`) kan tvinga till ett enda svar, utan att tillåta några verktygsanrop.
...
Bakgrundsuppgifter har vanligtvis ingen som kan klicka för att bekräfta. Om du vill att vissa färdigheter eller MCP-servrar ska utföra sändning, publicering, skrivning etc. i obevakad läge, vänligen ange uttryckligen en förhandsauktoriseringslista i begäran:
...
Värdena i `allowed_skills` är sluggar för anslutna färdigheter; värdena i `allowed_mcp_servers` är sluggar för anslutna MCP-servrar. Färdigheter / MCP-servrar som inte listas i förhandsauktoriseringen kan fortfarande bara förhandsgranska, köra torrkörning eller vägra att utföra skrivoperationer i obevakad läge.

Om mer finjusterad kontroll behövs kan du också använda det ekvivalenta `unattended_policy` objektet:
...
Observera: Förhandsauktorisering representerar endast "denna begäran tillåter dessa förmågor att hoppa över manuell bekräftelse i obevakad läge". Specifika färdigheter måste fortfarande stödja `--unattended-confirm` eller motsvarande säkerhetsmekanism; annars kommer det att fortsätta med torrkörning och inte direkt utföra skrivoperationer.
...
I fronten rendera denna händelse som ett kort för att låta användaren välja svar, och skicka nästa begäran med samma `id`, fyll i svaret genom `tool_results`:
...
I begäran måste `tool_use_id` **vara** helt identisk med den pausade `tool_id`; annars returneras 400. När `tool_results` finns i begäran kommer `question` / `message` / `references` att ignoreras.

Om användaren beslutar att överge denna fråga, skicka bara en ny `question` / `message` så kommer plattformen automatiskt att markera det pausade verktygsanropet som "användaren hoppade över".

## Sessionshantering (CRUD)

v2 erbjuder lättvikts sessionshantering via `action` fältet på samma endpoint, utan behov av att öppna en annan API.
...
Returnera `{ items: [...], total }`. **Översikten innehåller inte `messages`**, lämplig för sidofältlista; om användaren klickar på ett samtal, använd sedan `action: retrieve` för att hämta dess fullständiga meddelanden.
...
### `action: delete` —— Ta bort ett samtal
...
```json
{
  "error": {
    "code": "chat_error",
    "message": "modellservicen returnerade ett fel"
  },
...
- `404 not_found`: `action: retrieve / update / delete` när `id` motsvarande samtal inte finns.
- `429 too_many_requests`: Utlöst hastighetsbegränsning.
- `500 chat_error`: Uppströms LLM rapporterade ett fel eller denna omgång `completion_tokens=0` (behandlas som icke-använd, kommer inte att debiteras).

I strömmande svar skickas fel som `{"type":"error","message":"..."}` händelser, och strömmen avslutas omedelbart efter.
