...
Som i v1, skicka `stateful: true` för att aktivera sessionslagring, API:t kommer att returnera ett `id`; för efterföljande begärningar, ta med `id` för att fortsätta dialogen utan att själv behöva hantera meddelandehistorik.
...
För klienter som endast är intresserade av det slutgiltiga svaret, att sammanfoga allt `text_delta`-innehåll är likvärdigt med `answer` i `application/json`-läget.
...
v2:s kärnförbättring är att modellen kan självständigt anropa verktyg för att slutföra flerstegsuppgifter, **detta är som standard aktiverat**, utan att klienten behöver göra några ytterligare konfigurationer i begäran. Vanliga scenarier:
...
## Återställning av pausade konversationer
...
I fronten renderar du denna händelse som ett kort för att låta användaren välja svar, och sedan skickar du nästa begäran med samma `id`, fyller i svaret genom `tool_results`:
...
Om användaren bestämmer sig för att ge upp denna fråga, skicka bara en ny `question` / `message` så kommer plattformen automatiskt att markera det pausade verktygsanropet som "användaren hoppade över".

## Sessionshantering (CRUD)

v2 erbjuder lättvikts sessionshantering via `action` fältet på samma endpoint, utan att behöva öppna en annan API.
