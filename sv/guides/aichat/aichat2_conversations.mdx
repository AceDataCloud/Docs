...
- **Multimodal användarinmatning**: Genom det strukturerade `message`-fältet kan text + bilder + filblock skickas direkt utan att först behöva bifogas indirekt med `references`.
...
```json
{
  "answer": "AceDataCloud är en enhetlig API-plattform som samlar ledande AI-modeller och multimodala tjänster, där utvecklare kan anropa tjänster som GPT, Claude, Gemini, Midjourney, Suno, Veo med en nyckel.",
...
> `stateful` är som standard `true`, att utelämna det och att uttryckligen skicka `true` är likvärdigt. Om du inte vill att servern ska spara denna konversation kan du uttryckligen ställa in `stateful: false`.
...
| `compact`           | Händelser där intern kontext har komprimerats, ingen speciell behandling krävs.                                                                             |
...
För klienter som bara är intresserade av det slutgiltiga svaret, att sammanfoga allt `text_delta`-innehåll ger samma resultat som `answer` i `application/json`-läget.
...
I fronten renderar du denna händelse som ett kort så att användaren kan välja svar, och med samma `id` initierar du nästa begäran och fyller i svaret genom `tool_results`:
...
Returnera det fullständiga samtalsdokumentet (inklusive `messages` historia, `model`, `title`, `tools_used` osv.).
...
2. Om du tidigare skickade v1-modellnamn (som `gpt-3.5`, `gpt-4-browsing` osv.), rekommenderas det att uppgradera till moderna modeller (som `gpt-5.4`, `claude-opus-4-8`, `gemini-3.1-pro-preview` osv.) vid övergång till v2.
...
- `400 bad_request`: Saknar obligatoriska fält, `tool_use_id` matchar inte, `messages` schema är ogiltigt osv.
...
I strömmande svar skickas fel som `{"type":"error","message":"..."}` händelser, och strömmen avslutas omedelbart efteråt.

## Slutsats

AI Chat v2 API är bakåtkompatibelt med v1 samtidigt som det uppgraderar konversationer från "enkla/multipla frågor och svar" till "Agent-baserade observerbara konversationer": multimodala indata, verktygsanrop, pausbar/återupptagbar, strömmade strukturerade händelser, inbyggd CRUD. Det rekommenderas att nya integrationer direkt använder v2; befintliga v1-integrationer kan migreras smidigt i faser. Om du har några frågor, tveka inte att kontakta vårt tekniska supportteam.
