...
- **Multimodal användarinmatning**: Genom det strukturerade `message`-fältet kan man direkt skicka text + bilder + filblock utan att först använda `references` för att indirekt bifoga.
- **Agent-baserade verktygsanrop**: Inbyggd uppsättning av verktyg för nätverksökning, webbsökning, filavläsning med mera, och kan kopplas till användarauktoriserade MCP-servrar (Google Drive, Notion, Slack, GitHub etc.), modellen kan i en enda begäran självständigt anropa verktyg för att slutföra komplexa uppgifter.
- **Strukturerade strömmande händelser**: Genom `accept: text/event-stream` eller `application/x-ndjson` kan man få token-för-token `text_delta`, `tool_use`, `tool_result`, `thinking`, `citation`, `card`, `artifact` och andra händelser, vilket underlättar rendering i frontend enligt motsvarande typ.
...
Specifika avgiftsregler kan ses på tjänstsidan under Pricing-kortet.

## Fleromgångsdialog

Som med v1, skicka `stateful: true` för att aktivera sessionslagring, API:t kommer att returnera ett `id`; för efterföljande begärningar tar du med `id` för att fortsätta dialogen, utan att behöva hantera meddelandehistorik själv.
...
| `type`              | Betydelse                                                                                              |
| ------------------- | ----------------------------------------------------------------------------------------------- |
| `text_delta`        | Inkrementella textstycken från assistentens svar. `content` är det nya innehållet; för att vara kompatibel med v1, bär samma händelse också `delta_answer` (lika med `content`) och `id`.                  |
...
| `ask_user_question` | Modellen ställer en fråga till användaren när den behöver mer information, konversationen går in i `awaiting_user_input`-status, se nedan [återuppta pausad konversation](#återuppta-pausad-konversation).                           |
...
| `compact`           | Händelser där intern kontext har komprimerats, kräver ingen särskild behandling.                                                                             |
...
För klienter som bara är intresserade av det slutgiltiga svaret, att sammanfoga alla `text_delta`- `content` är likvärdigt med `answer` i `application/json`-läget.
...
Om du inte vill visa detaljer om verktygsanropet i fronten, ignorera `tool_use` / `tool_result` / `card` / `citation` dessa typer av händelser, modellen kommer fortfarande att leverera utdata genom `text_delta`.
...
Om användaren beslutar att ge upp denna fråga, skicka bara en ny `question` / `message` så kommer plattformen automatiskt att markera det pausade verktygsanropet som "användaren har hoppat över".
