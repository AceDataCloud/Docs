...
Vid första användningen av detta API behöver vi fylla i minst tre fält, ett är `authorization`, som du enkelt väljer från rullgardinsmenyn. Det andra parametern är `model`, `model` är den Gemini-modellkategori vi väljer att använda, här har vi huvudsakligen 6 olika modeller, detaljer kan ses i de modeller vi tillhandahåller. Det sista parametern är `messages`, `messages` är en array av våra inmatade frågor, den är en array som gör det möjligt att ladda upp flera frågor samtidigt, varje fråga innehåller `role` och `content`, där `role` representerar frågeställarens roll, vi erbjuder tre identiteter: `user`, `assistant`, `system`. Den andra `content` är det specifika innehållet i vår fråga.
...
Där `choices` innehåller Gemini:s svarsinformation, och `choices` inuti det är den specifika informationen om Geminis svar, som kan ses i bilden.

<p><img src="https://cdn.acedata.cloud/v4z6e0.png" width="400" className="m-auto" /></p>

Det kan ses att `choices` inuti `content`-fältet innehåller det specifika innehållet i Geminis svar.

## Bildförståelse (Multimodal Inmatning)

Gemini är en inbyggd multimodal modell som kan "se bilder" direkt. För att skicka in bilder, ändra `content` för ett meddelande från en sträng till en **innehållsblock-array**, där du samtidigt lägger in `text`-block och `image_url`-block - detta är helt i linje med OpenAI:s och den officiella Geminis OpenAI-kompatibla format.
...
När `stream` ändras till `true`, kommer API:t att returnera motsvarande JSON-data rad för rad, och vi behöver göra nödvändiga ändringar i koden för att få rad-för-rad resultat.
...
- `choices`, svarsinformation som Gemini ger för frågan.
JavaScript är också stödd, till exempel Node.js strömmande anropskod ser ut så här:
...
Som du kan se, innehåller informationen i `choices` samma innehåll som den grundläggande användningen, detta innehåller den specifika informationen som Gemini ger för flera dialoger, så att man kan svara på motsvarande frågor baserat på flera dialoginnehåll.
...
Gemini 3.1 Pro är en uppgraderad version av Gemini 3.0 Pro, med en underliggande modell av `gemini-3.1-pro-preview`, som också stöder text, bilder, video och andra multimodala inmatningar, med starkare resonemang och förståelse. Användningen är helt identisk med Gemini 3.0 Pro, du behöver bara byta `model`-parametern till `gemini-3.1-pro`.
...
Returformatet är detsamma som Gemini 3.0 Pro, se beskrivningen i avsnittet om Gemini-3.0 multimodala modeller ovan.
...
Genom detta dokument har du fått en förståelse för hur du enkelt kan implementera den officiella Gemini:s chattfunktionalitet med Gemini Chat Completion API. Vi hoppas att detta dokument kan hjälpa dig att bättre integrera och använda detta API. Om du har några frågor, tveka inte att kontakta vårt tekniska supportteam.
