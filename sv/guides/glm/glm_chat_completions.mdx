...
GLM (General Language Model) är en ny generation av stora språkmodeller som lanserats av Zhipu AI (智谱 AI / Z.ai), med starka förmågor inom förståelse och generering av både kinesiska och engelska. Modeller som GLM-5.1, GLM-4.7, GLM-4.6 och andra nya generationer har genomgått omfattande optimeringar för långa sammanhang, verktygsanrop och koduppgifter, och kan tillämpas brett inom intelligenta frågesystem, innehållsskapande, kodassistans, kundtjänstrobotar och andra scenarier.
...
- `model`: Välj den GLM-modell som ska anropas, för närvarande stöds följande modeller:
  ...
  - `glm-4.7`: Utmärkt prestanda inom resonemang, verktygsanrop och koduppgifter.
...
Du kan se att informationen i `choices` är i linje med grundläggande användning, modellen ger svar baserat på den kompletta dialoghistoriken, vilket stödjer flera rundor av kontextuell interaktion.
...
GLM-modellen stöder OpenAI-kompatibel Function Calling, du kan deklarera anropbara funktioner genom `tools` parametern, modellen kommer att returnera strukturerad funktionsanropsinformation i `choices[i].message.tool_calls` när det behövs.
...
Om modellen beslutar att anropa verktyget, kommer `finish_reason` i returresultatet att ändras till `tool_calls`, och i `message.tool_calls` kommer funktionsnamnet och JSON-strängformatet av parametrarna att ges. Du kan utföra den funktionen och återföra resultatet som ett meddelande med `role` som `tool` till modellen för att slutföra den fullständiga verktygsanropscykeln.
...
| `glm-3-turbo`  | Allmän textgenereringsuppgift, kostnadskänsliga scenarier |
...
Genom detta dokument har du fått en förståelse för hur man använder GLM Chat Completion API för att anropa Zhiyu AI:s GLM-serie modeller, inklusive grundläggande anrop, strömmande svar, flerrundiga dialoger, systempromptar och verktygsanrop. Vi hoppas att detta dokument kan hjälpa dig att bättre integrera och använda detta API. Om du har några frågor, tveka inte att kontakta vårt tekniska supportteam.
