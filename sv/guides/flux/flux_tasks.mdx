...
Flux Tasks API:s huvudfunktion är att genom att ange det uppdrag-ID som genererats av Flux Images Generation API för att fråga om statusen för det uppdraget.

Detta dokument kommer att detaljerat beskriva integrationen av Flux Tasks API, för att hjälpa dig att enkelt integrera och fullt utnyttja den kraftfulla funktionen hos detta API. Genom Flux Tasks API kan du enkelt fråga om statusen för uppdrag som utförs av Flux Images Generation API.

## Ansökningsprocess

För att använda Flux Images Generation API, börja med att gå till [Ace Data Cloud-konsolen](https://platform.acedata.cloud/console/applications) för att få din API-token, som du kan spara för framtida bruk.
...
Flux Tasks API kan användas för att fråga om resultaten från Flux Images Generation API. För information om hur man använder Flux Images Generation API, se dokumentationen [Flux Images Generation API](https://platform.acedata.cloud/documents/flux-images-integration).

Vi tar ett exempel med ett uppdrag-ID som returnerats av Flux Images Generation API och visar hur man använder detta API. Anta att vi har ett uppdrag-ID: 2db0168c-2373-4367-8d9a-9dc778802e8a, och vi visar hur man gör detta genom att ange ett uppdrag-ID.
...
Detta är för att fråga om detaljerna för flera uppdrag-ID:n, och till skillnad från ovanstående måste `action` väljas som `retrieve_batch`.
