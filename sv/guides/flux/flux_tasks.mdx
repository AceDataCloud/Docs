...
Flux Tasks API:s huvudsakliga funktion är att genom att ange det uppdragets ID som genererats av Flux Images Generation API, kunna kontrollera statusen för det uppdraget.
...
För att använda Flux Images Generation API, börja med att gå till [Ace Data Cloud-konsolen](https://platform.acedata.cloud/console/applications) för att hämta din API-token, som du kan spara för framtida bruk.
...
Vid en lyckad begäran kommer API:t att returnera detaljerad information om detta bilduppdrag. Till exempel:
...
Detta är för att fråga detaljer om flera uppdragets ID:n, och till skillnad från ovanstående behöver `action` väljas som `retrieve_batch`.
...
Vid en lyckad begäran kommer API:t att returnera detaljerad information om alla batchbilduppdrag. Till exempel:
```json
{
  "items": [
    {
