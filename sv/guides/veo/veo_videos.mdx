...
**En API-token kan användas för att anropa alla tjänster på plattformen, utan att behöva ansöka separat för varje tjänst.** Vid första ansökan får du en gratis kvot för att prova; om kvoten tar slut kan du ladda på allmän balans i [konsolen](https://platform.acedata.cloud/console/coin).
...
- `data`，resultatet av videoproduktionsuppgiften vid denna tidpunkt.
  ...
  - `video_url`，videoproduktionsuppgiftens video-länk vid denna tidpunkt.
...
Vi kan se att vi har fått tillfredsställande videoinformation, vi behöver bara hämta den genererade Veo-videon baserat på video-länken i resultatet `data`.
...
Om du vill generera en video baserat på första och sista bildramar kan du ställa in parametern `action` till `image2video` och ange en array av bildlänkar `image_urls` för första och sista bildramar.

Därefter måste vi fylla i nästa steg med utökade promptord för att anpassa den genererade videon, så att vi kan specificera följande innehåll:
...
## Generera video med specificerad storlek

Om du vill specificera en anpassad storlek för den genererade Veo-videon kan du ställa in parametern `aspect_ratio` till den önskade storleken. Därefter måste vi fylla i nästa steg med utökade promptord för att anpassa den genererade videon, så att vi kan specificera följande innehåll:
...
Genom detta dokument har du fått en förståelse för hur man använder Veo Videos Generation API för att generera video genom att ange promptord och en referensbild av första ramen. Vi hoppas att detta dokument kan hjälpa dig att bättre integrera och använda detta API. Om du har några frågor, tveka inte att kontakta vårt tekniska supportteam.
