---
title: "Recaptcha3 protokolligenkänning API integration instruktion"
description: "Recaptcha verification code recognition service API guide - Ace Data Cloud"
---

Denna artikel kommer att introducera en Recaptcha3 protokolligenkänning API integration instruktion, som gör det möjligt för användare att verifiera utan att behöva identifiera och klicka på Recaptcha3 verifieringsbilden, utan endast genom att skicka in Website Key för att uppnå automatisk avkodning i bakgrunden och slutföra verifieringen.

## Ansökningsprocess

För att använda Recaptcha3 protokolligenkänning API, börja med att gå till [Ace Data Cloud-konsolen](https://platform.acedata.cloud/console/applications) för att få din API Token, som du kan spara för framtida bruk.
...
Först, låt oss förstå den grundläggande användningen. Jämfört med Recaptcha2 behöver vi dessutom skicka in en parameter `page_action`, som måste hämtas från koden. Denna demonstrationswebbadress är: `https://recaptcha-demo.appspot.com/recaptcha-v3-request-scores.php`, nedan visas en metod för att hämta den:
...
- `token`, resultatet av Recaptcha3-verifieringen efter att uppgiften har behandlats.
...
Som standard är API:et synkront och blockerande: en begäran kommer att vänta tills tokenbehandlingen är klar innan den returneras. Om du gör en multipel kodlösning (multi-solver rotation) och vill "få task_id omedelbart efter att ha skickat uppgiften, för att sedan schemalägga andra kodlösningar och senare återkomma för att läsa resultatet", kan du skicka `async: true` i begäran.

När `async: true` skickas, kommer gränssnittet omedelbart att returnera ett `task_id` utan att blockera:
...
När behandlingen är klar kommer `status: ready` och token att returneras:
...
Avgiftsinformation: I asynkront läge debiteras inte skapandet av uppgifter och läsning av "behandlas"-status; **klienten debiteras en gång när den första framgångsrika resultatet läses** (samma som nuvarande beteende och priser för synkront läge). Servern kommer att driva uppgiften själv, men kommer inte att debitera i förväg bara för att bakgrunden slutförs först. Om uppgiften inte lyckas inom 120 sekunder kommer den att avslutas med HTTP 504 `timeout`, utan debitering. `/captcha/tasks` ansvarar inte för att driva uppgiften.
