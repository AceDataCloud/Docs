---
title: "Recaptcha3 protokollidentifiering API integration instruktion"
description: "Recaptcha verification code recognition service API guide - Ace Data Cloud"
---

Denna artikel kommer att introducera en Recaptcha3 protokollidentifiering API integration instruktion, som gör det möjligt för användare att verifiera utan att behöva identifiera och klicka på Recaptcha3 verifieringsbilder, utan endast genom att skicka in Website Key för att uppnå automatisk avkodning i bakgrunden och slutföra verifieringen.

## Ansökningsprocess

För att använda Recaptcha3 protokollidentifiering API, börja med att gå till [Ace Data Cloud-konsolen](https://platform.acedata.cloud/console/applications) för att få din API-token, som du kan spara för framtida bruk.
...
> 📘 Fullständig dokumentation: [Recaptcha3 protokollidentifiering API →](https://platform.acedata.cloud/documents/captcha-token-recaptcha3)

## Grundläggande användning

Först bör du förstå den grundläggande användningsmetoden. Jämfört med Recaptcha2 behöver vi dessutom skicka in en parameter `page_action`, som måste hämtas från koden. Denna demonstrationswebbadress är: `https://recaptcha-demo.appspot.com/recaptcha-v3-request-scores.php`, nedan visas en metod för att hämta den:

### Snabbmetod:

Öppna f12, och sök sedan på Element-sidan efter `.execute(`, i det röda rutan kan vi se att det finns en `action` parameter, samtidigt som execute följs av en sträng, vilket också är innehållet som behövs nedan, som visas i bilden nedan.

<p><img src="https://cdn.acedata.cloud/imwlto.png" width="500" className="m-auto" /></p>

Därefter behöver du ange URL:en för den webbplats som behöver hantera verifieringskoden, så att du kan få det bearbetade resultatet. Först behöver du enkelt skicka ett `website_url` fält, och slutligen behöver du ange parametern `website_key`, vilket kan hämtas från ovanstående text, också en sträng som följer efter execute. Vi kan nu fylla i motsvarande innehåll på gränssnittet, som visas i bilden nedan:
...
- `page_action`: som behöver hämtas från koden på webbplatsen med verifieringskoden.
...
- `token`, resultatet av verifieringen av Recaptcha3-koden efter bearbetning.
...
```
Det kan ses att `success` här representerar resultatet av valideringen, vilket innebär att vi framgångsrikt har passerat Recaptcha3-verifieringen.
...
Som standard är API:et synkront och blockerande: en begäran kommer att vänta tills tokenbehandlingen är klar innan den returneras. Om du gör en multipel lösare rotation och vill "omedelbart få task_id efter att ha skickat uppgiften, först schemalägga andra lösare och sedan återkomma för att hämta resultatet", kan du skicka `async: true` i begärningskroppen.

När du skickar `async: true` kommer gränssnittet omedelbart att returnera ett `task_id` utan att blockera:
...
Avgiftsinformation: I asynkront läge debiteras varken skapande av uppgifter eller polling "under bearbetning"; **debiteras endast en gång när resultatet har hämtats framgångsrikt** (samma pris som i synkront läge). Därför kommer avbokning av uppgifter som inte har slutförts under rotationen inte att medföra kostnader. `/captcha/tasks` är gemensamt för alla captcha-gränssnitt (token och recognition-serier) och kan pollas med samma `task_id`.
