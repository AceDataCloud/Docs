...
Du behöver också ange det relaterade identifieringsinnehållet för verifieringsbilden, parameter `question`, vi har bara tillhandahållit följande innehållstabell som referens:
...
- `accept`: vilken typ av svarformat du vill ta emot, här anges `application/json`, vilket innebär JSON-format.
...
- `solution`, resultatet av verifieringen efter att Recaptcha2 verifieringsbilden har bearbetats.
    ...
    - `label`, innehållet som identifierades i Recaptcha2 verifieringsbilden.
    - `confidences`, förtroendet för identifieringsområdet i Recaptcha2 verifieringsbilden, området börjar från 0.
...
Som standard är API:et synkront och blockerande: en begäran kommer att vänta tills resultatet av igenkänningen har behandlats innan det returneras. Om du gör en multi-solver rotation och vill "skicka uppgiften och omedelbart få task_id, först schemalägga andra lösare och sedan återkomma för att läsa resultatet", kan du skicka in `async: true` i begärningskroppen.

När du skickar in `async: true` kommer gränssnittet omedelbart att returnera ett `task_id` utan att blockera:
...
När behandlingen är klar kommer det att returnera `status: ready` och igenkänningsresultatet `solution` (fältstrukturen är helt identisk med den synkrona läget):
...
Avgiftsbeskrivning: I asynkront läge debiteras inte skapandet av uppgifter och läsning av "under behandling" status; **klienten debiteras en gång när den första framgångsrika resultatet läses** (i enlighet med befintligt beteende och priser för synkront läge). Servern kommer att driva uppgiften själv, men kommer inte att debitera i förväg bara för att bakgrunden har slutfört. Om uppgiften inte lyckas inom 120 sekunder kommer den att avslutas med HTTP 504 `timeout`, utan debitering. `/captcha/tasks` ansvarar inte för att driva uppgiften.
