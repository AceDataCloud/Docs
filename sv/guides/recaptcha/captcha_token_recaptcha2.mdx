...
**En API-token kan användas för att anropa alla plattformens tjänster, utan att behöva ansöka separat för varje tjänst.** Första ansökan ger en gratis kvot, så att du kan prova gratis; när kvoten är otillräcklig kan du ladda på allmän balans i [konsolen](https://platform.acedata.cloud/console/coin).
...
Från ovanstående bild kan vi se att URL:en för denna POST-förfrågan är: `https://www.google.com/recaptcha/api2/demo`, vi behöver bara skicka parametern `g-recaptcha-response`, och vi behöver bara överföra den bearbetade token i data nedan, koden för att anropa token för verifiering ser ut som följer:
...
Som standard är API:et synkront och blockerande: en förfrågan väntar tills token-behandlingen är klar innan den returneras. Om du gör en multipel kodlösning (multi-solver rotation) och vill "få task_id omedelbart efter att ha skickat in uppgiften, först schemalägga andra kodlösare och sedan återkomma för att hämta resultatet", kan du skicka `async: true` i begäran.
...
Avgiftsinformation: I asynkront läge debiteras inte skapandet av uppgifter och polling "under bearbetning"; **debiteras endast en gång när resultatet har hämtats framgångsrikt** (samma pris som i synkront läge). Därför kommer avbokning av uppgifter som ännu inte har slutförts under rotationen inte att medföra kostnader. `/captcha/tasks` är gemensam för alla verifieringsgränssnitt (token och recognition-serien), och du kan pollera med samma `task_id`.
