...
Denna artikel kommer att introducera en Recaptcha2 protokolligenkänning API integration instruktion, som gör att användare kan genomföra verifiering utan att behöva identifiera och klicka på Recaptcha2 verifieringsbilden, utan endast genom att skicka in Website Key för att uppnå automatisk avkodning i bakgrunden.

## Ansökningsprocess

För att använda Recaptcha2 protokolligenkänning API, börja med att gå till [Ace Data Cloud kontrollpanelen](https://platform.acedata.cloud/console/applications) för att få din API Token, som du kan spara för framtida bruk.
...
**En API Token räcker för att anropa alla plattformens tjänster, du behöver inte ansöka separat för varje tjänst.** Första ansökan ger en gratis kvot, så att du kan prova gratis; när kvoten är otillräcklig kan du ladda på allmän balans i [kontrollpanelen](https://platform.acedata.cloud/console/coin).
...
- `proxy`: valfritt, ta med egen proxy (Bring Your Own Proxy). När det är inställt kommer tjänsten att använda den proxy IP du tillhandahåller för att lösa verifieringskoden, för att kontrollera kvaliteten på utgående IP (till exempel för att undvika att offentliga proxy IP blockeras av målsidan och returnerar `410 Gone`). Formatet är `scheme://[user:pass@]host:port`, där `scheme` stöder `http`/`https`/`socks4`/`socks5`, till exempel `http://user:pass@1.2.3.4:8080`. Om det inte fylls i används plattformens standardproxy.
...
- Klicka sedan på skicka, och observera förändringarna i nätverkskonsolen, som visas i bilden nedan:
...
Som standard är API:et synkront och blockerande: en förfrågan kommer att vänta tills token-behandlingen är klar innan den returneras. Om du gör en multipel kodlösning (multi-solver rotation) och vill "omedelbart få task_id efter att ha skickat in uppgiften, först schemalägga andra kodlösare och sedan återkomma för att läsa resultatet", kan du skicka in `async: true` i begäran.

När du skickar in `async: true` kommer gränssnittet omedelbart att returnera ett `task_id` utan att blockera väntan:
...
Under behandling kommer det att returnera `status: processing`:
...
Avgiftsbeskrivning: I asynkront läge debiteras inte skapandet av uppgifter och läsning av "under behandling"-status; **klienten debiteras en gång när den första framgångsrika läsningen av resultatet görs** (samma som nuvarande beteende och priser för synkront läge). Servern kommer att driva uppgiften själv, men kommer inte att debitera i förväg bara för att bakgrunden slutförs först. Om uppgiften inte lyckas inom 120 sekunder kommer den att avslutas med HTTP 504 `timeout`, utan debitering. `/captcha/tasks` ansvarar inte för att driva uppgiften.
...
Genom detta dokument har du fått en förståelse för hur du använder Recaptcha2-protokollet för att identifiera API så att användare inte behöver identifiera och klicka på Recaptcha2-verifieringsbilden, utan bara behöver skicka in Website Key för att möjliggöra automatisk avkodning i bakgrunden och slutföra verifieringen. Vi hoppas att detta dokument kan hjälpa dig att bättre integrera och använda detta API. Om du har några frågor, tveka inte att kontakta vårt tekniska supportteam.
