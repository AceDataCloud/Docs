...
Antingen `id` eller `trace_id` måste anges. I allmänhet kan du direkt använda `id` från svaret på begäran, `trace_id` bör endast anges om du vill koppla det till din egen affärsidentifiering.
...
- `response`: Den slutliga responskroppen som returnerades när callbacken var klar.
...
| `ids`              | string[] | Hämta uppgifter baserat på en lista av uppgift-ID:n       |
...
## End-to-end exempel: Skicka och pollning
Tasks API tjänar huvudsakligen för asynkrona processer i callback-läge. I callback-läget kommer inlämningsgränssnittet att **omedelbart synkronisera och returnera en `task_id`** (dvs. uppgifts-ID), och därefter behöver du bara direkt använda denna `task_id` för att pollera Tasks-gränssnittet, utan att själv generera en `trace_id`.
...
# 2. Använd direkt task_id från inlämningssvaret för att pollera Tasks-gränssnittet tills uppgiften är klar
while True:
    task = requests.post(
        f"{API}/openai/tasks",
        headers=HEADERS,
...
- Tasks-gränssnittet **debiterar inte** och kan polleras utan bekymmer. Endast de ursprungliga bildgenererings-/redigeringsförfrågningarna kommer att debiteras.
