...
OpenAI bildredigeringstjänst kan ta emot bilder och instruktioner och leverera modifierade bilder. GPT Image-serien kan ta emot upp till 16 referensbilder samtidigt. För närvarande stöder API:et både `gpt-image-1`, den senaste **`gpt-image-2`**, samt **`nano-banana` / `nano-banana-2-lite` / `nano-banana-2` / `nano-banana-pro`** serien av modeller via samma gränssnitt.
...
- **Stöd för direktöverföring av URL**: Förutom traditionell `multipart/form-data` filuppladdning, stöder `gpt-image-2` även **extra stöd för att överföra bild-URL:er i JSON-format**, vilket eliminerar behovet av att först ladda ner bilderna lokalt, vilket är mycket lämpligt för serverbaserade arbetsflöden.
- **Stöd för direktöverföring av base64**: I enlighet med officiella riktlinjer kan `image`-fältet också direkt ta emot base64 (`data:image/png;base64,...` eller ren base64), vilket gör att lokala bilder kan redigeras utan att först behöva laddas upp till en bildvärd.
...
- **`gpt-image-2:official`**: Officiell kanal, stabil och regelrätt. Stöder verklig 2K / 4K högupplösning, **debitering per bild, enhetspriset är 2 gånger det vanliga `gpt-image-2`**. Om linjen inte är tillgänglig returneras ett fel direkt, utan automatisk nedgradering.
...
> Till exempel: Om originalbilden är `1024x1024`, när `size` anges som `2048x2048`, kommer modellen att omrita och leverera en 2K-bild enligt redigeringsinstruktionerna; när `size` anges som `3840x2160` kommer den att leverera en 4K liggande bild. Debiteringen för de tre är densamma.
> Om `auto` (eller om `size` utelämnas) anges, kommer utdata att **bevara referensbildens bildförhållande**—i det här exemplet är originalbilden 1:1, så resultatet kommer att bli 1:1 och inte pressas till en annan bildstorlek. Detta skiljer sig från genereringsgränssnittet: genereringsgränssnittet har ingen referensbild, `auto` väljer bildformat baserat på promptens betydelse. Om du vill ändra bildformatet, ange `size` uttryckligen.

> **Om `n`-parametern**
> `gpt-image-2` redigeringsgränssnittet stöder `n > 1`: en begäran kan returnera och debiteras för motsvarande antal redigeringsresultat (värdet för `n` är 1–10). Detta gäller också för `gpt-image-1` / `gpt-image-1.5`, samt `nano-banana` / `nano-banana-2-lite` / `nano-banana-2` / `nano-banana-pro` serien. Observera att `response_format=b64_json` endast stöder `n=1`, vid `n>1` bör du använda standard URL-retur. Om några av bilderna misslyckas med att genereras, kommer endast de framgångsrika delarna att returneras och debiteras.
...
payload = {
    ...
    "prompt": "Byt ut den träbokhyllan mot en elegant modern vit flytande hylla monterad på en pastellblå vägg. Behåll exakt samma arrangemang av böcker (1 bok ovanpå, 3 i mitten, 7 längst ner). Lägg till en liten krukväxt på den övre hyllan bredvid boken. Ljust luftigt dagsljus från vänster.",
    "size": "1024x1024"
}
```
...
Det kan ses att stil och miljö har bytts ut enligt prompten, men antalet böcker per hyllplan (1 / 3 / 7) har fortfarande strikt bevarats, och en krukväxt har lagts till enligt krav.
...
> - `image` kan antingen laddas upp som en fil via `multipart/form-data` (lokala filer konverteras automatiskt till base64), eller så kan bildens URL-sträng skickas direkt som ett formulärfält.
...
Nu kan vi använda kod för att göra anrop, nedan är ett exempel på hur man gör anrop med CURL:
...
Ett motsvarande Python-exempel för anropet:
...
För att använda Python-anropet behöver vi först importera två miljövariabler, en `OPENAI_BASE_URL`, som kan sättas till `https://api.acedata.cloud/openai`, och en annan variabel för autentisering `OPENAI_API_KEY`, detta värde hämtas från `authorization`, och på Mac OS kan vi ställa in miljövariabler med följande kommando:
...
Efter anropet kan vi se att ett resultat omedelbart erhålls, som följer:
...
Vi kan se att resultatet innehåller ett `task_id`-fält, `data`-fältet innehåller samma bildredigeringsresultat som vid synkron anrop, genom `task_id`-fältet kan uppgifterna kopplas samman.
