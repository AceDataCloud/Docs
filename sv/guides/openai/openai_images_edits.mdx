...
**En API-token kan användas för att anropa alla tjänster på plattformen, det behövs ingen separat ansökan för varje tjänst.** Första ansökan ger en gratis kvot för att prova; när kvoten är slut kan du ladda på allmänna medel i [konsolen](https://platform.acedata.cloud/console/coin).
...
- **Strukturen bibehålls mer stabil**: Vid byte av hud, färgschema eller bakgrund förstörs nästan aldrig den ursprungliga bildens layout och komposition.
...
- **Stöd för direktöverföring av URL**: Förutom traditionell `multipart/form-data` filuppladdning, stöder `gpt-image-2` även **extra stöd för att överföra bild-URL:er i JSON-format**, vilket gör det onödigt att först ladda ner bilderna lokalt, vilket är mycket lämpligt för serverbaserade arbetsflöden.
- **Stöd för direktöverföring av base64**: I enlighet med officiella riktlinjer kan `image`-fältet också direkt ta emot base64 (`data:image/png;base64,...` eller ren base64), vilket gör att lokala bilder kan redigeras utan att först laddas upp till en bildvärd.
- **Stöd för högupplöst omritning**: Du kan skicka en 1K originalbild och begära 2K / 4K-utdata med `size`-parametern, modellen kommer att förstora bilden under redigeringsprocessen.
...
Redigeringsgränssnittets begränsningar för `size` är helt i linje med genereringsgränssnittet — `gpt-image-2` kräver att `size` är `auto`, tomt, eller i formatet `WIDTHxHEIGHT`, alla andra former kommer att returnera 400. **Alla storlekar (1K / 2K / 4K / anpassade) debiteras enhetligt per bild, oavsett originalbildens upplösning och begärd `size`.**
...
Det går att se att modulstrukturen, informationspartitionerna och typsnittet har bevarats strikt, endast färgschemat har inverterats till mörkt tema.
...
Originalbild (genererad med `gpt-image-2` av träbokhylla):
...
Det går att se att stil och miljö har bytts ut enligt prompten, men antalet böcker på varje hylla (1 / 3 / 7) har fortfarande strikt bevarats, och en krukväxt har lagts till enligt krav.
...
| `nano-banana-pro`    | 0.35            | Flaggskeppet i serien, bäst på att bevara struktur, text och stil            |
...
> - `image` kan antingen laddas upp som en fil via `multipart/form-data` (lokala filer konverteras automatiskt till base64), eller så kan en bild-URL-sträng skickas direkt som ett formulärfält.
...
För att använda Python-anropet behöver vi först importera två miljövariabler, en `OPENAI_BASE_URL`, som kan ställas in på `https://api.acedata.cloud/openai`, och en variabel för autentisering `OPENAI_API_KEY`, detta värde hämtas från `authorization`, på Mac OS kan miljövariabler ställas in med följande kommando:
...
Så här har vi slutfört redigeringsoperationen av bilden, för närvarande stöder Edits API totalt två modeller, `gpt-image-1` och `gpt-image-2`, där `gpt-image-2` är den rekommenderade modellen, se avsnittet [GPT-Image-2-modell](#gpt-image-2-模型) ovan.
...
Det övergripande flödet är: när klienten initierar en begäran, specificeras ett extra `callback_url`-fält, efter att klienten har initierat API-anropet kommer API:t omedelbart att returnera ett resultat som innehåller ett `task_id`-fält, vilket representerar det aktuella uppdragets ID. När uppdraget är slutfört kommer resultatet av den redigerade bilden att skickas till klientens angivna `callback_url` i POST JSON-format, vilket också inkluderar `task_id`-fältet, så att uppdragets resultat kan kopplas ihop med ID.
...
Därefter kan vi ställa in fältet `callback_url` till ovanstående Webhook-URL och fylla i motsvarande parametrar, som visas i följande kod:
```shell
...
När anropet görs kan man omedelbart få ett resultat, som följer:
...
Efter en stund kan vi observera resultatet av bildredigeringen på Webhook-URL:en, innehållet är som följer:
...
Man kan se att resultatet innehåller ett `task_id`-fält, `data`-fältet innehåller samma bildredigeringsresultat som vid synkron anrop, och genom `task_id`-fältet kan man koppla uppgiften.

## Felhantering

Vid anrop av API:et, om man stöter på fel, kommer API:et att returnera motsvarande felkod och information. Till exempel:
