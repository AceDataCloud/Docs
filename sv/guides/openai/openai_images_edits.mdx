...
OpenAI bildredigeringstjänst kan ta emot valfritt antal bilder och instruktioner, och returnera de modifierade bilderna. För närvarande stöder API:et både `gpt-image-1`, den senaste **`gpt-image-2`**, samt modellerna **`nano-banana` / `nano-banana-2-lite` / `nano-banana-2` / `nano-banana-pro`** som ansluts via samma API.
...
- **Stöder direktöverföring av URL**: Förutom traditionell `multipart/form-data` filuppladdning, stöder `gpt-image-2` även **att ta emot bild-URL:er i JSON-format**, vilket gör det mycket lämpligt för serverbaserade arbetsflöden.
- **Stöder direktöverföring av base64**: I enlighet med officiella riktlinjer kan `image`-fältet också direkt ta emot base64 (`data:image/png;base64,...` eller ren base64), vilket gör att lokala bilder kan redigeras utan att först behöva laddas upp till en bildvärd.
- **Stöder högupplöst omritning**: Du kan skicka en 1K originalbild och begära 2K / 4K-utdata via `size`-parametern, modellen kommer att förstora bilden under redigeringsprocessen.
...
För att använda Python-anropet behöver vi först importera två miljövariabler, en `OPENAI_BASE_URL`, som kan ställas in på `https://api.acedata.cloud/openai`, och en annan variabel för autentisering `OPENAI_API_KEY`, detta värde hämtas från `authorization`, och på Mac OS kan miljövariabler ställas in med följande kommando:
...
Så här har vi slutfört redigeringsoperationen av bilden, för närvarande stöder Edits API totalt två modeller, `gpt-image-1` och `gpt-image-2`, där `gpt-image-2` är den rekommenderade modellen, se avsnittet [GPT-Image-2-modell](#gpt-image-2-modell) ovan.
...
Det övergripande flödet är: när klienten initierar en begäran, specificerar den ett extra fält `callback_url`, efter att klienten har initierat API-anropet kommer API:t omedelbart att returnera ett resultat som innehåller ett fält med `task_id`, vilket representerar det aktuella uppdragets ID. När uppdraget är slutfört kommer resultatet av den redigerade bilden att skickas till klientens angivna `callback_url` i POST JSON-format, vilket också inkluderar `task_id`-fältet, så att uppdragets resultat kan kopplas ihop med ID.
