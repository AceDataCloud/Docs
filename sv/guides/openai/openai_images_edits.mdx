...
OpenAI bildredigeringstjänst kan ta emot bilder och instruktioner och returnera modifierade bilder. GPT Image-serien kan ta emot upp till 16 referensbilder samtidigt. För närvarande stöder API:et både `gpt-image-1`, den senaste **`gpt-image-2`**, samt **`nano-banana` / `nano-banana-2-lite` / `nano-banana-2` / `nano-banana-pro`** serien av modeller via samma API.
...
**En API-token kan användas för att anropa alla tjänster på plattformen, utan att behöva ansöka separat för varje tjänst.** Första ansökan ger en gratis kvot för att prova på; om kvoten tar slut kan du ladda på allmänna medel i [konsolen](https://platform.acedata.cloud/console/coin).
...
- **Stöd för direktöverföring av URL**: Förutom traditionell `multipart/form-data` filuppladdning, stöder `gpt-image-2` även **att ta emot bild-URL:er i JSON-format**, vilket eliminerar behovet av att först ladda ner bilderna lokalt, vilket är mycket lämpligt för serverbaserade arbetsflöden.
...
- **Stöd för högupplöst omritning**: Du kan skicka en 1K originalbild och begära 2K / 4K-utdata via `size`-parametern, modellen kommer att förstora bilden under redigeringsprocessen.

### Linjevarianter (`:official` / `:reverse`)

`gpt-image-2` använder som standard den standardlinjen. Genom att använda suffixet på modellnamnet kan du uttryckligen välja linje:
...
> Till exempel: Om originalbilden är `1024x1024`, och `size` anges som `2048x2048`, kommer modellen att omrita och returnera en 2K-bild enligt redigeringsinstruktionerna; om `size` anges som `3840x2160` kommer en 4K liggande bild att returneras. Debiteringen för de tre är densamma.
> Att utelämna `size`-fältet är helt ekvivalent med att uttryckligen ange `auto`: `gpt-image-2` kommer först att läsa av den tydliga storleksintentionen i prompten, inklusive pixlar, förhållande, liggande/stående riktning, upplösningsnivå (t.ex. 4K / högupplöst) eller namngiven duk. När storleksintentionen identifieras kommer den att använda den planerade specifika storleken; om prompten inte har några storlekskrav eller automatisk bedömning inte är tillgänglig, kommer den att återgå till storleken på den första referensbilden. Den slutliga specifika storleken kommer att normaliseras till multiplar av 16, långsidan och total pixelgräns innan begäran skickas; för absolut kontroll, vänligen ange direkt `WIDTHxHEIGHT`. När genereringen är klar kommer den inte att automatiskt försöka igen på grund av olika utdata pixlar, för att undvika att generera dubbla kostnader.

> **Om `n` Parametern**
> `gpt-image-2` redigeringsgränssnittet stöder `n > 1`: en begäran kan returnera och debiteras för motsvarande antal redigeringsresultat (värdet på `n` är 1–10). Detta gäller även för `gpt-image-1` / `gpt-image-1.5`, samt `nano-banana` / `nano-banana-2-lite` / `nano-banana-2` / `nano-banana-pro` serien. Observera att `response_format=b64_json` endast stöder `n=1`, vid `n>1` vänligen använd standard URL-retur. Om några av bilderna misslyckas med att genereras, kommer endast de framgångsrika delarna att returneras och debiteras.
...
Till exempel, nedan är originalbilden som genererades med `gpt-image-2` som en populärvetenskaplig bild:
...
Vi hoppas kunna ändra den till en "nattläge"-färgpalett. Vi kan anropa så här:
```shell
...
märkt 'Relax & Unwind' med ett band och handskrift-liknande font, 
...
För att använda Python-anropet behöver vi först importera två miljövariabler, en `OPENAI_BASE_URL`, som kan ställas in till `https://api.acedata.cloud/openai`, och en annan variabel för autentisering `OPENAI_API_KEY`, detta värde hämtas från `authorization`, och på Mac OS kan vi ställa in miljövariabler med följande kommando:
...
Så här har vi slutfört bildredigeringsoperationen, för närvarande stöder Edits-gränssnittet totalt två modeller, `gpt-image-1` och `gpt-image-2`, där `gpt-image-2` är den rekommenderade modellen, se avsnittet [GPT-Image-2-modell](#gpt-image-2-模型) ovan.

## Asynkron återkoppling

Eftersom OpenAI Images Edits API kan ta längre tid att redigera bilder, om API:n inte svarar under en längre tid, kommer HTTP-förfrågan att hålla anslutningen öppen, vilket leder till extra systemresursförbrukning, så detta API erbjuder också stöd för asynkron återkoppling.

Det övergripande flödet är: när klienten initierar en begäran, specificerar den ett extra `callback_url`-fält, efter att klienten har initierat API-begäran kommer API:n omedelbart att returnera ett resultat som innehåller ett `task_id`-fält, vilket representerar det aktuella uppdragets ID. När uppdraget är slutfört kommer resultatet av bildredigeringen att skickas till klientens angivna `callback_url` i POST JSON-format, vilket också inkluderar `task_id`-fältet, så att uppdragets resultat kan kopplas ihop med ID. 

Nedan kommer vi att förstå hur man gör detta genom exempel.
Först och främst är Webhook-återkoppling en tjänst som kan ta emot HTTP-förfrågningar, utvecklare bör ersätta med URL:en till sin egen HTTP-server. Här för att underlätta demonstration använder vi en offentlig Webhook-exempelsida https://webhook.site/, öppna denna webbplats för att få en Webhook-URL, som visas i bilden nedan:
...
Vi kan se att resultatet innehåller ett `task_id`-fält, `data`-fältet innehåller samma bildredigeringsresultat som vid synkront anrop, genom `task_id`-fältet kan vi koppla uppgiften.
