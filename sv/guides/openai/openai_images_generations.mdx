...
OpenAI Images Generations API stöder för närvarande flera bildgenereringsmodeller, inklusive den klassiska `dall-e-3`, textrenderingskapaciteten `gpt-image-1`, den senaste generationen **`gpt-image-2`**, samt modellerna i serien **`nano-banana` / `nano-banana-2-lite` / `nano-banana-2` / `nano-banana-pro`** som är anslutna via samma gränssnitt. De kan alla generera högkvalitativa bilder baserat på textbeskrivningar.
...
- **`gpt-image-2:official`**: Officiell kanal, stabil och regelrätt. Stöder verklig 2K / 4K upplösning, **debiteras per bild, enhetspriset är 2 gånger det vanliga `gpt-image-2`**. Om linjen inte är tillgänglig returneras ett fel direkt, ingen automatisk nedgradering.
...
I prompten kan du använda filmtermer (35mm film, grunt skärpedjup, neonskyltar etc.) för att exakt kontrollera atmosfären och känslan.
...
`gpt-image-2` kräver vanligtvis 60–90 sekunder för en enstaka anrop. Om du inte vill upprätthålla en lång anslutning kan du använda den asynkrona callback-mekanismen `callback_url` som beskrivs senare i denna artikel, anropsflödet är helt i linje med andra modeller.
...
Ändra bara `model` till `nano-banana-pro`, övriga parametrar förblir helt desamma:
```python
payload = {
...
Vid första användningen av detta gränssnitt behöver vi fylla i minst tre innehåll, en är `authorization`, välj direkt i rullgardinslistan. En annan parameter är `model`, `model` är den vi väljer att använda OpenAI DALL-E officiella modellkategori, här har vi huvudsakligen 1 typ av modell, detaljer kan ses i de modeller vi tillhandahåller. Den sista parametern är `prompt`, `prompt` är den vi anger för att generera bildens ledtråd.
...
- `created `, ID för att generera denna bild, används för att unikt identifiera denna uppgift.
- `data`, innehåller information om bildgenereringen.

Där `data` innehåller den specifika informationen om den bild som modellen genererat, där `url` är detaljlänken till den genererade bilden, som kan ses i bilden nedan.
...
Det returnerade resultatet är i linje med innehållet i grundläggande användning, och vi kan se att bildkvalitetsparametern för den genererade bilden är `standard`, som visas i bilden nedan:
...
Det returnerade resultatet överensstämmer med grundanvändningen, och vi kan se att bildens storlek är `1024 * 1024`, den genererade bilden visas nedan:
...
Det returnerade resultatet överensstämmer med grundanvändningen, och vi kan se att bildstilparameter är `vivid`, den genererade bilden visas nedan:
