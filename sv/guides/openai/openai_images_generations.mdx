...
OpenAI Images Generations API stöder för närvarande flera bildgenereringsmodeller, inklusive den klassiska `dall-e-3`, textrenderingskapaciteten hos `gpt-image-1`, den senaste generationen **`gpt-image-2`**, samt modellerna i serien **`nano-banana` / `nano-banana-2-lite` / `nano-banana-2` / `nano-banana-pro`** som kan nås via samma gränssnitt. De kan alla generera högkvalitativa bilder baserat på textbeskrivningar.
...
> När du skickar `size: "auto"` (standard) kommer modellen att **läsa prompten för att bestämma formatet**: att skriva "superbred filmisk banner 21:9" ger `3024x1296`, att skriva "stående affisch 9:16" ger `1440x2560`, och om det inte finns några formatindikationer i prompten återgår det till `2048x2048`. Så när du har en komposition i åtanke men inte vill räkna exakta pixlar, är `auto` vanligtvis mer lämpligt än att hårdkoda en storlek. Att utelämna `size`-fältet ger samma effekt som `auto`.
> 1K-nivån garanterar inte strikt pixeljustering - du kan få `1254x1254` när du skickar `1024x1024`, men proportionerna förblir desamma. Om du skickar det igen som `size` kommer debiteringen att förbli densamma.
> 4K-enkelt anrop tar vanligtvis 4–8 minuter, det rekommenderas att använda det med `callback_url` för asynkron återkoppling.

> **Om `n`-parametern**
> `gpt-image-2` stöder `n > 1` (värden 1–10): en begäran kan returnera och debiteras för motsvarande antal bilder. För att få variation mellan flera resultat rekommenderas att samtidigt skicka olika `prompt` eller `seed`. Detta gäller även för `gpt-image-1` / `gpt-image-1.5`, samt serierna `nano-banana` / `nano-banana-2-lite` / `nano-banana-2` / `nano-banana-pro`; `dall-e-3` stöder endast `n = 1`. Observera att `response_format=b64_json` endast stöder `n=1`, använd standard URL-retur för `n>1`. Om några av bilderna misslyckas med att genereras kommer endast de framgångsrika delarna att returneras och debiteras.

Nedan följer några olika verkliga exempel för att intuitivt uppleva `gpt-image-2` kapabiliteter.

### Scenario 1: Filmisk Porträtt

I prompten kan filmtermer (35mm film, grunt skärpedjup, neonsken etc.) användas för att exakt kontrollera atmosfären och texturen.
...
`gpt-image-2` kräver vanligtvis 60–90 sekunder för en enstaka anrop. Om man inte vill upprätthålla en lång anslutning kan man använda den asynkrona callback-mekanismen som beskrivs senare i denna artikel, anropsflödet är helt i linje med andra modeller.

## Nano Banana-serien av modeller

`nano-banana`-serien är en bildgenereringsmodell baserad på Gemini, som har integrerats via samma `/openai/images/generations`-gränssnitt, utan att behöva byta endpoint, man behöver bara ändra `model` till någon av de modeller som anges i tabellen nedan.
...
> - `size` kommer att mappas till intern `aspect_ratio`, icke-listade storlekar kommer att degraderas till `1:1`:
...
Där `data` innehåller den specifika informationen om den bild som modellen har genererat, där `url` är detaljlänken till den genererade bilden, som kan ses i bilden nedan.
...
```
返回resultatet är i överensstämmelse med det grundläggande användningsinnehållet, och vi kan se att bildkvalitetsparametern är `standard`, den genererade bilden visas nedan:

<p><img src="https://cdn.acedata.cloud/j5v15b.png" width="500" class="m-auto"></p>

Genom att utföra samma operation som ovan, behöver vi bara ställa in bildkvalitetsparametern till `hd`, så kan vi få den genererade bilden som visas nedan:
...
Det returnerade resultatet är i överensstämmelse med det grundläggande användningsinnehållet, och vi kan se att bildens storlek är `1024 * 1024`, den genererade bilden visas nedan:

<p><img src="https://cdn.acedata.cloud/o4pvvx.png" width="500" class="m-auto"></p>

Genom att utföra samma operation som ovan, behöver vi bara ställa in bildens storlek till `1792 * 1024`, så kan vi få den genererade bilden som visas nedan:

![](https://cdn.acedata.cloud/4pilae.png)

Vi kan se att bildens storlek är tydligt olika, och vi kan också ställa in fler storlekar, för mer information, se vår officiella dokumentation.
...
Det returnerade resultatet är i överensstämmelse med det grundläggande användningsinnehållet, och vi kan se att bildstilparametern är `vivid`, den genererade bilden visas nedan:

<p><img src="https://cdn.acedata.cloud/e0rpc3.png" width="500" class="m-auto"></p>

Genom att utföra samma operation som ovan, behöver vi bara ställa in bildstilparametern till `natural`, så kan vi få den genererade bilden som visas nedan:
...
Kopiera denna URL, så kan den användas som Webhook, här är exemplet `https://webhook.site/3d32690d-6780-4187-a65c-870061e8c8ab`.
