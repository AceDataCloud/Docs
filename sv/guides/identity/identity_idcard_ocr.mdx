...
För att använda API:et måste du först gå till [ID-kortsigenkänning och informationsverifiering API](https://platform.acedata.cloud/documents/identity-idcard-ocr) motsvarande sida för att ansöka om den tjänst som behövs. När du kommer till sidan, klicka på knappen "Acquire", som visas i bilden nedan:
