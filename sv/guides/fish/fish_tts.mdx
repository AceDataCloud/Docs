...
**En API-token räcker för att anropa alla tjänster på plattformen, det behövs ingen separat ansökan för varje tjänst.** Första ansökan ger en gratis kvot för att prova; om kvoten tar slut kan du ladda på allmän balans i [konsolen](https://platform.acedata.cloud/console/coin).
...
| `prosody`      | object             | Nej  | Prosodiöverskrivning, stöder `speed` (talhastighet, 1.0 är originalhastighet) och `volume` (ljudvolym dB). Till exempel `{"speed":1.2,"volume":0}`.                                                                                              |
...
| `latency`      | string             | Nej  | `normal` eller `balanced`, standard är att detta gränssnitt automatiskt fyller i `normal` (att skicka en tom sträng kommer att avvisas av upstream).                                                                                                                    |
...
`audio_url` pekar på plattformens CDN, kan direkt laddas ner med GET eller spelas upp i `<audio>`. Länken är långvarig, men det rekommenderas fortfarande att spara en kopia i din egen lagring.
