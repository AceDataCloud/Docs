...
- `proxy`: valfritt, egen proxy (Bring Your Own Proxy). När det är inställt kommer upstream att använda den proxy-IP du tillhandahåller för att lösa verifieringen, vilket används för att kontrollera kvaliteten på utgående IP (till exempel för att undvika att offentliga proxy-IP blockeras av målsidan och returnerar `410 Gone`). Formatet är `scheme://[user:pass@]host:port`, där `scheme` stöder `http`/`https`/`socks4`/`socks5`, till exempel `http://user:pass@1.2.3.4:8080`. Om det inte fylls i används plattformens standardproxy.
...
Som standard är API:et synkront och blockerande: en begäran kommer att vänta tills tokenbehandlingen är klar innan den returneras. Om du gör en rotation av flera kodare (multi-solver rotation) och vill "få task_id omedelbart efter att ha skickat uppgiften, först schemalägga andra kodare och sedan återkomma för att hämta resultatet", kan du skicka `async: true` i begärningskroppen.
