...
För att använda API:et måste du först gå till [hCaptcha protokolligenkänning API](https://platform.acedata.cloud/documents/captcha-token-hcaptcha) motsvarande sida för att ansöka om den tjänst som behövs. När du kommer till sidan, klicka på "Acquire"-knappen, som visas i bilden nedan:
