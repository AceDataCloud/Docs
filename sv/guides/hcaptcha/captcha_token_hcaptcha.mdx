...
- Klicka sedan på submit och observera förändringarna i nätverkskonsolen, se bilden nedan:
...
Sedan observerar vi att konsolen visar följande resultat:
...
Under bearbetningen kommer det att returnera `status: processing`:
...
Avgiftsinformation: I asynkront läge debiteras varken skapande av uppgifter eller polling "under bearbetning"; **debiteras endast en gång när token framgångsrikt hämtas** (samma pris som i synkront läge). Därför kommer avbokning av ännu inte slutförda uppgifter under rotationen inte att medföra kostnader. `/captcha/tasks` är gemensam för alla verifieringsgränssnitt (token och recognition-serier, såsom hcaptcha, recaptcha2, recaptcha3, recognition/* etc.) och du kan använda samma `task_id` för att pollera.
