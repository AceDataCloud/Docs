...
- `accept`: vilken typ av format du vill ta emot som svar, här anges det som `application/json`, det vill säga JSON-format.
...
Om du vill generera motsvarande integrationskod kan du direkt kopiera den som genereras, till exempel CURL-koden som följer:
...
Avgiftsbeskrivning: I asynkront läge debiteras varken skapande av uppgifter eller polling "under bearbetning"; **endast när token framgångsrikt hämtas debiteras en gång** (till samma pris som i synkront läge). Därför kommer avbokning av ännu inte slutförda uppgifter under rotationen inte att medföra kostnader. `/captcha/tasks` är gemensam för alla token-relaterade gränssnitt (hcaptcha, recaptcha2, recaptcha3 etc.), och du kan använda samma `task_id` för att pollera.

## Felhantering

När du anropar API:et, om du stöter på fel, kommer API:et att returnera motsvarande felkod och meddelande. Till exempel:
