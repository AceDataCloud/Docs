...
- `proxy`: valfritt, egen proxy (Bring Your Own Proxy). När det är inställt kommer upstream att använda den proxy-IP du tillhandahåller för att lösa verifieringen, för att kontrollera kvaliteten på utgående IP (till exempel för att undvika att offentliga proxy-IP blockeras av målsidan och returnerar `410 Gone`). Formatet är `scheme://[user:pass@]host:port`, där `scheme` stöder `http`/`https`/`socks4`/`socks5`, till exempel `http://user:pass@1.2.3.4:8080`. Om det inte fylls i används plattformens standardproxy.
...
Avgiftsbeskrivning: I asynkront läge debiteras varken skapande av uppgifter eller polling "under bearbetning"; **debiteras endast en gång när token framgångsrikt hämtas** (samma pris som i synkront läge). Därför kommer avbokning av uppgifter som ännu inte är slutförda under rotationen inte att medföra kostnader. `/captcha/tasks` är gemensam för alla verifieringsgränssnitt (token och recognition-serier, såsom hcaptcha, recaptcha2, recaptcha3, recognition/* etc.) och kan pollas med samma `task_id`.
