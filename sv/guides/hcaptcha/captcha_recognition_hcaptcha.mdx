...
Där `queries`-fältet är skärmdumpen av verifieringsbilden ovan, bildstorleken bör inte överstiga 100 kb, och du behöver också ta en skärmdump av området som den röda pilen pekar på, och du behöver också själv komprimera bildstorleken och konvertera den till Base64-kodning, som visas nedan:
...
Vi kan se att vi har fått verifieringsresultatet för hCaptcha verifieringsbilden, vi behöver bara simulera ett klick på det område som anges av `box`-positionskoordinaterna för att klara verifieringen.
...
Som standard är API:et synkront blockerande: en begäran kommer att vänta tills identifieringsresultatet har bearbetats klart innan det returneras. Om du gör en rotation av flera kodare (multi-solver rotation) och vill "skicka uppgiften och omedelbart få task_id, först schemalägga andra kodare och sedan återkomma för att hämta resultatet", kan du ange `async: true` i begäran.
...
Sedan använd `task_id` för att pollera `POST /captcha/tasks` (rekommenderas var 3~5 sekund) för att få resultat:
...
Avgiftsbeskrivning: I asynkront läge debiteras varken skapande av uppgifter eller polling "bearbetning"; **debiteras endast en gång när igenkänningsresultatet har hämtats framgångsrikt** (samma pris som i synkront läge). Därför kommer avbokning av ännu inte slutförda uppgifter under rotationen inte att medföra kostnader. `/captcha/tasks` är gemensam för alla captcha-gränssnitt (token och recognition-serien), använd samma `task_id` för polling.
