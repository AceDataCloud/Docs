...
[`/webextrator/extract`](development_webextrator_extract) — det kör en hel typad extraktionspipeline
...
**En API-token kan användas för att anropa alla tjänster på plattformen, du behöver inte ansöka separat för varje tjänst.** Första ansökan ger en gratis kvot, så att du kan prova gratis; när kvoten är slut kan du ladda på allmänna saldot i [konsolen](https://platform.acedata.cloud/console/coin).
...
> Plattformens kontrakt använder enhetligt **snake_case**. Interna renderingtjänster stöder camelCase, men externa anrop använder alltid
> snake_case.
...
| `data.cacheStoredAt` | number?        | Unix-tidsstämpel i millisekunder när cacheposten först skrevs. |
...
| ---- | ---------------- | --------------------------------- |
| 400  | `bad_request`    | Begäran har inte passerat Zod-validering (saknar `url`, fel typ …).    |
| 401  | `unauthorized`   | Saknad eller ogiltig `Authorization: Bearer …`. |
| 402  | (x402)           | Plattformens saldo är otillräckligt, returnera x402 betalningskrav envelope.     |
| 408  | `timeout`        | Navigering överskrider `timeout`.                   |
| 429  | `queue_busy`     | Synkroniseringskön är överbelastad, vänligen försök igen eller använd `async=true`.       |
| 500  | `internal_error` | Servern har inte hanterat undantag (webbläsaren kraschade etc.), Worker försöker automatiskt igen en gång.  |
...
Återger omedelbart `{ "success": true, "task_id": "...", "trace_id": "...", "started_at": 1777717800.123 }`；
När uppgiften är klar kommer plattformen att POST:a det fullständiga resultatet till din `callback_url`.
...
- **Cache-nyckel ignorerar `bypass_cache` och `cache_ttl_seconds`.** Dessa två är operationsbrytare,
  påverkar inte svaret.
- **`cookies` och `headers` kommer att dela upp cache.** Anpassning av dessa två kommer att göra att första gången samma kombination misslyckas.
...
- **`block_resources` är den snabbaste vägen för att minska latens.** Standard har redan blockerat bilder / typsnitt / media;
