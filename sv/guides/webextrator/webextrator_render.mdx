...
WebExtrator webbrendering API är en tjänst för webbrendering baserad på headless Chromium. Ge en URL,
...
Alla WebExtrator-gränssnitt använder standard Bearer Token-auktorisering:
...
| `timeout`           | number   |  ❌  | `30`                       | Timeout för en navigering (**sekunder**).                                   |
...
> Plattformens kontrakt använder enhetligt **snake_case**. Interna renderingtjänster stöder camelCase, men externa anrop använder alltid snake_case.
...
| `data.finalUrl`      | string         | Slutlig URL efter omdirigering.                        |
| `data.title`         | string         | Renderad `document.title`.                            |
...
| `data.userAgent`     | string         | Faktiskt använd UA.                                   |
| `data.elapsedMs`     | number         | Endast tiden för webbläsarens rendering.              |
| `data.cached`        | boolean?       | `true` om cachen träffades.                           |
| `data.cacheStoredAt` | number?        | Unix-tidsstämpel i millisekunder för första skrivning av cacheposten. |
...
Omedelbart returnera `{ "success": true, "task_id": "...", "trace_id": "...", "started_at": "..." }`；
...
- **Tunga SPA:er överskrider ofta standard 30 sekunder.** Rekommenderar `timeout: 60`, `wait_until: "domcontentloaded"`, `delay: 4`, och kombinera med `wait_for_selector` för att vänta på de verkligt viktiga
  elementen.
- **`block_resources` är den snabbaste vägen att minska latens.** Standard har redan blockerat bilder / typsnitt / media;
  om du extraherar utan att vara beroende av CSS-layout, lägg till `stylesheet` för att bli ännu snabbare.
