...
2. **Typifierad LLM extrahering** — utlöses endast när schema.org inte träffar. Välj schema baserat på sidtyp, Zod strikt validering.
3. **Readability + Markdown fallback** — körs alltid, fyller i toppfält som de två första lagren inte har fyllt.

Upprepade URL-förfrågningar kommer att fångas av Redis-resultatcache, &lt;1 ms svar.
...
| `enable_llm`    | boolean |  ❌  | `false` | Tillåt LLM-extrahering när schema.org inte träffar. På sidor utan JSON-LD som Amazon / HN / Greenhouse, måste detta vara aktiverat för att få typifierade resultat. |

> När sidan har schema.org JSON-LD, är `enable_llm` ineffektivt — den deterministiska mapparen ger direkt resultat,
> kommer aldrig att slösa LLM-anrop. Du får **gratis** typifierade resultat.
...
| `title`         | string   | Readability `<title>` eller renderad `document.title`.                                        |
...
När `enable_llm: true` **och** schema.org inte har primary, extraherar mappen heuristiskt enligt URL
(eller `expected_type` hint) och väljer en av Zod Schema valideringsmodellens utdata:
...
LLM kommer också att göra en "last-resort" återfyllning till toppfältet när det finns "säkra" datakällor som **inte** har fyllt motsvarande fält — LLM är alltid den sista utvägen.
...
| Fält                     | Effekt                         |
| ---------------------- | -------------------------- |
| `bypass_cache: true`   | Hoppa över läsning; detta resultat kommer fortfarande att skrivas tillbaka till cachen, så nästa liknande begäran kan träffa. |
| `cache_ttl_seconds: 0` | Detta svar **cacheas inte**.               |
| `cache_ttl_seconds: N` | Anpassa TTL för denna post (standard 3600 sekunder).    |

Cache-träffar kommer att ha `data.cached: true` och `data.cacheStoredAt: <unix-ms>`.

## Asynkront läge och callback

Ställ in `async: true` för att gå in i asynkront läge (att tillhandahålla `callback_url` kommer också automatiskt att aktivera det). Plattformen returnerar omedelbart (HTTP 200):
