...
När du har röstkaraktärens ID kan du använda [Suno Audios Generation API](https://platform.acedata.cloud/documents/suno-audios) för musikgenerering. Sätt `action` till `generate` och `persona_id` till det röstkaraktärs-ID som returnerades ovan, så kommer den genererade låten att sjungas med den klonade rösten.
