...
> - Det returnerade `name` genereras automatiskt av systemet, vänligen använd det returnerade `persona_id` som referens

> **Vid misslyckande, försök igen**
> Ljudkloning är en resurskrävande uppgift, även om materialet är helt i enlighet med kraven finns det en viss sannolikhet för sporadiska misslyckanden, vanliga svar som `voices_sound_different` (röstverifiering misslyckades) etc. **Dessa misslyckanden har inget att göra med ljudkvaliteten, att försöka igen med samma material lyckas vanligtvis**,
> det rekommenderas att implementera 1-2 automatiska omförsök vid integration. Misslyckade förfrågningar debiteras inte.
...
Som du kan se, är `data` fältet `persona_id` ID:t för den skapade röstkaraktären. `is_public` fältet är alltid `false`, eftersom röstkaraktärer skapade genom att ladda upp ljud är privata. Observera att det returnerade `name` är automatiskt genererat av systemet, använd sedan `persona_id` för att referera till denna röstkaraktär.
...
Som du kan se, sjöngs den genererade låten med den klonade rösten. `persona_id` kan också användas tillsammans med `cover` åtgärden för att sjunga om befintliga låtar med den klonade rösten.
