...
`gpt-image-2` 仅接受下表中的 15 个尺寸，传入其它取值会直接返回 400 错误。**2K / 4K 档位的计费为 1K 档位的 1.5 倍**：

| 比例 | 1K（标准价） | 2K（×1.5） | 4K（×1.5） |
| --- | --- | --- | --- |
...
> 你也可以传 `size: "auto"` 或者**省略 `size` 字段**，此时由模型自行选择默认尺寸，按 1K 档位计费。
>
> 实际计费按 `n × 单图费用` 累加；4K 单次调用通常需要 4–8 分钟，建议配合后文的 `callback_url` 异步回调使用。
