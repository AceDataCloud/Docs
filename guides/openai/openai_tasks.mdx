...
OpenAI Tasks API 用于查询此前以 **回调模式** 提交到 OpenAI 图片接口的任务结果。当您无法等待同步 HTTP 响应、或希望事后通过 `id` 或自定义的 `trace_id` 再次查询任务时，请使用本接口。
...
| `id` | string | 二选一 | 提交图片请求时返回的任务 ID |
| `trace_id` | string | 二选一 | 您在原始请求中传入的自定义 `trace_id` |

`id` 和 `trace_id` 至少传一个。两者同时传入时优先使用 `trace_id`。
...
- `type`：上游接口类型，例如 `images_generations`、`images_edits`、`chat_completions_image` 等。
- `request`：原始请求的完整请求体。
...
| `type` | string | 按上游类型过滤（如 `images_generations`） |
...
Tasks API 主要服务于回调模式下的异步流程，下面是一个完整示例：
...
# 1. 提交图片生成任务，带上 callback_url 与 trace_id
