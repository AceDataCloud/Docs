...
除了上述 Version 1 模式之外，本 API 还支持 Version 2 模式，通过设置 `version` 参数为 `2` 即可启用。Version 2 模式支持更短的视频时长和像素级的分辨率控制。

### Version 2 参数说明

| 参数 | 类型 | 是否必填 | 说明 |
|------|------|----------|------|
| `version` | integer | 是 | 设置为 `2` |
...
在 Version 2 模式下，可以通过 `image_urls` 参数传入参考图片来引导视频生成（仅使用第一张图片）：
