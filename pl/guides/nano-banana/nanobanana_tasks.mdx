...
Dokument ten szczegółowo opisuje instrukcje integracji API Nano Banana Tasks, aby pomóc Ci łatwo zintegrować i w pełni wykorzystać potężne możliwości tego API. Dzięki API Nano Banana Tasks możesz łatwo sprawdzić status wykonania zadań API Nano Banana Images.

## Przykład żądania

API Nano Banana Tasks może być używane do sprawdzania wyników API Nano Banana Images. Aby dowiedzieć się, jak korzystać z API Nano Banana Images, zapoznaj się z dokumentem [API Nano Banana Images](https://platform.acedata.cloud/documents/veo-videos).
...
- `accept`: określa, że oczekiwany jest wynik odpowiedzi w formacie JSON, tutaj wpisujemy `application/json`.
...
```
Zwracany wynik zawiera wiele pól, z których `items` zawiera szczegółowe informacje o zadaniach zbiorczych, a szczegóły każdego zadania są takie same jak w powyższych polach, informacje o polach są następujące.
...
Dzięki temu dokumentowi zrozumiałeś, jak używać API Nano Banana Tasks do zapytań o szczegółowe informacje o pojedynczych lub zbiorczych zadaniach. Mamy nadzieję, że ten dokument pomoże Ci lepiej zintegrować i korzystać z tego API. W razie jakichkolwiek pytań, skontaktuj się z naszym zespołem wsparcia technicznego.
