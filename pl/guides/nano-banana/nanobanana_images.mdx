---
title: "Nano Banana Images API Integracja i Użytkowanie"
description: "Nano Banana Image Generation API guide - Ace Data Cloud"
---

Ten dokument przedstawia integrację i użycie Nano Banana Images API. Interfejs obsługuje dwie funkcjonalności: **generowanie obrazów (generate)** oraz **edycję obrazów (edit)**.
...
- **Model (model)** (opcjonalnie):
  ...
  - `nano-banana:official`, `nano-banana-2-lite:official`, `nano-banana-2:official`, `nano-banana-pro:official`: oficjalne wersje modeli, lepsza jakość obrazu i stabilność, różne koszty
- **Asynchroniczny callback**: opcjonalnie, przez `callback_url` można otrzymać powiadomienia o zakończeniu zadania i wyniki
- **Liczba obrazów**: opcjonalnie, przez `count` można określić 1–4 obrazy, domyślnie 1; w przypadku częściowych niepowodzeń zwracane są tylko udane obrazy i naliczane są opłaty za nie
...
payload = {
    ...
    "prompt": (
        ...
        "pracownia. Scena oświetlona jest miękkim, złotym światłem zachodzącego słońca wpadającym przez "
        "okno, podkreślającym delikatną teksturę gliny. Uchwycone obiektywem portretowym 85 mm, co skutkuje "
        "miękkim, rozmytym tłem (bokeh). Ogólny nastrój jest spokojny i mistrzowski. Orientacja portretowa w pionie."
    ),
    "count": 1
}
...
- `count`: Liczba obrazów do wygenerowania lub edytowania, obsługuje 1–4, domyślnie 1. W przypadku częściowych niepowodzeń `data` zawiera tylko udane obrazy.
- `data[]`: Lista wyników.
  - `prompt`: Użyty do generowania opis (echo).
...
## Edytowanie Obrazów (`action=edit`)

Gdy chcesz edytować istniejący obraz, ustaw `action` na `edit` i przekaż listę linków do obrazów do edytowania w `image_urls` (1 lub więcej), jednocześnie podając opis celu edycji w `prompt`.
...
- Pozostałe pola są takie same jak w odpowiedzi „generuj obraz”.

---

## Asynchroniczny callback (opcjonalnie, zalecane)
...
- **400 `token_mismatched`**: żądanie nieprawidłowe lub błąd parametrów.
...
- **Specjalne dla edycji**: `image_urls` (tablica, co najmniej 1 element)
