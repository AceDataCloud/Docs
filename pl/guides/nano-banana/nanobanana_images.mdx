...
Ten dokument przedstawia integrację i użytkowanie Nano Banana Images API. Interfejs obsługuje dwie funkcjonalności: **generowanie obrazów (generate)** oraz **edycję obrazów (edit)**.
...
- **Model (model)** (opcjonalnie):
  ...
  - `nano-banana:official`, `nano-banana-2-lite:official`, `nano-banana-2:official`, `nano-banana-pro:official`: oficjalne wersje modeli, lepsza jakość obrazu i stabilność, różne opłaty
- **Asynchroniczny callback**: opcjonalnie, przez `callback_url` można otrzymać powiadomienia o zakończeniu zadania i wyniki
- **Liczba obrazów**: opcjonalnie, przez `count` można określić 1–4 obrazy, domyślnie 1; w przypadku częściowych niepowodzeń zwracane są tylko udane obrazy i naliczane są opłaty
...
- Pozostałe pola są takie same jak w odpowiedzi „generowania obrazów”.
...
**Przykład ładunku callback** (struktura pól jest zgodna z synchronizowanym udanym zwrotem):
