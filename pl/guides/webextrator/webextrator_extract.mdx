...
| --------------- | ------- | :-: | ------- | --------------------------------------------------------------------------------------- |
| `expected_type` | enum    |  ❌  | automatyczne określenie    | Wskazanie typu strony: `product` / `article` / `general`. Pomija heurystykę URL / tekstu, przechodzi bezpośrednio do odpowiedniej gałęzi.                        |
...
| `title`         | string   | `<title>` Readability lub renderowany `document.title`.                                    |
...
| `images`        | string[] | Maksymalnie 50 `<img src />`, przekształcone na absolutne URL, zduplikowane, usunięte `data:` URI.                                    |
...
| ----------- | ------------------ | -------------------------------------------------------------------- |
| `schemaOrg` | Zawsze                 | `{ primary, breadcrumbs, all }`. `primary` to najwyższy priorytet typizowanego bytu; gdy nie znaleziony, to `null`. |
...
Wypełnienie uruchamia się tylko wtedy, gdy pewne źródło danych **nie wypełni** odpowiadających pól — LLM zawsze jest ostatnią deską ratunku.
...
`headers` **będą** przechowywane w oddzielnych koszach.
...
Odpowiedzi z trafionym cache będą zawierały `data.cached: true` oraz `data.cacheStoredAt: <unix-ms>`.
...
{
  "kind": "produkt",
  "name": "Apple - Odnowiony w doskonałym stanie - AirPods Pro (2. generacja) - Biały",
  "sku": "10845412",
  "model": "MQD83AM/A",
  "color": "Biały",
  "brand": "Apple",
...
- **Jeśli można przekazać `expected_type`, to przekazuj.** Darmowa wskazówka, pomija heurystyczne oceny, szczególnie przydatna dla stron, których wzór URL nie znajduje się na wbudowanej liście.
...
- **`structured.llmError` jest informacyjne.** Żądanie nadal jest udane, a wyniki heurystyczne nadal są zwracane. Sprawdź `llmError.error`, aby zlokalizować przyczynę (przekroczenie czasu, błąd analizy JSON, błąd walidacji Zod).
- **Linki `non-artykułowe` nie będą sortowane według powiązań.** Tylko według "maksymalnie 100 pozycji + filtruj nieprawidłowe protokoły", staraj się oczyścić.
