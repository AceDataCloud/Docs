...
1. **mapper JSON-LD schema.org** — deterministyczny, zerowy koszt LLM. Pokrywa Wikipedia / BestBuy / AllRecipes / YouTube / większość wiadomości / większość stron produktów.
...
```json
{
  ...
  "data": {
    ...
    "markdown": "# Diffbot\n\nDiffbot to deweloper algorytmów uczenia maszynowego ...",
    "text": "Diffbot to deweloper algorytmów uczenia maszynowego ...",
    "structured": {
...
| `contentType`   | enum     | `product` / `article` / `general`, określane kolejno przez `expected_type` → schema.org primary → heurystyka. |
| `title`         | string   | Readability `<title>` lub renderowany `document.title`.                                        |
...
| `markdown`      | string   | Markdown wyeksportowany przez Turndown.                                                               |
...
| --------------- | ------------------ | -------------------------------------------------------------------- |
| `schemaOrg`     | Zawsze                 | `{ primary, breadcrumbs, all }`. `primary` to najwyżej priorytetowy typowany byt; gdy nie znaleziony, to `null`. |
| `openGraph`     | Zawsze                 | `{ title, description, image, type }`, pochodzi z `<meta property="og:*" />`.   |
| `jsonLd`        | Zawsze                 | Wszystkie oryginalne tablice JSON z bloków `<script type="application/ld+json">`.               |
| `llm`           | Gdy LLM działa i zakończył się sukcesem | `{ kind, data, model, promptCharCount }`, typowane wyniki zweryfikowane przez Zod.              |
| `llmError`      | Gdy LLM działa, ale zakończył się niepowodzeniem | `{ kind, error, model }`, żądanie nie spowoduje awarii, heurystyczne wyniki nadal będą zwracane.                         |
...
- Kontener `@graph` (rekurencyjne rozwinięcie);
...
Gdy `enable_llm: true` **i** schema.org nie ma primary, ekstraktor według URL heurystycznie
(w lub `expected_type` wskazówka) wybiera jeden z poniższych modeli walidacji Zod:

| Rodzaj         | URL heurystyczny                                                                            | Wymagane pola       | Opcjonalne pola                                                                                                                                                                         |
| -------------- | ---------------------------------------------------------------------------------- | ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `artykuł`      | Tekst ≥400 słów i inne nie trafione                                                                    | `headline` | `description, byline, publishedAt, language, topics[], sections[{heading,summary}]`                                                                                          |
| `produkt`      | `amazon.* / ebay.* / aliexpress.* / temu.* / walmart.* / bestbuy.*`                | `name`     | `description, brand, sku, price, currency, availability, rating.{value,count}, bullets[], specifications[{name,value}]`                                                      |
| `dyskusja`     | `news.ycombinator.com / reddit.com / lobste.rs`                                    | `title`    | `author, postedAt, points, commentCount, body, url`                                                                                                                          |
| `przepis`      | `allrecipes / foodnetwork / seriouseats / epicurious / bonappetit / simplyrecipes` | `name`     | `description, author, cookTime, prepTime, totalTime, recipeYield, ingredients[], instructions[], nutrition, rating, keywords[]`                                              |
| `wideo`        | `youtube.com/watch / youtu.be / vimeo.com/<id> / tiktok.com/@/video`               | `name`     | `description, channel, uploadDate, duration, viewCount, likeCount, thumbnailUrl, transcript`                                                                                 |
| `praca`        | `greenhouse.io / lever.co / jobs.* / careers.* / workable.com / bamboohr`          | `title`    | `description, company, location, remote, employmentType, datePosted, validThrough, salaryMin, salaryMax, salaryCurrency, salaryPeriod, responsibilities[], qualifications[]` |

W przypadku sukcesu LLM również wypełni "last-resort" do pól najwyższego poziomu:

- `artykuł` → `description` / `byline` / `publishedAt` / `language`
- `produkt` → `description`
- `dyskusja` → `description` (= body pierwsze 280 znaków) / `byline` (= author) / `publishedAt` (= postedAt)
- `przepis` → `description` / `byline` (= author)
- `wideo` → `description` / `byline` (= channel) / `publishedAt` (= uploadDate)
- `praca` → `description` / `byline` (= company) / `publishedAt` (= datePosted)

Wypełnienie uruchamia się tylko wtedy, gdy źródło danych **nie wypełniło** odpowiadających pól — LLM zawsze jest ostatnią deską ratunku.
...
`headers` **będą** przechowywane w oddzielnych koszykach.
...
Ustawienie `async: true` włącza tryb asynchroniczny (podanie `callback_url` również automatycznie włączy). Platforma natychmiast zwraca (HTTP 200):
...
Po zakończeniu zadania pełny envelope zostanie wysłany `POST` do twojego `callback_url` (jeśli skonfigurowano). Można również
...
### 2. Strona produktu BestBuy (schema.org trafienie)
...
- **Linki na stronach innych niż artykuły nie będą sortowane według powiązań.** Tylko według "maksymalnie 100 pozycji + filtracja nieprawidłowych protokołów" staramy się oczyścić.
