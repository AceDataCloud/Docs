...
Wyniki będą przesyłane za pomocą `POST` do `callback_url` (jeśli skonfigurowano), lub można je aktywnie sprawdzić przez
...
### Wymuszone ominięcie pamięci podręcznej
...
- **Klucz pamięci podręcznej ignoruje `async`.** Synchronizacja i asynchroniczne żądania dla tego samego URL trafiają do tego samego wpisu w pamięci podręcznej, swobodne przełączanie nie spowoduje błędu.
...
- **Reaktywne SPA często przekraczają domyślne 30 sekund.** Zaleca się `timeout: 60`, `wait_until: "domcontentloaded"`, `delay: 4`, a następnie w połączeniu z `wait_for_selector`, aby poczekać na naprawdę istotne elementy.
