...
Możemy zobaczyć, że uzyskaliśmy wynik weryfikacji obrazu kodu weryfikacyjnego hCaptcha, wystarczy, że na podstawie informacji o współrzędnych `box` w wyniku klikniemy w odpowiedni obszar obrazu kodu weryfikacyjnego, aby przejść weryfikację.

Poniżej przedstawimy, jak kliknąć na podstawie informacji o lokalizacji `box`. Najpierw tworzymy prostokątny układ współrzędnych dla przesłanego obrazu kodu weryfikacyjnego, gdzie punkt centralny znajduje się w lewym dolnym rogu obrazu, 360 to odpowiadająca współrzędna pozioma, a 276 to odpowiadająca współrzędna pionowa. Musimy tylko symulować kliknięcie w odpowiednie współrzędne kodu weryfikacyjnego, jak pokazano na poniższym obrazie:
...
Aby aktywnie sprawdzić postęp, możesz użyć `task_id`, aby zapytać `POST /captcha/tasks` (zaleca się co 3-5 sekund). To API nie uruchomi ani nie przyspieszy przetwarzania zadania; nawet jeśli nie zapytasz, stracisz połączenie lub wyjdziesz z klienta, serwer nadal będzie przetwarzać:
...
Opis rozliczeń: w trybie asynchronicznym, tworzenie zadań i odczytywanie statusu „w trakcie przetwarzania” nie są obciążane opłatami; **klient jest obciążany jednorazowo przy pierwszym odczycie udanego wyniku** (zgodnie z istniejącym zachowaniem i ceną trybu synchronizacji). Serwer samodzielnie przyspiesza zadania, ale nie obciąży opłatą, jeśli zadanie zakończy się w tle. Jeśli zadanie nie zakończy się pomyślnie w ciągu 120 sekund, zostanie zakończone jako HTTP 504 `timeout`, bez obciążenia. `/captcha/tasks` nie odpowiada za przyspieszanie zadań.
