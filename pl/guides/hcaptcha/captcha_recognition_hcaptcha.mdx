...
Możemy zobaczyć, że otrzymaliśmy wynik weryfikacji obrazu kodu weryfikacyjnego hCaptcha, wystarczy, że na podstawie informacji o współrzędnych `box` w wyniku klikniemy w odpowiedni obszar obrazu kodu weryfikacyjnego, aby przejść weryfikację.
...
Domyślnie API działa w trybie synchronicznym: jedno żądanie będzie czekać, aż przetwarzanie wyniku rozpoznawania zostanie zakończone, zanim zostanie zwrócone. Jeśli wykonujesz rotację wielu rozwiązań (multi-solver rotation) i chcesz „natychmiast uzyskać task_id po złożeniu zadania, aby najpierw zlecić inne rozwiązania, a później wrócić po wyniki”, możesz przekazać `async: true` w ciele żądania.
...
Opis rozliczeń: w trybie asynchronicznym, tworzenie zadań i cykliczne wywoływanie „w trakcie przetwarzania” nie jest obciążane opłatami; **opłata jest naliczana tylko raz, gdy uda się uzyskać wynik rozpoznawania** (cena jest zgodna z trybem synchronicznym). Dlatego anulowanie zadań, które nie zostały jeszcze zakończone, podczas cyklu nie generuje kosztów. `/captcha/tasks` jest wspólne dla wszystkich interfejsów CAPTCHA (token i seria rozpoznawania), wystarczy cyklicznie wywoływać ten sam `task_id`.
