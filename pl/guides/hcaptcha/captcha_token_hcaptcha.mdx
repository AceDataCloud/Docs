---
title: "hCaptcha API do rozpoznawania protokołu - instrukcja integracji"
description: "hCaptcha verification code recognition service API guide - Ace Data Cloud"
---

W tym artykule przedstawimy instrukcję integracji API do rozpoznawania protokołu hCaptcha, która pozwala użytkownikom na automatyczne dekodowanie i weryfikację bez konieczności rozpoznawania i klikania w obrazy CAPTCHA, wystarczy przesłać klucz witryny.

## Proces aplikacji

Aby skorzystać z API do rozpoznawania protokołu hCaptcha, najpierw przejdź do [konsoli Ace Data Cloud](https://platform.acedata.cloud/console/applications), aby uzyskać swój token API, który należy zachować na przyszłość.
...
> 📘 Pełna dokumentacja: [API do rozpoznawania protokołu hCaptcha →](https://platform.acedata.cloud/documents/captcha-token-hcaptcha)

## Podstawowe użycie

Najpierw zapoznaj się z podstawowym sposobem użycia, czyli wprowadzeniem URL strony, na której znajduje się hCaptcha, aby uzyskać przetworzony wynik. Najpierw musisz przekazać pole `website_url`, nasza przykładowa strona to: `https://accounts.hcaptcha.com/demo`, musimy uzyskać `website_key` na stronie `website_url`, najpierw otwórz tę stronę, naciśnij F12, aby przejść do konsoli, a następnie w zakładce Elementy wykonaj globalne wyszukiwanie `hcaptcha-demo`, uzyskując poniższy wynik:
...
- Analizujemy konstrukcję żądania POST, a na końcu możemy kliknąć prawym przyciskiem myszy na to żądanie, aby skopiować kod CURL, jak pokazano na poniższym obrazku:

<p><img src="https://cdn.acedata.cloud/bif3mj.png" width="500" className="m-auto" /></p>

Z analizy powyższego obrazu wynika, że URL żądania POST to: `https://accounts.hcaptcha.com/demo`, musimy tylko przesłać parametry `g-recaptcha-response`, `h-captcha-response` i `email`, a następnie wystarczy wprowadzić przetworzony token do poniższych danych, wywołując kod CURL do weryfikacji tokena, jak poniżej:
...
Kod Pythona do weryfikacji tokena wygląda następująco:
