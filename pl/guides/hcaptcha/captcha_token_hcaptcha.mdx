...
W tym artykule przedstawimy sposób integracji API do rozpoznawania protokołu hCaptcha, które pozwala użytkownikom na automatyczne dekodowanie w tle bez konieczności rozpoznawania i klikania na obrazy weryfikacyjne hCaptcha, wystarczy przesłać klucz witryny.
...
Najpierw zapoznaj się z podstawowym sposobem użycia, czyli wprowadzeniem URL witryny, która wymaga przetworzenia kodu weryfikacyjnego hCaptcha, aby uzyskać przetworzony wynik. Najpierw musisz przekazać pole `website_url`, nasza przykładowa witryna to: `https://accounts.hcaptcha.com/demo`, musimy uzyskać `website_key` na stronie `website_url`, najpierw otwórz tę stronę, naciśnij F12, aby przejść do konsoli, a następnie w zakładce Elementy wykonaj globalne wyszukiwanie `hcaptcha-demo`, uzyskując poniższy wynik:
...
```
Można zobaczyć, że otrzymaliśmy wynik weryfikacji hCaptcha, który możemy wykorzystać do POST lub symulacji przesyłania do docelowej strony internetowej, jednorazowego użycia, ważność 120s, zaleca się użycie w ciągu 60s. Następnie zostanie podany fragment CURL, który przesyła przetworzony token do docelowej strony, aby przejść przez weryfikację Recaptcha2.
...
- Następnie klikamy submit, obserwując zmiany w konsoli network, jak pokazano na poniższym obrazku:
...
Opis rozliczeń: w trybie asynchronicznym, tworzenie zadań i cykliczne sprawdzanie „w trakcie przetwarzania” nie jest obciążane kosztami; **opłata jest naliczana tylko raz, gdy uda się uzyskać token** (cena jest taka sama jak w trybie synchronicznym). Dlatego w przypadku rotacji anulowanie zadań, które nie zostały jeszcze zakończone, nie generuje kosztów. `/captcha/tasks` jest uniwersalne dla wszystkich interfejsów związanych z tokenami (hcaptcha, recaptcha2, recaptcha3 itp.), można używać tego samego `task_id` do cyklicznego sprawdzania.
