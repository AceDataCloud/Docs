...
Główną funkcją API Flux Tasks jest możliwość sprawdzania statusu wykonania zadań za pomocą ID zadań generowanych przez API Flux Images Generation.
...
API Flux Tasks można wykorzystać do sprawdzania wyników API Flux Images Generation. Aby dowiedzieć się, jak korzystać z API Flux Images Generation, zapoznaj się z dokumentem [Flux Images Generation API](https://platform.acedata.cloud/documents/flux-images-integration).

Posłużymy się przykładem ID zadania zwróconego przez usługę API Flux Images Generation, aby pokazać, jak korzystać z tego API. Załóżmy, że mamy ID zadania: 2db0168c-2373-4367-8d9a-9dc778802e8a, a następnie pokażemy, jak to zrobić, przekazując ID zadania.
