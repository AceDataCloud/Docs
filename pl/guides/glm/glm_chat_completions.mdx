...
GLM（General Language Model）to nowa generacja modeli językowych wprowadzonych przez Zhipu AI (Zhipu AI / Z.ai), która posiada silne zdolności rozumienia i generowania tekstu w języku chińskim i angielskim, osiągając doskonałe wyniki w zadaniach związanych z chińskim kontekstem, generowaniem kodu, wnioskowaniem i wieloetapowymi rozmowami. Nowe modele, takie jak GLM-5.1, GLM-4.7, GLM-4.6, zostały znacznie zoptymalizowane pod kątem długiego kontekstu, wywoływania narzędzi i zadań związanych z kodem, co pozwala na szerokie zastosowanie w inteligentnych systemach odpowiedzi, tworzeniu treści, wsparciu kodu, chatbotach itp.
...
**Jeden token API wystarczy do wywołania wszystkich usług na platformie, nie ma potrzeby składania osobnych wniosków dla każdej usługi.** Przy pierwszym wniosku otrzymasz darmowy limit, aby móc go przetestować; gdy limit się wyczerpie, możesz doładować saldo ogólne w [kontrolerze](https://platform.acedata.cloud/console/coin).

> 📘 Pełna dokumentacja: [GLM Chat Completion API →](https://platform.acedata.cloud/documents/glm-chat-completions)

## 基本使用

Adres żądania GLM Chat Completion API to `https://api.acedata.cloud/glm/chat/completions`, używając autoryzacji Bearer Token, a ciało żądania jest zgodne z protokołem OpenAI Chat Completions.

Podczas pierwszego korzystania z tego interfejsu musimy wypełnić co najmniej trzy elementy:
...
- `model`: wybierz model GLM, który chcesz wywołać, obecnie obsługiwane modele to:
  - `glm-5.1`: najnowszy flagowy model, o najsilniejszych zdolnościach.
  - `glm-4.7`: doskonałe wyniki w wnioskowaniu, wywoływaniu narzędzi i zadaniach związanych z kodem.
  - `glm-4.6`: uniwersalny model konwersacyjny, zrównoważony pod względem efektywności i kosztów.
  - `glm-3-turbo`: klasyczny model konwersacyjny, odpowiedni do ogólnych zadań generowania tekstu.
...
- `top_p`: parametr próbkowania jądra, kontroluje próg skumulowanej prawdopodobieństwa dla kandydatów tokenów.
- `n`: ile kandydatów odpowiedzi ma być generowanych jednocześnie.
...
- `id`: unikalny identyfikator zadania rozmowy.
- `created`: czas utworzenia zadania rozmowy (znacznik czasu Unix, w sekundach).
- `model`: nazwa rzeczywiście wywołanego modelu GLM.
- `choices`: lista odpowiedzi wygenerowanych przez model. `choices[i].message.content` to konkretny tekst odpowiedzi modelu, a `finish_reason` oznacza powód zakończenia (np. `stop`, `length`, `tool_calls`, `content_filter` itp.).
- `usage`: statystyka zużycia tokenów dla tego żądania, zawierająca `prompt_tokens`, `completion_tokens`, `total_tokens`.

## 流式响应

Ten interfejs obsługuje odpowiedzi strumieniowe (Server-Sent Events), co jest bardzo przydatne w integracji z stronami internetowymi, umożliwiając wyświetlanie efektu literka po literce.
...
Model GLM obsługuje wywołania funkcji zgodne z OpenAI, można zadeklarować funkcje do wywołania za pomocą parametru `tools`, model w razie potrzeby zwróci zorganizowane informacje o wywołaniu funkcji w `choices[i].message.tool_calls`.

```python
payload = {
    "model": "glm-4.7",
    "messages": [
        {"role": "user", "content": "Jak dzisiaj wygląda pogoda w Pekinie?"}
    ],
    "tools": [
        {
            "type": "function",
...
## Sugestie dotyczące wyboru modelu
| Model           | Applicable Scenarios                     |
| --------------- | ---------------------------------------- |
| `glm-5.1`      | Najsilniejszy w ogólnych umiejętnościach, zalecany do złożonego wnioskowania, analizy długich dokumentów |
| `glm-4.7`      | Zadania związane z wywołaniem narzędzi, generowaniem kodu, orkiestracją agentów itp. |
| `glm-4.6`      | Zrównoważony wybór do ogólnych rozmów i tworzenia treści |
| `glm-3-turbo`  | Ogólne zadania generowania tekstu, wrażliwe na koszty scenariusze |
