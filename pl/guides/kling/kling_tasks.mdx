...
Główną funkcją API Kling Tasks jest umożliwienie zapytania o status wykonania zadania poprzez wprowadzenie ID zadania wygenerowanego przez API Kling Videos Generation.

Dokument ten szczegółowo opisuje instrukcje integracji API Kling Tasks, aby pomóc Ci łatwo zintegrować i w pełni wykorzystać potężne możliwości tego API. Dzięki API Kling Tasks możesz łatwo realizować zapytania o status wykonania zadań API Kling Videos Generation.

## Proces aplikacji

Aby korzystać z API Kling Videos Generation, najpierw przejdź do [konsoli Ace Data Cloud](https://platform.acedata.cloud/console/applications), aby uzyskać swój token API, który należy zachować na przyszłość.
...
API Kling Tasks może być używane do zapytań o wyniki API Kling Videos Generation. Aby dowiedzieć się, jak korzystać z API Kling Videos Generation, zapoznaj się z dokumentem [Kling Videos Generation API](https://platform.acedata.cloud/documents/kling-videos).

Posłużymy się przykładem ID zadania zwróconego przez usługę API Kling Videos Generation, aby pokazać, jak używać tego API. Załóżmy, że mamy ID zadania: 20068983-0cc9-4c6a-aeb6-9c6a3c668be0, a następnie pokażemy, jak to zrobić, przekazując ID zadania.
...
Dzięki temu dokumentowi zrozumieli Państwo, jak korzystać z API Kling Tasks do zapytań o szczegółowe informacje dotyczące pojedynczych lub zbiorczych zadań wideo. Mamy nadzieję, że ten dokument pomoże Państwu lepiej zintegrować się i korzystać z tego API. W razie jakichkolwiek pytań prosimy o kontakt z naszym zespołem wsparcia technicznego.
