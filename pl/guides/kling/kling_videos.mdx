...
- `action`: działanie w zadaniu generowania wideo, głównie obejmuje trzy działania: generowanie wideo z tekstu (`text2video`), generowanie wideo z obrazu (`image2video`), rozszerzenie wideo (`extend`).
...
- `camera_control`: opcjonalne, parametry obiektu kontrolującego ruch kamery, obsługiwane typy/simple oraz konfiguracje horizontal, vertical, pan, tilt, roll, zoom itp.
...
- `video_id`, ID wideo w zadaniu generowania wideo.
- `video_url`, link do wideo w zadaniu generowania wideo.
- `duration`, długość wideo w zadaniu generowania wideo.
...
| `kling-v1-6`        | std            | ❌                    | ❌                    | ❌                    | Wiele obrazów wideo, `extend` dostępne we wszystkich trybach                         |
| `kling-v1-6`        | pro            | ✅                    | ❌                    | ❌                    |                                              |
...
- `end_image_url` może być używane tylko w `action=image2video` w połączeniu z `start_image_url`. Przesłanie tylko `end_image_url` (bez `start_image_url`) zostanie odrzucone.
...
Model `kling-video-o1` wspiera zdolność „wsparcia wszechstronnego”: na podstawie generowania wideo z tekstu (`action=text2video`), dodatkowo można przesłać obrazy referencyjne, obiekty referencyjne lub wideo referencyjne, co pozwala na **wiele obrazów referencyjnych, referencje do obiektów, wideo referencyjne oraz bezpośrednią edycję istniejącego wideo**.

**Podstawowe ustalenie**: materiały referencyjne muszą być cytowane w `prompt` w formie `&lt;&lt;<image_1>>>`, `&lt;&lt;<element_1>>>`, `&lt;&lt;<video_1>>>` (numeracja zaczyna się od 1) w odniesieniu do odpowiednich pozycji w `image_list` / `element_list` / `video_list`, aby model mógł zastosować te odniesienia. Jeśli przesłane są tylko materiały bez ich cytowania w podpowiedzi, materiały zostaną zignorowane.
### Przykładowe wideo i edycja wideo (`video_list`)

`video_list` służy do przekazywania przykładowego wideo, jest to najczęściej używany scenariusz w tej funkcjonalności, elementy tablicy mają następujące pola:
...
### Przykład główny (`element_list`)

`element_list` służy do odwoływania się do elementów już przesłanych w "bibliotece głównej", elementy tablicy mają pole `element_id` (ID elementu w bibliotece głównej, liczba całkowita). Podczas użycia również należy w `prompt` odwołać się do `&lt;&lt;<element_1>>>`. Liczba referencyjnych elementów musi być zsumowana z liczbą obrazów referencyjnych: gdy istnieje przykładowe wideo, suma obu ≤ 4, gdy nie ma przykładowego wideo ≤ 7.
...
Najpierw, callback Webhook to usługa, która może odbierać żądania HTTP, deweloperzy powinni zastąpić to URL swojego serwera HTTP. W tym celu, dla wygody demonstracji, używamy publicznej strony przykładowej Webhook https://webhook.site/, otwierając tę stronę można uzyskać URL Webhook, jak pokazano na obrazku:
