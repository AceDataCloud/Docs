...
| `prosody`      | object             | Nie   | Przykrycie prozodii, wspiera `speed` (prędkość mowy, 1.0 to oryginalna prędkość) i `volume` (wzmocnienie głośności dB). Na przykład `{"speed":1.2,"volume":0}`.                                                                                              |
...
> Nazewnictwo pól jest całkowicie zgodne z upstream. Poza `callback_url`, pozostałe pola mają takie same znaczenie i wartości jak w [oficjalnej dokumentacji TTS Fish](https://docs.fish.audio/text-to-speech/text-to-speech).
...
`audio_url` wskazuje na CDN tej platformy, można bezpośrednio pobrać za pomocą GET lub odtworzyć w `<audio>`. Ostateczna odpowiedź sukcesu zwróci również górny `cost`, w którym `amount` to rzeczywiście odjęte kredyty; jeśli są zniżki na koncie, `list_amount` oznacza kwotę przed zniżką. Link jest długoterminowy, ale nadal zaleca się przechowywanie kopii w swoim własnym magazynie.
...
Przez nagłówek HTTP `model: s1` przełączamy na model stabilny, dodajemy `mp3_bitrate: 128` w ciele żądania, aby kontrolować bitrate MP3:
```shell
curl -X POST 'https://api.acedata.cloud/fish/tts' \
  -H 'authorization: Bearer {token}' \
  -H 'content-type: application/json' \
...
Dla długich tekstów, jednorazowa synteza może zająć od kilkunastu do kilkudziesięciu sekund, jeśli połączenie zostanie przerwane, należy spróbować ponownie. Po przesłaniu `callback_url` w ciele żądania, interfejs natychmiast zwróci `{task_id, started_at}`, a gdy zadanie zostanie zakończone, pełny wynik zostanie zwrócony w formacie JSON na ten URL, z tym samym `task_id` i ostatecznym `cost` dla tego zadania. Początkowe potwierdzenie zadania nie oznacza zakończenia syntezy, dlatego nie zawiera `cost`.
...
Można również użyć [Fish Tasks API](https://platform.acedata.cloud/documents/fish-tasks) do aktywnego pobierania wyników według `task_id`; końcowy zapis `response.cost` jest zgodny z `cost` w callbacku, szczegóły w tym dokumencie.
