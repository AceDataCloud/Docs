...
> Tworzenie brzmień opisano w [Fish Model Create API](https://platform.acedata.cloud/documents/fish-model), a paginowane listy można pobierać za pomocą [Fish Model Query API](https://platform.acedata.cloud/documents/fish-model-query).

## Proces aplikacji

Aby korzystać z API, należy najpierw złożyć wniosek o dostęp do odpowiedniej usługi na stronie [Fish Model API](https://platform.acedata.cloud/services/fish). Po wejściu na stronę kliknij przycisk „Acquire”.
...
- **Parametr ścieżki**: `{id}` to `_id` brzmienia, które jest zwracane podczas tworzenia w [Fish Model Create API](https://platform.acedata.cloud/documents/fish-model) lub można je uzyskać z paginowanych wyników [Fish Model Query API](https://platform.acedata.cloud/documents/fish-model-query).
...
Zwrócone `_id` może być użyte jako wartość pola `reference_id` w [Fish TTS API](https://platform.acedata.cloud/documents/fish-tts) do syntezy mowy z wykorzystaniem tego klonowanego brzmienia.
