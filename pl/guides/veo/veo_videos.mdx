...
- `action`: działanie tego zadania generowania wideo, które głównie obejmuje trzy działania: generowanie wideo z tekstu (`text2video`), generowanie wideo z obrazu (`image2video`), pobieranie wideo w 1080p (`get1080p`).
...
- `resolution`: wybór rozdzielczości generowanego wideo, model veo31 obsługuje rozdzielczość 4k, inne modele nie obsługują, wszystkie modele obsługują rozdzielczości 1080p oraz gif, jeśli nie przekażesz tej wartości, domyślnie używana jest rozdzielczość 720p, głównie dzieli się na: `1080p`, `gif`, `4k`.
- `prompt`: słowo kluczowe.
- `callback_url`: URL, na który mają być zwracane wyniki.
- `async`: opcjonalne, ustaw na `true`, aby interfejs natychmiast zwrócił `task_id`, nie ma potrzeby podawania `callback_url`, a następnie za pomocą odpowiedniego interfejsu do zapytań o zadania można uzyskać wyniki.

### 📌 Podsumowanie opisu modelu

| **Nazwa modelu**                   | **Obsługiwane tryby**                          | **Zasady dotyczące wejściowych obrazów**                        |
| -------------------------- | --------------------------------- | --------------------------------- |
| **veo3-fast**              | Generowanie wideo z tekstu (bez obrazu)<br />Tryb generowania wideo z obrazu (z obrazem)           | **1 obraz** → tryb pierwszego obrazu<br />**2 obrazy** → tryb pierwszego i ostatniego obrazu |
| **veo31-fast**             | Generowanie wideo z tekstu (bez obrazu)<br />Tryb generowania wideo z obrazu (z obrazem)           | **1 obraz** → tryb pierwszego obrazu<br />**2 obrazy** → tryb pierwszego i ostatniego obrazu |
...
| **veo3**                   | Generowanie wideo z tekstu (bez obrazu)<br />Tryb generowania wideo z obrazu (z obrazem)           | **1 obraz** → tryb pierwszego obrazu<br />**2 obrazy** → tryb pierwszego i ostatniego obrazu |
| **veo31**                  | Generowanie wideo z tekstu (bez obrazu)<br />Tryb generowania wideo z obrazu (z obrazem)           | **1 obraz** → tryb pierwszego obrazu<br />**2 obrazy** → tryb pierwszego i ostatniego obrazu |
