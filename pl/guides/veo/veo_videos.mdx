...
Aby korzystać z Veo Videos Generation API, najpierw przejdź do [konsoli Ace Data Cloud](https://platform.acedata.cloud/console/applications), aby uzyskać swój token API, który należy zachować na przyszłość.
...
Najpierw zapoznaj się z podstawowym sposobem użycia, czyli wprowadzeniem słowa kluczowego `prompt`, działania `action`, tablicy referencyjnych obrazów `image_urls` oraz modelu `model`, aby uzyskać przetworzony wynik. Najpierw musisz przekazać pole `action`, którego wartość to `text2video`, które głównie zawiera trzy działania: generowanie wideo z tekstu (`text2video`), generowanie wideo z obrazu (`image2video`), pobieranie wideo w 1080p (`get1080p`). Następnie musimy również wprowadzić model `model`, który obecnie obejmuje głównie modele `veo31-fast`, `veo3`, `veo31`, `veo2-fast`, `veo3-fast` i `veo31-fast-ingredients`, szczegóły są następujące:
...
- `model`: model do generowania wideo, głównie `veo31-fast`, `veo3`, `veo31`, `veo2-fast`, `veo3-fast` i `veo31-fast-ingredients`.
- `action`: działanie tego zadania generowania wideo, głównie obejmujące trzy działania: generowanie wideo z tekstu (`text2video`), generowanie wideo z obrazu (`image2video`), pobieranie wideo w 1080p (`get1080p`).
- `image_urls`: gdy wybierzesz działanie generowania wideo z obrazu `image2video`, musisz przesłać linki do referencyjnych obrazów. `veo2-fast` obsługuje tylko 1 obraz, `veo31-fast-ingredients` maksymalnie 3 obrazy (fuzja wielu obrazów), pozostałe modele maksymalnie 2 obrazy (tryb pierwszego i ostatniego obrazu).
- `resolution`: wybór rozdzielczości generowanego wideo, model veo31 obsługuje rozdzielczość 4k, inne modele nie obsługują, wszystkie modele obsługują 1080p oraz rozdzielczość gif, jeśli nie podasz tej wartości, domyślnie używana jest rozdzielczość 720p, głównie dzieli się na: `1080p`, `gif`, `4k`.
- `prompt`: słowo kluczowe.
- `callback_url`: URL, na który mają być zwracane wyniki.
- `async`: opcjonalne, ustawione na `true`, interfejs natychmiast zwraca `task_id`, nie ma potrzeby podawania `callback_url`, a następnie za pomocą odpowiedniego interfejsu do zapytań o zadania można cyklicznie uzyskiwać wyniki.

### 📌 Podsumowanie opisu modelu

| **Nazwa modelu**                   | **Obsługiwane tryby**                          | **Zasady dotyczące wejściowych obrazów**                        |
| -------------------------- | --------------------------------- | --------------------------------- |
| **veo2-fast**              | Generowanie wideo z tekstu (bez obrazu)<br />Generowanie wideo z obrazu (z obrazem)           | Obsługuje tylko **1 obraz** → tryb pierwszego obrazu                |
| **veo3-fast**              | Generowanie wideo z tekstu (bez obrazu)<br />Generowanie wideo z obrazu (z obrazem)           | **1 obraz** → tryb pierwszego obrazu<br />**2 obrazy** → tryb pierwszego i ostatniego obrazu |
| **veo31-fast**             | Generowanie wideo z tekstu (bez obrazu)<br />Generowanie wideo z obrazu (z obrazem)           | **1 obraz** → tryb pierwszego obrazu<br />**2 obrazy** → tryb pierwszego i ostatniego obrazu |
...
| **veo3**                   | Generowanie wideo z tekstu (bez obrazu)<br />Generowanie wideo z obrazu (z obrazem)           | **1 obraz** → tryb pierwszego obrazu<br />**2 obrazy** → tryb pierwszego i ostatniego obrazu |
| **veo31**                  | Generowanie wideo z tekstu (bez obrazu)<br />Generowanie wideo z obrazu (z obrazem)           | **1 obraz** → tryb pierwszego obrazu<br />**2 obrazy** → tryb pierwszego i ostatniego obrazu |
...
1. **Ogólna logika**:
   - **Brak wejściowych obrazów** → automatycznie uruchamia tryb generowania wideo z tekstu.
   - **Obrazy wejściowe** → uruchamia tryb generowania wideo z obrazu (konkretne działanie zależy od liczby obrazów).
2. **Typy trybów generowania wideo z obrazu**:
   - **Tryb pierwszego obrazu** (1 obraz): pierwszy obraz jest stały jako wprowadzony obraz.
   - **Tryb pierwszego i ostatniego obrazu** (2 obrazy): pierwszy i ostatni obraz są stałe jako wprowadzone obrazy.
   - **Tryb łączenia wielu obrazów** (1-3 obrazy): tylko `veo31-fast-ingredients` obsługuje, łączy treści wielu obrazów w celu generowania wideo.
3. **Klasyfikacja trybów**:
   - **Tryb Fast**: `veo2-fast`, `veo3-fast`, `veo31-fast`, `veo31-fast-ingredients`.
   - **Tryb Quality**: `veo3`, `veo31` (generuje wyższą jakość).
...
- **Ograniczenia liczby obrazów**:
  - `veo2-fast` obsługuje tylko **1 obraz** wejściowy (tryb pierwszego obrazu).
  - `veo31-fast-ingredients` obsługuje **1-3 obrazy** wejściowe (tryb łączenia wielu obrazów).
  - Inne modele obsługują maksymalnie **2 obrazy** wejściowe (tryb pierwszego i ostatniego obrazu).
...
Kliknij przycisk „Spróbuj”, aby przeprowadzić test, jak pokazano na powyższym obrazku, a oto uzyskany wynik:
```json
{
  "success": true,
...
- `data`, wynik zadania generowania wideo w tym momencie.
  ...
  - `complete_at`, czas zakończenia zadania generowania wideo w tym momencie.
  - `state`, stan zadania generowania wideo w tym momencie.

Możemy zobaczyć, że otrzymaliśmy zadowalające informacje o wideo, wystarczy, że uzyskamy wygenerowane wideo Veo na podstawie adresu linku wideo w `data`.

Jeśli chcesz wygenerować odpowiedni kod do integracji, możesz po prostu skopiować wygenerowane, na przykład kod CURL wygląda następująco:
...
- `model`: model generowania wideo, głównie `veo31-fast`, `veo3`, `veo31`, `veo2-fast`, `veo3-fast` i `veo31-fast-ingredients`.
- `image_urls`: gdy wybierasz działanie generowania wideo `image2video`, musisz przesłać linki do referencyjnych obrazów.
- `prompt`: słowo kluczowe.
...
Klikając uruchom, można zauważyć, że otrzymamy wynik, który wygląda następująco:
...
Można zauważyć, że treść wyniku jest zgodna z powyższą, co również realizuje funkcję generowania wideo z obrazów.
...
> Uwaga, tutaj `video_id` w wideo to ID wideo po wygenerowaniu, jeśli nie wiesz, jak wygenerować wideo, możesz odwołać się do powyższego podstawowego użycia, aby wygenerować wideo.

Następnie musimy wypełnić kolejne wymagane słowa kluczowe, aby dostosować generowane wideo, możemy określić następujące treści:

- `model`: model generowania wideo, głównie `veo31-fast`, `veo3`, `veo31`, `veo2-fast`, `veo3-fast` i `veo31-fast-ingredients`.
- `video_id`: ID referencyjnego wideo, używane do uzyskania wideo 1080p.
...
Można zauważyć, że treść wyniku jest zgodna z powyższą, co również realizuje funkcję uzyskiwania wideo 1080p.
...
Po wypełnieniu formularza automatycznie wygenerowano poniższy kod:
...
Po chwili możemy na stronie `https://webhook.site/aed5cd28-f8aa-4dca-9480-8ec9b42137dc` zaobserwować wynik generowania wideo, jak pokazano na obrazku:
...
Dzięki temu dokumentowi, zrozumieli Państwo, jak korzystać z API Veo Videos Generation, aby generować wideo na podstawie wprowadzonych słów kluczowych oraz obrazu referencyjnego. Mamy nadzieję, że ten dokument pomoże Państwu lepiej zintegrować i korzystać z tego API. W przypadku jakichkolwiek pytań, prosimy o kontakt z naszym zespołem wsparcia technicznego.
