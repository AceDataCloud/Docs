...
W tym artykule przedstawimy sposób integracji Hailuo Videos Generation API, które pozwala na generowanie oficjalnych filmów Hailuo poprzez wprowadzenie niestandardowych parametrów.
...
- `model`: model generujący wideo, głównie dwa modele: `minimax-i2v` i `minimax-t2v`.
- `action`: działanie związane z zadaniem generowania wideo.
- `first_image_url`: link do pierwszego obrazu referencyjnego, który musi być przesłany, gdy wybierzesz model `minimax-i2v`, nie obsługuje kodowania Base64.
- `prompt`: słowo kluczowe.
- `callback_url`: URL, na który mają być zwracane wyniki.
- `async`: opcjonalne, ustawione na `true`, gdy interfejs natychmiast zwraca `task_id`, nie ma potrzeby podawania `callback_url`, a następnie można uzyskać wyniki, korzystając z odpowiedniego interfejsu zapytań o zadania.
...
Możemy zobaczyć, że otrzymaliśmy satysfakcjonujące informacje o wideo, wystarczy, że uzyskamy wygenerowane wideo Hailuo na podstawie adresu linku wideo w `data`.
...
Najpierw, callback Webhook to usługa, która może odbierać żądania HTTP, deweloperzy powinni zastąpić ją URL swojego serwera HTTP. W celu wygodnej demonstracji używamy publicznej strony przykładowej Webhook https://webhook.site/, otwierając tę stronę, można uzyskać URL Webhook, jak pokazano na rysunku:
...
Następnie możemy ustawić pole `callback_url` na powyższy URL Webhook, a także wprowadzić odpowiednie parametry, szczegóły są pokazane na rysunku:
...
Po chwili możemy obserwować wyniki generowania wideo na `https://webhook.site/580b81f5-596e-4321-b03f-606770b0bb83`, jak pokazano na rysunku:
