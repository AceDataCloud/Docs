...
**Jeden token API wystarczy do korzystania ze wszystkich usług platformy, nie ma potrzeby składania osobnych wniosków dla każdej usługi.** Przy pierwszym wniosku otrzymasz darmowy limit, aby móc go przetestować; gdy limit się wyczerpie, możesz doładować saldo ogólne w [konsoli](https://platform.acedata.cloud/console/coin).
...
- `action`: sposób operacji na zadaniu, obsługuje `retrieve` (pojedyncze zapytanie) oraz `retrieve_batch` (zapytanie zbiorcze).
...
Po pomyślnym wykonaniu żądania API zwróci szczegółowe informacje o zadaniu z tego obrazu. Na przykład:
...
- `image_id`, unikalna identyfikacja zadania zapytania o obraz, która jest potrzebna przy kolejnych operacjach na obrazie.
...
- `started_at`, czas rozpoczęcia wykonania zadania, ciąg czasu UTC w formacie ISO-8601.
...
To jest zapytanie o szczegóły zadań dla wielu ID zadań, w przeciwieństwie do powyższego, należy wybrać action jako retrieve_batch.
...
Zwracany wynik zawiera wiele pól, z których `items` zawiera szczegółowe informacje o zadaniach obrazów w partii, a szczegółowe informacje o każdym zadaniu obrazu są takie same jak w powyższym formacie zwracania pojedynczego zadania. Informacje o polach są następujące.

- `items`, wszystkie szczegółowe informacje o zadaniach obrazów w partii. Jest to tablica, a każdy element tablicy ma taki sam format jak wynik zwracany dla pojedynczego zadania.
