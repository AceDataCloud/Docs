...
> Jeśli obecnie korzystasz z `/aichat/conversations`, stary interfejs nadal będzie dostępny, możesz migrować w swoim tempie.
...
Najprostsze użycie jest całkowicie zgodne z v1: przekaż `model` + `question`, aby uzyskać `{answer, id}`.
...
curl -X POST 'https://api.acedata.cloud/aichat2/conversations' \
  ...
  -d '{
    "model": "gpt-5.4",
    "question": "Opisz AceDataCloud w jednym zdaniu."
  }'
```
...
- Google: `gemini-3.1-pro-preview`, `gemini-3.1-pro-preview`, `gemini-3.1-flash-image`, `gemini-3.1-pro-preview`, `gemini-2.5-flash-lite` itp.
- xAI: `grok-4` itp.
...
v2 obsługuje dwa formaty strumieniowe, wybierane według nagłówka `accept`:
...
Głównym punktem wzmocnienia v2 jest to, że model może samodzielnie wywoływać narzędzia do realizacji zadań wieloetapowych, **jest to domyślnie włączone**, nie wymaga dodatkowej konfiguracji ze strony klienta w żądaniu. Typowe scenariusze:
...
Zadania w tle zazwyczaj nie mają nikogo, kto mógłby kliknąć potwierdzenie. Jeśli chcesz, aby niektóre umiejętności lub serwery MCP wykonywały działania takie jak wysyłanie, publikowanie, zapisywanie w trybie bez nadzoru, proszę wyraźnie przekazać listę wstępnych autoryzacji w ciele żądania:
...
Uwaga: wstępna autoryzacja oznacza tylko „w tym żądaniu te funkcje mogą być wykonywane w trybie bez nadzoru bez potwierdzenia przez człowieka”. Konkretna umiejętność nadal musi wspierać `--unattended-confirm` lub odpowiedni mechanizm bezpieczeństwa; w przeciwnym razie będzie kontynuować dry-run i nie wykona operacji zapisu.
...
W ciele żądania `tool_use_id` **musi** być dokładnie zgodne z `tool_id` w momencie wstrzymania; niezgodność spowoduje zwrócenie 400. Gdy w żądaniu znajdują się jednocześnie `tool_results`, `question` / `message` / `references` zostaną zignorowane.
...
2. Jeśli wcześniej przesyłałeś nazwę modelu v1 (np. `gpt-3.5`, `gpt-4-browsing` itp.), przy przejściu na v2 zaleca się aktualizację do współczesnych modeli (np. `gpt-5.4`, `claude-opus-4-8`, `gemini-3.1-pro-preview` itp.).
