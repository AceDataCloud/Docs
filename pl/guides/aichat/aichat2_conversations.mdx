...
- **Zautomatyzowane wywołania narzędzi**: wbudowany zestaw narzędzi do przeszukiwania internetu, zbierania danych z sieci, odczytu plików itp., z możliwością podłączenia serwera MCP (Google Drive, Notion, Slack, GitHub itp.) z autoryzacją użytkownika, model może w jednym żądaniu wielokrotnie wywoływać narzędzia do realizacji złożonych zadań.
...
- **Możliwość przerywania / wznawiania**: model wyśle zdarzenie `ask_user_question` i wstrzyma się, gdy będzie potrzebował dodatkowych informacji od użytkownika, a następne wywołanie może kontynuować, wypełniając odpowiedzią `tool_results`.
- **Nowe akcje CRUD**: na tym samym końcowym punkcie można zrealizować `retrieve` / `retrieve_batch` / `update` / `delete` za pomocą pola `action`, bez potrzeby dodatkowego API do zarządzania sesjami.
...
| `citation`          | Używane do uzupełnienia źródła URL dla odpowiadającego fragmentu tekstu.                                                                            |
...
| `done`              | Zakończenie odpowiedzi strumieniowej, zawiera `usage` (w tym `prompt_tokens` / `completion_tokens` / `total_tokens`) oraz `terminal_reason`. |
...
Dlatego, jeśli chcesz tylko migrować z v1 i nie chcesz zmieniać ciała żądania, wystarczy zmienić ścieżkę na `/aichat2/conversations`, a oryginalne użycie `references` będzie działać jak zwykle.
...
Następnie można użyć `action: retrieve` + `id`, aby sprawdzić wyniki sesji; można również podać `callback_url`, a po zakończeniu zadania platforma wyśle `{ status, answer, usage, error }` na twój adres zwrotny. `callback_url` musi używać `http` / `https` i nie może bezpośrednio zawierać adresu `localhost` lub prywatnego adresu IP.
...
Uwaga: wstępna autoryzacja oznacza tylko „w tym żądaniu zezwala się na te możliwości w trybie bez nadzoru na pominięcie potwierdzenia przez człowieka”. Konkretna umiejętność nadal musi wspierać `--unattended-confirm` lub odpowiedni mechanizm bezpieczeństwa; w przeciwnym razie będzie kontynuować dry-run i nie wykona operacji zapisu.
...
Na froncie przekształć to zdarzenie w kartę, aby użytkownik mógł wybrać odpowiedź, a następnie użyj tego samego `id`, aby zainicjować następne żądanie, wypełniając odpowiedź przez `tool_results`:
...
- `500 chat_error`: błąd LLM w górę lub w tej rundzie `completion_tokens=0` (traktowane jako niezużyte, nie będzie obciążenia).
