...
AI Chat v2 API (`/aichat2/conversations`) to nowa generacja interfejsu do rozmów, będąca kompleksową aktualizacją [AI Chat API](https://platform.acedata.cloud/documents/aichat-conversations). Rozszerza on funkcjonalności v1, które były proste i obsługiwały wieloetapowe rozmowy, o:

- **Wielomodalne wejście użytkownika**: poprzez zorganizowane pole `message` można bezpośrednio przesyłać tekst + obraz + plik, bez potrzeby wcześniejszego używania `references`.
- **Zautomatyzowane wywołania narzędzi**: wbudowany zestaw narzędzi do przeszukiwania sieci, zbierania danych z stron internetowych, odczytu plików itp., z możliwością podłączenia serwera MCP (Google Drive, Notion, Slack, GitHub itp.) z autoryzacją użytkownika, co pozwala modelowi na wielokrotne samodzielne wywoływanie narzędzi w jednej prośbie w celu realizacji złożonych zadań.
...
- **Możliwość przerywania / wznawiania**: model wyśle zdarzenie `ask_user_question` i wstrzyma się, gdy będzie potrzebował dodatkowych informacji od użytkownika, a następne wywołanie z `tool_results` pozwoli na kontynuację.
- **Nowe akcje CRUD**: na tym samym końcowym punkcie można zrealizować `retrieve` / `retrieve_batch` / `update` / `delete` za pomocą pola `action`, bez potrzeby dodatkowego zarządzania sesjami API.
...
Jednocześnie na poziomie ciała żądania **jest w pełni zgodny wstecz z v1**: wystarczy przesłać `model` + `question` (+ opcjonalnie `stateful` / `id` / `references` / `preset`), aby uzyskać równoważną odpowiedź JSON `{answer, id}` jak w v1, więc migracja z `/aichat/conversations` nie wymaga przepisania klienta, wystarczy zmienić ścieżkę na `/aichat2/conversations`.
...
```json
{
  "answer": "AceDataCloud to zintegrowana platforma API, która łączy wiodące modele AI i usługi wielomodalne, umożliwiająca deweloperom korzystanie z GPT, Claude, Gemini, Midjourney, Suno, Veo i innych usług za pomocą jednego klucza.",
...
Każda linia NDJSON to zorganizowane zdarzenie, najczęściej `text_delta`:
...
| `card`              | Strukturalna karta wyprodukowana przez narzędzie (np. obraz, podgląd linku), odpowiednia do bezpośredniego renderowania.                                                                    |
...
- Rozszerzenia URL to `jpg / jpeg / png / gif / bmp / webp / svg / heic / heif`, automatycznie przekształcane na blok `image_url`;
- Inne rozszerzenia przekształcane na blok `file_url`;
...
## Wywołania narzędzi i MCP

Głównym ulepszeniem v2 jest to, że model może samodzielnie wywoływać narzędzia do realizacji zadań wieloetapowych, **jest to domyślnie włączone**, nie wymaga dodatkowej konfiguracji w żądaniu klienta. Typowe scenariusze:
...
Następnie można użyć `action: retrieve` + `id`, aby sprawdzić wyniki sesji; można również podać `callback_url`, a po zakończeniu zadania platforma wyśle `{ status, answer, usage, error }` na twój adres zwrotny. `callback_url` musi używać `http` / `https` i nie może być bezpośrednio wpisany jako `localhost` lub prywatny adres IP.
...
W ciele żądania `tool_use_id` **musi** być dokładnie zgodny z `tool_id` w momencie wstrzymania; niezgodność spowoduje zwrócenie 400. Gdy w żądaniu znajdują się jednocześnie `tool_results`, `question` / `message` / `references` zostaną zignorowane.
...
Odpowiedzi błędów są jednolite:
