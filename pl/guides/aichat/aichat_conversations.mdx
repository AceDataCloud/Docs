...
API do pytań i odpowiedzi AI oferowane przez AceDataCloud zostało zoptymalizowane pod kątem powyższej sytuacji, zapewniając, że efekty pytań i odpowiedzi pozostają niezmienne, a realizacja ciągłych rozmów została opakowana, co oznacza, że podczas integracji nie musimy już martwić się o przekazywanie wiadomości ani o problem z przekroczeniem limitu Tokenów (API automatycznie zajmuje się tym wewnętrznie). Oferuje również funkcje zapytań i modyfikacji rozmów, co znacznie upraszcza całą integrację.

Dokument ten przedstawia opis integracji API do pytań i odpowiedzi AI.

## Proces aplikacji

Aby korzystać z API do pytań i odpowiedzi AI, najpierw przejdź do [konsoli Ace Data Cloud](https://platform.acedata.cloud/console/applications), aby uzyskać swój token API, który należy zachować na przyszłość.
...
> 📘 Pełna dokumentacja: [API do pytań i odpowiedzi AI →](https://platform.acedata.cloud/documents/aichat-conversations)
...
Na przykład zapytanie: „Jak masz na imię?”, następnie możemy wypełnić odpowiednie treści na interfejsie, jak pokazano na obrazku:

![](https://cdn.acedata.cloud/xqxda4.png)

Możemy zobaczyć, że ustawiliśmy nagłówki żądania, w tym:

- `accept`: jakiego formatu odpowiedzi chcemy otrzymać, tutaj wpisujemy `application/json`, czyli format JSON.
...
Klikając przycisk „Spróbuj”, możemy przeprowadzić test, jak pokazano na powyższym obrazku, otrzymujemy następujący wynik:

```json
{
  "answer": "Jestem modelem językowym AI opracowanym przez OpenAI i nie mam osobistego imienia. Możesz nazywać mnie GPT lub po prostu Chatbot. Jak mogę Ci dzisiaj pomóc?"
}
```
...
Jeśli chcesz zintegrować funkcję wieloetapowych rozmów, musisz przekazać dodatkowy parametr `stateful`, którego wartość wynosi `true`, a każdy kolejny wniosek musi zawierać ten parametr. Po przekazaniu parametru `stateful`, API dodatkowo zwróci parametr `id`, który reprezentuje ID bieżącej rozmowy, a następnie wystarczy przekazać ten ID jako parametr, aby łatwo zrealizować wieloetapową rozmowę.
...
```json
{
  "answer": "Jestem modelem językowym AI stworzonym przez OpenAI i nie mam osobistego imienia. Możesz po prostu nazywać mnie OpenAI lub ChatGPT. Jak mogę Ci dzisiaj pomóc?",
  "id": "7cdb293b-2267-4979-a1ec-48d9ad149916"
}
```

Drugie żądanie, przekazując pole `id` zwrócone w pierwszym żądaniu jako parametr, a parametr `stateful` nadal ustawiony na `true`, pytając „Co właśnie ci powiedziałem?”, jak pokazano na obrazku:
...
-d '{
  ...
  "stateful": true,
  "id": "7cdb293b-2267-4979-a1ec-48d9ad149916",
  "question": "Co właśnie ci powiedziałem?"
}'
```
...
```json
{
  "answer": "Zapytałeś mnie, jak mam na imię. Jako model językowy AI nie posiadam osobistej tożsamości, więc nie mam konkretnego imienia. Możesz jednak odnosić się do mnie jako OpenAI lub ChatGPT, nazwy używane dla tego modelu AI. Czy mogę w czymś jeszcze pomóc?",
  "id": "7cdb293b-2267-4979-a1ec-48d9ad149916"
}
```
...
Jeśli chcesz, aby odpowiedź była zwracana strumieniowo, możesz zmienić parametr `accept` w nagłówku żądania na `application/x-ndjson`.
...
Po zmianie `accept` na `application/x-ndjson`, API zwróci odpowiednie dane JSON w kolejnych liniach, a na poziomie kodu musimy dokonać odpowiednich zmian, aby uzyskać wyniki w kolejnych liniach.
...
{"answer": "Cześć! Jak mogę ci pomóc dzisiaj", "delta_answer": " dzisiaj", "id": "7cdb293b-2267-4979-a1ec-48d9ad149916"}
{"answer": "Cześć! Jak mogę ci pomóc dzisiaj?", "delta_answer": "?", "id": "7cdb293b-2267-4979-a1ec-48d9ad149916"}
```
...
Wiemy, że API związane z OpenAI ma odpowiednią koncepcję `system_prompt`, która ustawia domyślne ustawienia dla całego modelu, na przykład jego nazwę itd. To API AI Q&A również udostępnia ten parametr, nazywa się `preset`, dzięki któremu możemy dodać ustawienia do modelu. Użyjmy przykładu, aby to zobaczyć:
...
To API wspiera również modele online, w tym GPT-3.5 i GPT-4, które mogą wspierać, w tle API zachodzi automatyczne przeszukiwanie internetu i podsumowywanie, możemy wybrać model jako `gpt-3.5-browsing`, aby to przetestować, jak pokazano na obrazku:
...
Można zauważyć, że automatycznie połączył się z witryną The Weather Channel i uzyskał stamtąd informacje, a następnie zwrócił aktualne wyniki.
