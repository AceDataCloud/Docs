...
Niniejszy dokument przedstawia sposób integracji API weryfikacji trzech elementów karty bankowej, które można wykorzystać do weryfikacji autentyczności i zgodności informacji o numerze karty bankowej, imieniu i numerze dokumentu tożsamości.

## Proces aplikacji

Aby skorzystać z API weryfikacji trzech elementów karty bankowej, najpierw przejdź do [konsoli Ace Data Cloud](https://platform.acedata.cloud/console/applications), aby uzyskać swój token API, który należy zachować na przyszłość.
...
> 📘 Pełna dokumentacja: [API weryfikacji trzech elementów karty bankowej →](https://platform.acedata.cloud/documents/identity-bankcard-check-3e)

## Podstawowe użycie

Najpierw zapoznaj się z podstawowym sposobem użycia, polegającym na wprowadzeniu informacji o numerze karty bankowej, aby uzyskać przetworzony wynik w postaci obrazu. Należy najpierw przekazać prosty parametr `bank_card`. Następnie możemy wypełnić odpowiednie pola na interfejsie, jak pokazano na rysunku:
...
{
  "result": "0",
  "description": "Weryfikacja zakończona sukcesem"
}
```
...
- `result`, kod wyniku weryfikacji, sytuacja dotycząca opłat jest następująca.
    - Kody wyników płatnych:
        - 0: Weryfikacja zakończona sukcesem
        - -1: Weryfikacja nie powiodła się
