---
title: "Suno MIDI Generation API - instrukcja integracji"
description: "Suno Music Generation API guide - Ace Data Cloud"
---

SUNO pozwala nam na ponowną kreatywność w generowanej muzyce, uzyskując MIDI utworu. Niniejszy dokument wyjaśnia metody integracji związanych z API.

Podstawowym parametrem wejściowym tego API jest `audio_id`, który jest oficjalnym identyfikatorem utworu, zazwyczaj jest to identyfikator utworu z pełnym rozdzieleniem ścieżek (odpowiadający [generacji utworów](https://platform.acedata.cloud/documents/suno-audios) z parametrem `action` ustawionym na `all_stems`). Opcjonalnie wspiera również `callback_url` jako adres do asynchronicznego wywołania zwrotnego.

Wprowadzony przez nas `audio_id` to `c65e6ffd-ead3-4926-9c8c-a42ce202946b`.
...
Można zauważyć, że pola w `data` to informacje MIDI utworu po rozdzieleniu pełnej ścieżki.
