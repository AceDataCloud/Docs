...
Mając ID postaci głosowej, możemy użyć [Suno Audios Generation API](https://platform.acedata.cloud/documents/suno-audios) do generowania muzyki. Ustaw `action` na `generate` oraz `persona_id` na zwrócone wcześniej ID postaci głosowej — wygenerowana piosenka zostanie zaśpiewana klonowanym głosem.
