...
SUNO pozwala nam na tworzenie niestandardowych ról głosowych z dowolnego pliku audio, umożliwiając klonowanie głosu do generowania muzyki. W przeciwieństwie do istniejącego API Persona (używającego `audio_id` wygenerowanego przez Suno), to API akceptuje publicznie dostępny `audio_url`, czyli nagranie głosu, które samodzielnie dostarczasz. Niniejszy dokument wyjaśnia metodę integracji API klonowania dźwięku.
...
> - Należy unikać **szumów tła, akompaniamentu, echa, pogłosu**; pełne piosenki z akompaniamentem zazwyczaj nie przechodzą weryfikacji głosu
...
> Powyższy `https://cdn.acedata.cloud/suno_demo.mp3` to przykład materiału, który można bezpośrednio wykorzystać (MP3, 41 sekund, czysty głos jednej osoby).
...
Mając ID roli głosowej, możemy użyć [Suno Audios Generation API](https://platform.acedata.cloud/documents/suno-audios) do generowania muzyki. Ustaw `action` na `generate`, a `persona_id` na zwrócone wcześniej ID roli głosowej, a generowana piosenka będzie wykonywana przy użyciu sklonowanego głosu.

> **Uwaga:** Klonowanie głosu wspiera jedynie modele `chirp-v4-5` i wyższe (takie jak `chirp-v4-5`, `chirp-v5`, `chirp-v5-5`), nie wspiera `chirp-v4`.
...
Można zauważyć, że wygenerowana piosenka została wykonana przy użyciu sklonowanego głosu. `persona_id` może być również używane w połączeniu z akcją `cover`, aby sklonowany głos mógł wykonywać istniejące piosenki.
