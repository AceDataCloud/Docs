...
Dokument ten szczegółowo opisuje integrację API Luma Tasks, aby pomóc Ci łatwo zintegrować i w pełni wykorzystać potężne możliwości tego API. Dzięki API Luma Tasks możesz łatwo realizować zapytania o status wykonania zadań API Luma Videos Generation.
...
API Luma Tasks może być używane do zapytań o wyniki API Luma Videos Generation. Aby dowiedzieć się, jak korzystać z API Luma Videos Generation, zapoznaj się z dokumentem [Luma Videos Generation API](https://platform.acedata.cloud/documents/luma-videos-integration).

Posłużymy się przykładem ID zadania zwróconego przez usługę API Luma Videos Generation, aby pokazać, jak używać tego API. Załóżmy, że mamy ID zadania: 50fc6182-fa86-4c7d-ac12-2fa27ec2f151, a następnie pokażemy, jak to zrobić, przekazując ID zadania.
...
Po pomyślnym wykonaniu żądania API zwróci szczegółowe informacje o tym zadaniu wideo. Na przykład:
...
- `video_id`, unikalny identyfikator wideo w tym zadaniu, który należy przekazać przy kolejnych operacjach rozszerzających wideo.
