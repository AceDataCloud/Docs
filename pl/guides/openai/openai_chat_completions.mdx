...
Podczas pierwszego korzystania z tego interfejsu musimy wypełnić co najmniej trzy pola: jedno to `authorization`, które można wybrać bezpośrednio z rozwijanej listy. Kolejnym parametrem jest `model`, `model` to kategoria modelu, którą wybieramy do korzystania z oficjalnego modelu OpenAI ChatGPT, mamy tutaj głównie 20 modeli, szczegóły można zobaczyć w dostarczonych modelach. Ostatnim parametrem jest `messages`, `messages` to tablica naszych pytań, jest to tablica, która pozwala na jednoczesne przesyłanie wielu pytań, każde pytanie zawiera `role` i `content`, gdzie `role` oznacza rolę pytającego, oferujemy trzy tożsamości: `user`, `assistant`, `system`. Drugim `content` jest konkretna treść naszego pytania.
...
Po wywołaniu zauważamy, że wyniki zwracane są następujące:
...
Wśród `choices` znajduje się informacja o odpowiedzi ChatGPT, można zauważyć, jak pokazano na obrazku.
...
Możliwości przetwarzania obrazów modelu gpt-4o są głównie realizowane poprzez dodanie pola `type` do oryginalnej zawartości `content`, co pozwala określić, czy przesyłany jest tekst, czy obraz, a tym samym wykorzystać zdolności przetwarzania obrazów modelu gpt-4o. Poniżej przedstawiamy sposób wywołania tej funkcji za pomocą Curl i Pythona.
  ...
  "messages": [
    {
      ...
      "content": [
        {
          "type": "text",
          "text": "Wygeneruj obraz w stylu anime z kapeluszem"
        },
        {
          "type": "image_url",
          "image_url": {
...
Możemy poprosić o wygenerowanie obrazu na podstawie jednego podpowiedzi, a wynik zwróci nam w formie rozmowy, poniżej jako przykład użyjemy `Stwórz obraz zachodu słońca w przyszłym mieście`, poniżej konkretna próbka:
  ...
  "messages": [
    {
      ...
      "content": [
        {
          "type": "text",
          "text": "Stwórz obraz zachodu słońca w przyszłym mieście"
        }
      ]
    }
  ],
...
Można zauważyć, że wynik odpowiada podpowiedzi, jak pokazano poniżej:

<p><img src="https://cdn.acedata.cloud/q502uk.jpg" width="400" class="m-auto"></p>

### Generowanie jednego obrazu z wielu obrazów

Możemy również użyć wielu obrazów referencyjnych do wygenerowania jednego obrazu, na przykład używając jednego obrazu przystojnego mężczyzny i kawy, te dwa obrazy mogą posłużyć do wygenerowania obrazu przystojnego mężczyzny pijącego kawę, poniżej konkretne obrazy referencyjne:
...
Poniżej jako przykład użyjemy `Wygeneruj obraz mężczyzny trzymającego kawę i gotowego do picia`, poniżej konkretna próbka:
```json
{
  "model": "gpt-4o-image",
  "messages": [
    {
      ...
      "content": [
        {
          "type": "text",
          "text": "Wygeneruj chłopaka trzymającego kawę i gotowego do picia."
        },
        {
          "type": "image_url",
          "image_url": {
...
Można zauważyć, że wygenerowany wynik rzeczywiście łączy dwie zdjęcia, oto szczegółowy wynik:
