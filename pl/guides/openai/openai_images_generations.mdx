...
Aby korzystać z OpenAI Images Generations API, najpierw przejdź do [konsoli Ace Data Cloud](https://platform.acedata.cloud/console/applications), aby uzyskać swój token API, który należy zachować na przyszłość.
...
### Warianty linii (`:official` / `:reverse`)

`gpt-image-2` domyślnie korzysta z standardowej linii. Można jawnie wybrać linię, dodając sufiks do nazwy modelu:

- **`gpt-image-2:official`**: oficjalny kanał, stabilny i zgodny. Wspiera prawdziwe rozdzielczości 2K / 4K, **opłata za każdą wygenerowaną grafikę wynosi 2 razy więcej niż w przypadku domyślnego `gpt-image-2`**. Gdy linia jest niedostępna, zwraca bezpośrednio błąd, nie obniżając automatycznie jakości.
...
`gpt-image-2` sprawdza tylko format `size`, wystarczy, że nie jest to `auto` lub pusty ciąg, aby musiał pasować do formatu `WIDTHxHEIGHT` (np. `1024x1024`, `2048x1152`, `800x600`); wszelkie inne formy zwrócą 400. **Wszystkie rozmiary (1K / 2K / 4K / niestandardowe) są obciążane jednolitą opłatą za pojedynczy obraz, nie ma dodatkowych opłat za rozmiar.**

Ograniczenia rozmiaru: niestandardowe rozmiary muszą spełniać warunki, że zarówno szerokość, jak i wysokość są wielokrotnością 16, dłuższy bok ≤ 3840, całkowita liczba pikseli ≤ 8,294,400, przekroczenie tych wartości zwróci błąd 4xx.
...
> Przesyłając `size: "auto"` (domyślnie), model **odczyta sam tekst, aby zdecydować o formacie**: pisząc „super szeroki filmowy format 21:9” otrzymasz `3024x1296`, pisząc „plakat w pionie 9:16” otrzymasz `1440x2560`, a brak wskazówek dotyczących formatu zwróci do `2048x2048`. Dlatego, gdy masz na myśli kompozycję, ale nie chcesz obliczać konkretnych pikseli, `auto` zazwyczaj jest bardziej odpowiednie niż twarde wprowadzenie konkretnego rozmiaru. Pominięcie pola `size` ma taki sam efekt jak `auto`.
> W przypadku rozdzielczości 1K nie ma gwarancji ścisłego dopasowania pikseli — możesz przesłać `1024x1024`, a otrzymasz `1254x1254`, proporcje pozostaną zgodne. Jeśli ponownie przekażesz to jako `size`, opłata pozostanie bez zmian.
...
> **O parametrze `n`**
> `gpt-image-2` wspiera `n > 1` (wartości 1–10): w jednym żądaniu można zwrócić i obciążyć za odpowiadającą liczbę obrazów. Aby uzyskać różnorodność w wynikach, zaleca się jednoczesne przesyłanie różnych `prompt` lub `seed`. To samo dotyczy `gpt-image-1` / `gpt-image-1.5`, oraz serii `nano-banana` / `nano-banana-2-lite` / `nano-banana-2` / `nano-banana-pro`; `dall-e-3` wspiera tylko `n = 1`. Należy pamiętać, że `response_format=b64_json` wspiera tylko `n=1`, w przypadku `n>1` należy użyć domyślnego zwrotu URL. Jeśli niektóre obrazy nie zostaną wygenerowane, zwrócone i obciążone zostaną tylko te, które się powiodły.

Poniżej przedstawiamy kilka różnych rzeczywistych przykładów, aby bezpośrednio poczuć możliwości `gpt-image-2`.

### Scena 1: Portret filmowy

W podpowiedziach można używać terminów filmowych (film 35mm, płytka o małej głębi ostrości, neonowe światło itp.), aby precyzyjnie kontrolować atmosferę i jakość.
...
payload = {
    "model": "gpt-image-2",
    "prompt": "Klatka filmowa portret młodej kobiety stojącej w sklepie całodobowym w nocy, oświetlona miękkimi różowymi i cyjanowymi neonami przez okno. Zdjęcie na filmie 35mm, płytka głębia ostrości, lekka ziarnistość, melancholijny nastrój.",
    "size": "1024x1536"
}
...
```json
{
  ...
  "data": [
    {
      "revised_prompt": "Klatka filmowa portret młodej kobiety stojącej w sklepie całodobowym w nocy, oświetlona miękkimi różowymi i cyjanowymi neonami przez okno. Zdjęcie na filmie 35mm, płytka głębia ostrości, lekka ziarnistość, melancholijny nastrój.",
...
Wynik zwrócony w polu `url` odpowiadający obrazkowi wygląda następująco:
...
payload = {
    "model": "gpt-image-2",
    "prompt": "Drewniana półka na książki składająca się z trzech półek: Na górnej półce powinno być jedna książka. Na drugiej półce powinny być trzy książki. Na dolnej półce powinno być siedem książek. Miękkie ciepłe oświetlenie, fotorealistyczne, przytulna atmosfera biblioteki.",
    "size": "1024x1024"
}
```
...
| `nano-banana-pro`      | 0.35                  | Flagowy model w serii, najlepszy w kompozycji, szczegółach, tekście             |
...
```json
{
  ...
  "data": [
    {
      "url": "https://platform.cdn.acedata.cloud/gpt-image/example-image.png"
    }
  ]
}
```
...
Podczas pierwszego użycia tego interfejsu musimy wypełnić przynajmniej trzy pola, jedno to `authorization`, które można wybrać bezpośrednio z rozwijanej listy. Kolejnym parametrem jest `model`, `model` to kategoria modelu, którą wybieramy do użycia w OpenAI DALL-E, tutaj mamy głównie 1 rodzaj modelu, szczegóły można zobaczyć w dostarczonym modelu. Ostatnim parametrem jest `prompt`, `prompt` to słowo kluczowe, które wprowadzamy, aby wygenerować obraz.
...
```json
{
  ...
  "data": [
    {
      "revised_prompt": "Uroczy obraz przedstawiający młodą wydrzyk morskiego, który urodził się brązowy, z szerokimi, urokliwymi oczami. Leży uroczo na plecach, pływając w spokojnych wodach morza. Jego gęste, aksamitne futro wygląda na mokre i błyszczące, uchwycając istotę jego siedliska. Małe stworzenie ciekawie bawi się muszlą swoimi małymi łapkami, wyglądając absolutnie niewinnie i uroczo w swoim naturalnym środowisku.",
...
Poniżej ustawiamy parametr jakości obrazu na `standard`, szczegółowe ustawienia przedstawione są na poniższym obrazku:
...
```json
{
  ...
  "data": [
    {
      "revised_prompt": "Uroczy mały wydrzyk morski leży zabawnie na plecach w wodzie, a jego futro wygląda na błyszczące i miękkie. Jedna z jego małych łapek sięga ciekawie, a na jego twarzy widać wyraz czystej radości i ciepła, gdy patrzy w niebo. Jego ciało otoczone jest bąbelkami z powodu jego zabawnego kręcenia się w wodzie. Łagodny wiatr bawi się jego futrem, sprawiając, że wygląda jeszcze bardziej uroczo. Scena przedstawia spokój i urok życia morskiego.",
...
Po wywołaniu odkryliśmy, że zwrócone wyniki są następujące:
...
Zwrócone wyniki są zgodne z podstawowym użyciem, można zobaczyć, że rozmiar obrazu wynosi `1024 * 1024`, a generowany obraz pokazano na poniższym zdjęciu:
...
Cały proces polega na tym, że klient podczas wysyłania żądania dodatkowo określa pole `callback_url`, po wysłaniu żądania API natychmiast zwraca wynik, zawierający pole `task_id`, które reprezentuje aktualny identyfikator zadania. Po zakończeniu zadania, wynik generowania obrazu zostanie wysłany do określonego przez klienta `callback_url` w formie POST JSON, który również zawiera pole `task_id`, dzięki czemu wyniki zadania można powiązać za pomocą identyfikatora.

Poniżej przedstawiamy przykład, aby zrozumieć, jak to działa.

Najpierw, Webhook callback to usługa, która może odbierać żądania HTTP, deweloperzy powinni zastąpić ją URL swojego serwera HTTP. W tym celu, dla wygody demonstracji, używamy publicznej strony przykładowej Webhook https://webhook.site/, otwierając tę stronę można uzyskać URL Webhook, jak pokazano na obrazku:
...
Dzięki temu dokumentowi zrozumiałeś, jak łatwo korzystać z funkcji generowania obrazów OpenAI DALL-E za pomocą oficjalnego API OpenAI Images Generations. Mamy nadzieję, że ten dokument pomoże Ci lepiej zintegrować i korzystać z tego API. W razie jakichkolwiek pytań, skontaktuj się z naszym zespołem wsparcia technicznego.
