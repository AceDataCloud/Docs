...
Sposób wywołania jest całkowicie zgodny z innymi modelami, wystarczy ustawić pole `model` na `gpt-image-2`. W zwracanym wyniku `url` to link do obrazu, który jest trwale hostowany na `platform.cdn.acedata.cloud`, można go otworzyć bezpośrednio w przeglądarce lub osadzić na stronie internetowej.
...
- **`gpt-image-2:official`**: oficjalny kanał, stabilny i zgodny. Wspiera prawdziwe rozdzielczości 2K / 4K, **opłata za każdą wygenerowaną grafikę wynosi 2 razy więcej niż w przypadku domyślnego `gpt-image-2`**. Gdy trasa jest niedostępna, zwraca bezpośrednio błąd, nie obniżając automatycznie jakości.
- **`gpt-image-2:reverse`**: całkowicie równoważne z domyślnym `gpt-image-2`, bardziej opłacalne, cena pozostaje bez zmian.

### 支持的 `size` 取值

`gpt-image-2` sprawdza tylko format `size`, o ile nie jest to `auto` lub pusty ciąg, musi pasować do formatu `WIDTHxHEIGHT` (np. `1024x1024`, `2048x1152`, `800x600`); wszelkie inne formy zwrócą błąd 400. **Wszystkie rozmiary (1K / 2K / 4K / niestandardowe) są obciążane jednolitą opłatą za pojedynczy obraz, nie ma dodatkowych opłat za rozmiar.**

Ograniczenia rozmiaru: niestandardowe rozmiary muszą spełniać warunki, że zarówno szerokość, jak i wysokość są wielokrotnościami 16, dłuższy bok ≤ 3840, całkowita liczba pikseli ≤ 8,294,400, przekroczenie tych wartości zwróci błąd 4xx.
...
> Gdy jawnie przekażesz `size: "auto"`, platforma zaplanuje płótno w ciągłej przestrzeni proporcji i oceni według następujących priorytetów: wyraźne piksele lub proporcje w podpowiedziach, standardy nazewnictwa (papier / druk / formaty platformy / reklama / urządzenia / fotografia / film), zwyczaje mediów, a na końcu wnioski dotyczące kompozycji. Dlatego oprócz powszechnych proporcji `1:1`, `4:5`, `9:16`, `21:9`, mogą być również zachowane proporcje `1.91:1`, `1.85:1`, `2.39:1`, ISO papier `1:√2` i inne nieprzewidziane proporcje; ostateczny rozmiar zostanie automatycznie dostosowany do wspieranych przez usługę wielokrotności 16 i budżetu pikselowego. Gdy automatyczne określenie nie jest dostępne, wróci do domyślnego formatu modelu, nie przerywając generacji. Pominięcie pola `size` spowoduje użycie domyślnego formatu modelu; w przypadku ścisłych wymagań dotyczących pikseli zaleca się bezpośrednie przekazanie `WIDTHxHEIGHT`.
> Wyjście w zakresie 1K nie gwarantuje ścisłego dopasowania pikseli — możesz przekazać `1024x1024`, a otrzymać `1254x1254`, proporcje pozostaną zgodne. Jeśli ponownie przekażesz to jako `size`, opłata pozostanie bez zmian.
...
> **O parametrze `n`**
> `gpt-image-2` wspiera `n > 1` (wartości 1–10): jedna prośba może zwrócić i obciążyć za odpowiadającą liczbę obrazów. Aby uzyskać różnorodność w wynikach, zaleca się jednoczesne przekazywanie różnych `prompt` lub `seed`. To samo dotyczy `gpt-image-1` / `gpt-image-1.5`, oraz serii `nano-banana` / `nano-banana-2-lite` / `nano-banana-2` / `nano-banana-pro`; `dall-e-3` wspiera tylko `n = 1`. Uwaga: `response_format=b64_json` wspiera tylko `n=1`, w przypadku `n>1` należy użyć domyślnego zwrotu URL. Jeśli niektóre obrazy nie zostaną wygenerowane, zwrócone i obciążone będą tylko te, które się powiodły.
...
W podpowiedziach można używać terminów filmowych (35mm film, płytka głębia ostrości, neonowe światło itp.), aby precyzyjnie kontrolować atmosferę i jakość.
...
payload = {
    "model": "gpt-image-2",
    "prompt": "Filmowy portret młodej kobiety stojącej w sklepie całodobowym w nocy, oświetlonej miękkimi różowymi i cyjanowymi neonami przez okno. Nakręcone na filmie 35mm, płytka głębia ostrości, lekka ziarnistość, melancholijna atmosfera.",
    "size": "1024x1536"
}
...
```json
{
  ...
  "data": [
    {
      "revised_prompt": "Filmowy portret młodej kobiety stojącej w sklepie całodobowym w nocy, oświetlonej miękkimi różowymi i cyjanowymi neonami przez okno. Nakręcone na filmie 35mm, płytka głębia ostrości, lekka ziarnistość, melancholijna atmosfera.",
...
Seria `nano-banana` to modele generacji obrazów oparte na Gemini, które są dostępne przez ten sam interfejs `/openai/images/generations`, bez potrzeby zmiany punktu końcowego, wystarczy zmienić `model` na dowolny z poniższej tabeli.
...
Podczas pierwszego użycia tego interfejsu musimy wypełnić przynajmniej trzy pola, jedno to `authorization`, które można bezpośrednio wybrać z rozwijanej listy. Kolejnym parametrem jest `model`, `model` to kategoria modelu, którą wybieramy do użycia w OpenAI DALL-E, tutaj mamy głównie 1 model, szczegóły można znaleźć w dostarczonym modelu. Ostatnim parametrem jest `prompt`, `prompt` to słowo kluczowe, które wprowadzamy, aby wygenerować obraz.
...
```json
{
  ...
  "data": [
    {
      "revised_prompt": "Uroczy obraz przedstawiający młodą wydrzyk morskiego, który urodził się brązowy, z szerokimi, urokliwymi oczami. Leży uroczo na plecach, pływając w spokojnych wodach morskich. Jego gęste, aksamitne futro wygląda na mokre i błyszczące, uchwycając istotę jego siedliska. Małe stworzenie ciekawie bawi się muszlą morską swoimi małymi łapkami, wyglądając absolutnie niewinnie i uroczo w swoim naturalnym środowisku.",
...
- `created`, identyfikator generacji obrazu, używany do unikalnego oznaczenia tego zadania.
...
```json
{
  ...
  "data": [
    {
      "revised_prompt": "Uroczy mały wydrzyk morski leży zabawnie na plecach w wodzie, jego futro wygląda na błyszczące i miękkie. Jedna z jego małych łapek sięga ciekawie, a na jego twarzy widać wyraz czystej radości i ciepła, gdy patrzy w niebo. Jego ciało otoczone jest bąbelkami z powodu zabawnego kręcenia się w wodzie. Delikatny wiatr bawi się jego futrem, sprawiając, że wygląda jeszcze bardziej uroczo. Scena przedstawia spokój i urok życia morskiego.",
...
Zwrócone wyniki są zgodne z podstawowym użyciem, można zobaczyć, że rozmiar obrazu wynosi `1024 * 1024`, a generowany obraz przedstawiony jest na poniższym zdjęciu:
...
Można zauważyć, że `vivid` generuje bardziej żywe i realistyczne obrazy niż `natural`.
...
Cały proces polega na tym, że klient inicjuje żądanie, dodatkowo określając pole `callback_url`, po czym API natychmiast zwraca wynik, zawierający informację o polu `task_id`, które reprezentuje aktualne ID zadania. Po zakończeniu zadania, wynik generowania obrazu zostanie wysłany do określonego przez klienta `callback_url` w formie POST JSON, w tym również zawierając pole `task_id`, co pozwala na powiązanie wyniku zadania przez ID.
...
Można zauważyć, że wynik zawiera pole `task_id`, a pole `data` zawiera wyniki generowania obrazu takie same jak w przypadku wywołania synchronicznego, dzięki czemu można powiązać zadanie przez pole `task_id`.
