...
Ograniczenia rozmiaru: niestandardowy rozmiar musi spełniać warunki, że zarówno szerokość, jak i wysokość są wielokrotnością 16, dłuższy bok ≤ 3840, całkowita liczba pikseli ≤ 8,294,400, przekroczenie tych wartości zwróci 4xx.
...
> W przypadku rozmiaru 1K nie ma gwarancji ścisłego dopasowania pikseli — jeśli przekażesz `1024x1024`, możesz otrzymać `1254x1254`, proporcje pozostaną zgodne. Jeśli ponownie przekażesz to jako `size`, opłata pozostaje bez zmian.
...
W podpowiedziach można używać terminów filmowych (film 35mm, płytka głębi, neonowe światło itp.), aby precyzyjnie kontrolować atmosferę i jakość.
...
payload = {
    "model": "gpt-image-2",
    "prompt": "Drewniana półka na książki składająca się z trzech półek: Na górnej półce powinna znajdować się jedna książka. Na drugiej półce powinny być trzy książki. Na dolnej półce powinno być siedem książek. Miękkie ciepłe oświetlenie, fotorealistyczne, przytulna atmosfera biblioteki.",
    "size": "1024x1024"
}
```
...
`gpt-image-2` zazwyczaj wymaga 60-90 sekund na pojedyncze wywołanie. Jeśli nie chcesz utrzymywać długiego połączenia, możesz skorzystać z mechanizmu asynchronicznych wywołań zwrotnych `callback_url`, którego proces wywołania jest całkowicie zgodny z innymi modelami.
...
### Ulepszanie do flagowego modelu `nano-banana-pro`

Wystarczy zmienić `model` na `nano-banana-pro`, a pozostałe parametry pozostają takie same:
```python
payload = {
...
`callback_url` mechanizm asynchronicznego callbacku działa również w przypadku nano-banana, proces wywołania jest całkowicie zgodny z innymi modelami, szczegóły można znaleźć w sekcji [Asynchroniczny callback](#asynchroniczny-callback).
...
W tym przypadku `data` zawiera szczegółowe informacje o wygenerowanym obrazie, a `url` to link do szczegółów wygenerowanego obrazu, co można zobaczyć na rysunku.
...
Ustawiając parametr jakości obrazu na `standard`, szczegóły ustawienia przedstawione są na poniższym rysunku:
...
```json
{
  ...
  "data": [
    {
      "revised_prompt": "Uroczy mały wydrzyk morski leży zabawnie na plecach w wodzie, z futrem wyglądającym na błyszczące i miękkie. Jedna z jego małych łapek sięga ciekawie, a na jego twarzy widać wyraz czystej radości i ciepła, gdy patrzy w niebo. Jego ciało otoczone jest bąbelkami z jego zabawnego kręcenia się w wodzie. Łagodny wiatr bawi się jego futrem, sprawiając, że wygląda jeszcze bardziej uroczo. Scena przedstawia spokój i urok życia morskiego.",
...
Można zauważyć, że obraz `hd` ma bardziej szczegółowe detale i większą spójność niż `standard`.
...
Poniżej ustawiamy parametr stylu obrazu na `vivid`, szczegółowe ustawienia jak na poniższym obrazku:
...
Poniżej ustawiamy parametr formatu linku do obrazu na `url`, szczegółowe ustawienia jak na poniższym obrazku:
...
Zwrócony wynik jest zgodny z podstawowym użyciem, można zobaczyć, że link do obrazu w formacie parametrów `url` generowanego obrazu to [URL obrazu](https://dalleprodsec.blob.core.windows.net/private/images/87792c5f-8b6d-412e-81dd-f1a1baa19bd2/generated_00.png?se=2024-07-23T08%3A39%3A47Z&sig=zzRAn30TqIKHdLVqZPUUuSJdjCYpoJdaGU6BeoA76Jo%3D&ske=2024-07-23T13%3A32%3A13Z&skoid=e52d5ed7-0657-4f62-bc12-7e5dbb260a96&sks=b&skt=2024-07-16T13%3A32%3A13Z&sktid=33e01921-4d64-4f8c-a055-5bdaffd5e33d&skv=2020-10-02), co można bezpośrednio otworzyć, a treść obrazu przedstawia się jak na poniższym obrazku:
...
Można zauważyć, że w wyniku znajduje się pole `task_id`, a pole `data` zawiera wyniki generowania obrazów takie same jak w przypadku wywołania synchronicznego, dzięki czemu można powiązać zadanie za pomocą pola `task_id`.
...
## Wnioski
Dzięki temu dokumentowi zrozumiałeś, jak łatwo korzystać z funkcji generowania obrazów oficjalnego OpenAI DALL-E za pomocą OpenAI Images Generations API. Mamy nadzieję, że ten dokument pomoże Ci lepiej zintegrować i korzystać z tego API. W razie jakichkolwiek pytań, skontaktuj się z naszym zespołem wsparcia technicznego.
