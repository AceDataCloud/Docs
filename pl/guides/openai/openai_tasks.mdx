---
title: "Integracja i użycie OpenAI Tasks API"
...
W trybie callback, **oryginalny interfejs obrazów natychmiast zwraca `task_id` po przyjęciu żądania**. Posiadasz ten `task_id` i możesz go użyć do zapytania w tym interfejsie, gdy zajdzie taka potrzeba, bez konieczności dodatkowego przekazywania własnego `trace_id` (tylko jeśli chcesz powiązać to z własnym identyfikatorem biznesowym).

> Zadanie będzie trwałe tylko wtedy, gdy oryginalne żądanie obrazów zawiera `callback_url`. Żądania wywoływane w sposób synchronizowany (nie w trybie callback) nie będą przechowywane.
...
## Przykład end-to-end: przesyłanie i zapytanie cykliczne
Tasks API głównie służy do asynchronicznych procesów w trybie callback. W trybie callback, interfejs zgłoszeniowy **natychmiast synchronizuje i zwraca `task_id`** (czyli identyfikator zadania), a następnie wystarczy, że bezpośrednio użyjesz tego `task_id` do cyklicznego sprawdzania interfejsu Tasks, nie musisz samodzielnie generować `trace_id`.
...
# 1. Zgłoszenie zadania generowania obrazu (tryb callback: wystarczy dodać callback_url, aby natychmiast zwrócić task_id)
...
print("zgłoszone:", submit)

task_id = submit["task_id"]

# 2. Bezpośrednio użyj task_id z odpowiedzi zgłoszenia do cyklicznego sprawdzania interfejsu Tasks, aż zadanie zostanie zakończone
while True:
    task = requests.post(
        f"{API}/openai/tasks",
        headers=HEADERS,
    ...
    if task and task.get("response"):
        print("zakończone:", task["response"])
        break
    time.sleep(3)
```
...
- Interfejs Tasks sam w sobie **nie generuje opłat**, można go bez obaw cyklicznie sprawdzać. Tylko oryginalne żądania generowania/edycji obrazów będą obciążane opłatami.
- Tylko gdy oryginalne żądanie zawiera `callback_url`, zostanie zapisany rekord zadania; wywołania synchroniczne nie będą generować zadań do zapytania.
