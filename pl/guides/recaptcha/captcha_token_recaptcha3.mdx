...
Możemy zauważyć, że otrzymaliśmy wynik przetwarzania weryfikacji Recaptcha3, który możemy wykorzystać do symulacji przesyłania do docelowej strony internetowej za pomocą POST lub GET, jednorazowego użycia, ważny przez 120s, zaleca się użycie w ciągu 60s. Poniżej przedstawiamy krótki opis sposobu przesyłania wygenerowanego tokena do docelowej strony internetowej:
...
Informacje o opłatach: w trybie asynchronicznym, tworzenie zadań i cykliczne sprawdzanie „w trakcie przetwarzania” nie są obciążane opłatami; **opłata jest naliczana tylko raz, gdy uzyskasz wynik** (cena jest taka sama jak w trybie synchronizowanym). Dlatego anulowanie zadań, które nie zostały jeszcze zakończone w rotacji, nie generuje kosztów. `/captcha/tasks` jest wspólne dla wszystkich interfejsów CAPTCHA (token i seria rozpoznawania), wystarczy użyć tego samego `task_id`, aby cyklicznie sprawdzać.
