...
W tym artykule przedstawimy sposób integracji API rozpoznawania protokołu Recaptcha3, który pozwala użytkownikom na automatyczne dekodowanie w tle bez konieczności rozpoznawania i klikania na obrazy kodów weryfikacyjnych Recaptcha3, wystarczy przesłać klucz witryny.
...
Najpierw zapoznaj się z podstawowym sposobem użycia. W porównaniu do Recaptcha2, musimy dodatkowo przekazać jeden parametr `page_action`, który należy uzyskać z kodu. Prezentowany adres URL do testowania to: `https://recaptcha-demo.appspot.com/recaptcha-v3-request-scores.php`, poniżej przedstawiamy sposób jego uzyskania:

### Szybka metoda:

Otwórz f12, a następnie w zakładce Elementy wyszukaj `.execute(`, w obszarze zaznaczonym na czerwono zobaczysz parametr `action`, a także ciąg znaków, który również będzie potrzebny w dalszej części, jak pokazano na poniższym obrazku.
...
Kliknij przycisk „Try”, aby przeprowadzić test, jak pokazano na powyższym obrazku, a oto uzyskany wynik:
```json
{
...
Możemy zauważyć, że otrzymaliśmy wynik weryfikacji Recaptcha3, który możemy wykorzystać do symulacji przesyłania do docelowej strony internetowej za pomocą POST lub GET, jednorazowego użytku, ważny przez 120s, zaleca się użycie w ciągu 60s. Poniżej przedstawiamy krótki opis sposobu przesyłania wygenerowanego tokena do docelowej strony internetowej:
...
Domyślnie API jest synchronizowane i blokujące: jedno żądanie będzie czekać, aż przetwarzanie tokena zostanie zakończone, zanim zwróci wynik. Jeśli wykonujesz rotację wielu rozwiązań (multi-solver rotation) i chcesz „natychmiast uzyskać task_id po złożeniu zadania, aby najpierw zlecić inne rozwiązania, a później wrócić po wyniki”, możesz przekazać `async: true` w ciele żądania.
...
Aby aktywnie sprawdzić postęp, możesz użyć tego `task_id`, aby zapytać `POST /captcha/tasks` (zaleca się co 3-5 sekund). Ten interfejs nie będzie uruchamiać ani przyspieszać przetwarzania zadań; nawet jeśli nie zapytasz, stracisz połączenie lub wyjdziesz z klienta, serwer nadal będzie przetwarzać:
...
Opis rozliczeń: w trybie asynchronicznym, tworzenie zadań i odczytywanie statusu „w trakcie przetwarzania” nie są obciążane; **klient jest obciążany raz przy pierwszym odczycie pomyślnego wyniku** (zgodnie z istniejącym zachowaniem i ceną w trybie synchronizowanym). Serwer samodzielnie przyspiesza przetwarzanie zadań, ale nie obciąży wcześniej, ponieważ zadanie zostało zakończone w tle. Jeśli zadanie nie zakończy się pomyślnie w ciągu 120 sekund, zakończy się jako HTTP 504 `timeout`, bez obciążenia. `/captcha/tasks` nie odpowiada za przyspieszanie zadań.
