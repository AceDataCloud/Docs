...
- `started_at`, `finished_at`: czas rozpoczęcia przetwarzania i uzyskania wyniku (ISO-8601 UTC).
- `elapsed`: całkowity czas przetwarzania (sekundy).

Możemy zauważyć, że uzyskaliśmy wynik weryfikacji Recaptcha3, który możemy wykorzystać do symulacji przesyłania do docelowej strony za pomocą POST lub GET, jednorazowego użycia, ważny przez 120s, zaleca się użycie w ciągu 60s. Poniżej przedstawiamy krótki opis sposobu przesyłania wygenerowanego tokena do docelowej strony:

Wywołanie kodu w Pythonie odpowiadającego weryfikacji tokena:
