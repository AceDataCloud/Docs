...
Otwórz f12, a następnie w zakładce Elementy wyszukaj `.execute(`. W obszarze zaznaczonym na czerwono możemy zobaczyć parametr `action`, a także ciąg znaków, który pojawia się po execute, co również będzie potrzebne w dalszej części, jak pokazano na poniższym obrazku.

<p><img src="https://cdn.acedata.cloud/imwlto.png" width="500" className="m-auto" /></p>

Następnie musisz wprowadzić URL witryny, na której znajduje się kod weryfikacyjny, aby uzyskać przetworzony wynik. Najpierw musisz prosto przekazać pole `website_url`, a na końcu musisz wprowadzić parametr `website_key`, który można uzyskać powyżej, również jako ciąg znaków po execute. Następnie możemy wypełnić odpowiednie pola na interfejsie, jak pokazano na obrazku:
...
Możemy zobaczyć, że otrzymaliśmy wynik przetwarzania weryfikacji Recaptcha3, który możemy wykorzystać do symulacji przesyłania do docelowej strony internetowej za pomocą POST lub GET, jednorazowego użycia, ważny przez 120s, zaleca się użycie w ciągu 60s. Poniżej przedstawiamy krótki opis sposobu przesyłania wygenerowanego tokena do docelowej strony internetowej:
...
Domyślnie API działa w trybie synchronizacji blokującej: jedno żądanie będzie czekać, aż przetwarzanie tokena zostanie zakończone, zanim zwróci wynik. Jeśli wykonujesz rotację wielu rozwiązań (multi-solver rotation) i chcesz „natychmiast otrzymać task_id po złożeniu zadania, aby najpierw zająć się innymi rozwiązaniami, a później wrócić po wyniki”, możesz przekazać `async: true` w ciele żądania.
...
Opis rozliczeń: w trybie asynchronicznym, tworzenie zadań i cykliczne sprawdzanie „w trakcie przetwarzania” nie jest obciążane kosztami; **opłata jest naliczana tylko raz, gdy uzyskasz wynik** (cena jest taka sama jak w trybie synchronizacji). Dlatego anulowanie zadań, które nie zostały jeszcze zakończone w rotacji, nie generuje kosztów. `/captcha/tasks` jest wspólne dla wszystkich interfejsów CAPTCHA (token i seria rozpoznawania), wystarczy użyć tego samego `task_id` do cyklicznego sprawdzania.
