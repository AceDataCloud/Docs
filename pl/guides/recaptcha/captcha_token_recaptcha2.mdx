...
Wartość odpowiadająca `data-sitekey` to ciąg znaków, który jest wartością `website_key`, poniżej znajdują się konkretne wyniki parametrów:
...
Możemy zobaczyć, że otrzymaliśmy wynik weryfikacji po przetworzeniu Recaptcha2, który możemy wykorzystać do POST lub symulacji przesyłania do docelowej strony, jednorazowego użytku, ważny przez 120s, zaleca się użycie w ciągu 60s. Następnie dostarczymy fragment kodu w Pythonie, aby przesłać przetworzony token do docelowej strony, aby przejść przez Recaptcha2.
...
Z analizy powyższego obrazu wynika, że URL żądania POST to: `https://www.google.com/recaptcha/api2/demo`, musimy jedynie przesłać parametr `g-recaptcha-response`, a następnie przetworzony token wprowadzić do poniższych danych, a konkretne kod CURL do weryfikacji tokena wygląda następująco:
...
Po zakończeniu przetwarzania zwróci `status: ready` oraz token:
...
Opis rozliczeń: w trybie asynchronicznym, tworzenie zadań i cykliczne sprawdzanie „w trakcie przetwarzania” nie jest obciążane kosztami; **opłata jest naliczana tylko raz, gdy uzyskasz wynik** (w tej samej cenie co w trybie synchronicznym). Dlatego w rotacji anulowanie zadań, które nie zostały jeszcze zakończone, nie generuje kosztów. `/captcha/tasks` jest uniwersalne dla wszystkich interfejsów CAPTCHA (token i seria rozpoznawania), wystarczy użyć tego samego `task_id` do cyklicznego sprawdzania.

## Obsługa błędów

Podczas wywoływania API, jeśli napotkasz błąd, API zwróci odpowiedni kod błędu i informację. Na przykład:
