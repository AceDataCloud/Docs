...
W tym artykule przedstawimy sposób integracji API rozpoznawania protokołu Recaptcha2, które pozwala użytkownikom na automatyczne dekodowanie i zakończenie weryfikacji bez konieczności rozpoznawania i klikania na obrazy kodów weryfikacyjnych Recaptcha2, wystarczy przesłać klucz witryny.
...
- `proxy`: opcjonalne, własny proxy (Bring Your Own Proxy). Po ustawieniu, upstream będzie używał dostarczonego przez Ciebie adresu IP proxy do dekodowania kodu weryfikacyjnego, aby kontrolować jakość wyjściowego adresu IP (na przykład, aby uniknąć blokady przez docelową witrynę z powodu publicznego adresu IP proxy, co skutkuje zwróceniem `410 Gone`). Format to `scheme://[user:pass@]host:port`, `scheme` obsługuje `http`/`https`/`socks4`/`socks5`, na przykład `http://user:pass@1.2.3.4:8080`. Jeśli nie wypełnisz, użyje domyślnego proxy platformy.
...
Z powyższej analizy wynika, że URL żądania POST to: `https://www.google.com/recaptcha/api2/demo`, musimy tylko przesłać parametr `g-recaptcha-response`, a następnie wystarczy przekazać przetworzony token do poniższych danych, aby wywołać token do weryfikacji, a konkretny kod CURL wygląda następująco:
...
Kod Python do wywołania weryfikacji tokena wygląda następująco:
...
Domyślnie API jest synchroniczne i blokujące: jedno żądanie będzie czekać, aż przetwarzanie tokena zostanie zakończone, zanim zwróci wynik. Jeśli robisz rotację wielu rozwiązań (multi-solver rotation) i chcesz „natychmiast uzyskać task_id po złożeniu zadania, a następnie przejść do innych rozwiązań, a później wrócić po wynik”, możesz przekazać `async: true` w ciele żądania.

Po przekazaniu `async: true`, interfejs natychmiast zwróci `task_id`, a nie będzie blokować oczekiwania:
...
Opis rozliczeń: w trybie asynchronicznym, tworzenie zadań i cykliczne sprawdzanie „w trakcie przetwarzania” nie jest płatne; **opłata jest naliczana tylko raz, gdy uzyskasz wynik** (w tej samej cenie co w trybie synchronicznym). Dlatego w przypadku rotacji anulowanie zadań, które nie zostały jeszcze zakończone, nie generuje kosztów. `/captcha/tasks` jest uniwersalne dla wszystkich interfejsów CAPTCHA (token i seria rozpoznawania), wystarczy użyć tego samego `task_id`, aby cyklicznie sprawdzać.
