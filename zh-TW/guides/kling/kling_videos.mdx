...
- `authorization`：呼叫 API 的密鑰，申請之後可以直接下拉選擇。
...
- `video_list`：參考影片，透過URL方式獲取，只適用模型`kling-video-o1`,該參數的具體的使用方式參考[官網文件](https://docs.qingque.cn/d/home/eZQAyImcbaS0fz-8ANjXvU5ed?identityId=1oEG9JKKMFv#section=h.5t7wme23nn6z)。
- `prompt`：提示詞。
...
如果想對已經生成的Kling影片進行繼續生成的話，可以將參數 `action` 設置為 `extend` ，並且輸入需要繼續生成影片的 ID，影片 ID 的取得是根據基本使用來取得，如下圖所示：
...
- `duration`：此次影片生成任務的影片時長，主要包含5秒和10秒。
- `start_image_url`：當選擇圖生影片行為 `image2video` 就必須需要上傳的首幀參考圖片連結。
- `prompt`：提示詞。
...
由於 Kling Videos Generation API生成的時間相對較長，大約需要 1-2 分鐘，如果 API 長時間無響應，HTTP 請求會一直保持連線，導致額外的系統資源消耗，所以本 API 也提供了非同步回調的支援。
...
- `400 token_mismatched`：Bad request，可能是缺少或無效的參數。
- `400 api_not_implemented`：Bad request，可能是缺少或無效的參數。
