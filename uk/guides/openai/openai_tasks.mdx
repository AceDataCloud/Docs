...
OpenAI Tasks API використовується для запиту результатів завдань, які були подані до OpenAI зображення через **режим зворотного виклику**. Коли ви не можете дочекатися синхронної HTTP-відповіді або хочете повторно запитати завдання пізніше, використовуйте цей інтерфейс.

У режимі зворотного виклику, **оригінальний інтерфейс зображень відразу повертає `task_id`** після обробки запиту, ви просто тримаєте цей `task_id` і, коли потрібно, використовуєте його для запиту в цьому інтерфейсі, без необхідності додатково передавати власний `trace_id` (тільки якщо ви хочете пов'язати його з вашим бізнес-ідентифікатором).
...
OpenAI Tasks API спільно використовує авторизацію з існуючими сервісами OpenAI. Якщо ви вже подали заявку на OpenAI Images Generations, ви можете безпосередньо використовувати той же токен для виклику цього інтерфейсу, без додаткової заявки.
...
| Операція          | Опис                                                         |
| ---------------- | ---------------------------------------------------------- |
| `retrieve`       | Запит на одне завдання за `id` або `trace_id`                     |
| `retrieve_batch` | Пакетний запит на завдання за `ids` / `trace_ids` / `application_id` / `user_id` |
...
## Запит на одне завдання (`retrieve`)
...
- `trace_id`: Користувацький ідентифікатор відстеження, переданий в оригінальному запиті, для зручності зв'язку з бізнесом клієнта.
...
- `created_at` / `finished_at`: Unix-мітки часу (в секундах). `started_at` - це рядок у форматі ISO 8601.
- `elapsed`: Час виконання (в секундах, з плаваючою комою).
...
| `application_id` | string   | Запит на всі завдання за додатком                                                |
| `user_id`        | string   | Запит на всі завдання за кінцевим користувачем                                              |
...
Достатньо передати одне з `ids` / `trace_ids` / `application_id` / `user_id` або часові вікна `created_at_*`.
...
## Приклад енд-то-енд: подача та опитування

API завдань в основному служить для асинхронних процесів у режимі зворотного виклику. У режимі зворотного виклику, інтерфейс подачі **відразу синхронно повертає `task_id`** (тобто ID завдання), після чого вам просто потрібно безпосередньо взяти цей `task_id` для опитування інтерфейсу завдань, без необхідності самостійно генерувати `trace_id`.
...
# 2. Прямо використовуйте task_id з відповіді подачі для опитування інтерфейсу завдань, поки завдання не буде завершено
while True:
    task = requests.post(
        f"{API}/openai/tasks",
        headers=HEADERS,
