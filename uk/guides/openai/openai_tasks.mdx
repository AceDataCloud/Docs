...
> Завдання буде збережено лише тоді, коли оригінальний запит на зображення містить `callback_url`. Запити, виконані синхронно (не в режимі зворотного виклику), не зберігаються.
...
Необхідно передати хоча б одне з `id` або `trace_id`. У загальному випадку достатньо використовувати `id`, повернуте в відповіді на запит, `trace_id` передавайте лише якщо хочете пов'язати його з власним бізнес-ідентифікатором.
...
- `type`: Тип верхнього інтерфейсу. Завдання, записані в серії `gpt-image` (наприклад, `gpt-image-2`), мають тип `images`; старі канали OpenAI (наприклад, `gpt-image-1`, nano-banana) використовують `images_generations` / `images_edits`, деякі чати мають тип `chat_completions_image`.
- `request`: Повне тіло оригінального запиту.
- `response`: Остаточне тіло відповіді, повернуте верхнім інтерфейсом після завершення зворотного виклику.
...
| `application_id` | string   | Запит всіх завдань за додатком                                                |
| `user_id`        | string   | Запит всіх завдань за кінцевим користувачем                                              |
| `type`           | string   | Фільтрація за типом верхнього інтерфейсу (можливі значення: `images`, `images_generations`, `images_edits`) |
...
```
## Приклад з кінця в кінець: подання та опитування

API завдань в основному служить для асинхронних процесів у режимі зворотного виклику. У режимі зворотного виклику, інтерфейс подання **відразу синхронно повертає `task_id`** (тобто ID завдання), після чого вам просто потрібно безпосередньо використовувати цей `task_id` для опитування інтерфейсу Tasks, без необхідності самостійно генерувати `trace_id`.
...
# 2. Прямо використовуйте task_id з відповіді на подання для опитування інтерфейсу Tasks, поки завдання не буде завершено
while True:
    task = requests.post(
        f"{API}/openai/tasks",
        headers=HEADERS,
