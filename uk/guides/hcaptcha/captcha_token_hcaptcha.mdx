...
- `rqdata`: необов'язковий. Виклик hCaptcha Enterprise може надавати `data-rqdata` на сторінці, у разі таких викликів заповніть його початкове значення; звичайна hCaptcha не потребує заповнення.
...
Повернутий результат містить кілька полів, описані наступним чином:

- `token` — результат перевірки завдання hCaptcha після обробки.
- `started_at`, `finished_at` — час початку обробки запиту та час отримання результату у форматі Unix timestamp (секунди, з плаваючою точкою).
...
- Потім натискаємо submit, спостерігаємо за змінами в консолі network, конкретно, як на зображенні нижче:
...
З аналізу зображення видно, що URL цього POST запиту: `https://accounts.hcaptcha.com/demo`, нам потрібно лише подати параметри `g-recaptcha-response`, `h-captcha-response` та `email`, а потім ми просто передаємо оброблений токен у нижче наведених даних, виклик CURL коду для перевірки токена виглядає так:
...
Код інтеграції Python виглядає так:
