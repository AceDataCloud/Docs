...
З аналізу малюнка видно, що URL цього POST запиту: `https://accounts.hcaptcha.com/demo`, нам потрібно лише подати параметри `g-recaptcha-response`, `h-captcha-response` та `email`, а потім ми лише передаємо оброблений токен у нижче наведених даних, виклик коду CURL для перевірки токена виглядає так:
