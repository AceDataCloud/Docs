...
У цьому документі буде представлено опис інтеграції hCaptcha протоколу розпізнавання API, який дозволяє користувачам не розпізнавати та не натискати на зображення hCaptcha, а лише подати Website Key для автоматичного декодування на бекенді та завершення верифікації.
...
Опис тарифікації: в асинхронному режимі створення завдання та опитування «в обробці» не підлягає тарифікації; **тільки при успішному отриманні токена стягується плата один раз** (з ціною, аналогічною синхронному режиму). Тому скасування незавершених завдань під час ротації не призведе до витрат. `/captcha/tasks` є загальним для всіх інтерфейсів токенів (hcaptcha, recaptcha2, recaptcha3 тощо), можна використовувати один і той же `task_id` для опитування.
