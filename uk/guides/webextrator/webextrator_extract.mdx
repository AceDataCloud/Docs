...
Якщо ви ще не увійшли або не зареєструвалися, вас автоматично перенаправлять на сторінку входу, запрошуючи вас зареєструватися та увійти, після чого ви автоматично повернетеся на поточну сторінку.

**Один API токен дозволяє викликати всі послуги платформи, без необхідності окремо подавати заявку на кожну послугу.** Перша заявка надає безкоштовний ліміт, щоб ви могли безкоштовно спробувати; коли ліміт закінчиться, ви можете поповнити загальний баланс в [консолі](https://platform.acedata.cloud/console/coin).
...
| --------------- | ------- | :-: | ------- | --------------------------------------------------------------------------------------- |
| `expected_type` | enum    |  ❌  | Автоматичне визначення    | Підказка типу сторінки: `product` / `article` / `general`. Пропускає URL / текстову евристичну обробку, безпосередньо переходить до відповідної гілки.                        |
...
> Коли сторінка має schema.org JSON-LD, `enable_llm` неактивний — детермінований мапер безпосередньо видає результати,
> ніколи не витрачаючи виклики LLM. Ви **безкоштовно** отримуєте типізовані результати.
...
| `contentType`   | enum     | `product` / `article` / `general`, визначається послідовно: `expected_type` → schema.org primary → евристично. |
| `title`         | string   | Readability `<title>` або рендерений `document.title`。                                        |
| `description`   | string?  | Пріоритет: `<meta name="description" />` → `og:description` → schema.org / LLM витягування → перший абзац тексту.   |
...
| `cached`        | boolean? | `true`, якщо кеш було задействовано.                                                                       |
| `cacheStoredAt` | number?  | Unix мілісекундний таймстамп, коли кешовий запис був вперше записаний.                                                                |
...
| `llm`       | Коли LLM успішно запущено         | `{ kind, data, model, promptCharCount }`, типізовані результати, перевірені Zod.              |
| `llmError`  | Коли LLM запущено, але не вдалося         | `{ kind, error, model }`, запит не призведе до збоїв, евристичні результати все ще повертаються.                         |
...
```json
{
  ...
  "points": 173,
  "commentCount": 42,
  "body": "Привіт HN, ми створили самостійно розміщену альтернативу API аналізу Diffbot ..."
}
```
...
- **Якщо можна передати `expected_type`, то передавайте.** Безкоштовна порада, пропустіть евристичне визначення, особливо корисно для сторінок, URL-адреси яких не входять до вбудованого списку.
...
- **Під час налагодження спочатку перевірте `rawSignals.hasJsonLd`.** Якщо `true`, але `structured.schemaOrg.primary` дорівнює `null`, це означає, що сторінка використовує тип, який наш мапер ще не покриває — створіть issue, ми додамо.
