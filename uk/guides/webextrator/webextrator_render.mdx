...
Render — це найнижчий рівень інтерфейсу WebExtrator. Якщо вам потрібні **структуровані** результати витягування (основний текст статті, ціни на товари, інгредієнти рецептів тощо), будь ласка, використовуйте
...
| `delay`             | number   |  ❌  | `0`                        | **Додатковий час очікування в секундах** після спрацьовування `wait_until` (для повторного рендерингу SPA).                               |
...
| 402  | (x402)           | Недостатньо коштів на платформі, повертає x402 вимогу на оплату. |
...
Негайно повертає `{ "success": true, "task_id": "...", "trace_id": "...", "started_at": "..." }`; 
коли завдання завершиться, платформа надішле POST з повними результатами на ваш `callback_url`.
