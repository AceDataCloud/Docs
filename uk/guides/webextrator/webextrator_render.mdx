...
[`/webextrator/extract`](development_webextrator_extract) — він працює на тій же основі рендерингу, але проходить через повний набір типізованих конвеєрів витягування.

## Процес подачі заявки

Щоб використовувати сервіс WebExtrator, спочатку перейдіть до [консолі Ace Data Cloud](https://platform.acedata.cloud/console/applications), щоб отримати ваш API Token, зберігши його для подальшого використання.
...
| `timeout`           | number   |  ❌  | `30`                       | Час очікування для одного навігаційного запиту (в **секундах**).                                                          |
...
| `callback_url`      | string   |  ❌  | —                          | Адреса зворотного виклику в асинхронному режимі, платформа надішле повний результат на цю адресу після завершення завдання.                                    |
| `bypass_cache`      | boolean  |  ❌  | `false`                    | Пропустити читання кешу Redis (але все ще запише результати цього запиту в кеш).                                            |
| `cache_ttl_seconds` | number   |  ❌  | `3600`                     | Налаштування TTL кешу для цього запису, передайте `0`, щоб не кешувати цю відповідь.                                         |
| `async`             | boolean  |  ❌  | `false`                    | Встановіть `true`, щоб негайно повернути `task_id`, результати можна отримати через `callback_url` або API завдань.            |

> Платформні контракти використовують **snake_case**. Внутрішні сервіси рендерингу підтримують camelCase, але зовнішні виклики завжди використовують snake_case.
...
| `data.elapsedMs`     | number         | Час, витрачений лише на рендеринг браузером.                                         |
| `data.cached`        | boolean?       | Якщо кеш було використано, то `true`.                                    |
| `data.cacheStoredAt` | number?        | Unix мітка часу в мілісекундах, коли кешовий запис було вперше записано.                             |
...
[`/webextrator/tasks`](development_webextrator_tasks) активне запитування.

### Структура зворотного виклику

Платформа `POST` надсилає **повністю таку ж** оболонку, як і в синхронному режимі, на `callback_url`,
`Content-Type: application/json`. Повернення будь-якого `2xx` вважається підтвердженим; `5xx` буде повторюватися з експоненційною затримкою приблизно 5 хвилин.
...
| 500  | `internal_error` | Внутрішня помилка сервера (виникли винятки, такі як збій браузера), Worker автоматично повторить спробу. |
