...
Щоб використовувати API SeeDance Video Generation, спочатку перейдіть до [консолі Ace Data Cloud](https://platform.acedata.cloud/console/applications) для отримання вашого API Token, залиште його на випадок потреби.
...
```json
{
    ...
    "request": {
        ...
        "content": [
            {
                "type": "text",
                "text": "Кілька кадрів. Детектив входить у темну кімнату. Він перевіряє підказки на столі, беручи в руки один з предметів на столі. Камера переключається на його роздуми. --ratio 16:9"
            }
        ],
        "callback_url": "dummy"
    },
...
Повернення результату містить кілька полів, де `items` містить конкретну інформацію про пакетні завдання, кожне завдання має таку ж структуру, як і в попередньому запиті на одинокі завдання.

- `items`, всі конкретні деталі пакетного завдання. Це масив, кожен елемент масиву має таку ж структуру, як і в попередньому запиті на одинокі завдання.
