---
title: "Fish TTS API інтеграційна інструкція"
...
Щоб використовувати Fish TTS API, спочатку перейдіть до [консолі Ace Data Cloud](https://platform.acedata.cloud/console/applications) для отримання вашого API токена, зберігайте його на випадок потреби.
...
| `model`         | Ні           | TTS модель, може бути `s1`, `s2-pro` або `s2.1-pro`, за замовчуванням `s2-pro`. `s2.1-pro` - нове покоління, `s2-pro` має високу виразність; `s1` більш стабільний, довгі тексти не відхиляються. Всі три мають однакову ціну. |
...
| `format`       | string             | Ні           | Формат виходу аудіо, може бути `mp3` (за замовчуванням), `wav`, `pcm`. `wav` та `pcm` повертають контейнер WAV. `opus` не підтримується, передача призведе до повернення `400`. |
...
| `prosody`      | object             | Ні           | Параметри просодії, підтримує `speed` (швидкість мови, 1.0 - оригінальна швидкість) та `volume` (гучність у дБ). Наприклад, `{"speed":1.2,"volume":0}`.                                      |
...
| `latency`      | string             | Ні           | `normal` або `balanced`, за замовчуванням автоматично встановлюється `normal` (передача порожнього рядка призведе до відмови з боку upstream).                                          |
| `normalize`    | boolean            | Ні           | Чи потрібно нормалізувати текст.                                                                                                                                           |
| `callback_url` | string             | Ні           | Адреса асинхронного зворотного виклику, див. нижче в розділі «Асинхронний зворотний виклик». **Це розширення відносно офіційного інтерфейсу**.                                          |

> Імена полів повністю відповідають upstream. За винятком `callback_url`, інші поля мають те ж значення та варіанти, що й у [Fish офіційній TTS документації](https://docs.fish.audio/text-to-speech/text-to-speech).
...
`speed` більше 1 прискорює, менше 1 сповільнює; `volume` в дБ, 0 означає без змін, позитивні значення - підвищення, негативні - зниження.
...
Довгий текст може потребувати від десяти до кількох десятків секунд для синтезу, якщо з'єднання переривається, потрібно повторити спробу. Якщо в тілі запиту передати `callback_url`, інтерфейс відразу поверне `{task_id, started_at}`, а коли завдання буде завершено, повний результат буде надіслано POST JSON формою на цей URL, з тим же `task_id` в тілі запиту.
