...
> Імена полів повністю відповідають upstream. За винятком `callback_url`, інші поля мають те ж значення та значення, що й у [Fish офіційній TTS документації](https://docs.fish.audio/text-to-speech/text-to-speech).
...
`audio_url` вказує на CDN цієї платформи, його можна безпосередньо завантажити за допомогою GET або відтворити в `<audio>`. Остаточна успішна відповідь також поверне верхній `cost`, де `amount` - це фактично зняті кредити; якщо є знижка на рахунок, `list_amount` вказує на суму до знижки. Посилання доступне на тривалий термін, але все ж рекомендується зберегти копію у вашому власному сховищі.

## Приклад 2: Використання клонованого голосу `reference_id`

Нижче наведено приклад використання одного з публічних іспанських голосів на платформі Fish (`_id` можна отримати через [Fish Model Query](https://platform.acedata.cloud/documents/fish-model-query)):
...
Довгий текст може потребувати від десяти до кількох десятків секунд для синтезу, якщо з'єднання переривається, потрібно повторити спробу. Якщо в тілі запиту передати `callback_url`, інтерфейс відразу поверне `{task_id, started_at}`, а коли завдання буде завершено, повний результат буде надіслано POST JSON формою на цей URL, з `task_id` та остаточним `cost` у тілі запиту. Початкове завдання підтверджує, що синтез ще не завершено, тому не містить `cost`.
...
Також можна використовувати [Fish Tasks API](https://platform.acedata.cloud/documents/fish-tasks) для активного отримання результату за `task_id`; кінцевий запис `response.cost` буде відповідати `cost` у зворотному виклику, деталі дивіться в цій документації.
...
Помилка валідації параметрів міститиме оригінальний текст помилки pydantic у полі `message`, що полегшує виявлення, який саме параметр недійсний, наприклад:
...
Мінімальні витрати на інтеграцію Fish TTS: у вже існуючому коді, що викликає `api.fish.audio/v1/tts`, замініть авторизацію на токен цієї платформи та в тілі запиту **явно вкажіть** `format: "mp3"`. Для довгих текстів рекомендується використовувати асинхронний зворотний виклик `callback_url`; для виявлення клонованого голосу `reference_id` використовуйте [Fish Model Query](https://platform.acedata.cloud/documents/fish-model-query) та [Fish Model Get](https://platform.acedata.cloud/documents/fish-model-get).
