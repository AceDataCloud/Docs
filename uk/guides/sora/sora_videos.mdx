...
Щоб користуватися API, спочатку потрібно подати заявку на відповідний сервіс на сторінці [Sora Videos Generation API](https://platform.acedata.cloud/documents/sora-videos). Після переходу на сторінку натисніть кнопку «Acquire», як показано на зображенні:
