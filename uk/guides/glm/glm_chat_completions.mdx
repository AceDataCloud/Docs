...
- `model`: виберіть модель GLM, яку потрібно викликати, наразі підтримуються такі моделі:
  - `glm-5.1`: найновіша флагманська модель з найсильнішими загальними можливостями.
  - `glm-4.7`: відмінні результати в міркуванні, викликах інструментів та завданнях з коду.
  - `glm-4.6`: універсальна модель для діалогів, що балансує ефективність та вартість.
  - `glm-3-turbo`: класична модель для діалогів, підходить для загальних завдань генерації тексту.
...
- `choices`: список відповідей, згенерованих моделлю. `choices[i].message.content` — це конкретний текст відповіді моделі, `finish_reason` вказує причину завершення (`stop`, `length`, `tool_calls`, `content_filter` тощо).
...
Можна побачити, що у відповіді є багато `data`, кожен `data` містить інкрементальний фрагмент. `choices[i].delta.content` є новим текстовим фрагментом, який додається в поточний chunk, ви можете з'єднати ці фрагменти, щоб сформувати повну відповідь. Коли вміст `data` дорівнює `[DONE]`, це означає, що потік відповіді закінчився. Останній chunk з `usage` підсумовує використання токенів для цього запиту.
...
| --------------- | ------------------------------------- |
| `glm-5.1`      | Найсильніша загальна здатність, рекомендовано для складного міркування, аналізу довгих документів |
| `glm-4.7`      | Виклики інструментів, генерація коду, оркестрація агентів та інші завдання |
| `glm-4.6`      | Збалансований вибір для загальних діалогів та створення контенту        |
...
- `500 api_error`: Внутрішня помилка сервера або тимчасова недоступність upstream.
...
Коли повертається `api_error` і повідомлення `Служба тимчасово недоступна, будь ласка, спробуйте пізніше.`, це зазвичай означає, що служба GLM тимчасово недоступна, рекомендується спробувати з експоненційною затримкою або переключитися на іншу доступну модель GLM (наприклад, тимчасово переключитися з `glm-5.1` на `glm-4.7` або `glm-4.6`).
