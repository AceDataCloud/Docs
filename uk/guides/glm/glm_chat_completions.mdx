...
**Один API Token дозволяє викликати всі послуги платформи, без необхідності окремої заявки для кожної послуги.** При першій заявці ви отримаєте безкоштовний ліміт, щоб спробувати безкоштовно; якщо ліміт вичерпано, ви можете поповнити загальний баланс в [консолі](https://platform.acedata.cloud/console/coin).

> 📘 Повна документація: [GLM Chat Completion API →](https://platform.acedata.cloud/documents/glm-chat-completions)

## Основне використання

Запит до GLM Chat Completion API має адресу `https://api.acedata.cloud/glm/chat/completions`, використовуючи авторизацію Bearer Token, а тіло запиту сумісне з протоколом OpenAI Chat Completions.
...
- `model`: виберіть модель GLM, яку потрібно викликати, наразі підтримуються такі моделі:
  ...
  - `glm-4.7`: відзначається відмінними результатами в міркуванні, викликах інструментів та завданнях з коду.
...
- `usage`: статистика використання токенів для цього запиту, що включає `prompt_tokens`, `completion_tokens`, `total_tokens`.

## Потокова відповідь

Цей інтерфейс підтримує потокову відповідь (Server-Sent Events), що дуже корисно для веб-інтеграцій, дозволяючи веб-сторінці реалізувати ефект поетапного відображення.
...
Якщо ви хочете реалізувати функцію багатократного діалогу, потрібно поетапно додати історію діалогу в масив `messages`, зберігаючи чергування `user` та `assistant`.
...
| `glm-3-turbo`  | Загальні завдання генерації тексту, чутливі до витрат |
