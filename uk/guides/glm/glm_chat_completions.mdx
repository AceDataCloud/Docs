...
GLM (General Language Model) — це нове покоління серії великих мовних моделей, представлене компанією Zhipu AI (智谱 AI / Z.ai), яке має потужні можливості розуміння та генерації як китайською, так і англійською мовами. У китайських сценаріях, генерації коду, міркуванні та багатократних діалогах нові моделі, такі як GLM-5.1, GLM-4.7, GLM-4.6, продемонстрували відмінні результати. Моделі нового покоління значно оптимізовані для роботи з довгими контекстами, викликами інструментів та завданнями з коду, і можуть широко використовуватися в таких сферах, як інтелектуальні запитання та відповіді, створення контенту, допомога в кодуванні, чат-боти тощо.

Цей документ в основному описує процес використання GLM Chat Completion API, за допомогою якого ви можете легко викликати моделі серії GLM через єдиний інтерфейс, сумісний з OpenAI. Якщо вам потрібно використовувати `glm-5.2` в Coze (扣子), будь ласка, зверніться до [посібника з налаштування моделей Coze](https://platform.acedata.cloud/documents/coze-integration) та виберіть протокол OpenAI Chat Completions.

## Процес заявки

Щоб використовувати GLM Chat Completion API, спочатку перейдіть до [консолі Ace Data Cloud](https://platform.acedata.cloud/console/applications), щоб отримати ваш API Token, зберігши його для подальшого використання.

![](https://cdn.acedata.cloud/dvc3cg.jpg)

Якщо ви ще не увійшли в систему або не зареєстровані, вас автоматично перенаправлять на сторінку входу, запрошуючи вас зареєструватися та увійти. Після завершення ви будете автоматично повернені на поточну сторінку.

**Один API Token дозволяє викликати всі послуги платформи, без необхідності окремої заявки для кожної послуги.** При першій заявці ви отримаєте безкоштовний ліміт, щоб безкоштовно протестувати; якщо ліміт вичерпано, ви можете поповнити загальний баланс на [консолі](https://platform.acedata.cloud/console/coin).

> 📘 Повна документація: [GLM Chat Completion API →](https://platform.acedata.cloud/documents/glm-chat-completions)

## Основне використання

Запит до GLM Chat Completion API має адресу `https://api.acedata.cloud/glm/chat/completions`, використовуючи авторизацію Bearer Token, тіло запиту сумісне з протоколом OpenAI Chat Completions.
...
- `model`: виберіть модель GLM, яку потрібно викликати, наразі підтримуються такі моделі:
  - `glm-5.2`: новітня флагманська модель, підходить для складних міркувань, коду та завдань Agent.
  - `glm-5.1`: попередня флагманська модель, з сильними загальними можливостями.
  - `glm-4.7`: відмінно проявляє себе в міркуваннях, викликах інструментів та завданнях з коду.
  - `glm-4.6`: універсальна модель діалогу, що балансує ефективність та вартість.
  - `glm-3-turbo`: класична модель діалогу, підходить для загальних завдань генерації тексту.
...
Опис основних полів у повернутому результаті:
...
Цей інтерфейс підтримує потокову відповідь (Server-Sent Events), що дуже корисно для веб-інтерфейсів, дозволяючи реалізувати ефект поетапного відображення.

Якщо ви хочете отримати відповідь у потоковому режимі, просто встановіть параметр `stream` у `true` в тілі запиту.
...
Можна побачити, що у відповіді є багато `data`, кожен `data` містить інкрементальний фрагмент. `choices[i].delta.content` є новим текстовим фрагментом, який додається в поточному chunk, ви можете з'єднати ці фрагменти, щоб сформувати повну відповідь. Коли вміст `data` дорівнює `[DONE]`, це означає, що потік відповіді закінчився. Останній chunk з `usage` підсумовує використання токенів для цього запиту.
...
Якщо модель вирішить викликати інструмент, причина завершення в результаті зміниться на `tool_calls`, і в `message.tool_calls` буде вказано ім'я функції та параметри у формі JSON-рядка. Ви можете виконати цю функцію та повернути результат як повідомлення з `role` як `tool` назад до моделі, завершуючи повний цикл виклику інструменту.

## Рекомендації щодо вибору моделі
| Модель          | Сценарії використання                |
| --------------- | ----------------------------------- |
| `glm-5.2`      | Останній флагман, рекомендований для складного міркування, коду та завдань Agent |
| `glm-5.1`      | Попереднє покоління флагмана, підходить для складного міркування, аналізу довгих документів |
| `glm-4.7`      | Виклики інструментів, генерація коду, оркестрація Agent та інші завдання |
| `glm-4.6`      | Універсальний вибір для діалогу та створення контенту |
...
- `500 api_error`: Внутрішня помилка сервера або тимчасова недоступність.
...
Коли повертається `api_error` і повідомлення `Служба тимчасово недоступна, будь ласка, спробуйте пізніше.`, це зазвичай означає, що служба GLM тимчасово недоступна, рекомендується використовувати експоненціальне повторення спроб або переключитися на іншу доступну модель GLM (наприклад, тимчасово переключитися з `glm-5.1` на `glm-4.7` або `glm-4.6`).

## Висновок

Завдяки цьому документу ви дізналися, як використовувати API GLM Chat Completion для виклику моделей серії GLM від Zhizhu AI, включаючи базові виклики, потокові відповіді, багатократні діалоги, системні підказки та виклики інструментів. Сподіваємося, що цей документ допоможе вам краще інтегрувати та використовувати цей API. Якщо у вас є будь-які питання, будь ласка, звертайтеся до нашої команди технічної підтримки.
