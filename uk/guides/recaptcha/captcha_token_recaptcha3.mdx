---
title: "Recaptcha3 протокол розпізнавання API інтеграція"
description: "Recaptcha verification code recognition service API guide - Ace Data Cloud"
---

У цьому документі буде представлено опис інтеграції API розпізнавання протоколу Recaptcha3, який дозволяє користувачам не розпізнавати та не натискати на зображення коду Recaptcha3, а лише подати Website Key для автоматичного декодування на сервері та завершення перевірки.

## Процес подачі заявки

Щоб використовувати API розпізнавання протоколу Recaptcha3, спочатку перейдіть до [консолі Ace Data Cloud](https://platform.acedata.cloud/console/applications) для отримання вашого API Token, зберігайте його для подальшого використання.
...
> 📘 Повна документація: [API розпізнавання протоколу Recaptcha3 →](https://platform.acedata.cloud/documents/captcha-token-recaptcha3)

## Основне використання

Спочатку ознайомтеся з основними способами використання. У порівнянні з Recaptcha2, нам потрібно додатково передати параметр `page_action`, який потрібно отримати з коду. URL для демонстрації швидкості мережі: `https://recaptcha-demo.appspot.com/recaptcha-v3-request-scores.php`, нижче представлений один із способів отримання:
...
Далі потрібно ввести URL сайту, на якому потрібно обробити код, щоб отримати оброблений результат. Спочатку потрібно просто передати поле `website_url`, а потім ще потрібно ввести параметр `website_key`, цей вміст можна отримати вище, це також рядок символів після execute. Тепер ми можемо заповнити відповідні поля на інтерфейсі, як показано на малюнку:
...
За замовчуванням API є синхронним і блокує: один запит буде чекати, поки токен не буде оброблений, перш ніж повернути результат. Якщо ви займаєтеся ротацією багатьох розв'язувачів (multi-solver rotation) і хочете «одразу отримати task_id після подачі завдання, спочатку перейти до інших розв'язувачів, а потім повернутися за результатом», ви можете передати `async: true` у тілі запиту.
