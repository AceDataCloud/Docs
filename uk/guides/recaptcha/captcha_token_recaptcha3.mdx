...
Відкрийте f12, а потім на сторінці Element шукайте `.execute(`, у червоній рамці ми можемо побачити параметр `action`, одночасно після execute йде рядок символів, це також вміст, необхідний далі, конкретно, як показано на малюнку нижче.
...
За замовчуванням API є синхронним і блокує: один запит буде чекати, поки обробка токена не завершиться. Якщо ви займаєтеся ротацією багатьох розпізнавачів (multi-solver rotation) і хочете «одразу отримати task_id після подачі завдання, спочатку перейти до інших розпізнавачів, а потім повернутися за результатом», ви можете передати `async: true` в тілі запиту.
...
Завдяки цьому документу ви вже дізналися, як використовувати API для розпізнавання протоколу Recaptcha3, щоб користувачі не повинні були розпізнавати та натискати на зображення верифікації Recaptcha3, а лише подавати Website Key для автоматичного декодування на бекенді та завершення верифікації. Сподіваємося, що цей документ допоможе вам краще інтегрувати та використовувати цей API. Якщо у вас є будь-які питання, будь ласка, звертайтеся до нашої команди технічної підтримки.
