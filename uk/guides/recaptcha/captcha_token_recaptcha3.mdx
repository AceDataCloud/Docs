...
У цьому документі буде представлено опис інтеграції API ідентифікації протоколу Recaptcha3, який дозволяє користувачам не розпізнавати та не натискати на зображення коду Recaptcha3, а лише подавати Website Key для автоматичного декодування на сервері та завершення перевірки.

## Процес подачі заявки

Щоб використовувати API ідентифікації протоколу Recaptcha3, спочатку перейдіть до [консолі Ace Data Cloud](https://platform.acedata.cloud/console/applications) для отримання вашого API Token, зберігайте його для подальшого використання.
...
> 📘 Повна документація: [API ідентифікації протоколу Recaptcha3 →](https://platform.acedata.cloud/documents/captcha-token-recaptcha3)

## Основне використання

Спочатку ознайомтеся з основними способами використання, у порівнянні з Recaptcha2, нам потрібно додатково передати параметр `page_action`, цей параметр потрібно отримати з коду, URL швидкості мережі, що демонструється, є: `https://recaptcha-demo.appspot.com/recaptcha-v3-request-scores.php`, нижче показано один із способів отримання:
...
- `page_action`: потрібно отримати з коду сайту, на якому знаходиться код.
...
Якщо потрібно активно перевіряти прогрес, ви можете використовувати цей `task_id` для запиту `POST /captcha/tasks` (рекомендується кожні 3~5 секунд). Цей інтерфейс не відповідає за просування обробки завдання; навіть якщо не запитувати, відключити інтернет або вийти з клієнта, сервер все ще продовжить обробку:
...
Опис тарифікації: в асинхронному режимі створення завдання та читання статусу «в обробці» не підлягають тарифікації; **клієнт сплачує один раз при першому успішному читанні результату** (як і в поточній поведінці та ціні синхронного режиму). Сервер самостійно просуває завдання, але не стягує плату за те, що завдання завершилося раніше в фоновому режимі. Якщо завдання не буде успішно завершено протягом 120 секунд, воно буде зупинено з HTTP 504 `timeout`, без тарифікації. `/captcha/tasks` не відповідає за просування завдання.
