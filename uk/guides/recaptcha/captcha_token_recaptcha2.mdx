...
У цьому документі буде представлено інструкцію з інтеграції API для розпізнавання протоколу Recaptcha2, яка дозволяє користувачам не розпізнавати та не натискати на зображення коду Recaptcha2, а лише подавати Website Key для автоматичного декодування на сервері та завершення перевірки.

## Процес подачі заявки

Щоб використовувати API для розпізнавання протоколу Recaptcha2, спочатку перейдіть до [консолі Ace Data Cloud](https://platform.acedata.cloud/console/applications), щоб отримати ваш API Token, зберігши його для подальшого використання.
...
Опис тарифікації: в асинхронному режимі створення завдання та опитування «в обробці» не підлягають тарифікації; **тільки при успішному отриманні результату стягується плата один раз** (з ціною, аналогічною синхронному режиму). Тому скасування незавершених завдань під час ротації не призведе до витрат. `/captcha/tasks` є універсальним для всіх інтерфейсів перевірки (серії токенів і розпізнавання), можна використовувати один і той же `task_id` для опитування.
