...
У цьому документі буде представлено інструкцію з інтеграції API для розпізнавання протоколу Recaptcha2, яка дозволяє користувачам не розпізнавати та не натискати на зображення коду підтвердження Recaptcha2, а лише подати Website Key для автоматичного декодування на сервері та завершення перевірки.
...
Спочатку розглянемо основний спосіб використання, а саме введення URL сайту, на якому потрібно обробити код підтвердження, щоб отримати оброблений результат. Спочатку потрібно просто передати поле `website_url`, наш приклад сайту: `https://www.google.com/recaptcha/api2/demo`, нам потрібно отримати `website_key` на сторінці `website_url`, спочатку відкрийте цю веб-сторінку, натисніть F12 для входу в консоль, а потім у вкладці Element виконайте глобальний пошук за `recaptcha-demo`, ми можемо отримати наступний результат:
...
- `website_url`: URL сайту, на якому потрібно обробити код підтвердження.
...
- Проаналізувати конструкцію POST запиту, в результаті можна правою кнопкою миші скопіювати код CURL цього запиту, деталі наведені на малюнку нижче:
