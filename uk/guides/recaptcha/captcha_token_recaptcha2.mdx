...
У цій статті буде представлено інструкцію з інтеграції API для розпізнавання протоколу Recaptcha2, яка дозволяє користувачам не розпізнавати та не натискати на зображення коду Recaptcha2, а лише подавати Website Key для автоматичного декодування на бекенді та завершення верифікації.
...
**Один API Token дозволяє викликати всі послуги платформи, без необхідності окремої заявки для кожної послуги.** Перший запит на отримання токена надає безкоштовний ліміт, що дозволяє безкоштовно протестувати; при недостатньому ліміті ви можете поповнити загальний баланс на [консолі](https://platform.acedata.cloud/console/coin).
...
- `proxy`: необов'язковий, власний проксі (Bring Your Own Proxy). Після налаштування служба буде використовувати наданий вами проксі IP для розв'язання коду, щоб контролювати якість вихідного IP (наприклад, уникнути блокування цільовим сайтом через публічний проксі IP, що призводить до повернення `410 Gone`). Формат: `scheme://[user:pass@]host:port`, `scheme` підтримує `http`/`https`/`socks4`/`socks5`, наприклад, `http://user:pass@1.2.3.4:8080`. Якщо не заповнити, буде використано проксі за замовчуванням платформи.
...
- `token`, результат верифікації після обробки завдання Recaptcha2.
...
Ми отримали результат верифікації обробки Recaptcha2, який можна використовувати для POST або імітації відправки на цільовий сайт, одноразового використання, термін дії 120 секунд, рекомендується використовувати протягом 60 секунд. Далі буде надано фрагмент Python-коду, який відправляє оброблений токен на цільовий сайт для проходження Recaptcha2.

Спочатку нам потрібно дізнатися, як сайт надсилає POST-запити, щоб ми могли передати згенерований токен. Для цього потрібно відкрити консоль F12, потім вручну пройти перевірку, в результаті чого ми побачимо, що сайт надіслав POST-запит. Нам потрібно лише переглянути конструкцію цього POST-запиту, конкретний процес виглядає так:

- Спочатку вручну пройти перевірку, конкретно, як показано на малюнку нижче:

<p><img src="https://cdn.acedata.cloud/gwiflr.png" width="500" className="m-auto" /></p>

- Потім натиснути submit, спостерігаючи за змінами в мережі консолі, конкретно, як показано на малюнку нижче:
<p><img src="https://cdn.acedata.cloud/6hm5kj.png" width="500" className="m-auto" /></p>

- Проаналізуйте структуру POST-запиту, в кінці ви можете клацнути правою кнопкою миші на запиті, щоб скопіювати код CURL, конкретно, як показано на наступному зображенні:

<p><img src="https://cdn.acedata.cloud/m7y35w.png" width="500" className="m-auto" /></p>

З вище наведеного зображення видно, що URL цього POST-запиту: `https://www.google.com/recaptcha/api2/demo`, нам потрібно лише надіслати параметр `g-recaptcha-response`, після чого ми просто передаємо оброблений токен у нижче наведених даних, код CURL для перевірки токена виглядає так:
...
Код Python для перевірки токена виглядає так:
...
За замовчуванням API є синхронним і блокує: один запит буде чекати, поки токен не буде оброблено, перш ніж повернути результат. Якщо ви займаєтеся ротацією кількох розв'язувачів (multi-solver rotation) і хочете «після подання завдання відразу отримати task_id, спочатку перейти до інших розв'язувачів, а потім повернутися для отримання результату», ви можете передати `async: true` у тілі запиту.
...
Якщо потрібно активно перевірити прогрес, ви можете використовувати цей `task_id` для запиту `POST /captcha/tasks` (рекомендується кожні 3-5 секунд). Цей інтерфейс не буде ініціювати або просувати обробку завдання; навіть якщо не запитувати, відключити інтернет або вийти з клієнта, сервер все ще буде продовжувати обробку:
...
Опис тарифікації: в асинхронному режимі створення завдання та читання статусу «в обробці» не підлягають тарифікації; **клієнт сплачує один раз при першому успішному читанні результату** (як і в поточній поведінці та ціні синхронного режиму). Сервер самостійно просуває завдання, але не стягує плату за те, що завдання завершено раніше в фоновому режимі. Якщо завдання не буде успішно завершено протягом 120 секунд, воно буде зупинено з HTTP 504 `timeout`, без тарифікації. `/captcha/tasks` не відповідає за просування завдання.
