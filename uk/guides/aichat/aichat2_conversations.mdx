...
AI Chat v2 API (`/aichat2/conversations`) є новим поколінням діалогового інтерфейсу, який є повним оновленням [AI Chat API](https://platform.acedata.cloud/documents/aichat-conversations). Він розширює v1, що є простим, хостованим багатокроковим діалогом, на основі:
...
- **Інструменти для виклику агентів**: вбудований набір інструментів для пошуку в Інтернеті, веб-скрапінгу, читання файлів тощо, а також можливість підключення до серверів MCP, авторизованих користувачем (Google Drive, Notion, Slack, GitHub тощо), модель може в одному запиті багаторазово самостійно викликати інструменти для виконання складних завдань.
...
```json
{
  "answer": "AceDataCloud - це об'єднана API платформа, що агрегує основні AI моделі та багатомодальні послуги, розробники можуть викликати GPT, Claude, Gemini, Midjourney, Suno, Veo та інші послуги за допомогою одного ключа.",
...
Другий запит, передайте той же `id`:
```shell
curl -X POST 'https://api.acedata.cloud/aichat2/conversations' \
  -H 'accept: application/json' \
  -H 'authorization: Bearer {token}' \
...
| `type`              | Значення                                                                                              |
| ------------------- | ----------------------------------------------------------------------------------------------- |
| `text_delta`        | Інкрементальні текстові фрагменти відповіді помічника. `content` — новий вміст; для сумісності з v1, одна подія також містить `delta_answer` (дорівнює `content`) та `id`.                  |
...
Для більш детального контролю (наприклад, щоб розмістити кілька зображень між текстами або якщо порядок дуже важливий) використовуйте масив `message`.
...
```json
{
  "model": "gpt-5.5",
  "async": true,
  "question": "Моя служба сповіщає, використай особистий WeChat, щоб повідомити групу WeChat «Команда AceDataCloud»……"
}
```
...
Фонові завдання зазвичай не мають можливості підтвердження. Якщо ви хочете, щоб деякі навички або сервери MCP виконували дії надсилання, публікації, запису тощо в безлюдному режимі, будь ласка, явно передайте список попередньої авторизації в тілі запиту:
...
Значення в `allowed_skills` - це slug підключеної навички; значення в `allowed_mcp_servers` - це slug підключеного сервера MCP. Навички / сервери MCP, які не включені до попередньої авторизації, в безлюдному режимі можуть лише переглядати, виконувати dry-run або відмовлятися від виконання запису.
...
Попередня авторизація - це ці два списки: порожній список означає, що жодні можливості не авторизовані, без додаткових перемикачів.

Зверніть увагу: попередня авторизація лише означає «цей запит дозволяє цим можливостям пропустити ручне підтвердження в безлюдному режимі». Конкретна навичка все ще повинна підтримувати `--unattended-confirm` або відповідний механізм безпеки; в іншому випадку вона продовжить dry-run і не виконає запис.
...
На фронтенді відобразіть цю подію як картку, щоб користувач міг вибрати відповідь, а потім надішліть наступний запит з тим же `id`, заповнивши відповідь через `tool_results`:
...
2. Якщо ви раніше передавали назви моделей v1 (наприклад, `gpt-3.5`, `gpt-4-browsing` тощо), при переході на v2 рекомендується оновити до сучасних моделей (наприклад, `gpt-5.4`, `claude-opus-4-8`, `gemini-3.1-pro-preview` тощо).
...
- `400 bad_request`: відсутні обов'язкові поля, `tool_use_id` не збігається, схема `messages` недійсна тощо.
...
У потоковій відповіді помилки надсилаються як `{"type":"error","message":"..."}` подія, після чого потік закінчується.
