...
- **Можливість переривання / відновлення**: модель видає подію `ask_user_question` і призупиняється, коли їй потрібна додаткова інформація від користувача, наступний виклик може продовжити через заповнення відповіді за допомогою `tool_results`.
- **Нові CRUD дії**: на одному і тому ж кінцевому пункті через поле `action` можна виконувати `retrieve` / `retrieve_batch` / `update` / `delete`, без необхідності додаткового API для управління сесіями.
...
При цьому на рівні тіла запиту **повністю зберігається зворотна сумісність з v1**: достатньо передати `model` + `question` (+ необов'язкові `stateful` / `id` / `references` / `preset`), щоб отримати еквівалентну `{answer, id}` JSON відповідь, тому для міграції з `/aichat/conversations` не потрібно переписувати клієнт, достатньо змінити шлях на `/aichat2/conversations`.
...
Щоб використовувати AI Chat v2 API, спочатку перейдіть до [консолі Ace Data Cloud](https://platform.acedata.cloud/console/applications) для отримання вашого API токена, зберігайте його на всякий випадок.
...
```json
{
  "answer": "Добре, я запам'ятав 42. Що мені з ним зробити?",
  "id": "f2f4b3e8-0c0a-4d3a-aaa2-7ff80c0a1c44"
}
```
...
```json
{
  "model": "gpt-5.5",
  "async": true,
  "question": "Моя служба сповіщає, використовуючи особистий WeChat, щоб повідомити групу WeChat «Команда AceDataCloud»……"
}
```
...
Значення в `allowed_skills` - це slug підключеної навички; значення в `allowed_mcp_servers` - це slug підключеного сервера MCP. Навички / сервери MCP, які не включені до попередньої авторизації, в безлюдному режимі можуть лише переглядати, виконувати dry-run або відмовлятися виконувати операції запису.
...
Зверніть увагу: попередня авторизація лише означає «цей запит дозволяє цим можливостям пропустити ручне підтвердження в безлюдному режимі». Конкретна навичка все ще повинна підтримувати `--unattended-confirm` або відповідний механізм безпеки; в іншому випадку вона продовжить dry-run, не виконуючи безпосередньо операції запису.
...
Повертає `{ id, success: true }`. Після видалення не можна відновити, будь ласка, підтверджуйте перед викликом.
