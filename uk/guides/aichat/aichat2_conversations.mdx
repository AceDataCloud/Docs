---
title: "AI Chat v2 API Інструкція з інтеграції"
description: "AI Dialogue API guide - Ace Data Cloud"
---

AI Chat v2 API (`/aichat2/conversations`) — це нове покоління інтерфейсу діалогу, повністю оновлена версія [AI Chat API](https://platform.acedata.cloud/documents/aichat-conversations). Воно базується на v1, який був простим і підтримував багатокрокові діалоги, і розширює можливості:

- **Багатомодальний ввід користувача**: через структуроване поле `message` можна безпосередньо передавати текст + зображення + файли, без необхідності додавати їх опосередковано через `references`.
- **Agent-інструменти**: вбудований набір інструментів для пошуку в мережі, скрапінгу веб-сторінок, читання файлів тощо, а також можливість підключення MCP серверів користувача (Google Drive, Notion, Slack, GitHub тощо). Модель може в одному запиті багаторазово викликати інструменти для виконання складних завдань.
- **Структуровані потокові події**: через `accept: text/event-stream` або `application/x-ndjson` можна отримувати події по одному токену: `text_delta`, `tool_use`, `tool_result`, `thinking`, `citation`, `card`, `artifact` тощо, що полегшує їхнє відображення на фронтенді за типами.
- **Можливість переривання / відновлення**: модель при потребі додаткової інформації від користувача надсилає подію `ask_user_question` і призупиняється, наступний виклик з `tool_results` з відповіддю продовжує роботу.
- **Нові CRUD-операції**: на тому ж endpoint через поле `action` можна виконувати `retrieve` / `retrieve_batch` / `update` / `delete` без додаткових API для керування сесіями.
- **Постійно оновлюваний список моделей**: за замовчуванням доступні GPT-5.4, Claude Opus 4.8, Claude Sonnet 4.6, Gemini 3.1 Pro, GLM 5.1, DeepSeek V4, Kimi K3 та інші сучасні моделі.

Водночас він **повністю сумісний з v1 на рівні тіла запиту**: достатньо передати `model` + `question` (+ опціонально `stateful` / `id` / `references` / `preset`), щоб отримати JSON-відповідь `{answer, id}`, еквівалентну v1. Тож при переході з `/aichat/conversations` на `/aichat2/conversations` не потрібно переписувати клієнт, лише змінити шлях.

> Якщо ви зараз використовуєте `/aichat/conversations`, старий інтерфейс залишиться в роботі, можна мігрувати у зручному темпі.

## Процес отримання доступу

Щоб користуватися AI Chat v2 API, спершу отримайте API Token у [Ace Data Cloud консолі](https://platform.acedata.cloud/console/applications) для подальшого використання.

![](https://cdn.acedata.cloud/dvc3cg.jpg)

Якщо ви не увійшли або не зареєстровані, вас автоматично перенаправить на сторінку входу з пропозицією зареєструватися, після чого поверне на цю сторінку.

**Один API Token дає доступ до всіх сервісів платформи, не потрібно отримувати окремі ключі для кожного сервісу.** При першому отриманні надається безкоштовний ліміт для тестування; при вичерпанні ліміту можна поповнити баланс у [консолі](https://platform.acedata.cloud/console/coin).
...
Доступні значення `model` можна побачити у випадаючому списку панелі Try праворуч, поширені категорії включають:
...
- xAI: `grok-4` тощо
- DeepSeek: `deepseek-v4-pro`, `deepseek-v4-flash`, `deepseek-v3.2-exp`, `deepseek-r1-0528` тощо
- Moonshot: `kimi-k3`, `kimi-k2.6`, `kimi-k2.5` тощо
- Zhipu: `glm-5.1`, `glm-5`, `glm-5-turbo`, `glm-4.7`, `glm-4.5v` тощо

Правила тарифікації дивіться на сторінці сервісу у картці Pricing.

## Багатокрокові діалоги

Як і у v1, передайте `stateful: true` для збереження сесії, API поверне `id`; у наступних запитах передавайте цей `id`, щоб продовжити діалог без необхідності зберігати історію повідомлень самостійно.
...
NDJSON кожен рядок є структурованою подією, найпоширеніша з яких — `text_delta`:
...
На стороні браузера використання `EventSource` не підтримує користувацькі тіла запитів, рекомендується використовувати `fetch` + ручне розділення за `\n\n`:
...
| `type`              | Значення                                                                                              |
| ------------------- | ----------------------------------------------------------------------------------------------- |
| `text_delta`        | Інкрементальні текстові фрагменти відповіді помічника. `content` — новий вміст; для сумісності з v1, одна й та ж подія також містить `delta_answer` (дорівнює `content`) та `id`.                  |
...
Для клієнтів, які цікавляться лише остаточною відповіддю, з'єднання всіх `text_delta` `content` буде еквівалентним `answer` в режимі `application/json`.
...
Для більш точного контролю (наприклад, щоб розмістити кілька зображень між текстами або порядок був важливим) використовуйте масив `message`.
...
Якщо ваш виклик походить з вебхука сповіщення, CI/CD, системи моніторингу або інших фонових завдань, ви можете встановити `async: true`, щоб інтерфейс негайно повернув ID завдання, а фонова частина продовжила виконання:
...
Потім можна використовувати `action: retrieve` + `id` для запиту результату сесії; також можна надати `callback_url`, після завершення завдання платформа надішле `{ status, answer, usage, error }` POST на вашу адресу зворотного виклику. `callback_url` повинен використовувати `http` / `https`, і не може бути безпосередньо вказано `localhost` або приватну IP-адресу.

Фонові завдання зазвичай не мають людей, які можуть натискати підтвердження. Якщо ви хочете, щоб деякі навички або сервери MCP виконували дії надсилання, публікації, запису тощо в безлюдному режимі, будь ласка, явно передайте список попередньої авторизації в тілі запиту:
...
Значення в `allowed_skills` - це slug підключеної навички; значення в `allowed_mcp_servers` - це slug підключеного сервера MCP. Навички / сервери MCP, які не включені до попередньої авторизації, в безлюдному режимі все ще можуть лише переглядати, виконувати dry-run або відмовлятися від виконання запису.
...
Попередня авторизація - це самі ці два списки: порожній список означає, що жодні можливості не авторизовані, без додаткових перемикачів.
...
Деякі інструменти можуть змусити модель «запитати користувача», у цей момент модель видасть подію `ask_user_question`, розмова буде заморожена в стані `awaiting_user_input`:
...
У тілі запиту `tool_use_id` **повинен** повністю збігатися з `tool_id` під час призупинення; невідповідність призведе до повернення 400. Коли в запиті одночасно присутні `tool_results`, `question` / `message` / `references` будуть ігноруватися.
...
v2 на одному й тому ж кінцевому пункті пропонує легке управління сесією через поле `action`, без необхідності відкривати додатковий API.
...
Повертає `{ items: [...], total }`. **Підсумок не містить `messages`**, підходить для списку в бічній панелі; якщо користувач відкриває певну розмову, то знову використовуйте `action: retrieve`, щоб окремо отримати її повні повідомлення.
...
Повертає `{ id, success: true }`. Після видалення відновити не можна, будь ласка, підтверджуйте перед викликом.

## Плавний перехід з v1

Якщо ви вже використовуєте [`/aichat/conversations`](https://platform.acedata.cloud/documents/aichat-conversations), перехід на v2 майже не потребує зміни коду:
...
3. Поля потоку NDJSON залишаються назад сумісними: кожна подія `text_delta` все ще містить `delta_answer` та `id`, тому клієнти, які раніше аналізували `delta_answer` по рядках, не потребують змін.
...
```json
{
  "error": {
    "code": "chat_error",
    "message": "модель сервісу повернула помилку"
  },
