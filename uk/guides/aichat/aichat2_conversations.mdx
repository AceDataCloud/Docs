...
AI Chat v2 API (`/aichat2/conversations`) є новим поколінням діалогового інтерфейсу, який є повним оновленням [AI Chat API](https://platform.acedata.cloud/documents/aichat-conversations). Він розширює v1, що є простим і керованим багатократним діалогом, наступними можливостями:

- **Багатомодальне введення користувача**: через структуроване поле `message` можна безпосередньо передавати текст + зображення + файли, без необхідності спочатку використовувати `references` для непрямого додавання.
- **Інструменти для виклику агентів**: вбудований набір інструментів для пошуку в Інтернеті, веб-скрапінгу, читання файлів тощо, а також можливість підключення до серверів MCP, авторизованих користувачем (Google Drive, Notion, Slack, GitHub тощо), модель може самостійно викликати інструменти кілька разів в одному запиті для виконання складних завдань.
...
- **Можливість переривання / відновлення**: модель видає подію `ask_user_question` і призупиняється, коли їй потрібна додаткова інформація від користувача, наступний виклик може продовжити, заповнивши відповідь через `tool_results`.
...
Водночас на рівні тіла запиту **повністю зворотно сумісний з v1**: достатньо передати `model` + `question` (+ необов'язкові `stateful` / `id` / `references` / `preset`), щоб отримати еквівалентну `{answer, id}` JSON відповідь, тому для міграції з `/aichat/conversations` не потрібно переписувати клієнт, достатньо змінити шлях на `/aichat2/conversations`.
...
Щоб використовувати AI Chat v2 API, спочатку перейдіть до [консолі Ace Data Cloud](https://platform.acedata.cloud/console/applications), щоб отримати ваш API Token, зберігайте його на всякий випадок.
...
Найпростіший спосіб використання повністю ідентичний v1: передайте `model` + `question`, щоб отримати `{answer, id}`.
...
```json
{
  "answer": "AceDataCloud - це об'єднана API платформа, що агрегує основні AI моделі та багатомодальні послуги, розробники можуть викликати GPT, Claude, Gemini, Midjourney, Suno, Veo та інші сервіси за допомогою одного ключа.",
...
Доступні значення `model` можна безпосередньо побачити у випадаючому меню панелі Try праворуч, поширені категорії включають:
...
```json
{
  "answer": "Гаразд, я вже запам'ятав 42. Що мені з ним зробити?",
  "id": "f2f4b3e8-0c0a-4d3a-aaa2-7ff80c0a1c44"
}
```
...
| Сервер / CLI / Node потокова обробка | `application/x-ndjson` | Кожен рядок — один JSON об'єкт                               |
...
payload = {
    "model": "gpt-5.4",
    "stateful": True,
    "question": "Розкажіть про Ханчжоу трьома реченнями.",
}
...
| `thinking`          | Процес мислення моделі (з'являється лише тоді, коли вибрана модель відкриває reasoning).                                                                |
| `tool_use`          | Модель вирішує викликати інструмент, подія містить `tool_id`, `tool_name`, `input`.                                                  |
| `tool_result`       | Результат виконання інструменту, пов'язується з попереднім `tool_use` через `tool_id`, `is_error` вказує, чи сталася помилка.                                       |
...
| `artifact`          | Незалежний продукт, створений моделлю (наприклад, блоки коду, документи), може бути збережений або завантажений.                                                                      |
| `system_message`    | Повідомлення системи (не вміст користувача та помічника), використовується лише для підказок UI.                                                                     |
| `compact`           | Подія, в якій внутрішній контекст був стиснутий, не потребує спеціальної обробки.                                                                             |
| `error`             | Помилка, що сталася в цьому раунді, `message` описує зміст помилки.                                                                        |
...
Потім можна використовувати `action: retrieve` + `id` для запиту результату сесії; також можна надати `callback_url`, після завершення завдання платформа надішле `{ status, answer, usage, error }` на вашу адресу зворотного виклику. `callback_url` повинен використовувати `http` / `https`, і не може бути безпосередньо вказан `localhost` або приватна IP адреса.

Фонові завдання зазвичай не мають можливості підтвердження. Якщо ви хочете, щоб деякі навички або MCP сервери виконували дії надсилання, публікації, запису тощо в безлюдному режимі, будь ласка, явно передайте список попередньо авторизованих у тілі запиту:
...
Значення в `allowed_skills` - це slug підключеної навички; значення в `allowed_mcp_servers` - це slug підключеного MCP сервера. Навички / MCP сервери, які не включені до попередньо авторизованих, в безлюдному режимі можуть лише переглядати, виконувати dry-run або відмовлятися від виконання запису.
...
Зверніть увагу: попередня авторизація лише означає «цей запит дозволяє цим можливостям у безлюдному режимі пропустити ручне підтвердження». Конкретна навичка все ще повинна підтримувати `--unattended-confirm` або відповідний механізм безпеки; в іншому випадку вона продовжить dry-run, не виконуючи безпосередньо запис.

## Відновлення призупиненої розмови

Деякі інструменти можуть змусити модель «запитати користувача», у цей момент модель видає подію `ask_user_question`, розмова заморожується в стані `awaiting_user_input`:
...
```json
{
  ...
  "question": "Якою мовою ви хочете, щоб звіт був створений, китайською чи англійською?",
  "options": ["китайською", "англійською"],
  "id": "f2f4b3e8-..."
}
```

На фронтенді це подія рендериться як картка, щоб користувач міг вибрати відповідь, а потім за допомогою того ж `id` ініціюйте наступний запит, заповнивши відповідь через `tool_results`:
...
У тілі запиту `tool_use_id` **повинен** повністю збігатися з `tool_id` під час призупинення; невідповідність призведе до повернення 400. Коли в запиті одночасно присутні `tool_results`, `question` / `message` / `references` будуть проігноровані.
...
- `500 chat_error`: помилка з боку LLM або в цьому раунді `completion_tokens=0` (обробляється як не спожите, не буде стягнено плату).
