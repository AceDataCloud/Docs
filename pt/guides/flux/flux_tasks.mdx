...
A principal função da API Flux Tasks é consultar o status de execução de uma tarefa através do ID da tarefa gerado pela API Flux Images Generation.
...
Para usar a API Flux Images Generation, primeiro acesse o [Painel de Controle da Ace Data Cloud](https://platform.acedata.cloud/console/applications) para obter seu Token de API, que deve ser guardado para uso futuro.
...
A API Flux Tasks pode ser usada para consultar os resultados da API Flux Images Generation. Para saber como usar a API Flux Images Generation, consulte a documentação [Flux Images Generation API](https://platform.acedata.cloud/documents/flux-images-integration).
...
Esta operação é para consultar os detalhes das tarefas de imagem para vários IDs de tarefa, diferentemente do anterior, onde a ação deve ser selecionada como retrieve_batch.
