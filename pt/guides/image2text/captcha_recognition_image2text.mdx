...
Este documento apresenta uma descrição da API de reconhecimento de captchas alfanuméricos, que é baseada em tecnologia de aprendizado profundo e pode ser utilizada para reconhecer captchas alfanuméricos de comprimento variável. A entrada é a imagem do captcha e a saída é o resultado do captcha.
...
Primeiro, entenda a forma básica de uso, que consiste em inserir a imagem do captcha alfanumérico de comprimento variável que precisa ser processada para obter o resultado. Você deve simplesmente passar um campo `image`, que é a imagem do captcha alfanumérico, como mostrado na figura:

<p><img src="https://cdn.acedata.cloud/c50hi9.png" width="500" className="m-auto" /></p>

Em seguida, precisamos converter a imagem do captcha em uma codificação Base64. Para converter a codificação Base64, recomenda-se usar a extensão FeHelper do Google Chrome. O método de uso específico pode ser consultado na imagem abaixo:
...
Após a seleção, você verá que o código correspondente foi gerado à direita, como mostrado na figura:
...
Além disso, se você quiser gerar o código de integração correspondente, pode copiá-lo diretamente, como no código CURL abaixo:
