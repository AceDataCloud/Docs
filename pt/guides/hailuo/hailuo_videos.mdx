...
Podemos ver que obtivemos informações satisfatórias sobre o vídeo; precisamos apenas acessar o link do vídeo gerado em `data` para obter o vídeo Hailuo.
...
Como o tempo de geração da Hailuo Videos Generation API é relativamente longo, levando cerca de 1-2 minutos, se a API não responder por um longo período, a solicitação HTTP manterá a conexão, resultando em consumo adicional de recursos do sistema. Portanto, esta API também oferece suporte a retorno assíncrono.
