...
Este artigo apresentará uma forma de integração da API de reconhecimento de protocolo hCaptcha, que permite aos usuários evitar a identificação e seleção de imagens de captcha hCaptcha, bastando enviar a Chave do Website para realizar a decodificação automática em segundo plano e completar a verificação.
...
- `proxy`: opcional, traga seu próprio proxy (Bring Your Own Proxy). Após a configuração, o upstream usará o IP do proxy fornecido para decodificar o captcha, controlando a qualidade do IP de saída (por exemplo, evitando que um IP de proxy público seja bloqueado pelo site de destino e retorne `410 Gone`). O formato é `scheme://[user:pass@]host:port`, onde `scheme` suporta `http`/`https`/`socks4`/`socks5`, por exemplo, `http://user:pass@1.2.3.4:8080`. Se não preenchido, será usado o proxy padrão da plataforma.
...
Por padrão, a API é síncrona e bloqueante: uma solicitação aguardará até que o token seja processado antes de retornar. Se você estiver fazendo rotação de múltiplos solucionadores (multi-solver rotation) e quiser "submeter a tarefa e imediatamente obter o task_id, ir agendar outros solucionadores e depois voltar para pegar o resultado", você pode passar `async: true` no corpo da solicitação.
...
Descrição de cobrança: no modo assíncrono, a criação de tarefas e o polling "em processamento" não geram cobrança; **apenas quando o token é obtido com sucesso é cobrado uma vez** (o mesmo preço do modo síncrono). Portanto, cancelar tarefas que ainda não foram concluídas durante a rotação não gerará custos. `/captcha/tasks` é aplicável a todas as interfaces de captcha (token e reconhecimento, como hcaptcha, recaptcha2, recaptcha3, recognition/* etc.) e pode ser usado o mesmo `task_id` para polling.

## Tratamento de Erros

Ao chamar a API, se encontrar um erro, a API retornará o código e a mensagem de erro correspondente. Por exemplo:
...
Por meio deste documento, você já entendeu como usar a API de reconhecimento do protocolo hCaptcha para permitir que os usuários não precisem reconhecer e clicar nas imagens do captcha hCaptcha, bastando enviar a Website Key para realizar a decodificação automática em segundo plano e completar a verificação. Esperamos que este documento possa ajudá-lo a integrar e usar melhor essa API. Se tiver alguma dúvida, entre em contato com nossa equipe de suporte técnico.
