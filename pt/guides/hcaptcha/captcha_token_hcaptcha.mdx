...
Este artigo apresentará uma integração da API de reconhecimento de protocolo hCaptcha, que permite aos usuários evitar a identificação e seleção de imagens de captcha hCaptcha, bastando enviar a Chave do Website para realizar a decodificação automática em segundo plano e completar a verificação.
...
- `rqdata`: opcional. O desafio hCaptcha Enterprise pode fornecer `data-rqdata` na página; ao encontrar esse tipo de desafio, preencha com seu valor original; o hCaptcha comum não precisa ser preenchido.
...
Descrição de cobrança: no modo assíncrono, a criação de tarefas e o polling "em processamento" não são cobrados; **apenas quando o token é obtido com sucesso é cobrado uma vez** (o mesmo preço do modo síncrono). Portanto, cancelar tarefas que ainda não foram concluídas durante a rotação não gerará custos. `/captcha/tasks` é aplicável a todas as interfaces de captcha (token e reconhecimento, como hcaptcha, recaptcha2, recaptcha3, recognition/* etc.) e você pode usar o mesmo `task_id` para polling.
