...
Este artigo irá apresentar uma integração da API de reconhecimento de protocolo hCaptcha, que permite aos usuários evitar a identificação e seleção de imagens de captcha hCaptcha, bastando enviar a Chave do Site para realizar a decodificação automática em segundo plano e completar a verificação.
...
- `token`, o resultado da verificação após o processamento da tarefa do hCaptcha.
- `started_at`, `finished_at`: o horário de início e término do processamento desta solicitação (ISO-8601 UTC).
