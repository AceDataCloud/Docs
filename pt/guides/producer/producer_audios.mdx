---
title: "Documentação de Integração da API de Geração de Áudios do Producer"
...
Para usar a API de Geração de Áudios do Producer, primeiro acesse o [Painel de Controle da Ace Data Cloud](https://platform.acedata.cloud/console/applications) para obter seu Token de API, que deve ser guardado para uso futuro.
...
Para pensar em algumas músicas, você pode inserir qualquer texto, por exemplo, se eu quiser gerar uma música sobre o Natal, posso inserir `a song for Christmas`, como mostrado na imagem:
...
- `action`: a ação da tarefa de geração de música, que é `generate` para gerar uma música.
...
- `lyrics_strength`: intensidade da letra, indicando o grau de influência da letra na geração do áudio, podendo variar de 0 a 1, com padrão de 0.7.
- `sound_strength`: intensidade da palavra-chave de áudio, podendo variar de 0.2 a 1, com padrão de 0.7.
- `cover_strength`: intensidade da reinterpretação, variando de 0.2 a 1, com padrão de 1.
- `weirdness`: grau de peculiaridade do estilo, podendo variar de 0 a 1, com padrão de 0.5.
- `callback_url`: URL para onde os resultados devem ser retornados.
- `async`: opcional, se definido como `true`, a interface retorna imediatamente `task_id`, sem necessidade de fornecer `callback_url`, e os resultados podem ser obtidos através da interface de consulta de tarefas correspondente.
...
- custom：Preencha como `true`, representando geração personalizada, este parâmetro é `false` por padrão, representando geração por `prompt`.
- title：Título da música.

O exemplo preenchido é o seguinte:
...
Se você deseja realizar uma operação de canto em uma música já gerada, pode inserir a música gerada acima para cantar, em seguida, precisamos personalizar a geração da música com base na letra e no título.

Se você deseja continuar cantando uma música que você enviou, pode definir o parâmetro `action` como `upload_cover` e inserir o ID da música que precisa ser continuada, o ID da música pode ser obtido usando a [Producer Upload API](https://platform.acedata.cloud/documents/producer-upload), como mostrado na imagem abaixo:

<p><img src="https://cdn.acedata.cloud/z7r5fd.png" width="500" className="m-auto" /></p>

Você pode especificar o seguinte conteúdo:

- action：A ação da tarefa da música, atualmente suportando: generate, cover, extend, upload_cover, upload_extend, replace_section, swap_vocals, swap_instrumentals, variation, nesta reinterpretação usamos o parâmetro `cover`.
- lyric：Texto da letra
- title：Título da música.
...
Se você quiser continuar a música que enviou, pode definir o parâmetro `action` como `upload_extend` e inserir o ID da música que precisa ser continuada. O ID da música pode ser obtido usando a [API de Upload do Produtor](https://platform.acedata.cloud/documents/producer-upload), conforme mostrado na imagem abaixo:
...
Se você quiser substituir uma seção da música, precisamos passar o valor do parâmetro `action` como: `substituir_seção`, em seguida, devemos personalizar a geração da música com base na letra e no título.

Se você quiser continuar substituindo uma seção da música que você enviou, pode definir o parâmetro `action` como `upload_substituir_seção` e inserir o ID da música que precisa ser continuada, o ID da música pode ser obtido usando a [API de Upload do Produtor](https://platform.acedata.cloud/documents/producer-upload) para obter, como mostrado na imagem abaixo:
...
Se você deseja usar a versão oficial de acompanhamento, pode definir o valor do parâmetro `action` como: `swap_instrumentals`, em seguida, precisamos personalizar a geração da música com base na letra e no título.
...
O teste é permitido, o efeito gerado é semelhante.
...
Se você deseja usar a versão oficial de voz, pode definir o valor do parâmetro `action` como: `swap_vocals`, em seguida, precisamos personalizar a geração da música com base na letra, título e estilo.
...
Se você deseja usar a operação oficial de separação de letras e música, pode definir o valor do parâmetro `action` como: `stems`, em seguida, precisamos realizar a separação de letras e música com base no ID da música.
...
Como o tempo de geração da API Producer Audios pode ser relativamente longo, se a API não responder por um longo período, a solicitação HTTP manterá a conexão, resultando em consumo adicional de recursos do sistema. Portanto, esta API também oferece suporte a callbacks assíncronos.
...
Após um momento, podemos observar o resultado da tarefa gerada em [https://webhook.site/#!/view/0d73431d-f833-4be4-9276-b6e1690d55c1](https://webhook.site/#!/view/0d73431d-f833-4be4-9276-b6e1690d55c1), como mostrado na imagem:
...
Pode-se ver que o resultado contém um campo `task_id`, e os outros campos são semelhantes ao texto anterior, permitindo a associação da tarefa por meio desse campo.
...
Através deste documento, você já entendeu como usar a API de Geração de Áudios do Produtor para gerar música através de palavras-chave de entrada. Esperamos que este documento ajude você a integrar e usar melhor essa API. Se tiver alguma dúvida, entre em contato com nossa equipe de suporte técnico.
