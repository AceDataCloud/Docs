...
- `elapsed`，tempo gasto na execução da tarefa, em segundos (ponto flutuante, com 3 casas decimais). O campo não é retornado se a tarefa não estiver concluída.
...
- `action`：modo de operação na tarefa.
...
O retorno contém vários campos, onde os itens incluem detalhes específicos de tarefas em lote. As informações específicas de cada tarefa são as mesmas do formato de retorno de uma única consulta de tarefa.

- `itens`, todos os detalhes específicos da tarefa em lote. É um array, onde cada elemento do array tem o mesmo formato do resultado de consulta de uma única tarefa.
- `contagem`, o número de tarefas nesta consulta em lote.
...
Através deste documento, você já entendeu como usar a API de Tarefas do Produtor para consultar detalhes específicos de tarefas únicas ou em lote. Esperamos que este documento ajude você a integrar e usar melhor essa API. Se tiver alguma dúvida, entre em contato com nossa equipe de suporte técnico.
