...
- `action`: a ação da tarefa de geração de vídeo, que inclui três ações: vídeo gerado por texto (`text2video`), vídeo gerado por imagem (`image2video`), e vídeo estendido (`extend`).
...
- `aspect_ratio`: proporção do vídeo, opcional, suportando `16:9`, `9:16`, `1:1`, com padrão `16:9`.
...
- `image_url`: link da imagem de referência, não pode estar vazio. Requisitos: formato .jpg/.jpeg/.png; tamanho do arquivo ≤10MB; menor lado ≥300px; proporção largura-altura 1:2.5 ~ 2.5:1.
...
> Nota: Se `start_image_url` / `end_image_url` e `image_list` forem passados ao mesmo tempo, os primeiros / últimos quadros aparecerão antes de `image_list`, o que pode afetar a correspondência dos índices de `&lt;&lt;<image_N>>>`. Recomenda-se escolher um: se precisar de primeiro / último quadro, especifique diretamente em `image_list` usando `type`, não misture com `start_image_url` / `end_image_url`.
