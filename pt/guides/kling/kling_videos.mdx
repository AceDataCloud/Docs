...
Este documento apresentará uma descrição da integração da API de Geração de Vídeos Kling, que pode gerar vídeos oficiais da Kling através da entrada de parâmetros personalizados.

## Processo de Solicitação

Para usar a API de Geração de Vídeos Kling, primeiro acesse o [Console da Ace Data Cloud](https://platform.acedata.cloud/console/applications) para obter seu Token de API, que deve ser guardado para uso futuro.
...
- `action`: a ação da tarefa de geração de vídeo, que inclui três ações: vídeo gerado a partir de texto (`text2video`), vídeo gerado a partir de imagem (`image2video`), e vídeo expandido (`extend`).
...
- `end_image_url`: opcional ao gerar vídeo a partir de imagem, especifica o último quadro.
...
- `generate_audio`: se deve gerar áudio sincronizado, opcional, valor booleano. Suportado por `kling-v3`, `kling-v3-omni` e `kling-v2-6` (apenas no modo pro). O padrão é `false`.
...
Podemos ver que recebemos informações satisfatórias sobre o vídeo, e tudo o que precisamos fazer é acessar o link do vídeo gerado no resultado `data`.
...
`video_list` é usado para passar vídeos de referência, é o cenário mais comum desta capacidade, os campos dos elementos do array são os seguintes:
...
> Nota: quando há um vídeo de referência, `generate_audio` deve ser `false`. O vídeo com `refer_type=base` não pode ter o primeiro quadro / último quadro especificado.
...
-d '{
  ...
  "mode": "std",
  "duration": 5,
  "prompt": "transformar <<<video_1>>> em estilo de anime de nível cinematográfico, mantendo o movimento e a composição originais",
  "video_list": [
    {
...
- `type`: opcional. Se não for passado, será considerado como imagem de referência pura; se passado `first_frame` / `end_frame`, será considerado como primeiro quadro / último quadro (equivalente a `start_image_url` / `end_image_url`).

Ao usar, deve-se referenciar no `prompt` como `&lt;&lt;<image_1>>>`, `&lt;&lt;<image_2>>>`. Limite de quantidade: se não houver vídeo de referência, a soma de imagens de referência + múltiplos elementos deve ser ≤ 7; se houver vídeo de referência, ≤ 4. Ao passar apenas o primeiro / último quadro, também pode-se usar diretamente `start_image_url` / `end_image_url`.

> Nota: se `start_image_url` / `end_image_url` e `image_list` forem passados ao mesmo tempo, o primeiro / último quadro será priorizado antes de `image_list`, o que pode afetar a correspondência dos índices de `&lt;&lt;<image_N>>>`. Recomenda-se escolher um: se precisar do primeiro / último quadro, especifique diretamente em `image_list` usando `type`, não misture com `start_image_url` / `end_image_url`.
...
-d '{
  ...
  "mode": "std",
  "duration": 5,
  "prompt": "fazer com que os personagens de <<<image_1>>> estejam na cena de <<<image_2>>>, iluminação cinematográfica",
  "image_list": [
...
`element_list` é usado para referenciar elementos já carregados na "biblioteca de elementos", o campo do array é `element_id` (ID do elemento na biblioteca, inteiro). Ao usar, também deve-se referenciar no `prompt` como `&lt;&lt;<element_1>>>`. A quantidade de elementos de referência deve ser somada à quantidade de imagens de referência: se houver vídeo de referência, a soma deve ser ≤ 4; se não houver vídeo de referência, ≤ 7.

## Callback Assíncrono

Como a geração de vídeos pela API Kling pode levar um tempo relativamente longo, cerca de 1-2 minutos, se a API não responder por um longo período, a solicitação HTTP manterá a conexão, causando consumo adicional de recursos do sistema. Portanto, esta API também oferece suporte a callbacks assíncronos.
...
Primeiro, o callback Webhook é um serviço que pode receber solicitações HTTP, o desenvolvedor deve substituir pelo URL do servidor HTTP que ele configurou. Para facilitar a demonstração, usaremos um site de exemplo de Webhook público https://webhook.site/, ao abrir este site, você obterá um URL de Webhook, como mostrado na imagem:

![](https://cdn.acedata.cloud/tbcnai.png)

Copie este URL, que pode ser usado como Webhook, o exemplo aqui é `https://webhook.site/624b2c78-6dbd-4618-9d2b-b32eade6d8c3`.

Em seguida, podemos definir o campo `callback_url` para o URL do Webhook acima, ao mesmo tempo que preenchemos os parâmetros correspondentes, o conteúdo específico é mostrado na imagem:
