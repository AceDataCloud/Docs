...
- `image_list`: lista de imagens de referência Omni, aplicável aos modelos `kling-o1` e `kling-v3-omni`, uso descrito abaixo na seção "Referência Omni".
- `video_list`: lista de vídeos de referência Omni (suporta edição de vídeo), aplicável aos modelos `kling-o1` e `kling-v3-omni`, uso descrito abaixo na seção "Referência Omni".
...
Podemos ver que recebemos informações satisfatórias sobre o vídeo, e tudo o que precisamos fazer é acessar o link do vídeo gerado no `data` para obter o vídeo Kling.
...
| `kling-v1-6`          | std              | ❌                    | ❌                    | ❌                    | Vídeo de múltiplas imagens, `extend` disponível em todos os modos                         |
| `kling-v1-6`          | pro              | ✅                    | ❌                    | ❌                    |                                              |
...
| `kling-v3`            | 4k               | ✅                    | ✅                    | ❌                    | Modo 4K incompatível com controle de câmera                                  |
...
- `end_image_url` só pode ser usado com `action=image2video` em conjunto com `start_image_url`. Apenas passar `end_image_url` (sem `start_image_url`) será rejeitado.
...
Em seguida, devemos preencher os próximos prompts necessários para personalizar a geração do vídeo, podendo especificar o seguinte conteúdo:
...
- `start_image_url`: quando a ação de gerar vídeo é `image2video`, é necessário fazer o upload da imagem de referência do quadro inicial.
...
`kling-o1` e `kling-v3-omni` são dois modelos independentes, ambos suportam a capacidade de "referência total". Com base na geração de vídeo a partir de texto (`action=text2video`), é possível passar imagens ou vídeos de referência adicionais, permitindo **referência de múltiplas imagens, vídeo de referência e edição direta de vídeos existentes**.

**Convenção central**: O material de referência deve ser referenciado no `prompt` na forma `&lt;&lt;<image_1>>>`, `&lt;&lt;<video_1>>>` (números começando de 1) para que o modelo aplique essas referências. Se apenas o material for passado sem referência nas palavras-chave, o material será ignorado.

> Aviso de segurança: A API atual não abre `element_list`. O ID da Kling Element Library não é isolado por inquilino; antes de fornecer uma API de gerenciamento de elementos com isolamento de inquilino, use `image_list` para passar a imagem de referência principal.

A solicitação Omni não suporta `negative_prompt`, `cfg_scale` ou `camera_control`, e não pode usar `mode=4k`. Ao incluir vídeo de referência, `generate_audio` deve ser `false`.
