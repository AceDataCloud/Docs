...
Para usar a API OpenAI Images Edits, primeiro acesse o [console Ace Data Cloud](https://platform.acedata.cloud/console/applications) para obter seu Token de API, que deve ser guardado para uso futuro.
...
- **Suporte a URL direta**: além do tradicional upload de arquivos `multipart/form-data`, `gpt-image-2` **também suporta a entrada de URL de imagem em formato JSON**, sem a necessidade de baixar a imagem localmente, sendo ideal para integração em pipelines de servidor.
...
- **`gpt-image-2:official`**: canal oficial, estável e em conformidade. Suporta verdadeiras resoluções 2K / 4K, **com cobrança por imagem, a um preço 2 vezes maior que o padrão `gpt-image-2`**. Se a linha não estiver disponível, retornará um erro diretamente, sem downgrade automático.
...
A validação de formato do `size` na interface de edição é a mesma da interface de geração — `gpt-image-2` aceita `size` como `auto`, vazio ou no formato `LARGURAxALTURA`, qualquer outra forma retornará 400. **Todos os tamanhos (1K / 2K / 4K / personalizado) têm cobrança unificada por imagem, independentemente da resolução da imagem original e do valor solicitado em `size`.**

Limitações de tamanho: tamanhos personalizados devem ter largura e altura como múltiplos de 16, com lado maior ≤ 3840 e total de pixels ≤ 8.294.400; exceder esses limites resultará em retorno 4xx.
...
> Ao passar `auto` (ou omitir `size`), a saída **manterá a proporção da imagem de referência** — no exemplo acima, como a imagem original é 1:1, o resultado também será 1:1, sem ser comprimido em outra proporção. Isso difere da interface de geração: na interface de geração, não há imagem de referência, e `auto` é usado para escolher a proporção com base no significado do prompt. Para alterar a proporção, é necessário especificar `size` explicitamente.
...
payload = {
    ...
    "prompt": "Substitua a estante de madeira por uma prateleira flutuante branca moderna montada em uma parede azul pastel. Mantenha a exata mesma disposição dos livros (1 livro em cima, 3 no meio, 7 embaixo). Adicione uma pequena suculenta em um vaso na prateleira de cima ao lado do livro. Luz do dia brilhante e arejada vindo da esquerda.",
    "size": "1024x1024"
}
```
...
Assim, completamos a operação de edição de imagem, atualmente a interface Edits suporta os modelos `gpt-image-1` e `gpt-image-2`, sendo que `gpt-image-2` é o modelo recomendado para uso, consulte a seção [Modelo GPT-Image-2](#modelo-gpt-image-2) acima.
