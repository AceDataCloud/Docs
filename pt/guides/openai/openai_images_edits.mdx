...
- **Texto mais preciso**: imagens que contêm texto, como infográficos, pôsteres e menus, mantêm a clareza e legibilidade do texto após a edição.
- **Suporte a URL direta**: além do tradicional upload de arquivos `multipart/form-data`, `gpt-image-2` **também suporta a entrada de URL de imagem em formato JSON**, eliminando a necessidade de baixar a imagem localmente, ideal para integração em pipelines de servidor.
...
- **`gpt-image-2:official`**: canal oficial, estável e em conformidade. Suporta verdadeiras resoluções 2K / 4K, **com cobrança por imagem, a um preço 2 vezes maior que o padrão `gpt-image-2`**. Se a linha não estiver disponível, retornará um erro sem rebaixamento automático.
...
> Por exemplo: se a imagem original é `1024x1024`, ao passar `size` como `2048x2048`, o modelo irá redesenhar e gerar uma imagem 2K conforme as instruções de edição; ao passar `size` como `3840x2160`, a saída será uma imagem 4K em modo paisagem; se `auto` for passado ou omitido, o modelo escolherá automaticamente. A cobrança será a mesma para os três casos.
...
Desejamos alterá-la para uma paleta de cores "modo noturno". Podemos fazer a chamada assim:
...
Se você já está usando o SDK oficial OpenAI Python, o método de upload `multipart/form-data` também é aplicável, basta alterar o `model` para `gpt-image-2`:
...
A série `nano-banana` também se conecta ao cenário de edição através de `/openai/images/edits`, basta alterar o `model` para qualquer um dos listados na tabela abaixo.

| Modelo                   | Custo (Créditos / vez) | Cenário aplicável                     |
| -------------------- | --------------- | ------------------------------ |
...
> **Importante: Faixa de suporte a parâmetros**
...
> - Não suporta parâmetros como `mask`, `size`, `response_format`, etc.; se preenchidos, serão ignorados. `n > 1` é suportado (1–10), retornará e cobrará pela quantidade correspondente de resultados editados.
...
curl -s -D >(grep -i x-request-id >&2) \
  ...
  -F "model=gpt-image-1" \
  -F "image[]=@test.png" \
  -F 'prompt=Crie uma linda cesta de presente com estes itens dentro'
```
...
> **Dica**: `image[]` pode aparecer várias vezes para enviar várias imagens de referência, por exemplo, `-F "image[]=@a.png" -F "image[]=@b.png"`, a série de modelos GPT Image suporta no máximo 16 imagens (cada uma não excedendo 50MB, nos formatos png/webp/jpg). Exceder a quantidade retornará 400.
...
prompt = """
Gere uma imagem fotorrealista de uma cesta de presente em um fundo branco 
...
Assim, completamos a operação de edição da imagem, atualmente a interface Edits suporta dois modelos: `gpt-image-1` e `gpt-image-2`, sendo que `gpt-image-2` é o modelo recomendado atualmente, consulte a seção [Modelo GPT-Image-2](#modelo-gpt-image-2) acima.
...
Em seguida, podemos definir o campo `callback_url` para a URL do Webhook acima, ao mesmo tempo preenchendo os parâmetros correspondentes, como mostrado no código a seguir:
```shell
...
Após a chamada, podemos ver que imediatamente obtemos um resultado, como abaixo:
...
Através deste documento, você já entendeu como usar a API OpenAI Images Edits para utilizar facilmente a funcionalidade de edição de imagens da OpenAI. Esperamos que este documento possa ajudá-lo a integrar e usar melhor essa API. Se tiver alguma dúvida, sinta-se à vontade para entrar em contato com nossa equipe de suporte técnico.
