...
O serviço de edição de imagens da OpenAI permite enviar várias imagens e instruções, retornando as imagens modificadas. Atualmente, a API suporta simultaneamente `gpt-image-1`, a mais recente **`gpt-image-2`**, e os modelos da série **`nano-banana` / `nano-banana-2-lite` / `nano-banana-2` / `nano-banana-pro`** acessados pela mesma interface.
...
- **Texto mais preciso**: imagens com texto, como infográficos, pôsteres e menus, mantêm a clareza e legibilidade do texto após a edição.
...
- **`gpt-image-2:official`**: canal oficial, estável e em conformidade. Suporta verdadeiras resoluções de 2K / 4K, **com cobrança por imagem, a um preço 2 vezes maior que o padrão `gpt-image-2`**. Se a linha não estiver disponível, retornará um erro sem rebaixamento automático.
...
> Por exemplo: se a imagem original é `1024x1024`, ao passar `size` como `2048x2048`, o modelo irá reimaginar e retornar a imagem em 2K; ao passar `size` como `3840x2160`, retornará uma imagem 4K em modo paisagem; se passar `auto` ou omitir, o modelo escolherá automaticamente. A cobrança será a mesma para os três casos.

> **Sobre o parâmetro `n`**
> A interface de edição `gpt-image-2` suporta `n > 1`: uma única solicitação pode retornar e cobrar pela quantidade correspondente de resultados de edição (valores de `n` entre 1 e 10). Isso também se aplica a `gpt-image-1` / `gpt-image-1.5`, bem como à série `nano-banana` / `nano-banana-2-lite` / `nano-banana-2` / `nano-banana-pro`. Observe que `response_format=b64_json` suporta apenas `n=1`; para `n>1`, utilize o retorno padrão de URL. Se algumas imagens falharem na geração, apenas as partes bem-sucedidas serão retornadas e cobradas.
...
Assim, completamos a operação de edição da imagem, atualmente a interface Edits suporta dois modelos: `gpt-image-1` e `gpt-image-2`, onde `gpt-image-2` é o modelo recomendado para uso, consulte a seção [Modelo GPT-Image-2](#modelo-gpt-image-2) acima.
