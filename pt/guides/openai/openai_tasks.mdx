...
A API de Tarefas OpenAI é usada para consultar os resultados de tarefas submetidas anteriormente ao interface de imagem da OpenAI em **modo de callback**. Quando você não pode esperar por uma resposta HTTP síncrona ou deseja consultar a tarefa novamente mais tarde, utilize esta interface.

No modo de callback, **a interface de imagem original retornará imediatamente um `task_id` após aceitar o pedido**. Você possui esse `task_id` e pode usá-lo para consultar esta interface quando necessário, sem precisar passar um `trace_id` personalizado (apenas se você desejar associar com um identificador de negócio próprio).

> A tarefa só será persistida se o pedido de imagem original incluir um `callback_url`. Pedidos feitos de forma síncrona (não em modo de callback) não serão armazenados.

## Processo de Solicitação

A API de Tarefas OpenAI compartilha a autorização com os serviços existentes da OpenAI. Se você já solicitou a Geração de Imagens da OpenAI, pode usar o mesmo token para chamar esta interface, sem necessidade de solicitação adicional.
...
| `id`          | string   | Opcional    | ID da tarefa retornado na resposta síncrona ao submeter o pedido de imagem (recomendado) |
| `trace_id`    | string   | Opcional    | Necessário apenas se você passou um `trace_id` personalizado na solicitação original |
...
Quando não há correspondência com nenhuma tarefa, retorna um objeto vazio:
...
- `type`: Tipo da interface upstream. Tarefas escritas na série `gpt-image` (como `gpt-image-2`) são `images`; canais antigos da OpenAI (como `gpt-image-1`, nano-banana) usam `images_generations` / `images_edits`, e algumas interfaces de chat são `chat_completions_image`.
- `request`: Corpo completo da solicitação original.
- `response`: Corpo da resposta final retornada pela interface upstream quando o callback é concluído.
- `created_at` / `finished_at` / `duration`: Timestamp Unix (segundos) e tempo de execução (segundos).
- `application_id` / `user_id` / `credential_id`: ID do aplicativo, usuário final e ID de credencial.
...
| `type`              | string     | Filtra por tipo upstream (valores: `images`, `images_generations`, `images_edits`) |
| `offset`            | int        | Ponto de partida da paginação, padrão `0`                    |
| `limit`             | int        | Número de itens por página, padrão `12`                       |
| `created_at_min`    | float      | Timestamp inicial (segundos Unix)                             |
| `created_at_max`    | float      | Timestamp final (segundos Unix)                               |

É necessário passar pelo menos um dos campos `ids` / `trace_ids` / `application_id` / `user_id` ou um intervalo de tempo `created_at_*`.
  ...
  "items": [
    {
      ...
      "type": "images",
      "request": { "model": "gpt-image-2", "prompt": "Um gato" },
...
# 1. Submeter tarefa de geração de imagem (modo callback: basta incluir callback_url para retornar imediatamente o task_id)
...
# 2. Usar diretamente o task_id da resposta da submissão para fazer polling na interface de Tarefas, até que a tarefa seja concluída
while True:
    task = requests.post(
        f"{API}/openai/tasks",
        headers=HEADERS,
...
- A interface de Tarefas **não gera custos**, você pode fazer polling sem preocupações. Apenas as solicitações de geração/edição de imagem originais serão cobradas.
- Apenas quando a solicitação original contém `callback_url`, é que o registro da tarefa será gravado; chamadas síncronas não gerarão tarefas consultáveis.
