...
> A tarefa só será persistida se o pedido de imagem original incluir um `callback_url`. Pedidos feitos de forma síncrona (não em callback) não serão armazenados.
...
| `trace_id`    | string   | Um dos dois | Necessário apenas se você passou um `trace_id` personalizado na solicitação original |
...
- `type`: Tipo de tarefa. Tarefas escritas na série `gpt-image` (como `gpt-image-2`) são do tipo `images`; `gpt-image-1`, nano-banana, etc., usam `images_generations` / `images_edits`, e algumas interfaces de chat são `chat_completions_image`.
- `request`: Corpo completo da solicitação original.
- `response`: Corpo da resposta final retornada quando o callback é concluído.
- `created_at` / `finished_at` / `duration`: Timestamp Unix (segundos) e duração da execução (segundos).
...
Você pode passar um dos campos `ids` / `trace_ids` / `application_id` / `user_id` ou um intervalo de tempo `created_at_*`.
...
A API de Tarefas serve principalmente para processos assíncronos no modo de callback. No modo de callback, a interface de envio **retorna imediatamente um `task_id`** (ou seja, ID da tarefa), depois você só precisa usar esse `task_id` para fazer polling na interface de Tarefas, sem precisar gerar um `trace_id` por conta própria.
...
# 1. Enviar tarefa de geração de imagem (modo callback: basta incluir callback_url para retornar imediatamente o task_id)
...
# 2. Usar diretamente o task_id da resposta de envio para fazer polling na interface de Tarefas, até que a tarefa seja concluída
while True:
    task = requests.post(
        f"{API}/openai/tasks",
        headers=HEADERS,
...
- Apenas quando a solicitação original contém `callback_url`, é que será registrada a tarefa; chamadas síncronas não gerarão tarefas consultáveis.
