...
- **`gpt-image-2:reverse`**: equivalente ao `gpt-image-2` padrão, com melhor custo-benefício, sem alteração de preço.
...
> A saída na faixa de 1K não garante um alinhamento de pixels rigoroso — você pode passar `1024x1024` e receber `1254x1254`, mantendo a proporção. Se você passar isso novamente como `size`, a cobrança permanece a mesma.
...
payload = {
    "model": "gpt-image-2",
    "prompt": "Uma estante de madeira composta por três prateleiras: Na prateleira de cima, deve haver um livro. Na segunda prateleira, devem haver três livros. Na prateleira de baixo, devem haver sete livros. Iluminação suave e quente, fotorealista, atmosfera de biblioteca aconchegante.",
    "size": "1024x1024"
}
```
...
Pode-se ver que a quantidade de livros nas três prateleiras (1 / 3 / 7) corresponde exatamente ao prompt, algo que era difícil de se conseguir de forma estável na era do `dall-e-3`.
...
A série `nano-banana` é um modelo de geração de imagens baseado no Gemini, que foi integrado através do mesmo endpoint `/openai/images/generations`, sem necessidade de troca de endpoint, basta alterar o `model` para qualquer um dos listados na tabela abaixo.
...
Pode-se ver que `hd` gera imagens com detalhes mais refinados e maior consistência em comparação com `standard`.

## Parâmetro de Tamanho da Imagem `size`

Também podemos definir o tamanho da imagem gerada, podemos fazer as seguintes configurações.
```json
{
  "created": 1721637650,
  "data": [
...
Pode-se ver que o resultado contém um campo `task_id`, e o campo `data` inclui o mesmo resultado de geração de imagem que a chamada síncrona, permitindo que a tarefa seja associada pelo campo `task_id`.
