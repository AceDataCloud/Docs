...
Como mencionado acima, o parâmetro `question` deve ser definido como o correspondente ao hidrante de incêndio `/m/01pns0`, com o conteúdo específico mostrado abaixo:
...
Após a seleção, você verá que o código correspondente também é gerado à direita, como mostrado na imagem abaixo:
...
- `solution`, resultado da verificação após o processamento da imagem do captcha Recaptcha2.
    - `size`, tamanho da imagem do captcha Recaptcha2.
    - `label`, conteúdo reconhecido da imagem do captcha Recaptcha2.
    - `confidences`, confiança nas áreas de reconhecimento da imagem do captcha Recaptcha2, começando do 0.
    - `objects`, áreas que atendem ao conteúdo reconhecido da imagem do captcha Recaptcha2, começando do 0.
    - `type`, tipo da tarefa da imagem do captcha Recaptcha2, sendo `multi` quando há várias áreas.
- `started_at`, `finished_at`: tempos de início e término do processamento desta solicitação, em timestamp Unix (segundos, ponto flutuante).
- `elapsed`: tempo total de processamento (segundos).
...
As áreas começam do 0. A partir do resultado em `objects`, obtemos 2, 5, 7, e precisamos simular cliques nessas três áreas do captcha para passar na verificação.
...
Por padrão, a API é síncrona e bloqueante: uma solicitação aguardará até que o resultado do reconhecimento seja processado antes de retornar. Se você estiver fazendo rotação de múltiplos solucionadores (multi-solver rotation) e desejar "enviar a tarefa e imediatamente obter o task_id, ir agendar outros solucionadores e voltar mais tarde para obter o resultado", você pode passar `async: true` no corpo da solicitação.
...
Quando o processamento estiver concluído, retornará `status: ready` e o resultado do reconhecimento `solution` (a estrutura do campo é idêntica à do modo síncrono):
