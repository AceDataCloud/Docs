...
Abra o f12 e, em seguida, na página Element, pesquise por `.execute(`. Na área destacada em vermelho, podemos ver o parâmetro `action`, e logo após o execute, há uma sequência de caracteres que também é necessária, conforme mostrado na imagem abaixo.

<p><img src="https://cdn.acedata.cloud/imwlto.png" width="500" className="m-auto" /></p>

Em seguida, você precisa inserir a URL do site que precisa processar o captcha para obter o resultado processado. Primeiro, é necessário passar um campo `website_url`, e por fim, você deve inserir o parâmetro `website_key`, que pode ser obtido anteriormente, também é uma sequência de caracteres que aparece após o execute. Agora podemos preencher os campos correspondentes na interface, como mostrado na imagem abaixo:
...
Após a seleção, podemos notar que o código correspondente também foi gerado à direita, como mostrado na imagem abaixo:
...
Os resultados retornados têm vários campos, conforme descrito a seguir:
...
- `elapsed`: o tempo total gasto neste processamento (segundos).

Podemos ver que obtivemos o resultado da verificação do Recaptcha3, que podemos usar para simular o envio para o site-alvo via POST ou GET, para uso único, com validade de 120s, recomendando o uso dentro de 60s. Abaixo, uma breve introdução sobre uma maneira de enviar o token gerado para o site-alvo:

Chamada do código correspondente em Python para verificar o token:
...
Por padrão, a API é síncrona e bloqueante: uma solicitação aguardará até que o token seja processado antes de retornar. Se você estiver fazendo rotação de múltiplos solucionadores (multi-solver rotation) e quiser "submeter a tarefa e imediatamente obter o task_id, agendar outros solucionadores e voltar mais tarde para obter o resultado", você pode passar `async: true` no corpo da solicitação.
...
Quando o processamento estiver concluído, será retornado `status: ready` e o token:
...
Descrição de cobrança: no modo assíncrono, a criação de tarefas e o polling "em processamento" não são cobrados; **apenas quando o resultado é obtido com sucesso é cobrada uma vez** (com o mesmo preço do modo síncrono). Portanto, cancelar tarefas não concluídas durante a rotação não gerará custos. `/captcha/tasks` é aplicável a todas as interfaces de captcha (token e reconhecimento) e pode ser usado o mesmo `task_id` para polling.
