...
Abra o f12, e então na página Element, busque por `.execute(`. Na área do quadro vermelho, podemos ver o parâmetro `action`, e ao lado do execute há uma sequência de caracteres, que também é o conteúdo necessário a seguir, conforme mostrado na imagem abaixo.

<p><img src="https://cdn.acedata.cloud/imwlto.png" width="500" className="m-auto" /></p>

Em seguida, é necessário inserir a URL do site que precisa processar o captcha, para obter o resultado processado. Primeiro, precisamos passar um campo `website_url`, e por fim, precisamos inserir o parâmetro `website_key`, que pode ser obtido acima, também é uma sequência de caracteres que vem após o execute. Agora podemos preencher os conteúdos correspondentes na interface, como mostrado na imagem:
...
Por padrão, a API é síncrona e bloqueante: uma solicitação aguardará até que o processamento do token seja concluído antes de retornar. Se você estiver fazendo rotação de múltiplos solucionadores (multi-solver rotation) e quiser "enviar a tarefa e imediatamente obter o task_id, agendar outros solucionadores e voltar mais tarde para obter o resultado", você pode passar `async: true` no corpo da solicitação.
