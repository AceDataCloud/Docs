...
Abra o f12 e, em seguida, na página Element, pesquise por `.execute(`. Na área destacada em vermelho, podemos ver o parâmetro `action`, e ao lado do execute há uma sequência de caracteres, que também é o conteúdo necessário a seguir, conforme mostrado na imagem abaixo.

<p><img src="https://cdn.acedata.cloud/imwlto.png" width="500" className="m-auto" /></p>

Em seguida, você precisará inserir a URL do site que precisa processar o captcha para obter o resultado processado. Primeiro, é necessário passar um campo `website_url`, e por fim, você também precisará inserir o parâmetro `website_key`, que pode ser obtido acima, também sendo uma sequência de caracteres após o execute. Agora podemos preencher os campos correspondentes na interface, como mostrado na imagem:
...
Após a seleção, podemos notar que o código correspondente foi gerado à direita, como mostrado na imagem:
...
Podemos ver que obtivemos o resultado da verificação do Recaptcha3, que podemos usar para simular o envio para o site-alvo via POST ou GET, sendo de uso único, com validade de 120s, recomendando-se o uso dentro de 60s. Abaixo, uma breve introdução sobre uma maneira de enviar o token gerado para o site-alvo:
...
Por padrão, a API é síncrona e bloqueante: uma solicitação aguardará até que o token seja processado antes de retornar. Se você estiver fazendo rotação de múltiplos solucionadores (multi-solver rotation) e desejar "enviar a tarefa e imediatamente obter o task_id, para depois agendar outros solucionadores e retornar mais tarde para obter o resultado", você pode passar `async: true` no corpo da solicitação.
...
Em seguida, use esse `task_id` para fazer polling em `POST /captcha/tasks` (recomendado a cada 3~5 segundos) para obter o resultado:
...
Descrição de cobrança: no modo assíncrono, a criação de tarefas e o polling "em processamento" não geram cobrança; **apenas quando o resultado for obtido com sucesso será cobrado uma vez** (com o mesmo preço do modo síncrono). Portanto, cancelar tarefas não concluídas durante a rotação não gerará custos. `/captcha/tasks` é aplicável a todas as interfaces de captcha (token e série de reconhecimento), podendo usar o mesmo `task_id` para polling.
