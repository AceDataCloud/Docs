...
Este artigo apresentará uma descrição da integração da API de reconhecimento do protocolo Recaptcha3, que permite aos usuários validar sem a necessidade de reconhecer e clicar nas imagens do captcha do Recaptcha3, bastando enviar a Chave do Site para realizar a decodificação automática em segundo plano e completar a validação.
...
Primeiro, é importante entender a forma básica de uso. Comparado ao Recaptcha2, precisamos passar um parâmetro adicional `page_action`, que deve ser obtido no código. A URL de demonstração da velocidade da rede é: `https://recaptcha-demo.appspot.com/recaptcha-v3-request-scores.php`, e a seguir, apresentamos um método para obtê-lo:

### Método Rápido:

Abra o f12 e, em seguida, na página Element, pesquise por `.execute(`. Na área do quadro vermelho, podemos ver o parâmetro `action`, e ao lado do execute, há uma sequência de caracteres, que também é o conteúdo necessário a seguir, conforme mostrado na imagem abaixo.

<p><img src="https://cdn.acedata.cloud/imwlto.png" width="500" className="m-auto" /></p>

Em seguida, você precisa inserir a URL do site que precisa processar o captcha para obter o resultado processado. Primeiro, é necessário passar um campo `website_url`, e por fim, você deve inserir o parâmetro `website_key`, que pode ser obtido acima, também sendo uma sequência de caracteres após o execute. Agora podemos preencher os conteúdos correspondentes na interface, como mostrado na imagem:
...
Descrição de cobrança: no modo assíncrono, a criação de tarefas e o polling "em processamento" não são cobrados; **apenas quando o resultado é obtido com sucesso é cobrado uma vez** (com o mesmo preço do modo síncrono). Portanto, cancelar tarefas que ainda não foram concluídas durante a rotação não gerará custos. `/captcha/tasks` é aplicável a todas as interfaces de captcha (token e reconhecimento) e você pode usar o mesmo `task_id` para polling.
