...
Este artigo apresentará uma descrição da integração da API de reconhecimento do protocolo Recaptcha2, que permite aos usuários evitar a identificação e seleção de imagens de captcha do Recaptcha2, bastando enviar a Chave do Site para realizar a decodificação automática em segundo plano e completar a verificação.
...
Se você ainda não estiver logado ou registrado, será redirecionado automaticamente para a página de login, convidando-o a se registrar e fazer login; após a conclusão, você será retornado automaticamente à página atual.
...
- Em seguida, clique em enviar, observe as mudanças na rede do console, conforme mostrado na imagem abaixo:
...
Em seguida, ao executar o código, observamos que o console apresentou o seguinte resultado:
...
Em seguida, use esse `task_id` para fazer polling em `POST /captcha/tasks` (recomenda-se a cada 3~5 segundos) para obter o resultado:
...
Através deste documento, você já entendeu como usar a API de reconhecimento do protocolo Recaptcha2 para permitir que os usuários não precisem identificar e clicar nas imagens do captcha Recaptcha2, bastando enviar a Website Key para realizar a decodificação automática em segundo plano e completar a verificação. Esperamos que este documento possa ajudá-lo a integrar e usar melhor essa API. Se tiver alguma dúvida, entre em contato com nossa equipe de suporte técnico.
