...
Este documento irá apresentar uma integração da API de verificação dos três elementos do cartão bancário, que pode ser utilizada para validar a autenticidade e a consistência do número do cartão bancário, nome e número do documento de abertura.

## Processo de Solicitação

Para usar a API de verificação dos três elementos do cartão bancário, primeiro acesse o [console da Ace Data Cloud](https://platform.acedata.cloud/console/applications) para obter seu Token de API, que deve ser guardado para uso futuro.
...
> 📘 Documentação completa: [API de Verificação dos Três Elementos do Cartão Bancário →](https://platform.acedata.cloud/documents/identity-bankcard-check-3e)

## Uso Básico

Primeiro, entenda a forma básica de uso, que consiste em inserir as informações do número do cartão bancário para obter a imagem do resultado processado; para isso, é necessário passar um campo `bank_card`. Em seguida, podemos preencher o conteúdo correspondente na interface, como mostrado na imagem:
...
- `cert_type`: tipo de documento de abertura, deve ser consistente com o documento de abertura, caso contrário, não será possível verificar.
...
- `result`, código do resultado da autenticação, com as seguintes situações de cobrança:
    - Código de resultado de cobrança:
    ...
    - Código de resultado sem cobrança:
...
Através deste documento, você já entendeu como usar a API de verificação dos três elementos do cartão bancário para validar a autenticidade e a consistência do número do cartão bancário, nome e número do documento de abertura. Esperamos que este documento possa ajudá-lo a integrar e usar melhor esta API. Se tiver alguma dúvida, entre em contato com nossa equipe de suporte técnico.
