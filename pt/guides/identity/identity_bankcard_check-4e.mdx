...
Este documento irá apresentar uma integração da API de verificação dos quatro elementos do cartão bancário, que pode ser utilizada para inserir o número do cartão bancário, nome, número do documento de abertura e número de telefone registrado, verificando a veracidade e consistência das informações.

## Processo de Solicitação

Para usar a API de verificação dos quatro elementos do cartão bancário, primeiro acesse o [Painel de Controle da Ace Data Cloud](https://platform.acedata.cloud/console/applications) para obter seu Token de API, que deve ser guardado para uso futuro.
...
> 📘 Documentação Completa: [API de Verificação dos Quatro Elementos do Cartão Bancário →](https://platform.acedata.cloud/documents/identity-bankcard-check-4e)

## Uso Básico

Primeiro, entenda a forma básica de uso, que é inserir as informações do número do cartão bancário para obter a imagem do resultado processado; você precisa simplesmente passar um campo `bank_card`. Em seguida, podemos preencher o conteúdo correspondente na interface, como mostrado na imagem:
...
- `bank_card`: informações do número do cartão bancário a ser processado, é um parâmetro obrigatório.
- `id_card`: número do documento de abertura, é um parâmetro obrigatório.
- `name`: nome do usuário, é um parâmetro obrigatório.
- `phone`: número de telefone registrado.
...
Pode-se ver que o número do cartão bancário, nome, número do documento de abertura e número de telefone registrado deste cartão bancário possuem veracidade e consistência.
...
Através deste documento, você já entendeu como usar a API de verificação dos quatro elementos do cartão bancário para inserir o número do cartão bancário, nome, número do documento de abertura e número de telefone registrado, verificando a veracidade e consistência das informações. Esperamos que este documento possa ajudá-lo a integrar e usar melhor esta API. Se tiver alguma dúvida, entre em contato com nossa equipe de suporte técnico.
