...
Este artigo apresentará uma descrição da integração da API de verificação de três elementos do número de telefone, que pode ser usada para verificar a autenticidade e a consistência do número de telefone, nome e número de identidade.

## Processo de Solicitação

Para usar a API de verificação de três elementos do número de telefone, primeiro acesse o [Painel de Controle da Ace Data Cloud](https://platform.acedata.cloud/console/applications) para obter seu Token de API, que deve ser guardado para uso futuro.
...
> 📘 Documentação Completa: [API de Verificação de Três Elementos do Número de Telefone →](https://platform.acedata.cloud/documents/identity-phone-check-3e)

## Uso Básico

Primeiro, entenda a forma básica de uso, que é inserir o número de telefone para obter o resultado processado. Primeiro, é necessário passar um campo `phone`. Em seguida, podemos preencher o conteúdo correspondente na interface, como mostrado na imagem:
...
- `result`, código do resultado da autenticação, a situação de cobrança é a seguinte.
    ...
    - Códigos de resultado sem cobrança:
        ...
        - -4: Verificação de nome não aprovada
...
Através deste documento, você já entendeu como usar a API de verificação de três elementos do número de telefone para verificar a autenticidade e a consistência do número de telefone, nome e número de identidade. Esperamos que este documento possa ajudá-lo a integrar e usar melhor essa API. Se tiver alguma dúvida, entre em contato com nossa equipe de suporte técnico.
