...
- `size`: especifica as informações de tamanho da imagem gerada, suportando as seguintes duas maneiras, que não podem ser misturadas. Modo 1 | Especifica a resolução da imagem gerada e descreve a proporção largura-altura em linguagem natural no prompt. **As predefinições suportadas variam entre os modelos**: `doubao-seedream-5-0-pro-260628` suporta `1K`/`2K`; `doubao-seedream-5-0-260128` suporta `2K`/`3K`/`4K`; `doubao-seedream-4-5-251128` suporta apenas `2K`/`4K`; `doubao-seedream-4-0-250828` suporta `1K`/`2K`/`4K`. Modo 2 | Especifica os valores de pixel da largura e altura da imagem gerada: padrão `2048x2048`, o total de pixels e a proporção largura-altura variam conforme o modelo (por exemplo, o total de pixels do 5.0 Pro varia de [921600, 4194304], o limite inferior do 5.0 Lite / 4.5 é 3.686.400, e o limite inferior do 4.0 é 921.600).
...
- `stream`: controla se o modo de saída em streaming deve ser ativado. `doubao-seedream-5-0-260128`, `doubao-seedream-4-5-251128`, `doubao-seedream-4-0-250828` suportam esse parâmetro, padrão é `false`.
...
- `tools`: configura os ferramentas que o modelo deve chamar, atualmente suporta `web_search` (pesquisa na web). Apenas `doubao-seedream-5-0-260128` suporta.
...
- image: Carregue a imagem que precisa ser editada, uma ou mais.
...
Você pode ver que o efeito gerado é uma edição da imagem original, o resultado é semelhante ao mencionado acima.

## Callback Assíncrono

Como a geração de imagens da API SeeDream leva um tempo relativamente longo, cerca de 1-2 minutos, se a API não responder por um longo período, a solicitação HTTP manterá a conexão, causando consumo adicional de recursos do sistema. Portanto, esta API também oferece suporte a callbacks assíncronos.
...
Através deste documento, você já entendeu como usar a API SeeDream Images Generation para gerar imagens através da entrada de palavras-chave. Esperamos que este documento ajude você a integrar e usar melhor essa API. Se tiver alguma dúvida, entre em contato com nossa equipe de suporte técnico.
