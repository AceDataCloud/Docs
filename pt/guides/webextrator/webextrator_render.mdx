...
A API de Renderização de Páginas WebExtrator é um serviço de renderização de páginas baseado em Chromium sem cabeça. Dado um URL, retorna o HTML totalmente renderizado (incluindo conteúdo injetado por JS), texto puro, título da página e URL final.
...
| `wait_for_selector` | string    |     ❌      | —                          | Aguarda a aparição do seletor CSS, mais estável que `networkidle`.     |
...
| `headers`           | object    |     ❌      | —                          | Cabeçalhos HTTP adicionais (exemplo: `{"Accept-Language": "en-US"}`). |
| `cookies`           | array     |     ❌      | —                          | Cookies injetados antes da navegação, estrutura conforme abaixo.       |
| `callback_url`      | string    |     ❌      | —                          | Endereço de callback no modo assíncrono, a plataforma `POST` os resultados completos para este endereço ao concluir a tarefa. |
| `bypass_cache`      | boolean   |     ❌      | `false`                    | Ignorar leitura do cache Redis (mas ainda gravará o resultado atual no cache). |
| `cache_ttl_seconds` | number    |     ❌      | `3600`                     | TTL de cache personalizado para esta gravação, enviar `0` significa não armazenar a resposta atual. |
| `async`             | boolean   |     ❌      | `false`                    | Defina como `true` para retornar imediatamente `task_id`, os resultados podem ser recuperados através de `callback_url` ou da API de Tarefas. |
...
```json
{
  ...
  "data": {
    ...
    "title": "Exemplo de Domínio",
    "status": 200,
    "html": "<!DOCTYPE html><html>...</html>",
    "text": "Exemplo de Domínio\nEste domínio é para uso em exemplos ilustrativos...",
