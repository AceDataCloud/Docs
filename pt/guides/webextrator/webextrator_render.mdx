...
| `cache_ttl_seconds` | number   |    ❌      | `3600`                    | TTL de cache personalizado para esta gravação, passar `0` indica que não deve ser armazenado. |
| `async`             | boolean  |    ❌      | `false`                   | Se definido como `true`, retorna imediatamente `task_id`, e o resultado pode ser recuperado através de `callback_url` ou da API de Tarefas. |
...
Quando `async=true` (ou fornecido `callback_url`), retorna imediatamente (HTTP 200):
...
Os resultados serão enviados via `POST` para `callback_url` (se configurado), ou podem ser consultados ativamente através de [`/webextrator/tasks`](development_webextrator_tasks).

### Estrutura de Callback

A plataforma `POST` um envelope **idêntico** ao modo síncrono para `callback_url`, `Content-Type: application/json`. Retornar qualquer `2xx` é considerado confirmado; `5xx` será refeito com um backoff exponencial por cerca de 5 minutos.
