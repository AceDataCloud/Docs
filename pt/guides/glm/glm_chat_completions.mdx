...
- `model`: escolha o modelo GLM a ser chamado, atualmente os modelos suportados incluem:
  - `glm-5.3`: modelo mais recente e de ponta, suporta 1M de contexto e até 128K de saída, adequado para tarefas complexas de raciocínio, código e agentes. O raciocínio está sempre ativado, podendo ser selecionado através de `reasoning_effort` como `low`, `high` ou `max`.
...
- `stream`: se deve ativar a resposta em fluxo, padrão `false`.
...
Abaixo estão as explicações dos principais campos do resultado retornado:
...
- `usage`: estatísticas de uso de tokens da solicitação atual, incluindo `prompt_tokens`, `completion_tokens`, `total_tokens`.

## Resposta em Fluxo

Essa interface suporta resposta em fluxo (Server-Sent Events), o que é muito útil para integração com páginas da web, permitindo que a página exiba o efeito de exibição letra por letra.
...
data: {"id": "msg_2026042622521271f765bbc3734ce1", "object": "chat.completion.chunk", "created": 1777215133, "model": "glm-4.7", "choices": [{"delta": {"content": "Olá! O que posso"}, "finish_reason": null, "index": 0}], "usage": null}
