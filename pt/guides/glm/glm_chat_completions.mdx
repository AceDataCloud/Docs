...
Para usar a API GLM Chat Completion, primeiro acesse o [console Ace Data Cloud](https://platform.acedata.cloud/console/applications) para obter seu Token de API, que deve ser guardado para uso futuro.
...
Na primeira vez que usar esta interface, precisamos preencher pelo menos três conteúdos:
...
- `model`: escolha o modelo GLM a ser chamado, atualmente os modelos suportados incluem:
  - `glm-5.1`: modelo flagship da nova geração, com a maior capacidade geral.
...
O efeito de saída é o seguinte (trecho):
```text
...
Pode-se ver que as informações contidas em `choices` são consistentes com o uso básico, o modelo fornece uma resposta com base no histórico completo da conversa, permitindo a interação contextual em várias rodadas.
...
| --------------- | ----------------------------------------- |
| `glm-5.1`      | A mais poderosa em termos de capacidade, recomendada para raciocínio complexo e análise de documentos longos |
| `glm-4.7`      | Tarefas de chamada de ferramentas, geração de código, orquestração de agentes, etc. |
| `glm-4.6`      | Uma escolha equilibrada para diálogos gerais e criação de conteúdo |
| `glm-3-turbo`  | Tarefas gerais de geração de texto, em cenários sensíveis a custos |
...
Quando retornar `api_error` e a mensagem for `O serviço está temporariamente indisponível, por favor, tente novamente mais tarde.`, geralmente indica que o serviço GLM upstream está temporariamente indisponível, recomenda-se tentar novamente com um backoff exponencial ou mudar para outro modelo GLM disponível (por exemplo, mudar temporariamente de `glm-5.1` para `glm-4.7` ou `glm-4.6`).

## Conclusão

Através deste documento, você já entendeu como usar a API de Conclusão de Chat GLM para chamar os modelos da série GLM da Zhizhu AI, incluindo chamadas básicas, respostas em fluxo, diálogos de múltiplas rodadas, prompts de sistema e chamadas de ferramentas, entre outros usos típicos. Esperamos que este documento possa ajudá-lo a integrar e usar melhor essa API. Se tiver alguma dúvida, entre em contato com nossa equipe de suporte técnico.
