...
> Para criar timbres, consulte a [Fish Model Create API](https://platform.acedata.cloud/documents/fish-model). Para consultar detalhes de um timbre específico pelo `_id`, consulte a [Fish Model Get API](https://platform.acedata.cloud/documents/fish-model-get).
...
O `_id` retornado pode ser usado como valor do campo `reference_id` na [Fish TTS API](https://platform.acedata.cloud/documents/fish-tts) para síntese de voz utilizando o timbre clonado.
