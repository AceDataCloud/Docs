...
Para usar a API, é necessário solicitar o serviço correspondente na página [Fish TTS API](https://platform.acedata.cloud/services/fish). Ao acessar a página, clique no botão "Acquire".
...
Também é possível usar a [Fish Tasks API](https://platform.acedata.cloud/documents/fish-tasks) para consultar o status da tarefa via polling pelo `task_id`.
