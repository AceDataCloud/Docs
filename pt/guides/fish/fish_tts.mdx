...
| `references`   | object[]             | Não         | Amostras de referência inline, estrutura igual ao upstream, cada item contém `audio` e `text`. Um dos dois, `reference_id` ou `references`, deve ser fornecido.               |
| `sample_rate`  | integer              | Não         | Taxa de amostragem, comumente `16000`, `22050`, `44100`. `format=mp3` padrão é 44100.                                                                                       |
| `mp3_bitrate`  | integer              | Não         | Taxa de bits MP3, pode ser `64`, `128`, `192`. Apenas efetivo se `format=mp3`.                                                                                              |
...
`audio_url` aponta para o CDN da plataforma, podendo ser baixado diretamente via GET ou reproduzido em `<audio>`. O link é de longa duração, mas ainda é recomendado manter uma cópia em seu próprio armazenamento.

## Exemplo 2: Usando Clone de Voz `reference_id`

Abaixo, um exemplo com uma voz em espanhol pública na plataforma Fish (`_id` pode ser recuperado através da [Consulta de Modelos Fish](https://platform.acedata.cloud/documents/fish-model-query)):
...
Por meio do cabeçalho HTTP `model: s1`, mude para o modelo estável, adicionando `mp3_bitrate: 128` no corpo da solicitação:
```shell
curl -X POST 'https://api.acedata.cloud/fish/tts' \
  -H 'authorization: Bearer {token}' \
  -H 'content-type: application/json' \
...
- `400 token_mismatched`: Parâmetros de solicitação ausentes ou inválidos (o mais comum é `text` estar vazio ou `format` ter um valor diferente de `mp3`/`wav`/`pcm`).
- `401 invalid_token`: Token de autenticação não existe ou é inválido.
- `429 too_many_requests`: Limite de taxa da conta foi acionado.
...
O custo mínimo para integrar o Fish TTS é: substituir a autenticação pelo token da plataforma no código que já chama `api.fish.audio/v1/tts` e incluir explicitamente `format: "mp3"` no corpo do pedido. Para cenários de texto longo, recomenda-se usar o callback assíncrono `callback_url`; para descobrir o `reference_id` da clonagem de timbre, utilize a [Consulta de Modelos Fish](https://platform.acedata.cloud/documents/fish-model-query) e [Obter Modelo Fish](https://platform.acedata.cloud/documents/fish-model-get).
