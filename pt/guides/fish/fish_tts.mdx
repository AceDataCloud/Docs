...
Para usar a API Fish TTS, primeiro acesse o [console da Ace Data Cloud](https://platform.acedata.cloud/console/applications) para obter seu Token de API, que deve ser guardado para uso futuro.
...
| `chunk_length`    | integer              | Não         | Comprimento do fragmento do upstream, padrão decidido pelo upstream.                                                                                                          |
...
| `latency`         | string               | Não         | `normal` ou `balanced`, padrão é automaticamente preenchido como `normal` (enviar string vazia será rejeitado pelo upstream).                                                  |
...
`audio_url` aponta para o CDN desta plataforma, podendo ser baixado diretamente via GET ou reproduzido em `<audio>`. A resposta final bem-sucedida também retornará o `cost` de nível superior, onde `amount` é o total de Créditos descontados nesta operação; se houver desconto na conta, `list_amount` indica o valor antes do desconto. O link é de uso prolongado, mas ainda é recomendado manter uma cópia em seu próprio armazenamento.

## Exemplo 2: Usando a Voz Clonada `reference_id`

Abaixo, um exemplo com uma voz pública em espanhol na plataforma Fish (`_id` pode ser recuperado através da [Consulta de Modelos Fish](https://platform.acedata.cloud/documents/fish-model-query)):
...
curl -X POST 'https://api.acedata.cloud/fish/tts' \
  ...
  -H 'content-type: application/json' \
  -d '{
    "text": "Hoje o tempo está ótimo, vamos sair para uma caminhada.",
    "format": "mp3",
...
Você também pode usar a [API de Tarefas Fish](https://platform.acedata.cloud/documents/fish-tasks) para puxar resultados ativamente pelo `task_id`; o `response.cost` do estado final é igual ao `cost` no callback, veja o documento para mais detalhes.
...
O custo mínimo para integrar o Fish TTS é: substituir a autenticação pelo token da plataforma no código que já chama `api.fish.audio/v1/tts` e incluir explicitamente no corpo do pedido `format: "mp3"`. Para cenários de texto longo, recomenda-se usar o callback assíncrono `callback_url`; para descobrir o `reference_id` da clonagem de timbre, utilize a [Consulta de Modelos Fish](https://platform.acedata.cloud/documents/fish-model-query) e [Obter Modelo Fish](https://platform.acedata.cloud/documents/fish-model-get).
