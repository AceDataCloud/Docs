...
- `model`: o modelo para gerar o vídeo, que inclui `veo31-fast`, `veo3`, `veo31`, `veo3-fast` e `veo31-fast-ingredients`.
...
- `image_urls`: quando a ação de vídeo gerado por imagem `image2video` é escolhida, é necessário enviar os links das imagens de referência. O modelo `veo31-fast-ingredients` aceita até 3 imagens (fusão de múltiplas imagens), enquanto os outros modelos aceitam até 2 imagens (modo de quadro inicial e final).
- `resolution`: escolha a resolução do vídeo gerado, onde o modelo veo31 suporta resolução 4k, enquanto os outros modelos não suportam. Todos os modelos suportam 1080p e resolução gif; se esse valor não for passado, a resolução padrão será 720p, dividindo-se em: `1080p`, `gif`, `4k`.
...
| **veo31-fast-ingredients** | ❌ Vídeo gerado por texto (não suportado)<br />✅ **Fusão de múltiplas imagens obrigatória** (imagens devem ser enviadas) | **1-3 imagens** → Modo de fusão de múltiplas imagens (máximo de 3 imagens)        |
...
2. **Tipos de Modo de Vídeo Gerado por Imagem**:
   ...
   - **Modo de Quadro Inicial e Final** (2 imagens): o quadro inicial e final são fixos nas imagens de entrada.
...
- **Modelo que obrigatoriamente requer imagens**: `veo31-fast-ingredients` deve receber imagens (1-3 imagens), caso contrário, não funcionará.
...
O resultado retornado contém vários campos, que são descritos a seguir:
...
- `model`：o modelo de vídeo gerado, que pode ser `veo31-fast`、`veo3`、`veo31`、`veo3-fast` e `veo31-fast-ingredients`.
...
Se você quiser gerar um vídeo Veo com um tamanho personalizado, pode definir o parâmetro `aspect_ratio` como o tamanho desejado. Em seguida, precisamos preencher as palavras-chave que precisamos expandir para personalizar a geração do vídeo, podendo especificar o seguinte conteúdo:

- `model`：o modelo de vídeo gerado, que pode ser `veo31-fast`、`veo3`、`veo31`、`veo3-fast` e `veo31-fast-ingredients`.
- `aspect_ratio`：o tamanho do vídeo, atualmente suportando: `16:9`、`16:9`、`3:4`、`4:3`、`1:1`，o padrão é `16:9`.
