...
- Google: `gemini-3.1-pro-preview`, `gemini-3.1-flash-image`, `gemini-2.5-flash-lite`, etc.
- xAI: `grok-4`, etc.
...
Para um controle mais refinado (como colocar várias imagens entre textos, ou se a ordem for muito importante), use diretamente o array `message`.
...
A pré-autorização é apenas essas duas listas: uma lista vazia significa que nenhuma capacidade é autorizada, sem necessidade de campos de chave adicionais.
...
2. Se você estava usando nomes de modelos v1 (como `gpt-3.5`, `gpt-4-browsing`, etc.), ao mudar para v2, recomenda-se atualizar para modelos contemporâneos (como `gpt-5.4`, `claude-opus-4-8`, `gemini-3.1-pro-preview`, etc.).
...
- `429 too_many_requests`: atingiu o limite de taxa.
...
A API AI Chat v2, mantendo a compatibilidade com o v1, atualiza a conversa de "perguntas e respostas de uma única rodada / múltiplas rodadas" para "conversas observáveis em formato de agente": entrada multimodal, chamadas de ferramentas, pausáveis / recuperáveis, eventos estruturados em fluxo, CRUD embutido. Recomenda-se que novas integrações usem diretamente o v2; integrações existentes do v1 podem ser migradas suavemente em fases. Se houver qualquer dúvida, entre em contato com nossa equipe de suporte técnico.
