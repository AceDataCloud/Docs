...
AI Chat v2 API (`/aichat2/conversations`) é a nova geração da interface de diálogo, uma versão totalmente aprimorada da [AI Chat API](https://platform.acedata.cloud/documents/aichat-conversations). Ela expande a v1, que era simples e hospedava diálogos múltiplos, com as seguintes melhorias:

- **Entrada de usuário multimodal**: permite enviar texto + imagem + bloco de arquivo diretamente pelo campo estruturado `message`, sem a necessidade de anexar indiretamente com `references`.
...
Além disso, a API é **totalmente compatível com a v1** no nível do corpo da solicitação: basta enviar `model` + `question` (+ opcionalmente `stateful` / `id` / `references` / `preset`) para obter uma resposta JSON `{answer, id}` equivalente à v1, portanto, a migração de `/aichat/conversations` não requer reescrita do cliente, apenas a troca do caminho para `/aichat2/conversations`.
...
> `stateful` é `true` por padrão, omitir e passar `true` é equivalente. Se você não deseja que o servidor salve esta rodada de conversa, pode definir explicitamente `stateful: false`.

## Resposta em fluxo

v2 suporta dois formatos de fluxo, escolhidos de acordo com o cabeçalho `accept`:

| Cenário                    | `accept`               | Forma de dados                                       |
| --------------------- | ---------------------- | ------------------------------------------ |
...
O lado do navegador usando `EventSource` não suporta corpo de requisição personalizado, recomenda-se usar `fetch` + divisão manual por `\n\n`:
...
| `citation`          | Usado para complementar a fonte da citação do texto correspondente.                                                                            |
...
- O usuário pergunta "Ajude-me a procurar quais novas exposições estão acontecendo em Xangai" → o modelo chama a pesquisa na web embutida → organiza os resultados em uma resposta.
...
O `tool_use_id` no corpo da solicitação **deve** ser exatamente o mesmo que o `tool_id` no momento da pausa; se não for, retornará 400. Quando a solicitação contém `tool_results`, `question` / `message` / `references` serão ignorados.
...
Se você já está usando [`/aichat/conversations`](https://platform.acedata.cloud/documents/aichat-conversations), migrar para o v2 quase não requer alteração de código:
...
As respostas de erro são uniformes:
...
- `400 bad_request`: campos obrigatórios ausentes, `tool_use_id` não corresponde, schema de `messages` inválido, etc.
