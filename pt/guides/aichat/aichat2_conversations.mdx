...
A API AI Chat v2 (`/aichat2/conversations`) é a nova geração da interface de conversação, uma versão totalmente aprimorada da [AI Chat API](https://platform.acedata.cloud/documents/aichat-conversations). Baseando-se na simplicidade e no suporte a diálogos multi-turno da v1, ela expande com:
...
Para usar a API, acesse a página correspondente da [AI Chat v2 API](https://platform.acedata.cloud/documents/aichat2-conversations) e clique no botão "Acquire" para obter as credenciais necessárias para requisições.
...
Se você já usa [`/aichat/conversations`](https://platform.acedata.cloud/documents/aichat-conversations), migrar para a v2 quase não requer mudanças:
