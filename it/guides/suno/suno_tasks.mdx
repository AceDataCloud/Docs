...
Per utilizzare l'API Suno Audios Generation, prima vai al [Pannello di controllo di Ace Data Cloud](https://platform.acedata.cloud/console/applications) per ottenere il tuo API Token, da conservare per uso futuro.
...
- `finished_at`，任务完成时间，Unix 时间戳（秒，浮t）。任务未完成时不返回该字段。
- `elapsed`，任务执行耗时，单位为秒（浮点，保留 3 位小数）。任务未完成时不返回该字段。

## Operazione di query batch

Questo è per interrogare i dettagli delle canzoni per più ID di attività, a differenza di quanto sopra, è necessario selezionare l'azione come retrieve_batch.

**Request Body** include:

- `ids`：array di ID delle attività caricate.
- `action`：modo di operare sulle attività.

Impostare come mostrato nell'immagine qui sotto:

<p><img src="https://cdn.acedata.cloud/dqeknz.png" width="500" className="m-auto" /></p>

### Esempio di codice

Si può notare che, sul lato destro, sono già stati generati automaticamente vari codici, come mostrato nell'immagine:
...
Dopo una richiesta riuscita, l'API restituirà i dettagli specifici di tutte le attività di canzoni batch. Ad esempio:
...
Il risultato restituisce diversi campi, dove `items` contiene i dettagli specifici delle attività musicali in batch, ogni informazione specifica dell'attività musicale è simile ai campi del risultato della query di un singolo compito.

- `items`, tutti i dettagli specifici delle attività musicali in batch. È un array, ogni elemento dell'array ha lo stesso formato del risultato della query di un singolo compito.
- `count`, il numero di attività musicali in questa query batch.
...
Attraverso questo documento, hai compreso come utilizzare l'API Suno Tasks per interrogare i dettagli specifici delle attività musicali singole o in batch. Speriamo che questo documento possa aiutarti a integrare e utilizzare meglio questa API. Se hai domande, non esitare a contattare il nostro team di supporto tecnico.
