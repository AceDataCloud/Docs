---
title: "Suno MIDI Generation API Integrazione"
description: "Suno Music Generation API guide - Ace Data Cloud"
---

SUNO ci consente di effettuare una seconda creazione sulla musica generata, ottenendo il MIDI della musica. Questo documento spiega come integrare le API correlate.

Il parametro di input principale di questa API è `audio_id`, che è l'ID della canzone generata ufficialmente, generalmente è l'ID della canzone con tracce separate (corrispondente al parametro `action` in [generazione canzone](https://platform.acedata.cloud/documents/suno-audios) impostato su `all_stems`). È facoltativo anche supportare `callback_url` come indirizzo di callback asincrono.

Qui l'`audio_id` che inseriamo è `c65e6ffd-ead3-4926-9c8c-a42ce202946b`.
...
Si può vedere che i campi in `data` sono le informazioni MIDI della canzone dopo la separazione dell'intera traccia.
