...
Apri f12, quindi cerca nella pagina Elementi `.execute(`; nell'area evidenziata in rosso possiamo vedere il parametro `action`, mentre dopo execute c'è una stringa, che è anche il contenuto necessario in seguito, come mostrato nell'immagine sottostante.
...
Possiamo vedere che abbiamo ottenuto il risultato della verifica del Recaptcha3, che possiamo utilizzare per simulare una submission POST o GET al sito target, utilizzabile una sola volta, con una validità di 120 secondi, si consiglia di utilizzarlo entro 60 secondi. Di seguito verrà brevemente descritta una modalità per inviare il token generato al sito target:
