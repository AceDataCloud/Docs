...
- `proxy`: opzionale, porta il tuo proxy (Bring Your Own Proxy). Una volta impostato, il servizio utilizzerà l'IP del proxy fornito per decifrare il captcha, utile per controllare la qualità dell'IP di uscita (ad esempio, per evitare che l'IP del proxy pubblico venga bloccato dal sito di destinazione e restituisca `410 Gone`). Il formato è `scheme://[user:pass@]host:port`, dove `scheme` supporta `http`/`https`/`socks4`/`socks5`, ad esempio `http://user:pass@1.2.3.4:8080`. Se non specificato, verrà utilizzato il proxy predefinito della piattaforma.
...
Se desideri controllare attivamente i progressi, puoi utilizzare il `task_id` per interrogare `POST /captcha/tasks` (si consiglia ogni 3~5 secondi). Questa interfaccia non attiverà o accelererà l'elaborazione del compito; anche se non interroghi, perdi la connessione o esci dal client, il server continuerà a elaborare:
...
Informazioni di fatturazione: in modalità asincrona, la creazione di un compito e la lettura dello stato "in elaborazione" non comportano costi; **il cliente verrà addebitato una volta al primo successo della lettura dei risultati** (coerente con il comportamento attuale e il prezzo della modalità sincrona). Il server gestirà autonomamente l'avanzamento del compito, ma non addebiterà in anticipo se il backend completa prima. Se il compito non ha successo entro 120 secondi, verrà terminato con un HTTP 504 `timeout`, senza costi. `/captcha/tasks` non è responsabile dell'avanzamento del compito.
