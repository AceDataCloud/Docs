...
- `proxy`: opzionale, porta il tuo proxy (Bring Your Own Proxy). Una volta impostato, il server upstream utilizzerà l'IP del proxy fornito per decifrare il captcha, utile per controllare la qualità dell'IP di uscita (ad esempio, per evitare che l'IP del proxy pubblico venga bloccato dal sito di destinazione e restituisca `410 Gone`). Il formato è `scheme://[user:pass@]host:port`, dove `scheme` supporta `http`/`https`/`socks4`/`socks5`, ad esempio `http://user:pass@1.2.3.4:8080`. Se non specificato, verrà utilizzato il proxy predefinito della piattaforma.
