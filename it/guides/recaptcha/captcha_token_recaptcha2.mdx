...
- `proxy`: opzionale, porta il tuo proxy (Bring Your Own Proxy). Dopo l'impostazione, il server upstream utilizzerà l'IP del proxy fornito per decifrare il captcha, per controllare la qualità dell'IP di uscita (ad esempio, per evitare che l'IP del proxy pubblico venga bloccato dal sito di destinazione e restituisca `410 Gone`). Il formato è `scheme://[user:pass@]host:port`, dove `scheme` supporta `http`/`https`/`socks4`/`socks5`, ad esempio `http://user:pass@1.2.3.4:8080`. Se non specificato, verrà utilizzato il proxy predefinito della piattaforma.
...
I risultati della richiesta contengono più campi, descritti come segue:
...
Possiamo vedere che abbiamo ottenuto il risultato della verifica del Recaptcha2, che possiamo utilizzare per POST o simulare l'invio al sito target, utilizzabile una sola volta, con una validità di 120 secondi, si consiglia di utilizzarlo entro 60 secondi. Di seguito verrà fornito un segmento di codice Python per inviare il token elaborato al sito target per superare il Recaptcha2.

Prima di tutto, dobbiamo capire come il sito invia la richiesta POST, in modo da poter inserire il token generato. Dobbiamo prima aprire la console F12 e poi completare manualmente la verifica. Alla fine, possiamo vedere che il sito ha inviato una richiesta POST; dobbiamo solo esaminare la costruzione di questa richiesta POST. Il processo specifico è il seguente:
