...
| **veo31-fast-ingredients** | ❌ Generazione video da testo (non supportata)<br />✅ **Fusione di più immagini obbligatoria** (deve essere fornita un'immagine) | **1-3 immagini** → modalità fusione di più immagini (fino a 3 immagini)        |
...
Cliccando sul pulsante "Prova" puoi effettuare un test, come mostrato nell'immagine sopra, e qui abbiamo ottenuto il seguente risultato:
...
Possiamo vedere che il contenuto del risultato è coerente con quanto sopra, il che realizza la funzione di generazione video da immagini.
...
> Nota: l'ID del video qui è l'ID del video generato. Se non sai come generare un video, puoi fare riferimento all'uso di base sopra per generare un video.
...
Possiamo vedere che il contenuto del risultato è coerente con quanto sopra, il che realizza la funzione di ottenere video in 1080p.
