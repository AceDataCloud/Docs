...
Per utilizzare l'API di traduzione Localization, prima vai al [Pannello di controllo di Ace Data Cloud](https://platform.acedata.cloud/console/applications) per ottenere il tuo token API, da conservare per uso futuro.
...
**Un token API è sufficiente per accedere a tutti i servizi della piattaforma, senza dover richiedere separatamente per ogni servizio.** La prima richiesta ti darà un credito gratuito, per un'esperienza gratuita; quando il credito è insufficiente, puoi ricaricare il saldo generale nel [pannello di controllo](https://platform.acedata.cloud/console/coin).

> 📘 Documentazione completa: [API di traduzione Localization →](https://platform.acedata.cloud/documents/localization-translate)

## Esempio di richiesta

Prendiamo un esempio di input in inglese per dimostrare come utilizzare questa API. Supponiamo che la descrizione in inglese sia: `# Title 1\n\nThis is a paragraph.\n\n## Title 2\n\nThis is another paragraph.`, e ora dimostreremo come caricare la descrizione in inglese e ottenere il risultato della traduzione in cinese, presentandolo in formato markdown.

### Impostazione dell'intestazione e del corpo della richiesta
...
- `input`: il contenuto da tradurre. Quando `extension=md`, si invia una stringa; quando `extension=json`, si invia un oggetto JSON.
- `locale`: la lingua di traduzione personalizzata.
- `extension`: il formato del contenuto di input, può essere `md` o `json`.
- `model`: il grande modello utilizzato per la traduzione, il predefinito è gpt-3.5.

| `extension` | Tipo di `input` | Esempio                                   |
| ----------- | --------------- | ----------------------------------------- |
| `md`        | string          | `"# Title\n\nParagraph"`                 |
| `json`      | object          | `{"title": "Hello", "button": "Submit"}` |

`extension` deve corrispondere al tipo di `input`; le stringhe non possono utilizzare `json`, gli oggetti JSON non possono utilizzare `md`.
...
Si può notare che nel risultato c'è un campo `data`, che contiene il testo tradotto in cinese, restituendolo anche in formato markdown; altre informazioni sono le seguenti:

- `data`, il risultato di questo compito di traduzione.
- `model`, il grande modello di linguaggio utilizzato per questo compito di traduzione.
