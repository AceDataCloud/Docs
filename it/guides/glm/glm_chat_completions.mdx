...
- `model`: scegli il modello GLM da chiamare, i modelli attualmente supportati includono:
  - `glm-5.1`: il modello di punta della nuova generazione, con la massima capacità complessiva.
  - `glm-4.7`: eccellente in inferenza, chiamate a strumenti e compiti di codice.
  - `glm-4.6`: modello di dialogo generico, bilancia efficacia e costo.
...
Questa interfaccia supporta la risposta in streaming (Server-Sent Events), che è molto utile per l'integrazione con pagine web, consentendo di visualizzare il testo parola per parola.
...
```
| Modello         | Scenari di applicazione                     |
| -------------- | ------------------------------------- |
| `glm-5.1`     | La capacità complessiva più forte, raccomandato per ragionamenti complessi, analisi di documenti lunghi |
| `glm-4.7`     | Chiamate a strumenti, generazione di codice, orchestrazione di agenti e altri compiti |
...
Attraverso questo documento, hai appreso come utilizzare l'API di completamento chat GLM per chiamare i modelli della serie GLM di Zhiyu AI, inclusi utilizzi tipici come chiamate di base, risposte in streaming, dialoghi multipli, suggerimenti di sistema e chiamate a strumenti. Speriamo che questo documento possa aiutarti a integrare e utilizzare meglio questa API. Se hai domande, non esitare a contattare il nostro team di supporto tecnico.
