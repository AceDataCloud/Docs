...
- `model`: scegli il modello GLM da chiamare, i modelli attualmente supportati includono:
  - `glm-5.2`: il modello di punta più recente, adatto per inferenze complesse, codice e compiti di Agent.
...
Si può vedere che ci sono molti `data` nella risposta, ogni `data` contiene un frammento incrementale. `choices[i].delta.content` è il frammento di testo aggiunto nel chunk corrente, puoi concatenare questi frammenti per formare una risposta completa. Quando il contenuto di `data` è `[DONE]`, indica che la risposta in streaming è terminata. L'ultima chunk con `usage` riassumerà l'uso dei token per questa richiesta.
...
| -------------- | ------------------------------------- |
| `glm-5.2`      | Ultimo flagship, raccomandato per inferenze complesse, codice e compiti di Agent |
| `glm-5.1`      | Flagship della generazione precedente, adatto per inferenze complesse, analisi di documenti lunghi |
...
Attraverso questo documento, avete appreso come utilizzare l'API di completamento chat GLM per chiamare i modelli della serie GLM di Zhizhu AI, inclusi utilizzi tipici come chiamate di base, risposte in streaming, conversazioni multi-turno, prompt di sistema e chiamate a strumenti. Speriamo che questo documento possa aiutarvi a integrare e utilizzare meglio questa API. Se avete domande, non esitate a contattare il nostro team di supporto tecnico.
