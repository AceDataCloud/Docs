...
Dopo di che, dobbiamo convertire l'immagine del CAPTCHA in un'immagine codificata in Base64. Si consiglia di utilizzare l'estensione per Google Chrome FeHelper per la conversione; per il metodo specifico, puoi fare riferimento all'immagine sottostante:
...
Possiamo vedere che abbiamo ottenuto il risultato della verifica dell'immagine del CAPTCHA alfanumerico di lunghezza variabile; dobbiamo solo fare riferimento al contenuto testuale nel campo `text` per superare la verifica.
