...
Per impostazione predefinita, l'API è sincrona e bloccante: una richiesta attenderà fino a quando il risultato del riconoscimento non sarà completato. Se stai effettuando un cambio di risolutori multipli (multi-solver rotation) e desideri "inviare il compito e ricevere immediatamente il task_id, per poi programmare altri risolutori e tornare in seguito a leggere il risultato", puoi passare `async: true` nel corpo della richiesta.
...
Informazioni di fatturazione: nella modalità asincrona, la creazione di un compito e la lettura dello stato "in elaborazione" non comportano costi; **il cliente verrà addebitato una volta al primo successo nella lettura del risultato** (coerente con il comportamento attuale e il prezzo della modalità sincrona). Il server gestirà autonomamente l'avanzamento del compito, ma non addebiterà in anticipo se il backend completa prima. Se il compito non viene completato entro 120 secondi, verrà terminato con un HTTP 504 `timeout`, senza costi. `/captcha/tasks` non è responsabile dell'avanzamento del compito.
...
Attraverso questo documento, hai appreso come utilizzare l'API di riconoscimento dei codici di verifica numerici in inglese, che può essere utilizzata per riconoscere codici di verifica numerici in inglese di lunghezza variabile. Inserisci il contenuto dell'immagine del codice di verifica e ottieni il risultato del codice di verifica. Speriamo che questo documento possa aiutarti a integrare e utilizzare meglio questa API. Se hai domande, non esitare a contattare il nostro team di supporto tecnico.
