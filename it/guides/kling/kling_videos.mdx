...
- `element_list`: elenco di riferimenti ai soggetti, applicabile solo al modello `kling-video-o1`, per citare gli ID degli elementi nella libreria dei soggetti, come descritto nella sezione "Riferimento Omni".
- `image_list`: elenco di immagini di riferimento (riferimento multiplo), applicabile solo al modello `kling-video-o1`, come descritto nella sezione "Riferimento Omni".
- `video_list`: elenco di video di riferimento (supporta l'editing video), applicabile solo al modello `kling-video-o1`, come descritto nella sezione "Riferimento Omni".
...
| `kling-video-o1`     | std / pro      | ✅                                         | ❌                                          | ❌                                        | Supporta solo `duration=5/10`                |
...
- `kling-v3` / `kling-v3-omni` accettano qualsiasi intero `duration` tra 3 e 15 secondi; gli altri modelli accettano solo 5 o 10.
- `generate_audio` è impostato su `false` per impostazione predefinita. Solo `kling-v3`, `kling-v3-omni` e `kling-v2-6` (modalità pro) lo supportano.
...
- `start_image_url`: quando si sceglie l'azione di generazione video da immagine `image2video`, è necessario caricare il link dell'immagine di riferimento del primo frame.
- `prompt`: suggerimento.
...
- `refer_type`: tipo di riferimento, può essere `base` (predefinito, **video di base da modificare**, ovvero "modificare direttamente il video", è possibile aggiungere/rimuovere/modificare elementi, cambiare composizione, stile, colore, tempo atmosferico, ecc.) o `feature` (**riferimento alle caratteristiche**, riferimento al suo stile / movimento della camera / continuazione della scena successiva).
...
### Riferimento multiplo ( `image_list` )
...
Quando si utilizza, è necessario fare riferimento in `prompt` con `&lt;&lt;<image_1>>>`, `&lt;&lt;<image_2>>>`. Limite di quantità: se non ci sono video di riferimento, la somma delle immagini di riferimento + soggetti multipli ≤ 7; se ci sono video di riferimento ≤ 4. È possibile utilizzare direttamente `start_image_url` / `end_image_url` quando si passano solo frame iniziali / finali.
...
Esempio di CURL per generare un video con riferimento multiplo:
...
### Riferimento principale ( `element_list` )

`element_list` è utilizzato per fare riferimento agli elementi già caricati nella "libreria degli elementi", i campi degli elementi dell'array sono `element_id` (ID dell'elemento nella libreria). Quando si utilizza, è necessario fare riferimento in `prompt` con `&lt;&lt;<element_1>>>`. Il numero di elementi di riferimento deve essere calcolato insieme al numero di immagini di riferimento: se ci sono video di riferimento, la somma di entrambi deve essere ≤ 4, se non ci sono video di riferimento ≤ 7.

## Callback asincrona

Poiché il tempo di generazione dell'API Kling Videos Generation è relativamente lungo, circa 1-2 minuti, se l'API non risponde per lungo tempo, la richiesta HTTP manterrà la connessione, causando un consumo aggiuntivo di risorse di sistema, quindi questa API offre anche supporto per callback asincroni.
