...
- `duration`: durata del video, in secondi. Per i modelli `kling-v3` e `kling-v3-omni` supporta durate flessibili da 3 a 15 secondi (interi), altri modelli supportano 5 o 10 secondi.
...
- `mode=4k` è supportato solo da `kling-v3` e `kling-v3-omni`; inoltre è incompatibile con `camera_control` (controllo della camera).
...
Quando si utilizza, è necessario fare riferimento in `prompt` con `&lt;&lt;<image_1>>>`, `&lt;&lt;<image_2>>>`. Limite di quantità: se non esiste un video di riferimento, la somma delle immagini di riferimento + soggetti multipli ≤ 7; se esiste un video di riferimento ≤ 4. È possibile utilizzare direttamente `start_image_url` / `end_image_url` se si passano solo il primo / ultimo frame.
