...
Per utilizzare l'API di Generazione Video Luma, prima vai al [Pannello di Controllo di Ace Data Cloud](https://platform.acedata.cloud/console/applications) per ottenere il tuo API Token, da conservare per uso futuro.
...
> 📘 Documentazione completa: [API di Generazione Video Luma →](https://platform.acedata.cloud/documents/luma-videos)
...
- `async`: opzionale, impostato su `true` per restituire immediatamente `task_id`, senza fornire `callback_url`, quindi utilizzare l'interfaccia di query del compito corrispondente per ottenere i risultati.
...
Possiamo vedere che in questo momento abbiamo ottenuto le informazioni relative a questo video, inclusi video ID, link video, copertina video e altro.

La spiegazione dei campi è la seguente:

- success: se la generazione è avvenuta con successo, se sì è `true`, altrimenti `false`
- task_id: l'ID unico di questo compito di generazione video
- video_id: l'ID unico del video generato da questo compito di generazione video
- prompt: la parola chiave di questo compito di generazione video
- video_url: il link al video risultante di questo compito di generazione video
- video_height: l'altezza dell'immagine di copertura del video generato
- video_width: la larghezza dell'immagine di copertura del video generato
- state: lo stato di questo compito di generazione video, se il compito è completato sarà `completed`.
- thumbnail_url: il link all'immagine di copertura del video generato
- thumbnail_width: la larghezza dell'immagine di copertura del video generato
- thumbnail_height: l'altezza dell'immagine di copertura del video generato
...
- action: l'azione del compito di generazione video, solitamente generazione normale `generate` e generazione estesa `extend`, predefinita `generate`.
...
Ultimamente si è giunti ai seguenti risultati:
...
- action: in questo caso, l'azione di estensione del video, che deve essere `extend`.
...
Si può notare che il video è stato esteso sulla base del video da estendere, il contenuto del risultato è coerente con quanto sopra, realizzando così la funzione di continuazione della generazione della canzone.
...
Dal risultato si può vedere che anche utilizzando il link del video è possibile realizzare la funzione di estensione del video.
...
Successivamente, sulla base di quanto sopra, aggiungiamo le informazioni sull'immagine del fotogramma finale, come segue:
...
Si può notare che, sulla base dell'estensione del video precedente, è possibile anche specificare un'immagine del fotogramma finale per l'estensione.
...
Copia questo URL e puoi usarlo come Webhook; l'esempio qui è https://webhook.site/0c87ca0e-cd74-4577-8d68-f2b80fbf8a13.
