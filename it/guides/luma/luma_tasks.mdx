...
Per utilizzare l'API Luma Videos Generation, prima di tutto vai al [Pannello di controllo di Ace Data Cloud](https://platform.acedata.cloud/console/applications) per ottenere il tuo token API, da conservare per uso futuro.
...
**Un token API è sufficiente per accedere a tutti i servizi della piattaforma, senza necessità di richiederne uno separato per ogni servizio.** La prima richiesta ti darà un credito gratuito, per un'esperienza senza costi; quando il credito è insufficiente, puoi ricaricare il saldo generale nel [pannello di controllo](https://platform.acedata.cloud/console/coin).

> 📘 Documentazione completa: [API Luma Videos Generation →](https://platform.acedata.cloud/documents/luma-videos)

## Esempio di richiesta

L'API Luma Tasks può essere utilizzata per interrogare i risultati dell'API Luma Videos Generation. Per informazioni su come utilizzare l'API Luma Videos Generation, consulta la documentazione [API Luma Videos Generation](https://platform.acedata.cloud/documents/luma-videos-integration).

Prendiamo come esempio un ID attività restituito dal servizio Luma Videos Generation, per dimostrare come utilizzare questa API. Supponiamo di avere un ID attività: 50fc6182-fa86-4c7d-ac12-2fa27ec2f151, e ora dimostreremo come passare un ID attività.
...
- `video_id`, l'identificativo unico del video nell'attività video interrogata, necessario per operazioni di estensione del video in futuro.
- `request`, informazioni sulla richiesta nell'attività video interrogata.
- `response`, informazioni sulla risposta nell'attività video interrogata.
...
Questa è per interrogare i dettagli delle attività video per più ID attività, a differenza di quanto sopra, è necessario selezionare l'azione come retrieve_batch.
