...
Dopo una richiesta riuscita, l'API restituirà i dettagli di questa attività. Ad esempio:
...
- `elapsed`，tempo di esecuzione del compito, unità in secondi (float, con 3 decimali). Questo campo non viene restituito se il compito non è completato.

## Operazione di query in batch

Questa è una query per dettagli di più ID di compiti, a differenza di quanto sopra, è necessario selezionare l'azione come retrieve_batch.
...
I risultati restituiti contengono più campi, dove gli elementi includono i dettagli specifici del compito batch, e le informazioni specifiche di ciascun compito sono simili al formato del risultato della query di un singolo compito sopra. Le informazioni sui campi sono le seguenti.
