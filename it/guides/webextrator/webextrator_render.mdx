...
**Un API Token è sufficiente per chiamare tutti i servizi della piattaforma, non è necessario richiederne uno separato per ogni servizio.** La prima richiesta riceverà un credito gratuito, per un'esperienza gratuita; quando il credito è insufficiente, puoi ricaricare il saldo generale nel [pannello di controllo](https://platform.acedata.cloud/console/coin).
...
| `timeout`           | number    |      ❌      | `30`                         | Timeout per singola navigazione (**secondi**).                          |
...
```json
{
  ...
  "data": {
    ...
    "title": "Esempio di Dominio",
    "status": 200,
    "html": "<!DOCTYPE html><html>...</html>",
    "text": "Esempio di Dominio\nQuesto dominio è per uso in esempi illustrativi...",
...
Quando il compito è completato, la piattaforma invierà POST il risultato completo al tuo `callback_url`.
...
- **La chiave della cache ignora `async`.** Le richieste sincrone e asincrone per lo stesso URL colpiscono la stessa voce di cache,
  passare liberamente non invalida.
