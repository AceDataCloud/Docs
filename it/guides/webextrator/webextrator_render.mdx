...
Per utilizzare il servizio WebExtrator, prima vai al [Pannello di controllo di Ace Data Cloud](https://platform.acedata.cloud/console/applications) per ottenere il tuo API Token, da conservare per uso futuro.
...
| `timeout`           | number    |      ❌      | `30`                         | Timeout per la navigazione singola (**secondi**).                       |
| `wait_until`        | enum      |      ❌      | `networkidle`                | Evento di completamento del caricamento: `load` / `domcontentloaded` / `networkidle` / `commit`. |
| `delay`             | number    |      ❌      | `0`                          | **Secondi di attesa aggiuntivi** dopo il trigger di `wait_until` (per il rendering secondario di SPA). |
...
| `async`             | boolean   |      ❌      | `false`                      | Imposta su `true` per restituire immediatamente `task_id`, i risultati possono essere recuperati tramite `callback_url` o API Tasks. |

> I contratti della piattaforma utilizzano uniformemente **snake_case**. I servizi di rendering interni supportano camelCase, ma tutte le chiamate esterne utilizzano snake_case.
...
| `data.title`          | string           | `document.title` dopo il rendering.                 |
...
| `data.text`           | string           | Snapshot di `document.body.innerText` (per un testo più pulito, utilizzare Extract). |
...
| `data.cacheStoredAt`  | number?          | Timestamp Unix in millisecondi della prima scrittura dell'elemento nella cache. |
...
[`/webextrator/tasks`](development_webextrator_tasks) per una query attiva.

### Struttura del callback

La piattaforma `POST` un envelope **identico** a quello della modalità sincrona a `callback_url`,
`Content-Type: application/json`. Restituire qualsiasi `2xx` è considerato confermato; `5xx` verrà riprovato con un backoff esponenziale per circa 5 minuti.
...
- **La chiave della cache ignora `async`.** Le richieste sincrone e asincrone per lo stesso URL colpiscono la stessa voce di cache,
  passare liberamente non causerà invalidità.
...
- **`block_resources` è il modo più veloce per ridurre la latenza.** Per impostazione predefinita, sono già bloccati immagini / font / media;
  se l'estrazione non dipende dal layout CSS, aggiungere `stylesheet` può rendere ancora più veloce.
