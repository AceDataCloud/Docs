...
3. **Readability + Markdown di fallback** — sempre in esecuzione, completa i campi di livello superiore non riempiti dalle prime due fasi.
...
Se non hai ancora effettuato il login o registrato, verrai automaticamente reindirizzato alla pagina di login che ti invita a registrarti e accedere, e una volta completato, verrai riportato automaticamente alla pagina corrente.
...
Ordinato per priorità (il primo colpo viene considerato come `structured.schemaOrg.primary`):
...
| `BreadcrumbList`                                                                                           | (appeso a sibling) | Sempre restituito a `structured.schemaOrg.breadcrumbs[]`, non sarà considerato come primary.                                                                                                                                 |
...
- URL delle immagini relative (risolti in assoluti secondo `finalUrl`).
...
In caso di successo, LLM riempirà anche i campi di alto livello come "ultima risorsa":
...
Il riempimento viene attivato solo quando le fonti di dati certe **non** hanno riempito i campi corrispondenti — LLM è sempre l'ultima risorsa.
...
Quando il compito è completato, invia l'intero envelope `POST` al tuo `callback_url` (se configurato). Puoi anche
consultare successivamente [`/webextrator/tasks`](development_webextrator_tasks) per controllare attivamente.
...
- **`structured.llmError` è informativo.** La richiesta è comunque riuscita, il risultato euristico è comunque restituito. Controlla `llmError.error` per individuare la causa (timeout, errore di parsing JSON, errore di validazione Zod).
- **I `links[]` non articolo non saranno ordinati per rilevanza.** Solo secondo "fino a 100 voci + filtraggio dei protocolli non validi" si cerca di pulire.
- **Le cache colpite sono anche a pagamento.** La cache serve per ritardi e per proteggere il pool del browser, non per risparmiare.
