...
```json
{
  "answer": "Va bene, ho già ricordato 42. Cosa devo farne?",
  "id": "f2f4b3e8-0c0a-4d3a-aaa2-7ff80c0a1c44"
}
```
...
| `error`             | Errore verificatosi in questo turno, `message` descrive il contenuto dell'errore.                                                                        |
...
Per un controllo più dettagliato (ad esempio, per inserire più immagini tra i testi, o se l'ordine è molto importante) si utilizza direttamente l'array `message`.

## Chiamata agli strumenti e MCP
...
Nella stream NDJSON / SSE, la chiamata agli strumenti viene presentata attraverso eventi di tipo `tool_use` e `tool_result`, ad esempio:
...
Se non si desidera visualizzare i dettagli della chiamata agli strumenti nel frontend, è possibile ignorare gli eventi `tool_use` / `tool_result` / `card` / `citation`, l'output finale del modello continuerà a fluire attraverso `text_delta`.
...
Se l'utente decide di abbandonare questa domanda, basta inviare una nuova `question` / `message` e la piattaforma segnerà automaticamente la chiamata allo strumento sospesa come "saltata dall'utente".
...
L'API AI Chat v2, mantenendo la retrocompatibilità con v1, ha aggiornato le conversazioni da "domande e risposte a turno singolo/multiplo" a "conversazioni osservabili in stile agente": input multimodale, chiamate agli strumenti, pause/ripristini, eventi strutturati in streaming, CRUD integrato. Si consiglia di utilizzare direttamente v2 per le nuove integrazioni; le integrazioni esistenti di v1 possono essere migrate in fasi. Se hai domande, non esitare a contattare il nostro team di supporto tecnico.
