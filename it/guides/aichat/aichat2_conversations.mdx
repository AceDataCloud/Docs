...
- **Interrompibile / Recuperabile**: il modello emetterà un evento `ask_user_question` e si fermerà quando ha bisogno di ulteriori informazioni dall'utente; la prossima chiamata può continuare semplicemente riempiendo la risposta tramite `tool_results`.
...
curl -X POST 'https://api.acedata.cloud/aichat2/conversations' \
  ...
  -d '{
    ...
    "stateful": true,
    "id": "f2f4b3e8-0c0a-4d3a-aaa2-7ff80c0a1c44",
    "question": "Qual è il numero che ti ho chiesto di ricordare poco fa?"
  }'
```
...
| `ask_user_question` | Il modello emette una richiesta di informazioni aggiuntive dall'utente, la conversazione entra nello stato `awaiting_user_input`, vedi sotto [Ripristinare una conversazione in pausa](#ripristinare-una-conversazione-in-pausa).                           |
...
- `text` — Testo normale, il campo `text` è obbligatorio.
- `image_url` — Immagine, il campo `image_url.url` è obbligatorio.
- `file_url` — File (PDF, CSV, TXT, ecc.), il campo `file_url.url` è obbligatorio.
...
Se si desidera un controllo più preciso (ad esempio, inserire più immagini tra i testi, o se l'ordine è molto importante) si utilizza direttamente l'array `message`.
...
Se non si desidera mostrare i dettagli della chiamata degli strumenti nel frontend, è possibile ignorare gli eventi di tipo `tool_use` / `tool_result` / `card` / `citation`, l'output finale del modello continuerà a fluire attraverso `text_delta`.
...
Dopo puoi usare `action: retrieve` + `id` per interrogare il risultato della conversazione; puoi anche fornire `callback_url`, una volta completato il compito, la piattaforma invierà `{ status, answer, usage, error }` tramite POST al tuo indirizzo di callback. `callback_url` deve utilizzare `http` / `https`, e non può contenere direttamente indirizzi `localhost` o IP privati.
...
I valori in `allowed_skills` sono gli slug delle Skill connesse; i valori in `allowed_mcp_servers` sono gli slug dei MCP Server connessi. Le Skill / MCP Server non incluse nell'autorizzazione preventiva possono solo visualizzare, eseguire dry-run o rifiutare l'esecuzione delle operazioni di scrittura in modalità senza supervisione.
...
Nota: l'autorizzazione preventiva rappresenta solo "questa richiesta consente a queste capacità di saltare la conferma umana in modalità senza supervisione". Le Skill specifiche devono comunque supportare `--unattended-confirm` o meccanismi di sicurezza corrispondenti; altrimenti continueranno a eseguire dry-run e non eseguiranno direttamente operazioni di scrittura.

## Ripristino di conversazioni sospese

Alcuni strumenti possono far sì che il modello "ponda domande all'utente", in questo caso il modello emetterà un evento `ask_user_question`, la conversazione sarà congelata nello stato `awaiting_user_input`:
...
```json
{
  ...
  "question": "Preferisci che il rapporto generato sia in cinese o in inglese?",
  "options": ["中文", "英文"],
  "id": "f2f4b3e8-..."
}
```
...
Se l'utente decide di rinunciare a questa domanda, basta inviare una nuova `question` / `message`, la piattaforma segnerà automaticamente la chiamata dello strumento sospesa come "saltata dall'utente".
...
- `401 invalid_token`: l'intestazione `authorization` non è corretta.
