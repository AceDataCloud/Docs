...
Le regole di fatturazione specifiche possono essere consultate nella scheda Pricing della pagina dei servizi.
...
| Server / CLI / Node parsing in streaming | `application/x-ndjson` | Un oggetto JSON per riga                               |
| Non serve streaming                 | `application/json` (predefinito) | Restituzione unica di `{answer, id}`                       |
...
Nel frontend, questo evento viene visualizzato come una scheda per consentire all'utente di scegliere una risposta, quindi invia una nuova richiesta utilizzando lo stesso `id`, riempiendo la risposta tramite `tool_results`:
...
Nel corpo della richiesta, `tool_use_id` **deve** essere esattamente identico a `tool_id` al momento della sospensione; una discrepanza restituirà un errore 400. Quando ci sono `tool_results` nella richiesta, `question` / `message` / `references` verranno ignorati.
...
v2 offre una gestione leggera delle conversazioni attraverso il campo `action` sullo stesso endpoint, senza necessità di un'API separata.
