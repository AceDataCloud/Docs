...
Per utilizzare l'API di domande e risposte AI, prima di tutto, vai al [Pannello di Controllo di Ace Data Cloud](https://platform.acedata.cloud/console/applications) per ottenere il tuo Token API, da tenere come riserva.
...
**Un Token API è sufficiente per accedere a tutti i servizi della piattaforma, senza la necessità di richiederne uno separato per ogni servizio.** La prima richiesta ti darà un credito gratuito, per un'esperienza senza costi; quando il credito è insufficiente, puoi ricaricare il saldo generale nel [pannello di controllo](https://platform.acedata.cloud/console/coin).

> 📘 Documentazione Completa: [API di Domande e Risposte AI →](https://platform.acedata.cloud/documents/aichat-conversations)

## Utilizzo di Base

Iniziamo a comprendere il modo di utilizzo di base, che consiste nell'inserire una domanda e ottenere una risposta, semplicemente trasmettendo un campo `question` e specificando il modello corrispondente.
...
Se desideri restituire una risposta in streaming, puoi modificare il parametro `accept` nelle intestazioni della richiesta, impostandolo su `application/x-ndjson`.

La modifica è mostrata nell'immagine, ma il codice di chiamata deve essere adeguatamente modificato per supportare le risposte in streaming.

![](https://cdn.acedata.cloud/axt1aa.png)

Dopo aver modificato `accept` in `application/x-ndjson`, l'API restituirà i dati JSON corrispondenti riga per riga, e a livello di codice dovremo apportare le modifiche necessarie per ottenere i risultati riga per riga.
...
```python
{
    "answer": "Come artista professionista, posso offrire una gamma di servizi e assistenza a seconda delle tue esigenze specifiche. Ecco alcuni modi in cui posso aiutarti:\n\n1. Arte personalizzata: Se hai una visione o un'idea specifica, posso creare opere d'arte personalizzate per te. Questo può includere dipinti, disegni, arte digitale o qualsiasi altro mezzo tu preferisca.\n\n2. Opere su commissione: Se hai un soggetto o un concetto specifico in mente, posso creare opere d'arte su commissione su misura per le tue preferenze. Questo potrebbe essere per piacere personale o come regalo unico per qualcuno di speciale.\n\n3. Consultazione artistica: Se hai bisogno di orientamento sulla selezione dell'arte, design degli interni o su come mostrare e esporre l'arte nel tuo spazio, posso fornire consigli professionali per aiutarti a migliorare il tuo senso estetico e creare un aspetto coeso."
}
```

Possiamo vedere che qui abbiamo detto a GPT che è un robot, poi gli abbiamo chiesto cosa può fare per noi e lui può interpretare il ruolo di un robot per rispondere alle domande.
...
Il link di quell'immagine è [https://cdn.acedata.cloud/ht05g0.png](https://cdn.acedata.cloud/ht05g0.png), possiamo semplicemente passarla come parametro `references`, e bisogna notare che il modello deve essere scelto per supportare il riconoscimento visivo, attualmente supportato è `gpt-4-vision`, quindi l'input è il seguente:
...
Questa API supporta anche modelli online, inclusi GPT-3.5 e GPT-4, entrambi possono supportare, c'è un processo automatico di ricerca su Internet e sintesi dietro l'API, possiamo scegliere il modello come `gpt-3.5-browsing` per sperimentarlo, come mostrato nell'immagine:
...
-d '{
  ...
  "question": "Com'è il tempo a New York oggi?"
}'
```
