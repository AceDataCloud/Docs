...
OpenAI ha recentemente fornito un'interfaccia per creare risposte del modello. Fornisce input di testo o immagini per generare output di testo o immagini. Consente al modello di chiamare il proprio codice personalizzato o di utilizzare strumenti integrati, come la ricerca web o la ricerca nei file, per utilizzare i propri dati come input per le risposte del modello.

Questo documento presenta principalmente il flusso di utilizzo delle operazioni dell'API OpenAI Responses; utilizzandola, possiamo usare facilmente la funzionalità ufficiale di OpenAI per creare risposte del modello.

## Procedura di richiesta

Per utilizzare l'API OpenAI Responses, prima vai alla [console Ace Data Cloud](https://platform.acedata.cloud/console/applications) per ottenere il tuo API Token, da conservare come riserva.

![](https://cdn.acedata.cloud/dvc3cg.jpg)

Se non hai ancora effettuato l'accesso o la registrazione, verrai automaticamente reindirizzato alla pagina di accesso per invitarti a registrarti e accedere; una volta completato, tornerai automaticamente alla pagina corrente.

**Un solo API Token può chiamare tutti i servizi della piattaforma, senza necessità di richiederne uno separatamente per ogni servizio.** La prima richiesta offre crediti gratuiti, per un'esperienza gratuita; quando i crediti sono insufficienti, puoi ricaricare il saldo universale dalla [console](https://platform.acedata.cloud/console/coin).

> 📘 Documentazione completa: [OpenAI Responses API →](https://platform.acedata.cloud/documents/openai-responses)

## Utilizzo di base

Successivamente puoi compilare i contenuti corrispondenti nell'interfaccia, come mostrato nella figura:

<p><img src="https://cdn.acedata.cloud/8lu8di.png" width="400" className="m-auto" /></p>

Quando si utilizza questa interfaccia per la prima volta, dobbiamo compilare almeno tre contenuti: uno è `authorization`, che può essere selezionato direttamente dall'elenco a discesa. Un altro parametro è `model`; `model` è la categoria di modello del sito ufficiale OpenAI ChatGPT che scegliamo di utilizzare. Qui disponiamo principalmente di 20 tipi di modelli; per i dettagli puoi vedere i modelli che forniamo. L'ultimo parametro è `input`; `input` è l'array delle parole di domanda che inseriamo, è un array, il che indica che è possibile caricare contemporaneamente più parole di domanda. Ogni parola di domanda contiene `role` e `content`, dove `role` indica il ruolo di chi pone la domanda; forniamo tre identità, ovvero `user`, `assistant`, `system`. L'altro `content` è il contenuto specifico della nostra domanda.

Allo stesso tempo, puoi notare che sulla destra viene generato il codice di chiamata corrispondente; puoi copiare il codice ed eseguirlo direttamente, oppure puoi fare clic direttamente sul pulsante «Try» per effettuare un test.

Parametri opzionali comuni:

- `max_tokens`: limita il numero massimo di token per una singola risposta.
- `temperature`: casualità della generazione, tra 0 e 2; maggiore è il valore, più è divergente.
- `n`: quante risposte candidate generare in una volta.
- `response_format`: impostazione del formato di ritorno.
- `tools`: definizione della chiamata di funzioni/strumenti.
...
Il risultato restituito ha più campi, presentati come segue:
...
- `output`, le informazioni di risposta fornite da ChatGPT alla parola di domanda.
- `usage `: informazioni statistiche sui token per questa sessione di domande e risposte.

Tra questi, `output` contiene le informazioni della risposta di ChatGPT; l'`output` al suo interno è ChatGPT, come si può vedere nella figura.
...
Questa interfaccia supporta anche le risposte in streaming, il che è molto utile per l'integrazione con le pagine web e può consentire alla pagina web di ottenere un effetto di visualizzazione carattere per carattere.

Se vuoi restituire la risposta in streaming, puoi modificare il parametro `stream ` nell'intestazione della richiesta, impostandolo su `true`.

La modifica è mostrata nella figura; tuttavia, il codice di chiamata deve essere modificato corrispondentemente per supportare la risposta in streaming.
...
data: {"type": "response.output_text.delta", "sequence_number": 9, "item_id": "msg_68a9837c49f081908f568bf9c6065c620721186e8fbb89d0", "output_index": 0, "content_index": 0, "delta": " aiutarti", "model": "gpt-4.1"} 
...
data: {"type": "response.output_text.delta", "sequence_number": 11, "item_id": "msg_68a9837c49f081908f568bf9c6065c620721186e8fbb89d0", "output_index": 0, "content_index": 0, "delta": " oggi", "model": "gpt-4.1"} 
...
Come si può vedere, nella risposta sono presenti molti `data`; il `delta` all'interno di `data` è il contenuto della risposta più recente, coerente con quanto descritto sopra. `delta` è il contenuto della risposta aggiunto di recente; potete integrarlo nel vostro sistema in base al risultato. La risposta in streaming termina con `response.completed` o `response.incomplete`; l'`usage` nello stato finale è l'utilizzo finale di token per questa richiesta ed è anche la base per la fatturazione.

Se il client interrompe la connessione prima dell'arrivo dello stato finale, questa richiesta verrà registrata come chiusa dal client (499) e non verrà fatturata utilizzando una stima locale dei token; se la connessione termina normalmente ma non viene ricevuto lo stato finale e l'`usage` finale, questa richiesta verrà registrata come risposta incompleta (502) e allo stesso modo non verrà fatturata utilizzando una stima dei token. Quando si verificano queste due situazioni, inviate nuovamente la richiesta.

Il risultato `data` restituito contiene diversi campi, descritti di seguito:

- `item_id`, l'ID del task di dialogo generato questa volta, utilizzato per identificare univocamente questo task di dialogo.
- `type`, il tipo del task Responses di dialogo generato questa volta.
- `model `, il modello del sito ufficiale OpenAI ChatGPT selezionato.
- `delta`, le informazioni di risposta fornite da ChatGPT alla parola di domanda.

Anche JavaScript è supportato; ad esempio, il codice per la chiamata in streaming con Node.js è il seguente:
...
Se desiderate integrare la funzionalità di dialogo multi-turno, dovete caricare più parole di domanda nel campo `input`; un esempio specifico di più parole di domanda è mostrato nella figura seguente:
...
Caricando più parole di domanda, è possibile realizzare facilmente un dialogo multi-turno e ottenere la seguente risposta:
...
Come si può vedere, le informazioni contenute in `output` sono coerenti con il contenuto dell'utilizzo di base; questo include il contenuto specifico delle risposte di ChatGPT a più dialoghi, in modo da poter rispondere alle domande corrispondenti in base al contenuto di più dialoghi.

## Modello visivo

gpt-4o è un grande modello linguistico multimodale sviluppato da OpenAI; sulla base di GPT-4, aggiunge capacità di comprensione visiva. Questo modello può elaborare contemporaneamente input di testo e immagini, realizzando comprensione e generazione multimodali.
L'elaborazione del testo con il modello gpt-4o è coerente con i contenuti di utilizzo di base descritti sopra; di seguito verrà brevemente illustrato come utilizzare le capacità di elaborazione delle immagini del modello.

L'utilizzo delle capacità di elaborazione delle immagini del modello gpt-4o avviene principalmente aggiungendo un campo `type` sulla base del contenuto `content` originale; attraverso questo campo è possibile sapere se viene caricato testo o un'immagine, utilizzando così le capacità di elaborazione delle immagini del modello gpt-4o. Di seguito viene principalmente descritto come chiamare questa funzionalità tramite Curl e Python.
...
Successivamente è possibile ottenere il seguente risultato; le informazioni dei campi nel risultato sono coerenti con quanto sopra, nello specifico come segue:
...
Oltre a gpt-4.1, esiste anche un modello a costo inferiore, chiamato gpt-4o-mini. gpt-4o-mini è il modello linguistico di grandi dimensioni di ultima generazione sviluppato da OpenAI; non solo risponde rapidamente, ma ha anche un prezzo più economico e supporta inoltre la multimodalità. Per l'utilizzo della funzionalità vision, è possibile fare riferimento ai contenuti sopra relativi all'utilizzo del modello gpt-4.1.
...
Si può vedere che abbiamo anche elaborato i file di input, con risultati simili a quelli sopra.
...
- `400 token_mismatched`: Richiesta non valida, possibilmente dovuta a parametri mancanti o non validi.
- `400 api_not_implemented`: Richiesta non valida, possibilmente dovuta a parametri mancanti o non validi.
...
Attraverso questo documento, hai già appreso come utilizzare OpenAI Responses API per implementare facilmente la funzionalità ufficiale di OpenAI per la creazione di Responses. Speriamo che questo documento possa aiutarti a integrare e utilizzare meglio questa API. In caso di domande, non esitare a contattare il nostro team di supporto tecnico.
