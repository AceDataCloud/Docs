...
- **Maggiore capacità di seguire le istruzioni**: in grado di comprendere accuratamente istruzioni strutturate complesse riguardanti composizione, conteggio, relazioni spaziali, ecc.
...
- **`gpt-image-2:reverse`**: completamente equivalente al predefinito `gpt-image-2`, con un miglior rapporto qualità-prezzo, senza variazione di prezzo.
...
Limitazioni delle dimensioni: le dimensioni personalizzate devono soddisfare che sia la larghezza che l'altezza siano multipli di 16, con il lato lungo ≤ 3840, e il numero totale di pixel ≤ 8.294.400; oltre questo limite restituirà un errore 4xx.
...
> Se si specifica `size: "auto"`, la piattaforma pianificherà la tela nello spazio proporzionale continuo e giudicherà secondo la seguente priorità: pixel o proporzioni esplicite nel prompt, standard di denominazione (carta / stampa / posizionamento della piattaforma / pubblicità / dispositivo / fotografia / cinema), consuetudini del mezzo, e infine inferenza compositiva. Pertanto, oltre alle comuni proporzioni `1:1`, `4:5`, `9:16`, `21:9`, possono essere mantenute anche proporzioni non predefinite come `1.91:1`, `1.85:1`, `2.39:1`, e proporzioni ISO `1:√2`; la dimensione finale verrà automaticamente regolata ai multipli di 16 supportati dal servizio e al budget di pixel. Se l'autovalutazione non è disponibile, si tornerà al formato predefinito del modello, senza interrompere la generazione. Se si omette il campo `size`, verrà utilizzato direttamente il formato predefinito del modello; se ci sono requisiti rigorosi sui pixel, si consiglia comunque di specificare direttamente `WIDTHxHEIGHT`.
> L'output sotto 1K non garantisce un allineamento rigoroso dei pixel: se invii `1024x1024`, potresti ricevere `1254x1254`, mantenendo la proporzione. Se lo reinserisci come `size`, il costo rimane invariato.
> Una chiamata singola a 4K richiede solitamente 4-8 minuti, si consiglia di utilizzarla in combinazione con il `callback_url` per callback asincroni.

> **Riguardo al parametro `n`**
> `gpt-image-2` supporta `n > 1` (valori da 1 a 10): una singola richiesta può restituire e addebitare il numero corrispondente di immagini. Per garantire che i risultati siano diversi, si consiglia di inviare contemporaneamente diversi `prompt` o `seed`. Questo vale anche per `gpt-image-1` / `gpt-image-1.5`, e per la serie `nano-banana` / `nano-banana-2-lite` / `nano-banana-2` / `nano-banana-pro`; `dall-e-3` supporta solo `n = 1`. Si noti che `response_format=b64_json` supporta solo `n=1`, per `n>1` si prega di utilizzare il ritorno URL predefinito. Se alcune immagini non vengono generate correttamente, verranno restituite e addebitate solo le parti riuscite.
...
Nella frase di suggerimento si possono utilizzare termini cinematografici (pellicola 35mm, profondità di campo ridotta, luci al neon, ecc.) per controllare con precisione l'atmosfera e la qualità.
...
payload = {
    "model": "gpt-image-2",
    "prompt": "Un ritratto cinematografico di una giovane donna che si trova in un negozio di alimentari di notte, illuminata da morbide insegne al neon rosa e ciano attraverso la finestra. Ripresa su pellicola 35mm, profondità di campo ridotta, leggero grano, umore malinconico.",
    "size": "1024x1536"
}
...
```json
{
  ...
  "data": [
    {
      "revised_prompt": "Un ritratto cinematografico di una giovane donna che si trova in un negozio di alimentari di notte, illuminata da morbide insegne al neon rosa e ciano attraverso la finestra. Ripresa su pellicola 35mm, profondità di campo ridotta, leggero grano, umore malinconico.",
...
### Scena 4: Stile illustrazione (orizzontale)
...
payload = {
    "model": "gpt-image-2",
    "prompt": "Un'illustrazione morbida e poetica per un libro per bambini di una piccola volpe che legge un libro sotto un fungo luminoso in una foresta illuminata dalla luna. Texture ad acquerello e matita, colori pastello delicati, atmosfera da sogno, sensazione di disegno a mano.",
    "size": "1536x1024"
}
```
...
> Nano Banana si integra con il protocollo OpenAI tramite un livello di adattamento, rispetto a `gpt-image-*` supporta solo i seguenti parametri: `model`, `prompt`, `size`, `n`.
...
> - La struttura di ritorno segue il formato OpenAI (`data[].url`), ma `created` è fisso a `0`, e non restituirà `b64_json`, `revised_prompt` sarà sempre uguale al `prompt` originale.
...
```json
{
  ...
  "data": [
    {
      "revised_prompt": "Un adorabile cucciolo di lontra marina è sdraiato giocando sulla schiena nell'acqua, con il suo pelo che appare lucido e morbido. Una delle sue piccole zampe si allunga curiosamente, e ha un'espressione di pura gioia e calore sul viso mentre guarda il cielo. Il suo corpo è circondato da bolle provenienti dal suo giocoso gironzolare nell'acqua. Una leggera brezza gioca con il suo pelo rendendolo ancora più affascinante. La scena ritrae la tranquillità e il fascino della vita marina.",
...
Con la stessa operazione sopra descritta, basta impostare il parametro di qualità dell'immagine su `hd`, per ottenere l'immagine mostrata qui sotto:
...
Il risultato restituito è coerente con il contenuto di base utilizzato, si può vedere che la dimensione dell'immagine generata è `1024 * 1024`, mostrata qui sotto:

<p><img src="https://cdn.acedata.cloud/o4pvvx.png" width="500" className="m-auto" /></p>

Con la stessa operazione sopra descritta, basta impostare la dimensione dell'immagine a `1792 * 1024`, per ottenere l'immagine mostrata qui sotto:
...
Il risultato restituito è coerente con il contenuto di base utilizzato, si può vedere che il parametro di stile dell'immagine è `vivid`, l'immagine generata è mostrata qui sotto:

<p><img src="https://cdn.acedata.cloud/e0rpc3.png" width="500" className="m-auto" /></p>

Con la stessa operazione sopra descritta, basta impostare il parametro di stile dell'immagine su `natural`, per ottenere l'immagine mostrata qui sotto:
...
Si può notare che nel risultato c'è un campo `task_id`, il campo `data` contiene i risultati di generazione dell'immagine simili a quelli della chiamata sincrona, attraverso il campo `task_id` è possibile realizzare l'associazione del compito.
