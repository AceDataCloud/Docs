...
In modalità callback, **l'interfaccia immagini originale restituisce immediatamente un `task_id` dopo aver elaborato la richiesta**; è possibile utilizzare direttamente questo `task_id` e interrogarlo tramite questa interfaccia quando necessario, senza dover fornire un `trace_id` personalizzato (solo se si desidera associare con un identificatore di business proprio).

> L'attività verrà persa solo se la richiesta originale per le immagini include un `callback_url`. Le richieste effettuate in modo sincrono (non in callback) non verranno memorizzate.
...
| `id`         | string | Opzionale    | ID dell'attività restituito nella risposta sincrona della richiesta di immagini (si consiglia di utilizzare) |
| `trace_id`   | string | Opzionale    | Necessario solo se hai esplicitamente fornito un `trace_id` personalizzato nella richiesta originale |
...
- `id`: ID dell'attività generato al momento dell'elaborazione della richiesta originale per le immagini.
...
- `type`: Tipo di interfaccia upstream. Le attività scritte nella serie `gpt-image` (come `gpt-image-2`) sono di tipo `images`; i canali OpenAI precedenti (come `gpt-image-1`, nano-banana) utilizzano `images_generations` / `images_edits`, mentre alcune interfacce di chat sono di tipo `chat_completions_image`.
- `request`: Corpo completo della richiesta originale.
- `response`: Corpo della risposta finale restituito dall'interfaccia upstream al termine del callback.
- `created_at` / `finished_at` / `duration`: Timestamp Unix (secondi) e tempo di esecuzione (secondi).
...
| `type`             | string   | Filtra in base al tipo upstream (valori: `images`, `images_generations`, `images_edits`) |
| `offset`           | int      | Punto di partenza per la paginazione, predefinito `0`     |
| `limit`            | int      | Numero di elementi per pagina, predefinito `12`           |
| `created_at_min`   | float    | Timestamp di inizio (secondi Unix)                        |
| `created_at_max`   | float    | Timestamp di fine (secondi Unix)                          |

È possibile fornire uno tra `ids` / `trace_ids` / `application_id` / `user_id` o il periodo di tempo `created_at_*`.
  ...
  "items": [
    {
      ...
      "type": "images",
      "request": { "model": "gpt-image-2", "prompt": "Un gatto" },
...
L'API Tasks serve principalmente per i flussi asincroni in modalità callback. In modalità callback, l'interfaccia di invio restituirà **immediatamente un `task_id`** (cioè l'ID del compito), dopodiché è sufficiente utilizzare direttamente questo `task_id` per effettuare il polling sull'interfaccia Tasks, senza dover generare un `trace_id` da soli.
...
# 2. Utilizzare direttamente il task_id nella risposta di invio per effettuare il polling sull'interfaccia Tasks, fino al completamento del compito
while True:
    task = requests.post(
        f"{API}/openai/tasks",
        headers=HEADERS,
...
- L'interfaccia Tasks **non addebita costi**, è possibile effettuare il polling senza preoccupazioni. Solo le richieste di generazione/modifica delle immagini originali comporteranno costi.
