...
> L'attività verrà persa solo se la richiesta originale per le immagini include un `callback_url`. Le richieste effettuate in modo sincrono (non callback) non verranno memorizzate.
...
- `created_at` / `finished_at` / `duration`: Timestamp Unix (secondi) e durata dell'esecuzione (secondi).
...
| `ids`              | string[] | Interroga in base a un elenco di ID attività               |
| `trace_ids`        | string[] | Interroga in base a un elenco di `trace_id`               |
...
- L'interfaccia Tasks **non comporta costi**, è possibile effettuare il polling senza preoccupazioni. Solo le richieste originali di generazione/modifica delle immagini comporteranno costi.
