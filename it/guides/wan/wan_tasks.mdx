...
La principale funzionalità dell'API Wan Tasks è quella di interrogare lo stato di esecuzione di un'attività utilizzando l'ID dell'attività generato dall'API Wan Videos Generation.

Questo documento fornirà una spiegazione dettagliata dell'integrazione dell'API Wan Tasks, aiutandoti a integrare facilmente e sfruttare appieno le potenti funzionalità di questa API. Con l'API Wan Tasks, puoi facilmente interrogare lo stato di esecuzione delle attività dell'API Wan Videos Generation.

## Processo di richiesta

Per utilizzare l'API Wan Videos Generation, prima vai al [Pannello di controllo di Ace Data Cloud](https://platform.acedata.cloud/console/applications) per ottenere il tuo API Token, da conservare per uso futuro.
...
> 📘 Documentazione completa: [API Wan Videos Generation →](https://platform.acedata.cloud/documents/wan-videos)

## Esempio di richiesta

L'API Wan Tasks può essere utilizzata per interrogare i risultati dell'API Wan Videos Generation. Per informazioni su come utilizzare l'API Wan Videos Generation, consulta la documentazione [API Wan Videos Generation](https://platform.acedata.cloud/documents/wan-videos).

Prendiamo come esempio un ID di attività restituito dal servizio API Wan Videos Generation, per dimostrare come utilizzare questa API. Supponiamo di avere un ID di attività: a4bca552-d964-46a1-8ff7-fd922f916582, e ora dimostriamo come passare un ID di attività.
...
- `started_at`, data di inizio dell'esecuzione dell'attività, timestamp Unix (secondi, float).
...
Attraverso questo documento, hai compreso come utilizzare l'API Wan Tasks per interrogare i dettagli specifici di singoli o batch di video task. Speriamo che questo documento possa aiutarti a integrare e utilizzare meglio questa API. Se hai domande, non esitare a contattare il nostro team di supporto tecnico.
