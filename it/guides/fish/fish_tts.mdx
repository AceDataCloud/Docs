...
**Un API Token è sufficiente per chiamare tutti i servizi della piattaforma, non è necessario richiederne uno per ogni servizio.** La prima richiesta ti darà un credito gratuito, per un'esperienza gratuita; quando il credito è insufficiente, puoi ricaricare il saldo generale nella [console](https://platform.acedata.cloud/console/coin).
...
| `prosody`         | object               | No           | Copertura prosodica, supporta `speed` (velocità di parola, 1.0 è la velocità originale) e `volume` (guadagno del volume in dB). Ad esempio `{"speed":1.2,"volume":0}`.                                          |
...
| `latency`         | string               | No           | `normal` o `balanced`, di default questo API completa automaticamente `normal` (l'invio di una stringa vuota verrà rifiutato dall'upstream).                                                                                                                    |
...
> La denominazione dei campi è completamente identica all'upstream. Ad eccezione di `callback_url`, gli altri campi significano e assumono valori come indicato nella [documentazione ufficiale TTS di Fish](https://docs.fish.audio/text-to-speech/text-to-speech).
...
Di seguito un timbro vocale spagnolo pubblico sulla piattaforma Fish (`_id` può essere recuperato tramite [Fish Model Query](https://platform.acedata.cloud/documents/fish-model-query)):
...
`speed` maggiore di 1 accelera, minore di 1 rallenta; `volume` in dB, 0 significa invariato, numeri positivi guadagno, numeri negativi attenuazione.
...
Per scenari in cui è necessario effettuare un'unione in tempo reale nel browser, o per ulteriori elaborazioni (mixaggio, variazione di velocità) sul client, si consiglia di utilizzare `pcm`:
...
Il costo minimo per integrare Fish TTS è: sostituire il token di autenticazione con il token della piattaforma nel codice esistente che chiama `api.fish.audio/v1/tts`, e includere esplicitamente nel corpo della richiesta `format: "mp3"`. Per scenari di testi lunghi, si consiglia di utilizzare `callback_url` per il callback asincrono; per la scoperta del `reference_id` del timbro vocale, si prega di utilizzare [Fish Model Query](https://platform.acedata.cloud/documents/fish-model-query) e [Fish Model Get](https://platform.acedata.cloud/documents/fish-model-get).
