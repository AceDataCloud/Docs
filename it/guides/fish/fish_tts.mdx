...
| `format`          | string               | No           | Formato audio in uscita, opzioni `mp3` (predefinito), `wav`, `pcm`. `wav` e `pcm` restituiscono entrambi contenitori WAV. `opus` non è supportato, l'invio restituirà direttamente `400`. |
...
| `references`      | object[]             | No           | Campioni di riferimento in linea, struttura identica a quella upstream, ogni voce contiene `audio` e `text`. Uno dei due con `reference_id`.                                      |
| `sample_rate`     | integer              | No           | Frequenza di campionamento, comunemente `16000`, `22050`, `44100`. `format=mp3` predefinito 44100.                                                                              |
...
| `latency`         | string               | No           | `normal` o `balanced`, di default questo interfaccia compensa automaticamente con `normal` (l'invio di una stringa vuota verrà rifiutato dall'upstream).                           |
...
> La denominazione dei campi è completamente identica a quella upstream. Ad eccezione di `callback_url`, gli altri campi hanno significato e valori come indicato nella [documentazione ufficiale TTS di Fish](https://docs.fish.audio/text-to-speech/text-to-speech).
...
La sintesi di testi lunghi può richiedere da alcuni secondi a decine di secondi, se la connessione si interrompe è necessario riprovare. Dopo aver passato `callback_url` nel corpo della richiesta, l'API restituirà immediatamente `{task_id, started_at}`, quando il processo sarà completato, il risultato completo verrà richiamato a quell'URL in formato JSON POST, includendo lo stesso `task_id` e il `cost` finale di questa richiesta. La conferma del compito iniziale non include quindi `cost` poiché la sintesi non è ancora completata.
...
È anche possibile utilizzare [Fish Tasks API](https://platform.acedata.cloud/documents/fish-tasks) per richiamare attivamente i risultati in base a `task_id`; il `response.cost` registrato nello stato finale corrisponde al `cost` nel callback, vedere il documento per maggiori dettagli.
...
Il costo minimo per integrare Fish TTS è: sostituire il token di autenticazione con il token della piattaforma nel codice già esistente che chiama `api.fish.audio/v1/tts`, e includere esplicitamente nel corpo della richiesta `format: "mp3"`. Per scenari di testi lunghi, si consiglia di utilizzare il callback asincrono `callback_url`; per la scoperta del `reference_id` del tono vocale clone, si prega di utilizzare [Fish Model Query](https://platform.acedata.cloud/documents/fish-model-query) e [Fish Model Get](https://platform.acedata.cloud/documents/fish-model-get).
