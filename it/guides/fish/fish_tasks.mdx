...
Per utilizzare l'API Fish Audios Generation, prima di tutto vai al [Pannello di controllo Ace Data Cloud](https://platform.acedata.cloud/console/applications) per ottenere il tuo API Token, da conservare per uso futuro.
...
L'API Fish Tasks può essere utilizzata per interrogare i risultati dell'API Fish Audios Generation. Per informazioni su come utilizzare l'API Fish Audios Generation, consulta la documentazione [Fish Videos Generation API](https://platform.acedata.cloud/documents/fish-tts-integration).
...
Se la richiesta ha successo, l'API restituirà le informazioni dettagliate di questa attività. Ad esempio:
...
Se la richiesta ha successo, l'API restituirà le informazioni dettagliate di tutte le attività in batch. Ad esempio:
```json
{
  "items": [
    {
