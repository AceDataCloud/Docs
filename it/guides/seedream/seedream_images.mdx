...
Possiamo vedere che abbiamo ottenuto informazioni sull'immagine soddisfacenti, dobbiamo solo ottenere l'immagine SeeDream generata dal link dell'immagine in `data`.
...
Si può vedere che l'effetto generato è il risultato della modifica dell'immagine originale, simile a quanto descritto sopra.
...
Se non si dispone di un indirizzo pubblico per il callback, è possibile non specificare `callback_url`, ma impostare il campo `async` nella richiesta su `true`. In questo caso, l'interfaccia restituirà comunque immediatamente `task_id`, ma non invierà il risultato, sarà necessario portare quel `task_id` per chiamare l'interfaccia `/seedream/tasks` per interrogare lo stato del compito e ottenere il risultato finale.
...
Si può vedere che nel risultato c'è un campo `task_id`, gli altri campi sono simili a quanto descritto sopra, tramite quel campo è possibile realizzare l'associazione del compito.
