...
- `started_at`, `finished_at`: il tempo di inizio e di produzione del risultato per questa richiesta, timestamp Unix (secondi, float).
- `elapsed`: il tempo totale impiegato per questa elaborazione (secondi).

Possiamo vedere che abbiamo ottenuto il risultato della verifica dell'immagine del codice captcha hCaptcha; dobbiamo solo simulare un clic sull'area corrispondente alle coordinate di posizione `box` per superare la verifica.
