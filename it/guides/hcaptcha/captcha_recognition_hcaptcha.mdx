...
Al termine dell'elaborazione verrà restituito `status: ready` e il risultato di riconoscimento `solution` (la struttura dei campi è completamente identica a quella della modalità sincrona):
...
Spiegazione della fatturazione: nella modalità asincrona, la creazione di un compito e la lettura dello stato "in elaborazione" non comportano costi; **il client verrà addebitato una volta al primo successo della lettura dei risultati** (coerente con il comportamento attuale e il prezzo della modalità sincrona). Il server progredirà autonomamente nel compito, ma non addebiterà in anticipo solo perché il backend ha completato prima. Se il compito non ha successo entro 120 secondi, verrà terminato con HTTP 504 `timeout`, senza costi. `/captcha/tasks` non è responsabile per il progresso del compito.
