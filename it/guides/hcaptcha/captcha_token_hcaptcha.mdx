...
Iniziamo a comprendere il modo di utilizzo di base, che consiste nell'inserire l'URL del sito web che deve elaborare il captcha hCaptcha, per ottenere il risultato elaborato. Prima di tutto, è necessario semplicemente passare un campo `website_url`. Il nostro sito di esempio è: `https://accounts.hcaptcha.com/demo`, dobbiamo ottenere il `website_key` dalla pagina `website_url`, per farlo apriamo questa pagina, premiamo F12 per accedere alla console e infine cerchiamo globalmente `hcaptcha-demo` nella pagina Element, possiamo ottenere il seguente risultato:
