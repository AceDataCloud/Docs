...
Per prima cosa, è necessario comprendere il modo di utilizzo di base, che consiste nell'inserire l'URL del sito web che deve elaborare il captcha hCaptcha, per ottenere il risultato elaborato. È necessario semplicemente passare un campo `website_url`, il nostro sito di esempio è: `https://accounts.hcaptcha.com/demo`, dobbiamo ottenere il `website_key` dalla pagina `website_url`, per farlo, apri questa pagina, premi F12 per accedere alla console e infine cerca globalmente `hcaptcha-demo` nella pagina Element, otterremo il seguente risultato:
