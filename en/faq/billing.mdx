...
## Service quota vs global balance — which is used first?

- **Service quota** is allocated when you subscribe to a service package; only usable for that service
- **Global balance** is usable across all services
- Deduction order: **service quota → global balance**

If a credential is bound to a service and global fallback is enabled, exhausting the service quota falls back to the global balance.
