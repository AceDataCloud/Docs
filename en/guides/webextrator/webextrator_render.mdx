...
> The platform contract uniformly uses **snake_case**. Internal rendering services support camelCase, but external calls always use snake_case.
...
| `data.text`            | string         | A snapshot of `document.body.innerText` (use Extract for cleaner body). |
