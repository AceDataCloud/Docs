...
| `enable_llm`      | boolean  |   ❌     | `false`      | Allows LLM extraction when schema.org is not hit. Needs to be enabled to get typed results on pages like Amazon / HN / Greenhouse that lack JSON-LD. |

> When the page has its own schema.org JSON-LD, `enable_llm` is ineffective — the deterministic mapper directly produces results, never wasting LLM calls. You get typed results for **free**.
...
| `description`     | string?   | Priority: `<meta name="description" />` → `og:description` → schema.org / LLM extraction → first paragraph of the text. |
...
| `BreadcrumbList`                                                                                           | (attached to sibling) | Always outputs to `structured.schemaOrg.breadcrumbs[]`, will not be primary.                                                                                                                                 |
...
When the task is complete, it will `POST` the complete envelope to your `callback_url` (if configured). You can also actively query later through [`/webextrator/tasks`](development_webextrator_tasks).
...
- **`enable_llm: true` is free on pages with schema.org hits.** LLM is only called when there is no primary in schema.org, so it's safe to keep it on by default.
- **Check `rawSignals.hasJsonLd` first during debugging.** If it's `true` but `structured.schemaOrg.primary` is `null`, it means the page used a type that our mapper hasn't covered yet — please file an issue, and we will add it.
...
- **Non-article page `links[]` will not be sorted by relevance.** Only filtered by "up to 100 entries + filtering invalid protocols" to the best of our ability.
