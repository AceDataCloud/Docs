...
The `action` determines the generation mode of this request:

- `generate`: Text-to-video, default action, supports `happyhorse-1.0-t2v` and `happyhorse-1.1-t2v`, must pass in `prompt`.
- `image_to_video`: First-frame image-to-video, supports `happyhorse-1.0-i2v` and `happyhorse-1.1-i2v`, must pass in `image_url`.
- `reference_to_video`: Reference image-to-video, supports `happyhorse-1.0-r2v` and `happyhorse-1.1-r2v`, must pass in `prompt` and 1–9 `image_urls`.
- `video_edit`: Video editing, supports `happyhorse-1.0-video-edit`, must pass in `prompt` and `video_url`, can additionally pass in 0–5 reference images `image_urls`.
...
- `data`: List of video results.
  ...
  - `duration`: Billing video duration, in seconds; for `video_edit`, it is the sum of input and output video durations.
...
When using `reference_to_video`, you can pass in 1–9 reference images in `image_urls`. In the prompt, you can reference the corresponding images using `character1`, `character2`, etc.
...
- `400`: Request parameters are incorrect, such as action and model mismatch, missing `prompt` / `image_url` / `video_url`, or `duration` exceeding the 3–15 seconds range.
