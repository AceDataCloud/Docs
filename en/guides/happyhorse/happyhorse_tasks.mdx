...
Upon a successful request, the API will return the details of the task. The `request` field contains the request body when the task was created, and the `response` field contains the response body returned after the task is completed, for example:
...
- `started_at`: The time when the task started executing, Unix timestamp (seconds, float).
- `finished_at`: The time when the task was completed, Unix timestamp (seconds, float). This field is not returned if the task is not completed.
