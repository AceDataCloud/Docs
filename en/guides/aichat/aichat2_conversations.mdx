...
The `tool_use_id` in the request body **must** exactly match the `tool_id` at the time of pause; otherwise, it will return 400. When `tool_results` are present in the request, `question` / `message` / `references` will all be ignored.
...
v2 provides lightweight session management through the `action` field on the same endpoint, without needing to open a separate API.
