...
The values in `allowed_skills` are the slugs of the connected Skills; the values in `allowed_mcp_servers` are the slugs of the connected MCP Servers. Skills / MCP Servers not listed in the pre-authorization will still only be able to preview, dry-run, or refuse to execute write operations in unattended mode.
...
```json
{
  "error": {
    "code": "chat_error",
    "message": "model service LLM returned an error"
  },
