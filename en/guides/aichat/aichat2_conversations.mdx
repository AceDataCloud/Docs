...
Available `model` values can be seen directly in the dropdown on the Try panel on the right, with common categories including:
...
The core enhancement of v2 is that the model can autonomously invoke tools to complete multi-step tasks, **this is enabled by default**, and the client does not need to make any additional configurations in the request. Common scenarios:

- The user asks, "Help me find out what new exhibitions are in Shanghai recently" → the model invokes the built-in web search → organizes the results into a response.
- The user asks, "Read this PDF and then write a summary" → the model invokes file_read → writes a summary.
- The user has authorized Google Drive / GitHub / Notion, etc., in [Connections](https://platform.acedata.cloud/connections) → the model can invoke the corresponding MCP tools to read and write their data.

In NDJSON / SSE streams, tool invocation is presented through two types of events: `tool_use` and `tool_result`, for example:
...
If you do not want to display the details of tool invocation on the front end, you can ignore the `tool_use` / `tool_result` / `card` / `citation` events, and the model's final output will still flow out through `text_delta`.
...
The `tool_use_id` in the request body **must** be exactly the same as the `tool_id` when paused; otherwise, it will return 400. When `tool_results` are present in the request, `question` / `message` / `references` will all be ignored.
...
Returns `{ items: [...], total }`. **Summaries do not include `messages`**, suitable for sidebar lists; if the user opens a conversation, use `action: retrieve` to fetch its full messages separately.
...
`messages` can also be passed, but the server will perform strict schema validation (must be in the folded `ToolUseContent` form), otherwise it returns 400. Generally, it is only recommended to change the `title`.
...
Returns `{ id, success: true }`. Deletion is irreversible, please confirm before calling.
...
2. If you previously used v1 model names (such as `gpt-3.5`, `gpt-4-browsing`, etc.), it is recommended to upgrade to contemporary models (such as `gpt-5.4`, `claude-opus-4-8`, `gemini-3.1-pro-preview`, etc.) when switching to v2.
3. NDJSON stream fields remain backward compatible: each `text_delta` event still carries `delta_answer` and `id`, so clients parsing `delta_answer` line-by-line do not need to change.

After migration, you can enable v2 new features as needed (multimodal `message`, SSE, tool calls, `action` CRUD) and proceed at your own pace.
...
- `404 not_found`: conversation with the given `id` does not exist for `action: retrieve / update / delete`.
- `429 too_many_requests`: rate limit triggered.
- `500 chat_error`: upstream LLM error or current round `completion_tokens=0` (treated as unconsumed, no charge).

In streaming responses, errors are sent as `{"type":"error","message":"..."}` events, immediately followed by stream termination.

## Conclusion

The AI Chat v2 API maintains backward compatibility with v1 while upgrading conversations from "single/multi-turn Q&A" to "Agent-based observable conversations": multimodal input, tool calls, pausable/resumable, streaming structured events, built-in CRUD. It is recommended to use v2 directly for new integrations; existing v1 integrations can migrate smoothly in stages. If you have any questions, please feel free to contact our technical support team.
