...
- **Interruptible / Resumable**: The model will issue an `ask_user_question` event and pause when it needs additional information from the user; the next call can continue by filling in the answer through `tool_results`.
...
Background tasks usually cannot be confirmed by anyone. If you want certain Skills or MCP Servers to perform actions like sending, publishing, writing, etc., in unattended mode, please explicitly pass the pre-authorization list in the request body:
...
Return `{ items: [...], total }`. **The summary does not include `messages`**, suitable for a sidebar list; if the user clicks on a conversation, use `action: retrieve` to fetch its complete messages separately.
