...
**One API Token can call all services on the platform, without needing to apply separately for each service.** The first application will grant a free quota for you to experience it for free; when the quota is insufficient, you can recharge the general balance in the [console](https://platform.acedata.cloud/console/coin).
...
- `task_id`: The ID of the video generation task, which will be used to poll for results via the [Maestro Task Query API](development_maestro_tasks.md).
...
After the task is completed, each language will correspond to a `variant` in the results (see [Maestro Task Query API](development_maestro_tasks.md)).
...
Since video production takes a long time, this interface will immediately return `task_id` after submission, and you need to use it to poll for results via the [Maestro Task Query API](development_maestro_tasks.md):
...
- [Maestro Task Query API Integration Instructions](development_maestro_tasks.md): Use the `task_id` returned by `POST /maestro/videos` to query the task status and results, or to pull the history of task lists (polling is free).
