...
| `limit`            | int      | No       | Number of returns, default 20   |
...
After a successful request, the API will return the historical task list of the current user:
```json
{
  "count": 2,
  "items": [
...
- [Maestro Video Generation API Integration Instructions](development_maestro_videos.md): Automatically produce subtitled videos with a natural language prompt, return `task_id` after submission, and then poll the results using this interface.
