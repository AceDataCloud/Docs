...
This document will provide detailed instructions for integrating the Maestro Task Query API. Since video generation is an asynchronous task, after submission, this interface needs to be polled to obtain progress and the final video, **polling is free and does not consume credits.**
...
- `status`: The task status, which can be `pending → planning → producing → succeeded` (or `failed`). Whether the task is completed is determined by this top-level `status`.
...
- `response`: The return information of the task.
  ...
  - `data.variants`: Each language corresponds to a completed video object, containing `lang`, `aspect`, `title`, `output_url` (download link for the completed video), etc.
...
| `limit`           | int     | No       | Number of items to return, default is 20 |
...
Upon a successful request, the API will return the current user's historical task list:
...
- `items`: An array of tasks, with each element's format consistent with the "query a single task" return result.
