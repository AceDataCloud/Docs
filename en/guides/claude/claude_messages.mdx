...
- `usage`: Token usage statistics. `input_tokens` is uncached input; `cache_creation_input_tokens` and `cache_read_input_tokens` are for cache writing and reading, respectively; `output_tokens` is the number of output tokens. Non-streaming responses may also include `cost` recorded by Ace Data Cloud.
...
The error responses from the public interface use the Ace Data Cloud platform envelope: `error.code` is a stable error code, `error.message` is an explanation, and `trace_id` is used for troubleshooting requests. Common HTTP statuses include:
