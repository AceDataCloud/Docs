...
The Claude Messages Count Tokens API can calculate the number of input Tokens for a Message without actually creating a message, including Token counting for content such as tools, images, and documents. This is very useful when you need to estimate costs or check whether the input exceeds the model context limit.
...
- `model`: Select the Claude model to use, such as the latest flagship `claude-fable-5-1`; the original `claude-fable-5` remains compatible and available.
- `messages`: An array of input messages, where each message contains `role` (role) and `content` (content).
...
- Images, PDFs, tool definitions, system prompts, and thinking configurations are included in the result according to the input rules of the selected model.
