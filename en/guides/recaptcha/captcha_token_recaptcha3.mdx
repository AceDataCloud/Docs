...
First, understand the basic usage method. Compared to Recaptcha2, we need to additionally pass a parameter `page_action`, which needs to be obtained from the code. The demo URL for this presentation is: `https://recaptcha-demo.appspot.com/recaptcha-v3-request-scores.php`. Below is a method to obtain it:
...
Next, you need to input the URL of the website that requires captcha processing to obtain the processed result. First, you need to simply pass a `website_url` field, and finally, you need to input the parameter `website_key`, which can be obtained from the previous text, also a string following execute. We can then fill in the corresponding content on the interface, as shown in the image:
...
- `token`, the verification result after processing the Recaptcha3 verification task.

As we can see, we have obtained the verification result for processing the Recaptcha3 verification, which we can use for POST or GET to simulate submission to the target website, valid for one-time use with a validity period of 120 seconds, and it is recommended to use it within 60 seconds. Below is a brief introduction to a method to submit the generated token to the target website:
...
As we can see, the `success` indicates the processing result of this verification, confirming that we have successfully passed the Recaptcha3 verification.
