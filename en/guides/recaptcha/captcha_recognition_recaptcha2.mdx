...
```json
{
  ...
  "/m/013xlm": "tractors",
  "/m/01jk_4": "chimneys",
  "/m/014xcs": "crosswalks", // pedestrian crossings are the same
...
We can see that the areas start from 0. From the result in `objects`, we obtained 2, 5, and 7, and we only need to simulate clicking on these three areas of the verification code to pass the verification.
...
By default, the API is synchronous and blocking: a request will wait until the recognition result is processed before returning. If you are doing multi-solver rotation and want to "get the task_id immediately after submitting the task, schedule other solvers, and come back later for the result," you can pass `async: true` in the request body.
