...
First, understand the basic usage method, which is to input the URL of the website needing to process the verification code to obtain the processed result. You first need to simply pass a `website_url` field. Our example website is: `https://www.google.com/recaptcha/api2/demo`, and we need to obtain the `website_key` from the `website_url` page. First, open this webpage, press F12 to enter the console, and then perform a global search for `recaptcha-demo` in the Element page, and we can get the following result:
