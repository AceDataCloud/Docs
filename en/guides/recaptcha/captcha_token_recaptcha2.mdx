...
This document will introduce a Recaptcha2 protocol recognition API integration instruction, which allows users to bypass recognizing and clicking on Recaptcha2 verification images, achieving automatic decoding in the background by simply submitting the Website Key.
...
First, understand the basic usage method, which is to input the URL of the website needing to process the verification code to obtain the processed result. You first need to simply pass a `website_url` field. Our example website is: `https://www.google.com/recaptcha/api2/demo`. We need to obtain the `website_key` from the `website_url` page. First, open this webpage, press F12 to enter the console, and then perform a global search for `recaptcha-demo` in the Element page. We can get the following result:
...
- Then click submit, watch the changes in the console's network, as shown in the figure below:

<p><img src="https://cdn.acedata.cloud/6hm5kj.png" width="500" className="m-auto" /></p>

- Analyze the construction of the submitted POST request, and finally, right-click the request to copy the CURL code, as shown in the figure below:
