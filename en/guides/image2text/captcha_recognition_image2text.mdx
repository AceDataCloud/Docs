...
First, understand the basic usage method, which is to input the variable-length English numeric captcha image that needs to be processed to obtain the processed result. You need to simply pass a field called `image`, which is the specific English numeric captcha image, as shown in the figure:
...
We can see that we have obtained the verification result for processing the variable-length English numeric captcha image, and we only need to verify based on the text content in the result.
...
Once processing is complete, it will return `status: ready` and the recognition result `text` (the field structure is completely consistent with the synchronous mode):
