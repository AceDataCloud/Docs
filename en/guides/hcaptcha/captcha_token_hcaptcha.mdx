...
```
We can see that we have obtained the verification result for processing the hCaptcha verification code, which we can use for POST or simulate submission to the target website, for one-time use, with a validity period of 120 seconds, and it is recommended to use it within 60 seconds. Next, a CURL version will be provided to submit the processed token to the target website to pass the Recaptcha2 verification.

First, we need to understand how the website sends the POST request so that we can pass the generated token into it. We need to open the F12 console and manually go through the verification. Finally, we can see that the website sent a POST request. We only need to check the construction of this POST request, and the specific process is as follows:
...
By default, the API is synchronous and blocking: a request will wait until the token processing is complete before returning. If you are doing multi-solver rotation and want to "get the task_id immediately after submitting the task, schedule other solvers, and come back later to read the result," you can pass `async: true` in the request body.
...
If you need to actively check the progress, you can use this `task_id` to query `POST /captcha/tasks` (it is recommended to do this every 3-5 seconds). This interface will not trigger or advance task processing; even if you do not query, lose network connection, or exit the client, the server will continue processing:
...
Billing Explanation: In asynchronous mode, creating tasks and reading the "processing" status are not charged; **the client is charged once when it first reads the successful result** (consistent with existing behavior and synchronous mode pricing). The server will autonomously advance the task, but will not charge in advance just because the background completes first. If the task is not successful within 120 seconds, it will terminate with HTTP 504 `timeout`, and no charge will be incurred. `/captcha/tasks` does not advance the task.
