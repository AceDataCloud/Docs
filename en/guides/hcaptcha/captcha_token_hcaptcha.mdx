...
**One API Token can call all services on the platform without needing to apply separately for each service.** The first application will grant a free quota for trial; when the quota is insufficient, you can recharge the general balance in the [console](https://platform.acedata.cloud/console/coin).

> 📘 Complete Documentation: [hCaptcha Protocol Recognition API →](https://platform.acedata.cloud/documents/captcha-token-hcaptcha)

## Basic Usage

First, understand the basic usage method, which is to input the website URL that needs to process the hCaptcha verification code to obtain the processed result. You first need to simply pass a `website_url` field. Our example website is: `https://accounts.hcaptcha.com/demo`, and we need to obtain the `website_key` from the `website_url` page. First, open this webpage, press F12 to enter the console, and then perform a global search for `hcaptcha-demo` on the Element page. We can get the following result:
...
- `token`, the verification result after processing the hCaptcha task.
