...
When you want to edit an existing image, set `action` to `edit`, and pass a list of image URLs to be edited via `image_urls` (1 or more), while also providing a `prompt` describing the editing goal.
