---
title: "Producer Lyrics Generation API Integration Guide"
description: "Producer Music Generation Integration Guide - Ace Data Cloud"
---

Producer allows us to generate lyrics through prompts. This document explains the integration method for the related API.

This API has only one input parameter, which is `prompt`, the prompt for generating lyrics.

Here, the input `prompt` is `A song about winter`, which is the prompt for generating lyrics.
...
As you can see, the `lyrics` field in `data` is the generated lyrics content.