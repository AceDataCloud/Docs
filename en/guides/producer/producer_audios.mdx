...
To use the Producer Audios Generation API, first go to the [Ace Data Cloud Console](https://platform.acedata.cloud/console/applications) to obtain your API Token for future use.
...
If you have a song in mind, you can input any text, for example, if I want to generate a song about Christmas, I can input `a song for Christmas`, as shown in the image:
...
- `model`: the model used to create the song, currently mainly: **FUZZ-2.0 Pro**, **FUZZ-2.0**, **FUZZ-2.0 Raw**, **FUZZ-1.1 Pro**, **FUZZ-1.0 Pro**, **FUZZ-1.0**, **FUZZ-1.1**, **FUZZ-0.8**.
- `lyric`: the content of the song's lyrics.
...
- `audio_id`: the reference song ID, used for continuation/revision operations.
- `continue_at`: continue writing the song from a specified second.
- `replace_section_start`/`replace_section_end`: the start and end time of the replacement segment, in seconds.
- `lyrics_strength`: the strength of the lyrics, indicating the degree of influence of the lyrics in audio generation, optional between 0-1, default 0.7.
- `sound_strength`: the strength of the audio prompt, optional between 0.2-1, default 0.7.
- `cover_strength`: the cover strength between 0.2-1, default 1.
- `weirdness`: the uniqueness of the style, optional between 0-1, default 0.5.
...
Click the "Try" button to test, as shown in the image above, we got the following result:
...
We can see that we have obtained the music information we want to generate, and we only need to access the generated Producer music through the music link address in the `data` result.
...
Next, we need to customize the song generation based on the lyrics, title, and style, and we can specify the following content:
- lyric: Lyric text
...
If you want to perform a cover operation on an already generated song, you can input the song generated above for covering, and next we will customize the song generation based on the lyrics and title.

If you want to continue covering a song you uploaded, you can set the parameter `action` to `upload_cover` and input the ID of the song you need to continue covering. The song ID can be obtained using the [Producer Upload API](https://platform.acedata.cloud/documents/producer-upload), as shown in the image below:
...
If you want to continue writing the song, we need to pass the `action` parameter value as: `extend`, then we need to customize the generation of the song based on the lyrics, title, and style.

If you want to continue writing a song you uploaded, you can set the `action` parameter to `upload_extend` and input the ID of the song you need to continue writing. The song ID can be obtained using the [Producer Upload API](https://platform.acedata.cloud/documents/producer-upload), as shown in the image below:
...
If you want to generate a new piece of music with a similar effect based on the seed value of the previous music, we need to pass the `action` parameter value as: `variation`, then we can customize the generation of a new piece of music with a similar effect.
...
If you want to continue replacing a section of the song you uploaded, you can set the `action` parameter to `upload_replace_section` and input the ID of the custom uploaded song you want to continue writing. The song ID can be obtained using the [Producer Upload API](https://platform.acedata.cloud/documents/producer-upload), as shown in the image below:
...
If you want to use the official accompaniment version operation, you can set the `action` parameter value to: `swap_instrumentals`, then we will customize the song generation based on the lyrics and title.

The sample to fill out is as follows:
...
If you want to use the official vocal version operation, you can set the `action` parameter value to: `swap_vocals`, then we will customize the song generation based on the lyrics, title, and style.
...
The sample for filling out is as follows:
...
Since the Producer Audios Generation API can sometimes take a relatively long time to generate, if the API does not respond for a long time, the HTTP request will keep the connection open, leading to additional system resource consumption. Therefore, this API also provides support for asynchronous callbacks.
...
First, the Webhook callback is a service that can receive HTTP requests, and developers should replace it with the URL of their own HTTP server. For demonstration purposes, we use a public Webhook sample website [https://webhook.site/](https://webhook.site/). Open this website to get a Webhook URL, as shown in the image:
...
After a moment, we can observe the result of the generated task at [https://webhook.site/#!/view/0d73431d-f833-4be4-9276-b6e1690d55c1](https://webhook.site/#!/view/0d73431d-f833-4be4-9276-b6e1690d55c1), as shown in the image:
