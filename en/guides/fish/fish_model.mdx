...
To use the Fish Model API, first obtain your API Token from the [Ace Data Cloud Console](https://platform.acedata.cloud/console/applications) for future reference.
...
`_id` can be used as the value of the `reference_id` field in the subsequent [Fish TTS API](https://platform.acedata.cloud/documents/fish-tts) for voice synthesis using this cloned voice:
...
To call this interface, you only need to prepare a publicly accessible sample audio URL and place it in the `voices` field to obtain the `_id`, then feed it to the `reference_id` of `/fish/tts` to complete the end-to-end "clone → synthesis." A common pitfall is that `voices` must be a string, not an array.
