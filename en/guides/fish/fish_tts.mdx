...
> Field names are completely consistent with upstream. Except for `callback_url`, other fields' meanings and values refer to the [Fish Official TTS Documentation](https://docs.fish.audio/text-to-speech/text-to-speech).
...
`audio_url` points to the platform CDN, which can be directly downloaded via GET or played in `<audio>`. The final successful response will also return the top-level `cost`, where `amount` is the actual Credits deducted this time; if there is an account discount, `list_amount` indicates the amount before the discount. The link is long-term available, but it is still recommended to keep a copy in your own storage.

## Example 2: Using Cloned Voice `reference_id`

Below is an example using a public Spanish voice on the Fish platform (the `_id` can be retrieved via [Fish Model Query](https://platform.acedata.cloud/documents/fish-model-query)):
...
## Example 3: Adjusting Speed / Volume (`prosody`)
...
You can also actively pull results by `task_id` using the [Fish Tasks API](https://platform.acedata.cloud/documents/fish-tasks); the `response.cost` in the final state record is consistent with the `cost` in the callback, see that document for details.

## Error Handling

- `400 token_mismatched`: Request parameters are missing or invalid (most commonly, `text` is empty, or `format` has a value other than `mp3`/`wav`/`pcm`).
...
Parameter validation errors will place the upstream pydantic error message in the `message` field, making it easier to locate which field is invalid, for example:
