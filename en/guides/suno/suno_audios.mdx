...
> The `lyric` and `style` limits in the table above are the upper limits under the custom mode (`custom` is `true`). The non-custom inspiration mode (`custom` is `false`) only fills in the `prompt`, with a length limit of 500 characters (consistent across models).

Suno now supports `chirp-v6`, `chirp-v6-wild`, and `chirp-v6-mini`. It is recommended to use `chirp-v6`; the old model names remain compatible.

However, the official Suno does not provide an API. AceDataCloud offers a set of Suno APIs that simulate the official Suno integration, making it convenient and quick to generate desired music.

## Application and Usage

To use the Suno Audios Generation API, first go to the [Ace Data Cloud Console](https://platform.acedata.cloud/console/applications) to obtain your API Token for backup.

![](https://cdn.acedata.cloud/dvc3cg.jpg)

If you are not logged in or registered, you will be automatically redirected to the login page inviting you to register and log in, after which you will automatically return to the current page.

**One API Token can call all services on the platform, no need to apply separately for each service.** The first application will grant free credits for a free experience; when credits are insufficient, you can recharge the general balance in the [console](https://platform.acedata.cloud/console/coin).
...
- `action`: The action for this music generation task, default is `generate`, mainly includes: `extend`, `upload_extend`, `cover`, `upload_cover`, `replace_section`, `replace_section`, `concat`, `stems`, `all_stems`, `remaster`.
- `prompt`: The official inspiration mode prompt from Suno (effective when `custom` is `false`), maximum 500 characters.
- `model`: The model for this music generation task. The v6 series includes `chirp-v6`, `chirp-v6-wild`, and `chirp-v6-mini`; old model names remain compatible.
- `lyric`: The lyrics content for Suno's official custom mode. `chirp-v3-5` and `chirp-v4` maximum 3000 characters; `chirp-v4-5` and above (including `chirp-v5`, `chirp-v5-5`) maximum 5000 characters.
...
- `title`: The music title for Suno's official custom mode. `chirp-v3-5`, `chirp-v4` maximum 80 characters; `chirp-v4-5` and above maximum 100 characters.
- `style`: The music style for Suno's official custom mode. `chirp-v3-5` and `chirp-v4` maximum 200 characters; `chirp-v4-5` and above (including `chirp-v5`, `chirp-v5-5`) maximum 1000 characters.
- `negative_tags`: Music styles or genres to exclude from the generated results in custom mode (`custom` is `true`).
...
- `continue_at`: The boundary for continuation, in seconds. For example, 213.5 means starting to generate subsequent segments from 3 minutes and 33.5 seconds. `lyric` and `style` only guide new content after the boundary and will not replace the lyrics or singing before the boundary in the source audio.
...
- `duration`: The expected song length, in seconds, must be an integer, range 10 to 360. This parameter is used for song generation in custom mode (`custom` is `true`). It is a tendency prompt rather than a hard constraint: the model will reference it but does not guarantee to meet it, the actual product length is based on the `duration` field in the response, usually shorter than the expected value.
...
- `async`: Optional, set to `true` for the interface to immediately return `task_id`, no need to provide `callback_url`, then poll the corresponding task query interface to get results.
...
- data: A list that contains detailed information about the generated songs.
  ...
  - audio_url: Final audio address of the song. The platform will prioritize returning the Ace Data Cloud CDN address; if persistence fails, it may return the original media address, please download it in time.
  - video_url: Video file of the song, opening it is an mp4 video.
...
> Note that the `\n` in the lyrics is a newline character. If you do not know how to generate lyrics, you can use the lyrics generation API provided by Ace DataCloud to generate lyrics through the prompt. The API is [Suno Lyrics Generation API](https://platform.acedata.cloud/documents/suno-lyrics).

Next, we will customize the generation of songs based on lyrics, title, and style, and you can specify the following content:
...
If you want to continue generating an already created Suno song, you can set the parameter `action` to `extend`, and input the ID of the song to continue generating. The song ID can be obtained based on the basic usage, as mentioned above, where you can see the song ID is:
...
Next, you must fill in the lyrics of the continuation segment and can specify the style:

- lyric: used only to guide the new generated segment's lyrics after `continue_at`, and will not replace the lyrics before that time point in the source audio.
- custom: set to `true` for custom generation; this parameter defaults to false, which means using `prompt` for generation.
- style: the style of the continuation segment's song, optional.
- continue_at: the boundary for continuation, in seconds. For example, 213.5 means starting to generate the subsequent segment from 3 minutes and 33.5 seconds.

> `extend` is used to continue creating from an existing song, not to replace the entire song's lyrics. If you want to use new lyrics for the entire song from the beginning, please use `generate` to regenerate; if you want to reinterpret an existing song, you can use `cover`. When passing a whole new set of lyrics, the original lyrics before `continue_at` will still not be replaced.
...
The following is a real historical call snapshot, where `continue_at` is set to 2 seconds, so only the first 2 seconds belong to the original content before the continuation boundary. When actually approaching the end of the song for continuation, this value should be set to the expected start time for continuation, and only the new paragraph to be sung after the boundary should be provided in `lyric`.
...
It can be seen that the `lyric` in the result will return the lyrics text used for this continuation task. This field is not a verbatim transcription of the complete finished audio; for `extend`, the source audio before `continue_at` still uses the original lyrics, and the new lyrics are only used to guide the continuation segment.

## Get the Complete Song

The current model's `extend` results usually already include the source audio before `continue_at` and the new content after the boundary, so first determine whether it is already a complete song based on the actual duration and content of the returned audio. If the returned audio is an independent continuation segment, or if multiple continuation histories need to be explicitly merged into one song, then use the concatenation function:
...
Other parameters remain unchanged, and the returned result will be a complete song, which is the concatenation result of all song segments, but the result will only be one song, as shown in the example below:
```json
{
  "success": true,
...
When generating a song based on an existing song, the style of the returned song may not be suitable. If you want to create a cover of the previously generated song (custom uploaded music is also supported), you need to use the music cover method, which allows you to specify the following content:

- action: The content is `cover`. When performing a cover operation on custom uploaded music, it must be specified as: `upload_cover`.
...
Other parameters remain unchanged, and the returned result will be a cover of the previously generated song, as shown below:
...
The generated result is similar to the above, completing the process of generating a cover of the previously generated song.

## Replace Section

When a song is generated and you need to perform a secondary creation by replacing a specific section of the song, you can replace a segment of the song.

> ⚠️ **Note**: `replace_section_result_mode` defaults to `full_song`: the system will concatenate 2 new generated candidates separately and return 2 complete songs. If you only need the unspliced replacement segment candidates, please explicitly set it to `candidates`, and then select the candidates to call [Music Splicing](#Music-Splicing).
...
- replace_section_result_mode: Return mode, default is `full_song`. `full_song` concatenates 2 candidates separately and returns 2 complete songs; `candidates` returns 2 unspliced candidate segments.

### Step 1: Initiate Replace Section Task

For example, if the ID of the previously generated song is: 18db7ed0-2b8a-41db-91c1-b0781dcca0d4 (duration 94.12 seconds), and you want to replace the chorus from the 30th second to the 60th second with new lyrics, you can set the parameters as follows:
```json
{
...
如果不需要比较两个候选，可以在步骤一把 `replace_section_result_mode` 设为 `full_song`。接口会自动选择第一个候选完成拼接，并直接返回包含 1 首完整歌曲的 `data` 数组；此时无需再调用 `concat`，费用中会包含自动拼接步骤。
...
```
At this point, the `duration` has been restored to the full song length (105.28 seconds, approximately equal to the original song length), and the `audio_url` points to the completed song after replacement. This completes the secondary creation process of "generation → replacement of segments → splicing the entire song."

## Vocal and Instrument Separation

After generating the song, if separate operations for accompaniment and vocals are needed, pure instrumental accompaniment and clean vocals can be separated. The following content can be specified:

- action: content is `stems`.
- audio_id: the ID of the previously generated song.

For example, if the ID of the previously generated song is: ec13e502-d043-4eb2-92ee-e900c6da69d1, the parameters can be set as follows:
...
The generated result is similar to the above, completing the process of vocal and instrumental separation for the previously generated song.

## Full Track Vocal and Instrument Separation

When full track vocal and instrumental separation is needed after generating the song, the following content can be specified:

- action: content is `all_stems`.
- audio_id: the ID of the previously generated song.

For example, if the ID of the previously generated song is: bdf23a5a-59f5-4103-b452-054a824a7f9f, the parameters can be set as follows:
...
The generated result is similar to the previous text, completing the process of voice and music separation for the originally generated song.
...
It is important to note that the `duration` in the request is the **expected duration**, while the `duration` field of each song in the response `data` is the **actual duration** of that song. Both have the same name but different meanings, and the actual duration is not guaranteed to equal the expected value. The length of the lyrics is one of the main factors affecting the duration of the product; if a longer product is needed, it is recommended to provide more complete lyrics as well.

The interface will not perform additional validation on `duration`, and the parameter will be passed to the model as is. If unsupported values for the current mode or model are passed, it may result in that value being ignored. It is recommended to confirm the effect with a single request before using it in bulk.

## Add Instrumental Function

In August 2025, Suno introduced the Add Instrumental function. First, you need to upload a song with only vocals and no accompaniment, allowing Suno to provide the music. You can first go to the [Suno Upload API](https://platform.acedata.cloud/documents/suno-upload) to upload a song with only vocals, corresponding to the operations shown in the image below:
...
Finally, you will get an `audio_id`: 92254cab-3372-4d9e-bce9-cdcfdbc39070, and then we also need to fill in the following parameters:
```json
{
  "success": true,
...
In December 2025, Suno released the new Remaster function, which can regenerate songs, not cross-account, and we also need to fill in the following parameters:

- action: content is `remaster`.
- audio_id: ID of the song to be regenerated.
...
In December 2025, Suno released the new Mashup function, which can generate songs based on two reference songs, and we also need to fill in the following parameters:

- action: content is `mashup`.
- mashup_audio_ids: IDs of the two reference songs.
...
This completes the operation of generating a mashup from reference songs, with results similar to the above.
...
The overall process is: when the client initiates a request, an additional `callback_url` field is specified. After the client initiates the API request, the API will immediately return a result containing a `task_id` field, representing the current task ID. When the task is completed, the result of the generated music will be sent to the client-specified `callback_url` in the form of a POST JSON, which also includes the `task_id` field, allowing the task result to be associated by ID.
...
Clicking run, we can find that we immediately get a result, as follows:
...
We can see that the result contains a `task_id` field, and the other fields are similar to the previous text. The task can be associated through this field.
...
The result obtained is similar to the basic call, and multiple results above achieve streaming calls.
...
> Note: Different upstream account limits and error messages may vary. Generally, `chirp-v3-5`/`chirp-v4` has lower `style` restrictions (200), while `chirp-v4-5` and above usually support up to 1000; when hitting old upstreams, messages like `Tags too long.` or `style must be less than or equal 120` may appear.
