...
If you are not logged in or registered, you will be automatically redirected to the login page inviting you to register and log in, and after completion, you will automatically return to the current page.
...
> Note that the `\n` in the lyrics is a newline character. If you do not know how to generate lyrics, you can use the lyrics generation API provided by AceDataCloud to generate lyrics through the prompt. The API is [Suno Lyrics Generation API](https://platform.acedata.cloud/documents/suno-lyrics).
...
> `extend` is used to continue creating from an existing song, not to change the lyrics of the entire song. If you want to use new lyrics for the entire song from the beginning, please use `generate` to regenerate; if you want to reinterpret an existing song, you can use `cover`. When passing a complete new set of lyrics, the original lyrics before `continue_at` will still not be replaced.
...
The current model's `extend` results usually already include the source audio before `continue_at` and the new content after the boundary, so first determine whether it is already a complete song based on the actual duration and content of the returned audio. If the return is an independent continuation segment, or if multiple continuation histories need to be explicitly merged into one song, then use the concatenation function:
...
Other parameters remain unchanged, and the return will be a complete song, which is the concatenation result of all song segments, but the result will only be one song, as shown in the example below:
```json
{
  "success": true,
...
When a song is generated and you need to perform a separate operation to replace a section of the song, you can replace a specific section of the song.

> ⚠️ **Note**: `replace_section_result_mode` defaults to `full_song`: the system will concatenate 2 newly generated candidates separately and return 2 complete songs. If you only need the unstitched replacement section candidates, please explicitly set it to `candidates`, and then select the candidates to call [Music Splicing](#music-splicing).
...
- replace_section_result_mode: Return mode, default is `full_song`. `full_song` concatenates 2 candidates separately and returns 2 complete songs; `candidates` returns 2 unstitched candidate sections.
...
```
At this point, the `duration` has been restored to the full song length (105.28 seconds, approximately equal to the original song length), and the `audio_url` points to the completed song after the replacement. This completes the secondary creation process of "generation → replacement of segments → splicing the whole song."
...
For example, if the ID of the previously generated song is: ec13e502-d043-4eb2-92ee-e900c6da69d1, then the parameters can be set as follows:
...
With the above parameters, the result of the vocal and instrumental separation can be obtained, as follows:
...
This parameter is used for song generation in custom mode (`custom` is `true`). It should be noted that `duration` is a **preference hint, not a hard constraint**: the model will refer to this value during creation, but it does not guarantee to achieve it. In practice, the actual duration is often significantly shorter than the expected value, and the durations of two songs returned in the same request may also differ by several times. Even for completely identical requests, the durations obtained from multiple submissions may vary greatly. Therefore, do not treat it as precise duration control; if a fixed duration is required for business purposes, please trim or retry after obtaining the product.
...
The interface will not perform additional validation on `duration`, and the parameters will be passed to the model as is. If unsupported values for the current mode or model are passed, it may result in that value being ignored. It is recommended to confirm the effect with a single request before using it in bulk.
...
In December 2025, Suno released a new Remaster function, which can regenerate songs, not cross-account, and we also need to fill in the following parameters:
...
In December 2025, Suno released a new Mashup function, which can generate songs based on two reference songs, and we also need to fill in the following parameters:
...
In December 2025, Suno released the Samples feature, which can generate songs based on two reference songs, and we also need to fill in the following parameters:
...
As we can see, the result contains a `task_id` field, and the other fields are similar to the previous text. This field allows for task association.
