...
With the widespread application of AI, various AI programs have gradually become popular. AI has increasingly penetrated all aspects of people's work and life. The industries involving AI are also growing, from initial writing, to medical education, and now to music.

Suno is a professional high-quality AI song and music creation platform. Users only need to input simple text prompts to generate songs with vocals based on genre style and lyrics. This AI music generator is developed by team members from well-known tech companies such as Meta, TikTok, Kensho, aiming to allow everyone to create beautiful music without any musical instrument tools.

Below is the progress of model updates:
...
However, Suno official does not provide an API. AceDataCloud offers a set of Suno APIs that simulate the official Suno, enabling convenient and fast generation of desired music.
...
You can input any text for the kind of song you want. For example, if I want to generate a song about Christmas, I can input `a song for Christmas`, as shown in the figure:
...
- `accept`: the format of the response you want to receive, here filled as `application/json`, i.e., JSON format.
...
- `lyric`: The lyrics content of the custom mode from Suno. Maximum 3000 characters for `chirp-v3-5` and `chirp-v4`; maximum 5000 characters for `chirp-v4-5` and above (including `chirp-v5`, `chirp-v5-5`).
- `custom`: Whether to use the custom mode, default is: `false`.
- `instrumental`: The pure music option of the inspiration mode from Suno.
- `title`: The music title of the custom mode from Suno. Maximum 80 characters for `chirp-v3-5`, `chirp-v4`; maximum 100 characters for `chirp-v4-5` and above.
- `style`: The music style of the custom mode from Suno. Maximum 200 characters for `chirp-v3-5` and `chirp-v4`; maximum 1000 characters for `chirp-v4-5` and above (including `chirp-v5`, `chirp-v5-5`).
...
- `overpainting_start`/`overpainting_end`: The start and end time for adding vocals to existing pure music, in seconds.
- `underpainting_start`/`underpainting_end`: The start and end time for adding accompaniment to a cappella, in seconds.
...
- `async`: Optional, set to `true` for the interface to immediately return `task_id`, no need to provide `callback_url`, and then poll the corresponding task query interface to get results.
...
> `extend` is used to continue creating from an existing song, not to replace the entire song's lyrics. If you want the entire song to use new lyrics from the beginning, please use `generate` to regenerate; if you want to reinterpret an existing song, you can use `cover`. When passing a complete new set of lyrics, the original lyrics before `continue_at` will still not be replaced.
...
The following is a real historical call snapshot, where `continue_at` is set to 2 seconds, so only the first 2 seconds belong to the original content before the continuation boundary. When actually continuing close to the end of the song, this value should be set to the expected start time for continuation, and only the new paragraphs to be sung after the boundary should be provided in `lyric`.
...
When continuing to generate a song based on the original song, the style of the returned song may not be suitable. If you want to create a cover of the previously generated song (custom uploaded music is also supported), you need to use the music cover method, which allows you to specify the following content:
...
Other parameters remain unchanged, and the returned result will be a cover of the previously generated song, as shown in the example below:
...
When a song is generated and you need to perform a separate operation to replace a section of the song, you can replace a certain section of the song.

> ⚠️ **Note**: When `replace_section` is used alone, it **will only return the new generated "replacement section" itself** (i.e., the new audio for the replaced section, with a duration approximately equal to `replace_section_end - replace_section_start`, and accompanied by a small amount of context), **and will not return the complete song that has been spliced together**. To obtain the complete product spliced with the original song, you need to initiate a [music splicing](#music-splicing) task again based on the returned section ID after `replace_section` is successful. The complete process is detailed below.
...
- replace_section_start: The start time (in seconds) of the section being replaced in the original song.
- replace_section_end: The end time (in seconds) of the section being replaced in the original song.
...
The returned new generated replacement segments (a total of 2 candidates) are as follows:
...
As can be seen, the durations of the two returned audio segments (45.16 seconds, 33.8 seconds) are much shorter than the original song (94.12 seconds), and they are the replacement segments themselves (including a little bit of context before and after for transition), and **are not** the entire song spliced together. You can choose one of the two candidates that you are satisfied with, and the next step will be to splice this segment back into the original song.

### Step Two: Splicing the Replacement Segment Back into the Original Song

For the selected segment (for example, `364f9d8b-ca25-463b-9a5e-d0b7139e2d6a`), initiate a `concat` task according to the method in the [Music Splicing](#音乐拼接) section:
...
At this point, the `duration` has been restored to the full song length (105.28 seconds, approximately equal to the original song length), and the `audio_url` points to the completed song after the replacement. This completes the "generation → replacement segment → splicing the complete song" creative process.

## Voice and Music Separation

When the song is generated and needs to be operated separately for accompaniment and vocal in secondary creation, you can separate the pure music accompaniment and the clear vocal. You can specify the following content:
...
By using the above parameters, the result of vocal separation can be obtained, as follows:
...
Using the above parameters, the result of full track vocal separation can be obtained, as follows:
...
The generated result is similar to the above, completing the process of separating the original generated song into different tracks.
...
The range of advanced parameters is between 0-1, as shown in the figure below:
...
This completes the operation of adding music to the uploaded acapella song, with results similar to the above.

## Add Vocals Feature

In August 2025, Suno released the new Add Vocals feature. First, you need to upload a pure instrumental track for Suno to fill in the lyrics and provide vocal singing. You can first go to the [Suno Upload API](https://platform.acedata.cloud/documents/suno-upload) to upload an acapella song without accompaniment, as shown in the following operation:
...
- audio_id: The ID of the uploaded acapella song.
...
In December 2025, Suno released the Remaster feature, which can regenerate songs and cannot cross accounts. Then you need to fill in the following parameters:
...
As can be seen, the result contains a `task_id` field, and other fields are similar to the above text, allowing the task to be associated through this field.
