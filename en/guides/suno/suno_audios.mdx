---
title: "Suno Song Generation API Integration Guide"
description: "Suno Music Generation integration guide - Ace Data Cloud"
---

As AI applications become more widespread, various AI programs have gradually become popular. AI has gradually penetrated every aspect of people's work and lives. The industries involved in AI are also becoming increasingly diverse, from initial writing, to medical care and education, and now to music.

Suno is a professional, high-quality AI song and music creation platform. Users only need to enter simple text prompts to generate songs with vocals based on genre styles and lyrics. This AI music generator was developed by team members from well-known technology companies such as Meta, TikTok, and Kensho, with the goal of allowing everyone to create wonderful music without requiring any musical instruments or tools.
...
> The `lyric` and `style` limits in the table above are the upper limits in custom mode (when `custom` is `true`). In non-custom inspiration mode (when `custom` is `false`), only `prompt` is filled in, and its length limit is 500 characters (consistent across all models).
...
However, Suno officially does not provide an API. AceDataCloud provides a set of Suno APIs that simulate integration with the official Suno service, allowing you to generate the music you want conveniently and quickly.
...
If you have not yet logged in or registered, you will automatically be redirected to the login page to register and log in, and you will automatically return to the current page after completion.

**One API Token can call all platform services; there is no need to apply separately for each service.** A free quota will be granted upon first application, allowing you to try it for free; when the quota is insufficient, you can recharge the general balance in the [Console](https://platform.acedata.cloud/console/coin).

> 📘 Full documentation: [Suno Audios Generation API →](https://platform.acedata.cloud/documents/suno-audios)

## Basic Usage

For whatever song you want, you can enter any text. For example, if I want to generate a song about Christmas, I can enter `a song for Christmas`, as shown in the image:
...
- `accept`: The format of the response result you want to receive. Here it is filled in as `application/json`, which is JSON format.
- `authorization`: The key for calling the API. After applying, you can directly select it from the dropdown.

Additionally, the Request Body is set, including:
- `action`: The action for this music generation task, defaults to `generate`, mainly includes: `extend`, `upload_extend`, `cover`, `upload_cover`, `replace_section`, `replace_section`, `concat`, `stems`, `all_stems`, `remaster`.
- `prompt`: The prompt for Suno's official inspiration mode (effective when `custom` is `false`), with a maximum of 500 characters.
- `model`: The model for this music generation task. The v6 series includes `chirp-v6`, `chirp-v6-wild`, and `chirp-v6-mini`; legacy model names remain compatible.
- `lyric`: The lyrics content for Suno's official custom mode. `chirp-v3-5` and `chirp-v4` have a maximum of 3000 characters; `chirp-v4-5` and above (including `chirp-v5`, `chirp-v5-5`) have a maximum of 5000 characters.
- `custom`: Whether to use custom mode, defaults to: `false`.
- `instrumental`: The instrumental music option for Suno's official inspiration mode.
- `title`: The music title for Suno's official custom mode. `chirp-v3-5` and `chirp-v4` have a maximum of 80 characters; `chirp-v4-5` and above have a maximum of 100 characters.
- `style`: The music style for Suno's official custom mode. `chirp-v3-5` and `chirp-v4` have a maximum of 200 characters; `chirp-v4-5` and above (including `chirp-v5`, `chirp-v5-5`) have a maximum of 1000 characters.
- `negative_tags`: Music styles or genres to exclude from the generated results in custom mode (when `custom` is `true`).
- `audio_weight`: The proportion of the uploaded reference audio, ranging from 0 to 1; the larger it is, the more the output relies on the reference audio.
- `audio_id`: The ID of the reference music.
- `overpainting_start`/`overpainting_end`: The start and end times for adding vocals to existing instrumental music, in seconds.
- `underpainting_start`/`underpainting_end`: The start and end times for adding accompaniment to a cappella vocals, in seconds.
- `persona_id`: The artist's song ID.
- `continue_at`: The continuation boundary, in seconds. For example, 213.5 indicates generating subsequent segments starting from 3 minutes and 33.5 seconds. `lyric` and `style` only guide new content after the boundary and will not replace lyrics or singing before the boundary in the source audio.
- `style_influence`: The advanced "Style Influence" parameter in custom mode, ranging from 0 to 1; the larger it is, the more closely it matches the selected style.
- `replace_section_end`: The final time of the replacement segment.
- `replace_section_start`: The start time of the replacement segment.
- `vocal_gender`: Controls the preference for female or male vocals: female vocals `f`, male vocals `m`; effective for models 4.5 and above; this is a preference and strict adherence is not guaranteed.
- `weirdness`: The advanced "Weirdness" parameter in custom mode, ranging from 0 to 1; the larger it is, the more creative and experimental it is.
- `duration`: The desired song duration, in seconds, must be an integer, ranging from 10 to 360. This parameter is used for song generation in custom mode (when `custom` is `true`). It is a tendency prompt rather than a hard constraint: the model will refer to it but does not guarantee reaching it. The actual finished duration is subject to the `duration` field in the response and is usually shorter than the desired value.
- `lyric_prompt`: The prompt for generating lyrics, effective if and only if `custom` is `true` and `lyric` is not provided.
- `callback_url`: The URL that needs to receive callback results.
- `async`: Optional. When set to `true`, the API immediately returns `task_id`; there is no need to provide `callback_url`, and the result can subsequently be obtained by polling through the corresponding task query API.
...
- data: A list containing detailed information about the generated songs.
  ...
  - video_url: The video file of the song; opening it displays an mp4 video.
...
If you want to generate custom lyrics, you can enter lyrics:

At this point, the `lyric` field can pass in content similar to the following:
...
Next, to custom-generate a song based on the lyrics, title, and style, you can specify the following content:
...
Finally, you need to set this song as a singer style, then you need to enter the [Suno Persona API](https://platform.acedata.cloud/documents/suno-persona) to generate a singer style ID parameter `persona_id` based on the music ID `audio_id` generated officially. The specific parameters are shown in the image below:
...
We use the above `audio_id` and `persona_id`, namely `97efc9f4-0e8d-4b3e-88df-14568fa1b11f` and `e0d7319e-aa2a-44cb-b00a-916218d7cb0b`, respectively, as the example data for this time. Then, you can set the parameter `action` to `artist_consistency` (if it is the new singer style Persona-v2-vox, `action` must be set to `artist_consistency_vox`), and enter the ID of the song that needs to continue being generated and the singer style ID. An example is filled in as follows:
...
If you want to continue generating an already generated Suno song, you can set the parameter `action` to `extend`, and enter the ID of the song that needs to continue being generated. The song ID is obtained through the basic usage. As can be seen above, at this time you can see that the song ID is:
...
If you want to continue generating your own uploaded song, you can set the parameter `action` to `upload_extend`, and enter the ID of the custom uploaded song that needs to continue being generated. The song ID is obtained using the [Suno Upload Generation API](https://platform.acedata.cloud/documents/suno-upload), as shown in the image below:
...
- continue_at: The continuation boundary, in seconds. For example, 213.5 indicates that subsequent segments are generated starting from 3 minutes and 33.5 seconds.

> `extend` is used to continue creating after an existing song, not to replace the lyrics of the entire song. If you want to use new lyrics for the entire song from the beginning, please use `generate` to regenerate it; if you want to reinterpret it with reference to an existing song, you can use `cover`. When passing in an entire set of new lyrics, the original lyrics before `continue_at` will still not be replaced.

An example is filled in as follows:
...
It can be seen that `lyric` in the result returns the lyrics text used by this continuation task. This field is not a verbatim transcription of the complete finished audio; for `extend`, the source audio before `continue_at` still uses the original lyrics, and the new lyrics are only used to guide the continuation segment.

## Obtain the Complete Song
...
- audio_id: The ID of the final continuation segment.
...
Other parameters remain unchanged. What is returned is one complete song, which is the concatenation result of all song segments, but the result contains only one song, as shown in the following example:
```json
{
  "success": true,
...
After continuing to generate a song based on the original song, the style of the returned song may not be quite suitable. If you want to create a cover of the previously generated song (custom-uploaded music is also supported), you need to use the music cover method, where you can specify the following content:
...
For example, if the ID of the previously generated song is: 0a1e1b10-c36a-41c9-9bfb-b26d9d25db98, then the parameters can be set as follows:
...
The other parameters remain unchanged, and what is returned is a covered song, which is the result of creating a cover of the previously generated song, as shown in the following example:
...
The generated result is similar to the above, which completes the process of generating a cover of the previously generated song.

## Replace Section

When secondary creation requires a separate operation to replace a song section after generating a song, a section of the song can be replaced.

> ⚠️ **Note**: `replace_section_result_mode` defaults to `full_song`: the system will separately concatenate 2 newly generated candidates and return 2 complete songs. If only the replacement section candidates that have not yet been concatenated are needed, explicitly set it to `candidates`, then select a candidate and call [Music Concatenation](#音乐拼接).
...
- replace_section_result_mode: Return mode, defaults to `full_song`. `full_song` separately concatenates 2 candidates and returns 2 complete songs; `candidates` returns 2 candidates that have not yet been concatenated.

### Step 1: Initiate a Replace Section Task

For example, if the ID of the previously generated song is: 18db7ed0-2b8a-41db-91c1-b0781dcca0d4 (duration 94.12 seconds), and you want to replace the chorus from 30 seconds to 60 seconds with new lyrics, then the parameters can be set as follows:
```json
{
  ...
  "custom": false,
  "instrumental": false,
  "lyric": "[Intro]\nGongs and drums resound, red lanterns hang high\n[Verse 1]\nThe sound of firecrackers bids farewell to the old year\nThe warm spring breeze enters thousands of homes\nRed envelopes for New Year bring smiling faces\nThe golden snake dances to celebrate the Lunar New Year\n[Chorus]\nPlum blossoms bloom, the spring spirit overflows across the ground\nPlum blossoms bloom, the spring spirit overflows across the ground\nPlum blossoms bloom, the spring spirit overflows across the ground\nPlum blossoms bloom, the spring spirit overflows across the ground\n[Verse 2]\nDumplings give off their fragrance at the New Year's Eve dinner\nLanterns sway and illuminate reunion",
  "prompt": "Plum blossoms bloom, the spring spirit overflows across the ground\nPlum blossoms bloom, the spring spirit overflows across the ground\nPlum blossoms bloom, the spring spirit overflows across the ground\nPlum blossoms bloom, the spring spirit overflows across the ground",
...
By default, 2 complete songs are returned, each assembled from one of the two candidates. If `replace_section_result_mode` is set to `candidates`, 2 replacement clips that have not yet been assembled are returned, with a structure consistent with the example below:
...
In `candidates` mode, the durations of the two returned audio clips (45.16 seconds and 33.8 seconds) are much shorter than the original song (94.12 seconds). They are replacement clips containing a small amount of context, and **are not** complete songs. You can select a satisfactory candidate and continue assembling it manually. The default `full_song` mode will complete two assemblies and directly return 2 complete songs.

### Directly return complete songs

If there is no need to compare two candidates, you can set `replace_section_result_mode` to `full_song` in step one. The API will automatically select the first candidate to complete the assembly, and directly return a `data` array containing 1 complete song; there is no need to call `concat` again, and the cost will include the automatic assembly step.

### Step Two: Assemble the replacement clip back into the original song

For the clip selected above (for example, `364f9d8b-ca25-463b-9a5e-d0b7139e2d6a`), initiate a `concat` task according to the method in the [Music Assembly](#音乐拼接) section:
...
What is returned is the assembled complete song, as shown in the following example:
...
```
At this point, `duration` has been restored to the full song length (105.28 seconds, approximately equal to the original song length), and the `audio_url` points to the complete song after replacement. This completes the secondary creation process of "generate → replace segment → stitch full song".

## Vocal-Instrumental Separation

When secondary creation requires separate operations on the accompaniment and vocals after generating a song, pure instrumental accompaniment and a cappella vocals can be separated. You can specify the following content:
...
The vocal-instrumental separation result can be obtained through the above parameters, as follows:
...
The generated result is similar to the above, which completes the process of vocal-instrumental separation for the originally generated song.

## Full-Track Vocal-Instrumental Separation

When full-track vocal-instrumental separation is required after generating a song, you can specify the following content:
...
The full-track vocal-instrumental separation result can be obtained through the above parameters, as follows:
...
The generated result is similar to the one above, which completes the process of separating vocals and instruments from the originally generated song.

## Advanced Parameters for Custom Generation

The official platform allows advanced parameters `weirdness`==>`Weirdness`, `style_influence`==>`Style Influence`, and `audio_weight`==>`Audio Influence` to be used for generation in custom mode, corresponding to the official example below:

<p><img src="https://cdn.acedata.cloud/1xonxy.png" width="500" className="m-auto" /></p>

The ranges of the advanced parameters are all between 0 and 1. The specific parameters are shown in the image below:
...
This way, advanced parameters are used to generate a custom song, and the result is similar to the one above.

## Controlling Song Duration

By default, the duration of generated songs is determined by the model itself, usually between 30 seconds and 4 minutes. If a longer or shorter finished product is needed, you can specify the desired duration through the `duration` parameter, in seconds, with an integer value between 10 and 360.

This parameter is used for song generation in custom mode (`custom` is `true`). It should be particularly noted that `duration` is a **preference hint, not a hard constraint**: the model will refer to this value when creating, but it does not guarantee that it will be reached. In testing, the actual duration is usually significantly shorter than the desired value, and the durations of the two songs returned by the same request may also differ by several times. Even for completely identical requests, the durations obtained from multiple submissions may vary greatly. Therefore, do not use it as precise duration control. If a fixed duration is required for business purposes, please trim it yourself or retry after obtaining the finished product.
...
It should be noted that the `duration` in the request is the **desired duration**, while the `duration` field of each song in the response `data` is the **actual duration** of that song. Although they have the same name, they have different meanings, and the actual duration is not guaranteed to equal the desired value. Lyric length is one of the main factors affecting the duration of the finished product. If a longer finished product is needed, it is recommended to provide more complete lyrics at the same time.

The API does not perform additional validation on `duration`; the parameter is passed to the model as is. If a value unsupported by the current mode or model is passed, it may result in the value being ignored. It is recommended to confirm the effect with one request before using it in batches.

## Add Instrumental Feature

In August 2025, Suno launched the new Add Instrumental feature. First, you need to upload a song with only vocals and no accompaniment, and let Suno add accompaniment for you. First, you can go to [Suno Upload API](https://platform.acedata.cloud/documents/suno-upload) to upload a song with only vocals and no accompaniment. The corresponding operation is shown in the image below:

<p><img src="https://cdn.acedata.cloud/fxl914.png" width="500" className="m-auto" /></p>

Then you need to record the uploaded `audio_id`. The specific result is shown in the image below:

<p><img src="https://cdn.acedata.cloud/47t6wj.png" width="500" className="m-auto" /></p>

Finally, an `audio_id` is obtained: 92254cab-3372-4d9e-bce9-cdcfdbc39070, and then we also need to fill in the following parameters:
...
- audio_id: The ID of the uploaded a cappella song without accompaniment.
- style: The style of the accompaniment. It is best not to use lyrics, since it is accompaniment after all.
...
This completes the operation of adding accompaniment to the uploaded a cappella song without accompaniment, and the result is similar to the above.

## Add Vocals Feature

In August 2025, Suno introduced the Add Vocals feature. First, you need to upload an instrumental track, so that Suno can write lyrics and generate vocal singing. You can first go to [Suno Upload API](https://platform.acedata.cloud/documents/suno-upload) to upload an a cappella song without accompaniment. The corresponding operation is shown in the image below:
...
Finally, an `audio_id` is obtained: 92254cab-3372-4d9e-bce9-cdcfdbc39070. Then we also need to fill in the following parameters:
...
- audio_id: The ID of the uploaded a cappella song without accompaniment.
- custom: In this mode, custom mode must be used to enter lyrics.
...
This completes the operation of adding vocals to an uploaded a cappella song without dubbing, and the result is similar to the above.

## Remaster Feature

In December 2025, Suno released the Remaster feature. This feature can regenerate songs and cannot be used across accounts. Then we also need to fill in the following parameters:

- action: The content is `remaster`.
- audio_id: The ID of the song that needs to be regenerated.
- model: Only v4.5+ and v5 are supported.
- variation_category: Only supported in v5 and above, and there are only 3 values: high, normal, subtle.
...
In December 2025, Suno released the Mashup feature. This feature can generate songs based on two reference songs. Then we also need to fill in the following parameters:
...
This completes the operation of generating a mashup based on reference songs, and the result is similar to the above.

## Samples Sampling to Generate Songs

The `samples` here refers to sampling a single audio segment: selecting a start and end time from an existing audio track, and using that segment as sample material for creation. It differs from the Inspo inspiration creation feature below, which uses 1 to 4 complete reference audio tracks. The following parameters need to be filled in:
...
This completes the operation of generating a song from sampling, and the result is similar to the above.

## Inspo Inspiration Creation Feature

The Inspo inspiration creation feature can generate new music based on 1 to 4 complete reference audio clips, supporting dragging and dropping or uploading audio as the source of inspiration. The API uses the existing `inspo` action: the client provides publicly accessible audio URLs, and the service automatically completes reference audio preparation and creation, without requiring the client to separately read lyrics, style, or duration and then assemble the request. It differs from the range sampling above, and also differs from Cover, which replicates the original song’s style. The following parameters need to be filled in when using it:

- action: The value is `inspo`.
- audio_urls: A list of URLs for reference audio; 1 to 4 publicly accessible audio addresses must be provided.
- model: The model to use; `chirp-v6` is recommended.
- prompt: Lyrics or a creative prompt (optional).

> Note: The reference audio must be a publicly accessible audio file. If the reference audio exactly matches a known recording in the platform's music library, Suno may refuse generation due to copyright verification. It is recommended to use audio you own or audio generated by Suno as the inspiration source.
...
This completes the operation of creating with inspiration, and the returned result is consistent with ordinary song generation.

## Asynchronous Callback

Since Suno takes a relatively long time to generate music, approximately 1–2 minutes, if the API does not respond for a long time, the HTTP request will remain connected, resulting in additional system resource consumption, so this API also provides support for asynchronous callbacks.

The overall process is: when the client initiates a request, it additionally specifies a `callback_url` field. After the client initiates the API request, the API will immediately return a result containing a `task_id` field, representing the current task ID. When the task is completed, the result of generating music will be sent in POST JSON format to the `callback_url` specified by the client, which also includes the `task_id` field, so that the task results can be associated through the ID.

Below, we will learn how to perform the specific operation through an example.

First, a Webhook callback is a service that can receive HTTP requests. Developers should replace it with the URL of their own deployed HTTP server. For the convenience of demonstration here, a public Webhook example website https://webhook.site/ is used. Opening this website will provide a Webhook URL, as shown in the figure:
...
Next, we can set the field `callback_url` to the above Webhook URL, while filling in `prompt`, as shown in the figure:
...
After waiting for a moment, we can observe the result of the generated song at https://webhook.site/03e60575-3d96-4132-b681-b713d78116e2, as shown in the figure:
...
Of course, we can also obtain the result through streaming calls. We only need to set the value of `accept` in the request header to `application/x-ndjson`. Below, an example input is used for demonstration:
...
The obtained result is similar to the basic call; the multiple results above implement streaming calls.
...
> Note: Limits and error messages may vary across different upstream accounts. Usually, `chirp-v3-5`/`chirp-v4` have a lower `style` limit (200), while `chirp-v4-5` and above usually support up to 1000; when an older upstream is hit, compatibility messages such as `Tags too long.` or `style must be less than or equal 120` may appear.
