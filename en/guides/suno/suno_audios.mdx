...
As AI applications become more widespread, various AI programs have gradually become popular. AI has gradually penetrated every aspect of people's work and life. The industries involved in AI are also becoming increasingly diverse, from initial writing, to medical care and education, and now music.

Suno is a professional, high-quality AI song and music creation platform. Users only need to enter simple text prompts to generate songs with vocals based on genre styles and lyrics. This AI music generator was developed by team members from well-known technology companies such as Meta, TikTok, and Kensho, with the goal of allowing everyone to create wonderful music without needing any instrument tools.
...
> The `lyric` and `style` limits in the table above are the upper limits in custom mode (when `custom` is `true`). The non-custom inspiration mode (when `custom` is `false`) only fills in `prompt`, with a maximum length of 500 characters (consistent across all models).
...
**One API Token can call all platform services; there is no need to apply separately for each service.** Your first application will receive free credits for a free trial; when credits are insufficient, you can recharge the general balance in the [console](https://platform.acedata.cloud/console/coin).

> 📘 Full documentation: [Suno Audios Generation API →](https://platform.acedata.cloud/documents/suno-audios)

## Basic Usage

For whatever song you want, you can enter any piece of text. For example, if I want to generate a song about Christmas, I can enter `a song for Christmas`, as shown below:
...
- `accept`: The format of the response result you want to receive. Here, it is filled in as `application/json`, which is the JSON format.
...
- `action`: The action for this music generation task. The default is `generate`, mainly including: `extend`, `upload_extend`, `cover`, `upload_cover`, `replace_section`, `replace_section`, `concat`, `stems`, `all_stems`, `remaster`.
- `prompt`: The prompt for Suno's official inspiration mode (effective when `custom` is `false`), with a maximum of 500 characters.
- `model`: The model for this music generation task. The v6 series includes `chirp-v6`, `chirp-v6-wild`, and `chirp-v6-mini`; legacy model names continue to be supported.
- `lyric`: The lyrics content for Suno's official custom mode. `chirp-v3-5` and `chirp-v4` support a maximum of 3000 characters; `chirp-v4-5` and above (including `chirp-v5` and `chirp-v5-5`) support a maximum of 5000 characters.
- `custom`: Whether to use custom mode. The default is: `false`.
- `instrumental`: The instrumental option for Suno's official inspiration mode.
- `title`: The music title for Suno's official custom mode. `chirp-v3-5` and `chirp-v4` support a maximum of 80 characters; `chirp-v4-5` and above support a maximum of 100 characters.
- `style`: The music style for Suno's official custom mode. `chirp-v3-5` and `chirp-v4` support a maximum of 200 characters; `chirp-v4-5` and above (including `chirp-v5` and `chirp-v5-5`) support a maximum of 1000 characters.
- `negative_tags`: Music styles or genres to exclude from the generated result in custom mode (when `custom` is `true`).
- `audio_weight`: The proportion of the uploaded reference audio, ranging from 0 to 1. The larger the value, the more dependent it is on the reference audio.
...
- `continue_at`: The continuation boundary, in seconds. For example, 213.5 indicates generating the subsequent segment starting from 3 minutes and 33.5 seconds. `lyric` and `style` only guide new content after the boundary and do not replace the lyrics or singing before the boundary in the source audio.
- `style_influence`: The advanced "Style Influence" parameter in custom mode, ranging from 0 to 1. The larger the value, the more closely it matches the selected style.
...
- `vocal_gender`: Controls the preference for female or male vocals: `f` for female vocals and `m` for male vocals. Effective for models 4.5 and above; it is a preference and strict adherence is not guaranteed.
- `weirdness`: The advanced "Weirdness" parameter in custom mode, ranging from 0 to 1. The larger the value, the more creative and experimental it is.
- `duration`: The desired song duration, in seconds. It must be an integer ranging from 10 to 360. This parameter is used for song generation in custom mode (when `custom` is `true`). It is a preferential prompt rather than a hard constraint: the model will refer to it but does not guarantee achieving it. The actual final duration is subject to the `duration` field in the response and is usually shorter than the desired value.
...
- `async`: Optional. When set to `true`, the API immediately returns `task_id`, and `callback_url` does not need to be provided. The result can then be obtained by polling through the corresponding task query API.
...
- data: A list containing detailed information about the generated songs.
  ...
  - audio_url: The final audio URL of the song. The platform will prioritize returning the Ace Data Cloud CDN URL; if persistence fails, the original media URL may be returned. Please download it promptly.
  - video_url: The video file of the song. Opening it will show an mp4 video.
...
> Note that `\n` in the lyrics here represents a line break. If you do not know how to generate lyrics, you can use the lyrics generation API provided by AceDataCloud to generate lyrics through a prompt. The API is [Suno Lyrics Generation API](https://platform.acedata.cloud/documents/suno-lyrics).

Next, to customize a song based on the lyrics, title, and style, you can specify the following content:
...
Finally, after obtaining the song, you need to set this song as a singer style. Then you need to access the [Suno Persona API](https://platform.acedata.cloud/documents/suno-persona) to generate a singer-style ID parameter `persona_id` based on the music ID `audio_id` generated officially. The specific parameters are shown in the image below:
...
Click Run, and you can find that a result will be returned, as follows:
...
We use the above `audio_id` and `persona_id`, namely `97efc9f4-0e8d-4b3e-88df-14568fa1b11f` and `e0d7319e-aa2a-44cb-b00a-916218d7cb0b`, respectively, as the example data this time. Then, you can set the parameter `action` to `artist_consistency` (if it is the new singer style Persona-v2-vox, `action` must be set to `artist_consistency_vox`), and enter the ID of the song to continue generating and the singer style ID. An example is filled in as follows:
...
It can be seen that the result content is consistent with the above, which implements the function of using a singer style to generate songs.

## Continue Generation Feature

If you want to continue generating an already generated Suno song, you can set the parameter `action` to `extend`, and enter the ID of the song to continue generating. The song ID is obtained through the basic usage. As can be seen from the above, at this point you can see that the song ID is:
...
If you want to continue generating your own uploaded song, you can set the parameter `action` to `upload_extend`, and enter the ID of the custom uploaded song to continue generating. The song ID is obtained using the [Suno Upload Generation API](https://platform.acedata.cloud/documents/suno-upload), as shown in the image below:
...
- lyric: Used only to guide the lyrics of the newly generated segment after `continue_at`, and will not replace the lyrics before that point in the source audio.
- custom: Set to `true`, indicating custom generation. This parameter defaults to false, indicating generation using `prompt`.
- style: The song style of the continuation segment, optional.
- continue_at: The continuation boundary, in seconds. For example, 213.5 indicates generating the subsequent segment starting from 3 minutes and 33.5 seconds.

> `extend` is used to continue creating after an existing song, not to replace the lyrics of the entire song. If you want to use new lyrics for the entire song from the beginning, please use `generate` to regenerate it; if you want to re-perform a song by referring to an existing song, you can use `cover`. When passing in a complete set of new lyrics, the original lyrics before `continue_at` will still not be replaced.
...
Keep the other parameters unchanged. What is returned is a complete song, namely the concatenation result of all song segments, but the result contains only one song, as shown below:
```json
{
  "success": true,
...
After continuing to generate a song based on the original song, the style of the returned song may not be suitable. If you want to create a cover of the previously generated song (custom-uploaded music is also supported), you need to use the music cover method, and you can specify the following content:

- action: The value is `cover`; when creating a cover of custom-uploaded music, the value must be specified as: `upload_cover`.
...
Other parameters remain unchanged, and what is returned is a covered song, which is the result of creating a cover of the originally generated song, as shown in the following example:
...
When you need to perform secondary creation by replacing a song section separately after generating a song, you can replace a section of the song.

> ⚠️ **Note**: `replace_section_result_mode` defaults to `full_song`: the system will respectively concatenate 2 newly generated candidates and return 2 complete songs. If you only need the unmerged replacement section candidates, explicitly set it to `candidates`, then select a candidate to call [Music Concatenation](#音乐拼接).
...
- style: The style of the song, optional.
- replace_section_start: The start time (in seconds) of the replaced section in the original song.
- replace_section_end: The end time (in seconds) of the replaced section in the original song.
- replace_section_result_mode: The return mode, defaulting to `full_song`. `full_song` respectively concatenates 2 candidates and returns 2 complete songs; `candidates` returns 2 unmerged candidate sections.

### Step 1: Initiate a Replace Section Task

For example, if the ID of the originally generated song is: 18db7ed0-2b8a-41db-91c1-b0781dcca0d4 (duration: 94.12 seconds), and you want to replace the chorus from 30 seconds to 60 seconds with new lyrics, then you can set the parameters as follows:
```json
{
...
By default, 2 complete songs, each stitched together from two candidates, are returned. If `replace_section_result_mode` is set to `candidates`, 2 unstitched replacement segments are returned, with a structure consistent with the example below:
...
In `candidates` mode, the durations of the two returned audio tracks (45.16 seconds and 33.8 seconds) are much shorter than the original song (94.12 seconds). They are replacement segments containing a small amount of context, and are **not** complete songs. You can select a satisfactory candidate and continue stitching it manually. The default `full_song` mode will complete two stitching operations and directly return 2 complete songs.

### Directly return complete songs

If there is no need to compare two candidates, you can set `replace_section_result_mode` to `full_song` in step one. The API will automatically select the first candidate to complete the stitching and directly return a `data` array containing 1 complete song; at this point, there is no need to call `concat` again, and the cost will include the automatic stitching step.

### Step Two: Stitch the replacement segment back into the original song

For the segment selected above (for example, `364f9d8b-ca25-463b-9a5e-d0b7139e2d6a`), initiate a `concat` task according to the method in the [Music Stitching](#音乐拼接) section:
...
What is returned is the complete stitched song, as shown in the following example:
...
```
At this point, `duration` has been restored to the full song length (105.28 seconds, approximately equal to the original song length), and the `audio_url` points to the full song after replacement is completed. This completes the secondary creation process of "generate → replace segment → stitch full song".

## Vocal and Instrumental Separation

When secondary creation requires separate operations on accompaniment and vocals after generating a song, pure instrumental accompaniment and a cappella vocals can be separated. You can specify the following:

- action: The value is `stems`.
- audio_id: The ID of the previously generated song.

For example, if the previously generated song ID is: ec13e502-d043-4eb2-92ee-e900c6da69d1, then the parameters can be set as follows:
...
The result of vocal and instrumental separation can be obtained through the above parameters, as follows:
...
The generated result is similar to the above, which completes the process of vocal and instrumental separation for the previously generated song.

## Full-Track Vocal and Instrumental Separation

When full-track vocal and instrumental separation is required after generating a song, you can specify the following:

- action: The value is `all_stems`.
- audio_id: The ID of the previously generated song.

For example, if the previously generated song ID is: bdf23a5a-59f5-4103-b452-054a824a7f9f, then the parameters can be set as follows:
...
The result of full-track vocal and instrumental separation can be obtained through the above parameters, as follows:
    ...
    },
{
      ...
      "title": "String of Safety (Woodwinds)",
...
The generated result is similar to the above, which completes the process of separating vocals and music from the originally generated song.

## Advanced Parameters for Custom Generation

The official platform allows the use of advanced parameters `weirdness`==>`Weirdness`, `style_influence`==>`Style Influence`, and `audio_weight`==>`Audio Influence` for generation in custom mode, corresponding to the following official example:

<p><img src="https://cdn.acedata.cloud/1xonxy.png" width="500" className="m-auto" /></p>

The ranges of the advanced parameters are all between 0 and 1, and the specific parameters are shown in the figure below:
...
In this way, advanced parameters are used to generate a custom song, and the result is similar to the above.

## Controlling Song Duration

By default, the duration of generated songs is determined by the model itself, usually between 30 seconds and 4 minutes. If a longer or shorter finished product is needed, the desired duration can be specified through the `duration` parameter, in seconds, with an integer value between 10 and 360.

This parameter is used for song generation in custom mode (where `custom` is `true`). It should be particularly noted that `duration` is a **tendency prompt, not a hard constraint**: the model will reference this value during creation, but reaching it is not guaranteed. In actual testing, the actual duration is usually significantly shorter than the expected value, and the durations of the two songs returned from the same request may also differ by several times. Even for exactly the same request, the durations obtained through multiple submissions may vary considerably. Therefore, do not use it as precise duration control. If a fixed duration is required for business purposes, please trim the finished product yourself or retry after obtaining it.
...
It should be noted that the `duration` in the request is the **expected duration**, while the `duration` field of each song in the response `data` is the **actual duration** of that song. They have the same name but different meanings, and the actual duration is not guaranteed to equal the expected value. Lyric length is one of the main factors affecting the duration of the finished product. If a longer finished product is needed, it is recommended to provide more complete lyrics at the same time.

The API does not perform additional validation on `duration`, and the parameter will be passed to the model as-is. If a value unsupported by the current mode or model is passed in, it may appear that the value is ignored. It is recommended to first use one request to confirm the effect before using it in batches.

## Add Instrumental Feature

In August 2025, Suno introduced the Add Instrumental feature. First, you need to upload an a cappella song without accompaniment, and let Suno add music for you. First, you can go to [Suno Upload API](https://platform.acedata.cloud/documents/suno-upload) to upload an a cappella song without accompaniment. The corresponding operation is shown in the figure below:

<p><img src="https://cdn.acedata.cloud/fxl914.png" width="500" className="m-auto" /></p>

Then you need to record the uploaded `audio_id`. The specific result is shown in the figure below:
...
- audio_id: The ID of the uploaded a cappella song without voice-over.
- style: The style of the accompaniment; it is best not to use lyrics since it is voice-over after all.
...
Click Run, and you can see that you will get a result as follows:
...
This completes the operation of adding accompaniment to the uploaded a cappella song without voice-over, and the result is similar to the above.

## Add Vocals Feature

In August 2025, Suno introduced the Add Vocals feature. First, you need to upload an instrumental track, let Suno write lyrics and generate vocal singing. First, you can go to [Suno Upload API](https://platform.acedata.cloud/documents/suno-upload) to upload an a cappella song without accompaniment. The corresponding operation is shown in the image below:
...
This completes the operation of adding vocals to the uploaded a cappella song without voiceover, and the result is similar to the above.

## Remaster Feature

In December 2025, Suno introduced the new Remaster feature. This feature can regenerate songs and cannot be used across accounts. Then we also need to fill in the following parameters:
...
- variation_category: Only supported in v5 and above versions, and there are only 3 values: high, normal, subtle.
...
In December 2025, Suno introduced the new Mashup feature. This feature can generate a song based on two reference songs. Then we also need to fill in the following parameters:
...
## Samples: Generate Songs from Samples
...
- samples_start: Sample start time.
- samples_end: Sample end time.
...
> Note: The reference audio must be publicly accessible audio files. If the reference audio exactly matches a known recording in the platform's music library, Suno may refuse to generate due to copyright verification. It is recommended to use audio you own or audio generated by Suno as the inspiration source.
...
Below is a verified historical response snapshot; the snapshot used `chirp-v5` at the time, while `chirp-v6` is currently recommended for request examples, and the returned structure is the same:
```json
{
  "success": true,
...
Since Suno takes a relatively long time to generate music, approximately 1–2 minutes, if the API remains unresponsive for a long time, the HTTP request will stay connected continuously, resulting in additional system resource consumption. Therefore, this API also provides support for asynchronous callbacks.

The overall process is: when the client initiates a request, it additionally specifies a `callback_url` field. After the client initiates the API request, the API will immediately return a result containing a `task_id` field, representing the current task ID. When the task is completed, the music generation result will be sent via POST JSON to the `callback_url` specified by the client, which also includes the `task_id` field, so that the task results can be associated through the ID.

Below, we will use an example to understand how to operate it specifically.

First, a Webhook callback is a service that can receive HTTP requests. Developers should replace it with the URL of their own HTTP server. For convenience of demonstration, a public Webhook sample website https://webhook.site/ is used here. Open this website to obtain a Webhook URL, as shown in the image:
...
Next, we can set the field `callback_url` to the above Webhook URL, while entering `prompt`, as shown in the image:
...
After waiting for a moment, we can observe the generated song result at https://webhook.site/03e60575-3d96-4132-b681-b713d78116e2, as shown in the image:
...
You can see that there is a `task_id` field in the result, and all other fields are similar to the above. Task association can be achieved through this field.

Of course, we can also obtain the result through streaming calls. We only need to set the value of `accept` in the request header to `application/x-ndjson`. Below is an example input for demonstration:
...
The obtained result is similar to a basic call; the multiple results above implement streaming calls.
...
> Note: Limits and error messages may vary across different upstream accounts. Typically, `chirp-v3-5`/`chirp-v4` have lower `style` limits (200), while `chirp-v4-5` and above usually support up to 1000; when an older upstream is hit, compatibility messages such as `Tags too long.` or `style must be less than or equal 120` may appear.
