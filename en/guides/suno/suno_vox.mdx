---
title: "Suno Vox API Integration Guide"
description: "Suno Music Generation Integration Guide - Ace Data Cloud"
---

SUNO allows us to create the new version of Persona-v2-vox: singer style, which differs from the old version by requiring the `vox_audio_id` first. This document explains the integration method for creating the new Persona-v2-vox.

First, you need to use this API to obtain the `vox_audio_id` parameter value. This API accepts multiple input parameters, such as `audio_id`, `vocal_start`, and `vocal_end`, which refer to the song ID and the selected time range.
...
As you can see, the `id` field in `data` is the `vox_audio_id` we want. Then, we use the [Persona API](https://platform.acedata.cloud/documents/78bb6c62-6ce0-490f-a7df-e89d80ec0583) to create the new Persona-v2-vox: singer style. The specific input is as follows:
