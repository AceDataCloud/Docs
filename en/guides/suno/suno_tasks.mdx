...
The main function of the Suno Tasks API is to query the execution status of tasks by inputting the task ID generated by the Suno Audios Generation API or Suno Lyrics Generation API.
...
To use the Suno Audios Generation API, first obtain your API Token from the [Ace Data Cloud Console](https://platform.acedata.cloud/console/applications) for future reference.
...
This is for querying song task details for multiple task IDs, and unlike above, the action needs to be set to retrieve_batch.
...
The return result contains multiple fields, where items include the specific details of the batch song tasks, and each song task's specific information is the same as the fields above.
...
Through this document, you have learned how to use the Suno Tasks API to query all specific details of single or batch song tasks. We hope this document helps you better integrate and use the API. If you have any questions, please feel free to contact our technical support team.
