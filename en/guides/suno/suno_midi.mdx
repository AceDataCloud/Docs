---
title: "Suno MIDI Generation API Integration Guide"
description: "Suno Music Generation Integration Guide - Ace Data Cloud"
---

SUNO allows us to perform secondary creation on generated music and obtain the MIDI of the music. This document explains the integration method for the related API.

The core input parameter of this API is `audio_id`, which is the official generated song ID, generally the full stem-separated song ID (corresponding to the parameter `action` set to `all_stems` in [Song Generation](https://platform.acedata.cloud/documents/suno-audios)). Optionally, a `callback_url` for asynchronous callback is also supported.

Here, the input `audio_id` is `c65e6ffd-ead3-4926-9c8c-a42ce202946b`.
...
As you can see, the fields inside `data` represent the MIDI information of the full stem-separated song.