...
The exclusive music model can learn consistent musical features from 6 to 24 authorized audio tracks and be used for subsequent song creation. Creation is an asynchronous operation; it is recommended to use 24 tracks with consistent style and clear audio quality for more stable results.
...
The interface will immediately return the platform model ID and task ID. After creation is complete, the `status` in the callback or query result will change to `ready`. Only successful model creation will be charged **10 Credits**, approximately **¥6.75 each** based on the maximum package; failed creations will not incur charges.
...
All models belong to the Suno application that created them. API credential rotation will not change model ownership, and other applications cannot query or use that model.
...
The `id` must be the exclusive music model under the current application with a status of `ready`. Requests will fail if the model is unavailable and will not automatically switch to another model. Successful song generation consumes **0.90 Credits**, approximately **¥0.61 per time** based on the maximum package; failed generations will not incur charges. Querying and archiving models are free.
