---
title: "Suno Reference Audio Upload API Integration Guide"
...
> `audio_url` is the input address and will be passed directly to the audio processing workflow. It will not be pre-transferred by the final media persistence service. Please ensure that it is publicly accessible during task processing.
...
The API immediately returns `task_id` and `trace_id`. Use the [Suno Tasks API](https://platform.acedata.cloud/documents/suno-tasks) to query the task, and after success, read the uploaded audio ID from `response.data.audio_id`. You can also provide an HTTPS `callback_url` to receive the final status.
