...
Ordinary uploads only require `audio_url`, which is a publicly accessible audio CDN address. The `mode` defaults to `standard`, so existing calls do not need to be modified. A successful ordinary upload consumes **0.06 Credits**, which is approximately **¥0.041 per time** based on the maximum package; failed uploads incur no charges.
...
When ordinary uploads cannot handle the audio you own or have been authorized to use, you can set `mode` to `enhanced`. This mode also requires providing a `name` of 1 to 100 characters and using a publicly accessible HTTPS `audio_url`.

Enhanced uploads are processed asynchronously and typically take 2 minutes or longer. Successful processing consumes **1.87 Credits**, which is approximately **¥1.26 per time** based on the maximum package; failed processing incurs no charges.
