---
title: "Google SERP API Integration Guide"
description: "Search Engine Integration Guide - Ace Data Cloud"
---

Google SERP (Search Engine Results Page) is the results page users see after entering a query in the Google search engine. It displays organic search results, ads, featured snippets, knowledge graphs, as well as various content types such as images and videos, aiming to provide users with the most relevant information.

This article provides a detailed introduction to the Google SERP API, which can return results for queries entered in the Google search engine. The results include many types, such as featured snippets, knowledge graphs, images, and more.

This document will explain how to integrate with the Google SERP API.

## Application Process

To use the Google SERP API, you need to apply for the corresponding service on the [Google SERP API](https://platform.acedata.cloud/documents/44c86226-8eaa-49bf-85f3-1fae8d2e23f1) page. After entering the page, click the "Acquire" button, as shown below:
...
First, let's understand the basic usage: by inputting the search resource type and keywords, you can get search results. You only need to pass the `query` field and specify the corresponding model.

For example, to search for information about "apple inc", you can fill in the corresponding content on the interface as shown below:
...
- `type`: the type of search resource, currently supporting six types, default is `search`.
- `query`: the search keyword.
- `country`: the country where the search results are located, default is the United States (US).
- `language`: the language of the search results, default is English (en).
- `range`: the time range of the search results, default is unlimited.
- `number`: the page size of the search results, default is 10.
...
You can see that the returned result includes an `organic` field, which mainly contains the search results for the keywords.
...
If you want to customize the type of search resources, you can modify the `type` parameter. It includes ordinary resources `search`, image resources `images`, news resources `news`, map resources `maps`, place resources `places`, and video resources `videos`. This article demonstrates with the video resource `videos`.

Let's demonstrate the specific operation.

First, set the `type` parameter to `videos` and pass the `query` parameter normally, as shown below:
...
- `videos`: the list of video information in the search results.

You can see that the returned result includes a `videos` field, which mainly contains the search results for the keywords.

## Custom Search Country

This API also supports limiting the country of the search results. We can add the `country` parameter to specify the country, with the input being the country abbreviation, such as cn (China), us (United States). This article uses China as an example, as shown below:
...
You can see that the results are search results within China, and the content is similar to the above.

## Custom Search Result Language

You can also customize the language of the search results by adding the `language` field, with the value `zh-cn`, which refers to Simplified Chinese. Other languages are supported but must be input as language abbreviations, such as en (English), fr (French), zh-cn (Chinese (Simplified)), etc., as shown below:
...
You can see that the results are displayed in Simplified Chinese, and the content is similar to above.

## Custom Search Result Time Range

You can also customize the time range of the search results. There are five options: `qdr:h` (past hour), `qdr:d` (past day), `qdr:w` (past week), `qdr:m` (past month), and default unlimited. You can pass the corresponding time range through the `range` parameter. For example, here we set it to `qdr:d`, meaning to search results from the past day. The input is as follows:
...
You can see that we have successfully obtained search results from the past day, and the content is similar to above.

## Custom Search Result Pagination

This API also supports custom pagination for search results. `number` and `page` represent the page size and page number respectively. This article sets the page size to 20 results per page for pagination display, as shown below:
...
You can see that the search results are paginated, showing 20 results per page, and the content is similar to above.