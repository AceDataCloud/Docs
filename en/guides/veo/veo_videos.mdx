...
To use the Veo Videos Generation API, first go to the [Ace Data Cloud Console](https://platform.acedata.cloud/console/applications) to obtain your API Token for future use.

![](https://cdn.acedata.cloud/5hmkdg.jpg)

If you are not logged in or registered, you will be automatically redirected to the login page inviting you to register and log in, and after completing this, you will be automatically returned to the current page.
...
First, understand the basic usage method, which involves inputting the prompt `prompt`, the action `action`, an array of reference images for the first and last frames `image_urls`, and the model `model` to obtain the processed result. You first need to simply pass a field `action`, with the value `text2video`, which mainly includes three actions: text to video (`text2video`), image to video (`image2video`), and get 1080p video (`get1080p`). Then, we also need to input the model `model`, which currently mainly includes `veo31-fast`, `veo3`, `veo31`, `veo2-fast`, `veo3-fast`, and `veo31-fast-ingredients`, with specific content as follows:
...
- `model`: the model for generating the video, mainly including `veo31-fast`, `veo3`, `veo31`, `veo2-fast`, `veo3-fast`, and `veo31-fast-ingredients`.
...
- `image_urls`: when selecting the image to video action `image2video`, it is necessary to upload the reference image links. `veo2-fast` only supports 1 image, `veo31-fast-ingredients` supports up to 3 images (multi-image fusion), and other models support a maximum of 2 images (first and last frame mode).
- `resolution`: choose the resolution of the generated video, where the veo31 model supports 4k resolution, while other models do not. All models support 1080p and gif resolutions, and if this value is not provided, it defaults to 720p resolution, mainly divided into: `1080p`, `gif`, `4k`.
- `prompt`: the prompt.
...
1. **General Logic**:
   ...
   - **With image input** → Triggers image to video mode (specific behavior determined by the number of images).
...
- **The only model that must provide images**: `veo31-fast-ingredients` must provide images (1-3 images), otherwise it cannot run.
- **Image Quantity Limitations**:
  - `veo2-fast` only supports **1 image** input (first frame mode).
  - `veo31-fast-ingredients` supports **1-3 images** input (multi-image fusion mode).
  - Other models support a maximum of **2 images** input (first and last frame mode).
...
It can be seen that we have obtained satisfactory video information, and we only need to obtain the generated Veo video based on the video link address in the `data` result.

In addition, if you want to generate the corresponding integration code, you can directly copy the generated code, for example, the CURL code is as follows:
...
- `model`: the model for generating the video, mainly including `veo31-fast`, `veo3`, `veo31`, `veo2-fast`, `veo3-fast`, and `veo31-fast-ingredients`.
- `image_urls`: when selecting the image to video action `image2video`, it is necessary to upload the reference image links.
...
If you want to get 1080p for an already generated Veo video, you can set the parameter `action` to `get1080p`, and input the ID of the video that needs to be obtained in 1080p. The video ID can be obtained based on basic usage, as shown in the following image:
...
- `translation`: whether to enable automatic translation of the prompt words, with the default being `false`.
  The sample input is as follows:
```
...
It can be seen that the result content is consistent with the above text, thus achieving the function of generating a video of specified dimensions.
...
First, the Webhook callback is a service that can receive HTTP requests, and developers should replace it with the URL of their own HTTP server. For convenience in demonstration, a public Webhook sample site https://webhook.site/ is used. Opening this site will provide a Webhook URL, as shown in the image:
...
It can be seen that the result contains a `task_id` field, and other fields are similar to the above text, allowing task association through this field.
...
Through this document, you have learned how to use the Veo Videos Generation API to generate videos by inputting prompt words and reference images for the first frame. It is hoped that this document can help you better integrate and use this API. If you have any questions, please feel free to contact our technical support team.
