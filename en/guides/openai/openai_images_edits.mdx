...
- **`gpt-image-2:official`**: Official channel, stable and compliant. Supports true 2K / 4K high resolution, **billed per image, with a unit price 2 times that of the default `gpt-image-2`**. If the line is unavailable, it will return an error directly without automatic downgrade.
...
Size limitations: Custom sizes must meet the criteria of both width and height being multiples of 16, with a long edge ≤ 3840 and a total pixel count ≤ 8,294,400; exceeding these will return a 4xx error.
...
> For example: If the original image is `1024x1024`, and `size` is set to `2048x2048`, the model will redraw according to the editing instructions and output a 2K image; if `size` is set to `3840x2160`, it will output a 4K landscape image. The billing for all three is the same.
> Omitting the `size` field is completely equivalent to explicitly passing `auto`: `gpt-image-2` will first read the explicit size intent in the prompt, including pixels, ratios, orientation, resolution tiers (e.g., 4K / high-res), or named canvases. When a size intent is recognized, it will adopt the planned specific size; if there are no size requirements in the prompt or automatic judgment is unavailable, it will revert to the size of the first reference image. The final specific size will be normalized to multiples of 16, with long edge and total pixel limits applied before the request is submitted; if absolute control is needed, please directly pass `WIDTHxHEIGHT`. After generation is complete, it will not automatically retry due to different output pixels, avoiding duplicate generation fees.
...
Send the request directly in `application/json` format, filling the `image` field with the URL of an image, and the model will fetch that image and edit it according to the `prompt`.
...
We hope to change it to a "night mode" color scheme. This can be called as follows:
```shell
