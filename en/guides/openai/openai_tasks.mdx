...
At least one of `id` or `trace_id` must be provided. Generally, you can directly use the `id` from the submission response; `trace_id` should only be passed if you want to associate it with a custom business identifier.
...
- `trace_id`: The custom tracking identifier passed in the original request, facilitating client-side business associations.
...
# 1. Submit an image generation task (callback mode: just include the callback_url to immediately return task_id)
...
- Task records will only be written when the original request includes a `callback_url`; synchronous calls will not generate queryable tasks.
