...
- **`gpt-image-2:official`**: Official channel, stable and compliant. Supports true 2K / 4K resolution, **billed per image, with a unit price that is 2 times that of the default `gpt-image-2`**. If the line is unavailable, it will return an error directly and will not automatically downgrade.
...
`gpt-image-2` only checks the format of `size`; as long as it is not `auto` or an empty string, it needs to match `WIDTHxHEIGHT` (for example, `1024x1024`, `2048x1152`, `800x600`); any other form will return 400. **All sizes (1K / 2K / 4K / custom) are charged uniformly per image, without additional charges based on size.**

Size limitations: Custom sizes must meet the criteria of both width and height being multiples of 16, long edge ≤ 3840, total pixel count ≤ 8,294,400; exceeding these limits will return a 4xx error.
...
In the prompt, you can use cinematic terms (35mm film, shallow depth of field, neon light, etc.) to precisely control the atmosphere and texture.
...
It can be seen that the image generated with `vivid` is more vibrant and realistic compared to `natural`.
...
It can be seen that the result contains a `task_id` field, and the `data` field includes the same image generation result as the synchronous call, allowing the task to be associated through the `task_id` field.
