...
The OpenAI Images Generations API currently supports various image generation models, including the classic `dall-e-3`, the text rendering-capable `gpt-image-1`, the latest generation **`gpt-image-2`**, and the **`nano-banana` / `nano-banana-2-lite` / `nano-banana-2` / `nano-banana-pro`** series models that can be accessed through the same interface. They can all generate high-quality images based on text descriptions.
...
> When passing `size: "auto"` (default), the model will **read the prompt itself to determine the aspect ratio**: writing "ultra-wide cinematic banner 21:9" will yield `3024x1296`, writing "portrait poster 9:16" will yield `1440x2560`, and prompts without aspect ratio hints will revert to `2048x2048`. Therefore, when you have a composition in mind but do not want to calculate specific pixels, `auto` is usually more suitable than hardcoding a size. Omitting the `size` field has the same effect as `auto`.
> The output at the 1K tier does not guarantee strict pixel alignment—if you pass `1024x1024`, you might receive `1254x1254`, maintaining the ratio. If you pass it back as `size`, the billing remains unchanged.
> A single call at 4K typically requires 4–8 minutes, and it is recommended to use it with the `callback_url` for asynchronous callbacks.
...
The `callback_url` asynchronous callback mechanism is also effective for nano-banana, and the calling process is completely consistent with other models. For details, see the section [Asynchronous Callback](#asynchronous-callback) below.
...
```
The returned results are consistent with the basic usage content, and we can see the generated image with the quality parameter set to `standard` as shown in the image below:
...
The returned result is consistent with the basic usage content, and we can see the generated image with the style parameter set to `vivid` as shown in the image below:
...
The last image link format parameter `response_format` also has two options, the first one `b64_json` encodes the image link in Base64, while the other `url` is a regular image link that can be viewed directly.
...
The returned result is consistent with the basic usage content, and we can see that the image link format parameter for `url` generates the image link as [Image URL](https://dalleprodsec.blob.core.windows.net/private/images/87792c5f-8b6d-412e-81dd-f1a1baa19bd2/generated_00.png?se=2024-07-23T08%3A39%3A47Z&sig=zzRAn30TqIKHdLVqZPUUuSJdjCYpoJdaGU6BeoA76Jo%3D&ske=2024-07-23T13%3A32%3A13Z&skoid=e52d5ed7-0657-4f62-bc12-7e5dbb260a96&sks=b&skt=2024-07-16T13%3A32%3A13Z&sktid=33e01921-4d64-4f8c-a055-5bdaffd5e33d&skv=2020-10-02&sp=r&spr=https&sr=b&sv=2020-10-02) which can be accessed directly, and the image content is shown below:
