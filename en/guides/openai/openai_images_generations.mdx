...
The OpenAI Images Generations API supports several models, including the classic `dall-e-3`, the typography-friendly `gpt-image-1`, and the latest **`gpt-image-2`**. All of them can generate high-quality images from a text description.

This document mainly introduces the usage process of the OpenAI Images Generations API, which lets us easily access OpenAI's image generation capabilities.
...
`gpt-image-2` is OpenAI's next-generation image model. Compared with `dall-e-3` and `gpt-image-1`, it brings clear improvements in:

- **Stronger instruction following** — accurately handles complex composition, counting and spatial constraints.
- **Sharper text rendering** — typography on posters, menus, infographics and signage is rendered cleanly, with very few hallucinated characters.
- **Richer styles** — natively handles cinematic portraits, retro posters, children's book illustrations, product photography, infographics, and more.
- **Native multi-aspect output** — `1024x1024` (square), `1024x1536` (portrait) and `1536x1024` (landscape) are all first-class.

Calling it is identical to other models — just set `model` to `gpt-image-2`. The `url` field in the response is a permanent, publicly accessible link hosted on `platform.cdn.acedata.cloud`, which can be opened in a browser or embedded directly in a webpage.

The following examples showcase `gpt-image-2` from several different angles using real production calls.

### Scenario 1 — Cinematic portrait

Cinematography terms (35mm film, shallow depth of field, neon lighting, etc.) can be used to precisely steer mood and texture.
...
<p><img src="https://platform.cdn.acedata.cloud/gpt-image/6738988a-ea3c-44ae-932f-488b14e5902b_0.png" width="500" className="m-auto" /></p>

### Scenario 2 — Vintage travel poster (with text rendering)

`gpt-image-2` is excellent at typography, which makes it well-suited for posters, menus, greeting cards and any design that needs readable text.
...
The image returned by the `url` field:

<p><img src="https://platform.cdn.acedata.cloud/gpt-image/b74900da-7ca1-455b-9fb2-2434522a44b8_0.png" width="500" className="m-auto" /></p>

The model not only nails the Art Deco poster aesthetic — it also renders both `AMALFI` and `ITALIA 1958` cleanly and correctly.

### Scenario 3 — Complex composition and counting

This prompt tests how well the model follows structured constraints such as item counts and positions.
...
The shelf contains exactly **1 / 3 / 7** books from top to bottom — an instruction-following result that was hard to achieve reliably with `dall-e-3`.

### Scenario 4 — Illustration style (landscape)

By specifying an art medium and emotional keywords, you can steer the model toward stylized illustration output.
...
The resulting landscape illustration:

![](https://platform.cdn.acedata.cloud/gpt-image/e4921c51-5182-4412-873b-24019280ab23_0.png)

### Async / callback

A single `gpt-image-2` call typically takes 60–90 seconds. If you don't want to keep an HTTP connection open for that long, use the `callback_url` mechanism described later in this document — it works identically across all supported models.
