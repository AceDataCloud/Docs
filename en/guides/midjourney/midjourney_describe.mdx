---
title: "Integration and Usage of Midjourney Describe API"
description: "Midjourney generation integration guide - Ace Data Cloud"
---

The main function of the Midjourney Describe API is to obtain descriptions of images by uploading them. Using this API, you only need to provide the image file URL, and the API will return a detailed description of the image. There is no need for complicated parameter settings to get high-quality image descriptions.

It supports multiple image formats: whether JPEG, PNG, or GIF and other mainstream image formats, they can all be easily recognized and processed.
...
To use the Midjourney Describe API, you need to apply for the corresponding service on the application page [Midjourney Describe API](https://platform.acedata.cloud/documents/870e973b-712a-4686-ab8b-beae27f129ce). After entering the page, click the "Acquire" button as shown:
...
A free quota will be granted for the first application, allowing you to use the API for free.

## Request Example

We will use an example image to demonstrate how to use this API. Suppose we have a landscape image; next, we will demonstrate how to upload this image and get a description.
...
- `accept`: Specifies to receive a JSON format response, set to `application/json` here.
...
After a successful request, the API will return 4 description entries for the image. For example:
...
As you can see, the result contains a `descriptions` field with four entries, each being a candidate description.
...
Through this document, you have learned how to use the Midjourney Describe API for image description. We hope this guide helps you better integrate and use the API. If you have any questions, please feel free to contact our technical support team.