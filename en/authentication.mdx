...
| Type | Use case |
|---|---|
| **Service credential** | Bound to a single service; deducts from that service's subscription quota. **Recommended** — least privilege. |
| **Global credential** | Shared across all subscribed services; deducts from the global balance. Convenient when calling many AI services. |
...
- `used_up` — service quota or global balance is exhausted
