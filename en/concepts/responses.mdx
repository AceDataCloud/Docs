...
| `used_up` | 403 | Service quota or global balance exhausted |
...
- **429 / 5xx may be retried** with exponential backoff: `1s, 2s, 4s, 8s, 16s` + 0–1s jitter, up to 5 attempts
- Persist `trace_id` to your logs for the fastest support resolution

```python
import time, random, requests

def call_with_retry(url, headers, body, max_attempts=5):
