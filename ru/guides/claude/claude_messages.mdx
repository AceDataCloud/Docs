...
Чтобы использовать Claude Messages API, сначала перейдите на страницу [Claude Messages API](https://platform.acedata.cloud/documents/claude-chat-completions) и нажмите кнопку «Acquire», чтобы получить необходимые для запроса учетные данные:
