...
Чтобы использовать API завершения чата Claude, сначала можно перейти на страницу [API завершения чата Claude](https://platform.acedata.cloud/documents/claude-chat-completions) и нажать кнопку «Acquire», чтобы получить необходимые для запроса учетные данные:
