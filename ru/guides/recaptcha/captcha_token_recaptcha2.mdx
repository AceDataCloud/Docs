...
**Один API Token позволяет вызывать все услуги платформы, не нужно отдельно запрашивать для каждой услуги.** При первом запросе предоставляется бесплатный лимит, чтобы вы могли попробовать бесплатно; при недостаточном лимите вы можете пополнить общий баланс в [консоли](https://platform.acedata.cloud/console/coin).
...
Результат возвращает несколько полей, описание которых следующее:

- `token` — результат проверки после обработки задачи Recaptcha2.
- `started_at`, `finished_at` — время начала обработки запроса и получения результата (ISO-8601 UTC).
...
Сначала нам нужно выяснить, как сайт отправляет POST-запрос, чтобы мы могли передать сгенерированный токен. Для этого нам нужно открыть консоль F12 и вручную пройти проверку, после чего мы увидим, что сайт отправил POST-запрос. Нам нужно только посмотреть, как был построен этот POST-запрос, конкретный процесс следующий:
...
- Затем нажать на submit и наблюдать за изменениями в сети в консоли, конкретно как на изображении ниже:
...
Из вышеуказанного анализа видно, что URL этого POST-запроса: `https://www.google.com/recaptcha/api2/demo`, нам нужно только отправить параметр `g-recaptcha-response`, затем мы просто передаем обработанный токен в данные ниже, конкретный код CURL для вызова токена для проверки выглядит следующим образом:
