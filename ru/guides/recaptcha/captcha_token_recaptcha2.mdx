...
В этой статье будет представлена инструкция по интеграции API распознавания протокола Recaptcha2, которая позволяет пользователям не распознавать и не нажимать на изображения капчи Recaptcha2, а просто отправить Website Key для автоматической декодировки на сервере и завершения проверки.
...
Если вы еще не вошли в систему или не зарегистрировались, вы будете автоматически перенаправлены на страницу входа, где вас пригласят зарегистрироваться и войти в систему, после чего вы будете автоматически возвращены на текущую страницу.
...
- `proxy`: необязательный, собственный прокси (Bring Your Own Proxy). После установки, сервер будет использовать предоставленный вами прокси IP для декодирования капчи, чтобы контролировать качество выходного IP (например, чтобы избежать блокировки целевым сайтом из-за использования общих прокси IP и получения ответа `410 Gone`). Формат: `scheme://[user:pass@]host:port`, `scheme` поддерживает `http`/`https`/`socks4`/`socks5`, например `http://user:pass@1.2.3.4:8080`. Если не заполнено, будет использоваться прокси по умолчанию от платформы.
...
С помощью этого документа вы узнали, как использовать API для распознавания протокола Recaptcha2, позволяя пользователям не распознавать и не нажимать на изображения капчи Recaptcha2, а просто отправив Website Key, чтобы автоматически декодировать на сервере и завершить проверку. Надеемся, что этот документ поможет вам лучше интегрировать и использовать этот API. Если у вас есть какие-либо вопросы, пожалуйста, не стесняйтесь обращаться в нашу техническую поддержку.
