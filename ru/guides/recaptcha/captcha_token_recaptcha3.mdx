...
**Один API Token позволяет вызывать все услуги платформы, не нужно отдельно запрашивать для каждой услуги.** При первом запросе предоставляется бесплатный лимит для тестирования; при недостаточном лимите вы можете пополнить общий баланс в [консоли](https://platform.acedata.cloud/console/coin).
...
После выбора можно заметить, что справа также сгенерировался соответствующий код, как показано на изображении:
...
Мы получили результат проверки Recaptcha3, который можно использовать для POST или GET запросов к целевому сайту, одноразовый, срок действия 120 секунд, рекомендуется использовать в течение 60 секунд. Далее будет кратко описан способ отправки сгенерированного токена на целевой сайт:
