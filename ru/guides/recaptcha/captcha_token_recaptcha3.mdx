...
Откройте f12, затем на странице Element выполните поиск по `.execute(`, в области красного прямоугольника мы можем увидеть параметр `action`, а также за ним следует строка, которая также потребуется далее, конкретно это показано на изображении ниже.
...
Мы получили результат обработки проверки Recaptcha3, который можно использовать для POST или GET запросов к целевому сайту, одноразовый, срок действия 120 секунд, рекомендуется использовать в течение 60 секунд. Ниже кратко описан способ отправки сгенерированного токена на целевой сайт:
