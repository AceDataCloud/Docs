...
**Один API Token позволяет использовать все услуги платформы, не нужно отдельно запрашивать для каждой услуги.** При первом запросе предоставляется бесплатный лимит, который можно использовать бесплатно; при недостаточном лимите можно пополнить общий баланс в [консоли](https://platform.acedata.cloud/console/coin).
...
Откройте f12, затем на странице Element выполните поиск по `.execute(`, в красной рамке мы можем увидеть параметр `action`, а также за ним следует строка, которая также необходима в дальнейшем, как показано на следующем изображении.
...
Результат запроса состоит из нескольких полей, описание которых приведено ниже:

- `token` — результат проверки после обработки задачи Recaptcha3.
...
Мы получили результат проверки Recaptcha3, который можно использовать для POST или GET запросов к целевому сайту, одноразовый, срок действия 120 секунд, рекомендуется использовать в течение 60 секунд. Ниже кратко описан способ отправки сгенерированного токена на целевой сайт:
