...
Чтобы использовать Gemini Chat Completion API, сначала перейдите на страницу [Kimi Chat Completion API](https://platform.acedata.cloud/documents/kimi-chat-completions) и нажмите кнопку «Acquire», чтобы получить необходимые для запроса учетные данные:
