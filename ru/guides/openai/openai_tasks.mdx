...
В режиме обратного вызова **оригинальный интерфейс изображений сразу после обработки запроса возвращает `task_id`**. Вы просто держите этот `task_id` и можете использовать его для запроса в этом интерфейсе, когда это необходимо, без дополнительной передачи пользовательского `trace_id` (только если вы хотите связать его с вашим собственным бизнес-идентификатором).
...
| `retrieve_batch` | Пакетный запрос по `ids` / `trace_ids` / `application_id` / `user_id` |
...
- `response`: Окончательное тело ответа, возвращенное при завершении обратного вызова.
- `created_at` / `finished_at`: Unix временные метки (в секундах). `started_at` - строка в формате ISO 8601.
- `elapsed`: Время выполнения (в секундах, с плавающей точкой).
...
# 1. Отправка задачи на генерацию изображения (режим обратного вызова: укажите callback_url, чтобы немедленно получить task_id)
