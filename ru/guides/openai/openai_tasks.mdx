...
В режиме обратного вызова **оригинальный интерфейс изображений сразу после обработки запроса возвращает `task_id`**. Вы напрямую обладаете этим `task_id` и можете использовать его для запроса в этом интерфейсе, когда это необходимо, без дополнительной передачи пользовательского `trace_id` (только если вы хотите связать его с вашим собственным бизнес-идентификатором).

> Задача будет сохранена только в том случае, если в оригинальном запросе изображений присутствует `callback_url`. Запросы, выполненные синхронно (не в режиме обратного вызова), не будут храниться.

## Процесс подачи заявки

OpenAI Tasks API использует ту же авторизацию, что и существующие сервисы OpenAI. Если вы уже подали заявку на OpenAI Images Generations, вы можете напрямую использовать тот же токен для вызова этого интерфейса, без дополнительной заявки.
...
- `type`: Тип интерфейса. Задачи, записанные в серии `gpt-image` (например, `gpt-image-2`), имеют тип `images`; старые каналы OpenAI (например, `gpt-image-1`, nano-banana) используют `images_generations` / `images_edits`, некоторые интерфейсы чата имеют тип `chat_completions_image`.
- `request`: Полное тело оригинального запроса.
- `response`: Окончательное тело ответа, возвращенное интерфейсом после завершения обратного вызова.
...
| `type`             | string   | Фильтрация по типу интерфейса (возможные значения: `images`, `images_generations`, `images_edits`) |
| `offset`           | int      | Начальная точка для постраничного вывода, по умолчанию `0`   |
...
```
## Пример от конца до конца: отправка и опрос

API задач в основном служит для асинхронных процессов в режиме обратного вызова. В режиме обратного вызова интерфейс отправки **немедленно синхронно возвращает `task_id`** (то есть идентификатор задачи), после чего вам просто нужно использовать этот `task_id` для опроса интерфейса задач, без необходимости самостоятельно генерировать `trace_id`.
...
- Интерфейс задач сам по себе **не облагается платой**, можете смело опрашивать. Только оригинальные запросы на генерацию/редактирование изображений будут тарифицироваться.
- Запись задачи будет создана только в том случае, если оригинальный запрос содержит `callback_url`; синхронные вызовы не создадут задачи, которые можно будет запросить.
- Записи задач, превышающие срок хранения на платформе, могут быть очищены.
