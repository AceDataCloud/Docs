...
Чтобы использовать OpenAI Chat Completion API, сначала можно перейти на страницу [OpenAI Chat Completion API](https://platform.acedata.cloud/documents/openai-chat-completions) и нажать кнопку «Acquire», чтобы получить необходимые для запроса учетные данные:
