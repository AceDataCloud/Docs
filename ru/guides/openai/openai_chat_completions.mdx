...
Где `choices` содержит информацию о ответах ChatGPT, внутри него `choices` — это ответы ChatGPT, как показано на изображении.
...
Внутри `choices` поле `message.content` — это полный результат диалога, изображение включено в формате Markdown (ссылка на изображение является временной, пожалуйста, загрузите и сохраните). Можно увидеть, что сгенерированное изображение действительно в аниме-стиле, как показано на рисунке ниже:

<p><img src="https://cdn.acedata.cloud/qmr391.jpg" width="400" class="m-auto"></p>

### Генерация изображения только по тексту

Мы можем сгенерировать изображение с помощью подсказки и вернуть его в виде диалогового результата, ниже мы используем `Создать изображение заката в будущем городе` в качестве примера, конкретный пример:
