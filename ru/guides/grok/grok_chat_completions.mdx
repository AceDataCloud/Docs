...
Чтобы использовать Grok Chat Completion API, сначала можно перейти на страницу [Grok Chat Completion API](https://platform.acedata.cloud/documents/grok-chat-completions) и нажать кнопку «Acquire», чтобы получить необходимые для запроса учетные данные:
