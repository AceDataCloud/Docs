...
Возвращаемый результат содержит несколько полей, поле request — это тело запроса, отправленного для задачи, в то время как поле response — это тело ответа, возвращенное после завершения задачи. Описание полей следующее.

- `id`: ID задачи, сгенерированной для этого изображения, используется для уникальной идентификации этой задачи по созданию изображения.
...
После успешного запроса API вернет конкретные детали всех пакетных задач изображения. Например:
```json
{
  "items": [
    {
