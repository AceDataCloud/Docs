...
Возвращаемый результат содержит несколько полей, поле request — это тело запроса, отправленное при создании задачи, а поле response — это тело ответа, возвращенное после завершения задачи, результаты которого совпадают с запросом и ответом Luma Videos Generation API. Описание полей следующее.

- `id`: ID задачи по созданию этого видео, используется для уникальной идентификации данной задачи по созданию видео.
...
- `response`: информация о ответе в задаче видео.
...
Это запрос для получения деталей видео задач по нескольким ID, в отличие от вышеуказанного, необходимо выбрать action как retrieve_batch.
