...
AI Chat v2 API (`/aichat2/conversations`) — это новое поколение интерфейса диалогов, являющееся полной модернизацией [AI Chat API](https://platform.acedata.cloud/documents/aichat-conversations). Он расширяет возможности v1, сохраняя простоту и поддержку многошаговых диалогов, добавляя:
...
Для использования API необходимо подать заявку на соответствующую услугу на странице [AI Chat v2 API](https://platform.acedata.cloud/documents/aichat2-conversations). После перехода нажмите кнопку «Acquire», чтобы получить необходимые учетные данные для запросов.
...
Если вы уже используете [`/aichat/conversations`](https://platform.acedata.cloud/documents/aichat-conversations), переход на v2 почти не требует изменений:
