...
- **Прерываемость / восстановимость**: модель будет отправлять событие `ask_user_question` и приостанавливать выполнение, когда потребуется дополнительная информация от пользователя, следующая вызов может продолжить выполнение, заполнив ответ через `tool_results`.
...
with requests.post(url, json=payload, headers=headers, stream=True) as resp:
    ...
    for line in resp.iter_lines():
        ...
        if event.get("type") == "text_delta":
            ...
            print()
            print("использование =", event.get("usage"))
```
...
| `compact`           | Событие, в котором внутренний контекст был сжат, не требует специальной обработки.                                                                             |
...
## Асинхронное выполнение и безлюдное авторизованное выполнение
...
Значения в `allowed_skills` — это slug подключенных навыков; значения в `allowed_mcp_servers` — это slug подключенных MCP серверов. Навыки / MCP серверы, не включенные в предварительную авторизацию, в безлюдном режиме могут только просматривать, выполнять dry-run или отклонять выполнение операций записи.
...
API AI Chat v2, сохраняя обратную совместимость с v1, обновляет беседы с «одностороннего / многослойного вопроса» до «агентных наблюдаемых бесед»: мультимодальный ввод, вызовы инструментов, возможность приостановки / восстановления, потоковые структурированные события, встроенный CRUD. Рекомендуется новым пользователям сразу использовать v2; существующие интеграции v1 могут плавно перейти поэтапно. Если у вас есть какие-либо вопросы, пожалуйста, не стесняйтесь обращаться в нашу техническую поддержку.
