...
- **Инструменты вызова в стиле Agent**: встроенный набор инструментов для сетевого поиска, веб-скрапинга, чтения файлов и возможность подключения серверов MCP с разрешением пользователя (Google Drive, Notion, Slack, GitHub и др.), модель может многократно вызывать инструменты в одном запросе для выполнения сложных задач.
...
- **Прерываемый / восстанавливаемый**: модель будет отправлять событие `ask_user_question` и приостанавливать выполнение, когда потребуется дополнительная информация от пользователя, а при следующем вызове можно будет продолжить, заполнив ответ через `tool_results`.
- **Новые действия CRUD**: на одном и том же конечном пункте можно выполнять `retrieve` / `retrieve_batch` / `update` / `delete` через поле `action`, без необходимости в дополнительном API для управления сессиями.
...
При этом на уровне тела запроса **полностью сохраняется обратная совместимость с v1**: достаточно передать `model` + `question` (+ опционально `stateful` / `id` / `references` / `preset`), чтобы получить эквивалентный ответ в формате JSON `{answer, id}`, поэтому для миграции с `/aichat/conversations` не требуется переписывать клиент, достаточно изменить путь на `/aichat2/conversations`.
...
**Один API Token позволяет вызывать все сервисы платформы, не нужно запрашивать отдельный токен для каждого сервиса.** При первом запросе предоставляется бесплатный лимит, чтобы вы могли попробовать; при недостатке лимита можно пополнить общий баланс в [консоли](https://platform.acedata.cloud/console/coin).
...
- Google: `gemini-3.1-pro-preview`, `gemini-3.1-pro-preview`, `gemini-3.1-flash-image`, `gemini-3.1-pro-preview`, `gemini-2.5-flash-lite` и др.
- xAI: `grok-4` и др.
...
| `system_message`    | Системное сообщение (не содержание пользователя и помощника), используется только для UI подсказок.                                                                     |
...
Значения в `allowed_skills` — это slug подключенных навыков; значения в `allowed_mcp_servers` — это slug подключенных MCP серверов. Навыки / MCP серверы, не включенные в предварительное разрешение, в безлюдном режиме могут только просматривать, выполнять dry-run или отклонять выполнение операций записи.
...
Предварительное разрешение — это сами два списка: пустой список означает отсутствие разрешений, дополнительных переключателей не требуется.
...
```json
{
  ...
  "question": "Какой язык вы хотите для отчета, китайский или английский?",
  "options": ["китайский", "английский"],
  "id": "f2f4b3e8-..."
}
```
...
v2 предоставляет легковесное управление сессиями через поле `action` на том же конечном пункте, без необходимости открывать дополнительный API.
...
Возвращает `{ items: [...], total }`. **Сводка не содержит `messages`**, подходит для списка в боковой панели; если пользователь откроет определённую беседу, затем с помощью `action: retrieve` можно отдельно получить её полные сообщения.
...
2. Если вы ранее использовали имена моделей v1 (например, `gpt-3.5`, `gpt-4-browsing` и т.д.), при переходе на v2 рекомендуется обновить до современных моделей (например, `gpt-5.4`, `claude-opus-4-8`, `gemini-3.1-pro-preview` и т.д.).
...
API AI Chat v2, оставаясь обратно совместимым с v1, обновляет диалог с «однократного / многократного вопроса» на «агентные наблюдаемые диалоги»: мультимодальный ввод, вызовы инструментов, возможность приостановки / восстановления, потоковые структурированные события, встроенный CRUD. Рекомендуется новым пользователям сразу использовать v2; существующие интеграции v1 могут плавно перейти поэтапно. Если у вас есть какие-либо вопросы, пожалуйста, не стесняйтесь обращаться в нашу техническую поддержку.
