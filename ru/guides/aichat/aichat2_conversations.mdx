...
AI Chat v2 API (`/aichat2/conversations`) — это новое поколение диалогового интерфейса, являющееся полным обновлением [AI Chat API](https://platform.acedata.cloud/documents/aichat-conversations). Он расширяет возможности v1, основанные на простоте и управлении многократными диалогами:
...
- **Прерываемость / восстановимость**: модель отправляет событие `ask_user_question` и приостанавливается, когда ей требуется дополнительная информация от пользователя, следующая вызов может продолжить с помощью `tool_results`.
...
При этом на уровне тела запроса **полностью обратно совместим с v1**: достаточно передать `model` + `question` (+ опционально `stateful` / `id` / `references` / `preset`), чтобы получить эквивалентный ответ в формате `{answer, id}` JSON, поэтому миграция с `/aichat/conversations` не требует переписывания клиента, достаточно изменить путь на `/aichat2/conversations`.
...
```json
{
  "answer": "AceDataCloud — это единая API-платформа, объединяющая основные AI-модели и мультимодальные сервисы, позволяющая разработчикам вызывать GPT, Claude, Gemini, Midjourney, Suno, Veo и другие сервисы с помощью одного ключа.",
...
- xAI: `grok-4` и др.
- DeepSeek: `deepseek-v4-pro`, `deepseek-v4-flash`, `deepseek-v3.2-exp`, `deepseek-r1-0528` и др.
- Moonshot: `kimi-k3`, `kimi-k2.6`, `kimi-k2.5` и др.
- Zhipu: `glm-5.3`, `glm-5.2`, `glm-5.1`, `glm-5`, `glm-5-turbo`, `glm-4.7`, `glm-4.5v` и др.

Конкретные правила тарификации см. на странице сервиса в карточке Pricing.
...
v2 поддерживает два типа потокового формата, в зависимости от заголовка `accept`:

| Сцена                    | `accept`               | Формат данных                                       |
| --------------------- | ---------------------- | ------------------------------------------ |
| Веб интерфейс / EventSource  | `text/event-stream`    | `data: {json}\n\n`, последняя строка `data: [DONE]\n\n` |
...
payload = {
    "model": "gpt-5.4",
    "stateful": True,
    "question": "Представь Ханчжоу тремя предложениями.",
}
...
Каждая строка NDJSON представляет собой структурированное событие, наиболее распространенное из которых — `text_delta`:
...
| `type`              | Значение                                                                                              |
| ------------------- | ----------------------------------------------------------------------------------------------- |
| `text_delta`        | Инкремальные текстовые фрагменты ответа помощника. `content` — это новый контент; для совместимости с v1, это событие также содержит `delta_answer` (равный `content`) и `id`.                  |
...
| `tool_result`       | Результат выполнения инструмента, сопоставляется с предыдущим `tool_use` по `tool_id`, `is_error` указывает на неудачу.                                       |
...
| `artifact`          | Независимый продукт, созданный моделью (например, блок кода, документ), может быть сохранен или загружен.                                                                      |
| `system_message`    | Системное сообщение (не контент пользователя и помощника), используется только для UI подсказок.                                                                     |
...
Если ввод пользователя содержит изображения или файлы, передайте `message` (массив) вместо `question`. Каждый элемент массива — это блок контента:
...
- Если также предоставлен `question`, то он будет передан как блок `text` перед другими.
...
Если требуется более тонкий контроль (например, разместить несколько изображений между текстами или порядок имеет значение), используйте массив `message`.

## Вызов инструментов и MCP

Основное улучшение v2 заключается в том, что модель может самостоятельно вызывать инструменты для выполнения многократных задач, **это включено по умолчанию**, клиенту не нужно делать никаких дополнительных настроек в запросе. Распространенные сценарии:
...
- Пользователь спрашивает «Прочитай этот PDF и напиши резюме» → модель вызывает file_read → пишет резюме.
...
Если вы не хотите отображать детали вызова инструментов на фронтенде, просто игнорируйте события `tool_use` / `tool_result` / `card` / `citation`, итоговый вывод модели по-прежнему будет передаваться через `text_delta`.
...
## Асинхронное выполнение и безлюдное авторизование
...
Затем можно использовать `action: retrieve` + `id` для запроса результата сессии; также можно предоставить `callback_url`, после завершения задачи платформа отправит `{ status, answer, usage, error }` на ваш адрес обратного вызова. `callback_url` должен использовать `http` / `https`, и не может быть напрямую указан `localhost` или частный IP-адрес.

Фоновые задачи обычно не требуют подтверждения от человека. Если вы хотите, чтобы некоторые навыки или MCP серверы выполняли действия отправки, публикации, записи и т.д. в безлюдном режиме, явно передайте список предварительных авторизаций в теле запроса:
...
Значения в `allowed_skills` — это slug подключенных навыков; значения в `allowed_mcp_servers` — это slug подключенных MCP серверов. Навыки / MCP серверы, не включенные в предварительную авторизацию, в безлюдном режиме все равно могут только просматривать, выполнять dry-run или отклонять выполнение операций записи.
...
Предварительная авторизация — это сами два списка: пустой список означает отсутствие разрешения на любые действия, без дополнительных переключателей.

Обратите внимание: предварительная авторизация только обозначает «в этом запросе разрешены эти возможности в безлюдном режиме без подтверждения человека». Конкретный навык все равно должен поддерживать `--unattended-confirm` или соответствующий механизм безопасности; в противном случае он продолжит выполнять dry-run и не будет напрямую выполнять операции записи.

## Восстановление приостановленного диалога

Некоторые инструменты могут заставить модель «задать вопрос пользователю», в этом случае модель отправит событие `ask_user_question`, диалог будет заморожен в состоянии `awaiting_user_input`:
...
На фронтенде это событие можно отобразить как карточку, чтобы пользователь выбрал ответ, а затем с тем же `id` отправить следующий запрос, заполнив ответ через `tool_results`:
...
Возвращает `{ items: [...], total }`. **Сводка не содержит `messages`**, подходит для бокового списка; если пользователь откроет какую-либо беседу, затем с помощью `action: retrieve` можно отдельно получить ее полные сообщения.
...
`messages` также можно передать, но сервер будет проводить строгую проверку схемы (должен быть в форме свернутого `ToolUseContent`), несоответствие приведет к ошибке 400. Обычно рекомендуется использовать только для изменения `title`.
...
2. Если вы ранее использовали имя модели v1 (например, `gpt-3.5`, `gpt-4-browsing` и т.д.), при переходе на v2 рекомендуется обновить до современных моделей (например, `gpt-5.4`, `claude-opus-4-8`, `gemini-3.1-pro` и т.д.).
3. Поля потока NDJSON остаются обратно совместимыми: каждое событие `text_delta` по-прежнему содержит `delta_answer` и `id`, поэтому клиент, который ранее разбирал `delta_answer` построчно, не требует изменений.

После миграции можно по мере необходимости включать новые возможности v2 (мультимодальные `message`, SSE, вызовы инструментов, CRUD `action`), продвигаясь в своем темпе.
...
```json
{
  "error": {
    "code": "chat_error",
    "message": "модельный сервис вернул ошибку"
  },
...
- `500 chat_error`: ошибка на стороне LLM или текущий `completion_tokens=0` (обрабатывается как неиспользованный, плата не взимается).

В потоковом ответе ошибки отправляются как `{"type":"error","message":"..."}` событие, после чего поток завершится.

## Заключение

API AI Chat v2, сохраняя обратную совместимость с v1, обновляет беседы с «однократного / многократного вопроса» до «агентных наблюдаемых бесед»: мультимодальный ввод, вызовы инструментов, возможность приостановки / восстановления, потоковые структурированные события, встроенный CRUD. Рекомендуется новым пользователям сразу использовать v2; существующие интеграции v1 могут плавно перейти поэтапно. Если у вас есть какие-либо вопросы, пожалуйста, не стесняйтесь обращаться в нашу техническую поддержку.
