...
Extract принимает **все** параметры [Render API](development_webextrator_render) (`url`, `user_agent`, `timeout`, `wait_until`, `delay`, `wait_for_selector`, `block_resources`, `headers`, `cookies`, `callback_url`, `bypass_cache`, `cache_ttl_seconds`, `async`), плюс два эксклюзивных поля Extract:
...
> Когда страница содержит schema.org JSON-LD, `enable_llm` неактивен — детерминированный маппер сразу выдает результат, никогда не тратя вызов LLM. Вы **бесплатно** получаете типизированный результат.
...
| `cached`        | boolean? | При попадании в кэш равно `true`.                                                                       |
...
### 3. Страница рецепта AllRecipes (с питательными веществами и шагами)
...
### 4. Страница обсуждения HN (без JSON-LD — требуется включить LLM)
...
### 5. Страница товара Amazon (Amazon без JSON-LD — требуется включить LLM)
...
- **`enable_llm: true` на страницах с совпадением schema.org бесплатно.** LLM вызывается только тогда, когда в schema.org нет primary, поэтому по умолчанию это также безопасно.
- **При отладке сначала смотрите `rawSignals.hasJsonLd`.** Если это `true`, но `structured.schemaOrg.primary` равно `null`, это означает, что страница использует тип `@type`, который еще не покрыт нашим маппером — создайте issue, мы добавим.
...
- **Кэширование также подлежит оплате.** Кэш предназначен для задержки и защиты пула браузеров, а не для экономии денег.
