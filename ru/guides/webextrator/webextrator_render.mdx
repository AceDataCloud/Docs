...
| `cookies`           | array    |     ❌      | —                            | Cookie, инжектируемые перед навигацией, структура указана ниже.          |
...
> Платформенные контракты используют **snake_case**. Внутренние рендеринговые сервисы поддерживают camelCase, но для внешних вызовов используется только snake_case.
...
Платформа `POST` отправляет **полностью идентичный** envelope в синхронном режиме на `callback_url`, `Content-Type: application/json`. Возврат любого `2xx` считается подтвержденным; `5xx` будет повторяться с экспоненциальной задержкой в течение примерно 5 минут.
