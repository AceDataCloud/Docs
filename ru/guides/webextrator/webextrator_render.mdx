...
| `data.status`        | number \| null | HTTP статус код основного навигации.                  |
...
| `data.cacheStoredAt` | number?        | Unix временная метка в миллисекундах, когда запись в кэш была выполнена впервые. |
...
- **Ключ кэша игнорирует `async`.** Синхронные и асинхронные запросы к одному и тому же URL попадают в одну и ту же запись кэша, 
  произвольный переход не приведет к сбою.
- **Ключ кэша игнорирует `bypass_cache` и `cache_ttl_seconds`.** Эти два являются переключателями, 
  не влияющими на содержимое ответа.
- **`cookies` и `headers` будут кэшироваться по отдельным корзинам.** Настройка этих двух приведет к сбою попадания 
  в кэш при первом совпадении.
- **Перезагрузка SPA часто превышает стандартные 30 секунд.** Рекомендуется `timeout: 60`, `wait_until: "domcontentloaded"`, `delay: 4`, 
  а также использовать `wait_for_selector` для ожидания действительно интересующих элементов.
