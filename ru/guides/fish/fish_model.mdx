...
```json
{
  ...
  "error": {
    "code": "api_error",
    "message": "необходим параметр voices, который должен быть HTTP(S) URL, указывающим на аудиофайл"
  },
...
Для вызова этого интерфейса необходимо подготовить доступный для публичного просмотра URL образца аудио, поместить его в поле `voices`, чтобы получить `_id`, а затем передать его в `reference_id` для `/fish/tts`, чтобы завершить процесс «клонирование → синтез». Распространенная ошибка заключается в том, что `voices` должен быть строкой, а не массивом.
