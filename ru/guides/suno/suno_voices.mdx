...
После получения ID голосового персонажа можно использовать [Suno Audios Generation API](https://platform.acedata.cloud/documents/suno-audios) для генерации музыки. Установите `action` в `generate`, а `persona_id` — в ID созданного голосового персонажа. Сгенерированная песня будет исполнена клонированным голосом.
