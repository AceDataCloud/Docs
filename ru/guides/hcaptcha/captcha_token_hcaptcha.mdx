...
Чтобы использовать hCaptcha протокол распознавания API, сначала перейдите в [консоль Ace Data Cloud](https://platform.acedata.cloud/console/applications) и получите ваш API Token для резервного копирования.
...
Сначала ознакомьтесь с основным способом использования, который заключается в вводе URL сайта, на котором необходимо обработать hCaptcha, чтобы получить обработанный результат. Сначала нужно просто передать поле `website_url`, наш пример сайта: `https://accounts.hcaptcha.com/demo`, нам нужно получить `website_key` на странице `website_url`, для этого откройте эту веб-страницу, нажмите F12 для входа в консоль, и затем выполните глобальный поиск по элементам на странице `hcaptcha-demo`, мы можем получить следующий результат:
...
- `authorization`: ключ для вызова API, после подачи заявки вы можете выбрать его из выпадающего списка.
...
- `proxy`: необязательный, собственный прокси (Bring Your Own Proxy). После установки, сервер будет использовать предоставленный вами прокси IP для решения капчи, чтобы контролировать качество выходного IP (например, чтобы избежать блокировки целевым сайтом из-за использования общих прокси IP, что может привести к возврату `410 Gone`). Формат: `scheme://[user:pass@]host:port`, `scheme` поддерживает `http`/`https`/`socks4`/`socks5`, например `http://user:pass@1.2.3.4:8080`. Если не указано, будет использоваться прокси по умолчанию от платформы.
...
Возвращаемый результат состоит из нескольких полей, описание которых приведено ниже:

- `token` — результат проверки после обработки задачи hCaptcha.
