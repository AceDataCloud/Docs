...
**Один API Token позволяет использовать все услуги платформы, не нужно подавать отдельные заявки на каждую услугу.** При первой подаче заявки предоставляется бесплатный лимит для тестирования; при недостатке лимита вы можете пополнить общий баланс в [консоли](https://platform.acedata.cloud/console/coin).
...
- `rqdata`: необязательный. Вызов hCaptcha Enterprise может предоставить `data-rqdata` на странице, в случае таких вызовов укажите его исходное значение; обычная hCaptcha не требует заполнения.
- `proxy`: необязательный, собственный прокси (Bring Your Own Proxy). После установки система будет использовать предоставленный вами IP прокси для решения капчи, чтобы контролировать качество выходного IP (например, избежать блокировки целевым сайтом из-за использования общих прокси IP, что может привести к возврату `410 Gone`). Формат: `scheme://[user:pass@]host:port`, `scheme` поддерживает `http`/`https`/`socks4`/`socks5`, например, `http://user:pass@1.2.3.4:8080`. Если не указано, будет использоваться прокси по умолчанию от платформы.
...
Результат запроса содержит несколько полей, описание которых приведено ниже:
