...
Чтобы использовать DeepSeek Chat Completion API, сначала можно перейти на страницу [DeepSeek Chat Completion API](https://platform.acedata.cloud/documents/deepseek-chat-completions) и нажать кнопку «Acquire», чтобы получить необходимые для запроса учетные данные:
