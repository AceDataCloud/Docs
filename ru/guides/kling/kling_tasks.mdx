...
В этом документе подробно описывается интеграция API Kling Tasks, чтобы помочь вам легко интегрировать и в полной мере использовать мощные возможности этого API. С помощью API Kling Tasks вы можете легко реализовать запрос статуса выполнения задач API Kling Videos Generation.

## Процесс подачи заявки

Чтобы использовать API Kling Videos Generation, сначала перейдите в [консоль Ace Data Cloud](https://platform.acedata.cloud/console/applications), чтобы получить ваш API Token для резервного копирования.
...
API Kling Tasks можно использовать для запроса результатов API Kling Videos Generation. Для получения информации о том, как использовать API Kling Videos Generation, пожалуйста, обратитесь к документации [Kling Videos Generation API](https://platform.acedata.cloud/documents/kling-videos).

Мы возьмем в качестве примера идентификатор задачи, возвращаемый сервисом API Kling Videos Generation, и продемонстрируем, как использовать этот API. Предположим, у нас есть идентификатор задачи: 20068983-0cc9-4c6a-aeb6-9c6a3c668be0, далее мы покажем, как передать этот идентификатор задачи.
...
- `id`, идентификатор задачи, сгенерированной для этого видео, используется для уникальной идентификации этой задачи на создание видео.
...
Это запрос для получения деталей видео задач по нескольким идентификаторам задач, в отличие от предыдущего, здесь необходимо выбрать action как retrieve_batch.
