---
title: "Veo Extend API 接口说明"
description: "Veo Video Generation API guide - Ace Data Cloud"
---

本文将介绍 Veo Extend API 的接口说明。该接口用于对一段已经生成的 Veo 视频进行**时长延展**，AI 会自动续写后续画面。
...
- `aspect_ratio`（选填）：`16:9` 或 `9:16`，默认 `16:9`。
- `resolution`（选填）：`720p` 或 `1080p`。
...
`video_id` 必须来自 TTAPI 线路生成或延展完成后的 `data[].id`。切换 TTAPI 之前由旧线路生成的历史视频 ID 无法用于该接口，调用时会返回 `400 bad_request`；请通过 `/veo/videos` 重新生成后再延展。
