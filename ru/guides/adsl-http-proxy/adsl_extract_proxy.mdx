---
title: "ADSL Proxy API Заявка и использование"
description: "ADSL Direct Connection Proxy API guide - Ace Data Cloud"
---

В этом документе в основном описывается процесс использования ADSL Proxy API, с его помощью мы можем легко извлекать IP ADSL и добавлять IP в белый список ADSL Proxy.

## Процесс заявки

Чтобы использовать извлечение IP ADSL и добавление IP в белый список ADSL Proxy, сначала можно перейти на страницы [ADSL Proxy Extract API](https://platform.acedata.cloud/documents/adsl-extract) и [ADSL Proxy Whitelist API](https://platform.acedata.cloud/documents/adsl-whitelist) и нажать кнопку «Acquire», чтобы получить необходимые для запроса учетные данные:

![](https://cdn.acedata.cloud/nyq0xz.png)

Если вы еще не вошли в систему или не зарегистрированы, вас автоматически перенаправит на страницу входа, приглашая вас зарегистрироваться и войти в систему. После входа или регистрации вы автоматически вернетесь на текущую страницу.

При первой заявке будет предоставлен бесплатный лимит, который можно использовать для бесплатного использования этого API.

## Извлечение IP ADSL

Далее вы можете заполнить соответствующие данные на странице [ADSL Proxy Extract API](https://platform.acedata.cloud/documents/adsl-extract), далее демонстрируется, как извлечь IP ADSL.
...
- `authorization`: ключ для вызова API, после заявки можно выбрать из выпадающего списка.

**Request Body** включает:

- `number`: количество извлекаемых IP ADSL, по умолчанию 1.
- `type`: тип извлекаемого IP ADSL, в основном делится на `http` и `socks`, по умолчанию `http`.
- `province`: провинция, в которой находится извлекаемый IP, по умолчанию случайная.
- `city`: город, в котором находится извлекаемый IP, по умолчанию случайный.

Для параметров `province` и `city` есть требования, они должны соответствовать друг другу, например, `city`: Наньчан должен соответствовать `province`: Цзянси, необходимо, чтобы они соответствовали провинции, конкретные правила следующие:
```json
