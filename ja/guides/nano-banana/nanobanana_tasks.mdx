...
Nano Banana Tasks API は、Nano Banana Images API の結果を照会するために使用できます。Nano Banana Images API の使用方法については、文書 [Nano Banana Images API](https://platform.acedata.cloud/documents/veo-videos) を参照してください。
...
- `403 forbidden`：提供者のネイティブセキュリティポリシーがリクエストまたは生成結果を拒否しました；すべての生成呼び出しが拒否された場合、タスクは失敗状態で保存され、画像は含まれず、料金は発生しません。複数画像リクエストの他の成功した呼び出しは、実際の枚数に基づいて返され、料金が発生します。
