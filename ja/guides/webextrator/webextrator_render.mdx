...
| `wait_for_selector` | string   |  ❌  | —                          | この CSS セレクタが出現するのを待機します。`networkidle` よりも安定しています。                                       |
...
| `data.elapsedMs`     | number         | ブラウザのレンダリングにかかった時間。                                         |
...
プラットフォームは、同期モードと**完全に同じ**エンベロープを `callback_url` に `POST` します。
...
即座に `{ "success": true, "task_id": "...", "trace_id": "...", "started_at": "..." }` を返します；
...
- **`wait_until` の選択は非常に重要です。** `networkidle` は最も安定していますが最も遅い；`domcontentloaded`
  は速いですが非同期で注入されたコンテンツを見逃す可能性があります；`load` は従来の静的ページに適しています。
