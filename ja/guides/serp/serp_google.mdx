---
title: "Google SERP API 接続説明"
description: "Search Engine API guide - Ace Data Cloud"
---

Google SERP（検索エンジン結果ページ）は、ユーザーが Google 検索エンジンにクエリを入力した後に表示される結果ページです。自然検索結果、広告、特徴的な要約、知識グラフ、画像、動画など、さまざまなコンテンツを表示し、ユーザーに最も関連性の高い情報を提供することを目的としています。

この記事では、Google SERP API について詳しく説明します。これは、Google 検索エンジンにクエリを入力して結果を取得するためのもので、結果の内容には特徴的な要約、知識グラフ、画像など、さまざまなタイプが含まれています。

この文書では、Google SERP API の接続説明を紹介します。

## 申請プロセス

Google SERP API を使用するには、まず [Ace Data Cloud コントロールパネル](https://platform.acedata.cloud/console/applications) にアクセスして、API トークンを取得し、保管してください。

![](https://cdn.acedata.cloud/dvc3cg.jpg)

まだログインまたは登録していない場合は、自動的にログインページにリダイレクトされ、登録とログインを促されます。完了後、現在のページに自動的に戻ります。

**1つの API トークンでプラットフォームのすべてのサービスを呼び出すことができ、各サービスごとに個別に申請する必要はありません。** 初回申請時には無料のクレジットが付与され、無料で体験できます。クレジットが不足した場合は、[コントロールパネル](https://platform.acedata.cloud/console/coin) で一般残高をチャージできます。

> 📘 完全な文書：[Google SERP API →](https://platform.acedata.cloud/documents/serp-google)

## 基本的な使用法

まず、基本的な使用方法を理解しましょう。検索リソースのタイプとキーワードを入力することで、検索結果を得ることができます。`query` フィールドを単純に渡し、対応するモデルを指定するだけです。

例えば、「apple inc」に関する情報を探す場合、次に画面に対応する内容を入力できます。以下のように：

<p><img src="https://cdn.acedata.cloud/lnqiye.png" width="500" className="m-auto" /></p>

ここでは、リクエストヘッダーを設定しました。内容は以下の通りです：

- `accept`：受け取りたいレスポンス結果の形式。ここでは `application/json`、つまり JSON 形式を指定します。
- `authorization`：API を呼び出すためのキー。申請後、直接ドロップダウンから選択できます。

さらに、リクエストボディを設定しました。内容は以下の通りです：

- `type`：検索リソースのタイプ。現在は6種類のみサポートされており、デフォルトは `search` です。
- `query`：検索するキーワード。
- `country`：検索結果の国をカスタマイズ。デフォルトはアメリカ（US）です。
- `language`：検索結果の言語をカスタマイズ。デフォルトは英語（en）です。
- `range`：検索結果の時間範囲をカスタマイズ。デフォルトは制限なしです。
- `number`：検索結果のページサイズをカスタマイズ。デフォルトは10です。
- `page`：検索結果のページ数をカスタマイズ。デフォルトは1です。

選択後、右側にも対応するコードが生成されていることがわかります。以下のように：

<p><img src="https://cdn.acedata.cloud/1j81zr.png" width="500" className="m-auto" /></p>

「Try」ボタンをクリックするとテストが行えます。上の図のように、以下の結果が得られました：
```json
{
  "knowledge_graph": {
    "title": "アップル",
    "type": "テクノロジー企業",
    "website": "http://www.apple.com/",
    "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT5ITHsQzdzkkFWKinRe1Y4FUbC_Vy3R_M&s=0",
    "description": "Apple Inc.は、カリフォルニア州クパチーノに本社を置くアメリカの多国籍企業であり、テクノロジー企業です。主に消費者向け電子機器、ソフトウェア、サービスで知られています。",
    "description_source": "ウィキペディア",
    "description_link": "https://en.wikipedia.org/wiki/Apple_Inc.",
    "attributes": {
      "カスタマーサービス": "1 (800) 275-2273",
      "創業者": "スティーブ・ジョブズ、スティーブ・ウォズニアック、ロナルド・ウェイン",
      "設立": "1976年4月1日、ロスアルトス、CA",
      "本社": "クパチーノ、CA",
      "CEO": "ティム・クック (2011年8月24日–)"
    }
  },
  "organic": [
    {
      "title": "アップル",
      "link": "https://www.apple.com/",
      "snippet": "アップルの革新的な世界を発見し、iPhone、iPad、Apple Watch、Mac、Apple TVのすべてを購入し、アクセサリー、エンターテイメントなどを探索してください。",
      "sitelinks": [
        {
          "title": "サポート",
          "link": "https://support.apple.com/"
        },
        {
          "title": "キャリアの機会",
          "link": "https://www.apple.com/careers/us/"
        },
        {
          "title": "ビジネス",
          "link": "https://www.apple.com/business/"
        },
        {
          "title": "ストア",
          "link": "https://www.apple.com/store"
        },
        {
          "title": "投資家向け情報",
          "link": "https://investor.apple.com/investor-relations/default.aspx"
        }
      ],
      "position": 1
    },
    {
      "title": "Apple Inc. - ウィキペディア",
      "link": "https://en.wikipedia.org/wiki/Apple_Inc.",
      "snippet": "Apple Inc.は、カリフォルニア州クパチーノに本社を置くアメリカの多国籍企業であり、テクノロジー企業です。",
      "position": 2
    },
    {
      "title": "Apple Inc. | 歴史、製品、本社、事実 - ブリタニカ",
      "link": "https://www.britannica.com/money/Apple-Inc",
      "snippet": "アメリカのパーソナルコンピュータ、スマートフォン、タブレットコンピュータの製造業者。アップルは最初の成功したパーソナルコンピュータ会社であり...",
      "date": "7日前",
      "position": 3
    },
    {
      "title": "Apple Inc. (AAPL) 会社プロフィールと事実 - Yahoo Finance",
      "link": "https://finance.yahoo.com/quote/AAPL/profile/",
      "snippet": "Apple Inc.は、スマートフォン、パーソナルコンピュータ、タブレット、ウェアラブル、アクセサリーを世界中で設計、製造、販売しています。会社はiPhone、ライン...",
      "position": 4
    },
    {
      "title": "AAPL: Apple Inc 株価引用 - NASDAQ GS - Bloomberg",
      "link": "https://www.bloomberg.com/quote/AAPL:US",
      "snippet": "Apple Inc.は、スマートフォン、パーソナルコンピュータ、タブレット、ウェアラブル、アクセサリーを設計、製造、販売し、さまざまな関連アクセサリーを販売しています。",
      "position": 5
    },
    {
      "title": "Apple Inc. (AAPL) 今日の株価 - WSJ",
      "link": "https://www.wsj.com/market-data/quotes/AAPL?gaa_at=eafs&gaa_n=ASWzDAiLDLUKmLSxoaZi5n84id-eGDp9gI99K5VU1R7pK3SzSi_PbdpD61P1&gaa_ts=685148c8&gaa_sig=mGk3lG_2W7ZqrXfXSXv7GAP78KUi_uJEadAXuv2bgYJ8Ah42GhIZxrsN4ynbNg_oKD_7NZS1lRPSglUNacIheQ%3D%3D",
      "snippet": "Apple Inc.は、スマートフォン、パーソナルコンピュータ、タブレット、ウェアラブル、アクセサリー、その他の関連製品の設計、製造、販売に従事しています。",
      "position": 6
    }
  ],
  "people_also_ask": [
    {
      "question": "Apple Incとは何ですか？",
      "snippet": "Apple Inc.、元々はApple Computer, Inc.は、消費者向け電子機器とそれに付随するコンピュータソフトウェアを作成・販売する多国籍企業であり、メディアコンテンツのデジタル配信者です。Appleの主な製品ラインは、iPhoneスマートフォン、iPadタブレットコンピュータ、Macパーソナルコンピュータです。",
      "title": "Apple Incの歴史 - ウィキペディア",
      "link": "https://en.wikipedia.org/wiki/History_of_Apple_Inc."
    },
    {
      "question": "Apple Incの所有者は誰ですか？",
      "snippet": "Apple (AAPL) 所有権の概要 Apple (AAPL) 株の所有構造は、機関投資家、小売投資家、個人投資家の混合です。会社の株の約50.57%は機関投資家が所有し、0.06%は内部者が所有し、49.38%は公開企業と個人投資家が所有しています。",
      "title": "Appleの所有者は誰ですか？ AAPL株の所有権 - TipRanks.com",
      "link": "https://www.tipranks.com/stocks/aapl/ownership"
    },
    {
      "question": "Apple Incは何で最も知られていますか？",
      "snippet": "それは消費者向け電子機器、ソフトウェア、サービスで最も知られています。1976年にスティーブ・ジョブズ、スティーブ・ウォズニアック、ロナルド・ウェインによってApple Computer Companyとして設立され、翌年にジョブズとウォズニアックによってApple Computer, Inc.として法人化されました。",
      "title": "Apple Inc. - ウィキペディア",
      "link": "https://en.wikipedia.org/wiki/Apple_Inc."
    },
    {
      "question": "Apple IncのIncの意味は何ですか？",
      "snippet": "incorporatedの略：法的に設立された米国企業の名前に使用される：Bishop Computer Services, Inc.",
      "title": "Inc. | 英語の意味 - ケンブリッジ辞典",
      "link": "https://dictionary.cambridge.org/dictionary/english/inc"
    }
  ],
  "related_searches": [
    {
      "query": "Apple iPhone 13"
    },
    {
      "query": "apple inc とは何か"
    },
    {
      "query": "Apple Incの完全な形式"
    },
    {
      "query": "Apple Incの住所"
    },
    {
      "query": "Apple Incの業界"
    },
    {
      "query": "Apple Incの投資家向け情報"
    },
    {
      "query": "Apple Incの年次報告書"
    },
    {
      "query": "Apple Store"
    }
  ]
}
```

返す結果には複数のフィールドが含まれています。以下のように説明します：

- `knowledge_graph`、検索結果の知識グラフ。
- `organic`、検索結果の詳細情報。
- `people_also_ask`、検索キーワードに関連する質問。
- `related_searches`、検索キーワードに関連する検索。

ここで、`organic`フィールドには検索キーワードの結果が含まれています。

また、対応する接続コードを生成したい場合は、生成されたものを直接コピーできます。例えば、CURLのコードは以下の通りです：

```shell
curl -X POST 'https://api.acedata.cloud/serp/google' \
-H 'accept: application/json' \
-H 'authorization: Bearer {token}' \
-H 'content-type: application/json' \
-d '{
  "query": "apple inc"
}'
```

Pythonの接続コードは以下の通りです：

```python
import requests

url = "https://api.acedata.cloud/serp/google"

headers = {
    "accept": "application/json",
    "authorization": "Bearer {token}",
    "content-type": "application/json"
}

payload = {
    "query": "apple inc"
}

response = requests.post(url, json=payload, headers=headers)
print(response.text)
```

## カスタム検索タイプ

カスタム検索リソースのタイプを指定する場合、`type`パラメータを変更できます。これには、通常のリソース`search`、画像リソース`images`、ニュースリソース`news`、地図リソース`maps`、地域リソース`places`、動画リソース`videos`が含まれます。この記事では、動画リソース`videos`を例に示します。

次に、具体的な操作を示します。

まず、`type`パラメータを`videos`に設定し、`query`パラメータを正常に渡します。
<p><img src="https://cdn.acedata.cloud/czlt12.png" width="500" className="m-auto" /></p>

対応するコードは以下の通りです：

```shell
curl -X POST 'https://api.acedata.cloud/serp/google' \
-H 'accept: application/json' \
-H 'authorization: Bearer {token}' \
-H 'content-type: application/json' \
-d '{
  "type": "videos",
  "query": "apple inc"
}'
```

次のような回答が得られます：

```json
{
  "videos": [
    {
      "title": "ヘルスケア",
      "link": "https://www.apple.com/healthcare/",
      "snippet": "ショッピングの方法が増えました：近くのApple Storeまたは他の小売店を見つけるか、1-800-MY-APPLE（1-800-692-7753）に電話してください。アメリカ合衆国。著作権 © 2025 Apple Inc. 全て ...",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRQzUYLXB37srVLPW8JbVzhIlP9-SxMe3A8M98Eb7362as4&s",
      "source": "Apple",
      "date": "2025年4月16日",
      "position": 1
    },
    {
      "title": "Apple",
      "link": "https://www.youtube.com/apple",
      "snippet": "私たちの初の空間コンピュータ。· お母さんへの贈り物 | iPhoneで撮影した空間ビデオ | Apple Vision Proで体験 · Metallica — 公式トレーラー | Apple ...",
      "position": 2
    },
    {
      "title": "Appleはついに再びリスクを取るのか？",
      "link": "https://www.youtube.com/watch?v=JNZV9i5BwH0",
      "snippet": "Microsoftがどのように苦しんでいるかを考慮すると、MacをARMに移行することは「失敗すれば会社が潰れる」ようなものでした ...",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR0zvbxg3ozbeHNUJojmyRBhi3J-TviJQRinIS4DZiF-EM8&s",
      "video_url": "https://encrypted-vtbn0.gstatic.com/video?q=tbn:ANd9GcTfNZRuifDZb4zgj0d6wDxoHLI8yUr-8SIPNw",
      "duration": "13:04",
      "source": "YouTube",
      "channel": "ルーク・ミアーニ",
      "date": "2日前",
      "position": 3
    },
    {
      "title": "WWDC 25でAppleが話さなかったこととその重要性！",
      "link": "https://www.youtube.com/watch?v=2LTc3mzNh08",
      "snippet": "私は、憎悪のトレンドに従うために怒りを煽る動画を作るYouTuberのフォローを徐々にやめています。Appleは完璧からは程遠いですが、これらのテーマには ...",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTgGUJwgxdH-WHBmGOk6XgvijwnYX2H_D2Cl23aNab8CKyT&s",
      "video_url": "https://encrypted-vtbn0.gstatic.com/video?q=tbn:ANd9GcRTtYLkgIrCxERepA5KmCJDbZ7lTUa0OBHqzw",
      "duration": "13:05",
      "source": "YouTube",
      "channel": "ブライアン・トン",
      "date": "7時間前",
      "position": 4
    },
    {
      "title": "Apple",
      "link": "https://www.youtube.com/user/Apple",
      "snippet": "私たちの初の空間コンピュータ。· お母さんへの贈り物 | iPhoneで撮影した空間ビデオ | Apple Vision Proで体験 · Metallica — 公式トレーラー | Apple ...",
      "position": 5
    },
    {
      "title": "Apple Watch Ultra 2",
      "link": "https://www.apple.com/apple-watch-ultra-2/",
      "snippet": "Huish Outdoorsとのパートナーシップで設計されたOceanic+アプリは、Ultra 2をBühlmann減圧アルゴリズムを実行する完全なダイブコンピュータに変えます。",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSvZ58COtZS4zIIJDd1-Tl0GIKbFqoffFlhLa20OtLcILDR&s",
      "source": "Apple",
      "date": "2023年9月12日",
      "position": 6
    },
    {
      "title": "Apple at Work - 成功事例 - BDC",
      "link": "https://www.apple.com/business/enterprise/success-stories/financial-services/bdc/",
      "snippet": "Apple Business ManagerとAirWatchモバイルデバイス管理により、BDCはデバイスとアプリを会社全体で完全にゼロタッチで展開および管理できます ...",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRDQuxFNiitOIuPizPvTHwzQqyuLuypH_UNPqcZ-bGUAFy2&s",
      "source": "Apple",
      "date": "2021年11月2日",
      "position": 7
    },
    {
      "title": "高等教育 - 機関",
      "link": "https://www.apple.com/education/higher-education/",
      "snippet": "ショッピングの方法が増えました：近くのApple Storeまたは他の小売店を見つけるか、1-800-MY-APPLE（1-800-692-7753）に電話してください。アメリカ合衆国。著作権 © 2025 Apple Inc. 全て ...",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSIZRPg0iWQmiRPY-Ro1XmdUQYbwGmIxUgMjjL8TqdyutHF&s",
      "source": "Apple",
      "date": "2018年6月8日",
      "position": 8
    },
    {
      "title": "Apple: 1兆ドルの裏切り | あなたの ...についての真実",
      "link": "https://www.youtube.com/watch?v=wmfuE-RM7Rg",
      "snippet": "Appleは世界で最も価値のある会社であり、私たちの生活において他に類を見ない役割を果たしています。スマートフォンからスマートウォッチまで、 ...",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSvqw0lj-kWlq1HnowgEqzFxk5npWRpod2w1gogX-Dt4-jk&s",
      "video_url": "https://encrypted-vtbn0.gstatic.com/video?q=tbn:ANd9GcSG7N1JPdVZhrbuDckiwnYuWIQsXg_tdcmlvA",
      "duration": "52:40",
      "source": "YouTube",
      "channel": "モコノミー",
      "date": "2024年12月4日",
      "position": 9
    },
    {
      "title": "Apple at Work - 成功事例 - SAP",
      "link": "https://www.apple.com/business/enterprise/success-stories/tech/sap/",
      "snippet": "SAPは、従業員にこの選択肢を提供することが、単にコンピュータを選ぶことを超えていることを発見しました。Macは彼らの潜在能力を最大限に引き出します。一目で。105K+ ...",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSaplpBsRWWCJNlPtXsFX-jr3yw0mexpI6RtFKN6-TJAqrW&s",
      "source": "Apple",
      "date": "2022年4月13日",
      "position": 10
    }
  ]
}
```

返された結果には複数のフィールドがあり、以下のように説明されています：

- `news`、検索結果の動画情報リスト。

ここで返された結果には `videos` フィールドがあり、これは主に検索キーワードの結果を含んでいます。

## カスタム検索リソースの所在国

このインターフェースは、検索結果の所在国を制限することもサポートしており、`country` パラメータを追加して国を制限できます。入力するパラメータは国の略称で、例えば cn（中国）、us（アメリカ合衆国）です。この記事では中国を例にして以下の情報を示します：

<p><img src="https://cdn.acedata.cloud/gztpwi.png" width="500" className="m-auto" /></p>

出力効果は以下の通りです：
```json
{
  "news": [
    {
      "title": "Apple、iOS 26ベータ版の一般公開を7月に発表",
      "link": "https://www.tv47.digital/apple-announces-ios-26-beta-public-roll-out-set-for-july-105190/",
      "snippet": "テクノロジー大手のApple Inc.は、iPhone、Mac、iPad向けの最新のオペレーティングシステムのバージョンを発表し、重要な...",
      "date": "2時間前",
      "source": "TV47 Digital",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTBNrktH4AWMt8KS3hErOkI1oAeUvRm4Aaq10qnINCa3TmyabOCwiEMvxVWsA&s",
      "position": 1
    },
    {
      "title": "Bragar Eagel & Squire, P.C.がApple、Sable、Abacusを調査し、投資家に同社への連絡を促す",
      "link": "https://www.tradingview.com/news/reuters.com,2025-06-17:newsml_GNX87dzHl:0-bragar-eagel-squire-p-c-is-investigating-apple-sable-and-abacus-and-encourages-investors-to-contact-the-firm/",
      "snippet": "ニューヨーク、2025年6月16日（GLOBE NEWSWIRE） — Bragar Eagel & Squire, P.C.は、全国的に認知された株主権利法務事務所で...",
      "date": "10時間前",
      "source": "TradingView",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRdcHGK9sZ4M1wtCX6U74yV2ac9ClDX2hTSX4l4Brr1DWTvXfI4dKAn8sDU9g&s",
      "position": 2
    },
    {
      "title": "Apple Inc. (AAPL)のWWDCソフトウェアとAI発表に関する意見",
      "link": "https://www.nasdaq.com/articles/apple-inc-aapl-opinions-wwdc-software-and-ai-announcements",
      "snippet": "Apple Inc. (AAPL)に関する最近のXでの議論は、同社の最新のソフトウェアアップデートとAIイニシアチブに集中しており...",
      "date": "17時間前",
      "source": "Nasdaq",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSm5mhn6LRgzafnn34CrjlR3LS1_fHPmKNJckasyNgN4eYgBiCqGtulLQIViA&s",
      "position": 3
    },
    {
      "title": "ゴールドマン・サックス、Appleの購入を推奨、株価上昇を予想",
      "link": "https://finance.yahoo.com/news/goldman-sachs-recommends-buying-apple-042754922.html",
      "snippet": "Apple Inc. (NASDAQ:AAPL)は、次世代の配当貴族株の中で最も優れた株の一つです。ゴールドマン・サックスは最近、株のグループを強調しました...",
      "date": "6時間前",
      "source": "Yahoo Finance",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS6QgEG6x6t12L8YqJebmoiPNM_RMjmiVUpwam1YXZgKwBUPGe6yn76hR-n0g&s",
      "position": 4
    },
    {
      "title": "Apple、オランダのApp Storeの控訴に敗れ、さらなる不遵守罰金の脅威に直面",
      "link": "https://globalcompetitionreview.com/article/apple-threatened-more-noncompliance-fines-after-losing-dutch-app-store-appeal",
      "snippet": "オランダの消費者市場庁は、AppleのApp Storeルールを修正する命令に対する控訴を成功裏に退けました...",
      "date": "17時間前",
      "source": "Global Competition Review",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ02b47E8UHdyFL6A4Da4Wla_nXXc427Y9aMgE7vVuJpN4CzLQp0ip_G77FwA&s",
      "position": 5
    },
    {
      "title": "オランダの裁判所、Appleがデーティングアプリで優越的地位を濫用したことを確認",
      "link": "https://www.reuters.com/sustainability/boards-policy-regulation/dutch-court-confirms-apple-abused-dominant-position-dating-apps-2025-06-16/",
      "snippet": "オランダの裁判所は月曜日、2021年の消費者監視機関の判決を確認し、Appleが不当な条件を課すことによって優越的地位を濫用したと述べました...",
      "date": "19時間前",
      "source": "Reuters",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQLRbz3ypqJL6zJo1LBfMen68-D4csUJbGNNO7w8JODW6fiKliGk0yRyRGfvQ&s",
      "position": 6
    },
    {
      "title": "Appleは中国でAI iPhoneを救えるか？",
      "link": "https://www.bloomberg.com/opinion/articles/2025-06-10/can-apple-salvage-the-ai-iphone-in-china",
      "snippet": "Apple Inc.の人工知能の問題が米国で悪いと思っているなら、実際にはもっと悪化する可能性があることを知っておいてください。中国を見てください。",
      "date": "6日前",
      "source": "Bloomberg.com",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRzqIqxSek95GHKeeX9ekGIfzlZhz4jdEXqJPlk9Gyi5E8Yzk55iuhR5Soy_Q&s",
      "position": 7
    },
    {
      "title": "Apple、韓国に高精度地図データの海外転送を許可するよう要請",
      "link": "https://en.yna.co.kr/view/AEN20250617006800320",
      "snippet": "ソウル、6月17日（聯合ニュース） -- 米国のテクノロジー大手Apple Inc.は、韓国政府に...",
      "date": "5時間前",
      "source": "Yonhap News Agency",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS20gcfgCXs4W5Hzu78JWocLIX_9jQPbN1rUUPctbiJ1UKd7R7eotC4CodJdQ&s",
      "position": 8
    },
    {
      "title": "オランダの裁判所、AppleのApp Storeの濫用に対する判決を支持",
      "link": "https://www.webpronews.com/dutch-court-upholds-ruling-against-apples-app-store-abuse/",
      "snippet": "Apple Inc.にとって重要な打撃となる中、オランダの裁判所は、オランダの消費者市場庁による2021年の判決を支持しました...",
      "date": "13時間前",
      "source": "WebProNews",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSVZUsNtxTvIJYDjEGJN53uHWvK5NAGJrf-GS4wnzlSUtAbIYizew6HrYzr_w&s",
      "position": 9
    },
    {
      "title": "オランダの裁判所、Appleがデーティングアプリで優越的地位を濫用したことを確認",
      "link": "https://seekingalpha.com/news/4458712-dutch-court-confirms-apple-abused-dominant-position-for-dating-apps",
      "snippet": "オランダの裁判所は、Apple (AAPL)が不合理な条件を課すことによって優越的地位を濫用したという国の競争機関の判決を確認しました。",
      "date": "2時間前",
      "source": "Seeking Alpha",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSkf9LcqSBxw2DQZaDhfzZyJ0G2VRkkJ6dVHgBJFIGv_fgtPTDo2iAfQ_5fcQ&s",
      "position": 10
    }
  ]
}
```
私たちは検索結果の言語をカスタマイズすることもできます。ここでは、追加の `language` フィールドを追加し、その内容を `zh-cn` とします。これは簡体字中国語を指し、他の言語もサポートしていますが、入力する言語の略称を入力する必要があります。例えば、en（英語）、fr（フランス語）、zh-cn（中国語（簡体字））などの言語です。以下の図のように：

<p><img src="https://cdn.acedata.cloud/yyrssp.png" width="500" className="m-auto" /></p>

対応するコードは以下の通りです：

```shell
curl -X POST 'https://api.acedata.cloud/serp/google' \
-H 'accept: application/json' \
-H 'authorization: Bearer {token}' \
-H 'content-type: application/json' \
-d '{
  "type": "news",
  "query": "apple inc",
  "language": "zh-cn"
}'
```

実行結果は以下の通りです：

```json
{
  "knowledge_graph": {
    "title": "苹果",
    "type": "公司",
    "website": "http://www.apple.com/",
    "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRgbY1KzXEpuPeTpcw0GwN6BpQTcg1m06TDUsfdG6P-zW3eWrmu78AXyg&s=0",
    "attributes": {
      "创始人": "史蒂夫·乔布斯、史蒂夫·沃兹尼亚克和罗纳德·韦恩",
      "创立于": "1976 年 4 月 1 日，加利福尼亚洛思阿图斯",
      "总部": "加利福尼亚库比蒂诺"
    }
  },
  "organic": [
    {
      "title": "Apple",
      "link": "https://www.apple.com/",
      "snippet": "Discover the innovative world of Apple and shop everything iPhone, iPad, Apple Watch, Mac, and Apple TV, plus explore accessories, entertainment, ...",
      "sitelinks": [
        {
          "title": "Support",
          "link": "https://support.apple.com/"
        },
        {
          "title": "Careers at Apple",
          "link": "https://www.apple.com/careers/us/"
        },
        {
          "title": "Store",
          "link": "https://www.apple.com/store"
        },
        {
          "title": "Business",
          "link": "https://www.apple.com/business/"
        },
        {
          "title": "Apple UK",
          "link": "https://www.apple.com/uk/"
        }
      ],
      "position": 1
    },
    {
      "title": "Apple Inc. - Wikipedia",
      "link": "https://en.wikipedia.org/wiki/Apple_Inc.",
      "snippet": "Apple Inc. is an American multinational corporation and technology company headquartered in Cupertino, California, in Silicon Valley.",
      "sitelinks": [
        {
          "title": "History",
          "link": "https://en.wikipedia.org/wiki/History_of_Apple_Inc."
        },
        {
          "title": "Litigation involving Apple Inc.",
          "link": "https://en.wikipedia.org/wiki/Litigation_involving_Apple_Inc."
        },
        {
          "title": "Apple Park",
          "link": "https://en.wikipedia.org/wiki/Apple_Park"
        },
        {
          "title": "Apple Watch",
          "link": "https://en.wikipedia.org/wiki/Apple_Watch"
        }
      ],
      "position": 2
    },
    {
      "title": "Apple Inc. | History, Products, Headquarters, & Facts - Britannica",
      "link": "https://www.britannica.com/money/Apple-Inc",
      "snippet": "Apple Inc. is an American multinational technology company that revolutionized the technology sector through its innovation of computer software, ...",
      "position": 3
    },
    {
      "title": "Apple Inc - Company Profile and News - Bloomberg Markets",
      "link": "https://www.bloomberg.com/profile/company/AAPL:US",
      "snippet": "Apple Inc. designs, manufactures, and markets smartphones, personal computers, tablets, wearables and accessories, and sells a variety of related ...",
      "position": 4
    },
    {
      "title": "Apple Inc. (AAPL) Stock Price, News, Quote & History - Yahoo Finance",
      "link": "https://finance.yahoo.com/quote/AAPL/",
      "snippet": "Apple Inc. designs, manufactures, and markets smartphones, personal computers, tablets, wearables, and accessories worldwide.",
      "date": "2024年8月27日",
      "position": 5
    },
    {
      "title": "AAPL: Apple Inc Stock Price Quote - NASDAQ GS - Bloomberg",
      "link": "https://www.bloomberg.com/quote/AAPL:US",
      "snippet": "Apple Inc. designs, manufactures, and markets smartphones, personal computers, tablets, wearables and accessories, and sells a variety of related accessories.",
      "position": 6
    },
    {
      "title": "Apple Inc. (AAPL) Company Profile & Facts - Yahoo Finance",
      "link": "https://finance.yahoo.com/quote/AAPL/profile/",
      "snippet": "See the company profile for Apple Inc. (AAPL) including business summary, industry/sector information, number of employees, business summary, ...",
      "position": 7
    }
  ],
  "people_also_ask": [
    {
      "question": "What is Apple Inc stand for?",
      "snippet": "It was incorporated as Apple Computer, Inc. in January 1977, and sales of its computers, including the Apple II , saw significant momentum and revenue growth for the company. \"Inc.\" is the abbreviation for incorporated. A corporation is a separate legal entity from the person or people forming it.",
      "title": "What does the 'Inc.' in Apple Inc. mean? - Quora",
      "link": "https://www.quora.com/What-does-the-Inc-in-Apple-Inc-mean"
    },
    {
      "question": "Who owns Apple Inc.?",
      "snippet": "Apple (AAPL) Ownership Overview The ownership structure of Apple (AAPL) stock is a mix of institutional, retail and individual investors. Approximately 48.76% of the company's stock is owned by Institutional Investors, 0.11% is owned by Insiders and 51.13% is owned by Public Companies and Individual Investors.",
      "title": "Who owns Apple? AAPL Stock Ownership - TipRanks.com",
      "link": "https://www.tipranks.com/stocks/aapl/ownership"
    },
    {
      "question": "Why did Apple change to Apple Inc?",
      "snippet": "During his keynote speech at the Macworld Expo on January 9, 2007, Jobs announced the renaming of Apple Computer, Inc. to Apple Inc., because the company had broadened its focus from computers to consumer electronics. This event also saw the announcement of the iPhone and the Apple TV.",
      "title": "Apple Inc. - Wikipedia",
      "link": "https://en.wikipedia.org/wiki/Apple_Inc."
    },
    {
      "question": "Why Apple Inc is called Apple?",
      "snippet": "With the name Apple, the new computer company would appear ahead of Atari, where Jobs used to work. Jobs confirmed this theory in an 1980 presentation, stating that the name was partly chosen because he liked apples and partly because Apple was ahead of Atari in the phone book.",
      "title": "Why Is Apple Called Apple? - Apple Scoop",
      "link": "https://applescoop.org/story/why-is-apple-called-apple"
    }
  ],
  "related_searches": [
    {
      "query": "apple inc是什么"
    },
    {
      "query": "apple澳门"
    },
    {
      "query": "Apple ID"
    },
    {
      "query": "apple美国"
    },
    {
      "query": "apple官网"
    },
    {
      "query": "apple美国官网"
    },
    {
      "query": "Apple company introduction"
    },
    {
      "query": "apple id官网"
    }
  ],
  "credits": 1
}
```

ここに表示される結果はすべて簡体字中国語であり、結果の内容は上記と類似しています。

## カスタマイズされた検索結果の時間範囲
本文は、検索結果の時間範囲をカスタマイズすることもでき、5つのオプションがあります。それぞれ、`qdr:h`（過去1時間）、`qdr:d`（過去1日）、`qdr:w`（過去1週間）、`qdr:m`（過去1ヶ月）、デフォルトは無制限です。`range`を通じて対応する時間範囲を渡すことができます。例えば、ここでは`qdr:d`に設定されており、これは過去1日の結果を検索することを示しています。したがって、入力は以下のようになります：

<p><img src="https://cdn.acedata.cloud/qccfib.png" width="500" className="m-auto" /></p>

対応するコードは以下の通りです：

```shell
curl -X POST 'https://api.acedata.cloud/serp/google' \
-H 'accept: application/json' \
-H 'authorization: Bearer {token}' \
-H 'content-type: application/json' \
-d '{
  "range": "qdr:d",
  "query": "apple inc"
}'
```

実行結果は以下の通りです：

```json
{
  "answer_box": {
    "title": "Tim Cook (Aug 24, 2011–)Apple / CEO",
    "answer": "Tim Cook (Aug 24, 2011–)"
  },
  "knowledge_graph": {
    "title": "Apple",
    "type": "Technology company",
    "website": "http://www.apple.com/",
    "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT5ITHsQzdzkkFWKinRe1Y4FUbC_Vy3R_M&s=0",
    "description": "Apple Inc.は、カリフォルニア州クパチーノに本社を置くアメリカの多国籍企業およびテクノロジー企業で、シリコンバレーにあります。消費者向け電子機器、ソフトウェア、サービスで最もよく知られています。",
    "description_source": "Wikipedia",
    "description_link": "https://en.wikipedia.org/wiki/Apple_Inc.",
    "attributes": {
      "Customer service": "1 (800) 275-2273",
      "Founders": "スティーブ・ジョブズ、スティーブ・ウォズニアック、ロナルド・ウェイン",
      "Founded": "1976年4月1日、ロスアルトス、CA",
      "Headquarters": "クパチーノ、CA",
      "CEO": "ティム・クック (2011年8月24日–)"
    }
  },
  "organic": [
    {
      "title": "Apple",
      "link": "https://www.apple.com/",
      "snippet": "Appleの革新的な世界を発見し、iPhone、iPad、Apple Watch、Mac、Apple TVのすべてを購入し、アクセサリー、エンターテイメントなどを探索してください。",
      "sitelinks": [
        {
          "title": "サポート",
          "link": "https://support.apple.com/"
        },
        {
          "title": "キャリアの機会",
          "link": "https://www.apple.com/careers/us/"
        },
        {
          "title": "ビジネス",
          "link": "https://www.apple.com/business/"
        },
        {
          "title": "Appleのリーダーシップ",
          "link": "https://www.apple.com/leadership/"
        },
        {
          "title": "ストア",
          "link": "https://www.apple.com/store"
        }
      ],
      "position": 1
    },
    {
      "title": "Apple Inc. - Wikipedia",
      "link": "https://en.wikipedia.org/wiki/Apple_Inc.",
      "snippet": "Apple Inc.は、カリフォルニア州クパチーノに本社を置くアメリカの多国籍企業およびテクノロジー企業です。",
      "position": 2
    },
    {
      "title": "Apple Inc. | 歴史、製品、本社、事実 - ブリタニカ",
      "link": "https://www.britannica.com/money/Apple-Inc",
      "snippet": "アメリカのパーソナルコンピュータ、スマートフォン、タブレットコンピュータの製造業者。Appleは最初の成功したパーソナルコンピュータ会社であり...",
      "date": "7日前",
      "position": 3
    },
    {
      "title": "Apple Inc. (AAPL) 会社概要と事実 - Yahoo Finance",
      "link": "https://finance.yahoo.com/quote/AAPL/profile/",
      "snippet": "Apple Inc.は、スマートフォン、パーソナルコンピュータ、タブレット、ウェアラブル、アクセサリーを世界中で設計、製造、販売しています。会社はiPhoneを提供しています...",
      "position": 4
    },
    {
      "title": "AAPL: Apple Inc 株価引用 - NASDAQ GS - Bloomberg",
      "link": "https://www.bloomberg.com/quote/AAPL:US",
      "snippet": "Apple Inc.は、スマートフォン、パーソナルコンピュータ、タブレット、ウェアラブル、アクセサリーを設計、製造、販売し、さまざまな関連アクセサリーを販売しています。",
      "position": 5
    },
    {
      "title": "Apple Inc. (AAPL) 今日の株価 - WSJ",
      "link": "https://www.wsj.com/market-data/quotes/AAPL?gaa_at=eafs&gaa_n=ASWzDAjHf1Jvm_DfBkgGitLxGwlDC2VFfkyeU3ora5VpP1k1gJJfptWjtU4o&gaa_ts=6851a5a8&gaa_sig=FmQcgCrdYtgMPhMwfFTpCtEva8_OZ5j1fx1cEBn0_THBq1EUa8A0Tmo3piNM3OQ488O2rScdH4NuSLly_DIdpQ%3D%3D",
      "snippet": "Apple Inc.は、スマートフォン、パーソナルコンピュータ、タブレット、ウェアラブル、アクセサリーの設計、製造、販売に従事しており、その他の関連製品も販売しています。",
      "position": 6
    }
  ],
  "people_also_ask": [
    {
      "question": "Apple Incとは何ですか？",
      "snippet": "Apple Inc.、元々はApple Computer, Inc.は、消費者向け電子機器とそれに付随するコンピュータソフトウェアを作成・販売する多国籍企業であり、メディアコンテンツのデジタル配信者です。Appleの主力製品ラインは、iPhoneスマートフォン、iPadタブレットコンピュータ、Macパーソナルコンピュータです。",
      "title": "Apple Incの歴史 - Wikipedia",
      "link": "https://en.wikipedia.org/wiki/History_of_Apple_Inc."
    },
    {
      "question": "Apple Incの所有者は誰ですか？",
      "snippet": "Apple (AAPL) 所有権概要 約50.57%の株式は機関投資家が所有し、0.06%は内部者が所有し、49.38%は公開企業および個人投資家が所有しています。Apple (AAPL) 株の所有構造は、機関、リテール、個人投資家の混合です。",
      "title": "Appleの所有者は誰ですか？ AAPL株の所有権 - TipRanks.com",
      "link": "https://www.tipranks.com/stocks/aapl/ownership"
    },
    {
      "question": "Apple Incは何で最もよく知られていますか？",
      "snippet": "消費者向け電子機器、ソフトウェア、サービスで最もよく知られています。1976年にスティーブ・ジョブズ、スティーブ・ウォズニアック、ロナルド・ウェインによってApple Computer Companyとして設立され、翌年にジョブズとウォズニアックによってApple Computer, Inc.として法人化されました。",
      "title": "Apple Inc. - Wikipedia",
      "link": "https://en.wikipedia.org/wiki/Apple_Inc."
    },
    {
      "question": "現在のAppleの所有者は誰ですか？",
      "snippet": "ティム・クック (2011年8月24日–)\nApple / CEO"
    }
  ],
  "related_searches": [
    {
      "query": "Apple iPhone 13"
    },
    {
      "query": "apple inc とは何か"
    },
    {
      "query": "Apple Incの完全な形式"
    },
    {
      "query": "Apple Incの住所"
    },
    {
      "query": "Apple Incの投資家向け情報"
    },
    {
      "query": "Apple Incの業界"
    },
    {
      "query": "Apple Incの年次報告書"
    },
    {
      "query": "Apple Incのキャリア"
    }
  ]
}
```

過去1日の検索結果を成功裏に取得できたことがわかります。結果の内容は上記のように似ています。

## 検索結果のページネーションをカスタマイズ

このAPIは、検索結果のページネーション表示もサポートしており、`number`と`page`はページのサイズとページ数を示します。この記事では、1ページに20件の検索結果を表示する形式でページネーションを設定します。具体的な情報は図の通りです：

<p><img src="https://cdn.acedata.cloud/dqla1e.png" width="500" className="m-auto" /></p>

> 注意：1ページの結果が10件を超える場合、引かれるポイントは倍になります。

コードは以下の通りです：
```shell
curl -X POST 'https://api.acedata.cloud/serp/google' \
-H 'accept: application/json' \
-H 'authorization: Bearer {token}' \
-H 'content-type: application/json' \
-d '{
  "query": "apple inc",
  "number": 20,
  "page": 1
}'
```

実行結果は以下の通りです：

```json {
  "knowledge_graph": {
    "title": "Apple",
    "type": "テクノロジー企業",
    "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSHmAG45NsvdRfYlNghHFvinM0w2mqjd8A0lXKU2LA&usqp=CAE&s",
    "attributes": {
      "カスタマーサービス": "1 (800) 275-2273",
      "創業者": "スティーブ・ジョブズ、スティーブ・ウォズニアック、ロナルド・ウェイン",
      "設立": "1976年4月1日、ロスアルトス、CA",
      "株価": "AAPL (NASDAQ) $198.42 +1.97 (+1.00%) 6月16日、午後4:00 EDT - 免責事項",
      "本社": "クパチーノ、CA",
      "CEO": "ティム・クック (2011年8月24日–)"
    }
  },
  "organic": [
    {
      "title": "Apple",
      "link": "https://www.apple.com/",
      "snippet": "Appleの革新的な世界を発見し、iPhone、iPad、Apple Watch、Mac、Apple TVのすべてを購入し、アクセサリー、エンターテイメントなどを探索してください。",
      "sitelinks": [
        {
          "title": "サポート",
          "link": "https://support.apple.com/",
          "snippet": "iPhoneサポート - お問い合わせ - 請求とサブスクリプション - Apple修理"
        }, {
          "title": "キャリアの機会",
          "link": "https://www.apple.com/careers/us/",
          "snippet": "Appleで働く - ソフトウェアとサービス - サポートとサービス"
        }, {
          "title": "ビジネス",
          "link": "https://www.apple.com/business/",
          "snippet": "小規模ビジネス - 企業 - ビジネス向けの購入 - Apple at Work - ..."
        }, {
          "title": "ストア",
          "link": "https://www.apple.com/store",
          "snippet": "iPhoneを購入 - Macを購入 - iPadを購入 - App Store - ..."
        },
        {
          "title": "投資家向け情報",
          "link": "https://investor.apple.com/investor-relations/default.aspx",
          "snippet": "SEC提出書類 - 株価 - リーダーシップとガバナンス"
        }
      ],
      "position": 1
    }, {
      "title": "Apple Inc.",
      "link": "https://en.wikipedia.org/wiki/Apple_Inc.",
      "snippet": "Apple Inc.は、カリフォルニア州クパチーノに本社を置くアメリカの多国籍企業であり、テクノロジー企業です。",
      "position": 2
    }, {
      "title": "Apple Inc. | 歴史、製品、本社、事実",
      "link": "https://www.britannica.com/money/Apple-Inc",
      "snippet": "Apple Inc.は、コンピュータソフトウェア、個人用コンピュータ、個人用デバイスの革新を通じてテクノロジーセクターを革命的に変えたアメリカの多国籍テクノロジー企業です。",
      "date": "7日前",
      "position": 3
    }, {
      "title": "Apple Inc. (AAPL) 会社概要と事実",
      "link": "https://finance.yahoo.com/quote/AAPL/profile/",
      "snippet": "Apple Inc.は、スマートフォン、個人用コンピュータ、タブレット、ウェアラブルデバイス、アクセサリーを世界中で設計、製造、販売しています。",
      "position": 4
    }, {
      "title": "AAPL: Apple Inc 株価引用 - NASDAQ GS",
      "link": "https://www.bloomberg.com/quote/AAPL:US",
      "snippet": "Apple Inc.は、スマートフォン、個人用コンピュータ、タブレット、ウェアラブルデバイス、アクセサリーを設計、製造、販売し、さまざまな関連アクセサリーを販売しています。",
      "position": 5
    }, {
      "title": "Apple Inc. (AAPL) 今日の株価",
      "link": "https://www.wsj.com/market-data/quotes/AAPL?gaa_at=eafs&gaa_n=ASWzDAgqDxGuyZUvNqcmVhmXBoGHd3y-StLOZpo82befc3s9jFqSwwJzjPmn&gaa_ts=68514b6e&gaa_sig=a-sNk_J7mCIayZx21oIaY6Ge6vifnmxy7A4psXod_wP6ng9689uAwZYvUOUQPv_dxcArYe_RQbKxCkwngcK_bQ%3D%3D",
      "snippet": "Apple Inc.は、スマートフォン、個人用コンピュータ、タブレット、ウェアラブルデバイス、アクセサリー、その他の関連製品の設計、製造、販売に従事しています。",
      "position": 6
    }, {
      "title": "Apple",
      "link": "https://www.linkedin.com/company/apple",
      "snippet": "Appleの外部リンク。業界：コンピュータおよび電子機器製造。会社の規模：10,001人以上の従業員。本社：カリフォルニア州クパチーノ。タイプ：公開企業 ...",
      "position": 7
    }, {
      "title": "Apple | AAPL株価、会社概要とニュース",
      "link": "https://www.forbes.com/companies/apple/",
      "snippet": "Apple Inc.は、スマートフォン、個人用コンピュータ、タブレット、ウェアラブルデバイス、アクセサリー、その他の関連製品の設計、製造、販売に従事しています。",
      "position": 8
    }, {
      "title": "Apple Inc. (AAPL) 株価、ニュース、引用、歴史",
      "link": "https://finance.yahoo.com/quote/AAPL/",
      "snippet": "最新のApple Inc. (AAPL) 株価、歴史、ニュース、その他の重要な情報を見つけて、株取引や投資に役立ててください。",
      "position": 9
    }, {
      "title": "iCloud",
      "link": "https://www.icloud.com/",
      "snippet": "iCloud.comでは、任意のウェブブラウザから写真、ファイルなどにアクセスできます。行った変更は、iPhoneや他のデバイスに同期されます。",
      "position": 10
    }, {
      "title": "Apple Inc (AAPL) 株価とニュース - Google Finance",
      "link": "https://www.google.com/finance/quote/AAPL:NASDAQ?hl=en",
      "snippet": "Apple Inc.は、カリフォルニア州クパチーノに本社を置くアメリカの多国籍企業であり、テクノロジー企業です。",
      "position": 11
    }, {
      "title": "Apple Inc 会社概要 - GlobalData",
      "link": "https://www.globaldata.com/company-profile/apple-inc/",
      "snippet": "Apple Inc (Apple)は、スマートフォン、タブレット、個人用コンピュータ、ウェアラブルデバイスを設計、製造、販売しています。会社はソフトウェアアプリケーションを提供しています ...",
      "position": 12
    }, {
      "title": "Apple Computer, Inc.の設立 - 今月のビジネス歴史",
      "link": "https://guides.loc.gov/this-month-in-business-history/april/apple-computer-founded",
      "snippet": "Apple Computer, Inc.は、1976年4月1日に大学を中退したスティーブ・ジョブズとスティーブ・ウォズニアックによって設立され、新しい会社に変革のビジョンをもたらしました。",
      "position": 13
    }, {
      "title": "Apple Inc",
      "link": "https://vimeo.com/appleinc",
      "snippet": "Apple Incは、質の高いビデオとそれを愛する人々のためのホームであるVimeoのメンバーです。",
      "position": 14
    }
  ],
  "people_also_ask": [
    {
      "question": "Apple Incとは何ですか？"
    }, {
      "question": "Apple Incの所有者は誰ですか？"
    }, {
      "question": "Apple Incは何で最も知られていますか？",
      "title": "Apple Inc. - Wikipedia",
      "snippet": "消費者向け電子機器、ソフトウェア、サービスで最も知られています。1976年にスティーブ・ジョブズ、スティーブ・ウォズニアック、ロナルド・ウェインによってApple Computer Companyとして設立され、翌年にジョブズとウォズニアックによってApple Computer, Inc.として法人化されました。",
      "link": "https://en.wikipedia.org/wiki/Apple_Inc.#:~:text=It%20is%20best%20known%20for,%2C%20Inc.%20the%20following%20year."
    }, 
```
{
      "question": "Apple IncのIncの意味は何ですか？",
      "title": "Inc. | 英語の意味 - ケンブリッジ辞典",
      "snippet": "incorporatedの略語：法的に設立された米国企業の名前に使用される：Bishop Computer Services, Inc.",
      "link": "https://dictionary.cambridge.org/dictionary/english/inc#:~:text=abbreviation%20for%20incorporated%3A%20used%20in,Bishop%20Computer%20Services%2C%20Inc."
    } ],
  "related_searches": [
    {
      "query": "apple inc とは何か"
    }, {
      "query": "Apple Incの完全な形"
    }, {
      "query": "Apple Incの住所"
    }, {
      "query": "Apple Incの投資家向け情報"
    }, {
      "query": "Apple Incの業界"
    },
    {
      "query": "Apple Incの年次報告書"
    },
    {
      "query": "Apple Incのキャリア"
    },
    {
      "query": "Apple 17 pro"
    }
  ]
}
```

ここでは、検索結果がページ分けされて表示されており、一ページに20件の結果が表示され、その内容は上記と似ています。
