...
通常のアップロードでは、`audio_url` のみが必要で、これは公開アクセス可能な音声 CDN アドレスです。`mode` はデフォルトで `standard` であるため、既存の呼び出しを変更する必要はありません。通常のアップロードが成功すると **0.06 Credits** が消費され、最大パッケージ換算で約 **¥0.041/回** です；アップロードが失敗した場合は料金は発生しません。
...
`data` の `audio_id` フィールドがアップロード後の曲の ID です。
...
通常のアップロードで処理できない音声をお持ちの場合や使用許可を得ている場合は、`mode` を `enhanced` に設定できます。このモードでは、1 から 100 文字の `name` を提供する必要があり、公開アクセス可能な HTTPS `audio_url` を使用する必要があります。

強化アップロードは固定で非同期処理を行い、通常は 2 分以上かかります。成功した場合は **1.87 Credits** が消費され、最大パッケージ換算で約 **¥1.26/回** です；処理が失敗した場合は料金は発生しません。
...
強化アップロードで得られた `audio_id` は、Cover、Samples、Mashup に使用できます。伴奏と MIDI 抽出、全トラック分離、人声/伴奏分離は、音声の所属アカウントの制限を受ける可能性があります；未記載の操作はアカウント間での使用が保証されません。音声が短いほど、処理成功率は通常高くなります。
