...
通常のアップロードでは、`audio_url` のみが必要で、これは公開アクセス可能な音声 CDN アドレスです。`mode` はデフォルトで `standard` であるため、既存の呼び出しを変更する必要はありません。通常のアップロードが成功すると **0.06 クレジット** が消費され、アップロードが失敗した場合は料金は発生しません。
...
> `audio_url` は入力アドレスで、音声処理プロセスに直接渡され、最終メディア永続化サービスによって事前に転送されることはありません。タスク処理中に公開アクセス可能であることを確認してください。

通常のアップロードで処理できない音声をお持ちまたは使用許可を得ている場合、`mode` を `enhanced` に設定できます。このモードでは、1 から 100 文字の `name` を提供する必要があり、公開アクセス可能な HTTPS `audio_url` を使用する必要があります。

強化アップロードは固定で非同期処理を採用し、通常は 2 分以上の時間がかかります。成功した処理は **1.87 クレジット** を消費し、処理が失敗した場合は料金は発生しません。
...
強化アップロードで得られた `audio_id` は、カバー、サンプル、マッシュアップに使用できます。伴奏と MIDI 抽出、全トラック分離、およびボーカル/伴奏分離は、音声の所属アカウントの制限を受ける可能性があります。リストにない操作は、アカウント間での使用が保証されません。音声が短いほど、処理成功率は通常高くなります。
