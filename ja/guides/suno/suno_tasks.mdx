...
返却される結果には複数のフィールドがあり、itemsはバッチソングタスクの具体的な詳細情報を含んでいます。各ソングタスクの具体的な情報は上記のフィールドと同じです。

- `items`、バッチソングタスクのすべての具体的な詳細情報。これは配列であり、各配列の要素は上記の単一タスクの返却結果形式と同じです。
- `count`、ここでのバッチクエリソングタスクの数。
...
この文書を通じて、Suno Tasks APIを使用して単一またはバッチのソングタスクのすべての具体的な詳細情報を照会する方法を理解しました。この文書がAPIの接続と使用に役立つことを願っています。ご不明な点がございましたら、いつでも技術サポートチームにお問い合わせください。
