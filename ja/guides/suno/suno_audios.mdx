...
- `duration`：期待される曲の長さ、単位は秒、整数でなければならず、通常は10から360の間を推奨。このパラメータは主にカスタムモード（`custom` が `true`）の曲生成に使用され、一部のモデルや操作はサポートしていない場合があり、その場合はこの値が無視されるかエラーが返される。これは目標の長さであり、実際の完成品の長さは応答の `duration` フィールドに基づくため、わずかな偏差がある可能性がある。
...
他のパラメータはそのままで、返されるのは完全な曲であり、すべての曲のセグメントの結合結果ですが、結果は1曲のみで、以下のような例になります：
...
例えば、元の生成後の曲のIDが：ec13e502-d043-4eb2-92ee-e900c6da69d1の場合、パラメータを次のように設定できます：
...
以上のパラメータを使用することで、声曲分離の結果を得ることができます。結果は以下の通りです：
```
```json
{
  "success": true,
...
曲を生成した後、全トラック声曲分離操作を行う必要がある場合は、以下の内容を指定できます：
...
デフォルトでは、生成された曲の長さはモデルが自動的に決定し、通常は30秒から4分の間です。より長いまたは短い作品が必要な場合は、`duration`パラメータを使用して希望の長さを指定できます。単位は秒で、値は整数で、通常は10から360の間を推奨します。

このパラメータは主にカスタムモード（`custom`が`true`）での曲生成に向けられており、最新のモデル（例えば`chirp-v5-5`）と組み合わせると最も効果的です。異なるモデルや操作によって長さの制御のサポート程度は異なり、現在の組み合わせがサポートしていない場合、このパラメータは無視されるか、エラーメッセージが返されることがあります。返された結果に基づいて調整してください。
...
注意が必要なのは、リクエスト内の`duration`は**希望の長さ**であり、レスポンスの`data`内の各曲の`duration`フィールドはその曲の**実際の長さ**です。両者の名称は同じですが意味は異なります：`duration`は傾向的なヒントであり、厳密な制約ではありません。モデルはそれを参考にしますが、達成を保証するものではありません。実際の長さは通常、希望値よりも短く、同じリクエストで返される2曲の長さも大きく異なる可能性があります。歌詞の長さ、スタイル、モデルが最終的な作品に影響を与えます。より長い作品が必要な場合は、より完全な歌詞を同時に提供することをお勧めします。

特定のモデルや操作が長さの制御をサポートしていない場合、インターフェースはこのパラメータを無視するか、エラーメッセージを返すことがあります。まず1回のリクエストで効果を確認してから、一括使用することをお勧めします。

## Add Insterumental 機能

2025年8月にsunoはAdd Insterumental機能を新たに発表しました。まず、無伴奏の清唱曲をアップロードして、sunoに伴奏をつけてもらう必要があります。まず、[Suno Upload API](https://platform.acedata.cloud/documents/suno-upload)にアクセスして、無伴奏の清唱曲をアップロードします。対応する操作は以下のようになります：
...
- underpainting_end：アップロードした曲に伴奏を追加する終了時間、曲の総長より小さくなければなりません。
- audio_id：アップロードした無伴奏の清唱曲のID。
- style：伴奏のスタイル、歌詞は不要な方が良いです。
...
- overpainting_end：アップロードした曲にボーカルを追加する終了時間、曲の総時間よりも小さくなければなりません。
...
Sunoの新しいInspoインスピレーション創作機能は、1から4段の参考音声をインスピレーションの源として、全く新しい音楽を生成することができます。カバー（cover）とは異なり、Inspoは原曲を再現するのではなく、参考音声からスタイルのインスピレーションを抽出し、プロンプトやスタイルタグなどと組み合わせて新しい作品を生成します。使用する際には、以下のパラメータを記入する必要があります：
