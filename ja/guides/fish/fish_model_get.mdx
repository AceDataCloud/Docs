...
> 音色作成は [Fish Model Create API](https://platform.acedata.cloud/documents/fish-model) を、ページネーション付きリスト検索は [Fish Model Query API](https://platform.acedata.cloud/documents/fish-model-query) を参照してください。

## 申請手順

APIを利用するには、まず [Fish Model API](https://platform.acedata.cloud/services/fish) の該当ページでサービスを申請します。ページに入ったら「Acquire」ボタンをクリックしてください。
...
- **パスパラメータ**：`{id}` は音色の `_id` で、[Fish Model Create API](https://platform.acedata.cloud/documents/fish-model) の作成時に返されるか、[Fish Model Query API](https://platform.acedata.cloud/documents/fish-model-query) のページネーション検索で取得可能です。
...
返却される `_id` は、後続の [Fish TTS API](https://platform.acedata.cloud/documents/fish-tts) の `reference_id` フィールドの値として使用でき、このクローン音色を用いた音声合成に利用されます。
...
Fish Model Get API は Fish Audio 公式と完全互換の単一音色詳細取得機能を提供します。音色IDを取得後、本APIで ModelEntity オブジェクト（サンプル、状態、可視性、統計情報含む）を取得し、[Fish TTS API](https://platform.acedata.cloud/documents/fish-tts) と組み合わせてクローン＋合成のエンドツーエンド処理を実現できます。
