...
Fish Audios Generation API サービスから返されたタスクIDを例に、どのようにこの API を使用するかを示します。タスクID：2725a2d3-f87e-4905-9c53-9988d5a7b2f5 があると仮定し、次にタスクIDを渡す方法を示します。
...
- `response`：タスク内の返却情報を照会します；最終的に成功した場合、その中の `cost.amount` は今回の合成で実際に差し引かれたクレジットです。
...
- `elapsed`：タスク実行にかかった時間、単位は秒（浮動小数点、3 桁の小数点以下を保持）。タスクが未完了の場合、このフィールドは返されません。
...
この文書を通じて、Fish Tasks APIを使用して単一またはバッチタスクのすべての具体的な詳細情報を取得する方法を理解しました。この文書がAPIの接続と使用に役立つことを願っています。ご不明な点がございましたら、いつでも技術サポートチームにお問い合わせください。
