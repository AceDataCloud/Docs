...
この記事では、Gemini Tasks API の接続説明を詳しく紹介し、Gemini Videos Generation API のタスク実行状況を簡単に統合して照会できるようにします。
...
リクエストが成功すると、API はこのタスクの詳細情報を返します。ここでの `request` フィールドはタスクを発起したときのリクエストボディで、`response` フィールドはタスク完了後に返されるレスポンスボディです。例えば：
