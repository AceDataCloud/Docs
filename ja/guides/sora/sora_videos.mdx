...
API を利用するには、まず [Sora Videos Generation API](https://platform.acedata.cloud/documents/99a24421-2e22-4028-8201-e19cb834b67e) の該当ページでサービスを申請してください。ページにアクセスしたら、「Acquire」ボタンをクリックします。以下のように表示されます：

![](https://cdn.acedata.cloud/q6ytrc.png)

ログインまたは登録していない場合は、自動的にログインページにリダイレクトされ、登録・ログイン後に元のページに戻ります。

初回申請時には無料利用枠が付与され、この API を無料で利用可能です。

## 基本的な使い方（Version 1）

まず Version 1 の基本的な使い方を理解しましょう。`prompt`（プロンプト）、参照画像リンクの配列 `image_urls`、およびモデル `model` を入力すると、処理結果が得られます。具体的には以下の通りです：
...
- `model`：動画生成モデル。`sora-2`（標準モード）と `sora-2-pro`（高画質モード）をサポート。`sora-2-pro` は 25秒の動画に対応し、`sora-2` は 10秒または15秒のみ対応（Version 1のみ）。
- `size`：動画の解像度。`small` は標準解像度、`large` は HD 解像度（Version 1のみ）。
- `duration`：動画の長さ。10秒、15秒、25秒をサポート。25秒は `sora-2-pro` のみ対応（Version 1のみ）。
- `orientation`：画面の向き。`landscape`（横向き）、`portrait`（縦向き）（Version 1のみ）。
- `image_urls`：参照画像リンクの配列。画像から動画を生成（Version 1のみ）。
- `character_url`：キャラクター**###**リンク。動画内に実写人物が含まれてはいけません（Version 1のみ）。
- `character_start`/`character_end`：キャラクターの表示開始・終了秒数。範囲は1～3秒（Version 1のみ）。
- `prompt`：プロンプト（必須）。
- `callback_url`：非同期コールバック用の URL。
- `version`：API バージョン。`1`（デフォルト）または `2`。

選択すると、右側に対応するコードが生成されます。以下のように表示されます：
...
返却される結果には複数のフィールドがあります：

- `success`：動画生成タスクの状態。
- `task_id`：動画生成タスクのID。
- `trace_id`：動画生成のトレースID。
- `data`：動画生成タスクの結果リスト。
  - `id`：動画生成タスクの動画ID。
  - `video_url`：動画生成タスクの動画URL。
  - `state`：動画生成タスクの状態。

このように、`data` 内の動画リンクから生成された Sora 動画を取得できます。

また、対応する接続コードを生成してコピー可能です。例えば CURL のコードは以下の通りです：
...
画像から動画を生成したい場合、`image_urls` パラメータに参照画像のリンクを渡す必要があります。以下の内容を指定できます：

- `image_urls`：画像から動画生成タスクで使用する参照画像リンクの配列。実際の人物の顔写真などは送信しないでください。タスク失敗の原因となります。

入力例は以下の通りです：

<p><img src="https://cdn.acedata.cloud/ch7x3t.png" width="500" className="m-auto" /></p>

入力後、自動的に以下のコードが生成されます：
...
このように、画像から動画生成された結果が得られ、前述の例と同様です。

## キャラクター生成動画タスク（Version 1）

キャラクター生成動画を行う場合、`character_url` パラメータにキャラクター作成用の動画リンクを渡す必要があります。動画内に実写人物が含まれていると失敗します。以下の内容を指定できます：

- `character_url`：キャラクター作成用の動画リンク。動画内に実写人物が含まれていないことが必須。
...
このように、キャラクター生成動画の結果が得られ、前述の例と同様です。

## Version 2 モード

上記の Version 1 モードに加え、本 API は Version 2 モードもサポートしています。`version` パラメータを `2` に設定すると有効になります。Version 2 モードはより短い動画長さとピクセル単位の解像度制御をサポートします。

### Version 2 パラメータ説明

| パラメータ | 型 | 必須か | 説明 |
|------------|----|--------|------|
| `version` | integer | 必須 | `2` に設定 |
...
| `callback_url` | string | 任意 | 非同期コールバック用 URL |
...
Version 2 モードでは、`image_urls` パラメータに参照画像を渡して動画生成を誘導できます（最初の1枚のみ使用）：
...
> **注意**：参照画像のサイズは `size` パラメータと一致させてください。例えば `size` が `1280x720` の場合、参照画像のサイズも 1280×720 である必要があります。

### Version 1 と Version 2 のパラメータ比較

| パラメータ | Version 1 | Version 2 |
|------------|-----------|-----------|
| `version` | 1（デフォルト） | 2 |
| `prompt` | ✅ | ✅ |
...
| `image_urls` | ✅ 複数の参照画像 | ✅ 最初の1枚のみ |
| `character_url` | ✅ | ❌ |
| `callback_url` | ✅ | ✅ |

## 非同期コールバック

Sora Videos Generation API は生成に約1～2分かかるため、API が長時間応答しない場合、HTTP リクエストが接続を維持し続け、システムリソースを消費します。そのため、本 API は非同期コールバックもサポートしています。

全体の流れは、クライアントがリクエスト時に `callback_url` フィールドを指定し、API はすぐに `task_id` を含む結果を返します。タスク完了後、生成された動画結果が POST JSON 形式でクライアントの指定した `callback_url` に送信されます。`task_id` によりタスク結果を紐付けられます。

以下に具体的な操作例を示します。

まず、Webhook コールバックは HTTP リクエストを受け取れるサービスであり、開発者は自分で構築した HTTP サーバーの URL に置き換えてください。ここではデモ用に公開の Webhook サイト https://webhook.site/ を使用します。サイトを開くと Webhook URL が取得できます。以下のように表示されます：

![](https://cdn.acedata.cloud/cjjfly.png)

この URL をコピーして Webhook として使用します。例として `https://webhook.site/eb238c4f-da3b-47a5-a922-a93aa5405daa` を使用します。

次に、`callback_url` フィールドに上記 Webhook URL を設定し、その他のパラメータを入力します。内容は以下の通りです：

<p><img src="https://cdn.acedata.cloud/v1m05g.png" width="500" className="m-auto" /></p>

「実行」ボタンを押すとすぐに以下の結果が得られます：
...
しばらく待つと、`https://webhook.site/eb238c4f-da3b-47a5-a922-a93aa5405daa` で生成動画の結果を確認できます。以下のように表示されます：
...
結果には `task_id` フィールドが含まれ、他のフィールドは前述と同様です。この `task_id` によりタスクの紐付けが可能です。
...
本ドキュメントを通じて、Sora Videos Generation API の使い方、プロンプトや参照画像を入力して動画を生成する方法を理解いただけました。API の接続や利用に役立てていただければ幸いです。ご不明点があれば、いつでも技術サポートチームまでお問い合わせください。
