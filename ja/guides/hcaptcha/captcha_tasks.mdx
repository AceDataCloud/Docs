...
この記事では、CAPTCHA非同期タスククエリインターフェース `POST /captcha/tasks` について説明します。任意のCAPTCHAインターフェース（トークンシリーズまたは認識シリーズ）を呼び出す際に `async: true` を渡すと、インターフェースはすぐに `task_id` を返し、その後この `task_id` を使用してこのインターフェースをポーリングして最終結果を取得できます。これはマルチソルバー回転などのシナリオに適しています：タスクを提出した後、すぐに `task_id` を取得し、他のソルバーをスケジュールし、後で結果を取得することができます。
...
`status: ready` の応答には、タイミングフィールドも含まれます。
...
非同期モードでは、タスクの作成と「処理中」のポーリングは課金されません；**結果を正常に取得したときにのみ1回課金されます**（同期モードの価格と一致）。したがって、回転中に未完了のタスクをキャンセルしても費用は発生しません。
