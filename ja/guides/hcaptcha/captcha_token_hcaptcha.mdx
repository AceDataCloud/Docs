...
- `website_url`：処理する CAPTCHA のウェブサイトの URL。
...
返された結果には複数のフィールドが含まれており、以下のように説明されています：

- `token`：今回のhCaptchaの検証結果。
- `started_at`、`finished_at`：このリクエストの処理開始と結果生成の時間（ISO-8601 UTC）。
...
デフォルトではAPIは同期ブロッキングです：1回のリクエストはトークン処理が完了するまで待機します。複数のソルバーを回転させる場合（multi-solver rotation）、タスクを提出した後にすぐにtask_idを取得し、他のソルバーを調整し、後で結果を取得したい場合は、リクエストボディに `async: true` を渡すことができます。
...
料金説明：非同期モードでは、タスクの作成と「処理中」のポーリングは課金されません；**トークンを正常に取得したときのみ1回課金されます**（同期モードの価格と同じ）。したがって、回転中に未完了のタスクをキャンセルしても費用は発生しません。`/captcha/tasks` はすべてのCaptchaインターフェース（トークンおよび認識シリーズ、hcaptcha、recaptcha2、recaptcha3、recognition/* など）に共通で、同じ `task_id` でポーリングできます。
