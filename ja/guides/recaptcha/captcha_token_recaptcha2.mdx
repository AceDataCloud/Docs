...
- `proxy`：オプション、持ち込みプロキシ（Bring Your Own Proxy）。設定後、上流は提供されたプロキシ IP を使用して認証コードを解決します。出口 IP の品質を制御するために使用されます（例えば、公共プロキシ IP がターゲットサイトにブロックされて `410 Gone` が返されるのを避けるため）。形式は `scheme://[user:pass@]host:port` で、`scheme` は `http`/`https`/`socks4`/`socks5` をサポートします。例えば `http://user:pass@1.2.3.4:8080` のようになります。未記入の場合はプラットフォームのデフォルトプロキシが使用されます。
...
- `started_at`、`finished_at`：リクエストの処理開始と結果生成の時間（ISO-8601 UTC）。
...
まず、サイトがどのようにPOSTリクエストを送信するかを確認する必要があります。そうすれば、生成されたtokenを渡すことができます。最初にF12コンソールを開き、手動で確認します。最後に、サイトがPOSTリクエストを送信したことがわかります。このPOSTリクエストの構造を確認する必要があります。具体的な手順は以下の通りです：
...
料金説明：非同期モードでは、タスクの作成と「処理中」のポーリングは課金されません；**結果を正常に取得したときにのみ1回課金されます**（同期モードの価格と同じです）。したがって、回転中に未完了のタスクをキャンセルしても費用は発生しません。`/captcha/tasks` はすべてのCaptchaインターフェース（トークンと認識シリーズ）に共通で、同じ `task_id` でポーリングできます。
