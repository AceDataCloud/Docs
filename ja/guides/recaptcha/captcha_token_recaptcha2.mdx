...
この記事では、Recaptcha2 プロトコル認識 API 接続説明を紹介します。これにより、ユーザーは Recaptcha2 の CAPTCHA 画像を認識してクリックすることなく、Website Key を提出するだけでバックエンドで自動的にデコードし、検証を完了できます。
...
まず、基本的な使用方法を理解します。処理する必要のある CAPTCHA のウェブサイトの URL を入力するだけで、処理された結果を得ることができます。最初に `website_url` フィールドを簡単に渡す必要があります。私たちのサンプルサイトは：`https://www.google.com/recaptcha/api2/demo` です。`website_url` ページで `website_key` を取得する必要があります。まずこのウェブページを開き、F12 を押してコンソールに入ります。最後に Element ページで `recaptcha-demo` を全体検索すると、以下の結果が得られます：
...
- `website_url`：処理する必要のある CAPTCHA のウェブサイトの URL。
- `website_key`：Recaptcha2 におけるウェブサイトのキー識別子。
- `proxy`：オプション、持ち込みプロキシ（Bring Your Own Proxy）。設定後、上流は提供されたプロキシ IP を使用して CAPTCHA を解決します。出口 IP の品質を制御するために使用されます（例えば、公共プロキシ IP がターゲットサイトにブロックされて `410 Gone` が返されるのを避けるため）。形式は `scheme://[user:pass@]host:port` で、`scheme` は `http`/`https`/`socks4`/`socks5` をサポートします。例えば `http://user:pass@1.2.3.4:8080` のようになります。未記入の場合はプラットフォームのデフォルトプロキシが使用されます。
...
Recaptcha2の検証結果を取得できたことがわかります。これをPOSTまたはターゲットサイトに模擬的に送信するために使用できます。一度限りの使用で、有効期限は120秒、60秒以内の使用を推奨します。次に、処理されたtokenをターゲットサイトに送信するためのPythonコードを提供します。

まず、サイトがどのようにPOSTリクエストを送信しているかを確認する必要があります。そうすれば、生成されたtokenを渡すことができます。最初にF12コンソールを開き、手動で確認します。最後に、サイトがPOSTリクエストを送信したことがわかります。このPOSTリクエストの構造を確認する必要があります。具体的な手順は以下の通りです：
...
- 今回のPOSTリクエストの構造を分析し、最後にそのリクエストを右クリックしてCURLのコードをコピーできます。具体的には以下の図の通りです：
...
デフォルトでは、APIは同期的にブロックされます：1回のリクエストは、トークン処理が完了するまで待機します。もし、複数のソルバーを回転させる（multi-solver rotation）場合、「タスクを提出した後、すぐにtask_idを取得し、他のソルバーを調整し、後で結果を取得する」ことを希望する場合は、リクエストボディに `async: true` を渡すことができます。
...
料金説明：非同期モードでは、タスクの作成と「処理中」のポーリングは課金されません；**結果を成功裏に取得したときにのみ1回課金されます**（同期モードの価格と同じです）。したがって、回転中に未完了のタスクをキャンセルしても費用は発生しません。`/captcha/tasks` はすべてのキャプチャインターフェース（トークンおよび認識シリーズ）に共通で、同じ `task_id` でポーリングできます。
...
この文書を通じて、Recaptcha2プロトコルを使用してAPIを利用し、ユーザーがRecaptcha2の画像を認識したりクリックしたりすることなく、Website Keyを提出するだけでバックエンドで自動的にデコードし、検証を完了する方法を理解しました。この文書が、APIの接続と使用をより良くする手助けとなることを願っています。何か問題があれば、いつでも私たちの技術サポートチームにお問い合わせください。
