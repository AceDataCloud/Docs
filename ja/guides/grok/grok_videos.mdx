...
本 API は2つのモデルをサポートしています：

- `grok-imagine-video-1.5-fast`（デフォルト）：テキストから動画を生成（`prompt` のみを送信）および画像から動画を生成（`image_url` を送信）をサポートし、最大30秒の長さで、価格が低いです。
- `grok-imagine-video-1.5`：画像から動画を生成のみをサポートし、**必ず** `image_url` を送信する必要があります。最大15秒の長さです。
...
- `accept`：受け取りたいレスポンス結果の形式。ここでは `application/json`、すなわち JSON 形式を記入します。
- `authorization`：API を呼び出すためのキー。申請後、直接ドロップダウンから選択できます。

さらに、リクエストボディを設定しています。これには以下が含まれます：

- `prompt`：生成したい動画内容を説明するテキストプロンプト。`grok-imagine-video-1.5-fast` を使用してテキストから動画を生成する際は**必須**；`image_url` を送信する際はオプションです。
- `model`：生成する動画のモデル。`grok-imagine-video-1.5-fast`（デフォルト）または `grok-imagine-video-1.5` を選択できます。
- `image_url`：画像から動画を生成するための入力画像リンク。`model` が `grok-imagine-video-1.5` の場合は**必須**です。
...
- `duration`：生成する動画の長さ（秒）。`grok-imagine-video-1.5-fast` の範囲は 1–30、`grok-imagine-video-1.5` の範囲は 1–15、デフォルトは 6 です。6秒または10秒の使用を推奨します。これらの標準長は比較的安定しています。
...
入力画像に基づいて動画を生成したい場合は、`image_url` を送信できます。`grok-imagine-video-1.5` を使用する際は、このフィールドを必ず提供する必要があります：
...
動画生成には一定の処理時間が必要です。長接続を維持したくない場合は、`callback_url` を送信できます。この場合、API はすぐに `task_id` を返し、タスクが完了した後に最終結果をそのアドレスに POST します：
...
## タスク結果の照会
非同期コールバックを使用したり、タスクの状態を積極的に照会したい場合は、[Grok Tasks API](https://platform.acedata.cloud/documents/grok-tasks)（`POST https://api.acedata.cloud/grok/tasks`）を使用して `task_id` に基づいてタスクの最新の状態と結果を照会できます。

## 課金説明

本サービスの課金方式は `model` によって決まります：

- `grok-imagine-video-1.5-fast`：時間に応じて段階的に課金され、解像度には関係ありません——`1–10` 秒、`11–20` 秒、`21–30` 秒はそれぞれ異なる価格帯に対応します。
- `grok-imagine-video-1.5`：出力秒数に基づいて課金され、総額 = 単価 × `duration`、`1080p` は `480p`/`720p` よりも高価です。
...
- `400`：リクエストパラメータに誤りがあります。例えば、文生動画に `prompt` が欠けている、または `grok-imagine-video-1.5` に `image_url` が欠けている、または `duration` が範囲外（`grok-imagine-video-1.5-fast` は 1–30、`grok-imagine-video-1.5` は 1–15）です。
- `401`：認証に失敗しました。トークンが無効またはAPIと一致しません。
- `403`：残高不足、またはプロンプトがコンテンツ審査に引っかかり拒否されました。
- `429`：リクエストが頻繁すぎます。しばらくしてから再試行してください。
