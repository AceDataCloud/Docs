...
可変長英文数字验证码画像の検証結果が得られたことがわかります。結果の中の `text` の文字内容に基づいて検証を通過できます。
...
デフォルトでは、APIは同期ブロッキングです：1回のリクエストは、識別結果の処理が完了するまで待機します。複数の打码器のローテーション（multi-solver rotation）を行っている場合、「タスクを提出した後、すぐに task_id を取得し、他の打码器を調整し、後で結果を取得する」ことを希望する場合は、リクエストボディに `async: true` を渡すことができます。

`async: true` を渡すと、インターフェースはすぐに `task_id` を返し、待機しません：
...
請求の説明：非同期モードでは、タスクの作成と「処理中」のポーリングは課金されません；**識別結果を正常に取得したときにのみ、1回の課金が行われます**（同期モードの価格と一致）。したがって、ローテーション中に未完了のタスクをキャンセルしても費用は発生しません。`/captcha/tasks` はすべての验证码インターフェース（トークンと認識シリーズ）に共通で、同じ `task_id` でポーリングできます。
...
この文書を通じて、デジタル英数字認識APIの使用方法を理解しました。このAPIは、可変長の英数字認証コードを認識するために使用できます。認証コード画像の内容を入力し、認証コードの結果を出力します。この文書が、APIとの接続や使用をより良くする手助けとなることを願っています。ご不明な点がございましたら、いつでも技術サポートチームにお問い合わせください。
