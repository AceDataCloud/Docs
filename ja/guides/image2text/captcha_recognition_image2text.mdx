...
請求の説明：非同期モードでは、タスクの作成と「処理中」のポーリングは課金されません；**識別結果を正常に取得したときにのみ1回課金されます**（同期モードの価格と一致します）。したがって、ローテーション中に未完了のタスクをキャンセルしても費用は発生しません。`/captcha/tasks` はすべての验证码インターフェース（トークンと認識シリーズ）に共通で、同じ `task_id` でポーリングできます。
...
この文書を通じて、デジタル英数字認識APIを使用して可変長の英数字認証コードを認識する方法を理解しました。認証コード画像の内容を入力し、認証コードの結果を出力します。この文書がAPIの接続と使用に役立つことを願っています。ご不明な点がございましたら、いつでも技術サポートチームにお問い合わせください。
