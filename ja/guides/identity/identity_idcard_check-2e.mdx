---
title: "身分情報および有効期限検証 API 接続説明"
description: "Identity Authentication API guide - Ace Data Cloud"
---

本文では、身分情報および有効期限検証 API 接続説明について紹介します。これは、名前、身分証明書番号、身分証明書の有効期限の真実性と一貫性を検証するために使用できます。

## 申請プロセス

身分情報および有効期限検証 API を使用するには、まず [Ace Data Cloud コンソール](https://platform.acedata.cloud/console/applications) で API トークンを取得し、保管してください。
...
> 📘 完全なドキュメント：[身分情報および有効期限検証 API →](https://platform.acedata.cloud/documents/identity-idcard-check-2e)
...
- `result`、認証結果コード、料金状況は以下の通りです。
    ...
    - 無料結果コード：
        ...
        - -7: 認証回数が当日の制限を超えました。
- `description`、ここでの名前と身分証明書番号の検証結果。

身分証明書の名前、身分証明書番号、有効期限が真実性と一貫性を持っていることがわかります。
...
この文書を通じて、身分情報および有効期限検証 API を使用して名前、身分証明書番号、身分証明書の有効期限の真実性と一貫性を検証する方法を理解しました。この文書が、API の接続と使用をより良くする手助けとなることを願っています。何か問題があれば、いつでも技術サポートチームにお問い合わせください。
