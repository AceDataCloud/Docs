...
自分がアップロードした曲を続けて翻唱したい場合は、パラメータ `action` を `upload_cover` に設定し、続けて翻唱するカスタマイズアップロードの曲 ID を入力します。曲 ID の取得は [Producer Upload API](https://platform.acedata.cloud/documents/producer-upload) を使用して取得します。以下の図のように：
...
自分がアップロードした曲を続けて置き換えたい場合は、`action`パラメータを`upload_replace_section`に設定し、続けてカスタマイズしたいアップロードした曲のIDを入力します。曲のIDは[Producer Upload API](https://platform.acedata.cloud/documents/producer-upload)を使用して取得します。以下の図のように：
