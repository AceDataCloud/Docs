---
title: "Producer 歌詞生成インターフェース API 接続説明"
description: "Producer Music Generation API guide - Ace Data Cloud"
---

Producer は、プロンプトを通じて歌詞を生成することを許可します。本ドキュメントでは、関連する API の接続方法を説明します。

この API には、生成する歌詞のプロンプトである `prompt` という唯一の入力パラメータがあります。

ここで入力する `prompt` は `A song about winter` で、歌詞生成のためのプロンプトです。
...
`data` の `lyrics` フィールドが生成された歌詞の内容であることがわかります。
