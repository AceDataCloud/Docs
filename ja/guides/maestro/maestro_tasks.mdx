...
Maestro タスク照会 API の主な機能は、[Maestro 動画生成 API](development_maestro_videos.md)（`POST /maestro/videos`）から返されるタスク ID を通じて、そのタスクの実行状態と最終結果を照会することです。
...
動画タスクの作成方法については、ドキュメント [Maestro 動画生成 API](development_maestro_videos.md) を参照してください。ここでは、その返り値のタスク ID の例である `f57e99c4f60f4373a15517742ce2357d` を用いて、その状態と結果を照会する方法を説明します。
...
- [Maestro 動画生成 API 連携説明](development_maestro_videos.md)：自然言語のプロンプト一文で字幕付きの完成動画を自動生成し、送信後に `task_id` を返します。その後、本インターフェースで結果をポーリングします。
