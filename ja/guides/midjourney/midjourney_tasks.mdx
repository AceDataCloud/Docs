...
- `job_id`：今回の画像タスクの照会を生成するための ID で、今回の照会画像タスクを一意に識別します。
- `image_id`：ここで照会する画像タスクの画像の一意の識別子で、次回画像を変換する操作を行う際にこのパラメータを渡す必要があります。
...
- `elapsed`：タスク実行にかかる時間、単位は秒（浮動小数点、3 桁の小数点以下を保持）。タスクが未完了の場合はこのフィールドは返されません。
...
ページの右側には、さまざまな言語のコードが自動的に生成されていることがわかります。図のように：
