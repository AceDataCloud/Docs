...
まず、基本的な使用方法を理解します。これは、プロンプト `prompt`、生成アクション `action`、先頭と末尾のフレーム参照画像配列 `image_urls`、およびモデル `model` を入力することで、処理された結果を得ることができます。最初に、`action` フィールドを簡単に渡す必要があります。その値は `text2video` で、主に3つのアクションが含まれます：テキストから動画（`text2video`）、画像から動画（`image2video`）、1080p動画の取得（`get1080p`）。次に、モデル `model` を入力する必要があります。現在、主に `veo31-fast`、`veo3`、`veo31`、`veo2-fast`、`veo3-fast`、および `veo31-fast-ingredients` モデルがあります。具体的な内容は以下の通りです：
...
- `model`：生成する動画のモデル。主に `veo31-fast`、`veo3`、`veo31`、`veo2-fast`、`veo3-fast`、および `veo31-fast-ingredients` モデルがあります。
- `action`：今回の動画生成タスクのアクション。主に3つのアクションが含まれます：テキストから動画（`text2video`）、画像から動画（`image2video`）、1080p動画の取得（`get1080p`）。
- `image_urls`：画像から動画アクション `image2video` を選択した場合に必ずアップロードする必要がある参照画像のリンク。`veo2-fast` は1枚のみサポートし、`veo31-fast-ingredients` は最大3枚（複数画像融合）をサポートし、他のモデルは最大2枚（先頭と末尾のフレームモード）をサポートします。
...
| -------------------------- | --------------------------------- | --------------------------------- |
| **veo2-fast**              | テキストから動画（画像なし）<br />画像から動画モード（画像あり）           | **1枚のみ** → 先頭フレームモード                |
| **veo3-fast**              | テキストから動画（画像なし）<br />画像から動画モード（画像あり）           | **1枚** → 先頭フレームモード<br />**2枚** → 先頭と末尾フレームモード |
| **veo31-fast**             | テキストから動画（画像なし）<br />画像から動画モード（画像あり）           | **1枚** → 先頭フレームモード<br />**2枚** → 先頭と末尾フレームモード |
| **veo31-fast-ingredients** | ❌ テキストから動画（サポートなし）<br />✅ **強制的に複数画像融合**（画像必須） | **1-3枚** → 複数画像融合モード（最大3枚）        |
| **veo3**                   | テキストから動画（画像なし）<br />画像から動画モード（画像あり）           | **1枚** → 先頭フレームモード<br />**2枚** → 先頭と末尾フレームモード |
| **veo31**                  | テキストから動画（画像なし）<br />画像から動画モード（画像あり）           | **1枚** → 先頭フレームモード<br />**2枚** → 先頭と末尾フレームモード |
...
1. **一般的なロジック**：
   ...
   - **画像入力あり** → 画像から動画モードがトリガーされます（具体的な動作は画像の数によって決まります）。
2. **画像から動画モードのタイプ**：
   - **先頭フレームモード**（1枚の画像）：先頭フレームは入力画像に固定されます。
   - **先頭と末尾フレームモード**（2枚の画像）：先頭フレームと末尾フレームは入力画像に固定されます。
   - **複数画像融合モード**（1-3枚の画像）：`veo31-fast-ingredients` のみがサポートし、複数画像の内容を融合して動画を生成します。
3. **モード分類**：
   - **ファストモード**：`veo2-fast`、`veo3-fast`、`veo31-fast`、`veo31-fast-ingredients`。
...
- **唯一の強制画像入力モデル**：`veo31-fast-ingredients` は画像を必ず入力する必要があります（1-3枚）、そうでないと実行できません。
- **画像数の制限**：
  - `veo2-fast` は **1枚** の画像入力のみサポート（先頭フレームモード）。
  - `veo31-fast-ingredients` は **1-3枚** の画像入力をサポート（複数画像融合モード）。
  - 他のモデルは最大 **2枚** の画像入力をサポート（先頭と末尾フレームモード）。
...
返却された結果には複数のフィールドがあり、以下のように説明されています：

- `success`、この時の動画生成タスクの状態。
- `task_id`、この時の動画生成タスクID。
- `data`、この時の動画生成タスクの結果。
  - `id`、この時の動画生成タスクの動画ID。
  - `video_url`、この時の動画生成タスクの動画リンク。
  - `created_at`、この時の動画生成タスクの作成時間。
  - `complete_at`、この時の動画生成タスクの完了時間。
  - `state`、この時の動画生成タスクの状態。
...
もし、始まりと終わりのフレーム画像に基づいて動画を生成したい場合は、パラメータ `action` を `image2video` に設定し、始まりと終わりのフレーム画像リンクの配列 `image_urls` を入力する必要があります。

次に、生成する動画をカスタマイズするために拡張する必要があるプロンプトを入力する必要があります。以下の内容を指定できます：

- `model`：生成動画のモデル、主に `veo31-fast`、`veo3`、`veo31`、`veo2-fast`、`veo3-fast` および `veo31-fast-ingredients` があります。
- `image_urls`：動画生成行動 `image2video` を選択した場合に必要な参考画像リンク。
- `prompt`：プロンプト。
...
- `model`：生成動画のモデル、主に `veo31-fast`、`veo3`、`veo31`、`veo2-fast`、`veo3-fast` および `veo31-fast-ingredients` があります。
- `video_id`：参考動画ID、1080p動画を取得するために使用します。
...
結果の内容が前述のものと一致していることがわかります。これにより、動画の1080p取得機能が実現されました。
...
結果の内容が上記と一致していることがわかります。これにより、指定したサイズでの動画生成機能が実現されました。
...
全体の流れは次の通りです：クライアントがリクエストを発行する際に、追加で`callback_url`フィールドを指定します。クライアントがAPIリクエストを発行した後、APIはすぐに結果を返し、現在のタスクIDを示す`task_id`フィールド情報を含みます。タスクが完了すると、生成された動画の結果がPOST JSON形式でクライアントが指定した`callback_url`に送信され、その中にも`task_id`フィールドが含まれます。これにより、タスクの結果をIDで関連付けることができます。
...
このURLをコピーすればWebhookとして使用できます。このサンプルは`https://webhook.site/aed5cd28-f8aa-4dca-9480-8ec9b42137dc`です。
...
少し待つと、`https://webhook.site/aed5cd28-f8aa-4dca-9480-8ec9b42137dc`で生成された動画の結果を観察できます。以下のように示されています：
...
この文書を通じて、Veo Videos Generation APIを使用して、入力プロンプトと最初のフレームの参照画像を通じて動画を生成する方法を理解しました。この文書がAPIの接続と使用に役立つことを願っています。ご不明な点がございましたら、いつでも技術サポートチームにお問い合わせください。
