...
class SyncFromPlatformBackendTests(unittest.TestCase):
    ...
    def test_lobechat_has_an_integration_output_record(self) -> None:
        bundle = json.loads(sync.EXACT_MAP_PATH.read_text())
        record = next(row for row in bundle["records"] if row["source_doc_key"] == "development_lobechat")
        self.assertEqual(record["service_alias"], "integrations")
        self.assertEqual(record["output_path"], "guides/integrations/lobechat.mdx")
        self.assertEqual(record["canonical_alias"], "development_lobechat")
