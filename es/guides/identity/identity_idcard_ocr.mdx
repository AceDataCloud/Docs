...
Para utilizar la API de reconocimiento de identificación y verificación de información, primero dirígete a [la consola de Ace Data Cloud](https://platform.acedata.cloud/console/applications) para obtener tu token de API, que debes guardar como respaldo.
...
> 📘 Documentación completa: [API de reconocimiento de identificación y verificación de información →](https://platform.acedata.cloud/documents/identity-idcard-ocr)

## Uso básico

Primero, debes entender la forma básica de uso, que consiste en ingresar el enlace de la imagen de la identificación para obtener el resultado de verificación procesado. Primero, necesitas pasar un campo `image_url`, y luego puedes completar el contenido correspondiente en la interfaz, como se muestra en la imagen:
...
- `encryption`: opcional, parámetro de cifrado para campos sensibles (si se requiere enviar datos cifrados).
...
Haz clic en el botón "Try" para realizar la prueba, como se muestra en la imagen anterior, y obtendrás el siguiente resultado:
...
- `result`, código de resultado de autenticación, la situación de cobro es la siguiente.
    ...
    - Códigos de resultado sin cobro:
        ...
        - -6: Sistema de comparación autoritativa en actualización, por favor intenta más tarde
...
También ofrecemos una forma de verificar sin revelar la información de la imagen de la identificación, que consiste en enviar solo el nombre `name` y el número de identificación `id_card` para verificar la autenticidad y consistencia del nombre y el número de identificación. A continuación se muestran los detalles de la información ingresada:

<p><img src="https://cdn.acedata.cloud/mc5bbw.png" width="500" className="m-auto" /></p>

Al hacer clic en ejecutar, se obtiene el siguiente resultado:
...
- `429 too_many_requests`: Demasiadas solicitudes, has excedido el límite de tasa.
...
A través de este documento, has aprendido cómo utilizar la API de reconocimiento de identificación y verificación de información para verificar la autenticidad y consistencia del nombre y el número de identificación a partir de la imagen o la información del nombre y el número de identificación ingresados. Esperamos que este documento te ayude a integrar y utilizar mejor esta API. Si tienes alguna pregunta, no dudes en contactar a nuestro equipo de soporte técnico.
