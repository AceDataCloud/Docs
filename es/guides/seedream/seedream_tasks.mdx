...
Para utilizar la API de SeeDream Images Generation, primero dirígete a [la consola de Ace Data Cloud](https://platform.acedata.cloud/console/applications) para obtener tu token de API y guardarlo para uso futuro.
...
Los resultados devueltos contienen múltiples campos, donde `items` incluye detalles específicos de tareas por lotes, y cada tarea tiene la misma información de campo que el resultado de la consulta de una sola tarea anterior.

- `items`, todos los detalles específicos de la tarea por lotes. Es un arreglo, y cada elemento del arreglo tiene el mismo formato que el resultado de la consulta de una sola tarea anterior.
- `count`, el número de tareas en esta consulta por lotes.

Cada objeto de tarea en el resultado por lotes también incluye los siguientes campos de tiempo.
