...
También puedes notar que a la derecha hay un código de invocación correspondiente generado, puedes copiar el código para ejecutarlo directamente o hacer clic en el botón "Try" para realizar pruebas.
...
`kimi-k3` siempre habilita el razonamiento. El cuerpo de la solicitud admite el campo `reasoning_effort` en el nivel superior, y el único valor actualmente admitido es `max`; si se omite este campo, también se utilizará `max`. `standard`, `high` u otras cadenas pueden ser aceptadas de manera flexible por algunas implementaciones, pero no garantizan un cambio en el comportamiento del razonamiento, no se debe depender de ello.
...
- [Thinking Effort](https://platform.kimi.ai/docs/guide/use-thinking-effort): explica que Kimi K3 siempre habilita el razonamiento, y el único valor admitido actualmente para `reasoning_effort` es `max`.
...
Esta interfaz también admite respuestas en streaming, lo cual es muy útil para la integración web, ya que permite mostrar el contenido palabra por palabra.

Si deseas que la respuesta se devuelva en streaming, puedes cambiar el parámetro `stream` en el encabezado de la solicitud a `true`.

El cambio se muestra en la imagen, pero el código de invocación necesita tener los cambios correspondientes para admitir respuestas en streaming.
...
A continuación se extraen los bloques de datos de inicio, razonamiento, cuerpo, finalización y uso de una respuesta en streaming real de K3 Max:
```json
...
Al cargar múltiples consultas, se puede lograr fácilmente un diálogo en múltiples rondas. A continuación se muestra la respuesta real obtenida de K3 Max para esta solicitud (se omiten los campos de extensión no utilizados):
...
A través de este documento, ha aprendido cómo utilizar la API de Kimi Chat Completion para implementar conversaciones normales, respuestas en flujo, diálogos en múltiples rondas, así como controlar la intensidad de razonamiento de K3 a través de `reasoning_effort`.
