...
Para usar la API de Traducción de Localización, primero dirígete a [la consola de Ace Data Cloud](https://platform.acedata.cloud/console/applications) para obtener tu token de API, que debes guardar para uso futuro.
...
> 📘 Documentación completa: [API de Traducción de Localización →](https://platform.acedata.cloud/documents/localization-translate)

## Ejemplo de Solicitud

Tomemos un ejemplo de entrada en inglés para demostrar cómo usar esta API. Supongamos que la descripción en inglés es: `# Title 1\n\nThis is a paragraph.\n\n## Title 2\n\nThis is another paragraph.`, a continuación, se mostrará cómo subir la descripción en inglés y obtener el resultado de la traducción al chino, presentándolo en formato markdown.
...
- `input`: el contenido que necesita ser traducido. Se debe pasar como cadena cuando `extension=md`, y como objeto JSON cuando `extension=json`.
- `locale`: el idioma de traducción personalizado.
- `extension`: el formato del contenido de entrada, puede ser `md` o `json`.
- `model`: el modelo grande utilizado para la traducción, por defecto es gpt-3.5.

| `extension` | Tipo de `input` | Ejemplo                                   |
| ----------- | --------------- | ---------------------------------------- |
| `md`        | string          | `"# Title\n\nParagraph"`                 |
| `json`      | object          | `{"title": "Hello", "button": "Submit"}` |

`extension` debe coincidir con el tipo de `input`; las cadenas no pueden usar `json`, y los objetos JSON no pueden usar `md`.
...
A través de este documento, ha aprendido cómo usar la API de Traducción de Localización para traducir fácilmente el texto ingresado a un idioma específico y admite formas específicas para presentar los resultados de la traducción. Esperamos que este documento le ayude a integrar y utilizar mejor esta API. Si tiene alguna pregunta, no dude en ponerse en contacto con nuestro equipo de soporte técnico.
