---
title: "hCaptcha Protocolo de Reconocimiento API Instrucciones de Integración"
description: "hCaptcha verification code recognition service API guide - Ace Data Cloud"
---

Este artículo presenta una instrucción para la integración del API de reconocimiento del protocolo hCaptcha, que permite a los usuarios decodificar automáticamente en segundo plano el captcha hCaptcha sin necesidad de reconocer y hacer clic en las imágenes del captcha, simplemente enviando la Clave del Sitio (Website Key) para completar la verificación.

## Proceso de Solicitud

Para usar el API de reconocimiento del protocolo hCaptcha, primero vaya a la [Consola de Ace Data Cloud](https://platform.acedata.cloud/console/applications) para obtener su Token API, que debe guardar para uso futuro.

![](https://cdn.acedata.cloud/5hmkdg.jpg)

Si aún no ha iniciado sesión o registrado, será redirigido automáticamente a la página de inicio de sesión para registrarse e iniciar sesión, y luego volverá automáticamente a esta página.

**Un solo Token API puede llamar a todos los servicios de la plataforma, no es necesario solicitar uno por cada servicio.** La primera solicitud incluye un crédito gratuito para que pueda probarlo sin costo; cuando el crédito se agote, puede recargar saldo general en la [Consola](https://platform.acedata.cloud/console/coin).
...
Primero, entienda la forma básica de uso, que es ingresar la URL del sitio web que necesita procesar el captcha hCaptcha para obtener el resultado procesado. Primero debe enviar un campo `website_url`. Nuestro sitio de ejemplo es: `https://accounts.hcaptcha.com/demo`. Necesitamos obtener la `website_key` en la página `website_url`. Primero abra esta página web, presione F12 para abrir la consola, y finalmente en la pestaña Elementos realice una búsqueda global de `hcaptcha-demo`, donde obtendremos el siguiente resultado:

<p><img src="https://cdn.acedata.cloud/bmx7ik.png" width="500" className="m-auto" /></p>

La cadena correspondiente a `data-sitekey` es el valor de `website_key`. A continuación se muestran los parámetros específicos:

<p><img src="https://cdn.acedata.cloud/st6q7a.png" width="500" className="m-auto" /></p>

Aquí podemos ver que configuramos los Encabezados de la Solicitud (Request Headers), incluyendo:

- `accept`: el formato de respuesta que se desea recibir, aquí se establece como `application/json`, es decir, formato JSON.
- `authorization`: la clave para llamar al API, que puede seleccionar directamente después de solicitarla.

Además, configuramos el Cuerpo de la Solicitud (Request Body), que incluye:

- `website_url`: la URL del sitio web cuyo captcha se necesita procesar.
- `website_key`: el identificador de clave del sitio en hCaptcha.

Después de seleccionar, puede ver que en el lado derecho también se genera el código correspondiente, como se muestra en la imagen:

<p><img src="https://cdn.acedata.cloud/5gse4m.png" width="500" className="m-auto" /></p>

Haga clic en el botón «Try» para realizar la prueba. Como se muestra en la imagen anterior, obtuvimos el siguiente resultado:
```json
{
