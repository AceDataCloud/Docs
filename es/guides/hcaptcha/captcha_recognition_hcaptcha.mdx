...
Si aún no has iniciado sesión o registrado, serás redirigido automáticamente a la página de inicio de sesión que te invitará a registrarte e iniciar sesión, y una vez completado, serás redirigido de nuevo a la página actual.
...
Primero, debes entender la forma básica de uso, que consiste en ingresar la imagen del captcha de hCaptcha que necesitas procesar para obtener el resultado procesado. Primero, necesitas pasar un campo `queries`, que es la imagen del captcha de hCaptcha específica. Debes capturar esta imagen del captcha en un sitio web que tenga el captcha de hCaptcha; el enlace del sitio de ejemplo es: `https://democaptcha.com/demo-form-eng/hcaptcha.html`, haz clic en la casilla de verificación para mostrar la imagen completa del captcha, como se muestra en la siguiente imagen:
...
Además, necesitas ingresar el parámetro de contenido de reconocimiento relacionado con la imagen del captcha `question`, que admite traducción en chino e inglés, y puedes ingresar directamente el contenido relacionado. Según el contenido ejecutado en la imagen del sitio web anterior, el `question` que debes ingresar es `Please click on the UNIQUE object among the others.`. El contenido específico es el siguiente:
...
- `started_at`, `finished_at`: el tiempo de inicio del procesamiento de esta solicitud y el tiempo de producción del resultado, en marca de tiempo Unix (segundos, punto flotante).
- `elapsed`: el tiempo total de procesamiento de esta solicitud (segundos).

Como puedes ver, hemos obtenido el resultado de la verificación de la imagen del captcha de hCaptcha, y solo necesitamos simular un clic en el área correspondiente de la imagen del captcha según la información de coordenadas de `box` para pasar la verificación.
...
Al completar el procesamiento, se devolverá `status: ready` y el resultado de la identificación `solution` (la estructura de los campos es completamente consistente con el modo síncrono):
