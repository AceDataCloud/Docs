---
title: "Instrucciones de integración de la API de generación MIDI de Suno"
description: "Suno Music Generation API guide - Ace Data Cloud"
---

SUNO nos permite realizar una segunda creación de la música generada, obteniendo el MIDI de la música. Este documento explica cómo integrar la API relacionada.

El parámetro de entrada principal de esta API es `audio_id`, que es el ID de la canción generada oficialmente, generalmente es el ID de la canción con todas las pistas separadas (correspondiente al [generación de canciones](https://platform.acedata.cloud/documents/suno-audios) donde el parámetro `action` es `all_stems`). Opcionalmente, también se admite la dirección de callback `callback_url` para la devolución de llamada asíncrona.

El `audio_id` que ingresamos aquí es `c65e6ffd-ead3-4926-9c8c-a42ce202946b`.
...
Se puede ver que los campos en `data` son la información MIDI de la canción después de la separación de la pista completa.
