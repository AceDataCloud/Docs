...
Al igual que en v1, envía `stateful: true` para habilitar el guardado de sesiones, la API devolverá un `id`; las solicitudes posteriores deben incluir el `id` para continuar la conversación, sin necesidad de mantener el historial de mensajes.
...
2. Si anteriormente usabas nombres de modelos v1 (como `gpt-3.5`, `gpt-4-browsing`, etc.), al cambiar a v2 se recomienda actualizar a modelos contemporáneos (como `gpt-5.4`, `claude-opus-4-8`, `gemini-3.1-pro-preview`, etc.).
...
- `429 too_many_requests`: se ha activado el límite de velocidad.
