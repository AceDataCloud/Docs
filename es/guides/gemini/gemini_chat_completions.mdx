...
- `choices`, la información de respuesta que Gemini proporciona para las palabras clave.
...
Se puede observar que el campo `content` dentro de `choices` contiene el contenido específico de la respuesta de Gemini.

## Comprensión de Imágenes (Entrada Multimodal)

Gemini es un modelo multimodal nativo que puede "ver imágenes" directamente. Para enviar una imagen, simplemente cambia el `content` de un mensaje de cadena a un **array de bloques de contenido**, donde se incluyen bloques de `text` y `image_url` — esto es completamente compatible con el formato de OpenAI y el formato oficial de Gemini.
...
> 💡 `image_url` solo acepta el campo `url` (el valor puede ser una URL de imagen o un URI de datos base64), así como un campo `detail` opcional. **No envíes `media_type`** — eso es un campo de imagen de Anthropic Claude, que no pertenece al formato `image_url` de OpenAI / Gemini.
...
Como se puede ver, la respuesta contiene muchos `data`, y el `choices` dentro de `data` es el contenido de la respuesta más reciente, que coincide con el contenido presentado anteriormente. `choices` es el contenido de respuesta adicional, que puedes integrar en tu sistema según los resultados. Al mismo tiempo, el final de la respuesta en flujo se determina según el contenido de `data`, si el contenido es `[DONE]`, significa que la respuesta en flujo ha terminado por completo. Los resultados devueltos de `data` tienen varios campos, que se describen a continuación:
...
- `choices`, la información de respuesta que Gemini proporciona en respuesta a la consulta.
...
jsonObject.put("messages", new JSONArray().put(new JSONObject().put("role", "user").put("content", "Hola, ¿qué modelo eres?")));
...
Como se puede ver, la información contenida en `choices` es consistente con el contenido de uso básico, esto incluye el contenido específico de las respuestas de Gemini a múltiples diálogos, lo que permite responder a las preguntas correspondientes según el contenido de múltiples diálogos.
...
Como se puede ver, el modelo Gemini 3.0 puede soportar la comprensión multimodal.

## Gemini-3.1 Modelo Multimodal

Gemini 3.1 Pro es una versión mejorada de Gemini 3.0 Pro, con un modelo subyacente de `gemini-3.1-pro-preview`, que también admite entradas multimodales como texto, imágenes y videos, y tiene una mayor capacidad de razonamiento y comprensión. La forma de uso es completamente idéntica a la de Gemini 3.0 Pro, solo necesitas reemplazar el parámetro `model` por `gemini-3.1-pro`.
...
El formato de respuesta es el mismo que el de Gemini 3.0 Pro, ver la descripción en la sección del modelo multimodal Gemini-3.0 anterior.
