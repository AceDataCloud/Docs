...
Para usar la API de finalización de chat de Gemini, primero dirígete a [la consola de Ace Data Cloud](https://platform.acedata.cloud/console/applications) para obtener tu token API, que debes guardar como respaldo.
...
Gemini es un modelo multimodal nativo que puede "ver imágenes" directamente. Para enviar una imagen, simplemente cambia el `content` de un mensaje de cadena a un **array de bloques de contenido**, donde se incluyen bloques de `text` y `image_url` — esto es completamente compatible con el formato de OpenAI y el de Gemini.
...
# Leer la imagen local como un URI de datos base64
...
> 💡 `image_url` solo acepta el campo `url` (el valor puede ser una URL de imagen o un URI de datos base64), así como un campo opcional `detail`. **No envíes `media_type`** — eso es un campo de imagen de Anthropic Claude, que no pertenece al formato `image_url` de OpenAI / Gemini.

## Respuesta en flujo

Esta interfaz también admite respuestas en flujo, lo cual es muy útil para la integración web, permitiendo que la página muestre el contenido palabra por palabra.
...
Una vez que `stream` se modifica a `true`, la API devolverá los datos JSON correspondientes línea por línea, y a nivel de código necesitamos hacer los cambios necesarios para obtener los resultados línea por línea.
...
Como se puede ver, la respuesta contiene muchos `data`, y el `choices` dentro de `data` es el contenido de la respuesta más reciente, que coincide con el contenido presentado anteriormente. `choices` es el contenido de respuesta nuevo, que puedes integrar en tu sistema según los resultados. Al mismo tiempo, el final de la respuesta en flujo se determina según el contenido de `data`, si el contenido es `[DONE]`, significa que la respuesta en flujo ha terminado por completo. Los resultados de `data` devueltos tienen múltiples campos, que se describen a continuación:
...
Si desea integrar la función de diálogo en múltiples turnos, necesita subir múltiples preguntas en el campo `messages`, ejemplos específicos de múltiples preguntas se muestran en la imagen a continuación:
...
Al subir múltiples preguntas, se puede lograr fácilmente un diálogo en múltiples turnos, obteniendo respuestas como la siguiente:
...
Como se puede ver, la información contenida en `choices` es consistente con el contenido del uso básico, esto incluye el contenido específico de las respuestas de Gemini a múltiples diálogos, lo que permite responder a las preguntas correspondientes según el contenido de múltiples diálogos.
...
`gemini-3.1-pro-preview` es el ID oficial del modelo Gemini 3.1 Pro, que admite entradas multimodales como texto, imágenes y videos, adecuado para tareas complejas de razonamiento, codificación y comprensión.
...
El formato de respuesta es consistente con el de Gemini 3.0 Pro, consulte la sección anterior sobre el modelo multimodal Gemini-3.0 para más detalles.
