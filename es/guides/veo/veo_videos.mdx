...
- `resolution`: selecciona la resolución del video generado, donde el modelo veo31 admite resolución 4k, mientras que los otros modelos no la admiten; todos los modelos admiten resoluciones de 1080p y gif, y si no se proporciona este valor, se utilizará por defecto la resolución de 720p, que se clasifica en: `1080p`, `gif`, `4k`.
...
| **Nombre del modelo**                   | **Modos admitidos**                          | **Reglas de entrada de imágenes**                        |
| -------------------------- | --------------------------------- | --------------------------------- |
| **veo3-fast**              | Generación de video a partir de texto (sin imágenes)<br />Modo de generación de video a partir de imágenes (con imágenes)           | **1 imagen** → Modo de inicio<br />**2 imágenes** → Modo de inicio y fin |
| **veo31-fast**             | Generación de video a partir de texto (sin imágenes)<br />Modo de generación de video a partir de imágenes (con imágenes)           | **1 imagen** → Modo de inicio<br />**2 imágenes** → Modo de inicio y fin |
| **veo31-fast-ingredients** | ❌ Generación de video a partir de texto (no admitido)<br />✅ **Fusión de múltiples imágenes obligatoria** (debe proporcionar imágenes) | **1-3 imágenes** → Modo de fusión de múltiples imágenes (máximo 3 imágenes)        |
| **veo3**                   | Generación de video a partir de texto (sin imágenes)<br />Modo de generación de video a partir de imágenes (con imágenes)           | **1 imagen** → Modo de inicio<br />**2 imágenes** → Modo de inicio y fin |
| **veo31**                  | Generación de video a partir de texto (sin imágenes)<br />Modo de generación de video a partir de imágenes (con imágenes)           | **1 imagen** → Modo de inicio<br />**2 imágenes** → Modo de inicio y fin |
...
1. **Lógica general**:
   - **Sin entrada de imágenes** → Se activa automáticamente el modo de generación de video a partir de texto.
   - **Con entrada de imágenes** → Se activa el modo de generación de video a partir de imágenes (el comportamiento específico depende de la cantidad de imágenes).
2. **Tipos de modo de generación de video a partir de imágenes**:
   ...
   - **Modo de fusión de múltiples imágenes** (1-3 imágenes): solo `veo31-fast-ingredients` lo admite, fusionando el contenido de múltiples imágenes para generar el video.

3. **Clasificación de modos**:

  - **Modo Rápido**: `veo3-fast`, `veo31-fast`, `veo31-fast-ingredients`.
   - **Modo de Calidad**: `veo3`, `veo31` (generación de mayor calidad).
...
Haz clic en el botón "Probar" para realizar la prueba, como se muestra en la imagen anterior, y aquí obtuvimos el siguiente resultado:
...
Se puede ver que hemos obtenido información satisfactoria del video, solo necesitamos obtener el video generado a partir de la dirección del enlace de video en `data`.
...
- `image_urls`：cuando se elige la acción de video a partir de imágenes `image2video`, es necesario subir los enlaces de las imágenes de referencia.
...
El código correspondiente en Python es:
...
Si se desea obtener un video de Veo ya generado en 1080p, se puede establecer el parámetro `action` como `get1080p` y proporcionar el ID del video que se necesita obtener en 1080p. El ID del video se obtiene según el uso básico, como se muestra en la siguiente imagen:
...
Si se desea especificar la generación de un video de Veo de tamaño personalizado, se puede establecer el parámetro `aspect_ratio` como el tamaño deseado. A continuación, debemos completar el siguiente paso con las palabras clave que necesitamos para personalizar la generación del video, especificando el siguiente contenido:
