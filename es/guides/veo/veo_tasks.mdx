...
Para utilizar la API de Veo Videos Generation, primero debe obtener su token de API en el [panel de control de Ace Data Cloud](https://platform.acedata.cloud/console/applications) y guardarlo para uso futuro.
...
La API de Veo Tasks se puede utilizar para consultar los resultados de la API de Veo Videos Generation. Para saber cómo utilizar la API de Veo Videos Generation, consulte la documentación [API de Veo Videos Generation](https://platform.acedata.cloud/documents/veo-videos).
...
- `items`，批量视频任务的所有具体详情信息。它是一个数组，每个数组的元素 y el formato de la respuesta de una sola tarea es el mismo que el anterior.
- `count`，此处批量查询视频任务的个数。
