...
> Para crear un timbre, consulte la [Fish Model Create API](https://platform.acedata.cloud/documents/fish-model). Para consultas paginadas, consulte la [Fish Model Query API](https://platform.acedata.cloud/documents/fish-model-query).

## Proceso de solicitud

Para usar la API, primero debe solicitar el servicio correspondiente en la página de [Fish Model API](https://platform.acedata.cloud/services/fish). Al ingresar, haga clic en el botón «Acquire».
...
- **Parámetros de ruta**: `{id}` es el `_id` del timbre, que puede obtenerse al crear con la [Fish Model Create API](https://platform.acedata.cloud/documents/fish-model) o mediante la consulta paginada de la [Fish Model Query API](https://platform.acedata.cloud/documents/fish-model-query).
...
El `_id` devuelto puede usarse como valor del campo `reference_id` en la [Fish TTS API](https://platform.acedata.cloud/documents/fish-tts) para realizar síntesis de voz con este clon de timbre.
