...
| `format`       | string             | No   | Formato de audio de salida, puede ser `mp3` (por defecto), `wav`, `pcm`. Tanto `wav` como `pcm` devuelven contenedores WAV. `opus` no es compatible, si se envía, se devolverá directamente `400`.                                                                                      |
...
| `latency`      | string             | No   | `normal` o `balanced`, por defecto este interfaz automáticamente completa con `normal` (si se envía una cadena vacía, el upstream la rechazará).                                                                                                                    |
...
A continuación, se utiliza un tono de voz en español público en la plataforma Fish (el `_id` se puede recuperar a través de [Consulta de Modelos de Fish](https://platform.acedata.cloud/documents/fish-model-query)):
...
`speed` mayor que 1 acelera, menor que 1 desacelera; `volume` en dB, 0 significa sin cambio, números positivos son ganancia, números negativos son atenuación.
...
> La extensión del enlace sigue el `format` en la solicitud: `mp3` obtiene `.mp3`, `wav` y `pcm` obtienen `.wav` (contenedor WAV, PCM de 16 bits).

## Callback asíncrono (`callback_url`)

La síntesis de texto largo puede tardar de decenas de segundos a varios minutos, si la conexión se interrumpe, es necesario reintentar. Al enviar `callback_url` en el cuerpo de la solicitud, la interfaz devolverá inmediatamente `{task_id, started_at}`, y cuando se complete realmente el proceso, se enviará el resultado completo en formato JSON POST a esa URL, incluyendo el mismo `task_id` y el `cost` final de esta tarea. La tarea inicial confirmará que la síntesis aún no se ha completado, por lo que no incluirá `cost`.
...
También se puede usar [Fish Tasks API](https://platform.acedata.cloud/documents/fish-tasks) para obtener resultados de manera activa según `task_id`; el `response.cost` del estado final coincide con el `cost` en el callback, ver ese documento para más detalles.
...
El costo mínimo para integrar Fish TTS es: en el código que ya llama a `api.fish.audio/v1/tts`, cambiar la autenticación por el token de esta plataforma y en el cuerpo de la solicitud **incluir explícitamente** `format: "mp3"`. Para escenarios de texto largo, se recomienda usar `callback_url` para el callback asíncrono; para descubrir el `reference_id` de la clonación de voces, consulte [Fish Model Query](https://platform.acedata.cloud/documents/fish-model-query) y [Fish Model Get](https://platform.acedata.cloud/documents/fish-model-get).
