...
| `references`   | object[]           | No   | Muestras de referencia en línea, la estructura es la misma que la del upstream, cada elemento contiene `audio` y `text`. Uno de `reference_id` o `references` debe ser proporcionado.                                                                                                                  |
...
| `latency`      | string             | No   | `normal` o `balanced`, por defecto este interfaz automáticamente completa con `normal` (si se envía una cadena vacía, el upstream rechazará).                                                                                                                    |
...
A continuación, se utiliza un tono de voz en español público en la plataforma Fish (`_id` se puede recuperar a través de [Consulta de Modelos de Fish](https://platform.acedata.cloud/documents/fish-model-query)):
...
La síntesis de texto largo puede tardar de decenas de segundos a varios minutos, si la conexión se interrumpe, es necesario reintentar. Al enviar `callback_url` en el cuerpo de la solicitud, la interfaz devolverá inmediatamente `{task_id, started_at}`, y cuando se complete realmente, el resultado completo se enviará como un POST JSON a esa URL, incluyendo el mismo `task_id` en el cuerpo de la solicitud.
