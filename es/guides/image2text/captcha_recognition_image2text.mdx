...
Luego, necesitamos convertir la imagen del CAPTCHA en una imagen codificada en Base64. Se recomienda utilizar la extensión de Google Chrome FeHelper para realizar la conversión; puedes consultar la siguiente imagen para más detalles:
...
- `elapsed`: el tiempo total de procesamiento de esta tarea (en segundos).

Como puedes ver, hemos obtenido el resultado de la verificación del CAPTCHA alfanumérico de longitud variable, solo necesitamos verificar el contenido textual en el campo `text` para validar.
...
Si deseas verificar el progreso activamente, puedes usar el `task_id` para consultar `POST /captcha/tasks` (se recomienda cada 3 a 5 segundos). Esta interfaz no activará ni avanzará el procesamiento de la tarea; incluso si no consultas, pierdes la conexión o cierras el cliente, el servidor seguirá procesando:
