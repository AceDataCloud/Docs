...
Este documento presentará una descripción de la integración de la API de reconocimiento de CAPTCHA alfanumérico, que se basa en tecnología de aprendizaje profundo y puede utilizarse para reconocer CAPTCHA alfanuméricos de longitud variable. Se ingresa el contenido de la imagen del CAPTCHA y se obtiene el resultado del CAPTCHA.
...
Luego, necesitamos convertir la imagen del CAPTCHA a una codificación Base64. Se recomienda utilizar la extensión de Google Chrome FeHelper para realizar la conversión; puedes consultar la siguiente imagen para más detalles:
...
Después, puedes copiar la codificación Base64 obtenida de la extensión FeHelper, recuerda que no debe incluir el prefijo data:image/png;base64, el contenido específico es el siguiente:
...
- `started_at`, `finished_at`: la hora de inicio y finalización del procesamiento de esta solicitud y la producción del resultado (ISO-8601 UTC).
- `elapsed`: el tiempo total de procesamiento (en segundos).

Como puedes ver, hemos obtenido el resultado de la verificación del CAPTCHA alfanumérico de longitud variable, solo necesitamos verificar el contenido textual en `text` para pasar la verificación.
...
Descripción de facturación: en modo asíncrono, la creación de tareas y el polling "en procesamiento" no generan costos; **solo se cobrará una vez al obtener con éxito el resultado de reconocimiento** (el mismo precio que en modo sincrónico). Por lo tanto, cancelar tareas que aún no se han completado durante la rotación no generará costos. `/captcha/tasks` es común para todas las interfaces de CAPTCHA (token y serie de reconocimiento), puedes hacer polling con el mismo `task_id`.
