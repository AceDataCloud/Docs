...
- `created`: hora de creación de la tarea de diálogo actual (timestamp Unix, en segundos).
...
Esta interfaz soporta respuestas en streaming (Server-Sent Events), lo cual es muy útil para la integración en páginas web, permitiendo mostrar el contenido palabra por palabra.
...
El resultado de salida es el siguiente (extracto):
```text
...
| `glm-5.1`      | Generación anterior, adecuado para razonamiento complejo, análisis de documentos largos |
...
- `500 api_error`: Error interno del servidor o el model service no está disponible temporalmente.
...
```json
{
  ...
  "error": {
    "code": "api_error",
    "message": "El servicio no está disponible temporalmente, por favor intente de nuevo más tarde."
  }
}
```

Cuando se devuelve `api_error` y el mensaje es `El servicio no está disponible temporalmente, por favor intente de nuevo más tarde.`, generalmente indica que el servicio GLM upstream no está disponible temporalmente, se sugiere reintentar con retroceso exponencial, o cambiar a otro modelo GLM disponible (por ejemplo, cambiar temporalmente de `glm-5.1` a `glm-4.7` o `glm-4.6`).
