...
Para usar la API de GLM Chat Completion, primero dirígete a [Ace Data Cloud Console](https://platform.acedata.cloud/console/applications) para obtener tu API Token, que debes guardar como respaldo.
...
- `model`: elige el modelo GLM que deseas invocar, los modelos actualmente soportados incluyen:
  - `glm-5.1`: el modelo insignia de última generación, con la mayor capacidad integral.
...
A continuación, se muestra un ejemplo de llamada en Python más simple:
...
La descripción de los principales campos del resultado devuelto es la siguiente:
...
Esta interfaz soporta respuestas en streaming (Eventos Enviados por el Servidor), lo cual es muy útil para la integración en páginas web, permitiendo que la página muestre el contenido palabra por palabra.
...
| --------------- | ------------------------------------------ |
| `glm-5.1`      | La capacidad más fuerte, recomendado para razonamiento complejo y análisis de documentos largos |
...
| `glm-3-turbo`  | Tareas generales de generación de texto, en escenarios sensibles al costo |
...
- `500 api_error`: Error interno del servidor o el upstream está temporalmente no disponible.
...
A través de este documento, ha aprendido cómo utilizar la API de GLM Chat Completion para llamar a la serie de modelos GLM de Zhiyu AI, incluyendo llamadas básicas, respuestas en streaming, diálogos múltiples, mensajes del sistema y llamadas a herramientas, entre otros usos típicos. Esperamos que este documento le ayude a integrar y utilizar mejor esta API. Si tiene alguna pregunta, no dude en contactar a nuestro equipo de soporte técnico.
