...
GLM (Modelo de Lenguaje General) es una nueva generación de modelos de lenguaje lanzada por Zhipu AI (智谱 AI / Z.ai), que posee potentes capacidades de comprensión y generación en chino e inglés, destacándose en tareas como escenarios en chino, generación de código, razonamiento y diálogos múltiples. Los nuevos modelos de la generación GLM-5.3, GLM-5.2, GLM-4.7, entre otros, han realizado numerosas optimizaciones en contextos largos, llamadas a herramientas y tareas de código, y pueden aplicarse ampliamente en escenarios como preguntas y respuestas inteligentes, creación de contenido, asistencia en código, y chatbots de servicio al cliente.

Este documento describe principalmente el proceso de uso de la API de GLM Chat Completion, que le permite invocar fácilmente los modelos de la serie GLM a través de una interfaz compatible con OpenAI. Si necesita usar `glm-5.3` en Coze (扣子), consulte el [tutorial de modelos personalizados de Coze](https://platform.acedata.cloud/documents/coze-integration) y seleccione el protocolo de OpenAI Chat Completions.

## Proceso de Solicitud

Para usar la API de GLM Chat Completion, primero dirígete a la [consola de Ace Data Cloud](https://platform.acedata.cloud/console/applications) para obtener tu API Token, que debes guardar como respaldo.
...
- `authorization`: selecciona directamente el Bearer Token en la lista desplegable.
- `model`: elige el modelo GLM que deseas invocar, los modelos actualmente soportados incluyen:
  - `glm-5.3`: el modelo insignia más reciente, soporta 1M de contexto y hasta 128K de salida, adecuado para tareas complejas de razonamiento, código y agentes. El razonamiento está siempre habilitado, y puedes elegir `reasoning_effort` como `low`, `high` o `max`.
...
- `messages`: array de mensajes, cada mensaje incluye `role` y `content`, donde `role` soporta tres roles: `user`, `assistant`, `system`.
...
A través de este documento, ha aprendido cómo utilizar la API de GLM Chat Completion para llamar a los modelos de la serie GLM de Zhiyu AI, incluyendo llamadas básicas, respuestas en streaming, diálogos múltiples, palabras clave del sistema y llamadas a herramientas, entre otros usos típicos. Esperamos que este documento le ayude a integrar y utilizar mejor esta API. Si tiene alguna pregunta, no dude en contactar a nuestro equipo de soporte técnico.
