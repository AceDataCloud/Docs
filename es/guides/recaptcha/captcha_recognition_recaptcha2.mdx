...
Necesitamos hacer clic en la casilla de verificación del código de verificación para que aparezca la imagen del código de verificación. En la imagen anterior, la flecha amarilla señala un texto, que es el valor de `question` que se menciona más adelante. Primero, necesitamos pasar un campo `image`, que es la imagen específica del código de verificación Recaptcha2, la cual está indicada por la flecha roja en la imagen anterior. Además, la imagen debe ser escalada a un tamaño estándar (100x100, 300x300, 450x450) para que el servicio pueda determinar el tipo de imagen. La compresión de la imagen debe ser realizada por ti mismo; este documento recomienda un [sitio de compresión](https://www.photopea.com/), donde puedes ajustar el tamaño y la dimensión de la imagen. El resultado comprimido se muestra a continuación:
...
- `solution`, el resultado de la verificación después de procesar la imagen del código de verificación Recaptcha2.
    ...
    - `objects`, las áreas que cumplen con el contenido reconocido de la imagen del código de verificación Recaptcha2, comenzando desde 0.
...
Se puede observar que las áreas comienzan desde 0. En el resultado, de `objects` obtenemos 2, 5, 7, por lo que solo necesitamos simular clics en estas tres áreas para pasar la verificación.
