...
| `enable_llm`     | boolean   |   ❌    | `false`       | Permite la llamada a la extracción de LLM cuando schema.org no coincide. En páginas sin JSON-LD como Amazon / HN / Greenhouse, debe estar habilitado para obtener resultados tipificados. |

> Cuando la página tiene schema.org JSON-LD, `enable_llm` es ineficaz — el mapeador determinista devuelve resultados directamente, nunca desperdiciará una llamada a LLM. Obtienes resultados tipificados **sin costo**.
...
| `BreadcrumbList`                                                                                           | (anclado en sibling) | Siempre se envía a `structured.schemaOrg.breadcrumbs[]`, no se considera como primary.                                                                                                                                 |
...
- `product` → `description`
- `discussion` → `description` (primeros 280 caracteres del body) / `byline` (igual a author) / `publishedAt` (igual a postedAt)
...
El relleno solo se activa cuando la fuente de datos **no ha llenado** los campos correspondientes — LLM siempre es el último recurso.
...
`headers` **se** almacenan en caché por separado.
...
### 3. Página de recetas de AllRecipes (con nutrición y pasos)
...
- **`enable_llm: true` en páginas con schema.org hit es gratuito.** LLM solo se llama cuando schema.org no tiene primary, por lo que dejarlo activado es seguro.
- **Al depurar, primero mira `rawSignals.hasJsonLd`.** Si es `true` pero `structured.schemaOrg.primary` es `null`, significa que la página usó un tipo `@type` que nuestro mapeador aún no ha cubierto —— abre un issue, lo añadiremos.
- **`structured.llmError` es informativo.** La solicitud sigue siendo exitosa, el resultado heurístico aún se devuelve. Mira `llmError.error` para localizar la razón (tiempo de espera, error de análisis JSON, error de validación Zod).
- **Los `links[]` de páginas que no son artículos no se ordenarán por relevancia.** Solo se limpiarán según "hasta 100 elementos + filtrado de protocolos no válidos".
- **Los aciertos de caché también se facturan.** La caché es para retrasos y proteger el grupo de navegadores, no para ahorrar dinero.
