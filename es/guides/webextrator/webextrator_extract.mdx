...
Si aún no has iniciado sesión o registrado, serás redirigido automáticamente a la página de inicio de sesión que te invita a registrarte e iniciar sesión, y una vez completado, serás devuelto automáticamente a la página actual.
...
| `enable_llm`     | booleano  |    ❌     | `false`        | Permite la llamada a la extracción LLM cuando schema.org no coincide. En páginas sin JSON-LD como Amazon / HN / Greenhouse, debe estar habilitado para obtener resultados tipificados. |
...
| `cached`         | booleano?  | `true` cuando se ha alcanzado la caché.                                                      |
| `cacheStoredAt`  | number?    | Marca de tiempo Unix en milisegundos cuando se escribió por primera vez la entrada de caché. |
...
- `product` → `description`
- `discussion` → `description` (primeros 280 caracteres del body) / `byline` ( = author) / `publishedAt` ( = postedAt)
...
Cuando la tarea se complete, se enviará el sobre completo `POST` a tu `callback_url` (si está configurado). También puedes consultar
...
- **`structured.llmError` es informativo.** La solicitud sigue siendo exitosa, el resultado heurístico sigue siendo devuelto. Mira `llmError.error` para localizar la causa (tiempo de espera, error de análisis JSON, error de validación Zod).
