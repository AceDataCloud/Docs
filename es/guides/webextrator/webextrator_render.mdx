...
| `bypass_cache`      | boolean   |    ❌     | `false`                      | Omite la lectura de caché de Redis (pero aún escribirá el resultado en caché). |
...
| `data.cached`         | boolean?         | `true` si se ha alcanzado la caché.                  |
...
La plataforma `POST` un envelope **idéntico** al modo sincrónico a `callback_url`, `Content-Type: application/json`. Devolver cualquier `2xx` se considera confirmado; `5xx` se reintentará con retroceso exponencial durante aproximadamente 5 minutos.
...
Devuelve inmediatamente `{ "success": true, "task_id": "...", "trace_id": "...", "started_at": "..." }`; 
cuando la tarea se complete, la plataforma enviará un POST con el resultado completo a tu `callback_url`.
...
- **La clave de caché ignora `async`.** Las solicitudes sincrónicas y asincrónicas para la misma URL coinciden con la misma entrada de caché, cambiar entre ellas no invalidará.
...
- **Las `cookies` y `headers` se almacenan en caché por separado.** Personalizar estos dos hará que la primera coincidencia de la misma combinación falle.
- **Las SPA pesadas a menudo superan los 30 segundos predeterminados.** Se recomienda `timeout: 60`, `wait_until: "domcontentloaded"`, `delay: 4`, y combinar con `wait_for_selector` para esperar los elementos que realmente importan.
