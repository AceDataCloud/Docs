...
Todas las interfaces de WebExtrator utilizan autenticación estándar de Bearer Token:
...
| `delay`             | number    |    ❌     | `0`                             | **Tiempo adicional de espera en segundos** después de que se active `wait_until` (para re-renderizado de SPA). |
...
| `bypass_cache`      | boolean   |    ❌     | `false`                         | Omitir la lectura de caché de Redis (pero aún escribirá el resultado en la caché). |
...
| `async`             | boolean   |    ❌     | `false`                         | Establecer en `true` devuelve inmediatamente `task_id`, los resultados se recuperan a través de `callback_url` o la API de Tasks. |
...
| `data.text`           | string           | Instantánea de `document.body.innerText` (si necesitas un cuerpo más limpio, usa Extract). |
