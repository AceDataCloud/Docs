...
Este documento presenta principalmente el proceso de uso de la API de OpenAI Chat Completion, que nos permite utilizar fácilmente la función de conversación de OpenAI ChatGPT.

## Proceso de solicitud

Para usar la API de OpenAI Chat Completion, primero dirígete a [la consola de Ace Data Cloud](https://platform.acedata.cloud/console/applications) para obtener tu token de API, que debes guardar como respaldo.
...
En la primera vez que uses esta interfaz, necesitarás completar al menos tres campos: uno es `authorization`, que puedes seleccionar directamente en la lista desplegable. El otro parámetro es `model`, que es la categoría del modelo de OpenAI ChatGPT que elegimos usar; aquí tenemos principalmente 20 modelos, puedes ver los detalles en los modelos que proporcionamos. El último parámetro es `messages`, que es un array de las preguntas que ingresamos, representa un array que permite subir múltiples preguntas a la vez, cada pregunta contiene `role` y `content`, donde `role` indica el rol del preguntador, y ofrecemos tres identidades: `user`, `assistant`, `system`. El otro `content` es el contenido específico de nuestra pregunta.
...
Esta interfaz también admite respuestas en flujo, lo cual es muy útil para la integración en páginas web, permitiendo que la página muestre el efecto de visualización palabra por palabra.
...
Los modelos gpt-3.5-browsing y gpt-4-browsing son diferentes de otros modelos, ya que pueden realizar búsquedas en línea basadas en las preguntas y devolver los resultados de búsqueda ajustados adecuadamente. Este artículo demostrará la función de búsqueda en línea a través de un ejemplo específico, y luego podrás completar el contenido correspondiente en la interfaz de la API de OpenAI Chat Completion, como se muestra en la imagen:
