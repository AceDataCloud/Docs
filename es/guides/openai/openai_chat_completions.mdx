...
Este documento describe principalmente el proceso de uso de la API de OpenAI Chat Completion, que nos permite utilizar fácilmente la función de conversación de OpenAI ChatGPT.

## Proceso de Solicitud

Para usar la API de OpenAI Chat Completion, primero dirígete a [la consola de Ace Data Cloud](https://platform.acedata.cloud/console/applications) para obtener tu token API, que debes guardar como respaldo.
...
> 📘 Documentación completa: [API de OpenAI Chat Completion →](https://platform.acedata.cloud/documents/openai-chat-completions)
...
En la primera vez que uses esta interfaz, necesitarás completar al menos tres campos: uno es `authorization`, que puedes seleccionar directamente en la lista desplegable. El otro parámetro es `model`, `model` es la categoría del modelo de OpenAI ChatGPT que elegimos usar, aquí tenemos principalmente 20 modelos, puedes ver los detalles en los modelos que proporcionamos. El último parámetro es `messages`, `messages` es un array de las preguntas que ingresamos, que representa múltiples preguntas que se pueden subir simultáneamente, cada pregunta contiene `role` y `content`, donde `role` indica el rol del preguntador, y ofrecemos tres identidades: `user`, `assistant`, `system`. El otro `content` es el contenido específico de nuestra pregunta.

También puedes notar que a la derecha hay un código de llamada correspondiente generado, puedes copiar el código y ejecutarlo directamente, o simplemente hacer clic en el botón "Try" para probar.
...
- `usage`: información estadística sobre los tokens utilizados en esta pregunta-respuesta.

Dentro de `choices` se incluye la información de respuesta de ChatGPT, donde se puede observar como se muestra en la imagen.
...
Al subir múltiples preguntas, se puede lograr fácilmente un diálogo en múltiples rondas, obteniendo la siguiente respuesta:
...
Se puede ver que la información contenida en `choices` es consistente con el contenido de uso básico, que incluye el contenido específico de la respuesta de ChatGPT a múltiples diálogos, permitiendo así responder a las preguntas correspondientes basadas en múltiples contenidos de diálogo.

## Integración con OpenAI-Python

La API de OpenAI Chat Completion es compatible con la interfaz oficial de OpenAI y se puede integrar directamente utilizando el SDK oficial [OpenAI-Python](https://github.com/openai/openai-python). Este artículo presentará brevemente cómo usarlo.
...
3. Configura la variable de entorno `OpenAI`.
...
Los modelos gpt-3.5-browsing y gpt-4-browsing son diferentes de otros modelos, ya que pueden realizar búsquedas en línea basadas en las preguntas y devolver los resultados de búsqueda ajustados adecuadamente. Este artículo demostrará la funcionalidad en línea a través de un ejemplo específico, y luego podrás completar el contenido correspondiente en la interfaz de la API de OpenAI Chat Completion, como se muestra en la imagen:

<p><img src="https://cdn.acedata.cloud/249829.png" width="400" className="m-auto" /></p>

También puedes notar que a la derecha hay un código de llamada correspondiente generado, que puedes copiar y ejecutar directamente, o puedes hacer clic en el botón "Probar" para realizar pruebas.
...
Se puede ver que la información de respuesta en `choices` se basa en la consulta en línea y también proporciona enlaces relevantes. La información de respuesta en `choices` debe ser renderizada utilizando la sintaxis de `markdown` para obtener la mejor experiencia, lo que también refleja la poderosa ventaja de la funcionalidad en línea de nuestro modelo.

## Modelo visual

gpt-4o es un modelo de lenguaje multimodal de gran tamaño desarrollado por OpenAI, que ha aumentado la capacidad de comprensión visual sobre la base de GPT-4. Este modelo puede procesar simultáneamente entradas de texto e imagen, logrando una comprensión y generación cruzada de modalidades.
...
Se puede ver que el contenido de la respuesta se basa en la imagen, por lo que a través de las dos formas anteriores se puede utilizar fácilmente la capacidad de procesamiento de texto e imagen del modelo gpt-4-vision.

Además de gpt-4o, hay un modelo de menor costo llamado gpt-4o-mini. gpt-4o-mini es la última generación de modelos de lenguaje a gran escala desarrollada por OpenAI, que no solo tiene una velocidad de respuesta rápida, sino que también es más barato y admite múltiples modalidades. El uso de la función de visión se puede consultar en el contenido de uso del modelo gpt-4o anterior.

## Modelo de dibujo GPT-4o

### Generación de imágenes personalizadas según la imagen de referencia

A continuación se muestra un ejemplo de generación de una imagen de estilo personalizado a partir de una imagen, primero veamos la imagen que ingresamos, como se muestra a continuación:
  ...
  "messages": [
    {
      ...
      "content": [
        {
          "type": "text",
          "text": "Generar una imagen de estilo anime y con un sombrero"
        },
        {
          "type": "image_url",
          "image_url": {
...
Donde `choices` dentro de `message.content` es el resultado completo de la conversación generada, la imagen está incluida en formato Markdown (el enlace de la imagen es una dirección temporal, por favor descárgala y guárdala a tiempo). Se puede ver que la imagen generada es efectivamente de estilo anime, como se muestra a continuación:

<p><img src="https://cdn.acedata.cloud/qmr391.jpg" width="400" class="m-auto"></p>

### Generación de imágenes solo con texto

Podemos generar una imagen a través de una palabra clave y devolverla en un formato de conversación, a continuación tomamos `crear una imagen de una ciudad futura al atardecer` como ejemplo, el ejemplo específico es:
  ...
  "messages": [
    {
      ...
      "content": [
        {
          "type": "text",
          "text": "Crear una imagen de una ciudad futura al atardecer"
        }
      ]
    }
  ],
...
Se puede ver que el resultado es coherente con la palabra clave, como se muestra a continuación:
...
También podemos usar varias imágenes de referencia para generar una imagen, por ejemplo, usando una imagen de un chico guapo y una de café, utilizando estas dos imágenes se puede generar una imagen de un chico guapo bebiendo café, a continuación se muestran las imágenes de referencia específicas:
...
A continuación tomamos `generar una imagen de un chico sosteniendo café y a punto de beberlo` como ejemplo, el ejemplo específico es:
```json
{
  "model": "gpt-4o-image",
  "messages": [
    {
      ...
      "content": [
        {
          "type": "text",
          "text": "Generar un chico sosteniendo un café y que está a punto de beberlo"
        },
        {
          "type": "image_url",
          "image_url": {
...
Se puede ver que el resultado generado efectivamente combina las dos imágenes para crear el resultado, a continuación se muestra el resultado específico:
