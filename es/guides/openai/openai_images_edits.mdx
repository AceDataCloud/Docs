...
El servicio de edición de imágenes de OpenAI permite enviar imágenes y comandos, y devuelve las imágenes modificadas. La serie de modelos GPT Image puede recibir hasta 16 imágenes de referencia al mismo tiempo. Actualmente, la interfaz admite simultáneamente `gpt-image-1`, la más reciente **`gpt-image-2`**, así como los modelos de la serie **`nano-banana` / `nano-banana-2-lite` / `nano-banana-2` / `nano-banana-pro`** que se conectan a través de la misma interfaz.
...
Para utilizar la API de OpenAI Images Edits, primero dirígete a [Ace Data Cloud Console](https://platform.acedata.cloud/console/applications) para obtener tu token de API, que debes guardar para uso futuro.
...
- **La estructura se mantiene más estable**: al cambiar la piel, los colores o el fondo, casi no se altera la disposición y composición de la imagen original.
...
> La interfaz de edición de `gpt-image-2` soporta `n > 1`: una sola solicitud puede devolver y cobrar por la cantidad correspondiente de resultados de edición (`n` puede ser de 1 a 10). Esto también se aplica a `gpt-image-1` / `gpt-image-1.5`, así como a la serie `nano-banana` / `nano-banana-2-lite` / `nano-banana-2` / `nano-banana-pro`. Ten en cuenta que `response_format=b64_json` solo soporta `n=1`, y para `n>1` se debe usar la devolución de URL por defecto. Si algunas de las imágenes generadas fallan, solo se devolverán y cobrarán las que se generaron con éxito.

A continuación, a través de dos ejemplos reales desde diferentes ángulos, experimentaremos la capacidad de edición de `gpt-image-2`.
...
Por ejemplo, la siguiente imagen original es una infografía generada con `gpt-image-2`:
...
curl -X POST "https://api.acedata.cloud/openai/images/edits" \
  ...
  -d '{
    ...
    "prompt": "Convertir esta infografía a modo oscuro: fondo azul marino oscuro, texto crema claro, tarjetas de módulo redondeadas en gris profundo con sombras suaves. Mantener toda la disposición, estructura y arreglo de módulos idénticos — solo invertir el esquema de color.",
    "size": "1024x1536"
  }'
```
...
> **Transmisión directa en base64**: `image` (y cada elemento del arreglo) puede ser también en base64 — `data:image/png;base64,...` o base64 puro, adecuado para imágenes locales que no se desean subir a un servidor de imágenes primero. Por ejemplo:
> ```python
...
Se puede ver que el estilo y el entorno se han reemplazado completamente según las instrucciones, pero la cantidad de libros en cada estante (1 / 3 / 7) se ha mantenido estrictamente, y se ha añadido una planta suculenta como se solicitó.
...
La serie `nano-banana` también se ha integrado en el escenario de edición a través de `/openai/images/edits`, simplemente cambia `model` a cualquiera de los que se muestran en la tabla a continuación.

| Modelo                   | Costo (Créditos / vez) | Escenario aplicable                           |
| -------------------- | --------------- | ------------------------------ |
| `nano-banana`        | 0.14            | Edición de imágenes normal, más rápida y de menor costo               |
...
Así completamos la operación de edición de imágenes, actualmente la interfaz Edits soporta dos modelos: `gpt-image-1` y `gpt-image-2`, donde `gpt-image-2` es el modelo recomendado para su uso, consulte la sección anterior [Modelo GPT-Image-2](#modelo-gpt-image-2) para más detalles.
...
A continuación, podemos establecer el campo `callback_url` en la URL de Webhook anterior, al mismo tiempo que llenamos los parámetros correspondientes, como se muestra en el siguiente código:
```shell
