...
- `type`: Tipo de interfaz de upstream. Las tareas escritas en la serie `gpt-image` (como `gpt-image-2`) son `images`; los canales antiguos de OpenAI (como `gpt-image-1`, nano-banana) utilizan `images_generations` / `images_edits`, y algunas interfaces de chat son `chat_completions_image`.
- `request`: Cuerpo completo de la solicitud original.
- `response`: Cuerpo de respuesta final devuelto por la interfaz de upstream al completar el callback.
...
| `type`           | string   | Filtrar por tipo de upstream (valores: `images`, `images_generations`, `images_edits`) |
...
Se debe proporcionar al menos uno de `ids` / `trace_ids` / `application_id` / `user_id` o un rango de tiempo `created_at_*`.
...
La API de Tasks está diseñada principalmente para flujos asíncronos en modo de callback. En modo de callback, la interfaz de envío **devuelve inmediatamente un `task_id`** (es decir, ID de tarea), después solo necesita usar directamente este `task_id` para sondear la interfaz de Tasks, sin necesidad de generar un `trace_id` por su cuenta.
