...
Se debe proporcionar al menos uno de `id` o `trace_id`. En general, se puede usar directamente el `id` de la respuesta de la solicitud, y `trace_id` solo se pasa si desea asociarlo con un identificador de negocio personalizado.
...
- `type`: Tipo de tarea. Las tareas escritas en la serie `gpt-image` (como `gpt-image-2`) son de tipo `images`; `gpt-image-1`, nano-banana, etc., utilizan `images_generations` / `images_edits`, y algunas interfaces de chat son de tipo `chat_completions_image`.
- `request`: Cuerpo completo de la solicitud original.
- `response`: Cuerpo de respuesta final devuelto al completar el callback.
- `created_at` / `finished_at` / `duration`: Marca de tiempo Unix (segundos) y duración de ejecución (segundos).
- `application_id` / `user_id` / `credential_id`: ID de la aplicación, usuario final y credencial.

## Consulta por Lote (`retrieve_batch`)
...
Se puede pasar uno de `ids` / `trace_ids` / `application_id` / `user_id` o el rango de tiempo `created_at_*`.
...
## Ejemplo de extremo a extremo: enviar y consultar
Tasks API sirve principalmente para procesos asíncronos en modo de callback. En el modo de callback, la interfaz de envío **devuelve inmediatamente un `task_id`** (es decir, ID de tarea), después solo necesita usar directamente este `task_id` para hacer polling en la interfaz de Tasks, sin necesidad de generar un `trace_id` por su cuenta.
...
# 2. Usar directamente el task_id de la respuesta de envío para hacer polling en la interfaz de Tasks, hasta que la tarea esté completa
while True:
    task = requests.post(
        f"{API}/openai/tasks",
        headers=HEADERS,
...
- La interfaz de Tasks **no genera costos**, puede hacer polling sin preocupaciones. Solo las solicitudes originales de generación/edición de imágenes incurrirán en costos.
- Solo cuando la solicitud original incluye `callback_url`, se registrará la tarea; las llamadas síncronas no generarán tareas consultables.
- Los registros de tareas que excedan el período de retención de la plataforma pueden ser eliminados.
