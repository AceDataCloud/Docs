...
> Al pasar `size: "auto"` (por defecto), el modelo **leerá la indicación para decidir el formato**: escribir "banda ultra ancha de cine 21:9" dará como resultado `3024x1296`, escribir "cartel vertical 9:16" dará como resultado `1440x2560`, y si no hay indicaciones de formato en la indicación, volverá a `2048x2048`. Por lo tanto, cuando tengas una composición en mente pero no quieras calcular píxeles específicos, `auto` suele ser más adecuado que especificar un tamaño. Omitir el campo `size` tiene el mismo efecto que `auto`.
> La salida en 1K no garantiza un alineamiento de píxeles estricto: si pasas `1024x1024`, podrías recibir `1254x1254`, manteniendo la proporción. Si lo vuelves a pasar como `size`, el cobro no cambia.
...
En la indicación se pueden usar términos cinematográficos (película de 35 mm, profundidad de campo reducida, luz de neón, etc.) para controlar con precisión la atmósfera y la textura.
...
`gpt-image-2` generalmente requiere de 60 a 90 segundos por llamada, si no se desea mantener una conexión larga, se puede utilizar el mecanismo de callback asíncrono `callback_url` que se describe más adelante en este artículo, el flujo de llamada es completamente consistente con otros modelos.
...
| `nano-banana-2-lite` | 0.14            | Modelo de imagen ligero Gemini 3.1, solo soporta 1K, baja latencia de salida |
...
>   - `1792x1024` → `16:9`
>   - `1024x1792` → `9:16`
> - No se soportan parámetros como `quality`, `style`, `response_format`, `background`, `output_format`, etc.; si se ingresan, serán ignorados. `n > 1` es soportado (1–10), se devolverán y se cobrarán las imágenes correspondientes en cantidad.
...
Solo necesita cambiar `model` a `nano-banana-pro`, los demás parámetros son completamente consistentes:
...
El mecanismo de callback asíncrono `callback_url` también es efectivo para nano-banana, el flujo de llamada es completamente consistente con otros modelos, consulte la sección [Callback asíncrono](#callback-asíncrono) a continuación.
...
El resultado devuelto es consistente con el contenido de uso básico, se puede ver que el enlace de la imagen con el parámetro de formato `url` de la imagen generada es [URL de la imagen](https://dalleprodsec.blob.core.windows.net/private/images/87792c5f-8b6d-412e-81dd-f1a1baa19bd2/generated_00.png?se=2024-07-23T08%3A39%3A47Z&sig=zzRAn30TqIKHdLVqZPUUuSJdjCYpoJdaGU6BeoA76Jo%3D&ske=2024-07-23T13%3A32%3A13Z&skoid=e52d5ed7-0657-4f62-bc12-7e5dbb260a96&sks=b&skt=2024-07-16T13%3A32%3A13Z&sktid=33e01921-4d64-4f8c-a055-5bdaffd5e33d&skv=2020-10-02&sp=r&spr=https&sr=b&sv=2020-10-02) esto es accesible directamente, el contenido de la imagen se muestra a continuación:
