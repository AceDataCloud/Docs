...
Si aún no has iniciado sesión o registrado, serás redirigido automáticamente a la página de inicio de sesión que te invita a registrarte e iniciar sesión, y una vez completado, regresarás automáticamente a la página actual.
...
- **`gpt-image-2:reverse`**: completamente equivalente al `gpt-image-2` por defecto, con una mejor relación calidad-precio, sin cambio de precio.
...
Restricciones de tamaño: el tamaño personalizado debe cumplir que tanto el ancho como la altura sean múltiplos de 16, el lado más largo ≤ 3840, el número total de píxeles ≤ 8,294,400, cualquier excedente devolverá un 4xx.
...
> También puedes pasar `size: "auto"` o **omitir el campo `size`**, en este caso el modelo elegirá el tamaño predeterminado por sí mismo.
...
> `gpt-image-2` soporta `n > 1` (valores de 1 a 10): una sola solicitud puede devolver y cobrar por la cantidad correspondiente de imágenes. Para que los resultados múltiples tengan diferencias, se recomienda pasar diferentes `prompt` o `seed` al mismo tiempo. Esto también se aplica a `gpt-image-1` / `gpt-image-1.5`, así como a la serie `nano-banana` / `nano-banana-2-lite` / `nano-banana-2` / `nano-banana-pro`; `dall-e-2` también lo soporta de forma nativa, `dall-e-3` solo soporta `n = 1`. Ten en cuenta que `response_format=b64_json` solo soporta `n=1`, para `n>1` utiliza la URL predeterminada. Si algunas imágenes fallan en generarse, solo se devolverán y cobrarán las que se generaron con éxito.
...
`gpt-image-2` muestra un rendimiento estable en tipografía y renderizado de texto, siendo muy adecuado para generar diseños con texto como pósters, menús, tarjetas de felicitación, etc.
...
### Asincronía y callback

`gpt-image-2` generalmente requiere de 60 a 90 segundos por llamada, si no se desea mantener una conexión larga, se puede utilizar el mecanismo de callback asíncrono `callback_url` que se describe más adelante en este documento, el flujo de llamada es completamente igual al de otros modelos.

## Modelos de la serie Nano Banana

La serie `nano-banana` es un modelo de generación de imágenes basado en Gemini, que se ha integrado a través de la misma interfaz `/openai/images/generations`, sin necesidad de cambiar el endpoint, solo hay que cambiar `model` a cualquiera de los que se enumeran a continuación.
...
| `nano-banana-2-lite` | 0.14            | Modelo de imagen ligero Gemini 3.1, solo soporta 1K, salida de baja latencia |
...
> Nano Banana se integra al protocolo de OpenAI a través de una capa de adaptación, en comparación con `gpt-image-*` solo soporta los siguientes parámetros: `model`, `prompt`, `size`, `n`.
...
>   - `1792x1024` → `16:9`
>   - `1024x1792` → `9:16`
> - No se soportan parámetros como `quality`, `style`, `response_format`, `background`, `output_format`, etc.; si se incluyen, serán ignorados. `n > 1` es soportado (1–10), se devolverán y se cobrará por la cantidad correspondiente de imágenes.
...
El mecanismo de callback asíncrono `callback_url` es igualmente efectivo para nano-banana, el flujo de llamada es completamente consistente con otros modelos, consulte la sección [Callback asíncrono](#异步回调) a continuación.
...
```json
{
  ...
  "data": [
    {
      "revised_prompt": "Una linda cría de nutria marina está acostada juguetonamente sobre su espalda en el agua, con su piel luciendo brillante y suave. Una de sus pequeñas patas se extiende curiosamente, y tiene una expresión de pura alegría y calidez en su rostro mientras mira al cielo. Su cuerpo está rodeado de burbujas de su jugueteo en el agua. Una suave brisa juega con su piel haciéndola lucir más encantadora. La escena retrata la tranquilidad y el encanto de la vida marina.",
...
Con la misma operación anterior, simplemente configurando el parámetro de calidad de imagen en `hd`, se puede obtener la imagen como se muestra a continuación:
...
También podemos establecer el tamaño de la imagen generada, podemos realizar la siguiente configuración.
```json
{
  "created": 1721637650,
  "data": [
...
Con la misma operación anterior, solo se necesita cambiar el parámetro de formato de enlace de la imagen a `b64_json`, se puede obtener el enlace de la imagen codificado en Base64, el resultado específico se muestra a continuación:
...
Se puede ver que en el resultado hay un campo `task_id`, el campo `data` contiene el mismo resultado de generación de imágenes que en la llamada sincrónica, a través del campo `task_id` se puede lograr la asociación de la tarea.
