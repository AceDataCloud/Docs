...
Tous les interfaces WebExtrator utilisent l'authentification standard Bearer Token :
...
Lorsque `async=true` (ou `callback_url` est fourni), retourne immédiatement (HTTP 200) :
...
Retour immédiat `{ "success": true, "task_id": "...", "trace_id": "...", "started_at": "..." }` ;
Lorsque la tâche est terminée, la plateforme POST les résultats complets à votre `callback_url`.
...
- **Les `cookies` et `headers` seront mis en cache par compartiments.** La personnalisation de ces deux éléments fera échouer la première combinaison identique.
