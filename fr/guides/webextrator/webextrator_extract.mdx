...
```json
{
  ...
  "specifications": [{ "name": "Taille d'affichage", "value": "14,2 pouces" }, "..."]
}
```
...
- **`structured.llmError` est informatif.** La demande réussit toujours, les résultats heuristiques sont toujours renvoyés. Consultez `llmError.error` pour localiser la cause (délai d'attente, échec de l'analyse JSON, échec de la validation Zod).
