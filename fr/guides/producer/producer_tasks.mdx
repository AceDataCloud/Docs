...
Prenons un exemple avec un ID de tâche retourné par le service API Producer Audios Generation. Supposons que nous ayons un ID de tâche : 1a6ac2ad-10f2-4e2b-b500-66ec27fe82ad, nous allons maintenant démontrer comment procéder en passant cet ID de tâche.
...
- `started_at`，date de début d'exécution de la tâche, chaîne de caractères de date ISO-8601 UTC.
...
On peut voir que, sur le côté droit de la page, divers langages de code ont déjà été générés automatiquement, comme indiqué dans l'image :
...
Les résultats contiennent plusieurs champs, où les éléments incluent les détails spécifiques des tâches en lot, chaque tâche ayant des informations spécifiques au format des résultats de la requête d'une seule tâche ci-dessus, les champs d'information sont les suivants.
