...
- `model` : choisissez le modèle GLM à appeler, les modèles actuellement pris en charge incluent :
  - `glm-5.1` : le modèle phare de dernière génération, avec la capacité la plus forte.
  - `glm-4.7` : excellent en raisonnement, appels d'outils et tâches de code.
  - `glm-4.6` : modèle de dialogue général, équilibrant efficacité et coût.
  - `glm-3-turbo` : modèle de dialogue classique, adapté aux tâches de génération de texte générales.
...
Les principales explications des champs de résultat retournés sont les suivantes :

- `id` : ID unique de la tâche de dialogue actuelle.
- `created` : heure de création de la tâche de dialogue actuelle (timestamp Unix, en secondes).
...
Le modèle GLM prend en charge l'appel de fonctions compatible avec OpenAI, vous pouvez déclarer les fonctions appelables via le paramètre `tools`, le modèle renverra des informations structurées sur l'appel de fonction dans `choices[i].message.tool_calls` lorsque nécessaire.
...
| --------------- | ------------------------------------------- |
| `glm-5.1`      | Capacité globale la plus forte, recommandé pour le raisonnement complexe, l'analyse de documents longs |
