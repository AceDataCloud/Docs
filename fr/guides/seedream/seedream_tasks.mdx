...
Prenons un exemple avec un ID de tâche retourné par le service API SeeDream Images Generation. Supposons que nous ayons un ID de tâche : 20068983-0cc9-4c6a-aeb6-9c6a3c668be0, nous allons maintenant démontrer comment procéder en passant cet ID de tâche.
...
Vous pouvez constater que diverses langues de code ont déjà été générées automatiquement sur le côté droit, comme illustré dans l'image :
...
Retourne les résultats avec plusieurs champs, où `items` contient les détails spécifiques des tâches en lot, chaque tâche ayant les mêmes informations de champ que le résultat de la requête d'une seule tâche ci-dessus.
...
Chaque objet de tâche dans le résultat en lot contient également les champs de chronométrage suivants.
...
- `finished_at`, heure de fin de la tâche, horodatage Unix (secondes, flottant). Ce champ n'est pas retourné si la tâche n'est pas terminée.
- `elapsed`, temps d'exécution de la tâche, en secondes (flottant, conservé à 3 décimales). Ce champ n'est pas retourné si la tâche n'est pas terminée.
