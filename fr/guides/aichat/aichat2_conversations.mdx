...
- **Appels d'outils agentisés** : un ensemble d'outils intégrés pour la recherche en ligne, le scraping web, la lecture de fichiers, etc., et la possibilité de monter des serveurs MCP autorisés par l'utilisateur (Google Drive, Notion, Slack, GitHub, etc.), permettant au modèle d'appeler de manière autonome des outils en plusieurs tours dans une seule requête pour accomplir des tâches complexes.
- **Événements structurés en flux** : via `accept: text/event-stream` ou `application/x-ndjson`, il est possible d'obtenir des événements tels que `text_delta`, `tool_use`, `tool_result`, `thinking`, `citation`, `card`, `artifact`, etc., token par token, facilitant le rendu en front-end par type correspondant.
- **Interruption / Reprise** : le modèle émettra un événement `ask_user_question` et se mettra en pause lorsqu'il a besoin d'informations supplémentaires de l'utilisateur, permettant de continuer la prochaine fois en remplissant la réponse via `tool_results`.
...
```json
{
  "answer": "AceDataCloud est une plateforme API unifiée qui regroupe des modèles AI mainstream et des services multimodaux, permettant aux développeurs d'accéder à GPT, Claude, Gemini, Midjourney, Suno, Veo et d'autres services avec une seule clé.",
...
Les valeurs de `model` disponibles peuvent être directement vues dans le menu déroulant du panneau d'essai à droite, les catégories courantes incluent :
...
Les règles de facturation spécifiques peuvent être consultées dans la carte de tarification de la page de service.
...
| `compact`           | Événements dont le contexte interne a été compressé, sans besoin de traitement spécial.                                                                             |
...
- Les suffixes d'URL sont `jpg / jpeg / png / gif / bmp / webp / svg / heic / heif`, se convertissant automatiquement en bloc `image_url` ;
- Les autres extensions se convertissent en bloc `file_url` ;
- Si une `question` est également fournie, elle sera placée en tant que bloc `text` en préfixe.
...
- L'utilisateur demande « Lis ce PDF puis écris un résumé » → le modèle appelle file_read → écrit un résumé.
...
```json
{"type":"tool_use","tool_id":"toolu_01ABCDEF","tool_name":"web_search","input":{"query":"Shanghai 2026 Exposition de printemps"},"id":"f2f4b3e8-..."}
...
Si vous ne souhaitez pas afficher les détails des appels d'outils sur le front-end, ignorez les événements `tool_use` / `tool_result` / `card` / `citation`, la sortie finale du modèle passera toujours par `text_delta`.
...
Certains outils amènent le modèle à « poser une question à l'utilisateur », à ce moment-là, le modèle émettra un événement `ask_user_question`, la conversation étant gelée dans l'état `awaiting_user_input` :
...
Le `tool_use_id` dans le corps de la requête **doit** être exactement identique au `tool_id` lors de la suspension ; toute incohérence renverra une erreur 400. Lorsque `tool_results` est présent dans la requête, `question` / `message` / `references` seront tous ignorés.
...
2. Si vous avez précédemment utilisé des noms de modèles v1 (comme `gpt-3.5`, `gpt-4-browsing`, etc.), il est conseillé de passer à des modèles contemporains (comme `gpt-5.4`, `claude-opus-4-8`, `gemini-3.1-pro`, etc.) lors de la transition vers v2.
...
Après la migration, vous pouvez activer les nouvelles capacités de v2 selon vos besoins (message multimodal, SSE, appels d'outils, CRUD `action`), à votre rythme.
