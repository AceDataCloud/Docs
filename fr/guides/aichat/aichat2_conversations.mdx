...
- **Entrée utilisateur multimodale** : en passant directement des textes + images + fichiers via le champ structuré `message`, sans avoir besoin d'ajouter indirectement avec `references`.
- **Appels d'outils agentisés** : intégration d'un ensemble d'outils de recherche en ligne, de capture de pages web, de lecture de fichiers, etc., et possibilité de monter des serveurs MCP autorisés par l'utilisateur (Google Drive, Notion, Slack, GitHub, etc.), le modèle pouvant appeler de manière autonome des outils en plusieurs tours dans une seule requête pour accomplir des tâches complexes.
...
- **Interruption / Reprise** : le modèle émettra un événement `ask_user_question` et se mettra en pause lorsqu'il a besoin d'informations supplémentaires de l'utilisateur, la prochaine invocation pouvant reprendre en remplissant la réponse via `tool_results`.
...
```json
{
  "answer": "AceDataCloud est une plateforme API unifiée qui agrège des modèles AI mainstream et des services multimodaux, permettant aux développeurs d'appeler des services tels que GPT, Claude, Gemini, Midjourney, Suno, Veo, etc. avec une seule clé.",
...
Deuxième requête, en apportant le même `id` :
```shell
curl -X POST 'https://api.acedata.cloud/aichat2/conversations' \
  -H 'accept: application/json' \
  -H 'authorization: Bearer {token}' \
...
| `system_message`    | Informations de message système (non contenu utilisateur-assistant), uniquement utilisées pour les indications UI.                                                                     |
...
- Les suffixes d'URL sont `jpg / jpeg / png / gif / bmp / webp / svg / heic / heif`, se convertissent automatiquement en bloc `image_url` ;
...
```json
{"type":"tool_use","tool_id":"toolu_01ABCDEF","tool_name":"web_search","input":{"query":"Shanghai 2026 printemps exposition"},"id":"f2f4b3e8-..."}
...
Certains outils amènent le modèle à « poser une question à l'utilisateur », le modèle émet alors un événement `ask_user_question`, la conversation est gelée dans l'état `awaiting_user_input` :
...
Si l'utilisateur décide d'abandonner cette question, il suffit de transmettre une nouvelle `question` / `message`, la plateforme marquera automatiquement l'appel d'outil suspendu comme « utilisateur sauté ».
