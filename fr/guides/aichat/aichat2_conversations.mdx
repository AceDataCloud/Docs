...
- **Entrée utilisateur multimodale** : en passant directement des blocs de texte + image + fichier via le champ structuré `message`, sans avoir besoin de les ajouter indirectement avec `references`.
...
Comme avec la v1, passez `stateful: true` pour activer la sauvegarde de session, l'API renverra un `id` ; les requêtes suivantes doivent inclure cet `id` pour continuer la conversation, sans avoir à gérer l'historique des messages vous-même.
...
Dans le flux NDJSON / SSE, les appels d'outils sont présentés par les événements de type `tool_use` et `tool_result`, par exemple :
...
Vous pouvez ensuite utiliser `action: retrieve` + `id` pour interroger les résultats de la conversation ; vous pouvez également fournir `callback_url`, après l'achèvement de la tâche, la plateforme enverra `{ status, answer, usage, error }` en POST à votre adresse de rappel. `callback_url` doit utiliser `http` / `https`, et ne peut pas être directement rempli avec `localhost` ou une adresse IP privée littérale.
...
Sur le front-end, ce type d'événement est rendu sous forme de carte pour que l'utilisateur choisisse une réponse, puis avec le même `id`, vous lancez la prochaine requête, en renvoyant la réponse via `tool_results` :
