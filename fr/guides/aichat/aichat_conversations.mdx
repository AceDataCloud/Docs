...
Pour utiliser l'API de questions-réponses AI, commencez par obtenir votre Token API sur le [tableau de bord Ace Data Cloud](https://platform.acedata.cloud/console/applications) pour le garder en réserve.
...
> 📘 Documentation complète : [API de questions-réponses AI →](https://platform.acedata.cloud/documents/aichat-conversations)

## Utilisation de base

Tout d'abord, comprenez la méthode d'utilisation de base, qui consiste à poser une question et à obtenir une réponse. Il suffit de transmettre un champ `question` et de spécifier le modèle correspondant.

Par exemple, en demandant : « Quel est votre nom ? », nous pouvons ensuite remplir le contenu correspondant sur l'interface, comme illustré ci-dessous :

![](https://cdn.acedata.cloud/xqxda4.png)

Nous pouvons voir ici que nous avons défini les en-têtes de requête, y compris :

- `accept` : le format de réponse souhaité, ici défini comme `application/json`, c'est-à-dire au format JSON.
- `authorization` : la clé d'API, que vous pouvez sélectionner directement après l'avoir demandée.
...
```json
{
  "answer": "Je suis un modèle de langage AI développé par OpenAI et je n'ai pas de nom personnel. Cependant, vous pouvez m'appeler GPT ou simplement Chatbot. Comment puis-je vous aider aujourd'hui ?"
}
```
...
```json
{
  "answer": "Je suis un modèle de langage AI créé par OpenAI et je n'ai pas de nom personnel. Vous pouvez simplement m'appeler OpenAI ou ChatGPT. Comment puis-je vous aider aujourd'hui ?",
  "id": "7cdb293b-2267-4979-a1ec-48d9ad149916"
}
```

Pour la deuxième requête, transmettez le champ `id` retourné par la première requête comme paramètre, tout en maintenant le paramètre `stateful` sur `true`, en demandant « Qu'est-ce que je vous ai demandé tout à l'heure ? », comme illustré ci-dessous :
...
-d '{
  ...
  "stateful": true,
  "id": "7cdb293b-2267-4979-a1ec-48d9ad149916",
  "question": "Qu'est-ce que je vous ai demandé tout à l'heure ?"
}'
```
...
```json
{
  "answer": "Vous m'avez demandé quel est mon nom. En tant que modèle de langage AI, je ne possède pas d'identité personnelle, donc je n'ai pas de nom spécifique. Cependant, vous pouvez me désigner comme OpenAI ou ChatGPT, les noms utilisés pour ce modèle AI. Y a-t-il autre chose que je peux faire pour vous ?",
  "id": "7cdb293b-2267-4979-a1ec-48d9ad149916"
}
```
...
Si vous souhaitez retourner une réponse en flux, vous pouvez modifier le paramètre `accept` dans les en-têtes de requête, en le changeant en `application/x-ndjson`.

La modification est illustrée ci-dessous, mais le code d'appel doit également être modifié en conséquence pour prendre en charge les réponses en flux.

![](https://cdn.acedata.cloud/axt1aa.png)

Après avoir modifié `accept` en `application/x-ndjson`, l'API renverra les données JSON correspondantes ligne par ligne. Au niveau du code, nous devons faire les modifications nécessaires pour obtenir les résultats ligne par ligne.
...
-d '{
  "model": "gpt-4-vision",
  "question": "Combien de pommes dans l'image?",
...
```json
{
  "answer": "Il y a 5 pommes dans l'image."
}
```
