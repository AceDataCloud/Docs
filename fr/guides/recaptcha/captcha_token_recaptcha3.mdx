...
Cet article présentera une méthode d'intégration de l'API d'identification de protocole Recaptcha3, permettant aux utilisateurs de valider sans avoir à reconnaître et cliquer sur les images de captcha Recaptcha3, il leur suffit de soumettre la clé du site Web pour réaliser un décodage automatique en arrière-plan et compléter la validation.

## Processus de demande

Pour utiliser l'API d'identification de protocole Recaptcha3, commencez par obtenir votre jeton API sur le [tableau de bord Ace Data Cloud](https://platform.acedata.cloud/console/applications) pour le garder en réserve.
...
> 📘 Documentation complète : [API d'identification de protocole Recaptcha3 →](https://platform.acedata.cloud/documents/captcha-token-recaptcha3)
...
Ensuite, il faut entrer l'URL du site Web où le captcha doit être traité pour obtenir le résultat traité. Il suffit de transmettre un champ `website_url`, et enfin d'entrer le paramètre `website_key`, qui peut être obtenu ci-dessus, également une chaîne de caractères après l'exécution. Nous pouvons ensuite remplir les informations correspondantes sur l'interface, comme montré dans l'image suivante :
...
Cliquez sur le bouton « Essayer » pour effectuer le test, comme montré dans l'image ci-dessus, et nous avons obtenu le résultat suivant :
```json
{
