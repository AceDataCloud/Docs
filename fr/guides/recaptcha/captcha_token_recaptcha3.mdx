...
Cet article présentera une méthode d'intégration de l'API d'identification des protocoles Recaptcha3, permettant aux utilisateurs de valider sans avoir à reconnaître et cliquer sur les images de captcha Recaptcha3, il suffit de soumettre la clé du site Web pour réaliser un décodage automatique en arrière-plan et compléter la validation.

## Processus de demande

Pour utiliser l'API d'identification des protocoles Recaptcha3, commencez par obtenir votre jeton API sur le [tableau de bord Ace Data Cloud](https://platform.acedata.cloud/console/applications) pour le garder en réserve.
...
Tout d'abord, comprenons la méthode d'utilisation de base. Comparé à Recaptcha2, nous devons passer un paramètre supplémentaire `page_action`, qui doit être obtenu dans le code. L'URL de démonstration de la vitesse du réseau est : `https://recaptcha-demo.appspot.com/recaptcha-v3-request-scores.php`, ci-dessous une méthode pour l'obtenir :

### Méthode rapide :

Ouvrez f12, puis recherchez `.execute(` dans la page Element. Dans la zone encadrée en rouge, nous pouvons voir le paramètre `action`, et en plus de l'exécution, il y a une chaîne de caractères, qui est également le contenu nécessaire ci-dessous, comme illustré dans l'image.

<p><img src="https://cdn.acedata.cloud/imwlto.png" width="500" className="m-auto" /></p>

Ensuite, il faut entrer l'URL du site Web nécessitant le traitement du captcha pour obtenir le résultat traité. Il faut d'abord transmettre un champ `website_url`, puis entrer le paramètre `website_key`, qui peut être obtenu ci-dessus, également une chaîne de caractères après l'exécution. Nous pouvons ensuite remplir les informations correspondantes sur l'interface, comme montré dans l'image :
...
- `page_action` : à obtenir dans le code du site Web nécessitant le captcha.
...
Après sélection, vous pouvez constater que le code correspondant a également été généré à droite, comme illustré dans l'image :
