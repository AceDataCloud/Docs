---
title: "Recaptcha3 API de reconnaissance de protocole Documentation d'intégration"
description: "Recaptcha verification code recognition service API guide - Ace Data Cloud"
---

Cet article présentera une documentation d'intégration pour l'API de reconnaissance de protocole Recaptcha3, qui permet aux utilisateurs de valider sans avoir à reconnaître et cliquer sur les images de captcha Recaptcha3, il suffit de soumettre la clé du site Web pour réaliser un décodage automatique en arrière-plan et compléter la validation.

## Processus de demande

Pour utiliser l'API de reconnaissance de protocole Recaptcha3, commencez par obtenir votre jeton API sur le [tableau de bord Ace Data Cloud](https://platform.acedata.cloud/console/applications) pour le garder en réserve.
...
> 📘 Documentation complète : [API de reconnaissance de protocole Recaptcha3 →](https://platform.acedata.cloud/documents/captcha-token-recaptcha3)

## Utilisation de base

Tout d'abord, comprenons la méthode d'utilisation de base. Comparé à Recaptcha2, nous devons passer un paramètre supplémentaire `page_action`, ce paramètre doit être obtenu dans le code. L'URL de démonstration de la vitesse du réseau est : `https://recaptcha-demo.appspot.com/recaptcha-v3-request-scores.php`, ci-dessous une méthode pour l'obtenir :
...
Ensuite, il est également nécessaire d'entrer l'URL du site Web nécessitant le traitement du captcha pour obtenir le résultat traité. Il faut d'abord transmettre un champ `website_url`, et enfin entrer le paramètre `website_key`, ce contenu peut être obtenu ci-dessus, c'est aussi une chaîne de caractères après l'exécution. Nous pouvons ensuite remplir les contenus correspondants sur l'interface, comme illustré :
...
- `page_action` : à obtenir dans le code du site Web du captcha.
...
Après sélection, vous pouvez constater que le code correspondant a également été généré à droite, comme illustré :

<p><img src="https://cdn.acedata.cloud/3dahem.png" width="500" className="m-auto" /></p>

Cliquez sur le bouton « Essayer » pour effectuer le test, comme montré ci-dessus, ici nous avons obtenu le résultat suivant :
```json
{
...
Explication des frais : en mode asynchrone, la création de tâches et l'interrogation « en cours de traitement » ne sont pas facturées ; **seulement lorsque le résultat est obtenu avec succès, une facturation est effectuée une fois** (au même prix que le mode synchrone). Par conséquent, annuler des tâches non terminées lors de la rotation ne générera pas de frais. `/captcha/tasks` est commun à toutes les interfaces de captcha (séries token et reconnaissance), vous pouvez interroger avec le même `task_id`.
