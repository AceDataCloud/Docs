...
D'après l'analyse de l'image ci-dessus, l'URL de la requête POST est : `https://www.google.com/recaptcha/api2/demo`, nous devons simplement soumettre le paramètre `g-recaptcha-response`, puis nous devons simplement passer le token traité dans les données ci-dessous, le code CURL spécifique pour appeler le token pour la vérification est le suivant :
