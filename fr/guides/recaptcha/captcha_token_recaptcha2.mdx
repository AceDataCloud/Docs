---
title: "Recaptcha2 API de reconnaissance de protocole Documentation d'intégration"
description: "Recaptcha verification code recognition service API guide - Ace Data Cloud"
---

Cet article présentera une documentation d'intégration pour l'API de reconnaissance de protocole Recaptcha2, qui permet aux utilisateurs de valider sans avoir à reconnaître et à cliquer sur les images de captcha Recaptcha2, il leur suffit de soumettre la clé du site Web pour réaliser un décodage automatique en arrière-plan et compléter la validation.

## Processus de demande

Pour utiliser l'API de reconnaissance de protocole Recaptcha2, commencez par obtenir votre jeton API sur le [tableau de bord d'Ace Data Cloud](https://platform.acedata.cloud/console/applications) pour le garder en réserve.
...
- `started_at`, `finished_at` : le temps de début et de fin de cette demande, horodatage Unix (secondes, flottant).
- `elapsed` : le temps total de traitement (secondes).

On peut voir que nous avons obtenu le résultat de la vérification du Recaptcha2, que nous pouvons ensuite utiliser pour POST ou simuler une soumission au site cible, à usage unique, valable 120 secondes, recommandé d'utiliser dans les 60 secondes. Ensuite, nous fournirons un extrait de code Python pour soumettre le token traité au site cible afin de passer le Recaptcha2.
...
- D'abord, effectuez manuellement la vérification, comme illustré ci-dessous :
...
Le code Python correspondant à l'appel de vérification du token est le suivant :
...
Enfin, nous avons réussi à passer la vérification du protocole Recaptcha2.
