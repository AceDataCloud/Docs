---
title: "Recaptcha2 API d'identification d'image Documentation"
description: "Recaptcha verification code recognition service API guide - Ace Data Cloud"
---

Cet article présente une documentation sur l'API d'identification d'image Recaptcha2, qui peut identifier le contenu saisi par l'utilisateur et l'image du captcha Recaptcha2, puis renvoyer les coordonnées des petites images à cliquer pour compléter la vérification.

## Processus de demande

Pour utiliser l'API d'identification d'image Recaptcha2, commencez par obtenir votre token API sur le [tableau de bord Ace Data Cloud](https://platform.acedata.cloud/console/applications) pour le garder en réserve.
...
> 📘 Documentation complète : [API d'identification d'image Recaptcha2 →](https://platform.acedata.cloud/documents/captcha-recognition-recaptcha2)
...
D'après ce qui précède, nous définissons le paramètre `question` comme étant le hydrant correspondant `/m/01pns0`, le contenu spécifique est comme suit :
...
- `solution`, le résultat de vérification après le traitement de l'image du captcha Recaptcha2.
    ...
    - `confidences`, le niveau de confiance des zones d'identification de l'image du captcha Recaptcha2, les zones commencent à partir de 0.
...
Explication des frais : en mode asynchrone, la création de tâches et l'interrogation « en cours de traitement » ne sont pas facturées ; **seulement lorsque le résultat de reconnaissance est obtenu avec succès, une facturation est effectuée une fois** (au même prix que le mode synchrone). Par conséquent, annuler des tâches non terminées lors de la rotation ne générera pas de frais. `/captcha/tasks` est commun à toutes les interfaces de captcha (token et série de reconnaissance), vous pouvez interroger avec le même `task_id`.
