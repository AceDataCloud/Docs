...
- **Nombre d'images** : Optionnel, spécifier 1–4 images via `count`, par défaut 1 image ; en cas d'échec partiel, seules les images réussies seront retournées et facturées
...
payload = {
    ...
    "prompt": (
        ...
        "La scène est illuminée par une douce lumière dorée de l'heure dorée filtrant à travers "
...
- `count` : Nombre d'images demandées à générer ou à éditer, prend en charge 1–4, par défaut 1. En cas d'échec partiel, `data` ne contient que les images réussies.
...
Par exemple, ici nous fournissons une photo de personne et une photo de vêtement, pour faire porter ce vêtement à la personne, nous pouvons transmettre les liens d'images et spécifier l'action comme `edit`, l'URL peut être une URL HTTP, accessible publiquement via le protocole `https` ou `http`, ou peut être une image encodée en Base64, comme `data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAA+gAAAVGCAMAAAA6u2FyAAADAFBMVEXq6uwdHCEeHyMdHS....`
