...
-d '{
  ...
  "name": "Ma Voix",
  "description": "Exemple de voix humaine claire"
}'
```
...
> **Remarque :** Le clonage de voix ne prend en charge que les modèles `chirp-v4-5` et supérieurs (comme `chirp-v4-5`, `chirp-v5`, `chirp-v5-5`), ne prend pas en charge `chirp-v4`.
...
On peut voir que la chanson générée a été chantée avec la voix clonée. Le `persona_id` peut également être utilisé avec l'action `cover` pour reprendre des chansons existantes avec la voix clonée.
