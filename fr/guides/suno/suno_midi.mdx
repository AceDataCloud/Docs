---
title: "Documentation de l'API de génération MIDI Suno"
description: "Suno Music Generation API guide - Ace Data Cloud"
---

SUNO nous permet de faire une seconde création de la musique générée, d'obtenir le MIDI de la musique, ce document explique la méthode d'intégration de l'API associée.

Le paramètre d'entrée principal de cette API est `audio_id`, qui est l'ID de la chanson générée par l'officiel, généralement l'ID de la chanson avec toutes les pistes séparées (correspondant au [génération de chanson](https://platform.acedata.cloud/documents/suno-audios) où le paramètre `action` est `all_stems`). Il est également possible de fournir une adresse de rappel asynchrone `callback_url`.

L'`audio_id` que nous entrons ici est `c65e6ffd-ead3-4926-9c8c-a42ce202946b`.
...
On peut voir que les champs dans `data` contiennent les informations MIDI de la chanson après séparation de la piste complète.
