...
Vous pouvez voir que divers langages de code ont déjà été générés automatiquement sur le côté droit, comme indiqué dans l'image :
...
- `response`, les informations de retour dans la tâche ; en cas de succès, le `cost.amount` est le montant de crédits réellement déduits pour cette synthèse.
...
Retourne le résultat avec plusieurs champs, où les éléments contiennent des informations détaillées sur les tâches en lot, chaque tâche ayant des informations spécifiques identiques aux champs de la requête précédente pour une tâche unique.
