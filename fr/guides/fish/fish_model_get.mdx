...
> Pour la création de timbres, veuillez consulter l’[API Fish Model Create](https://platform.acedata.cloud/documents/fish-model), et pour les requêtes de liste paginée, référez-vous à l’[API Fish Model Query](https://platform.acedata.cloud/documents/fish-model-query).

## Processus de demande

Pour utiliser l’API, vous devez d’abord faire une demande de service correspondante sur la page [Fish Model API](https://platform.acedata.cloud/services/fish). Une fois sur la page, cliquez sur le bouton « Acquire ».
...
- **Paramètre de chemin** : `{id}` correspond à l’`_id` du timbre, retourné lors de la création via l’[API Fish Model Create](https://platform.acedata.cloud/documents/fish-model) ou obtenu via la requête paginée de l’[API Fish Model Query](https://platform.acedata.cloud/documents/fish-model-query).
...
L’`_id` retourné peut être utilisé comme valeur du champ `reference_id` dans la suite de l’[API Fish TTS](https://platform.acedata.cloud/documents/fish-tts) pour synthétiser la voix clonée.
