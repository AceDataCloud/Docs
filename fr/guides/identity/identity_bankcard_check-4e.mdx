---
title: "Documentation de l'API de vérification des quatre éléments de la carte bancaire"
...
Pour utiliser l'API de vérification des quatre éléments de la carte bancaire, commencez par obtenir votre token API sur le [tableau de bord d'Ace Data Cloud](https://platform.acedata.cloud/console/applications) pour le garder en réserve.
...
> 📘 Documentation complète : [API de vérification des quatre éléments de la carte bancaire →](https://platform.acedata.cloud/documents/identity-bankcard-check-4e)

## Utilisation de base

Tout d'abord, comprenez la méthode d'utilisation de base, qui consiste à entrer les informations du numéro de carte bancaire pour obtenir une image du résultat traité. Il suffit de transmettre un champ `bank_card`. Nous pouvons ensuite remplir le contenu correspondant sur l'interface, comme indiqué dans l'image :
...
- `encryption` : optionnel, paramètre de cryptage pour les champs sensibles (si un texte chiffré doit être envoyé).
...
Cliquez sur le bouton « Essayer » pour effectuer un test, comme indiqué dans l'image ci-dessus, et nous avons obtenu le résultat suivant :
...
On peut voir que le numéro de carte bancaire, le nom, le numéro de document d'ouverture de compte et le numéro de téléphone associé de cette carte bancaire sont véridiques et cohérents.
