...
Pour utiliser l'API d'informations d'identité et de vérification de la validité, commencez par obtenir votre API Token sur le [tableau de bord Ace Data Cloud](https://platform.acedata.cloud/console/applications) pour le garder en réserve.
...
> 📘 Documentation complète : [API d'informations d'identité et de vérification de la validité →](https://platform.acedata.cloud/documents/identity-idcard-check-2e)

## Utilisation de base

Tout d'abord, comprenez la méthode d'utilisation de base, qui consiste à entrer le lien de l'image de la carte d'identité pour obtenir le résultat de vérification après traitement. Vous devez d'abord transmettre un champ `name`, puis vous pouvez remplir le contenu correspondant sur l'interface, comme indiqué sur l'image :
...
- `image_url` : lien de l'image du visage à traiter.
- `validity_end` : date d'expiration de la carte d'identité, format : YYYYMMDD, remplacez par "00000000" pour une validité à long terme ;
