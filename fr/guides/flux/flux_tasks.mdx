...
Prenons un exemple avec un ID de tâche retourné par le service Flux Images Generation. Supposons que nous ayons un ID de tâche : 2db0168c-2373-4367-8d9a-9dc778802e8a, nous allons maintenant démontrer comment l'utiliser en passant cet ID de tâche.
...
Vous pouvez voir que divers langages de code ont déjà été générés automatiquement sur le côté droit de la page, comme illustré :
...
Après une demande réussie, l'API renverra les détails spécifiques de toutes les tâches d'image en masse. Par exemple :
```json
{
  "items": [
    {
...
Le résultat retourné contient plusieurs champs, parmi lesquels items contient les détails spécifiques des tâches d’images en lot. Chaque tâche d’image a les mêmes informations de champ que celles décrites précédemment, les champs sont les suivants.

- `items` : tous les détails spécifiques des tâches d’images en lot. C’est un tableau, chaque élément du tableau a le même format que le résultat de la requête d’une tâche unique décrite ci-dessus.
- `count` : le nombre de tâches d’images dans cette requête en lot.
...
- `400 token_mismatched` : Mauvaise requête, possiblement due à des paramètres manquants ou invalides.
- `400 api_not_implemented` : Mauvaise requête, possiblement due à des paramètres manquants ou invalides.
...
Grâce à ce document, vous avez appris comment utiliser l’API FLux Tasks pour interroger les détails spécifiques d’une ou plusieurs tâches d’images en lot. Nous espérons que ce document vous aidera à mieux intégrer et utiliser cette API. Pour toute question, n’hésitez pas à contacter notre équipe de support technique.
