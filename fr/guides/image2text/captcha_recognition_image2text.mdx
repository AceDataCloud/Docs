...
Après cela, vous pouvez copier le code Base64 obtenu à partir de l'extension FeHelper, en veillant à ne pas inclure le préfixe data:image/png;base64. Le contenu spécifique est comme suit :
...
Nous avons obtenu le résultat de vérification du traitement de l'image CAPTCHA numérique en anglais de longueur variable, et nous pouvons passer la vérification simplement en nous basant sur le contenu textuel du champ `text`.

Si vous souhaitez également générer le code d'intégration correspondant, vous pouvez le copier directement, par exemple, le code CURL est comme suit :
...
Pour vérifier activement la progression, vous pouvez utiliser ce `task_id` pour interroger `POST /captcha/tasks` (il est conseillé de le faire toutes les 3 à 5 secondes). Cette interface ne déclenchera ni n'accélérera le traitement des tâches ; même si vous ne vérifiez pas, si vous perdez la connexion ou quittez le client, le serveur continuera à traiter :
