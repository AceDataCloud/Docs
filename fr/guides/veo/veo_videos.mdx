...
Tout d'abord, comprenez la méthode d'utilisation de base, qui consiste à entrer le mot-clé `prompt`, l'action `action`, le tableau d'images de référence `image_urls` et le modèle `model`, afin d'obtenir le résultat traité. Vous devez d'abord transmettre un champ `action`, dont la valeur est `text2video`, qui comprend principalement trois actions : génération de vidéo à partir de texte (`text2video`), génération de vidéo à partir d'image (`image2video`), et obtenir une vidéo en 1080p (`get1080p`). Ensuite, nous devons également entrer le modèle `model`, qui comprend principalement les modèles `veo31-fast`, `veo3`, `veo31`, `veo3-fast` et `veo31-fast-ingredients`, dont le contenu est le suivant :
...
- `model` : le modèle pour générer la vidéo, principalement `veo31-fast`, `veo3`, `veo31`, `veo3-fast` et `veo31-fast-ingredients`.
- `action` : l'action de la tâche de génération de vidéo, qui comprend principalement trois actions : génération de vidéo à partir de texte (`text2video`), génération de vidéo à partir d'image (`image2video`), obtenir une vidéo en 1080p (`get1080p`).
- `image_urls` : lorsque vous choisissez l'action de génération de vidéo à partir d'image `image2video`, vous devez télécharger les liens d'images de référence. `veo31-fast-ingredients` nécessite jusqu'à 3 images (fusion multi-images), les autres modèles nécessitent jusqu'à 2 images (mode première et dernière image).
- `resolution` : choisissez la résolution de la vidéo générée, où le modèle veo31 prend en charge la résolution 4k, les autres modèles ne le prennent pas en charge, tous les modèles prennent en charge 1080p et gif, si cette valeur n'est pas transmise, la résolution par défaut est 720p, principalement divisée en : `1080p`, `gif`, `4k`.
- `prompt` : le mot-clé.
...
| -------------------------- | --------------------------------- | --------------------------------- |
| **veo3-fast**              | Génération de vidéo à partir de texte (sans image)<br />Mode génération de vidéo à partir d'image (avec image)           | **1 image** → Mode première image<br />**2 images** → Mode première et dernière image |
| **veo31-fast**             | Génération de vidéo à partir de texte (sans image)<br />Mode génération de vidéo à partir d'image (avec image)           | **1 image** → Mode première image<br />**2 images** → Mode première et dernière image |
| **veo31-fast-ingredients** | ❌ Génération de vidéo à partir de texte (non pris en charge)<br />✅ **Fusion multi-images obligatoire** (doit transmettre des images) | **1-3 images** → Mode fusion multi-images (maximum 3 images)        |
| **veo3**                   | Génération de vidéo à partir de texte (sans image)<br />Mode génération de vidéo à partir d'image (avec image)           | **1 image** → Mode première image<br />**2 images** → Mode première et dernière image |
| **veo31**                  | Génération de vidéo à partir de texte (sans image)<br />Mode génération de vidéo à partir d'image (avec image)           | **1 image** → Mode première image<br />**2 images** → Mode première et dernière image |
...
1. **Logique générale** :
   - **Pas d'entrée d'image** → Déclenche automatiquement le mode génération de vidéo à partir de texte.
   - **Entrée d'image** → Déclenche le mode génération de vidéo à partir d'image (le comportement spécifique dépend du nombre d'images).
2. **Types de mode de génération de vidéo à partir d'image** :
   - **Mode première image** (1 image) : La première image est fixe à l'image d'entrée.
   - **Mode première et dernière image** (2 images) : La première et la dernière image sont fixes aux images d'entrée.
   - **Mode fusion multi-images** (1-3 images) : Seul `veo31-fast-ingredients` est pris en charge, fusionnant le contenu de plusieurs images pour générer une vidéo.
3. **Classification des modes** :

  - **Mode rapide** : `veo3-fast`, `veo31-fast`, `veo31-fast-ingredients`.
   - **Mode qualité** : `veo3`, `veo31` (génération de qualité supérieure).

---

### ⚠️ Remarques

- **Modèle nécessitant obligatoirement des images** : `veo31-fast-ingredients` doit recevoir des images (1-3 images), sinon il ne pourra pas fonctionner.
...
- `data`，le résultat de la tâche de génération de vidéo à ce moment.
  ...
  - `complete_at`，le temps d'achèvement de la tâche de génération de vidéo à ce moment.
...
Ensuite, nous devons remplir les mots-clés d'extension nécessaires pour personnaliser la génération de la vidéo, ce qui nous permet de spécifier le contenu suivant :

- `model`：le modèle de génération de vidéo, principalement `veo31-fast`、`veo3`、`veo31`、`veo3-fast` et `veo31-fast-ingredients`.
- `image_urls`：lorsque l'on choisit l'action de génération de vidéo `image2video`, il est nécessaire de télécharger les liens d'images de référence.
...
À ce moment, on peut voir que l'ID de la vidéo est :
...
Si vous souhaitez spécifier la génération d'une vidéo Veo de dimensions personnalisées, vous pouvez définir le paramètre `aspect_ratio` sur la taille souhaitée. Ensuite, nous devons remplir les mots-clés d'extension nécessaires pour personnaliser la génération de la vidéo, ce qui nous permet de spécifier le contenu suivant :

- `model`：le modèle de génération de vidéo, principalement `veo31-fast`、`veo3`、`veo31`、`veo3-fast` et `veo31-fast-ingredients`.
- `aspect_ratio`：la taille de la vidéo, actuellement supportée : `16:9`、`16:9`、`3:4`、`4:3`、`1:1`, par défaut `16:9`.
- `translation`：si l'auto-traduction des mots-clés est activée, par défaut `false`.
  Un exemple de remplissage est comme suit :
...
On peut voir que le contenu du résultat est cohérent avec le texte précédent, ce qui permet de réaliser la fonction de génération de vidéo à une taille spécifiée.

## Callback asynchrone

Étant donné que le temps de génération de l'API Veo Videos Generation est relativement long, environ 1 à 2 minutes, si l'API ne répond pas pendant une longue période, la requête HTTP maintiendra la connexion, entraînant une consommation supplémentaire de ressources système. C'est pourquoi cette API propose également un support de callback asynchrone.
