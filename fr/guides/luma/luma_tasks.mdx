...
Pour utiliser l'API Luma Videos Generation, commencez par obtenir votre token API sur le [tableau de bord Ace Data Cloud](https://platform.acedata.cloud/console/applications) pour le garder en réserve.
...
> 📘 Documentation complète : [API Luma Videos Generation →](https://platform.acedata.cloud/documents/luma-videos)

## Exemple de demande

L'API Luma Tasks peut être utilisée pour interroger les résultats de l'API Luma Videos Generation. Pour savoir comment utiliser l'API Luma Videos Generation, veuillez consulter la documentation [API Luma Videos Generation](https://platform.acedata.cloud/documents/luma-videos-integration).
...
Le résultat de la réponse contient plusieurs champs, le champ request est le corps de la demande lors de l'initiation de la tâche, tandis que le champ response est le corps de la réponse retourné après l'achèvement de la tâche, dont les résultats sont cohérents avec ceux de l'API Luma Videos Generation. Les descriptions des champs sont les suivantes.

- `id`, l'ID de la tâche vidéo générée, utilisé pour identifier de manière unique cette tâche de génération vidéo.
- `video_id`, l'identifiant unique de la vidéo dans la tâche vidéo consultée, nécessaire pour toute opération d'extension sur la vidéo lors de la prochaine fois.
