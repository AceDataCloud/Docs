...
- `model` : le modèle pour générer la vidéo.
  ...
  - **Série Seedance 2.0** (supporte les entrées multimodales comme les références faciales / personnages) : `doubao-seedance-2-0-260128` (standard), `doubao-seedance-2-0-fast-260128` (rapide), `doubao-seedance-2-0-mini-260615` (léger). Voir la section ci-dessous « Références faciales et personnages (Seedance 2.0) ».
...
- `resolution` : résolution de sortie, options `480p` / `720p` / `1080p` (le modèle standard 2.0 supporte également `4k` ; les modèles `fast` / `mini` de 2.0 supportent un maximum de `720p`).
- `ratio` : rapport d'aspect, options `16:9` / `4:3` / `1:1` / `3:4` / `9:16` / `21:9` / `adaptive`.
- `duration` : durée de la vidéo (secondes, entier). Les plages varient selon les séries : **Série 1.0 2–12** ; **1.5 Pro 4–12** ; **Série 2.0 4–15**. Les séries 1.5 Pro et 2.0 supportent également `-1` (durée choisie automatiquement par le modèle).
...
- `generate_audio` : si une vidéo avec son doit être générée, `true` / `false`, **seulement `doubao-seedance-1-5-pro-251215` le supporte**.
...
À la fin du mot-clé `content[].text`, vous pouvez passer des paramètres de génération en ajoutant `--parameter value` (ancienne méthode, vérification faible, en cas d'erreur, les valeurs par défaut seront automatiquement utilisées). La liste complète des paramètres est la suivante :
...
| `--frames`         | `frames`           | Nombre de frames vidéo | Entiers satisfaisant 25+4n dans [29, 289] (**uniquement pris en charge par la série 1.0**) |
| `--fps`            | `framespersecond`  | Taux de frames | Prend uniquement en charge `24`                                   |
...
```json
{
  ...
  "content": [
    {
      "type": "text",
      "text": "Une fille tient un renard, le vent souffle dans ses cheveux, on peut entendre le son du vent"
    }
  ],
  "generate_audio": true,
  "ratio": "16:9",
...
## Générer la première image d'une vidéo à partir d'une image
...
## Générer la première et la dernière image d'une vidéo à partir d'images

Pour générer la première et la dernière image d'une vidéo, le paramètre `content` doit d'abord inclure un type `image_url`, et les rôles doivent être respectivement définis comme `first_frame` et `last_frame`, vous pouvez spécifier le contenu suivant :
...
**Série Seedance 2.0** (`doubao-seedance-2-0-260128`, `doubao-seedance-2-0-fast-260128`, `doubao-seedance-2-0-mini-260615`) prend en charge l'entrée de matériaux de référence « **réel / personnage ** » : ajoutez un élément de type `image_url` avec `role` défini comme `reference_image` dans `content`, en utilisant une photo de la personne comme référence, le modèle conservera les caractéristiques d'apparence de cette personne dans la vidéo générée, permettant ainsi de « placer » la même personne dans de nouveaux décors, actions ou angles de caméra.

> 📌 Les photos de personnes réelles seront automatiquement enregistrées par la plateforme comme matériaux de base avant d'être utilisées pour la génération, tout le processus est totalement transparent pour l'appelant : **le format de demande et de réponse reste inchangé**, aucun paramètre supplémentaire n'est requis, seule la première génération prendra quelques secondes supplémentaires pour le traitement des matériaux.
...
- **Exigences pour le matériel audio de référence (`audio_url`)** : format `wav` / `mp3` ; **durée unique de 2 à 15 secondes**, maximum 3 et **durée totale ne dépassant pas 15 secondes** ; chaque fichier ne doit pas dépasser 15 Mo. Dépasser la durée limite entraînera un échec lors du traitement du matériel.
- **Exigences pour le matériel vidéo de référence (`video_url`)** : format `mp4` / `mov` ; **durée unique de 2 à 15 secondes**, maximum 3 et **durée totale ne dépassant pas 15 secondes**.
...
### Exemple 1 : Gros plan en conservant l'apparence de la personne

Transmettez une photo de visage pour que la personne regarde la caméra en souriant et en agitant la main. Le code correspondant :
...
Le résultat est le suivant, la vidéo générée conserve l'apparence de la personne par rapport à la photo de référence :
...
### Exemple 2 : Mettre la même personne dans un tout nouveau décor

La puissance de `reference_image` réside dans le fait de conserver uniquement **l'identité de la personne**, tandis que le décor, les vêtements et les actions sont entièrement déterminés par les mots-clés. Utilisons la même photo de visage pour que la personne porte un manteau beige et marche dans un parc d'automne :
...
```json
{
  ...
  "content": [
    {
      "type": "text",
      "text": "La même femme portant un manteau beige marche à travers un parc d'automne ensoleillé, des feuilles dorées tombant autour d'elle, elle sourit doucement à la caméra, prise de vue cinématographique."
    },
    {
      "type": "image_url",
      "role": "reference_image",
...
Le résultat est le suivant, l'apparence de la personne est conservée, tandis que le décor a changé pour un parc d'automne :
...
Processus global : lorsque le client envoie une demande, il spécifie `callback_url`, l'API renvoie immédiatement une réponse contenant `task_id` ; une fois la tâche terminée, la plateforme envoie les résultats générés sous forme de POST JSON à `callback_url`, les résultats contenant également `task_id` pour permettre l'association.
...
Grâce à ce document, vous avez compris comment utiliser l'API de génération de vidéos SeeDance pour générer des vidéos à l'aide de mots-clés, d'images de référence, ainsi que des références de visages / personnages de Seedance 2.0. Nous espérons que ce document vous aidera à mieux intégrer et utiliser cette API. Si vous avez des questions, n'hésitez pas à contacter notre équipe de support technique.
