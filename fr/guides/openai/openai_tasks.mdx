...
> Les tâches ne seront persistées que si la demande d'image d'origine contient un `callback_url`. Les demandes appelées de manière synchrone (non par rappel) ne seront pas stockées.
...
## Exemple de bout en bout : soumission et interrogation
Tasks API sert principalement à des processus asynchrones en mode de rappel. En mode de rappel, l'interface de soumission renverra **immédiatement un `task_id`** (c'est-à-dire l'ID de la tâche), après quoi vous n'avez qu'à utiliser ce `task_id` pour interroger l'interface Tasks, sans avoir à générer vous-même un `trace_id`.
...
# 1. Soumettre une tâche de génération d'image (mode callback : en incluant callback_url, le task_id sera immédiatement renvoyé)
...
# 2. Utiliser directement le task_id de la réponse de soumission pour interroger l'interface Tasks, jusqu'à ce que la tâche soit terminée
while True:
    task = requests.post(
        f"{API}/openai/tasks",
        headers=HEADERS,
...
- Les enregistrements de tâches ne seront écrits que si la demande originale contient un `callback_url` ; les appels synchrones ne produiront pas de tâches consultables.
