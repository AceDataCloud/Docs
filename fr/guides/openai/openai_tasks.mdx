...
L'API OpenAI Tasks est utilisée pour interroger les résultats des tâches soumises précédemment au biais d'**un mode de rappel** à l'interface d'image d'OpenAI. Lorsque vous ne pouvez pas attendre une réponse HTTP synchrone, ou si vous souhaitez interroger à nouveau la tâche ultérieurement, veuillez utiliser cette interface.

En mode de rappel, **l'interface d'image d'origine renverra immédiatement un `task_id` après avoir traité la demande**. Vous détenez directement ce `task_id` et pouvez l'utiliser pour interroger cette interface lorsque nécessaire, sans avoir à transmettre un `trace_id` personnalisé (sauf si vous souhaitez établir une association avec votre propre identifiant commercial).

> Les tâches ne seront persistées que si la demande d'image d'origine contient un `callback_url`. Les demandes effectuées de manière synchrone (non par rappel) ne seront pas stockées.
...
| ------------ | ------ | --------- | ------------------------------------- |
| `action`     | string | Oui       | Doit être `retrieve`                     |
...
Vous devez transmettre au moins `id` ou `trace_id`. En général, il suffit d'utiliser directement l'`id` de la réponse de soumission, le `trace_id` n'est à transmettre que si vous souhaitez établir une association avec un identifiant commercial personnalisé.
...
- `id` : ID de tâche généré lors de la prise en charge de la demande d'image d'origine.
...
- `type` : Type d'interface en amont. Les tâches écrites par la série `gpt-image` (comme `gpt-image-2`) sont de type `images` ; les anciens canaux OpenAI (comme `gpt-image-1`, nano-banana) utilisent `images_generations` / `images_edits`, et certaines interfaces de chat sont de type `chat_completions_image`.
- `request` : Corps complet de la demande d'origine.
- `response` : Corps de réponse final retourné par l'interface en amont lorsque le rappel est terminé.
- `created_at` / `finished_at` / `duration` : Horodatages Unix (secondes) et durée d'exécution (secondes).
- `application_id` / `user_id` / `credential_id` : ID de l'application, de l'utilisateur final et des identifiants.
...
| ---------------- | -------- | ---------------------------------------------------------- |
| `action`         | string   | Doit être `retrieve_batch`                                   |
...
| `type`           | string   | Filtrer par type en amont (valeurs possibles : `images`, `images_generations`, `images_edits`) |
...
L'API Tasks sert principalement à des processus asynchrones en mode de rappel. En mode de rappel, l'interface de soumission renvoie **immédiatement un `task_id`** (c'est-à-dire l'ID de la tâche), après quoi vous pouvez simplement utiliser ce `task_id` pour interroger l'interface Tasks, sans avoir à générer vous-même un `trace_id`.
...
# 1. Soumettre une tâche de génération d'image (mode callback : il suffit d'inclure callback_url pour renvoyer immédiatement task_id)
...
- L'interface Tasks elle-même **n'entraîne pas de frais**, vous pouvez donc interroger sans souci. Seules les demandes de génération/édition d'images originales seront facturées.
...
- Les enregistrements de tâches dépassant la période de conservation de la plateforme peuvent être supprimés.
