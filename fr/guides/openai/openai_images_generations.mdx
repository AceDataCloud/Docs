...
- **`gpt-image-2:official`** : canal officiel, stable et conforme. Prend en charge de vraies résolutions 2K / 4K, **facturation par image, le prix unitaire est 2 fois celui du `gpt-image-2` par défaut**. Si la ligne n'est pas disponible, une erreur sera directement retournée, sans rétrogradation automatique.
- **`gpt-image-2:reverse`** : équivalent complet au `gpt-image-2` par défaut, avec un meilleur rapport qualité-prix, le prix reste inchangé.

### Valeurs supportées pour `size`

`gpt-image-2` vérifie uniquement le format de `size`, tant qu'il n'est pas `auto` ou une chaîne vide, il doit correspondre à `WIDTHxHEIGHT` (par exemple `1024x1024`, `2048x1152`, `800x600`) ; toute autre forme retournera 400. **Toutes les tailles (1K / 2K / 4K / personnalisées) sont facturées uniformément par image, sans majoration selon la taille.**
...
> En passant explicitement `size: "auto"`, la plateforme planifiera le canevas dans l'espace de rapport continu et déterminera selon les priorités suivantes : pixels ou rapports explicites dans le prompt, normes de nommage (papier / impression / emplacement de plateforme / publicité / appareil / photographie / cinéma), conventions de support, et enfin inférence de composition. Ainsi, en plus des rapports courants `1:1`, `4:5`, `9:16`, `21:9`, il est également possible de conserver des rapports non prédéfinis tels que `1.91:1`, `1.85:1`, `2.39:1`, et le papier ISO `1:√2` ; la taille finale sera automatiquement ajustée aux multiples de 16 et au budget de pixels supporté par le service. Si le jugement automatique n'est pas disponible, il reviendra au format par défaut du modèle, sans bloquer la génération. Si le champ `size` est omis, le format par défaut du modèle sera utilisé ; pour des exigences strictes en matière de pixels, il est toujours conseillé de transmettre directement `WIDTHxHEIGHT`.
...
payload = {
    "model": "gpt-image-2",
    "prompt": "Une affiche de voyage vintage de la Côte Amalfitaine, en Italie. Illustration art déco stylisée de maisons jaunes citron en falaise descendant vers une mer turquoise, avec un petit voilier blanc dans le port. Typographie audacieuse en haut lisant AMALFI et en bas ITALIA 1958. Palette de couleurs limitée : crème, bleu de mer, jaune citron, terre cuite. Légère texture de grain de papier.",
    "size": "1024x1536"
}
```
...
> Nano Banana est intégré via une couche d'adaptation au protocole OpenAI, et par rapport à `gpt-image-*`, il ne prend en charge que les paramètres suivants : `model`, `prompt`, `size`, `n`.
...
Le champ `data` contient les informations spécifiques sur l'image générée par le modèle, où `url` est le lien détaillé vers l'image générée, comme illustré.
...
```
Le résultat retourné est cohérent avec le contenu de base, on peut voir que le paramètre de qualité de l'image est `standard`, l'image générée est montrée ci-dessous :

<p><img src="https://cdn.acedata.cloud/j5v15b.png" width="500" className="m-auto" /></p>

Avec la même opération ci-dessus, il suffit de définir le paramètre de qualité de l'image sur `hd`, on peut obtenir l'image montrée ci-dessous :
...
Le résultat retourné est cohérent avec le contenu de base, on peut voir que la taille de l'image est de `1024 * 1024`, l'image générée est montrée ci-dessous :

<p><img src="https://cdn.acedata.cloud/o4pvvx.png" width="500" className="m-auto" /></p>

Avec la même opération ci-dessus, il suffit de définir la taille de l'image à `1792 * 1024`, on peut obtenir l'image montrée ci-dessous :
...
Avec la même opération ci-dessus, il suffit de définir le paramètre de style d'image sur `natural`, on peut obtenir l'image montrée ci-dessous :

<p><img src="https://cdn.acedata.cloud/q9tqwu.png" width="500" className="m-auto" /></p>

On peut voir que `vivid` génère des images plus vivantes et réalistes par rapport à `natural`.

## Paramètre de format de lien d'image `response_format`

Le dernier paramètre de format de lien d'image `response_format` a également deux types, le premier `b64_json` est un encodage Base64 du lien de l'image, l'autre `url` est un lien d'image ordinaire, qui peut être consulté directement.
...
On peut voir que le résultat contient un champ `task_id`, le champ `data` contient les mêmes résultats de génération d'image que l'appel synchrone, et le champ `task_id` permet d'associer la tâche.
