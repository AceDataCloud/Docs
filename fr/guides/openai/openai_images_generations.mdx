...
`gpt-image-2` vérifie uniquement le format de `size`, tant qu'il n'est pas `auto` ou une chaîne vide, il doit correspondre à `WIDTHxHEIGHT` (par exemple `1024x1024`, `2048x1152`, `800x600`) ; toute autre forme retournera 400. **Tous les formats (1K / 2K / 4K / personnalisé) sont facturés uniformément par image, sans augmentation de prix selon la taille.**

Limites de taille : les dimensions personnalisées doivent être des multiples de 16 pour la largeur et la hauteur, la longueur maximale ≤ 3840, le nombre total de pixels ≤ 8 294 400, tout dépassement sera retourné avec un code 4xx.
...
> En passant `size: "auto"` (par défaut), le modèle **lira le prompt lui-même pour décider du format** : écrire « bannière cinématographique ultra-large 21:9 » donnera `3024x1296`, écrire « affiche verticale 9:16 » donnera `1440x2560`, un prompt sans indication de format reviendra à `2048x2048`. Donc, lorsque vous avez une composition en tête mais ne souhaitez pas calculer des pixels spécifiques, `auto` est généralement plus approprié qu'un format fixe. L'omission du champ `size` a le même effet que `auto`.
> La sortie en 1K ne garantit pas un alignement strict des pixels — vous pouvez demander `1024x1024` et obtenir `1254x1254`, le rapport restant constant. Si vous le renvoyez en tant que `size`, la facturation ne changera pas.
> Un appel unique en 4K nécessite généralement 4 à 8 minutes, il est conseillé de l'utiliser avec le `callback_url` mentionné plus loin pour un retour asynchrone.
...
Dans le prompt, vous pouvez utiliser des termes cinématographiques (film 35 mm, faible profondeur de champ, lumière néon, etc.) pour contrôler précisément l'ambiance et la texture.
...
`gpt-image-2` montre une performance stable en typographie et rendu de police, ce qui le rend très adapté pour générer des affiches, des menus, des cartes de vœux et d'autres conceptions avec du texte.

```python
payload = {
    "model": "gpt-image-2",
    "prompt": "Une affiche de voyage vintage de la Côte Amalfitaine, Italie. Illustration art déco stylisée de maisons jaunes citron en cascade sur une mer turquoise, avec un petit voilier blanc dans le port. La typographie audacieuse en haut indique AMALFI et en bas ITALIA 1958. Palette de couleurs limitée : crème, bleu de mer, jaune citron, terre cuite. Légère texture de grain de papier.",
    "size": "1024x1536"
}
```
...
On peut voir que le modèle a non seulement reproduit avec précision le style visuel de l'affiche Art Déco, mais que le texte du titre `AMALFI` et `ITALIA 1958` a également été rendu de manière claire et correcte.
...
>   - `1792x1024` → `16:9`
>   - `1024x1792` → `9:16`
> - Ne prend pas en charge les paramètres `quality`, `style`, `response_format`, `background`, `output_format`, etc. ; même s'ils sont remplis, ils seront ignorés. `n > 1` est pris en charge (1–10), renverra et facturera le nombre d'images correspondant.
...
```json
{
  ...
  "data": [
    {
      "url": "https://platform.cdn.acedata.cloud/gpt-image/some-image-id.png"
    }
  ]
}
```
...
- `data`, contenant les informations de résultat de génération d'image.
...
On peut voir que la taille de l'image est clairement différente, de plus, il est possible de définir d'autres tailles, pour plus d'informations, veuillez consulter notre documentation sur le site officiel.
...
On peut voir qu'il y a un champ `task_id` dans le résultat, le champ `data` contient les mêmes résultats de génération d'image que l'appel synchrone, et le champ `task_id` permet de lier la tâche.
