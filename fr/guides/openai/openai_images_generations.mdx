...
> `gpt-image-2` prend en charge `n > 1` (valeurs de 1 à 10) : une seule demande peut retourner et facturer le nombre d'images correspondant. Pour que plusieurs résultats aient des différences, il est conseillé de passer simultanément différents `prompt` ou `seed`. Cela s'applique également à `gpt-image-1` / `gpt-image-1.5`, ainsi qu'à la série `nano-banana` / `nano-banana-2-lite` / `nano-banana-2` / `nano-banana-pro` ; `dall-e-2` prend également en charge nativement, `dall-e-3` ne prend en charge que `n = 1`. Notez que `response_format=b64_json` ne prend en charge que `n=1`, pour `n>1`, veuillez utiliser le retour d'URL par défaut. Si certaines images échouent à être générées, seules les parties réussies seront retournées et facturées.

Voici quelques exemples réels sous différents angles pour ressentir directement la capacité de `gpt-image-2`.
...
payload = {
    "model": "gpt-image-2",
    "prompt": "Un portrait cinématographique d'une jeune femme debout dans un magasin de proximité la nuit, éclairée par des enseignes néon roses et cyan à travers la fenêtre. Prise sur film 35 mm, faible profondeur de champ, léger grain, ambiance mélancolique.",
    "size": "1024x1536"
}
...
> - La structure de retour suit le format OpenAI (`data[].url`), mais `created` est fixé à `0`, et ne renverra pas `b64_json`, `revised_prompt` sera toujours égal au `prompt` d'origine.
...
Voici comment définir le paramètre de qualité d'image sur `standard`, les réglages sont comme indiqué sur l'image :
...
Le résultat retourné est cohérent avec le contenu de l'utilisation de base, vous pouvez voir que l'image générée avec le paramètre de qualité `standard` est montrée sur l'image ci-dessous :
...
On peut voir que l'image générée en `hd` a des détails plus fins et une plus grande cohérence par rapport à celle en `standard`.
...
## Conclusion
Grâce à ce document, vous avez compris comment utiliser facilement la fonction de génération d'images de l'API OpenAI Images Generations de l'official OpenAI DALL-E. Nous espérons que ce document vous aidera à mieux intégrer et utiliser cette API. Si vous avez des questions, n'hésitez pas à contacter notre équipe de support technique.
