...
> En passant explicitement `size: "auto"`, la plateforme planifiera le canevas dans l'espace de rapport continu et déterminera selon les priorités suivantes : pixels ou rapports explicites dans le prompt, normes de nommage (papier / impression / emplacement de plateforme / publicité / appareil / photographie / cinéma), pratiques de médium, et enfin inférence de composition. Ainsi, en plus des rapports courants `1:1`, `4:5`, `9:16`, `21:9`, il est également possible de conserver des rapports non prédéfinis tels que `1.91:1`, `1.85:1`, `2.39:1`, et le papier ISO `1:√2` ; la taille finale sera automatiquement ajustée aux multiples de 16 et au budget de pixels supporté par le service. Si l'évaluation automatique n'est pas disponible, elle reviendra à la largeur par défaut du modèle, sans bloquer la génération. Si le champ `size` est omis, la largeur par défaut du modèle sera utilisée ; pour des exigences strictes en matière de pixels, il est toujours conseillé de transmettre directement `WIDTHxHEIGHT`.
> La sortie en dessous de 1K ne garantit pas un alignement strict des pixels — vous pouvez transmettre `1024x1024` et obtenir `1254x1254`, le rapport restant cohérent. Si vous le retransmettez en tant que `size`, la facturation ne changera pas.
...
> `gpt-image-2` prend en charge `n > 1` (valeurs de 1 à 10) : une seule demande peut retourner et facturer le nombre correspondant d'images. Pour que plusieurs résultats aient des différences, il est conseillé de transmettre simultanément différents `prompt` ou `seed`. Cela s'applique également à `gpt-image-1` / `gpt-image-1.5`, ainsi qu'à la série `nano-banana` / `nano-banana-2-lite` / `nano-banana-2` / `nano-banana-pro` ; `dall-e-3` ne prend en charge que `n = 1`. Notez que `response_format=b64_json` ne prend en charge que `n=1`, pour `n>1`, veuillez utiliser le retour d'URL par défaut. Si certaines images échouent à être générées, seules les parties réussies seront retournées et facturées.
...
Les termes cinématographiques peuvent être utilisés dans les invites (film 35 mm, faible profondeur de champ, lumière néon, etc.) pour contrôler précisément l'atmosphère et la texture.
...
payload = {
    "model": "gpt-image-2",
    "prompt": "Un portrait cinématographique d'une jeune femme debout dans un magasin de proximité la nuit, illuminée par des enseignes néon roses et cyan à travers la fenêtre. Tiré sur film 35 mm, faible profondeur de champ, léger grain, ambiance mélancolique.",
    "size": "1024x1536"
}
...
```json
{
  ...
  "data": [
    {
      "revised_prompt": "Un portrait cinématographique d'une jeune femme debout dans un magasin de proximité la nuit, illuminée par des enseignes néon roses et cyan à travers la fenêtre. Tiré sur film 35 mm, faible profondeur de champ, léger grain, ambiance mélancolique.",
...
L'invite suivante est utilisée pour tester la capacité du modèle à suivre des instructions structurées concernant la "quantité" et la "position".
...
On peut voir que le nombre de livres sur les trois étagères (1 / 3 / 7) correspond exactement à l'invite, ce qui était difficile à réaliser de manière stable à l'époque de `dall-e-3`.
...
L'illustration générée en paysage est la suivante :
...
| `nano-banana-2-lite` | 0.14            | Modèle d'image léger Gemini 3.1, prend en charge uniquement 1K, faible latence |
...
Vous pouvez maintenant remplir le contenu correspondant sur l'interface, comme montré sur l'image :
...
- `created`, l'ID de génération de cette image, utilisé pour identifier de manière unique cette tâche.
- `data`, contenant les informations de résultat de la génération d'image.

Parmi `data`, il y a des informations spécifiques sur l'image générée par le modèle, où `url` est le lien détaillé de l'image générée, comme montré sur l'image.
...
Voici comment définir le paramètre de qualité d'image sur `standard`, les réglages spécifiques sont montrés sur l'image ci-dessous :
...
```
Le résultat retourné est cohérent avec le contenu de base, on peut voir que le paramètre de qualité d'image est `standard`, l'image générée est montrée ci-dessous :

<p><img src="https://cdn.acedata.cloud/j5v15b.png" width="500" className="m-auto" /></p>

En effectuant la même opération ci-dessus, il suffit de définir le paramètre de qualité d'image sur `hd`, on peut obtenir l'image montrée ci-dessous :
...
Voici le réglage de la taille de l'image à `1024 * 1024`, les réglages spécifiques sont montrés ci-dessous :

<p><img src="https://cdn.acedata.cloud/dx5rwh.png" width="500" className="m-auto" /></p>

Vous pouvez également remarquer qu'il y a un code d'appel correspondant à droite, vous pouvez copier le code pour l'exécuter directement, ou cliquer directement sur le bouton « Essayer » pour tester.
...
Le résultat retourné est cohérent avec le contenu de base, on peut voir que la taille de l'image est `1024 * 1024`, l'image générée est montrée ci-dessous :

<p><img src="https://cdn.acedata.cloud/o4pvvx.png" width="500" className="m-auto" /></p>

En effectuant la même opération ci-dessus, il suffit de définir la taille de l'image à `1792 * 1024`, on peut obtenir l'image montrée ci-dessous :

![](https://cdn.acedata.cloud/4pilae.png)

On peut voir que la taille de l'image est clairement différente, de plus, il est possible de définir d'autres tailles, pour plus d'informations, veuillez consulter notre documentation officielle.

## Paramètre de style d'image `style`

Le paramètre de style d'image `style` comprend deux paramètres, le premier `vivid` indique que l'image générée est plus vivante, l'autre `natural` indique que l'image générée est plus naturelle.

Voici le réglage du paramètre de style d'image sur `vivid`, les réglages spécifiques sont montrés ci-dessous :
...
Le résultat retourné est cohérent avec le contenu de base, on peut voir que le paramètre de style d'image est `vivid`, l'image générée est montrée ci-dessous :

<p><img src="https://cdn.acedata.cloud/e0rpc3.png" width="500" className="m-auto" /></p>

En effectuant la même opération ci-dessus, il suffit de définir le paramètre de style d'image sur `natural`, on peut obtenir l'image montrée ci-dessous :

<p><img src="https://cdn.acedata.cloud/q9tqwu.png" width="500" className="m-auto" /></p>

On peut voir que `vivid` génère des images plus vivantes et réalistes que `natural`.

## Paramètre de format de lien d'image `response_format`

Le dernier paramètre de format de lien d'image `response_format` a également deux options, la première `b64_json` est un encodage Base64 du lien de l'image, l'autre `url` est un lien d'image ordinaire, qui peut être consulté directement.

Voici le réglage du paramètre de format de lien d'image sur `url`, les réglages spécifiques sont montrés ci-dessous :
...
Le résultat retourné est conforme au contenu de base utilisé, on peut voir que le lien de l'image avec le paramètre de format `url` pour l'image générée est [URL de l'image](https://dalleprodsec.blob.core.windows.net/private/images/87792c5f-8b6d-412e-81dd-f1a1baa19bd2/generated_00.png?se=2024-07-23T08%3A39%3A47Z&sig=zzRAn30TqIKHdLVqZPUUuSJdjCYpoJdaGU6BeoA76Jo%3D&ske=2024-07-23T13%3A32%3A13Z&skoid=e52d5ed7-0657-4f62-bc12-7e5dbb260a96&sks=b&skt=2024-07-16T13%3A32%3A13Z&sktid=33e01921-4d64-4f8c-a055-5bdaffd5e33d&skv=2020-10-02&sp=r&spr=https&sr=b&sv=2020-10-02) c'est accessible directement, le contenu de l'image est illustré ci-dessous :

<p><img src="https://cdn.acedata.cloud/33hs4z.png" width="500" className="m-auto" /></p>

Avec la même opération ci-dessus, il suffit de définir le paramètre de format de lien d'image comme `b64_json`, pour obtenir le lien de l'image encodée en Base64, le résultat spécifique est illustré ci-dessous :
...
On peut voir que le résultat contient un champ `task_id`, le champ `data` contient les mêmes résultats de génération d'image que l'appel synchrone, et le champ `task_id` permet de relier les tâches.
