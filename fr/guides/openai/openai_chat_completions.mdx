...
Pour utiliser l'API de Complétion de Chat OpenAI, commencez par vous rendre sur le [tableau de bord Ace Data Cloud](https://platform.acedata.cloud/console/applications) pour obtenir votre jeton API, à conserver en réserve.
...
- `choices`, les informations de réponse fournies par ChatGPT pour les questions posées.
- `usage` : les informations statistiques sur les tokens pour cette question-réponse.

Parmi ces champs, `choices` contient les informations de réponse de ChatGPT, où l'on peut voir comme indiqué sur l'image.
...
D'autres langages peuvent être modifiés séparément, le principe est le même.
...
gpt-4o est un modèle de langage multimodal de grande taille développé par OpenAI, qui a ajouté des capacités de compréhension visuelle à GPT-4. Ce modèle peut traiter simultanément des entrées textuelles et d'images, réalisant ainsi une compréhension et une génération intermodales.
...
L'utilisation des capacités de traitement d'images du modèle gpt-4o se fait principalement en ajoutant un champ `type` à la base du contenu `content`, ce champ permet de savoir si ce qui est téléchargé est du texte ou une image, permettant ainsi d'utiliser les capacités de traitement d'images du modèle gpt-4o. Nous allons principalement aborder comment appeler cette fonctionnalité en utilisant deux méthodes : Curl et Python.
...
Dans lequel `choices` contient le `message.content` qui est le résultat complet de la conversation générée, l'image est incluse au format Markdown (le lien de l'image est une adresse temporaire, veuillez la télécharger et la sauvegarder rapidement). On peut voir que l'image générée est effectivement de style anime, comme illustré ci-dessous :

<p><img src="https://cdn.acedata.cloud/qmr391.jpg" width="400" className="m-auto" /></p>

### Génération d'images uniquement à partir de texte

Nous pouvons lui faire générer une image à partir d'un mot d'invite et retourner le résultat sous forme de conversation, prenons comme exemple `Créer une image d'une ville futuriste au coucher du soleil`, voici un exemple spécifique :
...
Nous pouvons également utiliser plusieurs images de référence pour générer une image, par exemple, en utilisant une image d'un beau garçon et une image de café, ces deux images peuvent être utilisées pour générer une image d'un beau garçon buvant du café, voici les images de référence spécifiques :
...
Prenons comme exemple `Générer un garçon tenant un café, prêt à boire`, voici un exemple spécifique :
```json
{
  "model": "gpt-4o-image",
  "messages": [
