...
Pour utiliser l'API OpenAI Images Edits, commencez par vous rendre sur le [tableau de bord Ace Data Cloud](https://platform.acedata.cloud/console/applications) pour obtenir votre jeton API, à conserver pour référence.

![](https://cdn.acedata.cloud/dvc3cg.jpg)

Si vous n'êtes pas encore connecté ou inscrit, vous serez automatiquement redirigé vers la page de connexion qui vous invite à vous inscrire et à vous connecter. Une fois terminé, vous serez automatiquement renvoyé à la page actuelle.

**Un seul jeton API suffit pour accéder à tous les services de la plateforme, sans avoir besoin de faire une demande séparée pour chaque service.** La première demande vous donnera un quota gratuit pour une expérience sans frais ; en cas de quota insuffisant, vous pouvez recharger votre solde général sur le [tableau de bord](https://platform.acedata.cloud/console/coin).
...
- **La conservation du texte est plus précise** : les images contenant du texte, comme les infographies, les affiches et les menus, conservent une lisibilité claire après l'édition.
- **Support de l'URL directe** : en plus du téléchargement de fichiers traditionnel `multipart/form-data`, `gpt-image-2` **supporte également l'entrée d'URL d'images au format JSON**, sans avoir besoin de télécharger d'abord l'image localement, ce qui est idéal pour l'intégration dans des pipelines côté serveur.
- **Support de l'entrée base64** : conformément à l'official, le champ `image` peut également accepter directement du base64 (`data:image/png;base64,...` ou base64 brut), permettant d'éditer des images locales sans avoir à les télécharger sur un hébergeur d'images.
- **Support de la redéfinition en haute résolution** : vous pouvez soumettre une image d'origine de 1K et demander une sortie de 2K / 4K via le paramètre `size`, le modèle effectuera simultanément un agrandissement lors de l'édition.
...
> Par exemple : si l'image d'origine est `1024x1024` et que `size` est `2048x2048`, le modèle redessinera selon les instructions d'édition et produira une image 2K ; si `size` est `3840x2160`, il produira une image 4K en mode paysage. Les frais sont identiques pour les trois.
> En passant `auto` (ou en omettant `size`), la sortie **conservera le rapport d'aspect de l'image de référence** — dans l'exemple ci-dessus, l'image d'origine est en 1:1, donc le résultat sera également en 1:1, sans être compressé dans un autre format. Cela diffère de l'interface de génération : l'interface de génération n'a pas d'image de référence, `auto` choisit le format en fonction du sens des mots-clés. Pour changer le format, spécifiez explicitement `size`.

> **À propos du paramètre `n`**
> L'interface d'édition `gpt-image-2` supporte `n > 1` : une seule demande peut renvoyer et facturer le nombre correspondant de résultats d'édition (`n` peut être compris entre 1 et 10). Cela s'applique également à `gpt-image-1` / `gpt-image-1.5`, ainsi qu'aux séries `nano-banana` / `nano-banana-2-lite` / `nano-banana-2` / `nano-banana-pro`. Notez que `response_format=b64_json` ne supporte que `n=1`, pour `n>1`, veuillez utiliser le retour par défaut d'URL. Si certaines images échouent à générer, seules les parties réussies seront renvoyées et facturées.
...
Par exemple, l'image d'origine ci-dessous a été générée avec `gpt-image-2` comme une illustration scientifique :
...
Nous souhaitons la modifier pour obtenir une palette de couleurs en "mode nuit". Nous pouvons l'appeler ainsi :
```shell
...
> **Remarque** : `image[]` peut apparaître plusieurs fois pour télécharger plusieurs images de référence, par exemple `-F "image[]=@a.png" -F "image[]=@b.png"`, la série de modèles GPT Image prend en charge jusqu'à 16 images (chaque image ne dépassant pas 50 Mo, au format png/webp/jpg). Un dépassement du nombre entraînera un retour 400.

<p><img src="https://cdn.acedata.cloud/jw9iwu.png" width="500" className="m-auto" /></p>

Exemple de code d'appel Python avec le même effet d'appel :
...
Tout d'abord, le rappel Webhook est un service capable de recevoir des requêtes HTTP, les développeurs doivent le remplacer par l'URL de leur propre serveur HTTP. Ici, pour des raisons de démonstration, nous utilisons un site Web de démonstration Webhook public https://webhook.site/, en ouvrant ce site, vous obtiendrez une URL Webhook, comme indiqué dans l'image ci-dessous :
