...
- `model`: Wählen Sie das GLM-Modell aus, das Sie aufrufen möchten. Die derzeit unterstützten Modelle sind:
  - `glm-5.1`: Das neueste Flaggschiffmodell mit den stärksten Gesamtfähigkeiten.
...
- `messages`: Array von Eingabemeldungen, jede Nachricht enthält `role` und `content`, `role` unterstützt die drei Rollen `user`, `assistant`, `system`.
...
- `top_p`: Kernstichprobenparameter, der die kumulative Wahrscheinlichkeitsgrenze der Kandidaten-Token steuert.
...
- `stop`: Benutzerdefinierte Stop-Sequenz.
...
Wenn Sie eine Streaming-Antwort zurückgeben möchten, setzen Sie den Parameter `stream` im Anforderungstext auf `true`.
...
| --------------- | ------------------------------------- |
| `glm-5.1`      | Die umfassendste Fähigkeit, empfohlen für komplexe Schlussfolgerungen und lange Dokumentenanalysen |
...
- `429 too_many_requests`: Frequenzbeschränkung ausgelöst, bitte später erneut versuchen.
