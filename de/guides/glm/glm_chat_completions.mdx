...
Dieses Dokument beschreibt hauptsächlich den Ablauf zur Nutzung der GLM Chat Completion API. Damit können Sie die GLM-Modellreihe einfach über eine einheitliche OpenAI-kompatible Schnittstelle aufrufen. Wenn Sie `glm-5.2` in Coze (扣子) verwenden möchten, beachten Sie bitte das [Coze Anpassungsmodell-Tutorial](https://platform.acedata.cloud/documents/coze-integration) und wählen Sie das OpenAI Chat Completions Protokoll.
...
- `model`: Wählen Sie das GLM-Modell aus, das Sie aufrufen möchten. Die derzeit unterstützten Modelle sind:
  - `glm-5.2`: Neuestes Flaggschiffmodell, geeignet für komplexe Schlussfolgerungen, Code- und Agentenaufgaben.
  - `glm-5.1`: Vorgängermodell, starke Gesamtfähigkeiten.
...
- `id`: Eindeutige ID der aktuellen Dialogaufgabe.
- `created`: Erstellungszeit der aktuellen Dialogaufgabe (Unix-Zeitstempel, in Sekunden).
- `model`: Name des tatsächlich aufgerufenen GLM-Modells.
...
- `usage`: Token-Nutzungsstatistik für diese Anfrage, einschließlich `prompt_tokens`, `completion_tokens`, `total_tokens`.
...
- `429 too_many_requests`: Auslösen der Frequenzbeschränkung, bitte später erneut versuchen.
- `500 api_error`: Interner Serverfehler oder upstream vorübergehend nicht verfügbar.
