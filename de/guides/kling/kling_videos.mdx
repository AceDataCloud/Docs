...
Um die Kling Videos Generation API zu nutzen, gehen Sie zuerst zur [Ace Data Cloud Konsole](https://platform.acedata.cloud/console/applications), um Ihr API-Token zu erhalten und für zukünftige Verwendung aufzubewahren.
...
- `model`: Das Modell zur Generierung des Videos, hauptsächlich `kling-v1`, `kling-v1-6`, `kling-v2-master`, `kling-v2-1-master`, `kling-v2-5-turbo`, `kling-v2-6`, `kling-v3`, `kling-v3-omni`, `kling-o1`.
- `mode`: Der Modus zur Generierung des Videos, wählbare Werte sind Standardmodus `std`, Hochgeschwindigkeitsmodus `pro` und nativer 4K-Modus `4k`. Der `4k`-Modus unterstützt nur `kling-v3` und `kling-v3-omni` und ist nicht mit `camera_control` (Kamerasteuerung) kompatibel.
...
- `duration`: Die Videolänge, Einheit Sekunden. `kling-v3` und `kling-v3-omni` unterstützen eine ganzzahlige Dauer von 3-15 Sekunden; `kling-o1` unterstützt nur 5 Sekunden; andere Modelle unterstützen 5 oder 10 Sekunden.
...
- `async`: Optional, wenn auf `true` gesetzt, gibt die Schnittstelle sofort `task_id` zurück, ohne dass `callback_url` bereitgestellt werden muss, und die Ergebnisse können dann über die entsprechende Aufgabenabfrage-Schnittstelle abgefragt werden.
...
> Hinweis: Hier ist die `video_id` die ID des generierten Videos. Wenn Sie nicht wissen, wie man ein Video generiert, können Sie sich auf die grundlegende Nutzung im obigen Text beziehen.
...
- `start_image_url`: Wenn der Bild-zu-Video-Vorgang `image2video` ausgewählt wird, muss ein Link zum Referenzbild des ersten Frames hochgeladen werden.
...
Es ist zu erkennen, dass der Inhalt des Ergebnisses mit dem oben genannten übereinstimmt, was die Erweiterungsfunktion für Videos ermöglicht.

## Omni All-in-One Referenz (Video-Bearbeitung / Referenzvideo / Mehrbildreferenz)

`kling-o1` und `kling-v3-omni` sind zwei unabhängige Modelle, die beide die Fähigkeit zur „All-in-One-Referenz“ unterstützen. Basierend auf der Text-zu-Video-Funktion (`action=text2video`) können zusätzlich Referenzbilder oder Referenzvideos übergeben werden, um **Mehrbildreferenzen, Referenzvideos und die direkte Bearbeitung vorhandener Videos** zu ermöglichen.

**Kernvereinbarung**: Referenzmaterialien müssen im `prompt` in der Form `&lt;&lt;<image_1>>>`, `&lt;&lt;<video_1>>>` (Nummerierung beginnt bei 1) zitiert werden, um auf die entsprechenden Materialien in `image_list` / `video_list` zuzugreifen, damit das Modell diese Referenzen anwendet. Wenn nur Materialien übergeben werden, ohne sie in der Eingabeaufforderung zu zitieren, werden sie ignoriert.

> Sicherheitshinweis: Derzeit ist die API für `element_list` nicht geöffnet. Die IDs der Kling Element Library sind nicht mandantenspezifisch. Bitte verwenden Sie `image_list`, um Hauptreferenzbilder zu übergeben, bis eine mandantenspezifische Element Management API bereitgestellt wird.

Omni-Anfragen unterstützen kein `negative_prompt`, `cfg_scale` oder `camera_control` und können `mode=4k` nicht verwenden. Wenn Referenzvideos enthalten sind, muss `generate_audio` auf `false` gesetzt sein.

### Referenzvideo und Video-Bearbeitung (`video_list`)
`video_list` wird verwendet, um Referenzvideos zu übergeben, und ist das am häufigsten verwendete Szenario dieser Funktion. Die Felder der Array-Elemente sind wie folgt:

- `video_url`: Referenzvideolink, darf nicht leer sein. Maximal 1 MP4/MOV-Video, Dateigröße ≤200MB, Bildrate 24–60fps. `kling-o1` erfordert eine Dauer von 3–10 Sekunden, Breite und Höhe jeweils 700–2160px; `kling-v3-omni` erfordert eine Dauer von 3–15,5 Sekunden, Breite und Höhe jeweils 700–4553px, Gesamtpixel ≤8.294.400, Seitenverhältnis 0,4–2.
- `refer_type`: Referenztyp, optional `base` (Standard, **Basisvideo zum Bearbeiten**, d.h. "direkte Bearbeitung des Videos", Elemente können hinzugefügt, entfernt oder geändert werden, Komposition, Stil, Farbe, Wetter usw. geändert werden) oder `feature` (**Merkmalsreferenz**, Bezug auf Stil / Kameraführung / Fortsetzung der nächsten Aufnahme).
...
-d '{
  ...
  "mode": "std",
  "duration": 5,
  "prompt": "Lass die Figur in <<<image_1>>> in der Szene von <<<image_2>>> stehen, filmische Lichtstimmung",
  "image_list": [
...
Da die von der Kling Videos Generation API erzeugte Zeit relativ lang ist, etwa 1-2 Minuten, wird die HTTP-Anfrage bei längerer Nichtreaktion der API die Verbindung aufrechterhalten, was zu einem zusätzlichen Systemressourcenverbrauch führt. Daher bietet diese API auch Unterstützung für asynchrone Rückrufe.

Der gesamte Ablauf ist: Der Client initiiert die Anfrage und gibt zusätzlich ein `callback_url`-Feld an. Nach der Initiierung der API-Anfrage gibt die API sofort ein Ergebnis zurück, das ein `task_id`-Feld enthält, das die aktuelle Aufgaben-ID darstellt. Wenn die Aufgabe abgeschlossen ist, werden die Ergebnisse des generierten Videos in Form von POST JSON an die vom Client angegebene `callback_url` gesendet, wobei auch das `task_id`-Feld enthalten ist, sodass die Aufgabenergebnisse über die ID verknüpft werden können.

Hier ist ein Beispiel, um zu verstehen, wie man konkret vorgeht.

Zunächst ist der Webhook-Rückruf ein Dienst, der HTTP-Anfragen empfangen kann. Entwickler sollten ihn durch die URL ihres eigenen HTTP-Servers ersetzen. Hier wird zur Demonstration eine öffentliche Webhook-Beispielwebsite https://webhook.site/ verwendet. Wenn Sie diese Website öffnen, erhalten Sie eine Webhook-URL, wie im Bild gezeigt:
...
Warten Sie einen Moment, und Sie können die Ergebnisse des generierten Videos unter `https://webhook.site/624b2c78-6dbd-4618-9d2b-b32eade6d8c3` beobachten, wie im Bild gezeigt:
...
Es ist zu sehen, dass im Ergebnis ein `task_id`-Feld vorhanden ist, und die anderen Felder sind ähnlich wie oben beschrieben. Über dieses Feld kann die Aufgabe verknüpft werden.
