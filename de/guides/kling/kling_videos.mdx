...
Um die Kling Videos Generation API zu nutzen, gehen Sie zuerst zur [Ace Data Cloud Konsole](https://platform.acedata.cloud/console/applications), um Ihr API-Token zu erhalten und es für später aufzubewahren.
...
Wir sehen, dass wir die gewünschten Videoinformationen erhalten haben. Wir müssen nur den Link zur Video-URL aus dem Ergebnis `data` abrufen, um das generierte Kling-Video zu erhalten.
...
> Hinweis: Hier ist die `video_id` die ID des nach der Generierung erstellten Videos. Wenn Sie nicht wissen, wie man ein Video generiert, können Sie sich auf die grundlegende Nutzung im obigen Text beziehen.
...
payload = {
    ...
    "model": "kling-v1",
    "video_id": "030bb06d-98d4-4044-9042-0aa0822e8c8c",
    "prompt": "Weißer Keramikkaffeebecher auf glänzendem Marmorarbeitsplatz mit Morgenlicht durch das Fenster. Die Kamera dreht sich langsam 360 Grad um die Tasse und hält kurz am Henkel an.",
    "duration": 10
}
...
## Omni Alleskönner Referenz (Video-Bearbeitung / Referenzvideo / Mehrbildreferenz)

`kling-o1` und `kling-v3-omni` sind zwei unabhängige Modelle, die beide die Fähigkeit zur „Alleskönner-Referenz“ unterstützen. Basierend auf der Text-zu-Video-Funktion (`action=text2video`) können zusätzlich Referenzbilder oder Referenzvideos übergeben werden, um **Mehrbildreferenzen, Referenzvideos und die direkte Bearbeitung vorhandener Videos** zu ermöglichen.
...
> Sicherheitshinweis: Derzeit ist die API für `element_list` nicht geöffnet. Die IDs der Kling Element Library sind nicht mandantenisoliert. Bitte verwenden Sie `image_list`, um das Hauptreferenzbild zu übergeben, bis eine mandantenisolierte Element Management API bereitgestellt wird.
...
> Hinweis: Wenn sowohl `start_image_url` / `end_image_url` als auch `image_list` übergeben werden, stehen Start- / Endbilder vor `image_list`, was die Zuordnung von `&lt;&lt;<image_N>>>` beeinflussen kann. Es wird empfohlen, eines von beiden zu wählen: Wenn Start- / Endbilder benötigt werden, direkt in `image_list` mit `type` angeben, nicht mit `start_image_url` / `end_image_url` mischen.
...
-d '{
  ...
  "mode": "std",
  "duration": 5,
  "prompt": "Lass die Figuren in <<<image_1>>> in der Szene von <<<image_2>>> stehen, filmische Beleuchtung",
  "image_list": [
...
Zunächst ist der Webhook-Rückruf ein Dienst, der HTTP-Anfragen empfangen kann. Entwickler sollten ihn durch die URL ihres eigenen HTTP-Servers ersetzen. Hier zur Veranschaulichung verwenden wir eine öffentliche Webhook-Beispielwebsite https://webhook.site/, auf der Sie eine Webhook-URL erhalten können, wie im Bild gezeigt:
...
Klicken Sie auf Ausführen, und Sie erhalten sofort ein Ergebnis, wie folgt:
...
Es ist zu sehen, dass das Ergebnis ein `task_id`-Feld enthält, und die anderen Felder sind ähnlich wie oben, sodass die Aufgabe über dieses Feld verknüpft werden kann.
