---
title: "Suno MIDI-Generierung API Integrationsanleitung"
description: "Suno Music Generation API guide - Ace Data Cloud"
---

SUNO ermöglicht es uns, die generierte Musik neu zu gestalten und die MIDI der Musik zu erhalten. Dieses Dokument erklärt die Integrationsmethoden der entsprechenden API.

Der zentrale Eingabeparameter dieser API ist `audio_id`, das ist die offizielle ID des generierten Songs, in der Regel die ID des vollständig separierten Songs (entsprechend dem Parameter `action` in [Songgenerierung](https://platform.acedata.cloud/documents/suno-audios), der auf `all_stems` gesetzt ist). Optional wird auch `callback_url` als asynchrone Rückrufadresse unterstützt.

Hier ist die eingegebene `audio_id`: `c65e6ffd-ead3-4926-9c8c-a42ce202946b`.
...
Es ist zu sehen, dass die Felder in `data` die MIDI-Informationen des Songs nach vollständiger Trennung der Spuren darstellen.
