---
title: "hCaptcha Protokoll Erkennungs-API Integrationsanleitung"
description: "hCaptcha verification code recognition service API guide - Ace Data Cloud"
---

Dieser Artikel beschreibt eine hCaptcha Protokoll Erkennungs-API Integrationsanleitung, die es Benutzern ermöglicht, hCaptcha Verifizierungscodes ohne Erkennung und Auswahl der Bilder zu übermitteln, sondern lediglich durch die Einreichung des Website-Schlüssels die automatische Dekodierung im Hintergrund zu erreichen und die Verifizierung abzuschließen.

## Antragsprozess

Um die hCaptcha Protokoll Erkennungs-API zu verwenden, müssen Sie zunächst im [Ace Data Cloud Dashboard](https://platform.acedata.cloud/console/applications) Ihr API-Token abrufen und für zukünftige Verwendung aufbewahren.
...
- `website_url`: Die URL der Website, die den Verifizierungscode verarbeiten muss.
- `website_key`: Der Website-Schlüsselidentifikator in hCaptcha.
...
Abrechnungsinformationen: Im asynchronen Modus werden die Erstellung von Aufgaben und die Abfrage „in Bearbeitung“ nicht berechnet; **es wird nur einmal berechnet, wenn das Token erfolgreich abgerufen wird** (zum gleichen Preis wie im synchronen Modus). Daher entstehen beim Abbrechen von noch nicht abgeschlossenen Aufgaben in der Rotation keine Kosten. `/captcha/tasks` ist für alle CAPTCHA-Schnittstellen (Token und Erkennungsserien wie hcaptcha, recaptcha2, recaptcha3, recognition/* usw.) allgemein und kann mit derselben `task_id` abgefragt werden.

## Fehlerbehandlung

Wenn beim Aufrufen der API ein Fehler auftritt, gibt die API den entsprechenden Fehlercode und die Fehlermeldung zurück. Zum Beispiel:
