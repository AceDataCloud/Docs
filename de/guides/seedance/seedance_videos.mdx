...
Wir können sehen, dass wir zufriedenstellende Videoinformationen erhalten haben. Wir müssen nur die Video-URL aus `data` verwenden, um das generierte SeeDance-Video abzurufen.
...
Die normale Vollmodalitätsgenerierung kann `omni_reference_task_type` weglassen, auf `auto` setzen oder explizit auf `reference` setzen. Video-Bearbeitung und Verlängerung müssen `reference_video` übergeben:
...
Da die Generierung von SeeDance-Videos über die API länger dauert (ca. 1-2 Minuten), können Sie das asynchrone Modell über das Feld `callback_url` verwenden, um zu vermeiden, dass die HTTP-Verbindung lange blockiert wird.
