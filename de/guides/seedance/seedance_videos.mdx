...
- `duration`: Videolänge (Sekunden, Ganzzahl). Die Bereiche variieren je nach Serie: **1.0-Serie 2–12**; **1.5 Pro 4–12**; **2.0-Serie 4–15**. 1.5 Pro und 2.0-Serie unterstützen zusätzlich `-1` (Länge wird automatisch vom Modell ausgewählt).
...
- `execution_expires_after`: Zeitüberschreitung für die Aufgabe (Sekunden), Bereich 3600–259200.
...
Wir können sehen, dass wir zufriedenstellende Videoinformationen erhalten haben. Wir müssen nur die Video-URL aus dem `data`-Ergebnis abrufen, um das generierte SeeDance-Video zu erhalten.
...
Am Ende der Eingabeaufforderung `content[].text` können Sie durch Hinzufügen von `--parameter value` Generierungsparameter übergeben (alte Methode, schwache Validierung, bei falscher Eingabe wird automatisch der Standardwert verwendet). Die vollständige Parameterliste ist wie folgt:
...
| `--frames`         | `frames`              | Videoanzahl der Frames | [29, 289] ganze Zahlen, die 25+4n erfüllen (**nur 1.0-Serie unterstützt**) |
...
> **Empfohlene Vorgehensweise**: Verwenden Sie direkt die entsprechenden Top-Level-Felder (wie `resolution`, `ratio` usw.) im Request Body für den strengen Validierungsmodus. Bei fehlerhaften Parametern wird eine klare Fehlermeldung zurückgegeben, was die Fehlersuche erleichtert.
...
```json
{
  ...
  "content": [
    {
      "type": "text",
      "text": "Ein Mädchen hält einen Fuchs, der Wind weht ihr Haar, man kann den Klang des Windes hören"
    }
  ],
  "generate_audio": true,
  "ratio": "16:9",
...
Wenn Sie ein Video aus Bildern generieren möchten, muss der `content`-Parameter zunächst einen Eintrag mit `type` als `image_url` enthalten, das `image_url`-Feld muss im Objektformat sein: `{"url": "https://..."}` oder im Base64-Format `{"url": "data:image/png;base64,..."}`.
...
Sie können sehen, dass das generierte Ergebnis ein Video aus Bildern ist, das Ergebnis ist ähnlich wie oben beschrieben.
...
Sie können sehen, dass das generierte Ergebnis ein Charakter-generiertes Video ist, das Ergebnis ist ähnlich wie oben beschrieben.

## Gesicht und Charakterreferenz (Seedance 2.0)

**Seedance 2.0-Serie** (`doubao-seedance-2-0-260128`, `doubao-seedance-2-0-fast-260128`, `doubao-seedance-2-0-mini-260615`) unterstützt die Übergabe von „**echten / Charakter**“ Referenzmaterialien: Fügen Sie im `content` einen Eintrag mit `type` als `image_url` und `role` als `reference_image` hinzu, um ein Personenfoto als Referenz zu verwenden. Das Modell wird im generierten Video **das Aussehen dieser Person beibehalten**, um dieselbe Person in **neue Szenen, Bewegungen oder Aufnahmen** zu setzen.

> 📌 Fotos von echten Personen werden automatisch als zugrunde liegendes Material auf der Plattform registriert und dann zur Generierung verwendet. Der gesamte Prozess ist für den Aufrufer völlig transparent: **Anfrage- und Antwortformat bleiben unverändert**, es sind keine zusätzlichen Parameter erforderlich, nur die erste Generierung benötigt einige Sekunden mehr für die Materialverarbeitung.

Wichtige Punkte zur Verwendung:
- Nur **Seedance 2.0 Serien** Modelle unterstützen `reference_image`; Modelle der Version 1.x verwenden bitte `first_frame` / `last_frame` (erstes und letztes Bild im Video).
- `reference_image` **darf nicht** mit `first_frame` / `last_frame` kombiniert werden, es kann nur eines von beiden verwendet werden.
- Obergrenze für die Anzahl der multimodalen Referenzen: `image_url` maximal **9** Bilder; 2.0 unterstützt auch `audio_url` (mit `role` als `reference_audio`, maximal 3 Einträge) und `video_url` (mit `role` als `reference_video`, maximal 3 Einträge).
- **Anforderungen an Referenzaudio (`audio_url`)**: Format `wav` / `mp3`; **Einzelne Dauer 2~15 Sekunden**, maximal 3 Einträge und **Gesamtdauer nicht mehr als 15 Sekunden**; Einzelne Datei nicht größer als 15 MB. Überschreitet die Dauer die Vorgaben, schlägt die Verarbeitung der Materialien fehl.
...
- Referenzbilder sollten **einzelne, frontal, klar und ungehindert** Fotos sein; je klarer das Gesicht, desto höher die Ähnlichkeit.
...
payload = {
    ...
    "content": [
        {
            "type": "text",
            "text": "Die Frau schaut in die Kamera, lächelt warm und natürlich und winkt mit der Hand, sanfte Studio-Beleuchtung, sanfter Kamerazoom."
        },
        {
            "type": "image_url",
...
Das `task_id` Feld in den Ergebnissen ist identisch mit dem, das bei der Anfrage zurückgegeben wurde, und ermöglicht die Zuordnung der Aufgabe.
