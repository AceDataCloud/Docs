---
title: "Luma Video-Generierung API Integrationsanleitung"
...
Allerdings bietet Luma offiziell keine API an. AceDataCloud stellt eine API für Luma zur Verfügung, die die offizielle Suno-Integration simuliert und es ermöglicht, schnell und einfach die gewünschten Videos zu generieren.
...
Um ein Video zu generieren, können Sie beliebigen Text eingeben. Wenn ich beispielsweise ein Video über Astronauten, die zwischen dem Weltraum und einem Vulkan pendeln, generieren möchte, kann ich `Astronauts shuttle from space to volcano` eingeben, wie im Bild gezeigt:
...
- `async`: Optional, wenn auf `true` gesetzt, gibt die Schnittstelle sofort `task_id` zurück, ohne dass `callback_url` bereitgestellt werden muss. Anschließend kann das Ergebnis über die entsprechende Abfrage-Schnittstelle abgefragt werden.
...
Hier sehen wir, dass wir die relevanten Informationen zu diesem Video erhalten haben, einschließlich Video-ID, Video-Link, Video-Vorschaubild und mehr.
...
- video_height: Die Höhe des generierten Vorschaubildes des Videos
- video_width: Die Breite des generierten Vorschaubildes des Videos
- state: Der Status dieser Video-Generierungsaufgabe, wenn die Aufgabe abgeschlossen ist, dann `completed`.
- thumbnail_url: Der Link zum generierten Vorschaubild des Videos
- thumbnail_width: Die Breite des generierten Vorschaubildes des Videos
- thumbnail_height: Die Höhe des generierten Vorschaubildes des Videos
...
In diesem Fall kann das Startbild im Feld `start_image_url` mit folgendem Bild als Startbild des Videos übergeben werden:
...
- prompt: Die Schlüsselwörter zur Generierung des Videos.
...
Das letztendlich erhaltene Ergebnis ist ähnlich wie das oben, das generierte Video-Startbild enthält das Bild, das wir übergeben haben. Natürlich können auch gleichzeitig Links zu Start- und Endbildern übergeben werden, um ein Video zu generieren. Sie müssen lediglich ein Endbild hinzufügen, das die Bildinformationen wie folgt enthält:
...
Das Ergebnis ist ähnlich wie oben, das generierte Video enthält sowohl das erste als auch das letzte Bild, was die benutzerdefinierte Erstellung von Anfangs- und Endbildern zur Videoerstellung abschließt.
...
Hier können Sie die Video-ID sehen:
...
> Hinweis: Hier sind die `video_id` und `video_url` des Videos nach der Erstellung, wenn Sie nicht wissen, wie man ein Video erstellt, können Sie sich auf die grundlegende Nutzung oben beziehen, um ein Video zu erstellen.
...
- action: In diesem Fall die Aktion zur Erweiterung des Videos, hier sollte es `extend` sein.
...
- end_image_url: Der Link zum Bild des Endbildes, das für die Erweiterung angegeben werden kann, optional.
...
Es ist zu erkennen, dass dieses Video auf dem benötigten erweiterten Video basiert, der Inhalt des Ergebnisses ist identisch mit dem oben, was die Funktion zur weiteren Erstellung von Songs realisiert.

Natürlich können wir auch den Link des Videos angeben, um die Erweiterung zu erstellen, indem wir die folgenden Informationen ausfüllen:

<p><img src="https://cdn.acedata.cloud/0cv0hg.png" width="500" className="m-auto" /></p>

Nach dem Ausführen erhalten Sie das folgende Ergebnis:
...
Anhand des Ergebnisses ist zu erkennen, dass auch die Funktion zur Erweiterung des Videos basierend auf dem Video-Link realisiert werden kann.

Zuletzt können wir auch ein Endbild für das erweiterte Video angeben, um die Erweiterung durchzuführen. Hier sind die Informationen zum Endbild:
...
Wenn Sie auf Ausführen klicken, erhalten Sie die folgenden Informationen:
...
Der gesamte Prozess ist: Wenn der Client eine Anfrage stellt, gibt er zusätzlich ein Feld `callback_url` an. Nachdem der Client die API-Anfrage gestartet hat, gibt die API sofort ein Ergebnis zurück, das ein Feld mit der `task_id`-Information enthält, das die aktuelle Aufgaben-ID darstellt. Wenn die Aufgabe abgeschlossen ist, wird das Ergebnis der Musikgenerierung in Form von POST JSON an die vom Client angegebene `callback_url` gesendet, wobei auch das Feld `task_id` enthalten ist, sodass die Aufgabenergebnisse über die ID miteinander verknüpft werden können.
...
Warten Sie einen Moment, und wir können das Ergebnis des generierten Songs auf https://webhook.site/0c87ca0e-cd74-4577-8d68-f2b80fbf8a13 beobachten, wie im Bild gezeigt:
...
Man kann sehen, dass im Ergebnis ein Feld `task_id` vorhanden ist, und die anderen Felder sind ähnlich wie oben, sodass die Aufgaben über dieses Feld verknüpft werden können.
