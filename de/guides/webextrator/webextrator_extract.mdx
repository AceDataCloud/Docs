...
2. **Typisierte LLM-Extraktion** – wird nur ausgelöst, wenn schema.org nicht zutrifft. Schema nach Seitentyp auswählen, Zod strenge Validierung.
...
`cache_ttl_seconds`, `async`), zusätzlich zu zwei Extract-spezifischen Feldern:
...
> Wenn die Seite bereits schema.org JSON-LD enthält, ist `enable_llm` unwirksam – der deterministische Mapper gibt direkt Ergebnisse zurück,
...
```json
{
  ...
  "data": {
    ...
    "markdown": "# Diffbot\n\nDiffbot ist ein Entwickler von maschinellen Lernalgorithmen ...",
    "text": "Diffbot ist ein Entwickler von maschinellen Lernalgorithmen ...",
    "structured": {
...
| `description`   | string?  | Priorität: `<meta name="description" />` → `og:description` → schema.org / LLM-Extraktion → Textabschnitt.   |
...
| `text`          | string   | Aus `textContent` extrahierter Text von Mozilla Readability。                                               |
| `structured`    | object   | Vollständige strukturierte Ergebnisse, siehe unten.                                                                          |
| `rawSignals`    | object   | Diagnostische Informationen für Debugging.                                                                            |
| `cached`        | boolean? | Bei Cache-Treffer `true`。                                                                       |
...
| `llm`           | Wenn LLM erfolgreich lief | `{ kind, data, model, promptCharCount }`, typisierte Ergebnisse, die von Zod validiert wurden.              |
...
| `amazon`        | Wenn die URL `amazon.*` ist | Ergebnisse des alten, speziellen Amazon-Crawlers (wird schrittweise eingestellt).                                            |
...
Wenn `enable_llm: true` **und** schema.org keinen primären Eintrag hat, extrahiert der Mapper basierend auf URL heuristisch
(oder `expected_type` Hinweis) eines der folgenden Zod Schema Validierungsmodell-Ausgaben:
...
### 1. Wikipedia Artikel (schema.org Treffer, kein LLM benötigt)
...
{
  "kind": "product",
  "name": "Apple - Generalüberholt Exzellent - AirPods Pro (2. Generation) - Weiß",
  "sku": "10845412",
  "model": "MQD83AM/A",
  "color": "Weiß",
  "brand": "Apple",
...
- **Überprüfen Sie zuerst `rawSignals.hasJsonLd` beim Debuggen.** Wenn es `true` ist, aber `structured.schemaOrg.primary` `null` ist, bedeutet das, dass die Seite einen `@type` verwendet hat, den unser Mapper noch nicht abgedeckt hat – eröffnen Sie ein Issue, wir fügen es hinzu.
