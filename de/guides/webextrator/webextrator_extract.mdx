...
| --------------- | ------- | :-: | ------- | --------------------------------------------------------------------------------------- |
| `expected_type` | enum    |  ❌  | Automatische Bestimmung    | Hinweis auf den Seitentyp: `product` / `article` / `general`. Umgeht URL / Textheuristik und geht direkt den entsprechenden Zweig.                        |
| `enable_llm`    | boolean |  ❌  | `false` | Erlaubt die LLM-Extraktion, wenn schema.org nicht zutrifft. Bei Seiten ohne JSON-LD wie Amazon / HN / Greenhouse muss dies aktiviert werden, um typisierte Ergebnisse zu erhalten. |
...
| `amazon`        | Wenn die URL `amazon.*` ist | Ergebnisse des alten, speziellen Amazon-Crawlers (werden schrittweise eingestellt).                                            |
...
Wenn `enable_llm: true` **und** schema.org keinen primären Eintrag hat, extrahiert der Mapper basierend auf der URL heuristisch
...
Wenn die Aufgabe abgeschlossen ist, wird das vollständige Envelope an Ihre `callback_url` `POST` gesendet (wenn konfiguriert). Sie können auch später aktiv über [`/webextrator/tasks`](development_webextrator_tasks) abfragen.
...
- **Beim Debuggen zuerst `rawSignals.hasJsonLd` überprüfen.** Wenn es `true` ist, aber `structured.schemaOrg.primary` `null` ist, bedeutet das, dass die Seite einen `@type` verwendet, den unser Mapper noch nicht abgedeckt hat – bitte ein Issue erstellen, wir fügen es hinzu.
