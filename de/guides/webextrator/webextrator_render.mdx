...
| `timeout`           | number   |  ❌        | `30`                         | Timeout für eine einzelne Navigation (**Sekunden**).                             |
...
Die Plattform sendet ein `POST` mit dem **genau gleichen** Envelope im synchronen Modus an `callback_url`,
`Content-Type: application/json`. Jede Rückgabe von `2xx` wird als bestätigt angesehen; `5xx` wird mit exponentiellem Backoff etwa 5 Minuten lang erneut versucht.
...
- **`cookies` und `headers` werden in separaten Caches gespeichert.** Die Anpassung dieser beiden führt dazu, dass die erste gleiche Kombination fehlschlägt.
- **Reaktive SPAs überschreiten häufig die standardmäßigen 30 Sekunden.** Es wird empfohlen, `timeout: 60`, `wait_until: "domcontentloaded"`, `delay: 4` zu verwenden, zusammen mit `wait_for_selector`, um auf wirklich relevante Elemente zu warten.
