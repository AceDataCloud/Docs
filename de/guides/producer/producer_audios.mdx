...
Wenn Sie einen selbst hochgeladenen Song weiter covern möchten, können Sie den Parameter `action` auf `upload_cover` setzen und die ID des Songs eingeben, den Sie weiter covern möchten. Die Song-ID kann über die [Producer Upload API](https://platform.acedata.cloud/documents/producer-upload) abgerufen werden, wie im folgenden Bild gezeigt:
