...
Dabei enthält `choices` die Antwortinformationen von ChatGPT, und innerhalb von `choices` können wir sehen, wie im Bild gezeigt.
...
Die Modelle gpt-3.5-browsing und gpt-4-browsing unterscheiden sich von anderen Modellen, da sie basierend auf den Fragen eine Online-Suche durchführen können und die Ergebnisse der Online-Suche entsprechend anpassen und zurückgeben. In diesem Artikel wird ein konkretes Beispiel zur Demonstration der Online-Funktionalität gegeben. Sie können dann im OpenAI Chat Completion API-Bereich die entsprechenden Inhalte ausfüllen, wie im Bild gezeigt:

<p><img src="https://cdn.acedata.cloud/249829.png" width="400" className="m-auto" /></p>

Gleichzeitig können Sie auf der rechten Seite den entsprechenden Code zur Aufrufgenerierung sehen, den Sie direkt ausführen oder auf die Schaltfläche „Try“ klicken können, um einen Test durchzuführen.
...
gpt-4o ist ein multimodales großes Sprachmodell, das von OpenAI entwickelt wurde und die visuelle Verständnisfähigkeit auf der Grundlage von GPT-4 hinzugefügt hat. Dieses Modell kann gleichzeitig Text- und Bilddaten verarbeiten und ermöglicht ein Verständnis und eine Generierung über Modalitäten hinweg.

Die Textverarbeitung mit dem gpt-4o-Modell ist mit dem grundlegenden Inhalt der vorherigen Verwendung identisch. Im Folgenden wird kurz erläutert, wie die Bildverarbeitungsfähigkeiten des Modells verwendet werden können.

Die Bildverarbeitungsfähigkeiten des gpt-4o-Modells werden hauptsächlich durch das Hinzufügen eines `type`-Feldes zu den ursprünglichen `content`-Inhalten aktiviert. Anhand dieses Feldes kann festgestellt werden, ob der Upload Text oder ein Bild ist, um die Bildverarbeitungsfähigkeiten des gpt-4o-Modells zu nutzen. Im Folgenden werden hauptsächlich zwei Methoden zur Verwendung dieser Funktion beschrieben: Curl und Python.
...
Man kann sehen, dass das Referenzbild ein Bild einer echten Person ist. Wir können es in einen Stil ändern, zum Beispiel in einen Anime-Stil. Hier ist ein konkretes Beispiel für die Anfrage:
...
Man kann sehen, dass das Ergebnis den Hinweisen entspricht, konkret wie im folgenden Bild gezeigt:
...
Hier verwenden wir als Beispiel `Generiere ein Bild von einem Jungen, der einen Kaffee hält und ihn gleich trinken möchte`, hier ist das konkrete Beispiel:
```json
{
  "model": "gpt-4o-image",
  "messages": [
