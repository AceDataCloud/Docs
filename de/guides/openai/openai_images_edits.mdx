...
> Wenn `auto` (oder `size` weggelassen) übergeben wird, wird die Ausgabe **das Seitenverhältnis des Referenzbildes beibehalten** – im obigen Beispiel ist das Originalbild 1:1, sodass das Ergebnis ebenfalls 1:1 sein wird und nicht in ein anderes Format gestaucht wird. Dies unterscheidet sich von der Generierungsschnittstelle: Die Generierungsschnittstelle hat kein Referenzbild, `auto` wählt das Format basierend auf der Semantik des Eingabetextes aus. Wenn Sie das Format ändern möchten, geben Sie `size` explizit an.

> **Über den `n` Parameter**
> Die Bearbeitungsschnittstelle von `gpt-image-2` unterstützt `n > 1`: Mit einer Anfrage können Sie eine entsprechende Anzahl von Bearbeitungsergebnissen zurückgeben und pro Bild abrechnen (Werte für `n` von 1–10). Dies gilt auch für `gpt-image-1` / `gpt-image-1.5` sowie für die Serien `nano-banana` / `nano-banana-2-lite` / `nano-banana-2` / `nano-banana-pro`. Beachten Sie, dass `response_format=b64_json` nur für `n=1` unterstützt wird; bei `n>1` verwenden Sie bitte die Standard-URL-Rückgabe. Wenn einige Bilder nicht erfolgreich generiert werden, werden nur die erfolgreichen Teile zurückgegeben und abgerechnet.
...
Wenn Sie bereits das offizielle OpenAI Python SDK verwenden, ist die ursprüngliche `multipart/form-data`-Upload-Methode ebenfalls anwendbar, ändern Sie einfach das `model` in `gpt-image-2`:
...
Die `nano-banana`-Reihe hat ebenfalls den Zugriff auf `/openai/images/edits` im Bearbeitungsszenario integriert, ändern Sie das `model` einfach in eines der untenstehenden Modelle.
...
> - Parameter wie `mask`, `size`, `response_format` werden nicht unterstützt; sie werden ignoriert, wenn sie ausgefüllt sind. `n > 1` wird unterstützt (1–10) und gibt die entsprechende Anzahl an Bearbeitungsergebnissen zurück und wird entsprechend abgerechnet.
...
Die `callback_url` asynchrone Rückruffunktionalität ist auch für nano-banana wirksam, der Aufrufprozess ist identisch mit anderen Modellen, siehe den folgenden Abschnitt [Asynchrone Rückrufe](#asynchrone-rückrufe).
...
Um Python zu verwenden, müssen wir zunächst zwei Umgebungsvariablen importieren, eine `OPENAI_BASE_URL`, die auf `https://api.acedata.cloud/openai` gesetzt werden kann, und eine Variable für die Anmeldeinformationen `OPENAI_API_KEY`, deren Wert aus `authorization` abgerufen wird. In Mac OS können Sie die Umgebungsvariablen mit den folgenden Befehlen setzen:
...
Der gesamte Prozess ist: Wenn der Client eine Anfrage stellt, gibt er zusätzlich ein `callback_url`-Feld an. Nach dem API-Aufruf gibt die API sofort ein Ergebnis zurück, das ein `task_id`-Feld enthält, das die aktuelle Aufgaben-ID darstellt. Wenn die Aufgabe abgeschlossen ist, wird das Ergebnis der Bildbearbeitung in Form von POST JSON an die vom Client angegebene `callback_url` gesendet, wobei auch das `task_id`-Feld enthalten ist, sodass die Aufgabenergebnisse über die ID miteinander verknüpft werden können.

Im Folgenden werden wir durch ein Beispiel verstehen, wie dies konkret funktioniert.
...
Es ist zu sehen, dass im Ergebnis ein Feld `task_id` vorhanden ist, das Feld `data` enthält die gleichen Bildbearbeitungsergebnisse wie bei einem synchronen Aufruf. Über das Feld `task_id` kann die Zuordnung der Aufgaben erfolgen.
