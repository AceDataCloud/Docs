...
Die OpenAI Tasks API wird verwendet, um die Ergebnisse von Aufgaben abzufragen, die zuvor im **Callback-Modus** an die OpenAI-Bildschnittstelle übermittelt wurden. Wenn Sie nicht auf die synchrone HTTP-Antwort warten können oder die Aufgabe später erneut abfragen möchten, verwenden Sie diese Schnittstelle.

Im Callback-Modus gibt die **ursprüngliche Bildschnittstelle nach der Annahme der Anfrage sofort eine `task_id` zurück**. Sie halten diese `task_id` direkt und können sie bei Bedarf verwenden, um sie in dieser Schnittstelle abzufragen, ohne eine benutzerdefinierte `trace_id` zusätzlich übermitteln zu müssen (nur wenn Sie eine eigene Geschäftskennzeichnung zur Verknüpfung verwenden möchten, ist dies erforderlich).
...
| `id`         | string | Wahlweise | Die Aufgaben-ID, die in der synchronen Antwort der Bildanfrage zurückgegeben wird (empfohlen) |
...
Entweder `id` oder `trace_id` muss übermittelt werden. In der Regel verwenden Sie einfach die `id`, die in der Antwort der Übermittlung enthalten ist; `trace_id` wird nur übermittelt, wenn Sie eine benutzerdefinierte Geschäftskennzeichnung zur Verknüpfung verwenden möchten.
...
- `id`: Die Aufgaben-ID, die bei der Annahme der ursprünglichen Bildanfrage generiert wurde.
- `trace_id`: Die benutzerdefinierte Verfolgungskennzeichnung, die in der ursprünglichen Anfrage übermittelt wurde, um die Verknüpfung mit der Client-Anwendung zu erleichtern.
- `type`: Aufgabentyp. Aufgaben, die in der `gpt-image`-Serie (z. B. `gpt-image-2`) geschrieben werden, haben den Typ `images`; `gpt-image-1`, nano-banana usw. verwenden `images_generations` / `images_edits`, einige Chat-Schnittstellen sind `chat_completions_image`.
...
- `created_at` / `finished_at` / `duration`: Unix-Zeitstempel (Sekunden) und Ausführungsdauer (Sekunden).
...
Entweder `ids` / `trace_ids` / `application_id` / `user_id` oder `created_at_*` Zeitfenster müssen übermittelt werden.
...
Tasks API dient hauptsächlich der asynchronen Verarbeitung im Callback-Modus. Im Callback-Modus gibt die Einreichungsschnittstelle **sofort eine `task_id`** (d.h. die Aufgaben-ID) zurück, danach müssen Sie nur noch direkt diese `task_id` verwenden, um die Tasks-Schnittstelle abzufragen, ohne selbst eine `trace_id` zu generieren.
...
# 1. Reichen Sie eine Bildgenerierungsaufgabe ein (Callback-Modus: Geben Sie die callback_url an, um sofort die task_id zurückzubekommen)
submit = requests.post(
    f"{API}/openai/images/generations",
    headers=HEADERS,
    json={
        "model": "gpt-image-1",
        "prompt": "Eine Katze im Aquarellstil sitzt auf einem Tisch",
...
# 2. Verwenden Sie direkt die task_id aus der Antwort der Einreichung, um die Tasks-Schnittstelle abzufragen, bis die Aufgabe abgeschlossen ist
while True:
    task = requests.post(
        f"{API}/openai/tasks",
        headers=HEADERS,
