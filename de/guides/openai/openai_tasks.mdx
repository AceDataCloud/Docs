---
title: "OpenAI Tasks API Integration und Nutzung"
...
Die OpenAI Tasks API verwendet die gleiche Autorisierung wie die bestehenden OpenAI-Dienste. Wenn Sie bereits OpenAI Images Generations beantragt haben, können Sie dieselbe Token verwenden, um diese Schnittstelle aufzurufen, ohne eine zusätzliche Anfrage stellen zu müssen.
...
| `id`         | string | Entweder | Die beim Einreichen der Bildanfrage in der synchronen Antwort zurückgegebene Aufgaben-ID (empfohlen) |
| `trace_id`   | string | Entweder | Nur erforderlich, wenn Sie in der ursprünglichen Anfrage eine benutzerdefinierte `trace_id` übermittelt haben |
...
- `type`: Typ der übergeordneten Schnittstelle. Aufgaben, die in der `gpt-image`-Serie (z. B. `gpt-image-2`) geschrieben wurden, haben den Typ `images`; ältere OpenAI-Kanäle (z. B. `gpt-image-1`, nano-banana) verwenden `images_generations` / `images_edits`, einige Chat-Schnittstellen haben den Typ `chat_completions_image`.
- `request`: Der vollständige Anfragekörper der ursprünglichen Anfrage.
- `response`: Der endgültige Antwortkörper, der von der übergeordneten Schnittstelle zurückgegeben wird, wenn der Callback abgeschlossen ist.
...
| `type`             | string   | Filterung nach übergeordnetem Typ (Werte: `images`, `images_generations`, `images_edits`) |
...
Die Tasks-API dient hauptsächlich einem asynchronen Prozess im Callback-Modus. Im Callback-Modus gibt die Einreichschnittstelle **sofort eine `task_id`** (d.h. die Aufgaben-ID) zurück, danach müssen Sie nur noch diese `task_id` verwenden, um die Tasks-Schnittstelle abzufragen, ohne selbst eine `trace_id` zu generieren.
...
# 2. Verwenden Sie direkt die task_id aus der Einreichantwort, um die Tasks-Schnittstelle abzufragen, bis die Aufgabe abgeschlossen ist
while True:
    task = requests.post(
        f"{API}/openai/tasks",
        headers=HEADERS,
