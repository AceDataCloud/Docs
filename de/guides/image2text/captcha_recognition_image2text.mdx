...
Dieser Artikel beschreibt eine digitale englische CAPTCHA-Erkennungs-API, die auf Deep-Learning-Technologie basiert und zur Erkennung von variablen englischen Ziffern-CAPTCHAs verwendet werden kann. Es wird der Inhalt des CAPTCHA-Bildes eingegeben und das Ergebnis des CAPTCHA ausgegeben.
...
- `text`: Der Textinhalt des verarbeiteten Bildes des variablen englischen Ziffern-CAPTCHAs.

Wir haben das Ergebnis der Verarbeitung des Bildes des variablen englischen Ziffern-CAPTCHAs erhalten. Wir müssen nur den Textinhalt im Ergebnisfeld `text` verwenden, um die Validierung zu bestehen.
...
Durch dieses Dokument haben Sie erfahren, wie Sie die API zur Erkennung von digitalen englischen Captchas verwenden können, die zur Erkennung von variablen englischen Zahlen-Captchas dient. Geben Sie den Inhalt des Captcha-Bildes ein, und die Ausgabe ist das Captcha-Ergebnis. Wir hoffen, dass dieses Dokument Ihnen hilft, die API besser zu integrieren und zu nutzen. Bei Fragen wenden Sie sich bitte jederzeit an unser technisches Support-Team.
