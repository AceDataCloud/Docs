...
Um die digitale englische CAPTCHA-Erkennungs-API zu nutzen, müssen Sie zunächst im [Ace Data Cloud-Dashboard](https://platform.acedata.cloud/console/applications) Ihr API-Token abrufen und für zukünftige Verwendung aufbewahren.
...
Anschließend können Sie den Base64-Code, den Sie mit dem Google Chrome-Plugin FeHelper erhalten haben, hierher kopieren. Denken Sie daran, dass der Präfix data:image/png;base64 nicht enthalten ist. Der genaue Inhalt sieht wie folgt aus:
...
Wir haben das Ergebnis der Verarbeitung des variablen englischen Ziffern-CAPTCHAs erhalten, und wir müssen nur den Textinhalt im `text`-Feld verwenden, um die Validierung zu bestehen.
