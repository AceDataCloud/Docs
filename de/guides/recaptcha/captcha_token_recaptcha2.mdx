...
Zunächst sollten Sie die grundlegende Nutzung verstehen, indem Sie die URL der Website eingeben, die den zu verarbeitenden Verifizierungscode enthält, um das verarbeitete Ergebnis zu erhalten. Zuerst müssen Sie einfach ein `website_url`-Feld übermitteln. Unsere Beispielwebsite ist: `https://www.google.com/recaptcha/api2/demo`, wir müssen auf der `website_url`-Seite den `website_key` abrufen. Zuerst öffnen Sie diese Webseite, drücken F12, um die Konsole zu betreten, und suchen dann im Element-Bereich global nach `recaptcha-demo`, wir können das folgende Ergebnis erhalten:
...
Wir können sehen, dass wir das Verifizierungsergebnis der Recaptcha2 erhalten haben, das wir dann für POST oder zur Simulation der Einreichung an die Zielwebsite verwenden können, einmalig, mit einer Gültigkeit von 120 Sekunden, empfohlen innerhalb von 60 Sekunden zu verwenden. Im Folgenden wird ein Python-Skript bereitgestellt, um das verarbeitete Token an die Zielwebsite zu übermitteln, um die Recaptcha2-Verifizierung zu bestehen.
...
- Zuerst manuell die Verifizierung durchführen, wie im folgenden Bild gezeigt:
...
- Analysieren Sie die Struktur der POST-Anfrage dieser Einreichung, schließlich können Sie mit der rechten Maustaste auf diese Anfrage klicken, um den CURL-Code zu kopieren, wie im folgenden Bild dargestellt:
