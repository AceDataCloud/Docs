...
Die Hauptfunktion der SeeDream Tasks API besteht darin, den Status der Ausführung eines Auftrags anhand der von der SeeDream Images Generation API generierten Auftrags-ID abzufragen.
...
Die Rückgabe enthält mehrere Felder, wobei `items` die spezifischen Detailinformationen der Batch-Aufgaben enthält. Die spezifischen Informationen jeder Aufgabe sind identisch mit den oben genannten Feldern.

- `items`, alle spezifischen Detailinformationen der Batch-Aufgaben. Es ist ein Array, und jedes Element des Arrays hat das gleiche Format wie das Ergebnis der Abfrage einer einzelnen Aufgabe.
- `count`, die Anzahl der Batch-Abfragen.

Jedes Aufgabenobjekt im Batch-Ergebnis enthält auch die folgenden Zeitstempel.
...
- `elapsed`, die Ausführungszeit der Aufgabe, Einheit Sekunden (Fließkomma, auf 3 Dezimalstellen gerundet). Wenn die Aufgabe nicht abgeschlossen ist, wird dieses Feld nicht zurückgegeben.
