...
Die AI Chat v2 API (`/aichat2/conversations`) ist die nächste Generation der Dialogschnittstelle und eine umfassende Upgrade-Version der [AI Chat API](https://platform.acedata.cloud/documents/aichat-conversations). Sie erweitert die v1, die einfache, gehostete Mehrfachdialoge unterstützt, um:
...
Die einfachste Verwendung ist identisch mit v1: Übermitteln Sie `model` + `question`, um `{answer, id}` zu erhalten.
...
```json
{
  "answer": "AceDataCloud ist eine einheitliche API-Plattform, die führende KI-Modelle und multimodale Dienste aggregiert, sodass Entwickler mit einem Schlüssel auf Dienste wie GPT, Claude, Gemini, Midjourney, Suno, Veo usw. zugreifen können.",
...
Verfügbare `model`-Werte können direkt im Dropdown-Menü des Try-Panels auf der rechten Seite gesehen werden, gängige Kategorien umfassen:
...
curl -X POST 'https://api.acedata.cloud/aichat2/conversations' \
  ...
  -d '{
    "model": "gpt-5.4",
    "stateful": true,
    "question": "Merke dir eine Zahl: 42."
  }'
```
...
```json
{
  "answer": "Okay, ich habe mir 42 gemerkt. Was soll ich damit machen?",
  "id": "f2f4b3e8-0c0a-4d3a-aaa2-7ff80c0a1c44"
}
```
...
Danach können Sie `action: retrieve` + `id` verwenden, um die Sitzungsergebnisse abzufragen; Sie können auch `callback_url` bereitstellen, die Plattform wird `{ status, answer, usage, error }` nach Abschluss der Aufgabe an Ihre Callback-Adresse POSTen. `callback_url` muss `http` / `https` verwenden und darf keine direkte Eingabe von `localhost` oder privaten IP-Literaladressen enthalten.
...
Die Vorabgenehmigung sind diese beiden Listen selbst: Eine leere Liste bedeutet, dass keine Fähigkeiten autorisiert sind, ohne dass zusätzliche Schalterfelder erforderlich sind.

Hinweis: Die Vorabgenehmigung bedeutet nur, dass „diese Fähigkeiten in dieser Anfrage im unbeaufsichtigten Modus ohne menschliche Bestätigung übersprungen werden dürfen“. Bestimmte Skills müssen weiterhin `--unattended-confirm` oder entsprechende Sicherheitsmechanismen unterstützen; andernfalls wird weiterhin ein Dry-Run durchgeführt und keine Schreiboperationen direkt ausgeführt.
...
Im Anfragekörper muss `tool_use_id` **genau** mit dem `tool_id` während der Pause übereinstimmen; eine Abweichung führt zu einem 400-Fehler. Wenn in der Anfrage gleichzeitig `tool_results` vorhanden sind, werden `question` / `message` / `references` ignoriert.

Wenn der Benutzer beschließt, diese Frage aufzugeben, senden Sie einfach eine neue `question` / `message`, die Plattform wird automatisch den pausierten Werkzeugaufruf als „Benutzer übersprungen“ markieren.
...
`messages` können ebenfalls übergeben werden, aber der Server führt eine strenge Schema-Überprüfung durch (muss in der gefalteten `ToolUseContent`-Form vorliegen), andernfalls wird 400 zurückgegeben. Es wird allgemein empfohlen, nur den `title` zu ändern.
...
```json
{
  "error": {
    "code": "chat_error",
    "message": "Das Modell-Dienst hat einen Fehler zurückgegeben"
  },
...
- `500 chat_error`: Der upstream LLM hat einen Fehler gemeldet oder in dieser Runde `completion_tokens=0` (wird als nicht verbraucht behandelt, es fallen keine Kosten an).
