...
Gleichzeitig ist die API auf der Anfrageebene **vollständig rückwärtskompatibel mit v1**: Es genügt, `model` + `question` (plus optional `stateful` / `id` / `references` / `preset`) zu übermitteln, um die äquivalente `{answer, id}` JSON-Antwort von v1 zu erhalten, sodass eine Migration von `/aichat/conversations` nicht erfordert, dass der Client neu geschrieben wird; es genügt, den Pfad auf `/aichat2/conversations` zu ändern.
...
Wie bei v1 können Sie `stateful: true` übermitteln, um die Sitzung zu speichern; die API gibt eine `id` zurück. Bei nachfolgenden Anfragen bringen Sie einfach die `id` mit, um das Gespräch fortzusetzen, ohne die Nachrichtenhistorie selbst verwalten zu müssen.
...
```json
{
  "answer": "Okay, ich habe 42 gemerkt. Was soll ich damit machen?",
  "id": "f2f4b3e8-0c0a-4d3a-aaa2-7ff80c0a1c44"
}
```
...
| `thinking`          | Der Denkprozess des Modells (erscheint nur, wenn das gewählte Modell Reasoning offenbart).                                                                |
...
Für Clients, die nur an der endgültigen Antwort interessiert sind, ist das Zusammenfügen aller `text_delta` `content` äquivalent zu `answer` im `application/json`-Modus.
...
- Der Benutzer hat in [Connections](https://platform.acedata.cloud/connections) Google Drive / GitHub / Notion usw. autorisiert → Modell kann die entsprechenden MCP-Tools aufrufen, um deren Daten zu lesen und zu schreiben.
...
Geben Sie `{ items: [...], total }` zurück. **Die Zusammenfassung enthält keine `messages`**, geeignet für eine Seitenleistenliste; wenn der Benutzer eine Sitzung öffnet, verwenden Sie `action: retrieve`, um die vollständigen Nachrichten einzeln abzurufen.
...
2. Wenn Sie zuvor v1 Modellnamen (z. B. `gpt-3.5`, `gpt-4-browsing` usw.) übergeben haben, wird empfohlen, beim Wechsel zu v2 auf moderne Modelle (z. B. `gpt-5.4`, `claude-opus-4-8`, `gemini-3.1-pro` usw.) zu aktualisieren.
