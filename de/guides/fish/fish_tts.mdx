...
| `callback_url` | string             | Nein | Asynchrone Callback-Adresse, siehe unten „Asynchrone Rückrufe“. **Dies ist eine Erweiterung der offiziellen Schnittstelle**.                                                                                                                                         |
...
curl -X POST 'https://api.acedata.cloud/fish/tts' \
  ...
  -H 'content-type: application/json' \
  -d '{
    "text": "Schnelleres Sprechen mit Prosodie-Overrides.",
...
Es kann auch die [Fish Tasks API](https://platform.acedata.cloud/documents/fish-tasks) verwendet werden, um aktiv Ergebnisse nach `task_id` abzurufen, siehe die entsprechende Dokumentation.
...
Parametervalidierungsfehler enthalten den Originalfehlertext von pydantic im `message`-Feld, um zu helfen, welches Feld ungültig ist, zum Beispiel:
