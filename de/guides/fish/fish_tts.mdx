...
| `format`       | string             | Nein | Ausgabe-Audioformat, wählbar `mp3` (Standard), `wav`, `pcm`. `wav` und `pcm` geben beide einen WAV-Container zurück. `opus` wird nicht unterstützt, eine Eingabe führt direkt zu `400`.                                                                                      |
...
| `prosody`      | object             | Nein | Prosodie-Overrides, unterstützt `speed` (Sprechgeschwindigkeit, 1.0 ist die Originalgeschwindigkeit) und `volume` (Lautstärkegewinn in dB). Beispiel `{"speed":1.2,"volume":0}`.                                                                                              |
...
| `latency`      | string             | Nein | `normal` oder `balanced`, standardmäßig wird von dieser Schnittstelle automatisch `normal` ergänzt (leere Zeichenfolgen werden vom Upstream abgelehnt).                                                                                                                    |
...
`audio_url` verweist auf das CDN dieser Plattform, kann direkt mit GET heruntergeladen oder in `<audio>` abgespielt werden. Die endgültige erfolgreiche Antwort enthält auch die oberste `cost`, wobei `amount` die tatsächlich abgezogenen Credits für diese Anfrage ist; bei Kontorabatten zeigt `list_amount` den Betrag vor dem Rabatt an. Der Link ist langfristig verfügbar, es wird jedoch empfohlen, eine Kopie in Ihrem eigenen Speicher aufzubewahren.
...
Für Szenarien, in denen Echtzeit-Zusammenführung im Browser oder nachträgliche Verarbeitung (Mixing, Geschwindigkeitsänderung) auf der Client-Seite erforderlich ist, wird die Verwendung von `pcm` empfohlen:
...
Die minimalen Kosten für die Integration von Fish TTS sind: Ersetzen des Authentifizierungstokens in bestehendem Code, der `api.fish.audio/v1/tts` aufruft, durch das Token dieser Plattform und das **explizite Hinzufügen** von `format: "mp3"` im Anfragekörper. Für lange Textszenarien wird empfohlen, `callback_url` für asynchrone Rückrufe zu verwenden; zur Entdeckung von Klonstimmen `reference_id` sollten die [Fish Model Query](https://platform.acedata.cloud/documents/fish-model-query) und [Fish Model Get](https://platform.acedata.cloud/documents/fish-model-get) verwendet werden.
