...
Die Fish Model API ist vollständig kompatibel mit der ModelEntity-Schnittstelle der offiziellen Fish Audio OpenAPI und ermöglicht eine Migration bestehender Klon-Stimmprofil-Verwaltungscode ohne Codeänderungen. Die erstellte `_id` des Stimmprofils kann direkt als `reference_id` im [Fish TTS API](https://platform.acedata.cloud/documents/fish-tts) für die Sprachsynthese verwendet werden.
