...
Um die Hailuo Videos Generation API zu nutzen, gehen Sie zuerst zur [Ace Data Cloud Konsole](https://platform.acedata.cloud/console/applications), um Ihr API-Token zu erhalten und für später aufzubewahren.
...
- `first_image_url`: Wenn das Bild-zu-Video-Modell `minimax-i2v` ausgewählt wird, muss der Link zum ersten Bild-Referenzbild hochgeladen werden, Base64-Codierung wird nicht unterstützt.
- `prompt`: Eingabewort.
...
Wir können sehen, dass wir die gewünschten Videoinformationen erhalten haben. Wir müssen nur den Link zur Video-URL im `data`-Ergebnis abrufen, um das generierte Hailuo-Video zu erhalten.
