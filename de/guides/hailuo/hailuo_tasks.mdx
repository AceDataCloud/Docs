...
Die Hauptfunktion der Hailuo Tasks API besteht darin, den Ausführungsstatus eines Tasks abzufragen, indem die Task-ID eingegeben wird, die durch die Hailuo Videos Generation API generiert wurde.

Dieses Dokument wird die Integrationsanleitung der Hailuo Tasks API detailliert beschreiben, um Ihnen zu helfen, diese API einfach zu integrieren und ihre leistungsstarken Funktionen voll auszuschöpfen. Mit der Hailuo Tasks API können Sie den Ausführungsstatus der Tasks der Hailuo Videos Generation API problemlos abfragen.
...
Die Rückgabe enthält mehrere Felder, das Feld request ist der request body, der beim Starten des Tasks gesendet wurde, während das Feld response die response body ist, die nach Abschluss des Tasks zurückgegeben wird. Die Feldbeschreibung ist wie folgt.
