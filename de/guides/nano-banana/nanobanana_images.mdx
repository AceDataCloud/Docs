...
Um die Nano Banana Images API zu nutzen, müssen Sie zunächst Ihr API-Token im [Ace Data Cloud Dashboard](https://platform.acedata.cloud/console/applications) abrufen und für spätere Verwendung aufbewahren.
...
- **Anzahl der Bilder**: Optional, durch `count` 1–4 Bilder angeben, Standard ist 1 Bild; jedes Bild wird durch einen separaten Generierungsaufruf erstellt; technische Fehler oder Sicherheitsverweigerungen des Anbieters betreffen nur den entsprechenden Aufruf, während die anderen erfolgreich zurückgegeben werden und nach der tatsächlichen Anzahl der erfolgreichen Bilder abgerechnet werden
...
- `count`: Anzahl der angeforderten generierten oder bearbeiteten Bilder, unterstützt 1–4, Standard ist 1. `data` enthält nur die erfolgreich generierten Bilder und wird nach der tatsächlichen Anzahl der zurückgegebenen Bilder abgerechnet. Jeder Generierungsaufruf verwendet die nativen Sicherheitsrichtlinien des Anbieters; wenn ein Aufruf abgelehnt wird, hat dies keine Auswirkungen auf andere erfolgreiche Aufrufe. Wenn alle Aufrufe abgelehnt werden, wird 403 zurückgegeben.
- `data[]`: Ergebnisliste.
...
- **403 `forbidden`**: Die native Sicherheitsrichtlinie des Anbieters hat die Anfrage oder das generierte Ergebnis abgelehnt. Dieser Aufruf gibt kein Bild zurück und wird nicht berechnet; Mehrfachanfragen können jedoch andere erfolgreiche Aufrufe zurückgeben und berechnen.
...
- **Idempotenz und Verfolgung**: Bewahren Sie `task_id` und `trace_id` auf, um Fehlerbehebung und Ergebnisverknüpfung zu erleichtern
