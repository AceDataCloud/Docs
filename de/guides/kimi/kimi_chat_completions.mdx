...
Um die Kimi Chat Completion API zu nutzen, müssen Sie zunächst im [Ace Data Cloud Dashboard](https://platform.acedata.cloud/console/applications) Ihr API-Token abrufen und für später aufbewahren.
...
Nachdem Sie `stream` auf `true` geändert haben, gibt die API die entsprechenden JSON-Daten zeilenweise zurück, und auf der Codeebene müssen wir entsprechende Änderungen vornehmen, um die zeilenweisen Ergebnisse zu erhalten.
...
data: [DONE]
```
Es ist zu sehen, dass in der Antwort viele `data` enthalten sind, wobei die `choices` in `data` den neuesten Antwortinhalt darstellen, der mit dem oben beschriebenen Inhalt übereinstimmt. `choices` sind die neuen Antwortinhalte, die Sie je nach Ergebnis in Ihr System integrieren können. Das Ende der Streaming-Antwort wird anhand des Inhalts von `data` bestimmt; wenn der Inhalt `[DONE]` ist, bedeutet dies, dass die Streaming-Antwort vollständig beendet ist. Die zurückgegebenen `data`-Ergebnisse bestehen aus mehreren Feldern, die wie folgt beschrieben werden:

- `id`, die ID der generierten Dialogaufgabe, die zur eindeutigen Identifizierung dieser Dialogaufgabe dient.
- `model`, das gewählte Kimi-Website-Modell.
...
Wenn Sie die Funktion für mehrere Dialoge integrieren möchten, müssen Sie mehrere Fragen im `messages`-Feld hochladen. Ein konkretes Beispiel für mehrere Fragen ist im folgenden Bild dargestellt:
...
Durch das Hochladen mehrerer Fragen können Sie problemlos mehrere Dialoge führen und erhalten folgende Antworten:
...
Es ist zu sehen, dass die Informationen in `choices` mit den grundlegenden Inhalten übereinstimmen, die Kimi zur Beantwortung mehrerer Dialoge bereitstellt, sodass Sie basierend auf mehreren Dialoginhalten die entsprechenden Fragen beantworten können.
...
Durch dieses Dokument haben Sie verstanden, wie Sie die Kimi Chat Completion API verwenden, um normale Dialoge, Streaming-Antworten, mehrere Dialoge zu implementieren und die K3-Inferenzstärke über `reasoning_effort` zu steuern.
