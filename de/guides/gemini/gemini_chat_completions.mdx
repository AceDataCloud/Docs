...
Google Gemini ist ein sehr leistungsstarkes KI-Dialogsystem, das in nur wenigen Sekunden flüssige und natürliche Antworten generieren kann, sobald ein Eingabewort eingegeben wird. Gemini bietet erstaunliche intelligente Unterstützung und steigert erheblich die Effizienz und Kreativität der Menschen.
...
Bei der ersten Nutzung dieser Schnittstelle müssen wir mindestens drei Inhalte ausfüllen: einer ist `authorization`, den Sie einfach aus der Dropdown-Liste auswählen können. Ein weiterer Parameter ist `model`, `model` ist die Kategorie des Modells, das wir von der Gemini-Website verwenden möchten. Hier haben wir hauptsächlich 6 Modelle zur Auswahl, die Details finden Sie in den von uns bereitgestellten Modellen. Der letzte Parameter ist `messages`, `messages` ist das Array der von uns eingegebenen Fragen, es handelt sich um ein Array, das mehrere Fragen gleichzeitig hochladen kann, wobei jede Frage `role` und `content` enthält. Dabei steht `role` für die Rolle des Fragestellers, wir bieten drei Identitäten an: `user`, `assistant`, `system`. Das andere `content` ist der spezifische Inhalt unserer Frage.
...
- **base64 `data:` URI (empfohlen, stabilste)**: Format ist `data:<Medientyp>;base64,<Daten>`, zum Beispiel `data:image/jpeg;base64,/9j/4AAQ...`. **Der Medientyp (MIME) ist bereits im `data:`-Präfix enthalten**, daher ist kein separates `media_type`-Feld erforderlich oder vorhanden.
...
Wenn eine Stream-Antwort gewünscht ist, kann der `stream`-Parameter im Anfrage-Header auf `true` geändert werden.
...
- `choices`, die Antwortinformationen von Gemini auf die gestellten Fragen.
JavaScript wird ebenfalls unterstützt, zum Beispiel der Code für den Streaming-Aufruf in Node.js ist wie folgt:
...
Andere Sprachen können separat umgeschrieben werden, das Prinzip ist dasselbe.
  ...
  "choices": [
    {
      ...
      "message": {
        ...
        "reasoning_content": "Okay, so würde ich das angehen, indem ich als fortschrittliche KI denke:\n\nZuerst fragt der Benutzer direkt nach meiner Kernidentität: \"Welches Modell bist du?\" Das ist eine einfache, direkte Frage. Meine Hauptaufgabe ist es, genaue und hilfreiche Informationen bereitzustellen, also muss ich das sofort beantworten.\n\nVon Anfang an weiß ich die Antwort. Ich *bin* ein großes Sprachmodell. Das ist meine grundlegende Architektur und mein Betriebsprinzip. Ich bin auf einem riesigen Netzwerk von Verbindungen aufgebaut und auf einem massiven Datensatz trainiert.\n\nDann muss ich den notwendigen Kontext hinzufügen. Entscheidend ist, dass ich die Quelle meines Trainings angeben muss. Ich *wurde* von Google trainiert. Das ist ein wichtiger Identifikator. Es gibt dem Benutzer wertvolle Informationen über meinen Ursprung und mögliche Vorurteile (obwohl ich darauf trainiert bin, diese zu mildern).\n\nJetzt muss ich die Antwort formulieren. Mein Ziel ist es, faktisch, prägnant und verständlich zu sein.\n\nDie synthetisierte Antwort ist etwa: \"Ich bin ein großes Sprachmodell, trainiert von Google.\"\n\nIch muss die Ausgabe bewerten: Entspricht sie den Kriterien? Es ist klar. Es sagt, was ich bin, es enthält ein kritisches Stück Information über meine Herkunft und vermeidet Fachjargon. Keine irreführenden Versprechungen.\n\nLetzte Überprüfung: Beantwortet es tatsächlich die Frage? Ja. Ist die Information korrekt und wahrheitsgemäß? Ja. Ist es prägnant? Absolut. Ist der Ton für jeden Benutzer angemessen? Ja. Und schließlich ist dies die standardmäßige, genehmigte Antwort. Ausgezeichnet.\n"
      },
      "finish_reason": "stop"
    }
  ],
...
Wie zu sehen ist, enthält die Information in `choices` den gleichen Inhalt wie die grundlegende Nutzung, dies umfasst die spezifischen Inhalte, die Gemini für mehrere Dialoge zur Beantwortung bereitstellt, sodass Fragen basierend auf mehreren Dialoginhalten beantwortet werden können.
...
Wie oben zu sehen, unterstützt das Gemini 3.0-Modell multimodales Verständnis.
...
Das Rückgabeformat ist identisch mit dem von Gemini 3.0 Pro, siehe die obigen Erläuterungen zum Kapitel über das multimodale Modell von Gemini-3.0.
...
Durch dieses Dokument haben Sie gelernt, wie Sie die Gemini Chat Completion API verwenden, um die offiziellen Gemini-Dialogfunktionen einfach zu implementieren. Wir hoffen, dass dieses Dokument Ihnen hilft, die API besser zu integrieren und zu nutzen. Bei Fragen können Sie sich jederzeit an unser technisches Support-Team wenden.
