...
Die Flux Tasks API kann verwendet werden, um die Ergebnisse der Flux Images Generation API abzufragen. Informationen zur Nutzung der Flux Images Generation API finden Sie in der Dokumentation [Flux Images Generation API](https://platform.acedata.cloud/documents/flux-images-integration).
...
- `response`, Informationen zur Antwort im Bildauftrag.
- `created_at`, Erstellungszeit des Auftrags, Unix-Zeitstempel (Sekunden, Fließkomma).
- `started_at`, Beginn der Ausführung des Auftrags, ISO-8601 UTC-Zeitzeichen.
