...
- `image_urls`: Wenn die Aktion Bild zu Video (`image2video`) gewählt wird, müssen die Referenzbild-Links hochgeladen werden. `veo31-fast-ingredients` unterstützt maximal 3 Bilder (Multi-Image-Fusion), andere Modelle unterstützen maximal 2 Bilder (First-Last-Frame-Modus).
- `resolution`: Wählen Sie die Auflösung des generierten Videos, wobei das veo31-Modell 4k-Auflösung unterstützt, andere Modelle nicht. Alle Modelle unterstützen 1080p und gif-Auflösung. Wenn dieser Wert nicht übergeben wird, wird standardmäßig 720p verwendet, unterteilt in: `1080p`, `gif`, `4k`.
- `prompt`: Eingabewort.
...
| **veo31-fast-ingredients** | ❌ Text zu Video (nicht unterstützt)<br />✅ **Zwangs-Multi-Image-Fusion** (Bilder müssen übergeben werden) | **1-3 Bilder** → Multi-Image-Fusion-Modus (maximal 3 Bilder)        |
...
2. **Bild zu Video-Modus-Typen**:
   - **First-Frame-Modus** (1 Bild): Der erste Frame ist festgelegt auf das Eingabebild.
   - **First-Last-Frame-Modus** (2 Bilder): Der erste und letzte Frame sind festgelegt auf die Eingabebilder.
   - **Multi-Image-Fusion-Modus** (1-3 Bilder): Nur `veo31-fast-ingredients` unterstützt dies, um Inhalte mehrerer Bilder zu fusionieren und ein Video zu generieren.
...
Es ist zu erkennen, dass der Ergebnisinhalt mit dem oben genannten übereinstimmt, was die Bild-zu-Video-Funktion realisiert.
...
- `model`：das Modell zur Videoerstellung, hauptsächlich `veo31-fast`、`veo3`、`veo31`、`veo3-fast` und `veo31-fast-ingredients`.
- `video_id`：die Referenzvideo-ID, um das 1080p-Video abzurufen.
...
Es ist zu erkennen, dass der Ergebnisinhalt mit dem oben genannten übereinstimmt, was die Funktion zum Abrufen von 1080p-Videos realisiert.
...
Nach kurzer Wartezeit können wir auf `https://webhook.site/aed5cd28-f8aa-4dca-9480-8ec9b42137dc` das Ergebnis der Videoerstellung beobachten, wie im Bild gezeigt:
...
Es ist zu sehen, dass im Ergebnis ein `task_id`-Feld vorhanden ist, und die anderen Felder sind ähnlich wie oben, sodass die Aufgabe über dieses Feld verknüpft werden kann.
