...
WebExtrator 서비스 페이지를 사용하려면 먼저 [Ace Data Cloud 콘솔](https://platform.acedata.cloud/console/applications)에서 API 토큰을 받아야 합니다. 이를 보관하세요.
...
| `bypass_cache`      | boolean  |  ❌  | `false`                     | Redis 캐시 읽기를 건너뜁니다(하지만 여전히 이번 결과를 캐시에 기록합니다).                |
| `cache_ttl_seconds` | number   |  ❌  | `3600`                      | 이번에 기록할 캐시 TTL을 사용자 정의합니다. `0`을 전달하면 이번 응답을 캐시하지 않습니다.   |
| `async`             | boolean  |  ❌  | `false`                     | `true`로 설정하면 즉시 `task_id`를 반환하며, 결과는 `callback_url` 또는 Tasks API를 통해 가져옵니다. |

> 플랫폼 계약은 통일하여 **snake_case**를 사용합니다. 내부 렌더링 서비스는 camelCase를 지원하지만 외부 호출은 모두
> snake_case를 사용합니다.
...
| `data.finalUrl`      | string         | 리디렉션 후의 최종 URL입니다.                             |
| `data.title`         | string         | 렌더링된 `document.title`입니다.                        |
| `data.status`        | number \| null | 주요 탐색의 HTTP 상태 코드입니다.                         |
...
플랫폼은 `callback_url`로 동기 모드와 **완전히 동일한** envelope을 `POST`합니다,
...
| 429  | `queue_busy`     | 동기 큐가 혼잡합니다. 다시 시도하거나 `async=true`로 변경하십시오.       |
...
작업이 완료되면 플랫폼이 귀하의 `callback_url`에 전체 결과를 POST합니다.
...
- **`wait_until` 선택이 중요합니다.** `networkidle`이 가장 안정적이지만 가장 느립니다; `domcontentloaded`
  빠르지만 비동기 주입된 내용을 놓칠 수 있습니다; `load`는 전통적인 정적 페이지에 적합합니다.
- **캐시 키는 `async`를 무시합니다.** 동일한 URL의 동기 및 비동기 요청이 동일한 캐시 항목에 적중하며,
  임의로 전환해도 무효화되지 않습니다.
- **캐시 키는 `bypass_cache` 및 `cache_ttl_seconds`를 무시합니다.** 이 두 가지는 작동 스위치이며,
  응답 내용에 영향을 미치지 않습니다.
- **`cookies` 및 `headers`는 분리된 캐시를 생성합니다.** 이 두 가지를 사용자 정의하면 최초 동일 조합의 적중이 실패합니다.
- **SPA는 기본 30초를 자주 초과합니다.** `timeout: 60`, `wait_until: "domcontentloaded"`, `delay: 4`를 권장하며, 
  `wait_for_selector`와 함께 실제로 관심 있는 요소를 기다립니다.
- **`block_resources`는 지연을 줄이는 가장 빠른 경로입니다.** 기본적으로 이미지 / 글꼴 / 미디어가 차단되어 있으며;
  CSS 레이아웃에 의존하지 않는 경우 `stylesheet`를 추가하면 더 빨라집니다.
