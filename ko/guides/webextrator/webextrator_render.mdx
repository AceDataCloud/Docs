...
| `timeout`           | number   |  ❌  | `30`                         | 단일 탐색 초과 시간(**초**).                                                  |
...
```json
{
  ...
  "finished_at": "...",
  "elapsed": 0.012,
  "error": { "code": "bad_request", "message": "url: 유효하지 않은 url" }
}
```
...
즉시 `{ "success": true, "task_id": "...", "trace_id": "...", "started_at": "..." }`를 반환합니다;
...
- **`block_resources`는 지연을 줄이는 가장 빠른 경로입니다.** 기본적으로 이미지 / 글꼴 / 미디어가 차단되어 있으며; CSS 레이아웃에 의존하지 않는 경우 `stylesheet`를 추가하면 더 빨라집니다.
