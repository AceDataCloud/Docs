...
> 페이지에 schema.org JSON-LD가 포함되어 있을 경우, `enable_llm`은 무효입니다 — 결정적 매퍼가 직접 결과를 반환하며, LLM 호출을 낭비하지 않습니다. 당신은 **공짜로** 유형화된 결과를 얻습니다.
...
| `cached`        | boolean? | 캐시가 적중할 경우 `true`입니다.                                                                       |
...
LLM 성공 시에도 최상위 필드에 "last-resort"로 다시 채워집니다:
...
- **`expected_type`를 전달할 수 있으면 전달하세요.** 무료 팁으로, 유도적 판단을 건너뛰고, URL 패턴이 내장 목록에 없는 페이지에 특히 유용합니다.
...
- **`structured.llmError`는 정보성입니다.** 요청은 여전히 성공적이며, 유도적 결과는 여전히 반환됩니다. `llmError.error`를 확인하여 원인을 파악하세요 (타임아웃, JSON 파싱 실패, Zod 검증 실패).
