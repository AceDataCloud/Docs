...
| `prosody`      | object             | 아니요 | 운율 조정, `speed`(속도, 1.0은 원속)와 `volume`(음량 증폭 dB)을 지원합니다. 예: `{"speed":1.2,"volume":0}`.                                                                                              |
...
| `latency`      | string             | 아니요 | `normal` 또는 `balanced`, 기본적으로 본 인터페이스에서 자동으로 `normal`로 보완합니다(빈 문자열을 전달하면 상류에서 거부됩니다).                                                                                                                    |
