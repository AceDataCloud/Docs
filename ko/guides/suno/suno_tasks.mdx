...
- `created_at`은 작업 생성 시간으로, Unix 타임스탬프(초, 부동 소수점)입니다.
- `started_at`은 작업 시작 실행 시간으로, ISO-8601 UTC 시간 문자열입니다.
- `finished_at`은 작업 완료 시간으로, Unix 타임스탬프(초, 부동 소수점)입니다. 작업이 완료되지 않은 경우 이 필드는 반환되지 않습니다.
- `elapsed`는 작업 실행 소요 시간으로, 단위는 초(부동 소수점, 소수점 3자리 유지)입니다. 작업이 완료되지 않은 경우 이 필드는 반환되지 않습니다.

## 배치 조회 작업

이는 여러 작업 ID에 대해 노래 작업 세부 정보를 조회하는 것이며, 위와는 다르게 action을 retrieve_batch로 설정해야 합니다.
