...
SUNO는 참조 오디오를 업로드하여 2차 창작하는 것을 지원합니다. 이 문서에서는 관련 API 연동 방법을 설명합니다.

일반 업로드에는 `audio_url`만 필요하며, 이는 공개적으로 접근 가능한 오디오 CDN 주소입니다. `mode`의 기본값은 `standard`이므로 기존 호출은 수정할 필요가 없습니다. 일반 업로드 성공 시 **0.06 Credits**가 소모되며, 업로드 실패 시에는 과금되지 않습니다.
...
확인할 수 있듯이, `data`는 항상 객체이며, `audio_id`는 업로드 후의 노래 ID입니다. `lyric`, `style`, `duration`, 제목 및 미디어 URL 등의 분석 결과는 선택적 필드이며, 구체적인 내용은 반환 결과를 기준으로 합니다.

노래 ID를 얻은 후에는 [Suno Audios Generation API](https://platform.acedata.cloud/documents/suno-audios)를 통해 `upload_extend`, `upload_cover` 또는 구간 샘플링 등 `audio_id`가 필요한 작업을 호출할 수 있습니다. Inspo 영감 창작 기능을 통해 1개에서 4개의 완전한 참조 오디오를 기반으로 새 음악을 창작하려면, 직접 `action=inspo`를 호출하고 원본 공개 오디오 URL 목록을 전달하십시오. 서비스가 자동으로 업로드와 분석을 완료하므로 클라이언트에서 오디오 metadata를 구성할 필요가 없습니다.

## 향상된 업로드 모드

> `audio_url`은 입력 주소이며, 오디오 처리 흐름에 직접 전달되고 최종 미디어 영구 저장 서비스에서 사전에 이전 저장되지 않습니다. 작업 처리 기간 동안 공개적으로 접근할 수 있도록 보장하십시오.

일반 업로드로 소유하거나 사용 권한을 부여받은 오디오를 처리할 수 없는 경우, `mode`를 `enhanced`로 설정할 수 있습니다. 이 모드에서는 1자에서 100자 사이의 `name`도 반드시 제공해야 하며, 공개적으로 접근 가능한 HTTPS `audio_url`을 사용해야 합니다.

향상된 업로드는 고정적으로 비동기 처리를 사용하며, 일반적으로 2분 이상이 필요합니다. 처리 성공 시 **1.87 Credits**가 소모되며, 처리 실패 시에는 과금되지 않습니다.
...
향상된 업로드로 얻은 `audio_id`는 Cover, Samples 및 Mashup에 사용할 수 있습니다. 반주 및 MIDI 추출, 전체 트랙 분리, 그리고 보컬/반주 분리는 여전히 오디오 소속 계정의 제한을 받을 수 있으며, 나열되지 않은 작업은 계정 간 사용을 지원한다고 보장되지 않습니다. 오디오가 짧을수록 일반적으로 처리 성공률이 더 높습니다.
