...
일반 업로드는 `audio_url`만 필요하며, 이는 공개적으로 접근 가능한 오디오 CDN 주소입니다. `mode`는 기본값이 `standard`이므로 기존 호출은 수정할 필요가 없습니다. 일반 업로드가 성공하면 **0.06 Credits**가 소모되며, 최대 패키지 기준으로 약 **¥0.041/회**입니다; 업로드 실패 시에는 요금이 부과되지 않습니다.
...
일반 업로드로 처리할 수 없는 오디오를 보유하거나 사용 권한을 얻은 경우, `mode`를 `enhanced`로 설정할 수 있습니다. 이 모드는 1에서 100자 사이의 `name`을 제공해야 하며, 공개적으로 접근 가능한 HTTPS `audio_url`을 사용해야 합니다.

강화 업로드는 고정적으로 비동기 처리되며, 일반적으로 2분 이상 소요됩니다. 성공적으로 처리되면 **1.87 Credits**가 소모되며, 최대 패키지 기준으로 약 **¥1.26/회**입니다; 처리 실패 시에는 요금이 부과되지 않습니다.
