...
본 문서는 Kling Tasks API의 연동 설명을 자세히 소개하여, 여러분이 이 API의 강력한 기능을 쉽게 통합하고 충분히 활용할 수 있도록 돕습니다. Kling Tasks API를 통해 Kling Videos Generation API의 작업 실행 상태를 쉽게 조회할 수 있습니다.
...
Kling Tasks API는 Kling Videos Generation API의 결과를 조회하는 데 사용할 수 있습니다. Kling Videos Generation API 사용 방법에 대한 자세한 내용은 [Kling Videos Generation API](https://platform.acedata.cloud/documents/kling-videos) 문서를 참조하십시오.

Kling Videos Generation API 서비스에서 반환된 작업 ID를 예로 들어, 해당 API를 사용하는 방법을 시연하겠습니다. 가정해 보겠습니다. 작업 ID가 20068983-0cc9-4c6a-aeb6-9c6a3c668be0인 경우, 다음과 같이 작업 ID를 전달하여 사용합니다.
  ...
  "items": [
    {
      ...
      "request": {
        "action": "text2video",
        "model": "kling-v1",
        "prompt": "광택이 나는 대리석 조리대 위에 있는 흰색 세라믹 커피 머그컵. 카메라가 머그컵 주위를 360도 천천히 회전하며 손잡이에서 잠시 멈춘다.",
        "callback_url": "https://webhook.site/624b2c78-6dbd-4618-9d2b-b32eade6d8c3"
      },
      "trace_id": "0a907f69-4ae2-4a08-b34c-ee15c1c47077",
      "type": "videos",
...
반환 결과는 여러 필드를 포함하고 있으며, items는 배치 비디오 작업의 구체적인 세부 정보가 포함되어 있습니다. 각 비디오 작업의 구체적인 정보는 위의 필드와 동일합니다.

- `items`, 배치 비디오 작업의 모든 구체적인 세부 정보. 배열이며, 각 배열의 요소는 위의 단일 작업 반환 결과 형식과 동일합니다.
...
이 문서를 통해 Kling Tasks API를 사용하여 단일 또는 배치 비디오 작업의 모든 구체적인 세부 정보를 쿼리하는 방법을 이해하셨습니다. 이 문서가 API를 더 잘 연결하고 사용하는 데 도움이 되기를 바랍니다. 질문이 있으시면 언제든지 기술 지원 팀에 문의해 주십시오.
