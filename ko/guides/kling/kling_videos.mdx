...
- `generate_audio`: 오디오를 동기 생성할지 여부로, 선택 사항이며 불리언 값입니다. `kling-v3`, `kling-v3-omni`, `kling-v2-6`(단, pro 모드)에서 지원됩니다. 기본값은 `false`입니다.
...
다양한 모델이 매개변수에 대한 지원 상황이 크게 다릅니다. 아래 매트릭스는 [Kling 공식 video models 문서](https://app.klingai.com/global/dev/document-api/apiReference/model/videoModels)에서 정리한 것으로, 호출 전에 현재 `model` / `mode` / `duration` 조합이 필요한 기능을 지원하는지 확인해야 하며, 그렇지 않을 경우 `model/mode/duration(...) is not supported with image_tail` 등의 오류가 발생할 수 있습니다.
| 모델                  | 모드             | `end_image_url`（첫 마지막 프레임） | `generate_audio`（배경음악） | `camera_control`（카메라 제어） | 비고                                           |
| ------------------- | -------------- | -------------------- | -------------------- | -------------------- | -------------------------------------------- |
...
| `kling-v2-6`        | std            | ❌                    | ❌                    | ❌                    |                                              |
| `kling-v2-6`        | pro            | ✅                    | ✅                    | ❌                    | 유일하게 배경음악을 동시에 지원하는 비 v3 모델                             |
...
- `mode=4k`는 단지 `kling-v3`와 `kling-v3-omni`가 지원하며, 카메라 제어와 상충됨.
...
- `generate_audio`는 기본값이 `false`이며, 단지 `kling-v3`、`kling-v3-omni`와 `kling-v2-6`（pro 모드）가 지원함.
...
- `mode`：비디오 생성을 위한 모드, 선택 가능한 값은 표준 모드 `std`、초고속 모드 `pro` 및 원본 4K 모드 `4k`（단지 `kling-v3`와 `kling-v3-omni`가 지원하며, 카메라 제어와 호환되지 않음）입니다.
...
`kling-o1`과 `kling-v3-omni`는 두 개의 독립 모델로, 두 모델 모두 "전방위 참조" 기능을 지원합니다. 텍스트에서 비디오(`action=text2video`)를 생성하는 것에 추가하여 참조 이미지나 참조 비디오를 추가하여 **다중 이미지 참조, 참조 비디오 및 기존 비디오 직접 편집**을 구현할 수 있습니다.
...
> 안전 설명: 현재 API는 `element_list`를 개방하지 않습니다. Kling Element Library의 ID는 테넌트 격리가 아니며, 테넌트 격리의 Element Management API가 제공되기 전까지는 `image_list`를 사용하여 주 참조 이미지를 전달해야 합니다.
...
- `video_url`：참조 비디오 링크, 비어 있을 수 없습니다. 최대 1개의 MP4/MOV 비디오, 파일 크기 ≤200MB, 프레임 속도 24–60fps. `kling-o1`은 길이 3–10초, 너비와 높이 각각 700–2160px; `kling-v3-omni`는 길이 3–15.5초, 너비와 높이 각각 700–4553px, 총 픽셀 ≤8,294,400, 너비 높이 비율 0.4–2.
- `refer_type`：참조 유형, 선택 사항 `base`(기본값, **편집할 기본 비디오**, 즉 "비디오를 직접 편집", 요소 추가/삭제/수정, 구성 변경, 스타일 변경, 색상 변경, 날씨 변경 등) 또는 `feature`(**특징 참조**, 스타일 / 촬영 / 다음 장면 촬영을 참조).
- `keep_original_sound`：원본 비디오 오디오를 유지할지 여부, 선택 사항 `yes`(유지) 또는 `no`(제거).

> 주의: 참조 비디오가 있을 경우, `generate_audio`는 `false`여야 합니다. `refer_type=base`의 비디오는 첫 프레임 / 마지막 프레임을 다시 지정할 수 없습니다.
...
- `image_url`：참조 이미지 링크, 비어 있을 수 없습니다. 요구 사항: 형식 .jpg/.jpeg/.png; 파일 크기 ≤10MB; 최단 변 ≥300px; 너비 높이 비율 1:2.5 ~ 2.5:1.
- `type`：선택 사항. 전달하지 않으면 순수 참조 이미지로 간주; `first_frame` / `end_frame`을 전달하면 각각 첫 프레임 / 마지막 프레임으로 사용됩니다(동일한 `start_image_url` / `end_image_url`과 동등).

사용 시 `prompt`에서 `&lt;&lt;<image_1>>>`, `&lt;&lt;<image_2>>>`로 참조해야 합니다. 수량 제한: 참조 비디오가 없을 경우 참조 이미지 ≤ 7; 참조 비디오가 있을 경우 참조 이미지 ≤ 4. 첫 / 마지막 프레임만 전달할 경우 `start_image_url` / `end_image_url`을 직접 사용할 수 있지만, 마지막 프레임은 첫 프레임과 함께 사용해야 합니다.

> 주의: `start_image_url` / `end_image_url`과 `image_list`를 동시에 전달할 경우, 첫 / 마지막 프레임은 `image_list` 이전에 배치되어 `&lt;&lt;<image_N>>>`의 번호 대응 관계에 영향을 미칠 수 있습니다. 하나를 선택하는 것이 좋습니다: 첫 / 마지막 프레임이 필요할 경우 `image_list`에서 `type`으로 지정하고, `start_image_url` / `end_image_url`과 혼용하지 마십시오.
...
Kling Videos Generation API의 생성 시간은 상대적으로 길어 약 1-2분이 소요되며, API가 오랜 시간 응답하지 않을 경우 HTTP 요청은 계속 연결을 유지하여 추가 시스템 리소스 소모를 초래할 수 있습니다. 따라서 이 API는 비동기 콜백 지원도 제공합니다.
...
이 URL을 복사하여 Webhook으로 사용할 수 있으며, 이 샘플은 `https://webhook.site/624b2c78-6dbd-4618-9d2b-b32eade6d8c3`입니다.
...
잠시 기다리면 `https://webhook.site/624b2c78-6dbd-4618-9d2b-b32eade6d8c3`에서 생성된 비디오 결과를 확인할 수 있습니다.
