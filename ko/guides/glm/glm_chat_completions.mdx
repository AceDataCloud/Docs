...
GLM(General Language Model)은 지표 AI(Zhipu AI / Z.ai)가 출시한 차세대 대형 언어 모델 시리즈로, 강력한 중문 및 영문 이해 및 생성 능력을 갖추고 있으며, 중국어 환경, 코드 생성, 추론 및 다중 회화 등 작업에서 뛰어난 성능을 보입니다. GLM-5.1, GLM-4.7, GLM-4.6 등 차세대 모델은 긴 문맥, 도구 호출 및 코드 작업에서 많은 최적화를 이루어, 스마트 Q&A, 콘텐츠 창작, 코드 보조, 고객 서비스 로봇 등 다양한 분야에 널리 활용될 수 있습니다.

이 문서는 GLM 채팅 완료 API의 사용 프로세스를 주로 소개하며, 이를 통해 통합된 OpenAI 호환 인터페이스를 통해 GLM 시리즈 모델을 쉽게 호출할 수 있습니다. Coze(코지)에서 `glm-5.2`를 사용하려면 [Coze 사용자 정의 모델 튜토리얼](https://platform.acedata.cloud/documents/coze-integration)을 참조하고 OpenAI Chat Completions 프로토콜을 선택하십시오.
...
GLM 채팅 완료 API의 요청 주소는 `https://api.acedata.cloud/glm/chat/completions`이며, Bearer Token으로 인증합니다. 요청 본문은 OpenAI Chat Completions 프로토콜과 호환됩니다.

이 인터페이스를 처음 사용할 때는 최소한 세 가지 내용을 입력해야 합니다:
...
- `temperature`: 생성의 무작위성으로, 0-2 사이의 값이며, 값이 클수록 더 분산됩니다.
...
응답에서 여러 개의 `data`가 있으며, 각 `data`는 증분 조각을 포함하고 있습니다. `choices[i].delta.content`는 현재 청크에 추가된 텍스트 조각이며, 이러한 조각을 연결하여 전체 응답을 형성할 수 있습니다. `data` 내용이 `[DONE]`일 때는 스트리밍 응답이 종료됨을 나타냅니다. 마지막으로 `usage`가 있는 청크는 이번 요청의 토큰 사용량을 요약합니다.
...
다중 회화 기능을 구현하려면 이전 대화를 순차적으로 `messages` 배열에 넣고 `user`와 `assistant`가 번갈아 나타나는 순서를 유지해야 합니다.
...
payload = {
    "model": "glm-4.7",
    "messages": [
        {"role": "system", "content": "당신은 숙련된 한국어 작문 도우미입니다. 간결하고 전문적인 어조로 응답해 주세요."},
...
- `429 too_many_requests`：빈도 제한이 발생함, 잠시 후 다시 시도하십시오.
