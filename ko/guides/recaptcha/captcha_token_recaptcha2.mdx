...
본 문서는 Recaptcha2 프로토콜 인식 API 연동 설명을 소개하며, 사용자가 Recaptcha2 인증 이미지의 인식 및 클릭 없이 Website Key를 제출하기만 하면 백엔드에서 자동으로 디코딩하여 인증을 완료할 수 있습니다.
...
먼저 기본 사용 방식을 이해해야 합니다. 처리할 인증 코드가 있는 웹사이트의 URL을 입력하면 처리된 결과를 얻을 수 있습니다. 먼저 간단히 `website_url` 필드를 전달해야 하며, 우리의 예시 웹사이트는 `https://www.google.com/recaptcha/api2/demo`입니다. `website_url` 페이지에서 `website_key`를 얻어야 하며, 이 웹페이지를 열고 F12를 눌러 콘솔에 들어간 후 Element 페이지에서 전역 검색으로 `recaptcha-demo`를 검색하면 아래와 같은 결과를 얻을 수 있습니다:
...
- `website_url`: 처리할 인증 코드가 있는 웹사이트의 URL.
- `website_key`: Recaptcha2에서의 웹사이트 키 식별자.
...
우리는 Recaptcha2 캡차 처리의 검증 결과를 얻었으며, 이를 POST하거나 목표 웹사이트에 제출하는 데 사용할 수 있습니다. 이는 일회용이며 유효 기간은 120초입니다. 60초 이내에 사용하는 것이 좋습니다. 다음으로, 처리된 토큰을 목표 웹사이트에 제출하여 Recaptcha2 캡차를 통과하는 Python 버전을 제공하겠습니다.
...
그런 다음 코드를 실행한 후 콘솔에서 다음과 같은 결과를 관찰했습니다:
...
기본적으로 API는 동기 차단 방식입니다: 한 번의 요청은 토큰 처리가 완료될 때까지 기다립니다. 만약 여러 해독기를 회전시키는 작업을 하고 있다면, "작업을 제출한 후 즉시 task_id를 받아 다른 해독기를 조정하고, 나중에 결과를 가져오고 싶다"면 요청 본문에 `async: true`를 전달할 수 있습니다.
