...
먼저 기본 사용 방식을 이해해야 합니다. 처리할 인증 코드가 있는 웹사이트의 URL을 입력하면 처리된 결과를 얻을 수 있습니다. 먼저 간단히 `website_url` 필드를 전달해야 합니다. 우리의 예제 웹사이트는 `https://www.google.com/recaptcha/api2/demo`입니다. `website_url` 페이지에서 `website_key`를 가져와야 하며, 이 웹페이지를 열고 F12를 눌러 콘솔에 들어간 후 Element 페이지에서 전역 검색을 통해 `recaptcha-demo`를 검색하면 아래와 같은 결과를 얻을 수 있습니다:
...
- `authorization`: API 호출을 위한 키, 신청 후 드롭다운에서 직접 선택할 수 있습니다.

또한 Request Body를 설정했습니다. 포함된 내용은 다음과 같습니다:

- `website_url`: 처리할 인증 코드가 있는 웹사이트의 URL입니다.
- `website_key`: Recaptcha2에서의 웹사이트 키 식별자입니다.
- `proxy`: 선택 사항, 자체 프록시(Bring Your Own Proxy). 설정 후 상류에서 제공한 프록시 IP를 통해 인증 코드를 해독하여 출구 IP 품질을 제어합니다(예: 공용 프록시 IP가 대상 사이트에 의해 차단되어 `410 Gone`이 반환되는 것을 방지). 형식은 `scheme://[user:pass@]host:port`이며, `scheme`은 `http`/`https`/`socks4`/`socks5`를 지원합니다. 예: `http://user:pass@1.2.3.4:8080`. 입력하지 않으면 플랫폼 기본 프록시를 사용합니다.
...
- `token`은 이번 Recaptcha2 캡차 작업 처리 후의 검증 결과입니다.
- `started_at` 및 `finished_at`은 이번 요청의 처리 시작 및 결과 생성 시간을 나타내며, Unix 타임스탬프(초, 부동 소수점)입니다.
- `elapsed`는 이번 처리의 총 소요 시간(초)입니다.

우리는 Recaptcha2 캡차의 검증 결과를 얻었으며, 이를 POST하거나 목표 웹사이트에 제출하는 데 사용할 수 있습니다. 이는 일회용이며 유효 기간은 120초입니다. 60초 이내에 사용하는 것이 좋습니다. 다음으로, 처리된 토큰을 목표 웹사이트에 제출하여 Recaptcha2 캡차를 통과하는 Python 버전을 제공하겠습니다.
...
기본적으로 API는 동기 차단 방식입니다: 한 번의 요청은 토큰 처리가 완료될 때까지 기다립니다. 만약 여러 해독기 회전을 수행하고 싶다면(다중 해독기 회전), "작업을 제출한 후 즉시 task_id를 받아 다른 해독기를 조정하고, 나중에 결과를 가져오고 싶다"면 요청 본문에 `async: true`를 전달할 수 있습니다.
...
그런 다음 해당 `task_id`를 사용하여 `POST /captcha/tasks`를 폴링하여 결과를 가져옵니다(권장: 3~5초마다):
...
과금 설명: 비동기 모드에서는 작업 생성 및 "처리 중" 폴링은 과금되지 않습니다; **결과를 성공적으로 가져올 때만 한 번 과금됩니다**(동기 모드의 가격과 동일). 따라서 회전 중에 완료되지 않은 작업을 취소해도 비용이 발생하지 않습니다. `/captcha/tasks`는 모든 CAPTCHA 인터페이스(토큰 및 인식 시리즈)에 공통적으로 사용되며, 동일한 `task_id`로 폴링하면 됩니다.
...
이 문서를 통해 Recaptcha2 프로토콜 인식 API를 사용하여 사용자가 Recaptcha2 CAPTCHA 이미지를 인식하고 클릭할 필요 없이, 단지 Website Key를 제출하여 백그라운드에서 자동으로 해독하고 검증을 완료하는 방법을 이해하셨습니다. 이 문서가 API를 더 잘 연동하고 사용하는 데 도움이 되기를 바랍니다. 질문이 있으시면 언제든지 기술 지원 팀에 문의해 주시기 바랍니다.
