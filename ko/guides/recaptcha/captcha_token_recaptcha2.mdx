...
로그인 또는 등록이 되어 있지 않으면 자동으로 로그인 페이지로 리디렉션되어 등록 및 로그인을 요청합니다. 완료 후에는 자동으로 현재 페이지로 돌아옵니다.
...
먼저 기본 사용 방식을 이해해야 합니다. 처리할 인증 코드가 있는 웹사이트의 URL을 입력하면 처리된 결과를 얻을 수 있습니다. 먼저 간단히 `website_url` 필드를 전달해야 합니다. 우리의 예시 웹사이트는 `https://www.google.com/recaptcha/api2/demo`이며, `website_url` 페이지에서 `website_key`를 얻어야 합니다. 이 웹페이지를 열고 F12를 눌러 콘솔에 들어간 후, Element 페이지에서 전역 검색을 통해 `recaptcha-demo`를 검색하면 아래와 같은 결과를 얻을 수 있습니다:
...
- `proxy`: 선택 사항으로, 자체 프록시(Bring Your Own Proxy)를 사용할 수 있습니다. 설정 후 상류에서 제공한 프록시 IP를 통해 인증 코드를 해독하여 출구 IP 품질을 제어합니다(예: 공용 프록시 IP가 대상 사이트에 의해 차단되어 `410 Gone`이 반환되는 것을 피하기 위해). 형식은 `scheme://[user:pass@]host:port`이며, `scheme`은 `http`/`https`/`socks4`/`socks5`를 지원합니다. 예를 들어 `http://user:pass@1.2.3.4:8080`입니다. 입력하지 않으면 플랫폼의 기본 프록시를 사용합니다.

선택 후 오른쪽에서 해당 코드도 생성된 것을 확인할 수 있습니다. 아래와 같이 표시됩니다:
...
- `started_at`, `finished_at`: 이번 요청의 처리 시작 및 결과 생성 시간(ISO-8601 UTC)입니다.
...
먼저, 웹사이트가 POST 요청을 어떻게 보내는지 확인해야 합니다. 그래야 생성된 토큰을 전달할 수 있습니다. 먼저 F12 콘솔을 열고, 수동으로 인증을 진행한 후, 마지막으로 웹사이트가 POST 요청을 보냈음을 확인할 수 있습니다. 우리는 이 POST 요청의 구성을 살펴보면 됩니다. 구체적인 과정은 다음과 같습니다:

- 먼저 수동으로 인증을 진행합니다. 구체적인 내용은 아래 그림과 같습니다:

<p><img src="https://cdn.acedata.cloud/gwiflr.png" width="500" className="m-auto" /></p>

- 그런 다음 submit을 클릭하고, 콘솔의 네트워크 변화를 관찰합니다. 구체적인 내용은 아래 그림과 같습니다:
...
기본적으로 API는 동기 차단 방식입니다: 한 번의 요청은 토큰 처리가 완료될 때까지 기다립니다. 만약 여러 해독기를 회전시키고 싶다면(다중 해독기 회전), "작업을 제출한 후 즉시 task_id를 받아 다른 해독기를 조정하고, 나중에 결과를 가져오고 싶다"면 요청 본문에 `async: true`를 전달할 수 있습니다.
...
그런 다음 해당 `task_id`를 사용하여 `POST /captcha/tasks`를 폴링하여 결과를 가져옵니다(권장 주기: 3~5초):
...
과금 설명: 비동기 모드에서는 작업 생성과 "처리 중" 폴링은 과금되지 않습니다; **결과를 성공적으로 가져올 때만 한 번 과금됩니다**(동기 모드의 가격과 동일). 따라서 회전 중에 완료되지 않은 작업을 취소해도 비용이 발생하지 않습니다. `/captcha/tasks`는 모든 CAPTCHA 인터페이스(토큰 및 인식 시리즈)에 공통적으로 사용되며, 동일한 `task_id`로 폴링하면 됩니다.
...
이 문서를 통해 Recaptcha2 프로토콜 인식 API를 사용하여 사용자가 Recaptcha2 CAPTCHA 이미지를 인식하고 클릭할 필요 없이, 단지 Website Key를 제출함으로써 백그라운드에서 자동으로 해독하고 검증을 완료하는 방법을 이해하셨습니다. 이 문서가 API를 더 잘 연동하고 사용하는 데 도움이 되기를 바랍니다. 질문이 있으시면 언제든지 기술 지원 팀에 문의해 주시기 바랍니다.
