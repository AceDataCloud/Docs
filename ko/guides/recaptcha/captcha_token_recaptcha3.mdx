...
f12를 열고 Element 페이지에서 `.execute(`를 검색합니다. 빨간 상자 영역에서 `action` 매개변수를 확인할 수 있으며, 동시에 execute 뒤에 따라오는 문자열도 필요합니다. 구체적인 내용은 아래 그림과 같습니다.
...
우리는 Recaptcha3验证码의 검증 결과를 얻었으며, 이를 POST 또는 GET으로 목표 웹사이트에 제출하는 데 사용할 수 있습니다. 이는 일회용이며 유효 기간은 120초로, 60초 이내에 사용하는 것이 좋습니다. 아래는 생성된 token을 목표 웹사이트에 제출하는 방법을 간략히 소개합니다:
...
기본적으로 API는 동기 차단 방식입니다: 한 번의 요청은 토큰 처리가 완료될 때까지 기다립니다. 만약 여러 해독기를 회전시키고 싶다면(다중 해독기 회전), "작업 제출 후 즉시 task_id를 받고, 다른 해독기를 조정한 후 나중에 결과를 가져오고 싶다"면 요청 본문에 `async: true`를 전달할 수 있습니다.
...
본 문서를 통해 Recaptcha3 프로토콜 인식 API를 사용하여 사용자가 Recaptcha3 CAPTCHA 이미지를 인식하고 클릭할 필요 없이, 웹사이트 키를 제출하여 백엔드에서 자동으로 디코딩하고 검증을 완료하는 방법을 이해하셨습니다. 본 문서가 API를 더 잘 연동하고 사용하는 데 도움이 되기를 바랍니다. 질문이 있으시면 언제든지 기술 지원 팀에 문의해 주시기 바랍니다.
