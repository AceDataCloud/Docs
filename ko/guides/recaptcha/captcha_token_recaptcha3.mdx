...
먼저 기본 사용 방식을 이해해야 하며, Recaptcha2와 비교할 때 추가로 `page_action`이라는 매개변수를 전달해야 합니다. 이 매개변수는 코드에서 가져와야 하며, 이번에 보여줄 웹 속도 URL은 `https://recaptcha-demo.appspot.com/recaptcha-v3-request-scores.php`입니다. 아래는 이를 얻는 방법 중 하나를 보여줍니다.
...
이 문서를 통해 Recaptcha3 프로토콜을 사용하여 사용자가 Recaptcha3 CAPTCHA 이미지를 인식하고 클릭할 필요 없이, 웹사이트 키를 제출하여 백그라운드에서 자동으로 디코딩하고 검증을 완료하는 방법을 이해하셨습니다. 이 문서가 API를 더 잘 연결하고 사용하는 데 도움이 되기를 바랍니다. 질문이 있으시면 언제든지 기술 지원 팀에 문의해 주십시오.
