...
먼저 기본 사용 방법을 이해해야 하며, Recaptcha2와 비교할 때 추가로 `page_action`이라는 매개변수를 전달해야 합니다. 이 매개변수는 코드에서 가져와야 하며, 이번에 보여줄 웹 속도 URL은 `https://recaptcha-demo.appspot.com/recaptcha-v3-request-scores.php`입니다. 아래는 이를 얻는 방법 중 하나입니다:
...
결과는 여러 개의 필드로 구성되어 있으며, 설명은 다음과 같습니다:

- `token`은 이번 Recaptcha3验证码 작업 처리 후의 검증 결과입니다.
- `started_at` 및 `finished_at`은 이번 요청의 처리 시작 및 결과 생성 시간으로, Unix 타임스탬프(초, 부동 소수점)입니다.
- `elapsed`는 이번 처리의 총 소요 시간(초)입니다.

우리는 Recaptcha3验证码의 처리 결과를 얻었으며, 이를 POST 또는 GET으로 목표 웹사이트에 제출하는 데 사용할 수 있습니다. 이는 일회용이며 유효 기간은 120초로, 60초 이내에 사용하는 것이 좋습니다. 아래는 생성된 token을 목표 웹사이트에 제출하는 방법을 간략히 소개합니다:
...
기본적으로 API는 동기 차단 방식입니다: 한 번의 요청은 토큰 처리가 완료될 때까지 기다립니다. 만약 여러 해독기를 회전시키고 싶다면(다중 해독기 회전), "작업 제출 후 즉시 task_id를 받아 다른 해독기를 조정하고 나중에 결과를 가져오고 싶다"면 요청 본문에 `async: true`를 전달할 수 있습니다.
...
이후 해당 `task_id`를 사용하여 `POST /captcha/tasks`를 폴링하여 결과를 가져옵니다(권장 주기: 3~5초):
...
과금 설명: 비동기 모드에서는 작업 생성과 "처리 중" 폴링은 과금되지 않습니다; **결과를 성공적으로 가져올 때만 한 번 과금됩니다**(동기 모드와 가격이 동일). 따라서 회전 중에 완료되지 않은 작업을 취소해도 비용이 발생하지 않습니다. `/captcha/tasks`는 모든 CAPTCHA 인터페이스(토큰 및 인식 시리즈)에 공통적으로 사용되며, 동일한 `task_id`로 폴링하면 됩니다.
