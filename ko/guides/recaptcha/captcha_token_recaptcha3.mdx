...
본 문서는 Recaptcha3 프로토콜 인식 API 연동 설명을 소개하며, 사용자가 Recaptcha3 인증 이미지의 인식 및 클릭 없이 Website Key를 제출하기만 하면 백엔드에서 자동으로 해독하여 인증을 완료할 수 있도록 합니다.
...
먼저 기본 사용 방식을 이해해야 하며, Recaptcha2와 비교할 때 추가로 `page_action`이라는 매개변수를 전달해야 합니다. 이 매개변수는 코드에서 가져와야 하며, 이번에 보여줄 웹 속도 URL은 `https://recaptcha-demo.appspot.com/recaptcha-v3-request-scores.php`입니다. 아래는 이를 얻는 방법 중 하나입니다:
...
우리는 Recaptcha3验证码의 처리 결과를 얻었으며, 이를 POST 또는 GET으로 목표 웹사이트에 제출하는 데 사용할 수 있습니다. 이는 일회용이며 유효 기간은 120초로, 60초 이내에 사용하는 것이 좋습니다. 아래는 생성된 토큰을 목표 웹사이트에 제출하는 방법을 간략히 소개합니다:

토큰 검증에 해당하는 Python 버전 코드 호출:
...
기본적으로 API는 동기 차단 방식입니다: 한 번의 요청은 토큰 처리가 완료될 때까지 기다립니다. 만약 여러 해독기를 회전시키고 싶다면(다중 해독기 회전), "작업 제출 후 즉시 task_id를 받아 다른 해독기를 조정하고 나중에 결과를 읽으러 돌아오고 싶다면", 요청 본문에 `async: true`를 전달할 수 있습니다.
...
진행 상황을 적극적으로 확인하려면 해당 `task_id`를 사용하여 `POST /captcha/tasks`를 조회할 수 있습니다(권장: 3~5초마다). 이 인터페이스는 작업 처리를 촉발하거나 진행하지 않습니다; 조회하지 않거나, 네트워크가 끊기거나, 클라이언트를 종료하더라도 서버는 계속해서 처리를 진행합니다:
...
과금 설명: 비동기 모드에서는 작업 생성 및 "처리 중" 상태 조회는 과금되지 않습니다; **클라이언트가 최초로 성공 결과를 읽을 때 한 번 과금됩니다**(기존 동기 모드 가격과 동일). 서버는 작업을 자율적으로 진행하지만, 백그라운드에서 먼저 완료되더라도 미리 과금되지 않습니다. 작업이 120초 이내에 성공하지 않으면 HTTP 504 `timeout`으로 종료되며, 과금되지 않습니다. `/captcha/tasks`는 작업을 진행하는 책임이 없습니다.
...
본 문서를 통해 Recaptcha3 프로토콜 인식 API를 사용하여 사용자가 Recaptcha3 인증 이미지의 인식 및 클릭 없이 단순히 Website Key를 제출하여 백그라운드에서 자동으로 해독하고 인증을 완료하는 방법을 이해하셨습니다. 본 문서가 귀하가 API를 더 잘 연동하고 사용하는 데 도움이 되기를 바랍니다. 질문이 있으시면 언제든지 기술 지원 팀에 문의해 주시기 바랍니다.
