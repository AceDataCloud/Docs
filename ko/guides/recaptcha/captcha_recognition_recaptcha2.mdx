...
우리는 Recaptcha2 인증 이미지의 검증 결과를 얻었으며, 먼저 인증 이미지를 영역으로 나누었습니다. 아래 그림과 같습니다:
...
요금 설명: 비동기 모드에서는 작업 생성 및 "처리 중" 폴링이 요금이 부과되지 않습니다; **인식 결과를 성공적으로 가져올 때만 한 번 요금이 부과됩니다**(동기 모드의 가격과 동일). 따라서 회전 중에 완료되지 않은 작업을 취소해도 비용이 발생하지 않습니다. `/captcha/tasks`는 모든 CAPTCHA 인터페이스(토큰 및 인식 시리즈)에 공통적으로 사용되며, 동일한 `task_id`로 폴링하면 됩니다.
