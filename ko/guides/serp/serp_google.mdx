---
title: "Google SERP API 연동 설명"
description: "Search Engine API guide - Ace Data Cloud"
---

Google SERP(검색 엔진 결과 페이지)는 사용자가 Google 검색 엔진에 쿼리를 입력한 후 보는 결과 페이지입니다. 이는 자연 검색 결과, 광고, 특징 요약, 지식 그래프 및 이미지, 비디오 등 다양한 콘텐츠를 표시하여 사용자에게 가장 관련성 높은 정보를 제공하는 것을 목표로 합니다.

본 문서에서는 Google SERP API에 대해 자세히 설명하며, 이는 Google 검색 엔진에 쿼리를 입력하여 결과를 얻을 수 있도록 해주며, 결과의 내용은 특징 요약, 지식 그래프 및 이미지 등 여러 유형을 포함합니다.

본 문서에서는 Google SERP API의 연동 설명을 소개합니다.

## 신청 절차

Google SERP API를 사용하려면 먼저 [Ace Data Cloud 콘솔](https://platform.acedata.cloud/console/applications)에서 API Token을 받아야 하며, 이를 보관해 두십시오.

![](https://cdn.acedata.cloud/dvc3cg.jpg)

로그인 또는 등록이 되어 있지 않은 경우, 자동으로 로그인 페이지로 리디렉션되어 등록 및 로그인을 초대하며, 완료 후 현재 페이지로 자동으로 돌아옵니다.

**하나의 API Token으로 플랫폼의 모든 서비스를 호출할 수 있으며, 각 서비스마다 별도로 신청할 필요가 없습니다.** 최초 신청 시 무료 한도가 제공되어 무료로 체험할 수 있으며, 한도가 부족할 경우 [콘솔](https://platform.acedata.cloud/console/coin)에서 일반 잔액을 충전할 수 있습니다.

> 📘 전체 문서: [Google SERP API →](https://platform.acedata.cloud/documents/serp-google)

## 기본 사용

먼저 기본 사용 방식을 이해해야 합니다. 즉, 검색 리소스 유형과 키워드를 입력하면 검색 결과를 얻을 수 있으며, `query` 필드를 간단히 전달하고 해당 모델을 지정하면 됩니다.

예를 들어 "apple inc"에 대한 정보를 찾는 경우, 다음과 같이 인터페이스에 해당 내용을 입력할 수 있습니다. 아래 그림과 같이:

<p><img src="https://cdn.acedata.cloud/lnqiye.png" width="500" className="m-auto" /></p>

여기서 우리는 Request Headers를 설정했습니다. 포함된 내용은 다음과 같습니다:

- `accept`: 어떤 형식의 응답 결과를 받고 싶은지, 여기서는 `application/json`으로 JSON 형식으로 입력합니다.
- `authorization`: API 호출을 위한 키로, 신청 후 드롭다운에서 직접 선택할 수 있습니다.

또한 Request Body를 설정했습니다. 포함된 내용은 다음과 같습니다:

- `type`: 검색 리소스의 유형으로, 현재는 6가지 유형만 지원되며 기본값은 `search`입니다.
- `query`: 검색할 키워드입니다.
- `country`: 검색 결과의 국가를 사용자 정의하며, 기본값은 미국(US)입니다.
- `language`: 검색 결과의 언어를 사용자 정의하며, 기본값은 영어(en)입니다.
- `range`: 검색 결과의 시간 범위를 사용자 정의하며, 기본값은 제한이 없습니다.
- `number`: 검색 결과의 페이지 크기를 사용자 정의하며, 기본값은 10입니다.
- `page`: 검색 결과의 페이지 수를 사용자 정의하며, 기본값은 1입니다.

선택 후, 오른쪽에 해당 코드가 생성된 것을 확인할 수 있습니다. 아래 그림과 같이:

<p><img src="https://cdn.acedata.cloud/1j81zr.png" width="500" className="m-auto" /></p>

"Try" 버튼을 클릭하면 테스트를 진행할 수 있으며, 위 그림과 같이 다음과 같은 결과를 얻을 수 있습니다:
```json
{
  "knowledge_graph": {
    "title": "애플",
    "type": "기술 회사",
    "website": "http://www.apple.com/",
    "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT5ITHsQzdzkkFWKinRe1Y4FUbC_Vy3R_M&s=0",
    "description": "애플 주식회사는 미국의 다국적 기업이자 기술 회사로, 실리콘 밸리의 캘리포니아 쿠퍼티노에 본사를 두고 있습니다. 소비자 전자 제품, 소프트웨어 및 서비스로 가장 잘 알려져 있습니다.",
    "description_source": "위키백과",
    "description_link": "https://en.wikipedia.org/wiki/Apple_Inc.",
    "attributes": {
      "고객 서비스": "1 (800) 275-2273",
      "창립자": "스티브 잡스, 스티브 워즈니악, 로널드 웨인",
      "설립": "1976년 4월 1일, 로스 알토스, CA",
      "본사": "쿠퍼티노, CA",
      "CEO": "팀 쿡 (2011년 8월 24일–)"
    }
  },
  "organic": [
    {
      "title": "애플",
      "link": "https://www.apple.com/",
      "snippet": "애플의 혁신적인 세계를 발견하고 모든 아이폰, 아이패드, 애플 워치, 맥, 애플 TV를 쇼핑하며, 액세서리, 엔터테인먼트 등을 탐색하세요.",
      "sitelinks": [
        {
          "title": "지원",
          "link": "https://support.apple.com/"
        },
        {
          "title": "경력 기회",
          "link": "https://www.apple.com/careers/us/"
        },
        {
          "title": "비즈니스",
          "link": "https://www.apple.com/business/"
        },
        {
          "title": "스토어",
          "link": "https://www.apple.com/store"
        },
        {
          "title": "투자자 관계",
          "link": "https://investor.apple.com/investor-relations/default.aspx"
        }
      ],
      "position": 1
    },
    {
      "title": "애플 주식회사 - 위키백과",
      "link": "https://en.wikipedia.org/wiki/Apple_Inc.",
      "snippet": "애플 주식회사는 미국의 다국적 기업이자 기술 회사로, 실리콘 밸리의 캘리포니아 쿠퍼티노에 본사를 두고 있습니다.",
      "position": 2
    },
    {
      "title": "애플 주식회사 | 역사, 제품, 본사 및 사실 - 브리태니카",
      "link": "https://www.britannica.com/money/Apple-Inc",
      "snippet": "개인용 컴퓨터, 스마트폰 및 태블릿 컴퓨터의 미국 제조업체입니다. 애플은 최초의 성공적인 개인용 컴퓨터 회사였습니다.",
      "date": "7일 전",
      "position": 3
    },
    {
      "title": "애플 주식회사 (AAPL) 회사 프로필 및 사실 - 야후 파이낸스",
      "link": "https://finance.yahoo.com/quote/AAPL/profile/",
      "snippet": "애플 주식회사는 스마트폰, 개인용 컴퓨터, 태블릿, 웨어러블 및 액세서리를 전 세계적으로 설계, 제조 및 판매합니다. 이 회사는 아이폰, ...",
      "position": 4
    },
    {
      "title": "AAPL: 애플 주식 가격 인용 - NASDAQ GS - 블룸버그",
      "link": "https://www.bloomberg.com/quote/AAPL:US",
      "snippet": "애플 주식회사는 스마트폰, 개인용 컴퓨터, 태블릿, 웨어러블 및 액세서리를 설계, 제조 및 판매하며, 다양한 관련 액세서리를 판매합니다.",
      "position": 5
    },
    {
      "title": "애플 주식회사 (AAPL) 오늘의 주식 가격 - WSJ",
      "link": "https://www.wsj.com/market-data/quotes/AAPL?gaa_at=eafs&gaa_n=ASWzDAiLDLUKmLSxoaZi5n84id-eGDp9gI99K5VU1R7pK3SzSi_PbdpD61P1&gaa_ts=685148c8&gaa_sig=mGk3lG_2W7ZqrXfXSXv7GAP78KUi_uJEadAXuv2bgYJ8Ah42GhIZxrsN4ynbNg_oKD_7NZS1lRPSglUNacIheQ%3D%3D",
      "snippet": "애플 주식회사는 스마트폰, 개인용 컴퓨터, 태블릿, 웨어러블 및 액세서리의 설계, 제조 및 판매에 참여하며, 기타 다양한 관련 제품을 판매합니다.",
      "position": 6
    }
  ],
  "people_also_ask": [
    {
      "question": "애플 주식회사는 무엇인가요?",
      "snippet": "애플 주식회사, 원래 애플 컴퓨터 주식회사, 는 소비자 전자 제품과 관련 소프트웨어를 제작 및 판매하는 다국적 기업이며, 미디어 콘텐츠의 디지털 유통업체입니다. 애플의 핵심 제품 라인은 아이폰 스마트폰, 아이패드 태블릿 컴퓨터, 맥 개인용 컴퓨터입니다.",
      "title": "애플 주식회사 역사 - 위키백과",
      "link": "https://en.wikipedia.org/wiki/History_of_Apple_Inc."
    },
    {
      "question": "애플 주식회사를 소유하고 있는 사람은 누구인가요?",
      "snippet": "애플 (AAPL) 소유 개요 애플 (AAPL) 주식의 소유 구조는 기관 투자자, 소매 투자자 및 개인 투자자의 혼합입니다. 회사 주식의 약 50.57%는 기관 투자자에 의해 소유되고, 0.06%는 내부자에 의해 소유되며, 49.38%는 공공 기업 및 개인 투자자에 의해 소유됩니다.",
      "title": "애플을 소유하고 있는 사람은 누구인가요? AAPL 주식 소유 - TipRanks.com",
      "link": "https://www.tipranks.com/stocks/aapl/ownership"
    },
    {
      "question": "애플 주식회사는 무엇으로 가장 잘 알려져 있나요?",
      "snippet": "소비자 전자 제품, 소프트웨어 및 서비스로 가장 잘 알려져 있습니다. 1976년 스티브 잡스, 스티브 워즈니악, 로널드 웨인에 의해 애플 컴퓨터 회사로 설립되었으며, 다음 해에 잡스와 워즈니악에 의해 애플 컴퓨터 주식회사로 법인화되었습니다.",
      "title": "애플 주식회사 - 위키백과",
      "link": "https://en.wikipedia.org/wiki/Apple_Inc."
    },
    {
      "question": "애플 주식회사의 Inc의 의미는 무엇인가요?",
      "snippet": "incorporated의 약어: 법적으로 설립된 미국 회사의 이름에 사용됨: Bishop Computer Services, Inc.",
      "title": "Inc. | 영어 의미 - 캠브리지 사전",
      "link": "https://dictionary.cambridge.org/dictionary/english/inc"
    }
  ],
  "related_searches": [
    {
      "query": "애플 아이폰 13"
    },
    {
      "query": "애플 주식회사는 무엇인가요"
    },
    {
      "query": "애플 주식회사의 전체 형태"
    },
    {
      "query": "애플 주식회사의 주소"
    },
    {
      "query": "애플 주식회사의 산업"
    },
    {
      "query": "애플 주식회사의 투자자 관계"
    },
    {
      "query": "애플 주식회사 연례 보고서"
    },
    {
      "query": "애플 스토어"
    }
  ]
}
```

반환 결과는 여러 필드를 포함하고 있으며, 소개는 다음과 같습니다:

- `knowledge_graph`, 검색 결과의 지식 그래프.
- `organic`, 검색 결과의 상세 정보.
- `people_also_ask`, 검색 키워드와 관련된 질문.
- `related_searches`, 검색 키워드와 관련된 검색. 

여기서 `organic` 필드에는 검색 키워드의 결과가 포함되어 있습니다.

또한, 해당 API와의 연결 코드를 생성하려면 아래와 같이 복사하여 사용할 수 있습니다. CURL의 코드는 다음과 같습니다:

```shell
curl -X POST 'https://api.acedata.cloud/serp/google' \
-H 'accept: application/json' \
-H 'authorization: Bearer {token}' \
-H 'content-type: application/json' \
-d '{
  "query": "애플 주식회사"
}'
```

Python의 연결 코드는 다음과 같습니다:

```python
import requests

url = "https://api.acedata.cloud/serp/google"

headers = {
    "accept": "application/json",
    "authorization": "Bearer {token}",
    "content-type": "application/json"
}

payload = {
    "query": "애플 주식회사"
}

response = requests.post(url, json=payload, headers=headers)
print(response.text)
```

## 사용자 정의 검색 유형

사용자가 검색 리소스의 유형을 사용자 정의하려면 `type` 매개변수를 수정할 수 있으며, 이는 일반 리소스 `search`, 이미지 리소스 `images`, 뉴스 리소스 `news`, 지도 리소스 `maps`, 지역 리소스 `places`, 비디오 리소스 `videos`를 포함합니다. 본문에서는 비디오 리소스 `videos`를 예로 들어 설명하겠습니다.

먼저, `type` 매개변수를 `videos`로 설정하고 `query` 매개변수를 정상적으로 전달합니다.
<p><img src="https://cdn.acedata.cloud/czlt12.png" width="500" className="m-auto" /></p>

해당 코드 아래와 같습니다:

```shell
curl -X POST 'https://api.acedata.cloud/serp/google' \
-H 'accept: application/json' \
-H 'authorization: Bearer {token}' \
-H 'content-type: application/json' \
-d '{
  "type": "videos",
  "query": "apple inc"
}'
```

다음과 같은 응답을 받을 수 있습니다:

```json
{
  "videos": [
    {
      "title": "Healthcare",
      "link": "https://www.apple.com/healthcare/",
      "snippet": "더 많은 쇼핑 방법: 가까운 Apple Store 또는 다른 소매점을 찾으세요. 또는 1-800-MY-APPLE (1-800-692-7753)로 전화하세요. 미국. 저작권 © 2025 Apple Inc. 모든 ...",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRQzUYLXB37srVLPW8JbVzhIlP9-SxMe3A8M98Eb7362as4&s",
      "source": "Apple",
      "date": "2025년 4월 16일",
      "position": 1
    },
    {
      "title": "Apple",
      "link": "https://www.youtube.com/apple",
      "snippet": "우리의 첫 번째 공간 컴퓨터. · 엄마를 위한 선물 | 공간 비디오에서 iPhone으로 촬영 | Apple Vision Pro에서 경험 · Metallica — 공식 예고편 | Apple ...",
      "position": 2
    },
    {
      "title": "Apple이 드디어 다시 위험을 감수하고 있나요?",
      "link": "https://www.youtube.com/watch?v=JNZV9i5BwH0",
      "snippet": "Microsoft가 어떻게 했는지, 그리고 계속해서 어려움을 겪고 있는 것을 고려할 때 Mac을 ARM으로 전환하는 것은 \"우리가 실패하면 회사를 죽이는 것\"과 같은 ...",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR0zvbxg3ozbeHNUJojmyRBhi3J-TviJQRinIS4DZiF-EM8&s",
      "video_url": "https://encrypted-vtbn0.gstatic.com/video?q=tbn:ANd9GcTfNZRuifDZb4zgj0d6wDxoHLI8yUr-8SIPNw",
      "duration": "13:04",
      "source": "YouTube",
      "channel": "Luke Miani",
      "date": "2일 전",
      "position": 3
    },
    {
      "title": "WWDC 25에서 Apple이 이야기하지 않은 것과 그 이유!",
      "link": "https://www.youtube.com/watch?v=2LTc3mzNh08",
      "snippet": "나는 분노를 유도하는 비디오를 만드는 YouTuber들을 천천히 언팔로우하고 있습니다. Apple은 완벽하지 않지만 이러한 주제들은 ...",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTgGUJwgxdH-WHBmGOk6XgvijwnYX2H_D2Cl23aNab8CKyT&s",
      "video_url": "https://encrypted-vtbn0.gstatic.com/video?q=tbn:ANd9GcRTtYLkgIrCxERepA5KmCJDbZ7lTUa0OBHqzw",
      "duration": "13:05",
      "source": "YouTube",
      "channel": "Brian Tong",
      "date": "7시간 전",
      "position": 4
    },
    {
      "title": "Apple",
      "link": "https://www.youtube.com/user/Apple",
      "snippet": "우리의 첫 번째 공간 컴퓨터. · 엄마를 위한 선물 | 공간 비디오에서 iPhone으로 촬영 | Apple Vision Pro에서 경험 · Metallica — 공식 예고편 | Apple ...",
      "position": 5
    },
    {
      "title": "Apple Watch Ultra 2",
      "link": "https://www.apple.com/apple-watch-ultra-2/",
      "snippet": "Huish Outdoors와 협력하여 설계된 Oceanic+ 앱은 Ultra 2를 Bühlmann 감압 알고리즘을 실행하는 완전한 다이빙 컴퓨터로 변환합니다.",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSvZ58COtZS4zIIJDd1-Tl0GIKbFqoffFlhLa20OtLcILDR&s",
      "source": "Apple",
      "date": "2023년 9월 12일",
      "position": 6
    },
    {
      "title": "Apple at Work - 성공 사례 - BDC",
      "link": "https://www.apple.com/business/enterprise/success-stories/financial-services/bdc/",
      "snippet": "Apple Business Manager와 AirWatch 모바일 장치 관리 기능을 통해 BDC는 장치와 앱을 회사 전반에 걸쳐 완전히 제로 터치로 배포하고 관리할 수 있습니다 ...",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRDQuxFNiitOIuPizPvTHwzQqyuLuypH_UNPqcZ-bGUAFy2&s",
      "source": "Apple",
      "date": "2021년 11월 2일",
      "position": 7
    },
    {
      "title": "고등 교육 - 기관",
      "link": "https://www.apple.com/education/higher-education/",
      "snippet": "더 많은 쇼핑 방법: 가까운 Apple Store 또는 다른 소매점을 찾으세요. 또는 1-800-MY-APPLE (1-800-692-7753)로 전화하세요. 미국. 저작권 © 2025 Apple Inc. 모든 ...",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSIZRPg0iWQmiRPY-Ro1XmdUQYbwGmIxUgMjjL8TqdyutHF&s",
      "source": "Apple",
      "date": "2018년 6월 8일",
      "position": 8
    },
    {
      "title": "Apple: 1조 달러의 배신 | 당신의 ...에 대한 진실",
      "link": "https://www.youtube.com/watch?v=wmfuE-RM7Rg",
      "snippet": "Apple은 세계에서 가장 가치 있는 회사이며 우리의 삶에서 다른 어떤 것과도 다른 역할을 하고 있습니다. 스마트폰에서 스마트워치까지, ...",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSvqw0lj-kWlq1HnowgEqzFxk5npWRpod2w1gogX-Dt4-jk&s",
      "video_url": "https://encrypted-vtbn0.gstatic.com/video?q=tbn:ANd9GcSG7N1JPdVZhrbuDckiwnYuWIQsXg_tdcmlvA",
      "duration": "52:40",
      "source": "YouTube",
      "channel": "Moconomy",
      "date": "2024년 12월 4일",
      "position": 9
    },
    {
      "title": "Apple at Work - 성공 사례 - SAP",
      "link": "https://www.apple.com/business/enterprise/success-stories/tech/sap/",
      "snippet": "SAP는 직원들에게 이러한 선택을 제공하는 것이 단순히 사람들이 자신의 컴퓨터를 선택하게 하는 것 이상이라는 것을 발견했습니다. Mac은 그들의 잠재력을 최대한 발휘합니다. 한눈에 보기. 105K+ ...",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSaplpBsRWWCJNlPtXsFX-jr3yw0mexpI6RtFKN6-TJAqrW&s",
      "source": "Apple",
      "date": "2022년 4월 13일",
      "position": 10
    }
  ]
}
```

응답 결과에는 여러 필드가 있으며, 소개는 다음과 같습니다:

- `news`, 검색 결과에서 비디오 정보 목록.

여기서 반환된 결과에는 `videos` 필드가 있으며, 이는 검색 키워드의 결과를 포함하고 있습니다.

## 사용자 정의 검색 리소스가 있는 국가

이 API는 검색 결과의 국가를 제한하는 것도 지원하며, `country` 매개변수를 추가하여 국가를 제한할 수 있습니다. 입력 매개변수는 국가의 약어로, 예를 들어 cn(중국), us(미국)입니다. 본문에서는 중국을 예로 들어 다음과 같은 정보를 제공합니다:

<p><img src="https://cdn.acedata.cloud/gztpwi.png" width="500" className="m-auto" /></p>

출력 효과는 다음과 같습니다:
```json
{
  "news": [
    {
      "title": "애플, iOS 26 베타 공개 롤아웃 7월 예정 발표",
      "link": "https://www.tv47.digital/apple-announces-ios-26-beta-public-roll-out-set-for-july-105190/",
      "snippet": "기술 대기업 애플 주식회사는 아이폰, 맥, 아이패드를 위한 최신 운영 체제 버전을 공개하며 중요한...",
      "date": "2시간 전",
      "source": "TV47 디지털",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTBNrktH4AWMt8KS3hErOkI1oAeUvRm4Aaq10qnINCa3TmyabOCwiEMvxVWsA&s",
      "position": 1
    },
    {
      "title": "브래거 이글 & 스콰이어, P.C. 애플, 세이블 및 아바쿠스를 조사 중이며 투자자들에게 연락을 권장",
      "link": "https://www.tradingview.com/news/reuters.com,2025-06-17:newsml_GNX87dzHl:0-bragar-eagel-squire-p-c-is-investigating-apple-sable-and-abacus-and-encourages-investors-to-contact-the-firm/",
      "snippet": "뉴욕, 2025년 6월 16일 (글로브 뉴스와이어) — 브래거 이글 & 스콰이어, P.C.는 전국적으로 인정받는 주주 권리 법률 회사입니다...",
      "date": "10시간 전",
      "source": "트레이딩뷰",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRdcHGK9sZ4M1wtCX6U74yV2ac9ClDX2hTSX4l4Brr1DWTvXfI4dKAn8sDU9g&s",
      "position": 2
    },
    {
      "title": "애플 주식회사 (AAPL) WWDC 소프트웨어 및 AI 발표에 대한 의견",
      "link": "https://www.nasdaq.com/articles/apple-inc-aapl-opinions-wwdc-software-and-ai-announcements",
      "snippet": "최근 X에서 애플 주식회사 (AAPL)에 대한 논의는 회사의 최신 소프트웨어 업데이트와 AI 이니셔티브에 집중되었습니다...",
      "date": "17시간 전",
      "source": "나스닥",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSm5mhn6LRgzafnn34CrjlR3LS1_fHPmKNJckasyNgN4eYgBiCqGtulLQIViA&s",
      "position": 3
    },
    {
      "title": "골드만 삭스, 애플 매수 추천, 주가 상승 예상",
      "link": "https://finance.yahoo.com/news/goldman-sachs-recommends-buying-apple-042754922.html",
      "snippet": "애플 주식회사 (NASDAQ:AAPL)는 차세대 배당 귀족 주식 중 하나입니다. 골드만 삭스는 최근 주식 그룹을 강조했습니다...",
      "date": "6시간 전",
      "source": "야후 파이낸스",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS6QgEG6x6t12L8YqJebmoiPNM_RMjmiVUpwam1YXZgKwBUPGe6yn76hR-n0g&s",
      "position": 4
    },
    {
      "title": "애플, 네덜란드 앱 스토어 항소 패소 후 추가 비준수 벌금 위협",
      "link": "https://globalcompetitionreview.com/article/apple-threatened-more-noncompliance-fines-after-losing-dutch-app-store-appeal",
      "snippet": "네덜란드 소비자 및 시장 당국은 애플의 앱 스토어 규칙 수정 명령에 대한 항소를 성공적으로 방어했습니다...",
      "date": "17시간 전",
      "source": "글로벌 경쟁 리뷰",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ02b47E8UHdyFL6A4Da4Wla_nXXc427Y9aMgE7vVuJpN4CzLQp0ip_G77FwA&s",
      "position": 5
    },
    {
      "title": "네덜란드 법원, 애플이 데이팅 앱에서 지배적 위치 남용 확인",
      "link": "https://www.reuters.com/sustainability/boards-policy-regulation/dutch-court-confirms-apple-abused-dominant-position-dating-apps-2025-06-16/",
      "snippet": "네덜란드 법원은 월요일 소비자 감시 기관의 2021년 판결을 확인하며 애플이 불공정한 조건을 부과하여 지배적 위치를 남용했다고 밝혔습니다...",
      "date": "19시간 전",
      "source": "로이터",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQLRbz3ypqJL6zJo1LBfMen68-D4csUJbGNNO7w8JODW6fiKliGk0yRyRGfvQ&s",
      "position": 6
    },
    {
      "title": "애플, 중국에서 AI 아이폰을 구할 수 있을까?",
      "link": "https://www.bloomberg.com/opinion/articles/2025-06-10/can-apple-salvage-the-ai-iphone-in-china",
      "snippet": "애플 주식회사의 인공지능 문제를 미국에서 나쁘다고 생각했다면, 실제로는 더 나쁠 수 있다는 것을 알아두세요. 중국을 보세요.",
      "date": "6일 전",
      "source": "블룸버그.com",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRzqIqxSek95GHKeeX9ekGIfzlZhz4jdEXqJPlk9Gyi5E8Yzk55iuhR5Soy_Q&s",
      "position": 7
    },
    {
      "title": "애플, 한국에 고정밀 지도 데이터 해외 전송 허용 요청",
      "link": "https://en.yna.co.kr/view/AEN20250617006800320",
      "snippet": "서울, 6월 17일 (연합뉴스) -- 미국 기술 대기업 애플 주식회사가 한국 정부에...",
      "date": "5시간 전",
      "source": "연합뉴스",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS20gcfgCXs4W5Hzu78JWocLIX_9jQPbN1rUUPctbiJ1UKd7R7eotC4CodJdQ&s",
      "position": 8
    },
    {
      "title": "네덜란드 법원, 애플의 앱 스토어 남용에 대한 판결 유지",
      "link": "https://www.webpronews.com/dutch-court-upholds-ruling-against-apples-app-store-abuse/",
      "snippet": "애플 주식회사에 대한 중대한 타격으로, 네덜란드 법원이 소비자 및 시장 당국의 2021년 판결을 유지했습니다...",
      "date": "13시간 전",
      "source": "웹프로뉴스",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSVZUsNtxTvIJYDjEGJN53uHWvK5NAGJrf-GS4wnzlSUtAbIYizew6HrYzr_w&s",
      "position": 9
    },
    {
      "title": "네덜란드 법원, 애플이 데이팅 앱에서 지배적 위치 남용 확인",
      "link": "https://seekingalpha.com/news/4458712-dutch-court-confirms-apple-abused-dominant-position-for-dating-apps",
      "snippet": "네덜란드 법원이 애플 (AAPL)이 불합리한 조건을 부과하여 지배적 위치를 남용했다는 국가 경쟁 기관의 판결을 확인했습니다...",
      "date": "2시간 전",
      "source": "시킹 알파",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSkf9LcqSBxw2DQZaDhfzZyJ0G2VRkkJ6dVHgBJFIGv_fgtPTDo2iAfQ_5fcQ&s",
      "position": 10
    }
  ]
}
```
우리는 검색 결과의 언어를 사용자 정의할 수 있으며, 여기서 추가로 `language` 필드를 추가하고, 내용은 `zh-cn`으로 설정합니다. 이는 간체 중국어를 의미하며, 다른 언어도 지원하지만 입력해야 하는 언어의 약어를 입력해야 합니다. 예를 들어 en(영어), fr(프랑스어), zh-cn(중국어(간체)) 등의 언어가 있습니다. 아래 그림과 같이:

<p><img src="https://cdn.acedata.cloud/yyrssp.png" width="500" className="m-auto" /></p>

해당 코드 예시는 다음과 같습니다:

```shell
curl -X POST 'https://api.acedata.cloud/serp/google' \
-H 'accept: application/json' \
-H 'authorization: Bearer {token}' \
-H 'content-type: application/json' \
-d '{
  "type": "news",
  "query": "apple inc",
  "language": "zh-cn"
}'
```

실행 결과는 다음과 같습니다:

```json
{
  "knowledge_graph": {
    "title": "苹果",
    "type": "公司",
    "website": "http://www.apple.com/",
    "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRgbY1KzXEpuPeTpcw0GwN6BpQTcg1m06TDUsfdG6P-zW3eWrmu78AXyg&s=0",
    "attributes": {
      "创始人": "史蒂夫·乔布斯、史蒂夫·沃兹尼亚克和罗纳德·韦恩",
      "创立于": "1976 年 4 月 1 日，加利福尼亚洛思阿图斯",
      "总部": "加利福尼亚库比蒂诺"
    }
  },
  "organic": [
    {
      "title": "Apple",
      "link": "https://www.apple.com/",
      "snippet": "Discover the innovative world of Apple and shop everything iPhone, iPad, Apple Watch, Mac, and Apple TV, plus explore accessories, entertainment, ...",
      "sitelinks": [
        {
          "title": "Support",
          "link": "https://support.apple.com/"
        },
        {
          "title": "Careers at Apple",
          "link": "https://www.apple.com/careers/us/"
        },
        {
          "title": "Store",
          "link": "https://www.apple.com/store"
        },
        {
          "title": "Business",
          "link": "https://www.apple.com/business/"
        },
        {
          "title": "Apple UK",
          "link": "https://www.apple.com/uk/"
        }
      ],
      "position": 1
    },
    {
      "title": "Apple Inc. - Wikipedia",
      "link": "https://en.wikipedia.org/wiki/Apple_Inc.",
      "snippet": "Apple Inc. is an American multinational corporation and technology company headquartered in Cupertino, California, in Silicon Valley.",
      "sitelinks": [
        {
          "title": "History",
          "link": "https://en.wikipedia.org/wiki/History_of_Apple_Inc."
        },
        {
          "title": "Litigation involving Apple Inc.",
          "link": "https://en.wikipedia.org/wiki/Litigation_involving_Apple_Inc."
        },
        {
          "title": "Apple Park",
          "link": "https://en.wikipedia.org/wiki/Apple_Park"
        },
        {
          "title": "Apple Watch",
          "link": "https://en.wikipedia.org/wiki/Apple_Watch"
        }
      ],
      "position": 2
    },
    {
      "title": "Apple Inc. | History, Products, Headquarters, & Facts - Britannica",
      "link": "https://www.britannica.com/money/Apple-Inc",
      "snippet": "Apple Inc. is an American multinational technology company that revolutionized the technology sector through its innovation of computer software, ...",
      "position": 3
    },
    {
      "title": "Apple Inc - Company Profile and News - Bloomberg Markets",
      "link": "https://www.bloomberg.com/profile/company/AAPL:US",
      "snippet": "Apple Inc. designs, manufactures, and markets smartphones, personal computers, tablets, wearables and accessories, and sells a variety of related ...",
      "position": 4
    },
    {
      "title": "Apple Inc. (AAPL) Stock Price, News, Quote & History - Yahoo Finance",
      "link": "https://finance.yahoo.com/quote/AAPL/",
      "snippet": "Apple Inc. designs, manufactures, and markets smartphones, personal computers, tablets, wearables, and accessories worldwide.",
      "date": "2024年8月27日",
      "position": 5
    },
    {
      "title": "AAPL: Apple Inc Stock Price Quote - NASDAQ GS - Bloomberg",
      "link": "https://www.bloomberg.com/quote/AAPL:US",
      "snippet": "Apple Inc. designs, manufactures, and markets smartphones, personal computers, tablets, wearables and accessories, and sells a variety of related accessories.",
      "position": 6
    },
    {
      "title": "Apple Inc. (AAPL) Company Profile & Facts - Yahoo Finance",
      "link": "https://finance.yahoo.com/quote/AAPL/profile/",
      "snippet": "See the company profile for Apple Inc. (AAPL) including business summary, industry/sector information, number of employees, business summary, ...",
      "position": 7
    }
  ],
  "people_also_ask": [
    {
      "question": "What is Apple Inc stand for?",
      "snippet": "It was incorporated as Apple Computer, Inc. in January 1977, and sales of its computers, including the Apple II , saw significant momentum and revenue growth for the company. \"Inc.\" is the abbreviation for incorporated. A corporation is a separate legal entity from the person or people forming it.",
      "title": "What does the 'Inc.' in Apple Inc. mean? - Quora",
      "link": "https://www.quora.com/What-does-the-Inc-in-Apple-Inc-mean"
    },
    {
      "question": "Who owns Apple Inc.?",
      "snippet": "Apple (AAPL) Ownership Overview The ownership structure of Apple (AAPL) stock is a mix of institutional, retail and individual investors. Approximately 48.76% of the company's stock is owned by Institutional Investors, 0.11% is owned by Insiders and 51.13% is owned by Public Companies and Individual Investors.",
      "title": "Who owns Apple? AAPL Stock Ownership - TipRanks.com",
      "link": "https://www.tipranks.com/stocks/aapl/ownership"
    },
    {
      "question": "Why did Apple change to Apple Inc?",
      "snippet": "During his keynote speech at the Macworld Expo on January 9, 2007, Jobs announced the renaming of Apple Computer, Inc. to Apple Inc., because the company had broadened its focus from computers to consumer electronics. This event also saw the announcement of the iPhone and the Apple TV.",
      "title": "Apple Inc. - Wikipedia",
      "link": "https://en.wikipedia.org/wiki/Apple_Inc."
    },
    {
      "question": "Why Apple Inc is called Apple?",
      "snippet": "With the name Apple, the new computer company would appear ahead of Atari, where Jobs used to work. Jobs confirmed this theory in an 1980 presentation, stating that the name was partly chosen because he liked apples and partly because Apple was ahead of Atari in the phone book.",
      "title": "Why Is Apple Called Apple? - Apple Scoop",
      "link": "https://applescoop.org/story/why-is-apple-called-apple"
    }
  ],
  "related_searches": [
    {
      "query": "apple inc是什么"
    },
    {
      "query": "apple澳门"
    },
    {
      "query": "Apple ID"
    },
    {
      "query": "apple美国"
    },
    {
      "query": "apple官网"
    },
    {
      "query": "apple美国官网"
    },
    {
      "query": "Apple company introduction"
    },
    {
      "query": "apple id官网"
    }
  ],
  "credits": 1
}
```

여기서 표시된 결과는 모두 간체 중국어로 되어 있으며, 결과의 내용은 위와 유사합니다.

## 사용자 정의 검색 결과 시간 범위
本文은 검색 결과의 시간 범위를 사용자 정의할 수 있으며, 다섯 가지가 포함되어 있습니다. 각각은 `qdr:h`(지난 한 시간), `qdr:d`(지난 하루), `qdr:w`(지난 일주일), `qdr:m`(지난 한 달), 기본적으로 제한 없음입니다. 우리는 `range`를 통해 해당 시간 범위를 전달할 수 있습니다. 예를 들어, 여기서 `qdr:d`로 설정하면 지난 하루의 결과를 검색하는 것을 의미하므로 입력은 다음과 같습니다:

<p><img src="https://cdn.acedata.cloud/qccfib.png" width="500" className="m-auto" /></p>

해당 코드입니다:

```shell
curl -X POST 'https://api.acedata.cloud/serp/google' \
-H 'accept: application/json' \
-H 'authorization: Bearer {token}' \
-H 'content-type: application/json' \
-d '{
  "range": "qdr:d",
  "query": "apple inc"
}'
```

실행 결과는 다음과 같습니다:

```json
{
  "answer_box": {
    "title": "Tim Cook (Aug 24, 2011–)Apple / CEO",
    "answer": "Tim Cook (Aug 24, 2011–)"
  },
  "knowledge_graph": {
    "title": "Apple",
    "type": "Technology company",
    "website": "http://www.apple.com/",
    "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT5ITHsQzdzkkFWKinRe1Y4FUbC_Vy3R_M&s=0",
    "description": "Apple Inc.는 미국의 다국적 기업이자 기술 회사로, 실리콘 밸리의 캘리포니아 쿠퍼티노에 본사를 두고 있습니다. 소비자 전자 제품, 소프트웨어 및 서비스로 가장 잘 알려져 있습니다.",
    "description_source": "Wikipedia",
    "description_link": "https://en.wikipedia.org/wiki/Apple_Inc.",
    "attributes": {
      "Customer service": "1 (800) 275-2273",
      "Founders": "Steve Jobs, Steve Wozniak, and Ronald Wayne",
      "Founded": "April 1, 1976, Los Altos, CA",
      "Headquarters": "Cupertino, CA",
      "CEO": "Tim Cook (Aug 24, 2011–)"
    }
  },
  "organic": [
    {
      "title": "Apple",
      "link": "https://www.apple.com/",
      "snippet": "Apple의 혁신적인 세계를 발견하고 모든 iPhone, iPad, Apple Watch, Mac 및 Apple TV를 쇼핑하며 액세서리, 엔터테인먼트 등을 탐색하세요.",
      "sitelinks": [
        {
          "title": "Support",
          "link": "https://support.apple.com/"
        },
        {
          "title": "Career Opportunities",
          "link": "https://www.apple.com/careers/us/"
        },
        {
          "title": "Business",
          "link": "https://www.apple.com/business/"
        },
        {
          "title": "Apple Leadership",
          "link": "https://www.apple.com/leadership/"
        },
        {
          "title": "Store",
          "link": "https://www.apple.com/store"
        }
      ],
      "position": 1
    },
    {
      "title": "Apple Inc. - Wikipedia",
      "link": "https://en.wikipedia.org/wiki/Apple_Inc.",
      "snippet": "Apple Inc.는 미국의 다국적 기업이자 기술 회사로, 실리콘 밸리의 캘리포니아 쿠퍼티노에 본사를 두고 있습니다.",
      "position": 2
    },
    {
      "title": "Apple Inc. | History, Products, Headquarters, & Facts - Britannica",
      "link": "https://www.britannica.com/money/Apple-Inc",
      "snippet": "개인용 컴퓨터, 스마트폰 및 태블릿 컴퓨터의 미국 제조업체입니다. Apple은 첫 번째 성공적인 개인용 컴퓨터 회사였습니다.",
      "date": "7 days ago",
      "position": 3
    },
    {
      "title": "Apple Inc. (AAPL) Company Profile & Facts - Yahoo Finance",
      "link": "https://finance.yahoo.com/quote/AAPL/profile/",
      "snippet": "Apple Inc.는 스마트폰, 개인용 컴퓨터, 태블릿, 웨어러블 및 액세서리를 전 세계적으로 설계, 제조 및 판매합니다. 이 회사는 iPhone, ...",
      "position": 4
    },
    {
      "title": "AAPL: Apple Inc Stock Price Quote - NASDAQ GS - Bloomberg",
      "link": "https://www.bloomberg.com/quote/AAPL:US",
      "snippet": "Apple Inc.는 스마트폰, 개인용 컴퓨터, 태블릿, 웨어러블 및 액세서리를 설계, 제조 및 판매하며 다양한 관련 액세서리를 판매합니다.",
      "position": 5
    },
    {
      "title": "Apple Inc. (AAPL) Stock Price Today - WSJ",
      "link": "https://www.wsj.com/market-data/quotes/AAPL?gaa_at=eafs&gaa_n=ASWzDAjHf1Jvm_DfBkgGitLxGwlDC2VFfkyeU3ora5VpP1k1gJJfptWjtU4o&gaa_ts=6851a5a8&gaa_sig=FmQcgCrdYtgMPhMwfFTpCtEva8_OZ5j1fx1cEBn0_THBq1EUa8A0Tmo3piNM3OQ488O2rScdH4NuSLly_DIdpQ%3D%3D",
      "snippet": "Apple Inc.는 스마트폰, 개인용 컴퓨터, 태블릿, 웨어러블 및 액세서리의 설계, 제조 및 판매에 참여하며, 기타 다양한 관련 제품을 판매합니다.",
      "position": 6
    }
  ],
  "people_also_ask": [
    {
      "question": "Apple Inc란 무엇인가요?",
      "snippet": "Apple Inc., 원래 Apple Computer, Inc.는 소비자 전자 제품과 관련 소프트웨어를 제작 및 판매하는 다국적 기업이며, 미디어 콘텐츠의 디지털 유통업체입니다. Apple의 핵심 제품 라인은 iPhone 스마트폰, iPad 태블릿 컴퓨터 및 Mac 개인용 컴퓨터입니다.",
      "title": "Apple Inc.의 역사 - Wikipedia",
      "link": "https://en.wikipedia.org/wiki/History_of_Apple_Inc."
    },
    {
      "question": "Apple Inc의 소유자는 누구인가요?",
      "snippet": "Apple (AAPL) 소유 개요 회사 주식의 약 50.57%는 기관 투자자에 의해 소유되며, 0.06%는 내부자에 의해 소유되고, 49.38%는 공공 기업 및 개인 투자자에 의해 소유됩니다. Apple (AAPL) 주식의 소유 구조는 기관, 소매 및 개인 투자자의 혼합입니다.",
      "title": "누가 Apple을 소유하고 있나요? AAPL 주식 소유 - TipRanks.com",
      "link": "https://www.tipranks.com/stocks/aapl/ownership"
    },
    {
      "question": "Apple Inc.는 무엇으로 가장 잘 알려져 있나요?",
      "snippet": "소비자 전자 제품, 소프트웨어 및 서비스로 가장 잘 알려져 있습니다. 1976년 Steve Jobs, Steve Wozniak 및 Ronald Wayne에 의해 Apple Computer Company로 설립되었으며, 다음 해에 Jobs와 Wozniak에 의해 Apple Computer, Inc.로 법인화되었습니다.",
      "title": "Apple Inc. - Wikipedia",
      "link": "https://en.wikipedia.org/wiki/Apple_Inc."
    },
    {
      "question": "현재 Apple의 소유자는 누구인가요?",
      "snippet": "Tim Cook (Aug 24, 2011–)\nApple / CEO"
    }
  ],
  "related_searches": [
    {
      "query": "Apple iPhone 13"
    },
    {
      "query": "apple inc는 무엇인가요"
    },
    {
      "query": "Apple Inc의 전체 형태"
    },
    {
      "query": "Apple Inc 주소"
    },
    {
      "query": "Apple Inc 투자자 관계"
    },
    {
      "query": "Apple Inc 산업"
    },
    {
      "query": "Apple Inc 연례 보고서"
    },
    {
      "query": "Apple Inc 경력"
    }
  ]
}
```

우리는 지난 하루의 검색 결과를 성공적으로 얻었으며, 결과 내용은 위와 유사합니다.

## 사용자 정의 검색 결과 페이지 매김

이 API는 검색 결과의 페이지 매김 표시를 사용자 정의할 수 있으며, `number`와 `page`는 페이지의 크기와 페이지 수를 나타냅니다. 본문에서는 한 페이지에 20개의 검색 결과가 있는 형태로 페이지 매김을 설정할 것입니다. 구체적인 정보는 그림과 같습니다:

<p><img src="https://cdn.acedata.cloud/dqla1e.png" width="500" className="m-auto" /></p>

> 주의: 각 페이지의 결과가 10개를 초과할 경우, 차감되는 포인트가 두 배가 됩니다.

코드는 다음과 같습니다:
```shell
curl -X POST 'https://api.acedata.cloud/serp/google' \
-H 'accept: application/json' \
-H 'authorization: Bearer {token}' \
-H 'content-type: application/json' \
-d '{
  "query": "apple inc",
  "number": 20,
  "page": 1
}'
```

실행 결과는 다음과 같습니다:

```json {
  "knowledge_graph": {
    "title": "Apple",
    "type": "기술 회사",
    "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSHmAG45NsvdRfYlNghHFvinM0w2mqjd8A0lXKU2LA&usqp=CAE&s",
    "attributes": {
      "고객 서비스": "1 (800) 275-2273",
      "창립자": "스티브 잡스, 스티브 워즈니악, 로널드 웨인",
      "설립": "1976년 4월 1일, 로스알토스, CA",
      "주가": "AAPL (NASDAQ) $198.42 +1.97 (+1.00%) 6월 16일, 오후 4:00 EDT - 면책 조항",
      "본사": "쿠퍼티노, CA",
      "CEO": "팀 쿡 (2011년 8월 24일~)"
    }
  },
  "organic": [
    {
      "title": "Apple",
      "link": "https://www.apple.com/",
      "snippet": "Apple의 혁신적인 세계를 발견하고 모든 iPhone, iPad, Apple Watch, Mac 및 Apple TV를 쇼핑하며 액세서리, 엔터테인먼트 등을 탐색하세요.",
      "sitelinks": [
        {
          "title": "지원",
          "link": "https://support.apple.com/",
          "snippet": "iPhone 지원 - 연락처 - 청구 및 구독 - Apple 수리"
        }, {
          "title": "경력 기회",
          "link": "https://www.apple.com/careers/us/",
          "snippet": "Apple에서 일하기 - 소프트웨어 및 서비스 - 지원 및 서비스"
        }, {
          "title": "비즈니스",
          "link": "https://www.apple.com/business/",
          "snippet": "소기업 - 기업 - 비즈니스를 위한 쇼핑 - Apple at Work - ..."
        }, {
          "title": "스토어",
          "link": "https://www.apple.com/store",
          "snippet": "iPhone 쇼핑 - Mac 쇼핑 - iPad 쇼핑 - App Store - ..."
        },
        {
          "title": "투자자 관계",
          "link": "https://investor.apple.com/investor-relations/default.aspx",
          "snippet": "SEC 제출 - 주가 - 리더십 및 거버넌스"
        }
      ],
      "position": 1
    }, {
      "title": "Apple Inc.",
      "link": "https://en.wikipedia.org/wiki/Apple_Inc.",
      "snippet": "Apple Inc.는 실리콘 밸리의 쿠퍼티노, 캘리포니아에 본사를 둔 미국의 다국적 기업이자 기술 회사입니다.",
      "position": 2
    }, {
      "title": "Apple Inc. | 역사, 제품, 본사 및 사실",
      "link": "https://www.britannica.com/money/Apple-Inc",
      "snippet": "Apple Inc.는 컴퓨터 소프트웨어, 개인용 ...의 혁신을 통해 기술 부문을 혁신한 미국의 다국적 기술 회사입니다.",
      "date": "7일 전",
      "position": 3
    }, {
      "title": "Apple Inc. (AAPL) 회사 프로필 및 사실",
      "link": "https://finance.yahoo.com/quote/AAPL/profile/",
      "snippet": "Apple Inc.는 스마트폰, 개인용 컴퓨터, 태블릿, 웨어러블 및 액세서리를 전 세계적으로 설계, 제조 및 판매합니다.",
      "position": 4
    }, {
      "title": "AAPL: Apple Inc 주가 인용 - NASDAQ GS",
      "link": "https://www.bloomberg.com/quote/AAPL:US",
      "snippet": "Apple Inc.는 스마트폰, 개인용 컴퓨터, 태블릿, 웨어러블 및 액세서리를 설계, 제조 및 판매하며 다양한 관련 액세서리를 판매합니다.",
      "position": 5
    }, {
      "title": "Apple Inc. (AAPL) 오늘의 주가",
      "link": "https://www.wsj.com/market-data/quotes/AAPL?gaa_at=eafs&gaa_n=ASWzDAgqDxGuyZUvNqcmVhmXBoGHd3y-StLOZpo82befc3s9jFqSwwJzjPmn&gaa_ts=68514b6e&gaa_sig=a-sNk_J7mCIayZx21oIaY6Ge6vifnmxy7A4psXod_wP6ng9689uAwZYvUOUQPv_dxcArYe_RQbKxCkwngcK_bQ%3D%3D",
      "snippet": "Apple Inc.는 스마트폰, 개인용 컴퓨터, 태블릿, 웨어러블 및 액세서리의 설계, 제조 및 판매에 참여하며, 기타 다양한 관련 ...",
      "position": 6
    }, {
      "title": "Apple",
      "link": "https://www.linkedin.com/company/apple",
      "snippet": "Apple에 대한 외부 링크. 산업: 컴퓨터 및 전자 제조. 회사 규모: 10,001명 이상. 본사: 캘리포니아주 쿠퍼티노. 유형: 공개 ...",
      "position": 7
    }, {
      "title": "Apple | AAPL 주가, 회사 개요 및 뉴스",
      "link": "https://www.forbes.com/companies/apple/",
      "snippet": "Apple Inc.는 스마트폰, 개인용 컴퓨터, 태블릿, 웨어러블 및 액세서리의 설계, 제조 및 판매에 참여하며, 기타 다양한 관련 ...",
      "position": 8
    }, {
      "title": "Apple Inc. (AAPL) 주가, 뉴스, 인용 및 역사",
      "link": "https://finance.yahoo.com/quote/AAPL/",
      "snippet": "최신 Apple Inc. (AAPL) 주가 인용, 역사, 뉴스 및 주식 거래 및 투자에 도움이 되는 기타 중요한 정보를 찾으세요.",
      "position": 9
    }, {
      "title": "iCloud",
      "link": "https://www.icloud.com/",
      "snippet": "iCloud.com에서 모든 웹 브라우저를 통해 사진, 파일 등을 액세스할 수 있습니다. 변경 사항은 iPhone 및 기타 장치와 동기화됩니다.",
      "position": 10
    }, {
      "title": "Apple Inc (AAPL) 주가 및 뉴스 - Google Finance",
      "link": "https://www.google.com/finance/quote/AAPL:NASDAQ?hl=en",
      "snippet": "Apple Inc.는 실리콘 밸리의 쿠퍼티노, 캘리포니아에 본사를 둔 미국의 다국적 기업이자 기술 회사입니다.",
      "position": 11
    }, {
      "title": "Apple Inc 회사 프로필 - GlobalData",
      "link": "https://www.globaldata.com/company-profile/apple-inc/",
      "snippet": "Apple Inc (Apple)는 스마트폰, 태블릿, 개인용 컴퓨터 및 웨어러블 장치를 설계, 제조 및 판매합니다. 이 회사는 소프트웨어 애플리케이션을 제공합니다 ...",
      "position": 12
    }, {
      "title": "Apple Computer, Inc.의 설립 - 이번 달의 ...",
      "link": "https://guides.loc.gov/this-month-in-business-history/april/apple-computer-founded",
      "snippet": "Apple Computer, Inc.는 1976년 4월 1일 대학 중퇴생 스티브 잡스와 스티브 워즈니악에 의해 설립되었으며, 새로운 회사에 대한 비전을 가지고 ...",
      "position": 13
    }, {
      "title": "Apple Inc",
      "link": "https://vimeo.com/appleinc",
      "snippet": "Apple Inc는 Vimeo의 회원으로, 고품질 비디오와 이를 사랑하는 사람들을 위한 공간입니다.",
      "position": 14
    }
  ],
  "people_also_ask": [
    {
      "question": "Apple Inc란 무엇인가요?"
    }, {
      "question": "누가 Apple Inc를 소유하고 있나요?"
    }, {
      "question": "Apple Inc.는 무엇으로 가장 잘 알려져 있나요?",
      "title": "Apple Inc. - 위키백과",
      "snippet": "소비자 전자 제품, 소프트웨어 및 서비스로 가장 잘 알려져 있습니다. 1976년 스티브 잡스, 스티브 워즈니악 및 로널드 웨인에 의해 Apple Computer Company로 설립되었으며, 다음 해에 잡스와 워즈니악에 의해 Apple Computer, Inc.로 법인화되었습니다.",
      "link": "https://en.wikipedia.org/wiki/Apple_Inc.#:~:text=It%20is%20best%20known%20for,%2C%20Inc.%20the%20following%20year."
    }, 
```
{
      "question": "Apple Inc에서 Inc의 의미는 무엇인가요?",
      "title": "Inc. | 영어 의미 - 케임브리지 사전",
      "snippet": "incorporated의 약어: 법적으로 설립된 미국 회사의 이름에 사용됨: Bishop Computer Services, Inc.",
      "link": "https://dictionary.cambridge.org/dictionary/english/inc#:~:text=abbreviation%20for%20incorporated%3A%20used%20in,Bishop%20Computer%20Services%2C%20Inc."
    } ],
  "related_searches": [
    {
      "query": "apple inc는 무엇인가요"
    }, {
      "query": "Apple Inc 전체 형태"
    }, {
      "query": "Apple Inc 주소"
    }, {
      "query": "Apple Inc 투자자 관계"
    }, {
      "query": "Apple Inc 산업"
    },
    {
      "query": "Apple Inc 연례 보고서"
    },
    {
      "query": "Apple Inc 경력"
    },
    {
      "query": "Apple 17 pro"
    }
  ]
}
```
 
여기에서 검색 결과가 페이지로 나누어 표시되며, 한 페이지에 20개의 결과가 표시되고, 결과의 내용은 위와 유사합니다.
