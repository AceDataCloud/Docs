...
OpenAI Tasks API는 기존 OpenAI 서비스와 권한을 공유합니다. 이미 OpenAI 이미지 생성에 신청한 경우, 동일한 토큰을 사용하여 이 인터페이스를 호출할 수 있으며, 추가 신청이 필요하지 않습니다.
...
`id`와 `trace_id` 중 하나는 반드시 전달해야 합니다. 일반적으로 제출 응답에서 `id`를 직접 사용하는 것이 좋으며, `trace_id`는 사용자 정의 비즈니스 식별자로 연관짓고 싶을 때만 전달합니다.
...
- `type`: 상위 인터페이스 유형. `gpt-image` 시리즈(예: `gpt-image-2`)로 작성된 작업은 `images`로, 구버전 OpenAI 채널(예: `gpt-image-1`, nano-banana)은 `images_generations` / `images_edits`를 사용하며, 일부 채팅 인터페이스는 `chat_completions_image`를 사용.
- `request`: 원본 요청의 전체 요청 본문.
- `response`: 콜백 완료 시 상위 인터페이스에서 반환된 최종 응답 본문.
...
| `type`           | string   | 상위 유형으로 필터링(값: `images`, `images_generations`, `images_edits`) |
...
Tasks API는 주로 콜백 모드에서의 비동기 프로세스를 위해 서비스됩니다. 콜백 모드에서는 제출 인터페이스가 **즉시 `task_id`**(즉, 작업 ID)를 동기적으로 반환하며, 이후에는 이 `task_id`를 사용하여 Tasks 인터페이스를 폴링하면 됩니다. `trace_id`를 따로 생성할 필요가 없습니다.
...
# 2. 제출 응답에서 task_id를 사용하여 Tasks 인터페이스를 폴링, 작업이 완료될 때까지
while True:
    task = requests.post(
        f"{API}/openai/tasks",
        headers=HEADERS,
...
- Tasks 인터페이스 자체는 **요금이 부과되지 않**으므로 안심하고 폴링할 수 있습니다. 원본 이미지 생성/편집 요청만 요금이 부과됩니다.
