...
- `created_at` / `finished_at`: Unix 타임스탬프(초). `started_at`은 ISO 8601 문자열.
  ...
  "items": [
    {
      ...
      "type": "images",
      "request": { "model": "gpt-image-2", "prompt": "고양이" },
...
Tasks API는 주로 콜백 모드에서 비동기 프로세스를 지원합니다. 콜백 모드에서는 제출 인터페이스가 **즉시 `task_id`**(즉, 작업 ID)를 동기적으로 반환하며, 이후에는 이 `task_id`를 사용하여 Tasks 인터페이스를 폴링하면 됩니다. `trace_id`를 직접 생성할 필요가 없습니다.
