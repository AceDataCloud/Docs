...
`gpt-image-2`는 OpenAI에서 출시한 차세대 이미지 생성 모델로, `dall-e-3` 및 `gpt-image-1`에 비해 다음과 같은 부분에서 뚜렷한 향상이 있습니다:
...
`gpt-image-2`는 기본적으로 표준 라인을 사용합니다. 모델 이름 접미사를 통해 명시적으로 라인을 선택할 수 있습니다:
...
`gpt-image-2`는 `size`의 형식만 검사하며, `auto` 또는 빈 문자열이 아닌 경우 `WIDTHxHEIGHT` 형식(예: `1024x1024`, `2048x1152`, `800x600`)에 맞춰야 합니다; 다른 형태는 400을 반환합니다. **모든 크기(1K / 2K / 4K / 사용자 정의)는 단일 이미지당 동일하게 요금이 부과되며, 크기에 따라 추가 요금이 없습니다.**
...
> `size: "auto"`(기본값)를 전달하면 모델이 **프롬프트 자체를 읽어 비율을 결정합니다**: "초광각 영화 느낌의 가로 21:9"라고 쓰면 `3024x1296`을 얻고, "세로 포스터 9:16"이라고 쓰면 `1440x2560`을 얻습니다. 비율 암시가 없는 프롬프트는 `2048x2048`로 돌아갑니다. 따라서 구성을 염두에 두고 있지만 구체적인 픽셀을 계산하고 싶지 않을 때는 `auto`가 일반적으로 고정된 크기를 입력하는 것보다 더 적합합니다. `size` 필드를 생략하는 효과는 `auto`와 동일합니다.
...
> **`n` 매개변수에 대하여**
> `gpt-image-2`는 `n > 1`(값 범위 1–10)을 지원합니다: 한 번의 요청으로 여러 개의 이미지를 반환하고 각 이미지에 대해 요금이 부과됩니다. 여러 결과에 차이를 주기 위해서는 동시에 다른 `prompt` 또는 `seed`를 전달하는 것이 좋습니다. 이는 `gpt-image-1` / `gpt-image-1.5`, 그리고 `nano-banana` / `nano-banana-2-lite` / `nano-banana-2` / `nano-banana-pro` 시리즈에도 동일하게 적용됩니다; `dall-e-3`는 `n = 1`만 지원합니다. `response_format=b64_json`은 `n=1`만 지원하며, `n>1`일 경우 기본 URL 반환을 사용하십시오. 생성에 실패한 이미지가 있을 경우, 성공한 부분만 반환되고 요금이 부과됩니다.
...
payload = {
    "model": "gpt-image-2",
    "prompt": "편의점에서 밤에 서 있는 젊은 여성의 영화 같은 초상화, 창문을 통해 부드러운 분홍색과 청록색 네온사인에 의해 조명이 비춰진 모습. 35mm 필름으로 촬영, 얕은 심도, 약간의 그레인, 우울한 분위기.",
    "size": "1024x1536"
}
...
```json
{
  ...
  "data": [
    {
      "revised_prompt": "편의점에서 밤에 서 있는 젊은 여성의 영화 같은 초상화, 창문을 통해 부드러운 분홍색과 청록색 네온사인에 의해 조명이 비춰진 모습. 35mm 필름으로 촬영, 얕은 심도, 약간의 그레인, 우울한 분위기.",
...
payload = {
    "model": "gpt-image-2",
    "prompt": "이탈리아 아말피 해안의 빈티지 여행 포스터. 절벽에 있는 레몬 노란색 집들이 청록색 바다로 이어지는 아르데코 스타일의 일러스트레이션, 항구에 작은 흰색 세일보트가 있습니다. 상단에는 AMALFI라는 대담한 타이포그래피가, 하단에는 ITALIA 1958이 적혀 있습니다. 제한된 색상 팔레트: 크림색, 바다 파랑, 레몬 노랑, 테라코타. 약간의 종이 그레인 질감.",
    "size": "1024x1536"
}
```
...
모델이 아르데코 포스터의 시각적 스타일을 정확하게 재현했으며, 제목 텍스트 `AMALFI`와 `ITALIA 1958`이 모두 선명하고 정확하게 렌더링된 것을 볼 수 있습니다.
...
payload = {
    "model": "gpt-image-2",
    "prompt": "세 개의 선반으로 구성된 나무 책장: 맨 위 선반에는 한 권의 책이 있어야 합니다. 두 번째 선반에는 세 권의 책이 있어야 합니다. 맨 아래 선반에는 일곱 권의 책이 있어야 합니다. 부드럽고 따뜻한 조명, 포토리얼리스틱, 아늑한 도서관 분위기.",
    "size": "1024x1024"
}
```
...
payload = {
    "model": "gpt-image-2",
    "prompt": "달빛이 비치는 숲 속에서 빛나는 버섯 아래에서 책을 읽고 있는 작은 여우의 부드럽고 시적인 아동 도서 일러스트. 수채화와 연필 질감, 부드러운 파스텔 색상, 꿈같은 분위기, 손으로 그린 느낌.",
    "size": "1536x1024"
}
```
...
>   - `1792x1024` → `16:9`
>   - `1024x1792` → `9:16`
> - `quality`, `style`, `response_format`, `background`, `output_format` 등의 매개변수는 지원되지 않으며, 입력해도 무시됩니다. `n > 1`은 지원되며 (1–10), 해당 수량의 이미지를 반환하고 그에 따라 요금이 부과됩니다.
...
```json
{
  ...
  "data": [
    {
      "revised_prompt": "젊은 바다 수달을 보여주는 즐거운 이미지로, 갈색으로 태어나 넓고 매력적인 눈을 가진 모습입니다. 평화로운 바다에서 등을 대고 누워 놀고 있으며, 촘촘하고 벨벳 같은 털이 젖어 반짝이고 있습니다. 작은 발로 조개껍데기를 호기심 가득하게 가지고 놀고 있으며, 자연 환경에서 순수하고 매력적인 모습입니다.",
...
그 중 `data`는 모델이 생성한 이미지의 구체적인 정보를 포함하고 있으며, 그 안의 `url`은 생성된 이미지의 상세 링크입니다. 아래 그림과 같이 확인할 수 있습니다.

<p><img src="https://cdn.acedata.cloud/dz7u0x.png" width="500" class="m-auto"></p>

## 이미지 품질 매개변수 `quality`

다음으로 이미지 생성 결과의 일부 세부 매개변수를 설정하는 방법을 소개합니다. 이미지 품질 매개변수 `quality`는 두 가지가 있습니다. 첫 번째는 `standard`로, 표준 이미지를 생성하는 것을 의미하며, 두 번째는 `hd`로, 생성된 이미지가 더 세밀한 디테일과 더 큰 일관성을 갖는 것을 의미합니다.
...
```json
{
  ...
  "data": [
    {
      "revised_prompt": "귀여운 아기 바다 수달이 물속에서 장난스럽게 등을 대고 누워 있으며, 털이 윤기 나고 부드럽게 보입니다. 작은 발 중 하나가 호기심 가득하게 뻗어 있으며, 하늘을 바라보며 순수한 기쁨과 따뜻함의 표정을 짓고 있습니다. 몸 주위에는 물속에서 장난치며 생긴 거품이 있습니다. 부드러운 바람이 털을 간지럽히며 더욱 매력적으로 보이게 합니다. 이 장면은 해양 생명의 평화로움과 매력을 묘사합니다.",
...
위와 동일한 작업을 수행하기 위해, 이미지 링크의 형식 매개변수를 `b64_json`으로 설정하면 Base64 인코딩된 이미지 링크 결과를 얻을 수 있으며, 구체적인 결과는 아래 그림과 같습니다:
  ...
  "data": [
    {
      ...
      "revised_prompt": "젊은 아기 바다 수달의 매력적인 이미지. 수달은 잔잔한 파란 바다 위에서 부드러운 햇빛이 쏟아지는 맑은 하늘 아래에서 따뜻한 금빛 햇살을 받으며 떠 있습니다. 수달의 털은 진한 초콜릿 갈색이며, 믿을 수 없을 만큼 부드럽고 푹신해 보입니다. 수달의 눈은 밝고 표현력이 풍부하며, 어린아이 같은 호기심과 기쁨으로 가득 차 있습니다. 작은 귀와 단추 같은 코가 전체적인 귀여움을 더합니다. 주변 바다에는 햇빛에 반짝이는 물방울들이 보이며, 그 모습은 확실히 즐거운 광경입니다."
    }
  ]
}
```
...
OpenAI Images Generations API가 이미지를 생성하는 데 시간이 다소 걸릴 수 있으므로, API가 오랜 시간 응답하지 않으면 HTTP 요청이 연결을 유지하여 추가 시스템 리소스를 소모하게 됩니다. 따라서 이 API는 비동기 콜백 지원도 제공합니다.
