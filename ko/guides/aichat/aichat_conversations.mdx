---
title: "AI 채팅 API 연동 설명"
description: "AI Dialogue API guide - Ace Data Cloud"
---

우리는 시장에 있는 일부 질문-답변 API의 연동이 상대적으로 쉽지 않다는 것을 알고 있습니다. 예를 들어 OpenAI의 Chat Completions API는 `messages` 필드가 있으며, 연속 대화를 완료하려면 모든 맥락 역사(context history)를 전달해야 하고, 동시에 토큰 초과 제한 문제를 처리해야 합니다.

AceDataCloud에서 제공하는 AI 질문-답변 API는 이러한 상황을 최적화하여 질문-답변 효과를 유지하면서 연속 대화의 구현을 캡슐화했습니다. 연동 시 `messages`의 전달에 대해 걱정할 필요가 없으며, 토큰 초과 제한 문제도 신경 쓸 필요가 없습니다(API 내부에서 자동으로 처리됨). 또한 대화 조회, 수정 등의 기능도 제공하여 전체 연동이 크게 간소화되었습니다.

이 문서에서는 AI 질문-답변 API의 연동 설명을 소개합니다.

## 신청 절차

AI 질문-답변 API를 사용하려면 먼저 [Ace Data Cloud 콘솔](https://platform.acedata.cloud/console/applications)에서 API 토큰을 받아야 합니다. 이를 백업용으로 보관하십시오.
...
> 📘 전체 문서: [AI 질문-답변 API →](https://platform.acedata.cloud/documents/aichat-conversations)
...
선택 후, 오른쪽에 해당 코드가 생성된 것을 확인할 수 있습니다. 다음과 같이 표시됩니다:
...
여기서 반환된 결과에는 `answer` 필드가 있으며, 이는 해당 질문에 대한 답변입니다. 우리는 임의의 질문을 입력하면 임의의 답변을 얻을 수 있습니다.
...
다중 대화 기능을 연동하려면 추가 매개변수 `stateful`을 전달해야 하며, 그 값은 `true`여야 합니다. 이후의 각 요청에는 이 매개변수를 포함해야 합니다. `stateful` 매개변수를 전달하면 API는 현재 대화의 ID를 나타내는 추가 `id` 매개변수를 반환합니다. 이후에는 이 ID를 매개변수로 전달하기만 하면 쉽게 다중 대화를 구현할 수 있습니다.

아래에서 구체적인 작업을 시연하겠습니다.

첫 번째 요청에서 `stateful` 매개변수를 `true`로 설정하고, `model` 및 `question` 매개변수를 정상적으로 전달합니다. 다음과 같이 표시됩니다:
...
두 번째 요청에서는 첫 번째 요청에서 반환된 `id` 필드를 매개변수로 전달하고, `stateful` 매개변수는 여전히 `true`로 설정합니다. "What I asked you just now?"라고 질문합니다. 다음과 같이 표시됩니다:
...
맥락에 따라 질문에 대한 답변을 할 수 있음을 확인할 수 있습니다.

## 스트리밍 응답

이 인터페이스는 스트리밍 응답도 지원합니다. 이는 웹 페이지 연동에 매우 유용하며, 웹 페이지에서 글자 단위로 표시하는 효과를 구현할 수 있습니다.

스트리밍 응답을 원하면 요청 헤더의 `accept` 매개변수를 `application/x-ndjson`으로 변경하면 됩니다.

변경 사항은 다음과 같으며, 호출 코드도 스트리밍 응답을 지원하도록 적절한 변경이 필요합니다.

![](https://cdn.acedata.cloud/axt1aa.png)

`accept`를 `application/x-ndjson`으로 변경하면 API는 해당 JSON 데이터를 한 줄씩 반환합니다. 코드 측면에서 우리는 줄 단위 결과를 얻기 위해 적절한 수정을 해야 합니다.

Python 샘플 호출 코드는 다음과 같습니다:
```python
import requests
...
```python
{
    "answer": "전문 아티스트로서, 귀하의 특정 요구에 따라 다양한 서비스와 도움을 제공할 수 있습니다. 제가 도와드릴 수 있는 몇 가지 방법은 다음과 같습니다:\n\n1. 맞춤형 예술 작품: 특정 비전이나 아이디어가 있다면, 귀하를 위해 맞춤형 예술 작품을 만들 수 있습니다. 여기에는 그림, 드로잉, 디지털 아트 또는 귀하가 선호하는 다른 매체가 포함될 수 있습니다.\n\n2. 의뢰 작품: 특정 주제나 개념이 있다면, 귀하의 선호에 맞춘 의뢰 예술 작품을 만들 수 있습니다. 이는 개인적인 즐거움을 위한 것이거나 특별한 사람을 위한 독특한 선물일 수 있습니다.\n\n3. 예술 상담: 예술 선택, 인테리어 디자인 또는 귀하의 공간에서 예술을 전시하고 보여주는 방법에 대한 안내가 필요하다면, 미적 감각을 향상시키고 일관된 모습을 만들기 위해 전문적인 조언을 제공할 수 있습니다."
}
```

여기서 우리는 GPT에게 그가 로봇이라고 말한 후, 그가 우리를 위해 무엇을 할 수 있는지 물어보았고, 그는 로봇의 역할을 맡아 질문에 답변할 수 있었습니다.

## 이미지 인식

본 AI는 첨부 파일을 추가하여 이미지 인식을 지원할 수 있으며, `references`를 통해 해당 이미지 링크를 전달하면 됩니다. 예를 들어, 여기 사과의 이미지가 있습니다:
