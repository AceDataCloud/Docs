...
AI Chat v2 API를 사용하려면 먼저 [Ace Data Cloud 콘솔](https://platform.acedata.cloud/console/applications)에서 API Token을 받아야 합니다.
...
주의: 사전 승인은 "이번 요청에서 이러한 기능이 무인 모드에서 인적 확인을 건너뛰도록 허용"을 의미합니다. 특정 Skill은 여전히 `--unattended-confirm` 또는 해당 보안 메커니즘을 지원해야 합니다; 그렇지 않으면 계속해서 dry-run을 수행하고 직접 쓰기 작업을 실행하지 않습니다.
...
요청 본체의 `tool_use_id`는 **반드시** 일시 중지 시의 `tool_id`와 완전히 일치해야 하며, 일치하지 않으면 400 오류가 반환됩니다. 요청에 `tool_results`가 동시에 존재하면, `question` / `message` / `references`는 모두 무시됩니다.
...
`messages`도 전달할 수 있지만, 서버는 엄격한 스키마 검증을 수행함(반드시 축소된 `ToolUseContent` 형태여야 하며, 그렇지 않으면 400을 반환함). 일반적으로 `title` 변경에만 사용하는 것이 좋음.
...
- `500 chat_error`: 상류 LLM 오류 또는 이번 라운드 `completion_tokens=0`(소비되지 않은 것으로 처리되며, 요금이 부과되지 않음).

스트리밍 응답에서 오류는 `{"type":"error","message":"..."}` 이벤트로 발송되며, 그 직후 스트림이 종료됨.

## 결론

AI Chat v2 API는 v1과의 하위 호환성을 유지하면서 대화를 "단일/다중 라운드 질문 응답"에서 "에이전트화된 관찰 가능한 대화"로 업그레이드함: 다중 모드 입력, 도구 호출, 일시 중지/재개 가능, 스트리밍 구조화된 이벤트, 내장 CRUD. 새로운 통합은 직접 v2를 사용하는 것이 좋으며, 기존 v1 통합은 단계적으로 부드럽게 마이그레이션할 수 있음. 질문이 있으면 언제든지 기술 지원 팀에 문의하십시오.
