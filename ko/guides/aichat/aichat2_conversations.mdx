...
AI Chat v2 API(`/aichat2/conversations`)는 새로운 세대의 대화 인터페이스로, [AI Chat API](https://platform.acedata.cloud/documents/aichat-conversations)의 전면 업그레이드 버전입니다. v1의 간결함과 다중 회차 대화를 호스팅하는 기반 위에 다음과 같은 기능이 확장되었습니다:
...
- **중단 가능 / 복구 가능**: 모델이 사용자에게 추가 정보를 요청할 때 `ask_user_question` 이벤트를 발송하고 일시 중지하며, 다음 호출 시 `tool_results`를 통해 답변을 다시 채워 넣으면 계속 진행할 수 있습니다.
...
```json
{
  "answer": "좋습니다, 42를 기억했습니다. 제가 그것으로 무엇을 할까요?",
  "id": "f2f4b3e8-0c0a-4d3a-aaa2-7ff80c0a1c44"
}
```
...
이후 `action: retrieve` + `id`를 사용하여 세션 결과를 조회할 수 있습니다; 또는 `callback_url`을 제공하면, 작업이 완료된 후 플랫폼이 `{ status, answer, usage, error }`를 귀하의 콜백 주소로 POST합니다. `callback_url`은 `http` / `https`를 사용해야 하며, `localhost` 또는 개인 IP의 리터럴 주소를 직접 입력할 수 없습니다.
...
일부 도구는 모델이 "사용자에게 재질문"하도록 하며, 이때 모델은 `ask_user_question` 이벤트를 발생시킵니다. 대화는 `awaiting_user_input` 상태에서 동결됩니다:
...
요청 본체의 `tool_use_id`는 **반드시** 일시 중지 시의 `tool_id`와 완전히 일치해야 합니다; 일치하지 않으면 400이 반환됩니다. 요청에 `tool_results`가 동시에 존재하는 경우, `question` / `message` / `references`는 모두 무시됩니다.

사용자가 이 문제를 포기하기로 결정하면, 새로운 `question` / `message`를 직접 전달하면 됩니다. 플랫폼은 일시 중지된 도구 호출을 "사용자가 건너뜀"으로 자동으로 표시합니다.
...
완전한 대화 문서(포함 `messages` 이력, `model`, `title`, `tools_used` 등)를 반환합니다.
...
`{ items: [...], total }`를 반환합니다. **요약에는 `messages`가 포함되지 않으며**, 사이드바 목록에 적합합니다; 사용자가 특정 대화를 클릭하면, `action: retrieve`를 사용하여 그 대화의 전체 메시지를 개별적으로 가져옵니다.
...
`messages`도 전달할 수 있지만, 서버는 엄격한 스키마 검증을 수행합니다(반드시 축소된 `ToolUseContent` 형태여야 하며), 불일치 시 400을 반환합니다. 일반적으로 `title` 변경에만 사용하는 것이 좋습니다.
...
`{ id, success: true }`를 반환합니다. 삭제 후에는 복구할 수 없으니, 확인 후 호출해 주시기 바랍니다.

## v1에서 부드럽게 마이그레이션

이미 [`/aichat/conversations`](https://platform.acedata.cloud/documents/aichat-conversations)를 사용 중이라면, v2로 마이그레이션할 때 코드 변경이 거의 필요하지 않습니다:

1. URL을 `https://api.acedata.cloud/aichat/conversations`에서 `https://api.acedata.cloud/aichat2/conversations`로 변경합니다.
2. 이전에 v1 모델 이름(예: `gpt-3.5`, `gpt-4-browsing` 등)을 사용했다면, v2로 전환할 때 최신 모델(예: `gpt-5.4`, `claude-opus-4-8`, `gemini-3.1-pro` 등)로 업그레이드하는 것이 좋습니다.
3. NDJSON 스트림의 필드는 하위 호환성을 유지합니다: 각 `text_delta` 이벤트는 여전히 `delta_answer`와 `id`를 포함하므로, 원래 행 단위로 `delta_answer`를 파싱하던 클라이언트는 변경할 필요가 없습니다.

마이그레이션 후에는 필요에 따라 v2의 새로운 기능(다중 모달 `message`, SSE, 도구 호출, `action` CRUD)을 활성화할 수 있으며, 원하는 속도로 진행하면 됩니다.
...
- `500 chat_error`: 상위 LLM에서 오류가 발생하거나 이번 라운드에서 `completion_tokens=0`(소비되지 않은 것으로 처리되며, 요금이 부과되지 않음).

스트리밍 응답에서 오류는 `{"type":"error","message":"..."}` 이벤트로 전송되며, 그 직후 스트림이 종료됩니다.

## 결론

AI Chat v2 API는 v1과의 하위 호환성을 유지하면서 대화를 "단일/다중 회답"에서 "에이전트화된 관찰 가능한 대화"로 업그레이드했습니다: 다중 모달 입력, 도구 호출, 일시 중지/재개 가능, 스트리밍 구조화된 이벤트, 내장 CRUD. 새로운 통합은 직접 v2를 사용하는 것이 좋으며, 기존 v1 통합은 단계적으로 부드럽게 마이그레이션할 수 있습니다. 질문이 있으시면 언제든지 기술 지원 팀에 문의해 주시기 바랍니다.
