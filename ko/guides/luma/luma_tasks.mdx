...
Luma Tasks API를 사용하려면 먼저 신청 페이지 [Luma Videos Generation API](https://platform.acedata.cloud/documents/luma-videos)에서 해당 서비스를 신청한 후, Luma Videos Generation API의 작업 ID를 복사해야 합니다. 아래 그림과 같이:

![](https://cdn.acedata.cloud/ynk4ow.png)

마지막으로 Tasks API 페이지 [Luma Tasks API](https://platform.acedata.cloud/documents/luma-tasks)에서 해당 서비스를 신청하고, 페이지에 들어가면 "Acquire" 버튼을 클릭합니다. 아래 그림과 같이:
