...
본 문서에서는 사용자 정의 매개변수를 입력하여 Veo 공식 비디오를 생성할 수 있는 Veo Videos Generation API 연동 방법을 소개합니다.
...
먼저 기본 사용 방법을 이해해야 합니다. 입력할 항목은 `prompt`(프롬프트), `action`(행동), 시작 및 끝 프레임 참조 이미지 배열 `image_urls`, 그리고 모델 `model`입니다. 이를 통해 처리된 결과를 얻을 수 있습니다. 먼저 간단히 `action` 필드를 전달해야 하며, 그 값은 `text2video`입니다. 이는 주로 세 가지 행동을 포함합니다: 문생 비디오(`text2video`), 그림생 비디오(`image2video`), 1080p 비디오 가져오기(`get1080p`)입니다. 그 다음 모델 `model`을 입력해야 하며, 현재 주요 모델은 `veo31-fast`, `veo3`, `veo31`, `veo3-fast`, `veo31-fast-ingredients`입니다. 구체적인 내용은 다음과 같습니다:
...
- `model`: 비디오를 생성할 모델로, 주요 모델은 `veo31-fast`, `veo3`, `veo31`, `veo3-fast`, `veo31-fast-ingredients`입니다.
- `action`: 이번 비디오 생성 작업의 행동으로, 주로 세 가지 행동이 포함됩니다: 문생 비디오(`text2video`), 그림생 비디오(`image2video`), 1080p 비디오 가져오기(`get1080p`).
- `image_urls`: 그림생 비디오 행동 `image2video`를 선택할 경우 반드시 업로드해야 하는 참조 이미지 링크입니다. `veo31-fast-ingredients`는 최대 3장(다중 이미지 융합), 나머지 모델은 최대 2장(시작 및 끝 프레임 모드)입니다.
...
| -------------------------- | --------------------------------- | --------------------------------- |
| **veo3-fast**              | 문생 비디오(무 이미지)<br />그림생 비디오 모드(유 이미지)           | **1장** → 시작 프레임 모드<br />**2장** → 시작 및 끝 프레임 모드 |
| **veo31-fast**             | 문생 비디오(무 이미지)<br />그림생 비디오 모드(유 이미지)           | **1장** → 시작 프레임 모드<br />**2장** → 시작 및 끝 프레임 모드 |
| **veo31-fast-ingredients** | ❌ 문생 비디오(지원하지 않음)<br />✅ **강제 다중 이미지 융합**(이미지 필수) | **1-3장** → 다중 이미지 융합 모드(최대 3장)        |
| **veo3**                   | 문생 비디오(무 이미지)<br />그림생 비디오 모드(유 이미지)           | **1장** → 시작 프레임 모드<br />**2장** → 시작 및 끝 프레임 모드 |
| **veo31**                  | 문생 비디오(무 이미지)<br />그림생 비디오 모드(유 이미지)           | **1장** → 시작 프레임 모드<br />**2장** → 시작 및 끝 프레임 모드 |
...
2. **그림생 비디오 모드 유형**:
   - **시작 프레임 모드**(1장 이미지): 시작 프레임이 입력 이미지로 고정됩니다.
   - **시작 및 끝 프레임 모드**(2장 이미지): 시작 프레임과 끝 프레임이 입력 이미지로 고정됩니다.
...
- **이미지 수 제한**:
  ...
  - 다른 모델은 최대 **2장** 이미지 입력을 지원합니다(시작 및 끝 프레임 모드).
...
Veo Videos Generation API의 생성 시간은 상대적으로 길어 약 1-2분이 소요됩니다. API가 오랜 시간 응답하지 않으면 HTTP 요청이 계속 연결을 유지하여 추가 시스템 리소스 소모를 초래하므로, 이 API는 비동기 콜백 지원도 제공합니다.
