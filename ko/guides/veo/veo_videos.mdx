...
본 문서는 Veo Videos Generation API 연동 설명을 소개하며, 이는 사용자 정의 매개변수를 입력하여 Veo 공식 비디오를 생성할 수 있습니다.
...
먼저 기본 사용법을 이해해야 하며, 이는 입력 프롬프트 `prompt`, 생성 행동 `action`, 첫 번째 및 마지막 프레임 참조 이미지 배열 `image_urls`, 모델 `model`을 입력하면 처리된 결과를 얻을 수 있습니다. 먼저 간단히 `action` 필드를 전달해야 하며, 그 값은 `text2video`입니다. 이는 주로 세 가지 행동을 포함합니다: 문생 비디오(`text2video`), 그림생 비디오(`image2video`), 1080p 비디오 가져오기(`get1080p`)입니다. 그런 다음 모델 `model`을 입력해야 하며, 현재 주요 모델은 `veo31-fast`, `veo3`, `veo31`, `veo3-fast`, `veo31-fast-ingredients`가 있습니다. 구체적인 내용은 다음과 같습니다:
...
- `model`: 비디오를 생성하는 모델로, 주요 모델은 `veo31-fast`, `veo3`, `veo31`, `veo3-fast`, `veo31-fast-ingredients`입니다.
- `action`: 이번 비디오 생성 작업의 행동으로, 주로 세 가지 행동을 포함합니다: 문생 비디오(`text2video`), 그림생 비디오(`image2video`), 1080p 비디오 가져오기(`get1080p`).
- `image_urls`: 그림생 비디오 행동 `image2video`를 선택할 경우 반드시 업로드해야 하는 참조 이미지 링크입니다. `veo31-fast-ingredients`는 최대 3장(다중 이미지 융합), 나머지 모델은 최대 2장(첫 번째 및 마지막 프레임 모드)입니다.
- `resolution`: 생성할 비디오의 해상도를 선택합니다. `veo31` 모델은 4k 해상도를 지원하며, 다른 모델은 지원하지 않습니다. 모든 모델은 1080p 및 gif 해상도를 지원하며, 이 값을 전달하지 않으면 기본적으로 720p 해상도를 사용합니다. 주요 해상도는: `1080p`, `gif`, `4k`입니다.
- `prompt`: 프롬프트입니다.
...
| -------------------------- | --------------------------------- | --------------------------------- |
| **veo3-fast**              | 문생 비디오(무 이미지)<br />그림생 비디오 모드(유 이미지)           | **1장** → 첫 프레임 모드<br />**2장** → 첫 마지막 프레임 모드 |
| **veo31-fast**             | 문생 비디오(무 이미지)<br />그림생 비디오 모드(유 이미지)           | **1장** → 첫 프레임 모드<br />**2장** → 첫 마지막 프레임 모드 |
| **veo31-fast-ingredients** | ❌ 문생 비디오(지원하지 않음)<br />✅ **강제 다중 이미지 융합**(이미지 필수) | **1-3장** → 다중 이미지 융합 모드(최대 3장)        |
| **veo3**                   | 문생 비디오(무 이미지)<br />그림생 비디오 모드(유 이미지)           | **1장** → 첫 프레임 모드<br />**2장** → 첫 마지막 프레임 모드 |
| **veo31**                  | 문생 비디오(무 이미지)<br />그림생 비디오 모드(유 이미지)           | **1장** → 첫 프레임 모드<br />**2장** → 첫 마지막 프레임 모드 |
...
2. **그림생 비디오 모드 유형**:
   - **첫 프레임 모드**(1장 이미지): 첫 프레임이 입력 이미지로 고정됩니다.
   - **첫 마지막 프레임 모드**(2장 이미지): 첫 프레임과 마지막 프레임이 입력 이미지로 고정됩니다.
   - **다중 이미지 융합 모드**(1-3장 이미지): 오직 `veo31-fast-ingredients`만 지원하며, 다중 이미지 내용을 융합하여 비디오를 생성합니다.
3. **모드 분류**:
...
- **유일하게 강제 이미지 입력 모델**: `veo31-fast-ingredients`는 이미지를 반드시 입력해야 하며(1-3장), 그렇지 않으면 실행할 수 없습니다.
- **이미지 수 제한**:
...
우리는 만족스러운 비디오 정보를 얻었으며, 결과의 `data`에서 비디오 링크 주소를 기반으로 생성된 Veo 비디오를 가져오기만 하면 됩니다.
...
- `model`：비디오 생성을 위한 모델, 주로 `veo31-fast`、`veo3`、`veo31`、`veo3-fast` 및 `veo31-fast-ingredients`가 있습니다.
- `image_urls`：비디오 생성 행동 `image2video`를 선택할 경우 업로드해야 하는 참조 이미지 링크입니다.
- `prompt`：프롬프트입니다.
...
결과 내용이 위와 일치함을 알 수 있으며, 이는 비디오의 이미지로 비디오 생성 기능을 구현한 것입니다.
...
결과 내용이 위와 일치함을 알 수 있으며, 이는 비디오의 1080p 비디오 가져오기 기능을 구현한 것입니다.
...
Veo Videos Generation API로 생성되는 시간은 상대적으로 길어 약 1-2분이 소요됩니다. API가 오랜 시간 응답하지 않으면 HTTP 요청은 계속 연결을 유지하여 추가 시스템 리소스 소모를 초래하므로, 이 API는 비동기 콜백 지원도 제공합니다.
...
잠시 후, `https://webhook.site/aed5cd28-f8aa-4dca-9480-8ec9b42137dc`에서 생성된 비디오 결과를 확인할 수 있습니다:
...
이 문서를 통해 Veo Videos Generation API를 사용하여 입력 프롬프트와 첫 번째 프레임 참조 이미지를 통해 비디오를 생성하는 방법을 이해하셨습니다. 이 문서가 API를 더 잘 연결하고 사용하는 데 도움이 되기를 바랍니다. 질문이 있으시면 언제든지 기술 지원 팀에 문의해 주시기 바랍니다.
