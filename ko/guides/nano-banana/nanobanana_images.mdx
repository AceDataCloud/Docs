...
나노 바나나 이미지 API를 사용하려면 먼저 [Ace Data Cloud 콘솔](https://platform.acedata.cloud/console/applications)에서 API 토큰을 받아야 합니다. 이를 보관해 두세요.
...
- **이미지 수**: 선택 사항, `count`를 통해 1–4장 지정, 기본값 1장; 각 이미지는 독립적으로 생성 호출 완료; 일반 기술 실패 또는 제공자의 보안 거부는 해당 호출에만 영향을 미치며, 나머지 성공 이미지는 정상적으로 반환되고 실제 성공 장수에 따라 요금이 청구됨
...
curl -X POST 'https://api.acedata.cloud/nano-banana/images' \
  ...
  -d '{
    ...
    "prompt": "일본의 노인 도예가의 포토리얼리스틱 클로즈업 초상화. 깊고 햇볕에 새겨진 주름과 따뜻하고 아는 듯한 미소를 지니고 있습니다. 그는 갓 유약을 바른 찻잔을 조심스럽게 검사하고 있습니다. 배경은 그의 소박하고 햇볕이 잘 드는 작업실입니다. 장면은 창문을 통해 부드러운 황금 시간의 빛이 비추어져 점토의 섬세한 질감을 강조합니다. 85mm 초상 렌즈로 촬영되어 부드럽고 흐릿한 배경(bokeh)이 만들어집니다. 전체적인 분위기는 평화롭고 장인정신이 느껴집니다. 세로 초상 방향.",
    "count": 1
  }'
```
...
payload = {
    ...
    "prompt": (
        ...
        "작업실입니다. 장면은 창문을 통해 부드러운 황금 시간의 빛이 비추어져 점토의 섬세한 "
        "질감을 강조합니다. 85mm 초상 렌즈로 촬영되어 부드럽고 흐릿한 배경(bokeh)이 만들어집니다. "
        "전체적인 분위기는 평화롭고 장인정신이 느껴집니다. 세로 초상 방향."
    ),
    "count": 1
}
...
```json
{
  ...
  "data": [
    {
      "prompt": "일본의 노인 도예가의 포토리얼리스틱 클로즈업 초상화. 깊고 햇볕에 새겨진 주름과 따뜻하고 아는 듯한 미소를 지니고 있습니다. 그는 조심스럽게 갓 유약을 바른 찻잔을 검사하고 있습니다. 배경은 그의 소박하고 햇볕이 잘 드는 작업실입니다. 장면은 창문을 통해 부드러운 황금 시간의 빛이 비추어져 점토의 섬세한 질감을 강조합니다. 85mm 초상 렌즈로 촬영되어 부드럽고 흐릿한 배경(bokeh)이 만들어집니다. 전체적인 분위기는 평화롭고 장인정신이 느껴집니다. 세로 초상 방향.",
...
- `count`: 요청한 생성 또는 편집 이미지 수, 1–4 지원, 기본값 1. `data`는 생성에 성공한 이미지만 포함되며 실제 반환된 장수에 따라 요금이 청구됨. 각 생성 호출은 제공자의 원래 보안 정책을 강제로 사용; 특정 호출이 거부되더라도 다른 성공 호출에는 영향을 미치지 않으며, 모든 호출이 거부될 경우 403을 반환.
- `data[]`: 결과 목록.
  - `prompt`: 생성에 사용된 프롬프트(회신).
  - `image_url`: 생성된 이미지의 직링크 URL.

> 주: `/nano-banana/images`는 `action`과 `prompt`만으로 이미지를 생성할 수 있습니다.
...
예를 들어, 인물 사진과 옷 사진을 제공하여 인물이 그 옷을 입도록 할 수 있으며, 이 경우 이미지 링크를 동시에 전달하고 `action`을 `edit`로 지정하면 됩니다. URL은 HTTP URL로, `https` 또는 `http` 프로토콜의 공개 접근 가능한 링크일 수 있으며, Base64 인코딩된 이미지도 가능합니다. 예: `data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAA+gAAAVGCAMAAAA6u2FyAAADAFBMVEXq6uwdHCEeHyMdHS....`
...
- `image_urls[]`：편집할 이미지 URL 목록(공개 접근 가능해야 함). 여러 장을 전달할 수 있으며, 서비스는 이러한 자료와 `prompt`를 결합하여 편집을 완료합니다.
...
2. API는 **즉시** `task_id`가 포함된 응답을 반환합니다(또는 기본 결과 포함).
...
- **403 `forbidden`**：제공자의 기본 보안 정책이 요청 또는 생성 결과를 거부했습니다. 이 호출은 이미지를 반환하지 않으며, 요금이 청구되지 않습니다; 다중 이미지 요청은 여전히 다른 성공 호출에 대해 반환 및 청구될 수 있습니다.
...
- **멱등성 및 추적**：`task_id`와 `trace_id`를 보존하여 문제 해결 및 결과 연결에 용이하게 합니다.
