...
본 문서는 나노 바나나 작업 API의 연동 설명을 자세히 소개하여, 여러분이 쉽게 통합하고 이 API의 강력한 기능을 충분히 활용할 수 있도록 돕습니다. 나노 바나나 작업 API를 통해 나노 바나나 이미지 API의 작업 실행 상태를 쉽게 조회할 수 있습니다.
...
나노 바나나 이미지 API 서비스에서 반환된 작업 ID를 예로 들어, 해당 API를 사용하는 방법을 시연하겠습니다. 가정해 보겠습니다, 작업 ID가 `4d320ead-4af4-4a55-8f3e-f2afebdf4fd0`입니다. 다음으로 작업 ID를 전달하여 사용하는 방법을 보여드리겠습니다.
...
- `request` : 작업 조회 중의 요청 정보.
- `response` : 작업 조회 중의 반환 정보.
...
- `items`, 배치 작업의 모든 구체적인 세부 정보. 이는 배열이며, 각 배열의 요소는 위의 단일 작업 쿼리 결과 형식과 동일합니다.
...
이 문서를 통해 Nano Banana Tasks API를 사용하여 단일 또는 배치 작업의 모든 구체적인 세부 정보를 쿼리하는 방법을 이해하셨습니다. 이 문서가 API를 더 잘 연결하고 사용하는 데 도움이 되기를 바랍니다. 질문이 있으시면 언제든지 기술 지원 팀에 문의해 주십시오.
