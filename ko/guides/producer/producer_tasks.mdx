...
반환 결과는 여러 필드로 구성되어 있으며, request 필드는 작업을 시작할 때의 request body이고, response 필드는 작업 완료 후 반환된 response body입니다. 필드 설명은 다음과 같습니다.
...
- `started_at`，작업 시작 실행 시간, ISO-8601 UTC 시간 문자열입니다.
...
- `items`, 배치 작업의 모든 구체적인 세부 정보. 이것은 배열이며, 각 배열의 요소는 위의 단일 작업 쿼리의 반환 결과 형식과 동일합니다.
