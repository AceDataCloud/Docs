...
자신이 업로드한 곡을 계속 번안하고 싶다면, 매개변수 `action`을 `upload_cover`로 설정하고, 계속 번안할 사용자 정의 업로드 곡 ID를 입력해야 합니다. 곡 ID는 [Producer Upload API](https://platform.acedata.cloud/documents/producer-upload)를 사용하여 얻을 수 있으며, 아래 그림과 같이 지정할 수 있습니다:
...
자신이 업로드한 노래를 계속 교체하고 싶다면, 매개변수 `action`을 `upload_replace_section`으로 설정하고, 계속 작성할 사용자 정의 업로드 노래 ID를 입력해야 합니다. 노래 ID는 [Producer Upload API](https://platform.acedata.cloud/documents/producer-upload)를 사용하여 가져올 수 있으며, 아래 그림과 같습니다:
