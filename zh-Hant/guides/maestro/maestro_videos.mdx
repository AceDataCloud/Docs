...
| `aspect`      | string   | 否   | `9:16`（默認）/ `16:9` / `1:1`（畫面比例提示，AI 可能依據 prompt 調整）                                                                                                                                                                         |
| `duration`    | int      | 否   | 目標時長（秒，**1–600**，即最長 10 分鐘），默認 30。**按實際成片時長計費**，越長越貴（見下方計費）                                                                                                                                                                  |
| `quality`     | string   | 否   | 製作檔位（作用在時長價格上的倍率）：`draft`（快速預覽粗剪，約為標準檔 **0.5×**）/ `standard`（默認，均衡，**1×**）/ `premium`（更精細、更出彩，耗時更久，約為標準檔 **2×**）                                                                                                             |
| `scenario`    | string   | 否   | 視頻類型路由提示（僅為提示，AI 導演仍決定最終結構）：`auto`（默認）/ `narrated`（多場景真實照片 + 旁白 + 數據卡）/ `drama`（角色 + 對白的短劇）/ `avatar`（數字人 / 口播，需用 `file_urls` 傳人像）/ `motion`（抽象動感字幕 / 數據 / logo 動效）/ `slideshow`（演示 / 路演 deck）                               |
...
- 短劇：`scenario: "drama"`（角色 + 對白）。
- 數字人 / 口播：`scenario: "avatar"`，並用 `file_urls` 傳一張人像。
- 動感字幕 / 數據動畫：`scenario: "motion"`；演示 / 路演：`scenario: "slideshow"`。
...
在 `langs` 中傳入多個語言即可一次產出多語言版本。第一個為主語言，之後每多一種語言會**復用同一套畫面**，只額外配音 + 渲染，因此**每多一種語言僅 +6 積分**。示例：
...
**任務完成後按實際成片計費，失敗的任務不扣費。** 計費以實際交付的成片時長與實際產出的語言數為準——例如請求 600 秒但最終成片 50 秒，只按約 50 秒計費；某個語言最終沒有產出時，也不會收取該語言的 +6 加價。提交任務本身不單獨計費（結果由 `/maestro/tasks` 免費輪詢）。

單個成片的積分大致按下式估算：

```
積分 ≈ 0.85 × 成片時長秒 × 質量倍率 × 場景倍率 + 6 × max(語言數 − 1, 0)
```

- 質量倍率：`draft` 0.5× / `standard` 1×（默認）/ `premium` 2×。
- 場景倍率：`drama` 1.35× / `avatar` 1.15× / 其他 1×。

以標準檔、非短劇 / 數字人、單語言為例，不同成片時長對應的積分約為：
```
| 成片時長                        | 標準檔積分 |
| --------------------------- | ----- |
| 30s                         | 25.5  |
