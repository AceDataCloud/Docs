...
| `aspect`      | string   | 否   | `9:16`（默認）/ `16:9` / `1:1`。Lite 輸出 720p/24fps，Standard 與 Pro 輸出 1080p/30fps                                                                                                                                                  |
| `duration`    | int      | 否   | 目標時長（秒），默認 30。Lite 為 **5–30 秒**，Standard 為 **5–120 秒**，Pro 為 **5–300 秒**。按實際成片時長計費，但不會超過請求時長                                                                                                                                 |
| `quality`     | string   | 否   | 製作檔位：`lite`（快速短視頻，0.20 積分/秒）/ `standard`（默認，均衡製作，0.60 積分/秒）/ `pro`（高級製作，1.20 積分/秒）                                                                                                                                           |
| `scenario`    | string   | 否   | 視頻類型：Lite 支持 `auto` / `narrated` / `captions`；Standard 增加 `avatar`；Pro 增加 `drama`。`captions` 需傳源視頻，`avatar` 需傳人像                                                                                                             |
...
```
積分 = 成片時長秒 × SKU 每秒價格 × 場景倍率 + 6 × max(語言數 − 1, 0)
```
| SKU        |    每秒价格 |  最长时长 | 输出                | 场景与动作                                |
| ---------- | ------: | ----: | ----------------- | ------------------------------------ |
| `lite`     | 0.20 積分 |  30 秒 | 720p / 24fps      | auto、narrated、captions；generate、edit |
| `standard` | 0.60 積分 | 120 秒 | 1080p / 30fps     | 增加 avatar；增加 remix                   |
| `pro`      | 1.20 積分 | 300 秒 | 1080p / 30fps 高碼率 | 增加 drama；增加 extend                   |
