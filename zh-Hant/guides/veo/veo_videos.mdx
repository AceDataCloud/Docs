...
要使用 Veo Videos Generation API，首先到 [Ace Data Cloud 控制台](https://platform.acedata.cloud/console/applications) 獲取您的 API Token，留作備用。
...
> 📘 完整文檔：[Veo Videos Generation API →](https://platform.acedata.cloud/documents/veo-videos)

## 基本使用

首先先了解下基本的使用方式，就是輸入提示詞 `prompt`、 生成行為 `action`、首尾幀參考圖片數組 `image_urls` 以及模型 `model`，便可獲得處理後的結果，首先需要簡單地傳遞一個 `action` 字段，它的值為 `text2video`，它主要包含三種行為：文生視頻（`text2video`）、圖生視頻（`image2video`）、獲取1080p視頻（`get1080p`），然後我們還需要輸入模型 `model`，目前主要有 `veo31-fast`、`veo3`、`veo31`、`veo2-fast`、`veo3-fast` 和 `veo31-fast-ingredients` 模型，具體的內容如下：
...
- `model`：生成視頻的模型，主要有 `veo31-fast`、`veo3`、`veo31`、`veo2-fast`、`veo3-fast` 和 `veo31-fast-ingredients` 模型。
- `action`：此次視頻生成任務的行為，主要包含三種行為，分別為：文生視頻（`text2video`）、圖生視頻（`image2video`）、獲取1080p視頻（`get1080p`）。
- `image_urls`：當選擇圖生視頻行為 `image2video` 就必須需要上傳的參考圖片鏈接。`veo2-fast` 僅支持 1 張，`veo31-fast-ingredients` 最多 3 張（多圖融合），其餘模型最多 2 張（首尾幀模式）。
...
| **veo3-fast**              | 文生視頻（無圖）<br />圖生視頻 模式（有圖）           | **1 張** → 首幀模式<br />**2 張** → 首尾幀模式 |
| **veo31-fast**             | 文生視頻（無圖）<br />圖生視頻 模式（有圖）           | **1 張** → 首幀模式<br />**2 張** → 首尾幀模式 |
| **veo31-fast-ingredients** | ❌ 文生視頻（不支持）<br />✅ **強制多圖融合**（必須傳圖） | **1-3 張** → 多圖融合模式（最多 3 張）        |
| **veo3**                   | 文生視頻（無圖）<br />圖生視頻 模式（有圖）           | **1 張** → 首幀模式<br />**2 張** → 首尾幀模式 |
| **veo31**                  | 文生視頻（無圖）<br />圖生視頻 模式（有圖）           | **1 張** → 首幀模式<br />**2 張** → 首尾幀模式 |
...
- **圖片數量限制**：
  - `veo2-fast` 僅支持 **1 張**圖片輸入（首幀模式）。
  - `veo31-fast-ingredients` 支持 **1-3 張**圖片輸入（多圖融合模式）。
  - 其他模型最多支持 **2 張**圖片輸入（首尾幀模式）。
...
-d '{
  "action": "text2video",
  "model": "veo31-fast",
  "prompt": "白色陶瓷咖啡杯在光滑的大理石檯面上，晨光透過窗戶照射。相機緩慢地圍繞咖啡杯旋轉360度，短暫停留在把手上。"
}'
```
...
- `model`：生成視頻的模型，主要有`veo31-fast`、`veo3`、`veo31`、`veo2-fast`、`veo3-fast` 和 `veo31-fast-ingredients`。
- `image_urls`：當選擇圖生視頻行為 `image2video` 就必須需要上傳的參考圖片鏈接。
- `prompt`：提示詞。
...
如果想指定生成自定義尺寸的Veo的視頻，可以將參數 `aspect_ratio` 設置為想要的尺寸，接下來我們要必須填下一步需要擴展的提示詞來自定義生成視頻，就可以指定如下內容：
