...
首先先了解下基本的使用方式，就是輸入提示詞 `prompt`、 生成行為 `action`、首尾幀參考圖片數組 `image_urls` 以及模型 `model`，便可獲得處理後的結果，首先需要簡單地傳遞一個 `action` 字段，它的值為 `text2video`，它主要包含三種行為：文生視頻（`text2video`）、圖生視頻（`image2video`）、獲取1080p視頻（`get1080p`），然後我們還需要輸入模型 `model`，目前主要有 `veo31-fast`、`veo3`、`veo31`、`veo3-fast` 和 `veo31-fast-ingredients` 模型，具體的內容如下：
...
- `model`：生成視頻的模型，主要有 `veo31-fast`、`veo3`、`veo31`、`veo3-fast` 和 `veo31-fast-ingredients` 模型。
- `action`：此次視頻生成任務的行為，主要包含三種行為，分別為：文生視頻（`text2video`）、圖生視頻（`image2video`）、獲取1080p視頻（`get1080p`）。
- `image_urls`：當選擇圖生視頻行為 `image2video` 就必須需要上傳的參考圖片鏈接。`veo31-fast-ingredients` 最多 3 張（多圖融合），其餘模型最多 2 張（首尾幀模式）。
...
- `model`：生成視頻的模型，主要有`veo31-fast`、`veo3`、`veo31`、`veo3-fast` 和 `veo31-fast-ingredients`。
