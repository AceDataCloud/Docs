...
本文檔主要介紹 GLM Chat Completion API 的使用流程，利用它您可以通過統一的 OpenAI 兼容接口輕鬆調用 GLM 系列模型。需要在 Coze（扣子）中使用 `glm-5.2` 時，請參考 [Coze 自定義模型教程](https://platform.acedata.cloud/documents/coze-integration)，並選擇 OpenAI Chat Completions 協議。
...
- `model`：選擇要調用的 GLM 模型，目前支持的模型包括：
  - `glm-5.2`：最新旗艦模型，適合複雜推理、代碼與 Agent 任務。
  - `glm-5.1`：上一代旗艦模型，綜合能力強。
