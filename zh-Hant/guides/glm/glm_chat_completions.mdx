...
GLM（General Language Model）是智譜 AI（Zhipu AI / Z.ai）推出的新一代大語言模型系列，具備強大的中英文理解與生成能力，在中文場景、代碼生成、推理與多輪對話等任務上都有出色表現。GLM-5.1、GLM-4.7、GLM-4.6 等新一代模型在長上下文、工具調用與代碼任務上做了大量優化，可廣泛應用於智能問答、內容創作、代碼輔助、客服機器人等場景。
...
| 模型            | 适用场景                  |
| ------------- | --------------------- |
| `glm-5.1`     | 綜合能力最強，推薦用於複雜推理、長文檔分析 |
| `glm-4.7`     | 工具調用、代碼生成、Agent 編排等任務 |
| `glm-4.6`     | 通用對話、內容創作的均衡選擇        |
| `glm-3-turbo` | 一般文本生成任務，對成本敏感的場景     |

## 錯誤處理

在調用 API 時，如果遇到錯誤，API 會返回相應的錯誤代碼和信息。例如：

- `400 token_mismatched`：請求參數缺失或無效。
- `400 api_not_implemented`：使用了不被支持的參數或模型。
- `401 invalid_token`：未授權，Bearer Token 缺失或失效。
- `429 too_many_requests`：觸發頻率限制，請稍後重試。
- `500 api_error`：伺服器內部錯誤或上游暫時不可用。
...
當返回 `api_error` 且消息為 `服務暫時不可用，請稍後重試。` 時，通常表示上游 GLM 服務暫時不可用，建議帶指數退避重試，或切換到其他可用的 GLM 模型（例如從 `glm-5.1` 臨時切換到 `glm-4.7` 或 `glm-4.6`）。

## 結論

通過本文檔，您已經了解了如何使用 GLM Chat Completion API 調用智譜 AI 的 GLM 系列模型，包括基本調用、流式響應、多輪對話、系統提示詞與工具調用等典型用法。希望本文檔能幫助您更好地對接和使用該 API。如有任何問題，請隨時聯繫我們的技術支持團隊。
