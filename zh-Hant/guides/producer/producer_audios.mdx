...
如果想對自己上傳的歌曲進行繼續翻唱的話，可以將參數 `action` 設置為 `upload_cover` ，並且輸入需要繼續翻唱自定義上傳的歌曲 ID，歌曲 ID 的獲取是使用 [Producer Upload API](https://platform.acedata.cloud/documents/producer-upload)來獲取，如下圖所示：
