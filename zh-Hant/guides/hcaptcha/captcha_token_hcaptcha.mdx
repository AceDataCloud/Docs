...
計費說明：非同步模式下，創建任務與輪詢「處理中」都不計費；**僅在成功取到 token 時計費一次**（與同步模式的價格一致）。因此在輪換中取消尚未完成的任務不會產生費用。`/captcha/tasks` 對所有 token 類接口（hcaptcha、recaptcha2、recaptcha3 等）通用，用同一個 `task_id` 輪詢即可。
