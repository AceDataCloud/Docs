...
本文介紹驗證碼非同步任務查詢介面 `POST /captcha/tasks`。當你在調用任意驗證碼介面（token 系列或 recognition 系列）時傳入 `async: true`，介面會立即返回一個 `task_id`，隨後即可用該 `task_id` 輪詢本介面獲取最終結果。適用於多打碼器輪換（multi-solver rotation）等場景：提交任務後立即拿到 `task_id`，先去調度其他打碼器，稍後再回來取結果。
...
如果任務無法在有效時間內完成，會返回 HTTP 504。該狀態是終態，客戶端應停止輪詢；重複查詢同一個 `task_id` 會穩定返回相同的失敗結果：
...
非同步模式下，創建任務與輪詢「處理中」都不計費；**僅在成功取到結果時計費一次**（與同步模式的價格一致）。HTTP 504 終態失敗也不計費。因此在輪換中取消尚未完成的任務，或任務最終失敗，都不會產生費用。
