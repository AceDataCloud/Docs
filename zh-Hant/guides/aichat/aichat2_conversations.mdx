...
> `stateful` 預設為 `true`，省略和明確傳遞 `true` 等價。如果你不希望服務端保存這一輪對話，可以明確設置 `stateful: false`。

## 流式響應

v2 支持兩種流式格式，按照 `accept` 標頭選擇：

| 場景                    | `accept`               | 數據形態                                       |
| --------------------- | ---------------------- | ------------------------------------------ |
| Web 前端 / EventSource  | `text/event-stream`    | `data: {json}\n\n`，最後一行 `data: [DONE]\n\n` |
| 服務端 / CLI / Node 流式解析 | `application/x-ndjson` | 每行一個 JSON 對象                               |
...
with requests.post(url, json=payload, headers=headers, stream=True) as resp:
    ...
    for line in resp.iter_lines():
        ...
        if event.get("type") == "text_delta":
            # 與 v1 兼容：增量片段同時通過 delta_answer 字段提供
...
瀏覽器端使用 `EventSource` 不支持自定義請求體，建議使用 `fetch` + 手動按 `\n\n` 切片解析：
...
| `type`              | 含義                                                                                              |
| ------------------- | ----------------------------------------------------------------------------------------------- |
| `text_delta`        | 助手回答的增量文本片段。`content` 為新增內容；為兼容 v1，同一事件還攜帶 `delta_answer`（等於 `content`）和 `id`。                  |
| `thinking`          | 模型的思考過程（僅在所選模型暴露 reasoning 時出現）。                                                                |
| `tool_use`          | 模型決定調用一個工具，事件攜帶 `tool_id`、`tool_name`、`input`。                                                  |
| `tool_result`       | 工具執行結果，與上一條 `tool_use` 通過 `tool_id` 配對，`is_error` 標識是否失敗。                                       |
| `card`              | 工具產出的結構化卡片（如圖片、鏈接預覽），適合直接渲染。                                                                    |
| `citation`          | 用於補充對應文本片段引用的來源 URL。                                                                            |
| `ask_user_question` | 模型需要用戶補充信息時發出，對話進入 `awaiting_user_input` 狀態，詳見下文 [恢復暫停的對話](#恢復暫停的對話)。                           |
| `artifact`          | 模型生成的獨立產物（如代碼塊、文檔），可保存或下載。                                                                      |
| `system_message`    | 系統提示信息（非用戶與助手內容），僅用於 UI 提示。                                                                     |
| `compact`           | 內部上下文被壓縮的事件，無需特殊處理。                                                                             |
| `error`             | 本輪發生錯誤，`message` 描述錯誤內容。                                                                        |
| `done`              | 流式響應結束，攜帶 `usage`（含 `prompt_tokens` / `completion_tokens` / `total_tokens`）和 `terminal_reason`。 |

對於只關心最終答案的客戶端，把所有 `text_delta` 的 `content` 拼接起來就和 `application/json` 模式下的 `answer` 等價。

## 多模態輸入

如果用戶輸入包含圖片或文件，傳 `message`（數組）代替 `question`。每個數組元素是一個內容塊：
...
為了兼容舊客戶端，v2 仍然識別 `references: ["https://...", ...]` 字段：
