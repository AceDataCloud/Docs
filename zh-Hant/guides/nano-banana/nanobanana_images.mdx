...
- **圖片數量**：可選，通過 `count` 指定 1–4 張，默認 1 張；每張圖片由獨立生成調用完成；普通技術失敗或提供方安全拒絕只影響對應調用，其餘成功圖片照常返回，並按實際成功張數計費
...
- `count`：請求生成或編輯的圖片數量，支持 1–4，默認 1。`data` 只包含生成成功的圖片，並按實際返回張數計費。每個生成調用都強制使用提供方原生安全策略；某個調用被拒不會影響其他成功調用，全部調用均被拒時返回 403。
- `data[]`：結果列表。
  - `prompt`：用於生成的提示詞（回顯）。
...
- **403 `forbidden`**：提供方原生安全策略拒絕了請求或生成結果。該調用不會返回圖片，也不會計費；多圖請求仍可返回並計費其他成功調用。
