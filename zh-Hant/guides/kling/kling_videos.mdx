...
本文將介紹一種 Kling Videos Generation API 對接說明，它是可以透過輸入自定義參數來生成Kling官方的視頻。
...
首先先了解下基本的使用方式，就是輸入提示詞 `prompt`、 生成行為 `action`、首幀參考圖片 `start_image_url` 以及模型 `model`，便可獲得處理後的結果，首先需要簡單地傳遞一個 `action` 字段，它的值為 `text2video`，它主要包含三種行為：文生視頻（`text2video`）、圖生視頻（`image2video`）、擴展視頻（`extend`），然後我們還需要輸入模型 `model`，目前主要有 `kling-v1`, `kling-v1-6`, `kling-v2-master`, `kling-v2-1-master`, `kling-v2-5-turbo`, `kling-v2-6`, `kling-v3`, `kling-v3-omni`, `kling-video-o1` 模型，具體的內容如下：
...
- `model`：生成視頻的模型，主要有 `kling-v1`, `kling-v1-6`, `kling-v2-master`, `kling-v2-1-master`, `kling-v2-5-turbo`, `kling-v2-6`, `kling-v3`, `kling-v3-omni`, `kling-video-o1` 模型。
...
- `duration`：視頻時長，單位秒。對於 `kling-v3` 和 `kling-v3-omni` 模型支持靈活時長 3-15 秒（整數），其他模型支持 5 或 10 秒。
- `generate_audio`：是否同步生成音頻，可選，布林值。支持 `kling-v3`、`kling-v3-omni` 以及 `kling-v2-6`（僅 pro 模式）。默認為 `false`。
...
- `element_list`：主體參考列表，只適用模型 `kling-video-o1`，用於引用主體庫中的元素 ID，用法見下文「Omni 全能參考」。
- `image_list`：參考圖片列表（多圖參考），只適用模型 `kling-video-o1`，用法見下文「Omni 全能參考」。
- `video_list`：參考視頻列表（支持視頻編輯），只適用模型 `kling-video-o1`，用法見下文「Omni 全能參考」。
- `prompt`：提示詞。
...
## 模型能力矩陣
不同模型對參數的支持情況差異較大。以下矩陣整理自 [Kling 官方 video models 文檔](https://app.klingai.com/global/dev/document-api/apiReference/model/videoModels)，調用前請先核對當前 `model` / `mode` / `duration` 組合是否支持你所需的功能，否則上游會返回 `model/mode/duration(...) is not supported with image_tail` 等錯誤。

| 模型                  | 模式             | `end_image_url`（首尾幀） | `generate_audio`（伴音） | `camera_control`（運鏡） | 備註                                           |
| ------------------- | -------------- | -------------------- | -------------------- | -------------------- | -------------------------------------------- |
| `kling-v1`          | std / pro      | ✅ 僅 `duration=5`     | ❌                    | ✅ 僅 `duration=5`     | `extend` 不支持 `negative_prompt` 與 `cfg_scale` |
| `kling-v1-6`        | std            | ❌                    | ❌                    | ❌                    | 多圖生視頻、`extend` 全模式可用                         |
| `kling-v1-6`        | pro            | ✅                    | ❌                    | ❌                    |                                              |
...
| `kling-v2-6`        | std            | ❌                    | ❌                    | ❌                    |                                              |
| `kling-v2-6`        | pro            | ✅                    | ✅                    | ❌                    | 唯一同時支持伴音的非 v3 模型                             |
| `kling-v3`          | std / pro      | ✅                    | ✅                    | ✅                    | `duration` 範圍 3–15 秒                         |
| `kling-v3`          | 4k             | ✅                    | ✅                    | ❌                    | 4K 模式與運鏡不兼容                                  |
| `kling-v3-omni`     | std / pro / 4k | ✅                    | ✅                    | ❌                    |                                              |
| `kling-video-o1`    | std / pro      | ✅                    | ❌                    | ❌                    | 僅支持 `duration=5/10`                          |

注意事項：

- `mode=4k` 僅 `kling-v3` 與 `kling-v3-omni` 支持；並且與 `camera_control`（運鏡）互斥。
...
- `generate_audio` 默認 `false`。僅 `kling-v3`、`kling-v3-omni` 和 `kling-v2-6`（pro 模式）支持。
...
- `mode`：生成視頻的模式，可選值為標準模式 `std`、極速模式 `pro` 和原生 4K 模式 `4k`（僅 `kling-v3` 和 `kling-v3-omni` 支持，與運鏡控制不兼容）。
...
## Omni 全能參考（視頻編輯 / 參考視頻 / 多圖 / 主體參考）

`kling-video-o1` 模型支持「全能參考」能力：在文生視頻（`action=text2video`）的基礎上，額外傳入參考圖片、參考主體或參考視頻，即可實現**多圖參考、主體參考、參考視頻、以及直接對已有視頻進行編輯**。

**核心約定**：參考素材必須在 `prompt` 中以 `&lt;&lt;<image_1>>>`、`&lt;&lt;<element_1>>>`、`&lt;&lt;<video_1>>>` 的形式（序號從 1 開始）引用 `image_list` / `element_list` / `video_list` 中對應位置的素材，模型才會應用這些參考。若只傳素材而不在提示詞中引用，素材會被忽略。

### 參考視頻與視頻編輯（`video_list`）
...
使用時需在 `prompt` 中以 `&lt;&lt;<image_1>>>`、`&lt;&lt;<image_2>>>` 引用。數量限制：不存在參考視頻時，參考圖片 + 多圖主體之和 ≤ 7；存在參考視頻時 ≤ 4。僅傳首 / 尾幀時也可直接用 `start_image_url` / `end_image_url`。
...
`element_list` 用於引用「主體庫」中已上傳的元素，數組元素字段為 `element_id`（主體庫中的元素 ID，整數）。使用時同樣需在 `prompt` 中以 `&lt;&lt;<element_1>>>` 引用。參考主體數量需與參考圖片數量合併計算：存在參考視頻時兩者之和 ≤ 4，不存在參考視頻時 ≤ 7。
...
- `401 invalid_token`：未授權，無效或缺失授權令牌。
