...
> 安全说明：当前 API 不开放 `element_list`。Kling Element Library 的上游 ID 属于提供商账号命名空间，在提供租户隔离的 Element Management API 之前，客户应使用 `image_list` 传入主体参考图。
...
由於 Kling Videos Generation API生成的時間相對較長，大約需要 1-2 分鐘，如果 API 長時間無響應，HTTP 請求會一直保持連接，導致額外的系統資源消耗，所以本 API 也提供了非同步回調的支持。
