...
不同模型對參數的支持情況差異較大。以下矩陣整理自 [Kling 官方 video models 文檔](https://app.klingai.com/global/dev/document-api/apiReference/model/videoModels)，調用前請先核對當前 `model` / `mode` / `duration` 組合是否支持你所需的功能，否則會返回 `model/mode/duration(...) is not supported with image_tail` 等錯誤。
...
- `video_url`：參考視頻鏈接，不可為空。最多 1 個 MP4/MOV 視頻，文件大小 ≤200MB，幀率 24–60fps。`kling-o1` 要求時長 3–10 秒、寬和高各 700–2160px；`kling-v3-omni` 要求時長 3–15.5 秒、寬和高各 700–4553px、總像素 ≤8,294,400、寬高比 0.4–2。
