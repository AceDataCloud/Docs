...
本文將介紹一種 Kling Motion Generation API 對接說明，它是可以透過輸入自定義參數來生成Kling官方的視頻。

## 申請流程

要使用 Kling Motion Generation API，首先到 [Ace Data Cloud 控制台](https://platform.acedata.cloud/console/applications) 獲取您的 API Token，留作備用。
...
> 📘 完整文檔：[Kling Motion Generation API →](https://platform.acedata.cloud/documents/kling-motion)
...
- `image_url`：人物外觀參考圖 URL。支持 JPG/JPEG/PNG，文件 ≤50MB，寬和高均 ≥300px，寬高比 1:2.5～2.5:1；人物應清晰顯示上半身或全身及頭部。
- `video_url`：動作參考視頻 URL。支持 MP4/MOV，文件 ≤100MB，寬和高各 340–3850px，至少 3 秒；`character_orientation=image` 時最長 10 秒，`character_orientation=video` 時最長 30 秒。建議使用人物始終在畫面內的連續單鏡頭視頻。
