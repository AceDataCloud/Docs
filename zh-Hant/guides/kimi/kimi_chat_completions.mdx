...
可以看到，`choices` 包含的信息與基本使用的內容是一致的，這個包含了 Kimi 對於多個對話進行回覆的具體內容，這樣就可以根據多個對話內容來回答對應的問題了。
...
通過本文檔，您已經了解了如何使用 Kimi Chat Completion API 實現普通對話、流式響應、多輪對話，以及通過 `reasoning_effort` 控制 K3 的推理強度。
