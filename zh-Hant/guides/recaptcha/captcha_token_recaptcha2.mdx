...
要使用 Recaptcha2 協議識別 API，首先到 [Ace Data Cloud 控制台](https://platform.acedata.cloud/console/applications) 獲取您的 API Token，留作備用。
...
> 📘 完整文檔：[Recaptcha2 協議識別 API →](https://platform.acedata.cloud/documents/captcha-token-recaptcha2)
...
- `proxy`：可選，自帶代理（Bring Your Own Proxy）。設置後上游會通過你提供的代理 IP 去解驗證碼，用於控制出口 IP 質量（例如避免因公共代理 IP 被目標站點攔截而返回 `410 Gone`）。格式為 `scheme://[user:pass@]host:port`，`scheme` 支持 `http`/`https`/`socks4`/`socks5`，例如 `http://user:pass@1.2.3.4:8080`。不填則使用平台默認代理。
...
- `token`，此次 Recaptcha2驗證碼任務處理後驗證結果。
- `started_at`、`finished_at`：本次請求開始處理與產出結果的時間，Unix 時間戳（秒，浮點）。
- `elapsed`：本次處理的總耗時（秒）。

可以看到我們得到了處理 Recaptcha2驗證碼的驗證結果，然後我們可以用於POST或模擬提交給目標網站，一次性使用，有效期120s，建議在60s內使用，接下來將提供一段Python版本將處理後token提交到目標網站來通過Recaptcha2驗證碼。
...
計費說明：異步模式下，創建任務與輪詢「處理中」都不計費；**僅在成功取到結果時計費一次**（與同步模式的價格一致）。因此在輪換中取消尚未完成的任務不會產生費用。`/captcha/tasks` 對所有驗證碼接口（token 與 recognition 系列）通用，用同一個 `task_id` 輪詢即可。
