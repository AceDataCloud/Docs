...
- `proxy`：可選，自帶代理（Bring Your Own Proxy）。設置後服務會通過你提供的代理 IP 去解驗證碼，用於控制出口 IP 質量（例如避免因公共代理 IP 被目標站點攔截而返回 `410 Gone`）。格式為 `scheme://[user:pass@]host:port`，`scheme` 支持 `http`/`https`/`socks4`/`socks5`，例如 `http://user:pass@1.2.3.4:8080`。不填則使用平台默認代理。
