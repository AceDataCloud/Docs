...
- `started_at`、`finished_at`：本次請求開始處理與產出結果的時間（ISO-8601 UTC）。
- `elapsed`：本次處理的總耗時（秒）。
...
隨後使用該 `task_id` 轮询 `POST /captcha/tasks`（建議每 3~5 秒一次）來獲取結果：
...
通過本文檔，您已經了解了如何使用 Recaptcha2 圖像識別 API 讓用戶輸入識別的內容和 Recaptcha2驗證碼圖像，最後返回需要點擊的小圖像的坐標，完成驗證。希望本文檔能幫助您更好地對接和使用該 API。如有任何問題，請隨時聯繫我們的技術支持團隊。
