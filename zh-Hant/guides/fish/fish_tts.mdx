...
本接口提供文本轉語音、已保存聲音調用和一次性即時聲音克隆，地址為 `POST https://api.acedata.cloud/fish/tts`。
...
| `reference_id` | string \| string[] | 否   | 已保存或公共聲音的模型 ID，可由 [Fish Model API](https://platform.acedata.cloud/documents/fish-model) 創建，或在 [Fish Model Query](https://platform.acedata.cloud/documents/fish-model-query) 中檢索。不能與 `references` 同時使用。 |
| `references`   | object[]           | 否   | 一次性即時聲音克隆，僅支持一個 `{audio, text}` 樣本：`audio` 是公開 HTTPS MP3/WAV URL，`text` 是音頻的準確逐字稿。不能與 `reference_id` 同時使用。                                                                                             |
...
> 一次性克隆只接受 HTTPS 音頻 URL，不接受 MessagePack、Base64、data URI 或帶憑據的 URL。參考音頻建議 10–270 秒；Studio 使用更保守的 10–60 秒範圍。
...
`audio_url` 指向本平台 CDN，可直接 GET 下載或在 `<audio>` 中播放。最終成功響應還會返回頂層 `cost`，其中 `amount` 是本次實際扣除的 Credits；如有賬戶折扣，`list_amount` 表示折扣前額度。鏈接長期可用，但仍建議在你自己的存儲裡留一份。
...
## 示例 3：一次性即時聲音克隆（`references`）

將參考音頻放在公開 HTTPS 地址，並提供其中實際說出的準確原文。該聲音只用於本次合成，不會創建長期模型：
