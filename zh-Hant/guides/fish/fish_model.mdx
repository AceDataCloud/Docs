...
Fish Model API 與 Fish Audio 官方 OpenAPI 的 ModelEntity 介面完全相容，可在零程式碼改動的前提下遷移已有的克隆音色管理程式碼。建立出的音色 `_id` 可以直接餵給 [Fish TTS API](https://platform.acedata.cloud/documents/fish-tts) 的 `reference_id` 欄位進行語音合成。
