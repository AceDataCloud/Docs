...
要使用 Fish Audios Generation API，首先到 [Ace Data Cloud 控制台](https://platform.acedata.cloud/console/applications) 獲取您的 API Token，留作備用。
...
> 📘 完整文檔：[Fish Audios Generation API →](https://platform.acedata.cloud/documents/fish-tts)

## 請求示例

Fish Tasks API 可以用於查詢 Fish Audios Generation API 的結果。關於怎樣使用 Fish Audios Generation API，請參考文檔 [Fish Videos Generation API ](https://platform.acedata.cloud/documents/fish-tts-integration)。

我們以 Fish Audios Generation API 服務返回的任務ID一個為例，演示如何使用該 API。假設我們有一個任務ID：2725a2d3-f87e-4905-9c53-9988d5a7b2f5，接下來演示如何通過傳入一個任務ID來。
