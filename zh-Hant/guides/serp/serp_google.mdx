---
title: "Google SERP API 對接說明"
description: "Search Engine 整合指南 - Ace Data Cloud"
---

Google SERP（Search Engine Results Page）是用戶在 Google 搜尋引擎中輸入查詢後看到的結果頁面。它顯示自然搜尋結果、廣告、特色摘要、知識圖譜以及圖片、視頻等多種內容，旨在為用戶提供最相關的信息。

本文將詳細介紹 Google SERP API，它可以提供在 Google 搜尋引擎中輸入查詢獲取結果，結果的內容也是包含了許多類型，比如特色摘要、知識圖譜以及圖片等多種結果。

本文檔會介紹下 Google SERP API 的對接說明。

## 申請流程

要使用 Google SERP API，首先到 [Ace Data Cloud 控制台](https://platform.acedata.cloud/console/applications) 獲取您的 API Token，留作備用。

![](https://cdn.acedata.cloud/dvc3cg.jpg)

如果你尚未登錄或註冊，會自動跳轉到登錄頁面邀請你註冊和登錄，完成後會自動返回當前頁面。

**一個 API Token 即可調用平台所有服務，無需為每個服務單獨申請。** 首次申請會贈送免費額度，可免費體驗；額度不足時可在 [控制台](https://platform.acedata.cloud/console/coin) 充值通用餘額。

> 📘 完整文檔：[Google SERP API →](https://platform.acedata.cloud/documents/serp-google)

## 基本使用

首先先了解下基本的使用方式，就是輸入搜尋資源類型和關鍵詞，就能獲得搜尋結果，只需要簡單地傳遞 `query` 欄位，並指定相應模型即可。

比如說查找：關於“apple inc”的信息，我們接下來就可以在介面上填寫對應的內容，如圖所示：

<p><img src="https://cdn.acedata.cloud/lnqiye.png" width="500" className="m-auto" /></p>

可以看到這裡我們設置了 Request Headers，包括：

- `accept`：想要接收怎樣格式的響應結果，這裡填寫為 `application/json`，即 JSON 格式。
- `authorization`：調用 API 的密鑰，申請之後可以直接下拉選擇。

另外設置了 Request Body，包括：

- `type`：搜尋資源的類型，目前只支持六種類型，默認是 `search`。
- `query`：搜尋的關鍵詞。
- `country`：自定義搜尋結果的所在國家，默認是美國（US）。
- `language`：自定義搜尋結果的語言，默認是英語（en）。
- `range`：自定義搜尋結果的時間範圍，默認是沒有限制的。
- `number`：自定義搜尋結果的分頁頁面大小，默認是 10。
- `page`：自定義搜尋結果的分頁頁數，默認是 1。

選擇之後，可以發現右側也生成了對應代碼，如圖所示：

<p><img src="https://cdn.acedata.cloud/1j81zr.png" width="500" className="m-auto" /></p>

點擊「Try」按鈕即可進行測試，如上圖所示，這裡我們就得到了如下結果：
```json
{
  "knowledge_graph": {
    "title": "蘋果",
    "type": "科技公司",
    "website": "http://www.apple.com/",
    "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT5ITHsQzdzkkFWKinRe1Y4FUbC_Vy3R_M&s=0",
    "description": "蘋果公司是一家美國跨國公司和科技公司，總部位於加利福尼亞州庫比蒂諾，位於矽谷。它以消費電子產品、軟體和服務而聞名。",
    "description_source": "維基百科",
    "description_link": "https://en.wikipedia.org/wiki/Apple_Inc.",
    "attributes": {
      "Customer service": "1 (800) 275-2273",
      "Founders": "史蒂夫·喬布斯、史蒂夫·沃茲尼亞克和羅納德·韋恩",
      "Founded": "1976年4月1日，加州洛斯阿圖斯",
      "Headquarters": "加州庫比蒂諾",
      "CEO": "蒂姆·庫克 (2011年8月24日–)"
    }
  },
  "organic": [
    {
      "title": "蘋果",
      "link": "https://www.apple.com/",
      "snippet": "探索蘋果的創新世界，購買所有iPhone、iPad、Apple Watch、Mac和Apple TV，還可以探索配件、娛樂等...",
      "sitelinks": [
        {
          "title": "支援",
          "link": "https://support.apple.com/"
        },
        {
          "title": "職業機會",
          "link": "https://www.apple.com/careers/us/"
        },
        {
          "title": "商業",
          "link": "https://www.apple.com/business/"
        },
        {
          "title": "商店",
          "link": "https://www.apple.com/store"
        },
        {
          "title": "投資者關係",
          "link": "https://investor.apple.com/investor-relations/default.aspx"
        }
      ],
      "position": 1
    },
    {
      "title": "蘋果公司 - 維基百科",
      "link": "https://en.wikipedia.org/wiki/Apple_Inc.",
      "snippet": "蘋果公司是一家美國跨國公司和科技公司，總部位於加利福尼亞州庫比蒂諾，位於矽谷。",
      "position": 2
    },
    {
      "title": "蘋果公司 | 歷史、產品、總部及事實 - 大英百科全書",
      "link": "https://www.britannica.com/money/Apple-Inc",
      "snippet": "美國個人電腦、智能手機和平板電腦的製造商。蘋果是第一家成功的個人電腦公司...",
      "date": "7天前",
      "position": 3
    },
    {
      "title": "蘋果公司 (AAPL) 公司簡介及事實 - 雅虎財經",
      "link": "https://finance.yahoo.com/quote/AAPL/profile/",
      "snippet": "蘋果公司設計、製造和銷售智能手機、個人電腦、平板電腦、可穿戴設備和配件，並在全球範圍內銷售各種相關配件。",
      "position": 4
    },
    {
      "title": "AAPL: 蘋果公司股票價格報價 - NASDAQ GS - 彭博",
      "link": "https://www.bloomberg.com/quote/AAPL:US",
      "snippet": "蘋果公司設計、製造和銷售智能手機、個人電腦、平板電腦、可穿戴設備和配件，並銷售各種相關配件。",
      "position": 5
    },
    {
      "title": "蘋果公司 (AAPL) 今日股票價格 - 華爾街日報",
      "link": "https://www.wsj.com/market-data/quotes/AAPL?gaa_at=eafs&gaa_n=ASWzDAiLDLUKmLSxoaZi5n84id-eGDp9gI99K5VU1R7pK3SzSi_PbdpD61P1&gaa_ts=685148c8&gaa_sig=mGk3lG_2W7ZqrXfXSXv7GAP78KUi_uJEadAXuv2bgYJ8Ah42GhIZxrsN4ynbNg_oKD_7NZS1lRPSglUNacIheQ%3D%3D",
      "snippet": "蘋果公司從事智能手機、個人電腦、平板電腦、可穿戴設備和配件的設計、製造和銷售，以及其他各種相關產品的銷售。",
      "position": 6
    }
  ],
  "people_also_ask": [
    {
      "question": "蘋果公司是什麼？",
      "snippet": "蘋果公司，最初名為蘋果電腦公司，是一家創造和銷售消費電子產品及相關電腦軟體的跨國公司，並且是數位媒體內容的數位分銷商。蘋果的核心產品線包括iPhone智能手機、iPad平板電腦和Mac個人電腦。",
      "title": "蘋果公司歷史 - 維基百科",
      "link": "https://en.wikipedia.org/wiki/History_of_Apple_Inc."
    },
    {
      "question": "誰擁有蘋果公司？",
      "snippet": "蘋果 (AAPL) 擁有概覽 蘋果 (AAPL) 股票的擁有結構是機構、零售和個人投資者的混合。約50.57%的公司股票由機構投資者擁有，0.06%由內部人士擁有，49.38%由公共公司和個人投資者擁有。",
      "title": "誰擁有蘋果？AAPL 股票擁有權 - TipRanks.com",
      "link": "https://www.tipranks.com/stocks/aapl/ownership"
    },
    {
      "question": "蘋果公司最著名的是什麼？",
      "snippet": "它以消費電子產品、軟體和服務而聞名。由史蒂夫·喬布斯、史蒂夫·沃茲尼亞克和羅納德·韋恩於1976年成立，該公司於次年由喬布斯和沃茲尼亞克註冊為蘋果電腦公司。",
      "title": "蘋果公司 - 維基百科",
      "link": "https://en.wikipedia.org/wiki/Apple_Inc."
    },
    {
      "question": "蘋果公司中的Inc代表什麼意思？",
      "snippet": "inc的縮寫：用於美國合法成立公司的名稱中：比肖電腦服務公司，Inc。",
      "title": "Inc. | 英文意思 - 劍橋詞典",
      "link": "https://dictionary.cambridge.org/dictionary/english/inc"
    }
  ],
  "related_searches": [
    {
      "query": "蘋果 iPhone 13"
    },
    {
      "query": "蘋果公司是什麼"
    },
    {
      "query": "蘋果公司的全名"
    },
    {
      "query": "蘋果公司的地址"
    },
    {
      "query": "蘋果公司的行業"
    },
    {
      "query": "蘋果公司的投資者關係"
    },
    {
      "query": "蘋果公司年報"
    },
    {
      "query": "蘋果商店"
    }
  ]
}
```

返回結果一共有多個字段，介紹如下：

- `knowledge_graph`，搜索結果的知識圖譜。
- `organic`，搜索結果的詳細信息。
- `people_also_ask`，跟搜索關鍵詞相關的問題。
- `related_searches`，對於搜索關鍵詞的相關搜索。

可以看到，這裡返回的結果中有一個 `organic` 字段，它主要包含了搜索關鍵詞的結果。

另外如果想生成對應的對接代碼，可以直接複製生成，例如 CURL 的代碼如下：

```shell
curl -X POST 'https://api.acedata.cloud/serp/google' \
-H 'accept: application/json' \
-H 'authorization: Bearer {token}' \
-H 'content-type: application/json' \
-d '{
  "query": "蘋果公司"
}'
```

Python 的對接代碼如下：

```python
import requests

url = "https://api.acedata.cloud/serp/google"

headers = {
    "accept": "application/json",
    "authorization": "Bearer {token}",
    "content-type": "application/json"
}

payload = {
    "query": "蘋果公司"
}

response = requests.post(url, json=payload, headers=headers)
print(response.text)
```

## 自定義搜索類型

如果您自定義搜索資源的類型，我們可以修改參數 `type`，它分別包含了普通資源 `search`、圖片資源 `images`、新聞資源 `news`、地圖資源 `maps`、地區資源 `places`、視頻資源 `videos`，本文將以視頻資源 `videos` 做一個示範。

下面我們來演示下具體的操作。

首先，將 `type` 參數設置為 `videos`，並正常傳遞 `query` 參數，如圖所示：
```
<p><img src="https://cdn.acedata.cloud/czlt12.png" width="500" class="m-auto"></p>

對應的程式碼如下：

```shell
curl -X POST 'https://api.acedata.cloud/serp/google' \
-H 'accept: application/json' \
-H 'authorization: Bearer {token}' \
-H 'content-type: application/json' \
-d '{
  "type": "videos",
  "query": "apple inc"
}'
```

可以得到如下回答：

```json
{
  "videos": [
    {
      "title": "Healthcare",
      "link": "https://www.apple.com/healthcare/",
      "snippet": "更多購物方式：尋找您附近的 Apple Store 或其他零售商。或致電 1-800-MY-APPLE (1-800-692-7753)。美國。版權 © 2025 Apple Inc. 保留所有權利 ...",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRQzUYLXB37srVLPW8JbVzhIlP9-SxMe3A8M98Eb7362as4&s",
      "source": "Apple",
      "date": "2025年4月16日",
      "position": 1
    },
    {
      "title": "Apple",
      "link": "https://www.youtube.com/apple",
      "snippet": "我們的第一台空間電腦。· 媽媽的禮物 | 在空間視頻中用 iPhone 拍攝 | 在 Apple Vision Pro 上體驗 · Metallica — 官方預告片 | Apple ...",
      "position": 2
    },
    {
      "title": "Apple 終於再次冒險了嗎？",
      "link": "https://www.youtube.com/watch?v=JNZV9i5BwH0",
      "snippet": "考慮到微軟的表現，以及持續的掙扎，我會說將 Mac 移至 ARM 是一種「如果失敗就會毀掉公司的」風險 ...",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR0zvbxg3ozbeHNUJojmyRBhi3J-TviJQRinIS4DZiF-EM8&s",
      "video_url": "https://encrypted-vtbn0.gstatic.com/video?q=tbn:ANd9GcTfNZRuifDZb4zgj0d6wDxoHLI8yUr-8SIPNw",
      "duration": "13:04",
      "source": "YouTube",
      "channel": "Luke Miani",
      "date": "2天前",
      "position": 3
    },
    {
      "title": "Apple 在 WWDC 25 上未談及的內容及其重要性！",
      "link": "https://www.youtube.com/watch?v=2LTc3mzNh08",
      "snippet": "我正在慢慢取消關注那些創作激怒誘餌視頻以追隨仇恨趨勢的 YouTuber。Apple 雖然遠非完美，但這些主題 ...",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTgGUJwgxdH-WHBmGOk6XgvijwnYX2H_D2Cl23aNab8CKyT&s",
      "video_url": "https://encrypted-vtbn0.gstatic.com/video?q=tbn:ANd9GcRTtYLkgIrCxERepA5KmCJDbZ7lTUa0OBHqzw",
      "duration": "13:05",
      "source": "YouTube",
      "channel": "Brian Tong",
      "date": "7小時前",
      "position": 4
    },
    {
      "title": "Apple",
      "link": "https://www.youtube.com/user/Apple",
      "snippet": "我們的第一台空間電腦。· 媽媽的禮物 | 在空間視頻中用 iPhone 拍攝 | 在 Apple Vision Pro 上體驗 · Metallica — 官方預告片 | Apple ...",
      "position": 5
    },
    {
      "title": "Apple Watch Ultra 2",
      "link": "https://www.apple.com/apple-watch-ultra-2/",
      "snippet": "與 Huish Outdoors 合作設計的 Oceanic+ 應用程序將 Ultra 2 轉變為一個完全功能的潛水電腦，運行 Bühlmann 脫壓算法。",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSvZ58COtZS4zIIJDd1-Tl0GIKbFqoffFlhLa20OtLcILDR&s",
      "source": "Apple",
      "date": "2023年9月12日",
      "position": 6
    },
    {
      "title": "Apple 在工作 - 成功故事 - BDC",
      "link": "https://www.apple.com/business/enterprise/success-stories/financial-services/bdc/",
      "snippet": "Apple Business Manager 和 AirWatch 移動設備管理使 BDC 能夠輕鬆地在整個公司中無觸控地部署和管理設備和應用程序 ...",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRDQuxFNiitOIuPizPvTHwzQqyuLuypH_UNPqcZ-bGUAFy2&s",
      "source": "Apple",
      "date": "2021年11月2日",
      "position": 7
    },
    {
      "title": "高等教育 - 機構",
      "link": "https://www.apple.com/education/higher-education/",
      "snippet": "更多購物方式：尋找您附近的 Apple Store 或其他零售商。或致電 1-800-MY-APPLE (1-800-692-7753)。美國。版權 © 2025 Apple Inc. 保留所有權利 ...",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSIZRPg0iWQmiRPY-Ro1XmdUQYbwGmIxUgMjjL8TqdyutHF&s",
      "source": "Apple",
      "date": "2018年6月8日",
      "position": 8
    },
    {
      "title": "Apple：一兆美元的背叛 | 關於您的 ...",
      "link": "https://www.youtube.com/watch?v=wmfuE-RM7Rg",
      "snippet": "Apple 是世界上最有價值的公司，對我們的生活有著無可比擬的影響。從智能手機到智能手錶， ...",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSvqw0lj-kWlq1HnowgEqzFxk5npWRpod2w1gogX-Dt4-jk&s",
      "video_url": "https://encrypted-vtbn0.gstatic.com/video?q=tbn:ANd9GcSG7N1JPdVZhrbuDckiwnYuWIQsXg_tdcmlvA",
      "duration": "52:40",
      "source": "YouTube",
      "channel": "Moconomy",
      "date": "2024年12月4日",
      "position": 9
    },
    {
      "title": "Apple 在工作 - 成功故事 - SAP",
      "link": "https://www.apple.com/business/enterprise/success-stories/tech/sap/",
      "snippet": "SAP 發現，為其員工提供這種選擇遠不止讓人們選擇他們的電腦。Mac 釋放了他們的全部潛力。概覽。105K+ ...",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSaplpBsRWWCJNlPtXsFX-jr3yw0mexpI6RtFKN6-TJAqrW&s",
      "source": "Apple",
      "date": "2022年4月13日",
      "position": 10
    }
  ]
}
```

返回結果一共有多個字段，介紹如下：

- `news`，搜索結果中視頻信息列表。

可以看到，這裡返回的結果中有一個 `videos` 字段，它主要包含了搜索關鍵詞的結果。

## 自定義搜索資源所在國家

該接口也支持限定搜索結果的所在國家，我們可以添加 `country` 參數來限定國家，輸入的參數為國家的簡寫，比如 cn（中國）、us（美國），本文將以中國做如下示例，下面為具體的信息：

<p><img src="https://cdn.acedata.cloud/gztpwi.png" width="500" class="m-auto"></p>

輸出效果如下：
```json
{
  "news": [
    {
      "title": "蘋果宣布 iOS 26 公測版將於七月推出",
      "link": "https://www.tv47.digital/apple-announces-ios-26-beta-public-roll-out-set-for-july-105190/",
      "snippet": "科技巨頭蘋果公司揭示了其 iPhone、Mac 和 iPad 的最新操作系統版本，介紹了一項重大...",
      "date": "2 小時前",
      "source": "TV47 Digital",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTBNrktH4AWMt8KS3hErOkI1oAeUvRm4Aaq10qnINCa3TmyabOCwiEMvxVWsA&s",
      "position": 1
    },
    {
      "title": "Bragar Eagel & Squire, P.C. 正在調查蘋果、Sable 和 Abacus，並鼓勵投資者聯繫該律所",
      "link": "https://www.tradingview.com/news/reuters.com,2025-06-17:newsml_GNX87dzHl:0-bragar-eagel-squire-p-c-is-investigating-apple-sable-and-abacus-and-encourages-investors-to-contact-the-firm/",
      "snippet": "紐約，2025年6月16日（GLOBE NEWSWIRE）— Bragar Eagel & Squire, P.C.，一家全國知名的股東權益律所，...",
      "date": "10 小時前",
      "source": "TradingView",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRdcHGK9sZ4M1wtCX6U74yV2ac9ClDX2hTSX4l4Brr1DWTvXfI4dKAn8sDU9g&s",
      "position": 2
    },
    {
      "title": "蘋果公司 (AAPL) 對 WWDC 軟體和 AI 公告的看法",
      "link": "https://www.nasdaq.com/articles/apple-inc-aapl-opinions-wwdc-software-and-ai-announcements",
      "snippet": "最近在 X 上關於蘋果公司 (AAPL) 的討論集中在該公司最新的軟體更新和在...",
      "date": "17 小時前",
      "source": "Nasdaq",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSm5mhn6LRgzafnn34CrjlR3LS1_fHPmKNJckasyNgN4eYgBiCqGtulLQIViA&s",
      "position": 3
    },
    {
      "title": "高盛建議購買蘋果，預期股價將上升",
      "link": "https://finance.yahoo.com/news/goldman-sachs-recommends-buying-apple-042754922.html",
      "snippet": "蘋果公司 (NASDAQ:AAPL) 是下一代股息貴族股票中最好的之一。高盛最近突出了幾隻股票...",
      "date": "6 小時前",
      "source": "Yahoo Finance",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS6QgEG6x6t12L8YqJebmoiPNM_RMjmiVUpwam1YXZgKwBUPGe6yn76hR-n0g&s",
      "position": 4
    },
    {
      "title": "蘋果在失去荷蘭應用商店上訴後面臨更多不合規罰款",
      "link": "https://globalcompetitionreview.com/article/apple-threatened-more-noncompliance-fines-after-losing-dutch-app-store-appeal",
      "snippet": "荷蘭消費者和市場管理局成功抵擋了蘋果對修改其應用商店規則的命令的上訴...",
      "date": "17 小時前",
      "source": "Global Competition Review",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ02b47E8UHdyFL6A4Da4Wla_nXXc427Y9aMgE7vVuJpN4CzLQp0ip_G77FwA&s",
      "position": 5
    },
    {
      "title": "荷蘭法院確認蘋果在約會應用中濫用主導地位",
      "link": "https://www.reuters.com/sustainability/boards-policy-regulation/dutch-court-confirms-apple-abused-dominant-position-dating-apps-2025-06-16/",
      "snippet": "荷蘭法院週一確認了2021年消費者監察機構的裁決，稱蘋果通過施加不公平的...",
      "date": "19 小時前",
      "source": "Reuters",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQLRbz3ypqJL6zJo1LBfMen68-D4csUJbGNNO7w8JODW6fiKliGk0yRyRGfvQ&s",
      "position": 6
    },
    {
      "title": "蘋果能否挽救中國的 AI iPhone？",
      "link": "https://www.bloomberg.com/opinion/articles/2025-06-10/can-apple-salvage-the-ai-iphone-in-china",
      "snippet": "如果你認為蘋果公司在美國的人工智慧困境已經很糟糕，那麼你要知道它們實際上可能更糟。看看中國。",
      "date": "6 天前",
      "source": "Bloomberg.com",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRzqIqxSek95GHKeeX9ekGIfzlZhz4jdEXqJPlk9Gyi5E8Yzk55iuhR5Soy_Q&s",
      "position": 7
    },
    {
      "title": "蘋果要求南韓允許將高精度地圖數據轉移到海外",
      "link": "https://en.yna.co.kr/view/AEN20250617006800320",
      "snippet": "首爾，6月17日（聯合新聞）-- 美國科技巨頭蘋果公司已要求南韓政府...",
      "date": "5 小時前",
      "source": "Yonhap News Agency",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS20gcfgCXs4W5Hzu78JWocLIX_9jQPbN1rUUPctbiJ1UKd7R7eotC4CodJdQ&s",
      "position": 8
    },
    {
      "title": "荷蘭法院維持對蘋果應用商店濫用的裁決",
      "link": "https://www.webpronews.com/dutch-court-upholds-ruling-against-apples-app-store-abuse/",
      "snippet": "對蘋果公司來說，這是一個重大打擊，荷蘭法院維持了荷蘭消費者和市場管理局在2021年的裁決，...",
      "date": "13 小時前",
      "source": "WebProNews",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSVZUsNtxTvIJYDjEGJN53uHWvK5NAGJrf-GS4wnzlSUtAbIYizew6HrYzr_w&s",
      "position": 9
    },
    {
      "title": "荷蘭法院確認蘋果在約會應用中濫用主導地位",
      "link": "https://seekingalpha.com/news/4458712-dutch-court-confirms-apple-abused-dominant-position-for-dating-apps",
      "snippet": "荷蘭法院確認該國競爭機構的裁決，稱蘋果 (AAPL) 濫用其主導地位，施加不合理的...",
      "date": "2 小時前",
      "source": "Seeking Alpha",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSkf9LcqSBxw2DQZaDhfzZyJ0G2VRkkJ6dVHgBJFIGv_fgtPTDo2iAfQ_5fcQ&s",
      "position": 10
    }
  ]
}
```
我們還可以自定義搜尋結果的語言，這裡我們額外添加 `language` 欄位，內容為 `zh-cn`，其中它指的是簡體中文的語言，同時還支持其他語言，但必須輸入的語言的簡寫，比如 en（English）、fr（French）、zh-cn（Chinese（Simplified））等語言，如圖所示：

<p><img src="https://cdn.acedata.cloud/yyrssp.png" width="500" class="m-auto"></p>

對應程式碼如下：

```shell
curl -X POST 'https://api.acedata.cloud/serp/google' \
-H 'accept: application/json' \
-H 'authorization: Bearer {token}' \
-H 'content-type: application/json' \
-d '{
  "type": "news",
  "query": "apple inc",
  "language": "zh-cn"
}'
```

運行結果如下：

```json
{
  "knowledge_graph": {
    "title": "蘋果",
    "type": "公司",
    "website": "http://www.apple.com/",
    "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRgbY1KzXEpuPeTpcw0GwN6BpQTcg1m06TDUsfdG6P-zW3eWrmu78AXyg&s=0",
    "attributes": {
      "創始人": "史蒂夫·喬布斯、史蒂夫·沃茲尼亞克和羅納德·韋恩",
      "創立於": "1976 年 4 月 1 日，加利福尼亞洛思阿圖斯",
      "總部": "加利福尼亞庫比蒂諾"
    }
  },
  "organic": [
    {
      "title": "Apple",
      "link": "https://www.apple.com/",
      "snippet": "Discover the innovative world of Apple and shop everything iPhone, iPad, Apple Watch, Mac, and Apple TV, plus explore accessories, entertainment, ...",
      "sitelinks": [
        {
          "title": "Support",
          "link": "https://support.apple.com/"
        },
        {
          "title": "Careers at Apple",
          "link": "https://www.apple.com/careers/us/"
        },
        {
          "title": "Store",
          "link": "https://www.apple.com/store"
        },
        {
          "title": "Business",
          "link": "https://www.apple.com/business/"
        },
        {
          "title": "Apple UK",
          "link": "https://www.apple.com/uk/"
        }
      ],
      "position": 1
    },
    {
      "title": "Apple Inc. - Wikipedia",
      "link": "https://en.wikipedia.org/wiki/Apple_Inc.",
      "snippet": "Apple Inc. is an American multinational corporation and technology company headquartered in Cupertino, California, in Silicon Valley.",
      "sitelinks": [
        {
          "title": "History",
          "link": "https://en.wikipedia.org/wiki/History_of_Apple_Inc."
        },
        {
          "title": "Litigation involving Apple Inc.",
          "link": "https://en.wikipedia.org/wiki/Litigation_involving_Apple_Inc."
        },
        {
          "title": "Apple Park",
          "link": "https://en.wikipedia.org/wiki/Apple_Park"
        },
        {
          "title": "Apple Watch",
          "link": "https://en.wikipedia.org/wiki/Apple_Watch"
        }
      ],
      "position": 2
    },
    {
      "title": "Apple Inc. | History, Products, Headquarters, & Facts - Britannica",
      "link": "https://www.britannica.com/money/Apple-Inc",
      "snippet": "Apple Inc. is an American multinational technology company that revolutionized the technology sector through its innovation of computer software, ...",
      "position": 3
    },
    {
      "title": "Apple Inc - Company Profile and News - Bloomberg Markets",
      "link": "https://www.bloomberg.com/profile/company/AAPL:US",
      "snippet": "Apple Inc. designs, manufactures, and markets smartphones, personal computers, tablets, wearables and accessories, and sells a variety of related ...",
      "position": 4
    },
    {
      "title": "Apple Inc. (AAPL) Stock Price, News, Quote & History - Yahoo Finance",
      "link": "https://finance.yahoo.com/quote/AAPL/",
      "snippet": "Apple Inc. designs, manufactures, and markets smartphones, personal computers, tablets, wearables, and accessories worldwide.",
      "date": "2024年8月27日",
      "position": 5
    },
    {
      "title": "AAPL: Apple Inc Stock Price Quote - NASDAQ GS - Bloomberg",
      "link": "https://www.bloomberg.com/quote/AAPL:US",
      "snippet": "Apple Inc. designs, manufactures, and markets smartphones, personal computers, tablets, wearables and accessories, and sells a variety of related accessories.",
      "position": 6
    },
    {
      "title": "Apple Inc. (AAPL) Company Profile & Facts - Yahoo Finance",
      "link": "https://finance.yahoo.com/quote/AAPL/profile/",
      "snippet": "See the company profile for Apple Inc. (AAPL) including business summary, industry/sector information, number of employees, business summary, ...",
      "position": 7
    }
  ],
  "people_also_ask": [
    {
      "question": "What is Apple Inc stand for?",
      "snippet": "It was incorporated as Apple Computer, Inc. in January 1977, and sales of its computers, including the Apple II , saw significant momentum and revenue growth for the company. \"Inc.\" is the abbreviation for incorporated. A corporation is a separate legal entity from the person or people forming it.",
      "title": "What does the 'Inc.' in Apple Inc. mean? - Quora",
      "link": "https://www.quora.com/What-does-the-Inc-in-Apple-Inc-mean"
    },
    {
      "question": "Who owns Apple Inc.?",
      "snippet": "Apple (AAPL) Ownership Overview The ownership structure of Apple (AAPL) stock is a mix of institutional, retail and individual investors. Approximately 48.76% of the company's stock is owned by Institutional Investors, 0.11% is owned by Insiders and 51.13% is owned by Public Companies and Individual Investors.",
      "title": "Who owns Apple? AAPL Stock Ownership - TipRanks.com",
      "link": "https://www.tipranks.com/stocks/aapl/ownership"
    },
    {
      "question": "Why did Apple change to Apple Inc?",
      "snippet": "During his keynote speech at the Macworld Expo on January 9, 2007, Jobs announced the renaming of Apple Computer, Inc. to Apple Inc., because the company had broadened its focus from computers to consumer electronics. This event also saw the announcement of the iPhone and the Apple TV.",
      "title": "Apple Inc. - Wikipedia",
      "link": "https://en.wikipedia.org/wiki/Apple_Inc."
    },
    {
      "question": "Why Apple Inc is called Apple?",
      "snippet": "With the name Apple, the new computer company would appear ahead of Atari, where Jobs used to work. Jobs confirmed this theory in an 1980 presentation, stating that the name was partly chosen because he liked apples and partly because Apple was ahead of Atari in the phone book.",
      "title": "Why Is Apple Called Apple? - Apple Scoop",
      "link": "https://applescoop.org/story/why-is-apple-called-apple"
    }
  ],
  "related_searches": [
    {
      "query": "apple inc是什麼"
    },
    {
      "query": "apple澳門"
    },
    {
      "query": "Apple ID"
    },
    {
      "query": "apple美國"
    },
    {
      "query": "apple官網"
    },
    {
      "query": "apple美國官網"
    },
    {
      "query": "Apple company introduction"
    },
    {
      "query": "apple id官網"
    }
  ],
  "credits": 1
}
```

可以看到這裡顯示的結果都是簡體中文的語言，結果的內容與上文類似。

## 自定義搜尋結果時間範圍
本文還可以自定義搜尋結果的時間範圍，包括了五種，分別是 `qdr:h`（過去一個小時）、`qdr:d`（過去一天）、`qdr:w`（過去一週）、`qdr:m`（過去一個月）、預設無限制，我們可以透過 `range` 傳遞對應的時間範圍即可，比如這裡設置為 `qdr:d`，這樣表示搜尋過去一天的結果，所以輸入如下：

<p><img src="https://cdn.acedata.cloud/qccfib.png" width="500" class="m-auto"></p>

對應的程式碼如下：

```shell
curl -X POST 'https://api.acedata.cloud/serp/google' \
-H 'accept: application/json' \
-H 'authorization: Bearer {token}' \
-H 'content-type: application/json' \
-d '{
  "range": "qdr:d",
  "query": "apple inc"
}'
```

運行結果如下：

```json
{
  "answer_box": {
    "title": "Tim Cook (Aug 24, 2011–)Apple / CEO",
    "answer": "Tim Cook (Aug 24, 2011–)"
  },
  "knowledge_graph": {
    "title": "Apple",
    "type": "Technology company",
    "website": "http://www.apple.com/",
    "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT5ITHsQzdzkkFWKinRe1Y4FUbC_Vy3R_M&s=0",
    "description": "Apple Inc. 是一家美國跨國公司和科技公司，總部位於加利福尼亞州庫比蒂諾，位於矽谷。它以消費電子產品、軟體和服務而聞名。",
    "description_source": "維基百科",
    "description_link": "https://en.wikipedia.org/wiki/Apple_Inc.",
    "attributes": {
      "Customer service": "1 (800) 275-2273",
      "Founders": "史蒂夫·喬布斯、史蒂夫·沃茲尼亞克和羅納德·韋恩",
      "Founded": "1976年4月1日，加州洛斯阿圖斯",
      "Headquarters": "加州庫比蒂諾",
      "CEO": "Tim Cook (Aug 24, 2011–)"
    }
  },
  "organic": [
    {
      "title": "Apple",
      "link": "https://www.apple.com/",
      "snippet": "探索Apple的創新世界，購買所有iPhone、iPad、Apple Watch、Mac和Apple TV，還可以探索配件、娛樂等...",
      "sitelinks": [
        {
          "title": "支援",
          "link": "https://support.apple.com/"
        },
        {
          "title": "職業機會",
          "link": "https://www.apple.com/careers/us/"
        },
        {
          "title": "商業",
          "link": "https://www.apple.com/business/"
        },
        {
          "title": "Apple領導團隊",
          "link": "https://www.apple.com/leadership/"
        },
        {
          "title": "商店",
          "link": "https://www.apple.com/store"
        }
      ],
      "position": 1
    },
    {
      "title": "Apple Inc. - 維基百科",
      "link": "https://en.wikipedia.org/wiki/Apple_Inc.",
      "snippet": "Apple Inc. 是一家美國跨國公司和科技公司，總部位於加利福尼亞州庫比蒂諾，位於矽谷。",
      "position": 2
    },
    {
      "title": "Apple Inc. | 歷史、產品、總部及事實 - 大英百科全書",
      "link": "https://www.britannica.com/money/Apple-Inc",
      "snippet": "美國個人電腦、智能手機和平板電腦的製造商。Apple是第一家成功的個人電腦公司...",
      "date": "7天前",
      "position": 3
    },
    {
      "title": "Apple Inc. (AAPL) 公司簡介及事實 - Yahoo 財經",
      "link": "https://finance.yahoo.com/quote/AAPL/profile/",
      "snippet": "Apple Inc. 設計、製造和銷售智能手機、個人電腦、平板電腦、可穿戴設備和配件，並在全球範圍內銷售各種相關配件。",
      "position": 4
    },
    {
      "title": "AAPL: Apple Inc 股票價格報價 - NASDAQ GS - 彭博社",
      "link": "https://www.bloomberg.com/quote/AAPL:US",
      "snippet": "Apple Inc. 設計、製造和銷售智能手機、個人電腦、平板電腦、可穿戴設備和配件，並銷售各種相關配件。",
      "position": 5
    },
    {
      "title": "Apple Inc. (AAPL) 今日股票價格 - WSJ",
      "link": "https://www.wsj.com/market-data/quotes/AAPL?gaa_at=eafs&gaa_n=ASWzDAjHf1Jvm_DfBkgGitLxGwlDC2VFfkyeU3ora5VpP1k1gJJfptWjtU4o&gaa_ts=6851a5a8&gaa_sig=FmQcgCrdYtgMPhMwfFTpCtEva8_OZ5j1fx1cEBn0_THBq1EUa8A0Tmo3piNM3OQ488O2rScdH4NuSLly_DIdpQ%3D%3D",
      "snippet": "Apple Inc. 從事智能手機、個人電腦、平板電腦、可穿戴設備和配件的設計、製造和銷售，以及其他各種相關產品的銷售。",
      "position": 6
    }
  ],
  "people_also_ask": [
    {
      "question": "Apple Inc是什麼？",
      "snippet": "Apple Inc.，最初為Apple Computer, Inc.，是一家創造和銷售消費電子產品及相關電腦軟體的跨國公司，並且是數位媒體內容的數位分銷商。Apple的核心產品線包括iPhone智能手機、iPad平板電腦和Mac個人電腦。",
      "title": "Apple Inc.的歷史 - 維基百科",
      "link": "https://en.wikipedia.org/wiki/History_of_Apple_Inc."
    },
    {
      "question": "誰擁有Apple Inc？",
      "snippet": "Apple (AAPL) 擁有概覽 約50.57%的公司股票由機構投資者擁有，0.06%由內部人擁有，49.38%由公共公司和個人投資者擁有。Apple (AAPL) 股票的擁有結構是機構、零售和個人投資者的混合。",
      "title": "誰擁有Apple？AAPL股票擁有權 - TipRanks.com",
      "link": "https://www.tipranks.com/stocks/aapl/ownership"
    },
    {
      "question": "Apple Inc. 最著名的是什麼？",
      "snippet": "它以消費電子產品、軟體和服務而聞名。成立於1976年，由史蒂夫·喬布斯、史蒂夫·沃茲尼亞克和羅納德·韋恩創立，該公司在次年由喬布斯和沃茲尼亞克註冊為Apple Computer, Inc。",
      "title": "Apple Inc. - 維基百科",
      "link": "https://en.wikipedia.org/wiki/Apple_Inc."
    },
    {
      "question": "誰是Apple的現任擁有者？",
      "snippet": "Tim Cook (Aug 24, 2011–)\nApple / CEO"
    }
  ],
  "related_searches": [
    {
      "query": "Apple iPhone 13"
    },
    {
      "query": "apple inc 是什麼"
    },
    {
      "query": "Apple Inc 全名"
    },
    {
      "query": "Apple Inc 地址"
    },
    {
      "query": "Apple Inc 投資者關係"
    },
    {
      "query": "Apple Inc 行業"
    },
    {
      "query": "Apple Inc 年報"
    },
    {
      "query": "Apple Inc 職業"
    }
  ]
}
```

可以看到，我們就成功得到了過去一天的搜尋結果，結果內容與上文類似。

## 自定義搜尋結果分頁

本 API 還支持自定義搜尋結果的分頁展示，`number` 和 `page` 表示分頁的頁面大小和頁數，本文將設置一頁裡面有 20 個搜尋結果的形式來進行分頁展示，具體的信息如圖所示：

<p><img src="https://cdn.acedata.cloud/dqla1e.png" width="500" class="m-auto"></p>

> 注意：當每頁結果超過 10 個時，扣除的積分會翻倍。

程式碼如下：
```shell
curl -X POST 'https://api.acedata.cloud/serp/google' \
-H 'accept: application/json' \
-H 'authorization: Bearer {token}' \
-H 'content-type: application/json' \
-d '{
  "query": "apple inc",
  "number": 20,
  "page": 1
}'
```

運行結果如下：

```json
{
  "knowledge_graph": {
    "title": "Apple",
    "type": "科技公司",
    "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSHmAG45NsvdRfYlNghHFvinM0w2mqjd8A0lXKU2LA&usqp=CAE&s",
    "attributes": {
      "客戶服務": "1 (800) 275-2273",
      "創辦人": "史蒂夫·喬布斯, 史蒂夫·沃茲尼亞克, 羅納德·韋恩",
      "成立": "1976年4月1日，加州洛斯阿圖斯",
      "股價": "AAPL (NASDAQ) $198.42 +1.97 (+1.00%) 6月16日，下午4:00 EDT - 免責聲明",
      "總部": "加州庫比蒂諾",
      "執行長": "蒂姆·庫克 (2011年8月24日–)"
    }
  },
  "organic": [
    {
      "title": "Apple",
      "link": "https://www.apple.com/",
      "snippet": "探索Apple的創新世界，購買所有iPhone、iPad、Apple Watch、Mac和Apple TV，還可以探索配件、娛樂等...",
      "sitelinks": [
        {
          "title": "支援",
          "link": "https://support.apple.com/",
          "snippet": "iPhone支援 - 聯絡 - 計費和訂閱 - Apple維修"
        }, {
          "title": "職業機會",
          "link": "https://www.apple.com/careers/us/",
          "snippet": "在Apple工作 - 軟體和服務 - 支援和服務"
        }, {
          "title": "商業",
          "link": "https://www.apple.com/business/",
          "snippet": "小型企業 - 企業 - 商業購物 - Apple在工作中 - ..."
        }, {
          "title": "商店",
          "link": "https://www.apple.com/store",
          "snippet": "購買iPhone - 購買Mac - 購買iPad - 應用商店 - ..."
        },
        {
          "title": "投資者關係",
          "link": "https://investor.apple.com/investor-relations/default.aspx",
          "snippet": "SEC檔案 - 股價 - 領導和治理"
        }
      ],
      "position": 1
    }, {
      "title": "Apple Inc.",
      "link": "https://en.wikipedia.org/wiki/Apple_Inc.",
      "snippet": "Apple Inc.是一家美國跨國公司和科技公司，總部位於加州庫比蒂諾，位於矽谷。",
      "position": 2
    }, {
      "title": "Apple Inc. | 歷史、產品、總部及事實",
      "link": "https://www.britannica.com/money/Apple-Inc",
      "snippet": "Apple Inc.是一家美國跨國科技公司，通過其計算機軟體、個人電腦等的創新，徹底改變了科技行業。",
      "date": "7天前",
      "position": 3
    }, {
      "title": "Apple Inc. (AAPL) 公司簡介及事實",
      "link": "https://finance.yahoo.com/quote/AAPL/profile/",
      "snippet": "Apple Inc.設計、製造和銷售智能手機、個人電腦、平板電腦、可穿戴設備和配件，遍及全球。",
      "position": 4
    }, {
      "title": "AAPL: Apple Inc 股價報價 - NASDAQ GS",
      "link": "https://www.bloomberg.com/quote/AAPL:US",
      "snippet": "Apple Inc.設計、製造和銷售智能手機、個人電腦、平板電腦、可穿戴設備和配件，並銷售各種相關配件。",
      "position": 5
    }, {
      "title": "Apple Inc. (AAPL) 今日股價",
      "link": "https://www.wsj.com/market-data/quotes/AAPL?gaa_at=eafs&gaa_n=ASWzDAgqDxGuyZUvNqcmVhmXBoGHd3y-StLOZpo82befc3s9jFqSwwJzjPmn&gaa_ts=68514b6e&gaa_sig=a-sNk_J7mCIayZx21oIaY6Ge6vifnmxy7A4psXod_wP6ng9689uAwZYvUOUQPv_dxcArYe_RQbKxCkwngcK_bQ%3D%3D",
      "snippet": "Apple Inc.從事智能手機、個人電腦、平板電腦、可穿戴設備和配件的設計、製造和銷售，以及其他各種相關產品。",
      "position": 6
    }, {
      "title": "Apple",
      "link": "https://www.linkedin.com/company/apple",
      "snippet": "Apple的外部連結。行業：計算機和電子製造。公司規模：10,001名以上員工。總部：加州庫比蒂諾。類型：上市公司...",
      "position": 7
    }, {
      "title": "Apple | AAPL股價、公司概覽及新聞",
      "link": "https://www.forbes.com/companies/apple/",
      "snippet": "Apple Inc.從事智能手機、個人電腦、平板電腦、可穿戴設備和配件的設計、製造和銷售，以及其他各種相關產品。",
      "position": 8
    }, {
      "title": "Apple Inc. (AAPL) 股價、新聞、報價及歷史",
      "link": "https://finance.yahoo.com/quote/AAPL/",
      "snippet": "查找最新的Apple Inc. (AAPL) 股價報價、歷史、新聞及其他重要信息，以幫助您進行股票交易和投資。",
      "position": 9
    }, {
      "title": "iCloud",
      "link": "https://www.icloud.com/",
      "snippet": "在iCloud.com上，您可以從任何網頁瀏覽器訪問您的照片、文件等。您所做的更改將同步到您的iPhone和其他設備。",
      "position": 10
    }, {
      "title": "Apple Inc (AAPL) 股價及新聞 - Google Finance",
      "link": "https://www.google.com/finance/quote/AAPL:NASDAQ?hl=en",
      "snippet": "Apple Inc.是一家美國跨國公司和科技公司，總部位於加州庫比蒂諾，位於矽谷。",
      "position": 11
    }, {
      "title": "Apple Inc 公司簡介 - GlobalData",
      "link": "https://www.globaldata.com/company-profile/apple-inc/",
      "snippet": "Apple Inc (Apple)設計、製造和銷售智能手機、平板電腦、個人電腦和可穿戴設備。該公司提供軟體應用程序...",
      "position": 12
    }, {
      "title": "Apple Computer, Inc.的創立 - 本月商業歷史...",
      "link": "https://guides.loc.gov/this-month-in-business-history/april/apple-computer-founded",
      "snippet": "Apple Computer, Inc.於1976年4月1日由大學輟學生史蒂夫·喬布斯和史蒂夫·沃茲尼亞克創立，他們為新公司帶來了改變世界的願景...",
      "position": 13
    }, {
      "title": "Apple Inc",
      "link": "https://vimeo.com/appleinc",
      "snippet": "Apple Inc是Vimeo的成員，這裡是高品質視頻和熱愛它們的人們的家。",
      "position": 14
    }
  ],
  "people_also_ask": [
    {
      "question": "Apple Inc是什麼？"
    }, {
      "question": "誰擁有Apple Inc？"
    }, {
      "question": "Apple Inc.最著名的是什麼？",
      "title": "Apple Inc. - 維基百科",
      "snippet": "它最著名的是其消費電子產品、軟體和服務。成立於1976年，作為Apple Computer Company，由史蒂夫·喬布斯、史蒂夫·沃茲尼亞克和羅納德·韋恩創立，該公司在次年由喬布斯和沃茲尼亞克註冊為Apple Computer, Inc。",
      "link": "https://en.wikipedia.org/wiki/Apple_Inc.#:~:text=It%20is%20best%20known%20for,%2C%20Inc.%20the%20following%20year."
    },
```
{
      "question": "Apple Inc 中的 Inc 是什麼意思？",
      "title": "Inc. | 英文意思 - 劍橋詞典",
      "snippet": "incorporated 的縮寫：用於合法成立的美國公司名稱中：Bishop Computer Services, Inc.",
      "link": "https://dictionary.cambridge.org/dictionary/english/inc#:~:text=abbreviation%20for%20incorporated%3A%20used%20in,Bishop%20Computer%20Services%2C%20Inc."
    } ],
  "related_searches": [
    {
      "query": "apple inc 是什麼"
    }, {
      "query": "Apple Inc 全名"
    }, {
      "query": "Apple Inc 地址"
    }, {
      "query": "Apple Inc 投資者關係"
    }, {
      "query": "Apple Inc 行業"
    },
    {
      "query": "Apple Inc 年度報告"
    },
    {
      "query": "Apple Inc 職業"
    },
    {
      "query": "Apple 17 pro"
    }
  ]
}
```

可以看到，這裡它對搜索結果進行了分頁展示，一頁裡面展示了 20 條結果，結果的內容與上文類似。
