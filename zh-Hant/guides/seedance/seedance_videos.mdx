...
普通全模態生成可省略 `omni_reference_task_type`，設為 `auto`，或顯式設為 `reference`。視頻編輯與延長必須傳入 `reference_video`：

```json
{
  "model": "doubao-seedance-2-5-260628",
  "content": [
    {"type": "text", "text": "在保留主體和相機運動的同時，用溫暖的日落替換天空。"},
...
- `reference`：至少傳入一個 `reference_image`、`reference_video` 或 `reference_audio`；2.5 支持僅傳參考音頻。
...
payload = {
    "content": [
        ...
        {
            "type": "text",
            "text": "一個女孩抱著一隻狐狸。她睜開眼睛，溫柔地凝視著鏡頭，而狐狸也親昵地回抱著她。當相機緩慢拉遠時，她的頭髮被風吹起。 --ratio adaptive  --dur 5"
        }
    ],
