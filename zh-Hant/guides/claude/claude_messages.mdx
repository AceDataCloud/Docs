...
- `model`：選擇使用的 Claude 模型。最新旗艦為 `claude-fable-5-1`（100 萬 Token 上下文、最大輸出 128K Token）；原 `claude-fable-5` 仍兼容保留。
...
- `usage`：token 使用統計。`input_tokens` 是未緩存輸入；`cache_creation_input_tokens` 和 `cache_read_input_tokens` 分別是緩存寫入與讀取；`output_tokens` 是輸出 token 數。Fable 5.1 的官方緩存讀取基價為 $0.25/百萬 Token，5 分鐘與 1 小時緩存寫入基價分別為 $12.50 和 $20/百萬 Token；平台實際價格按套餐折扣換算。非流式響應還可能包含 Ace Data Cloud 記錄的 `cost`。
...
- Fable 5.1、Fable 5、Opus 5、Sonnet 5、Opus 4.8 和 Opus 4.7 的 display 默認值為 `omitted`；Opus 4.6、Sonnet 4.6 及更早支持 thinking 的模型默認使用 `summarized`。
...
- `budget_tokens` 僅用於仍支持固定思考預算的舊模型。新模型應使用 `thinking.type=adaptive` 和 `output_config.effort`；Fable 5.1 的 thinking 始終開啟，不能顯式關閉。
...
頂層 `cache_control` 會自動把緩存斷點放在最後一個可緩存塊上：
...
需要精確控制位置時，也可把同樣的 `cache_control` 寫在 text、image、document、tool_use、tool_result 內容塊或工具定義上。`ttl` 支持 `5m`（默認）與 `1h`；請通過 `usage.cache_creation_input_tokens` 和 `usage.cache_read_input_tokens` 判斷緩存寫入與命中。
...
```json
{
  ...
  "content": [
    {
      "type": "text",
      "text": "這張圖片顯示了一個 API 請求配置介面，似乎是針對一個 AI 聊天完成服務。該介面包括模型選擇、消息、流模式和最大令牌設置的參數。"
    }
  ],
  "model": "claude-sonnet-4-20250514",
  "stop_reason": "end_turn",
...
| 回應結構     | `content` 陣列（支持多種類型）             | `choices` 陣列（包含 `message`）                   |
...
公開接口的錯誤回應使用 Ace Data Cloud 平台 envelope：`error.code` 是穩定錯誤碼，`error.message` 是說明，`trace_id` 用於排查請求。常見 HTTP 狀態包括：
...
通過本文檔，您已經了解了如何使用 Claude Messages API 以 Anthropic 原生格式調用 Claude 的對話功能。Messages API 支持基本對話、系統提示詞、流式回應、多輪對話、深度思考、視覺理解、PDF、提示快取和工具調用等豐富功能。如有任何問題，請隨時聯繫我們的技術支持團隊。
