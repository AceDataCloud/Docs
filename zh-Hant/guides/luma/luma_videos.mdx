...
Luma 是一個專業高品質的視頻生成平台，用戶只需上傳素材，即可根據不同風格和效果自動生成高品質視頻。該 AI 視頻生成器由來自知名科技公司的團隊成員開發，目標是無需複雜的編輯工具，讓每個人都能輕鬆製作出色的視頻。
...
可以看到這時我們就得到了這個視頻的相關信息，包括視頻ID、視頻鏈接、視頻封面等內容。
...
這時視頻首幀 `start_image_url` 字段可以傳入以下圖片作為視頻的首幀：
...
最後得到的結果與上文的類似，生成的視頻首幀包含了我們傳入的圖片，當然也可以同時傳入首尾幀圖片鏈接來生成視頻，只需要在上面的基礎上再加一個尾幀圖片即可，尾幀的圖片信息如下：
