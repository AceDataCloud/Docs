...
- **原生多比例 + 高分辨率支持**：覆蓋 5 種比例（1:1、4:3、3:4、16:9、9:16）共 3 檔分辨率（1K / 2K / 4K）。

調用方式與其它模型完全一致，只需將 `model` 欄位設置為 `gpt-image-2` 即可。返回結果中的 `url` 是一個永久托管在 `platform.cdn.acedata.cloud` 上的圖片鏈接，可以直接在瀏覽器中打開或嵌入到網頁中。
...
> 傳 `size: "auto"`（默認）時，模型會**讀取提示詞本身來決定畫幅**：寫「超寬電影感橫幅 21:9」會得到 `3024x1296`，寫「豎版海報 9:16」會得到 `1440x2560`，沒有畫幅暗示的提示詞則回到 `2048x2048`。所以當你心裡有構圖但不想算具體像素時，`auto` 通常比硬填一個尺寸更合適。省略 `size` 欄位的效果與 `auto` 相同。
> 1K 檔下輸出不保證嚴格像素對齊——你傳 `1024x1024` 可能拿到 `1254x1254`，比例保持一致。如果你重新把它當作 `size` 傳進來，計費不變。
...
提示詞中可以使用電影術語（35mm 膠卷、淺景深、霓虹光等）來精準控制氛圍與質感。
...
payload = {
    "model": "gpt-image-2",
    "prompt": "一幅年轻女性在便利店夜间站立的电影肖像，透过窗户被柔和的粉色和青色霓虹灯照亮。使用35mm胶卷拍摄，浅景深，轻微颗粒感，忧郁的氛围。",
    "size": "1024x1536"
}
...
```json
{
  ...
  "data": [
    {
      "revised_prompt": "一幅年轻女性在便利店夜间站立的电影肖像，透过窗户被柔和的粉色和青色霓虹灯照亮。使用35mm胶卷拍摄，浅景深，轻微颗粒感，忧郁的氛围。",
...
payload = {
    "model": "gpt-image-2",
    "prompt": "一幅意大利阿马尔菲海岸的复古旅行海报。风格化的艺术装饰插图，悬崖边的柠檬黄色房屋倾斜而下，直达碧绿色的海洋，港口有一只小白帆船。顶部的粗体字母写着 AMALFI，底部写着 ITALIA 1958。有限的色彩调色板：奶油色、海蓝色、柠檬黄、红土色。轻微的纸张颗粒质感。",
    "size": "1024x1536"
}
```
...
payload = {
    "model": "gpt-image-2",
    "prompt": "一个由三层架子组成的木书架：在顶层，应该有一本书。在第二层，应该有三本书。在底层，应该有七本书。柔和的暖光，照片真实感，舒适的图书馆氛围。",
    "size": "1024x1024"
}
```
...
payload = {
    "model": "gpt-image-2",
    "prompt": "一幅柔和、诗意的儿童书插图，描绘一只小狐狸在月光森林中，在发光的蘑菇下阅读一本书。水彩和铅笔质感，柔和的粉彩色，梦幻的氛围，手绘的感觉。",
    "size": "1536x1024"
}
```
...
```json
{
  ...
  "data": [
    {
      "revised_prompt": "一幅愉快的圖像展示了一隻年輕的海獺，它出生時是棕色的，擁有寬大的迷人眼睛。它愉快地躺在背上，在平靜的海水中划水。它濃密、柔軟的毛皮看起來濕潤而閃閃發光，捕捉了它棲息地的本質。這隻小生物好奇地用小爪子玩著一個海貝，看起來在它的自然環境中絕對無辜而迷人。",
...
```json
{
  ...
  "data": [
    {
      "revised_prompt": "一隻可愛的海獺寶寶在水中愉快地躺著，毛皮看起來光滑而柔軟。它的一隻小爪子好奇地伸出，臉上帶著純真的喜悅和溫暖的表情，仰望著天空。它的身體周圍環繞著因為在水中嬉戲而產生的氣泡。微風輕拂著它的毛皮，使它看起來更加迷人。這個場景描繪了海洋生物的寧靜和魅力。",
