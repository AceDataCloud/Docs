...
> **關於 `n` 參數**
> `gpt-image-2` 編輯接口支持 `n > 1`：一次請求即可返回並按張計費對應數量的編輯結果（`n` 取值 1–10）。同樣適用於 `gpt-image-1` / `gpt-image-1.5`，以及 `nano-banana` / `nano-banana-2-lite` / `nano-banana-2` / `nano-banana-pro` 系列；`dall-e-2` 也原生支持。注意 `response_format=b64_json` 僅支持 `n=1`，`n>1` 時請使用默認的 URL 返回。若其中部分圖片生成失敗，只會返回並計費成功的部分。
...
這樣我們就完成了對圖片的編輯操作，目前 Edits 接口共支持三種模型：`dall-e-2`、`gpt-image-1` 和 `gpt-image-2`，其中 `gpt-image-2` 是當前推薦使用的模型，詳見上文 [GPT-Image-2 模型](#gpt-image-2-模型) 一節。
