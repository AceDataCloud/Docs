...
編輯介面對 `size` 的約束與生成介面完全一致——`gpt-image-2` 只需要 `size` 為 `auto`、空，或者符合 `WIDTHxHEIGHT` 格式，任何其他形態會返回 400。**所有尺寸（1K / 2K / 4K / 自訂）按單張統一扣費，與原圖解析度和 `size` 請求值都無關。**

上游對自訂尺寸的硬約束同樣適用：寬高均為 16 的倍數、長邊 ≤ 3840、總像素數 ≤ 8,294,400。
...
使用 SDK 時需要先匯入兩個環境變數，`OPENAI_BASE_URL` 設為 `https://api.acedata.cloud/openai`，`OPENAI_API_KEY` 設為申請到的 token：
...
使用 Python 呼叫時我們需要先匯入兩個環境變數，一個 `OPENAI_BASE_URL`，可以設定為 `https://api.acedata.cloud/openai`，還有一個使用憑證變數 `OPENAI_API_KEY`，這個值是從 `authorization` 獲取到的，在 Mac OS 可以透過以下指令設定環境變數：
...
- `429 too_many_requests`：請求過多，超過速率限制。
