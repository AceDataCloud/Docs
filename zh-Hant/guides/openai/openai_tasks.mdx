...
- `type`：上游接口類型。`gpt-image` 系列（如 `gpt-image-2`）寫入的任務為 `images`；舊版 OpenAI 通道（如 `gpt-image-1`、nano-banana）則使用 `images_generations` / `images_edits`，部分聊天接口為 `chat_completions_image`。
- `request`：原始請求的完整請求體。
- `response`：回調完成時上游接口返回的最終回應體。
- `created_at` / `finished_at` / `duration`：Unix 時間戳（秒）與執行耗時（秒）。
...
| `type`           | string   | 按上游類型過濾（取值：`images`、`images_generations`、`images_edits`） |
...
Tasks API 主要服務於回調模式下的非同步流程。回調模式下，提交介面會**立即同步返回一個 `task_id`**（即任務 ID），之後您只需直接拿這個 `task_id` 去 Tasks 介面輪詢即可，無需自己再生成 `trace_id`。
...
# 2. 直接用提交響應中的 task_id 輪詢 Tasks 介面，直到任務完成
while True:
    task = requests.post(
        f"{API}/openai/tasks",
        headers=HEADERS,
...
- Tasks 介面本身**不計費**，可以放心輪詢。僅原始的圖片生成/編輯請求會扣費。
