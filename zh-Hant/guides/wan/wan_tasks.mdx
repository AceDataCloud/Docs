...
Wan Tasks API 可以用於查詢 Wan Videos Generation API 的結果。關於怎樣使用 Wan Videos Generation API，請參考文檔 [Wan Videos Generation API ](https://platform.acedata.cloud/documents/wan-videos)。

我們以 Wan Videos Generation API 服務返回的任務ID一個為例，演示如何使用該 API。假設我們有一個任務ID：a4bca552-d964-46a1-8ff7-fd922f916582，接下來演示如何通過傳入一個任務ID來。
...
這是是針對多個任務ID來進行查詢視頻任務詳細信息，與上面不同的是需要將action選中為retrieve_batch
...
返回結果一共有多個欄位，其中 items 是包含了批量影片任務的具體詳細資訊，每個影片任務的具體資訊與上文的欄位相同，欄位資訊如下。

- `items`，批量影片任務的所有具體詳細資訊。它是一個陣列，每個陣列的元素與上文查詢單個任務的返回結果格式相同。
- `count`，此處批量查詢影片任務的數量。
...
在呼叫 API 時，如果遇到錯誤，API 會返回相應的錯誤代碼和訊息。例如：

- `400 token_mismatched`：錯誤的請求，可能是缺少或無效的參數。
- `400 api_not_implemented`：錯誤的請求，可能是缺少或無效的參數。
- `401 invalid_token`：未授權，授權令牌無效或缺失。
- `429 too_many_requests`：請求過多，您已超過速率限制。
- `500 api_error`：伺服器內部錯誤，伺服器端發生問題。
...
透過本文檔，您已經了解如何使用 Wan Tasks API 查詢單個或批量影片任務的所有具體詳細資訊。希望本文檔能幫助您更好地對接和使用該 API。如有任何問題，請隨時聯繫我們的技術支援團隊。
