...
普通上傳只需要 `audio_url`，它是一個可以公開訪問的音頻 CDN 地址。`mode` 默認為 `standard`，因此現有調用無需修改。普通上傳成功消耗 **0.06 Credits**；上傳失敗不扣費。
...
有了歌曲 ID 之後，我們便可以使用 [Suno Audios Generation API](https://platform.acedata.cloud/documents/suno-audios) 來進行自定義歌曲生成了，比如說 `action` 傳入 `upload_extend`，同時 `audio_id` 傳入返回的歌曲 ID，就可以根據參考音頻生成新的歌曲了。

## 增強上傳模式

> `audio_url` 是輸入地址，會直接交給音頻處理流程，不會由終態媒體持久化服務預先轉存。請確保它在任務處理期間可公開訪問。

當普通上傳無法處理您擁有或已獲授權使用的音頻時，可以設置 `mode` 為 `enhanced`。該模式還必須提供 1 至 100 個字符的 `name`，並使用公開可訪問的 HTTPS `audio_url`。

增強上傳固定採用異步處理，通常需要 2 分鐘或更長時間。成功處理消耗 **1.87 Credits**；處理失敗不扣費。
...
接口會立即返回 `task_id` 和 `trace_id`。使用 [Suno Tasks API](https://platform.acedata.cloud/documents/suno-tasks) 查詢任務，成功後從 `response.data.audio_id` 讀取上傳後的音頻 ID。也可以提供 HTTPS `callback_url` 接收終態。

增強上傳得到的 `audio_id` 可用於 Cover、Samples 和 Mashup。伴奏與 MIDI 提取、全軌分離以及人聲/伴奏分離仍可能受音頻所屬帳號限制；未列出的操作不保證支持跨帳號使用。音頻越短，處理成功率通常越高。
