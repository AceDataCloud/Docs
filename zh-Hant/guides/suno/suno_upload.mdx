...
普通上傳只需要 `audio_url`，它是一個可以公開訪問的音頻 CDN 地址。`mode` 默認為 `standard`，因此現有調用無需修改。普通上傳成功消耗 **0.06 Credits**，按最大套餐折算約 **¥0.041/次**；上傳失敗不扣費。
...
增強上傳固定採用異步處理，通常需要 2 分鐘或更長時間。成功處理消耗 **1.87 Credits**，按最大套餐折算約 **¥1.26/次**；處理失敗不扣費。
