---
title: "Suno 歌曲生成 API 對接說明"
description: "Suno Music Generation 整合指南 - Ace Data Cloud"
---

隨著 AI 的應用變廣，各類 AI 程式已逐漸普及。AI 已逐漸深入到人們的工作生活方方面面。而 AI 涉及的行業也越來越多，從最初的寫作，到醫療教育，再到現在的音樂。

Suno 是一個專業高品質的 AI 歌曲和音樂創作平台，用戶只需輸入簡單的文本提示詞，即可根據流派風格和歌詞生成帶有人聲的歌曲。該 AI 音樂生成器由來自 Meta、TikTok、Kensho 等知名科技公司的團隊成員開發，目標是不需要任何樂器工具，讓所有人都可以創造美妙的音樂。

以下是模型更新的進度：

| 版本    | model           | 上線時間       | lyric 限制 | style 限制 | 歌曲最長時長 |
| ----- | --------------- | ---------- | -------- | -------- | ------ |
| v5.5  | chirp-v5-5      | 2026.03.27 | 5000     | 1000     | 8 分鐘   |
| v5    | chirp-v5        | 2025.09.23 | 5000     | 1000     | 8 分鐘   |
| v4.5+ | chirp-v4-5-plus | 2025.07.17 | 5000     | 1000     | 8 分鐘   |
| v4.5  | chirp-v4-5      | 2025.05.03 | 5000     | 1000     | 4 分鐘   |
| v4    | chirp-v4        | 2024.12.17 | 3000     | 200      | 150 秒  |
| v3.5  | chirp-v3-5      | ---        | 3000     | 200      | 120 秒  |

> 上表的 `lyric` 與 `style` 限制為自定義模式（`custom` 為 `true`）下的上限。非自定義的靈感模式（`custom` 為 `false`）只填 `prompt`，其長度上限為 500 字符（各模型一致）。

Suno 現已支持最新的 `chirp-v5-5` 模型。調用最新版本時，將 `model` 參數設置為 `chirp-v5-5` 即可；`chirp-v5` 及更早版本仍可繼續使用。

然而 Suno 官方是並沒有提供 API 的，AceDataCloud 提供了一套 Suno 的 API，模擬對接了 Suno 官方，可以方便快捷地生成想要的音樂。

## 申請和使用

要使用 Suno Audios Generation API，首先到 [Ace Data Cloud 控制台](https://platform.acedata.cloud/console/applications) 獲取您的 API Token，留作備用。

![](https://cdn.acedata.cloud/5hmkdg.jpg)

如果你尚未登錄或註冊，會自動跳轉到登錄頁面邀請你註冊和登錄，完成後會自動返回當前頁面。

**一個 API Token 即可調用平台所有服務，無需為每個服務單獨申請。** 首次申請會贈送免費額度，可免費體驗；額度不足時可在 [控制台](https://platform.acedata.cloud/console/coin) 充值通用餘額。

> 📘 完整文檔：[Suno Audios Generation API →](https://platform.acedata.cloud/documents/suno-audios)

## 基本使用

想些什麼歌曲，可以任意輸入一段文字，比如我想生成一個關於聖誕的歌曲，就可以輸入 `a song for Christmas`，如圖所示：

<p><img src="https://cdn.acedata.cloud/2kuuup.png" width="500" className="m-auto" /></p>

可以看到這裡我們設置了 Request Headers，包括：

- `accept`：想要接收怎樣格式的響應結果，這裡填寫為 `application/json`，即 JSON 格式。
- `authorization`：調用 API 的密鑰，申請之後可以直接下拉選擇。

另外設置了 Request Body，包括：
- `action`：此次音樂生成任務的行為，預設是 `generate`，主要包含：`extend`、`upload_extend`、`cover`、`upload_cover`、`replace_section`、`replace_section`、`concat`、`stems`、`all_stems`、`remaster`。
- `prompt`：Suno 官方的靈感模式提示詞（`custom` 為 `false` 時生效），最大 500 字符。
- `model`：此次音樂生成任務的模型，預設是 `chirp-v4`，主要包含：`chirp-v3`、`chirp-v4`、`chirp-v3-5`、`chirp-v4-5`、`chirp-v4-5-plus`、`chirp-v5`、`chirp-v5-5`。
- `lyric`：Suno 官方的自定義模式的歌詞內容。`chirp-v3-5` 與 `chirp-v4` 最大 3000 字符；`chirp-v4-5` 及以上（含 `chirp-v5`、`chirp-v5-5`）最大 5000 字符。
- `custom`：是否採用自定義模式，預設是：`false`。
- `instrumental`：Suno 官方的靈感模式的純音樂選項。
- `title`：Suno 官方的自定義模式的音樂標題。`chirp-v3-5`、`chirp-v4` 最大 80 字符；`chirp-v4-5` 及以上最大 100 字符。
- `style`：Suno 官方的自定義模式音樂風格。`chirp-v3-5` 與 `chirp-v4` 最大 200 字符；`chirp-v4-5` 及以上（含 `chirp-v5`、`chirp-v5-5`）最大 1000 字符。
- `style_negative`：Suno 官方的自定義模式的排除風格。
- `audio_weight`：上傳的參考音頻佔比，範圍 0-1，越大越依賴參考音頻。
- `audio_id`：參考音樂的 ID。
- `overpainting_start`/`overpainting_end`：為已有純音樂補充人聲的起止時間，單位秒。
- `underpainting_start`/`underpainting_end`：為清唱加伴奏的起止時間，單位秒。
- `persona_id`：藝術家的歌曲 ID。
- `continue_at`：以秒為單位繼續現有音頻的時間。例如，213.5 表示繼續到 3 分 33.5 秒。
- `style_influence`：自定義模式下的「Style Influence」高級參數，範圍 0-1，越大越貼合所選風格。
- `replace_section_end`：替換片段的最終時間。
- `replace_section_start`：替換片段的起始時間。
- `vocal_gender`：控制男女聲偏好，女聲 `f`，男聲 `m`，4.5 及以上模型有效；為偏好項，不保證嚴格遵循。
- `weirdness`：自定義模式下的「Weirdness」高級參數，範圍 0-1，越大越有創意和實驗性。
- `duration`：期望的歌曲時長，單位秒，取值 10-360 的整數。僅在 `action` 為 `generate`、`custom` 為 `true` 且 `model` 為 `chirp-v5-5` 時生效，其餘組合會返回 400。這是目標時長，實際成品時長以響應中的 `duration` 字段為準，可能有小幅偏差。
- `lyric_prompt`：生成歌詞的 prompt，當且僅當 `custom` 為 `true` 並且 `lyric` 沒有傳的時候生效。
- `callback_url`：需要回調結果的 URL。
- `async`：可選，設為 `true` 時接口立即返回 `task_id`，無需提供 `callback_url`，隨後通過對應的任務查詢接口輪詢獲取結果。

生成的代碼如下：

<p><img src="https://cdn.acedata.cloud/1xehwl.png" width="500" className="m-auto" /></p>

可以點擊「Try」按鈕直接測試 API，稍等 1-2 分鐘，結果如下：
```json
{
  "success": true,
  "task_id": "e72fb249-bd5b-4e2a-b20c-8a06fea5ac14",
  "trace_id": "7dbc5b6a-b2c0-4d85-9d39-fa8a8a785ccf",
  "data": [
    {
      "id": "b481b17a-bf50-4e10-8adc-4d5635050893",
      "title": "在槲寄生下",
      "image_url": "https://cdn2.suno.ai/image_b481b17a-bf50-4e10-8adc-4d5635050893.jpeg",
      "lyric": "[Verse]\n雪花飄落在地面上\n四周閃爍著點點燈光\n松樹的香氣瀰漫空氣\n聖誕魔法無處不在\n[Chorus]\n今晚在槲寄生下\n柔和月光中愛心閃耀\n笑聲迴盪\n心靈明亮\n這是聖誕時光\n感覺如此美好\n[Verse 2]\n襪子掛在火爐的光芒旁\n外面寒風吹拂而屋內溫暖\n餅乾正在烘烤\n甜蜜的喜悅\n一個閃耀著歡樂的季節\n[Chorus]\n今晚在槲寄生下\n柔和月光中愛心閃耀\n笑聲迴盪\n心靈明亮\n這是聖誕時光\n感覺如此美好\n[Bridge]\n燭光下唱響頌歌\n天空中的星星讓世界緊密相連\n和平與愛\n季節的信念\n填滿我們所需的一切心靈\n[Chorus]\n今晚在槲寄生下\n柔和月光中愛心閃耀\n笑聲迴盪\n心靈明亮\n這是聖誕時光\n感覺如此美好",
      "audio_url": "https://cdn1.suno.ai/b481b17a-bf50-4e10-8adc-4d5635050893.mp3",
      "video_url": "",
      "created_at": "2025-06-17T15:59:32.468Z",
      "model": "chirp-auk",
      "state": "succeeded",
      "prompt": "一首關於聖誕節的歌曲",
      "style": "節日、歡快、男聲演唱",
      "duration": 154.92
    },
    {
      "id": "fbf22dab-5e2b-4e02-84c0-6d7605f14c3d",
      "title": "在槲寄生下",
      "image_url": "https://cdn2.suno.ai/image_fbf22dab-5e2b-4e02-84c0-6d7605f14c3d.jpeg",
      "lyric": "[Verse]\n雪花飄落在地面上\n四周閃爍著點點燈光\n松樹的香氣瀰漫空氣\n聖誕魔法無處不在\n[Chorus]\n今晚在槲寄生下\n柔和月光中愛心閃耀\n笑聲迴盪\n心靈明亮\n這是聖誕時光\n感覺如此美好\n[Verse 2]\n襪子掛在火爐的光芒旁\n外面寒風吹拂而屋內溫暖\n餅乾正在烘烤\n甜蜜的喜悅\n一個閃耀著歡樂的季節\n[Chorus]\n今晚在槲寄生下\n柔和月光中愛心閃耀\n笑聲迴盪\n心靈明亮\n這是聖誕時光\n感覺如此美好\n[Bridge]\n燭光下唱響頌歌\n天空中的星星讓世界緊密相連\n和平與愛\n季節的信念\n填滿我們所需的一切心靈\n[Chorus]\n今晚在槲寄生下\n柔和月光中愛心閃耀\n笑聲迴盪\n心靈明亮\n這是聖誕時光\n感覺如此美好",
      "audio_url": "https://cdn1.suno.ai/fbf22dab-5e2b-4e02-84c0-6d7605f14c3d.mp3",
      "video_url": "",
      "created_at": "2025-06-17T15:59:32.468Z",
      "model": "chirp-auk",
      "state": "succeeded",
      "prompt": "一首關於聖誕節的歌曲",
      "style": "節日、歡快、男聲演唱",
      "duration": 158.48
    }
  ]
}
```

可以看到這時候我們就得到了兩首歌的內容，包括標題、預覽圖、歌詞、音頻、視頻等內容。

字段說明如下：

- success：生成是否成功，如果成功則為 `true`，否則為 `false`
- data：是一個列表，包含了生成的歌曲的詳細資訊。
  - state：歌曲生成狀態，主要包含四種，具體如下：
    - succeeded：生成成功
    - pending：隊列中
    - running：執行中
    - error：失敗
  - id：歌曲 ID
  - title：歌曲的標題
  - image_url：歌曲的封面圖片
  - lyric：歌曲的歌詞
  - audio_url：歌曲的音頻文件，打開就是一個 mp3 音頻。
  - video_url：歌曲的視頻文件，打開就是一個 mp4 視頻。
  - created_at：創建的時間
  - model：使用的模型，一般是最新的 v3 模型
  - style：風格

## 自定義生成

如果想自定義生成歌詞，可以輸入歌詞：

這時候 `lyric` 字段可以傳入類似如下內容：

```
[Verse]\nSnowflakes falling all around\nGlistening white\nCovering the ground\nChildren laughing\nFull of delight\nIn this winter wonderland tonight\nSanta's sleigh\nUp in the sky\nRudolph's nose shining bright\nOh my\nHear the jingle bells\nRinging so clear\nBringing joy and holiday cheer\n[Verse 2]\nRoasting chestnuts by the fire's glow\nChristmas lights\nThey twinkle and show\nFamilies gathering with love and cheer\nSpreading warmth to everyone near
```

> 注意，這裡的歌詞中 `\n` 是換行符，如果你不知道如何生成歌詞，可以使用 AceDataCloud 提供的歌詞生成 API 來通過 prompt 生成歌詞，API 是 [Suno Lyrics Generation API](https://platform.acedata.cloud/documents/suno-lyrics)。

接下來我們要根據歌詞、標題、風格自定義生成歌曲，就可以指定如下內容：

- lyric：歌詞文本
- custom：填寫為 `true`，代表自定義生成，該參數默認為 false，代表使用 `prompt` 生成。
- title：歌曲的標題。
- style：歌曲的風格，選填。

填寫樣例如下：

<p><img src="https://cdn.acedata.cloud/qp3iba.png" width="500" className="m-auto" /></p>

填寫完畢之後自動生成了代碼如下：

<p><img src="https://cdn.acedata.cloud/o5haei.png" width="500" className="m-auto" /></p>

對應的代碼：

```shell
curl -X POST 'https://api.acedata.cloud/suno/audios' \
-H 'accept: application/json' \
-H 'authorization: Bearer {token}' \
-H 'content-type: application/json' \
-d '{
  "action": "generate",
  "prompt": "A song for Christmas",
  "model": "chirp-v4-5",
  "lyric": "[Verse]\\nSnowflakes falling all around\\nGlistening white\\nCovering the ground\\nChildren laughing\\nFull of delight\\nIn this winter wonderland tonight\\nSanta's sleigh\\nUp in the sky\\nRudolph's nose shining bright\\nOh my\\nHear the jingle bells\\nRinging so clear\\nBringing joy and holiday cheer\\n[Verse 2]\\nRoasting chestnuts by the fire's glow\\nChristmas lights\\nThey twinkle and show\\nFamilies gathering with love and cheer\\nSpreading warmth to everyone near",
  "custom": true
}'
```

測試允許，生成的效果是類似的。

## 自定義歌手風格生成功能
```python
import requests

url = "https://api.acedata.cloud/suno/audios"

headers = {
    "accept": "application/json",
    "authorization": "Bearer {token}",
    "content-type": "application/json"
}

payload = {
    "action": "extend",
    "audio_id": "97efc9f4-0e8d-4b3e-88df-14568fa1b11f",
    "lyric": "你的歌词文本",
    "custom": True,
    "style": "你的歌曲风格",
    "continue_at": 213.5
}

response = requests.post(url, json=payload, headers=headers)
print(response.text)
```
```python
import requests

url = "https://api.acedata.cloud/suno/audios"

headers = {
    "accept": "application/json",
    "authorization": "Bearer {token}",
    "content-type": "application/json"
}

payload = {
    "action": "extend",
    "prompt": "A song for Christmas",
    "model": "chirp-v4-5",
    "audio_id": "97efc9f4-0e8d-4b3e-88df-14568fa1b11f",
    "continue_at": 2,
    "lyric": "[Verse]\\n雪花在四周飄落\\n閃閃發光的白色\\n覆蓋著大地\\n孩子們在笑\\n充滿了喜悅\\n在這個冬季仙境的夜晚\\n聖誕老人的雪橇\\n在天空中\\n魯道夫的鼻子閃閃發亮\\n哦我的\\n聽到叮噹鈴聲\\n響亮而清晰\\n帶來快樂和節日的喜悅\\n[Verse 2]\\n在火光中烤栗子\\n聖誕燈飾\\n閃爍著顯示\\n家庭聚在一起，充滿愛與歡樂\\n將溫暖傳遞給每一個人",
    "custom": True,
    "instrumental": False
}

response = requests.post(url, json=payload, headers=headers)
print(response.text)
```

可以看出，結果內容與上文的是一致的，這也就實現歌曲的繼續生成功能。

## 獲取完整歌曲

當基於原有的歌曲繼續生成歌曲之後，返回的歌曲並不包含原來的歌曲內容。如果要獲得完整的歌曲內容，需要使用拼接功能，就可以指定如下內容：

- action：內容為 `concat`。
- audio_id：最後一個片段的 ID。

比如擴展後的歌曲 ID 是：0a1e1b10-c36a-41c9-9bfb-b26d9d25db98，那麼可以設置參數如下：

```json
{
  "action": "concat",
  "audio_id": "0a1e1b10-c36a-41c9-9bfb-b26d9d25db98"
}
```

其他參數不變，返回的就是一首完整的歌曲，就是所有歌曲片段的拼接結果，但結果只有一首歌，樣例如下：

```json
{
  "success": true,
  "task_id": "0f794915-8418-4124-93f5-b7eb3a417167",
  "trace_id": "62afcce0-e7e5-44b8-8c2e-4bba12a4f414",
  "data": [
    {
      "id": "0efec7e0-11bf-4313-9981-2c0e7218d7dd",
      "title": "",
      "image_url": "https://cdn2.suno.ai/image_0a1e1b10-c36a-41c9-9bfb-b26d9d25db98.jpeg",
      "lyric": "[Verse]\\n雪花在四周飄落\\n閃閃發光的白色\\n覆蓋著大地\\n孩子們在笑\\n充滿了喜悅\\n在這個冬季仙境的夜晚\\n聖誕老人的雪橇\\n在天空中\\n魯道夫的鼻子閃閃發亮\\n哦我的\\n聽到叮噹鈴聲\\n響亮而清晰\\n帶來快樂和節日的喜悅\\n[Verse 2]\\n在火光中烤栗子\\n聖誕燈飾\\n閃爍著顯示\\n家庭聚在一起，充滿愛與歡樂\\n將溫暖傳遞給每一個人\n[Verse]\\n雪花在四周飄落\\n閃閃發光的白色\\n覆蓋著大地\\n孩子們在笑\\n充滿了喜悅\\n在這個冬季仙境的夜晚\\n聖誕老人的雪橇\\n在天空中\\n魯道夫的鼻子閃閃發亮\\n哦我的\\n聽到叮噹鈴聲\\n響亮而清晰\\n帶來快樂和節日的喜悅\\n[Verse 2]\\n在火光中烤栗子\\n聖誕燈飾\\n閃爍著顯示\\n家庭聚在一起，充滿愛與歡樂\\n將溫暖傳遞給每一個人",
      "audio_url": "https://cdn1.suno.ai/0efec7e0-11bf-4313-9981-2c0e7218d7dd.mp3",
      "video_url": "",
      "created_at": "2025-06-17T16:43:06.718Z",
      "model": "chirp-auk",
      "state": "succeeded",
      "style": "",
      "duration": 167.91997916666668,
      "concat_history": [
        {
          "continue_at": 2,
          "id": "97efc9f4-0e8d-4b3e-88df-14568fa1b11f",
          "infill": false,
          "source": "web",
          "type": "gen"
        },
        {
          "id": "0a1e1b10-c36a-41c9-9bfb-b26d9d25db98"
        }
      ]
    }
  ]
}
```

## 音樂翻版
當基於原有的歌曲繼續生成歌曲之後，返回的歌曲的風格可能不太合適。如果要對原先生成的歌曲（若是自定義上傳的音樂也支持）進行翻版，需要使用音樂翻版方法，就可以指定如下內容：

- action：內容為 `cover`，當是對自定義上傳的音樂進行翻版操作的時候必須指定內容為：`upload_cover`。
- audio_id：之前生成歌曲的 ID。

比如原先生成後的歌曲 ID 是：0a1e1b10-c36a-41c9-9bfb-b26d9d25db98，那麼可以設置參數如下：

```json
{
  "action": "cover",
  "audio_id": "0a1e1b10-c36a-41c9-9bfb-b26d9d25db98",
  "prompt": "A song for Christmas",
  "model": "chirp-v4-5"
}
```

其他參數不變，返回的就是一首翻版後的歌曲，就是對原先生成的歌曲進行翻版後的結果，樣例如下：

```json
{
  "success": true,
  "task_id": "b9d43d06-2e0a-4b5e-9e0d-7dfab32e00ab",
  "trace_id": "6c5e567d-0fdc-4c44-9b36-4090d9a75ff5",
  "data": [
    {
      "id": "6988fa57-f810-41cf-afab-7838db2c77dc",
      "title": "",
      "image_url": "https://cdn2.suno.ai/image_6988fa57-f810-41cf-afab-7838db2c77dc.jpeg",
      "lyric": "",
      "audio_url": "https://cdn1.suno.ai/6988fa57-f810-41cf-afab-7838db2c77dc.mp3",
      "video_url": "",
      "created_at": "2025-06-17T16:44:13.007Z",
      "model": "chirp-auk",
      "state": "succeeded",
      "style": "",
      "duration": 182.4
    },
    {
      "id": "ce98b991-0258-4f05-8245-e43d4efa8fb8",
      "title": "",
      "image_url": "https://cdn2.suno.ai/image_ce98b991-0258-4f05-8245-e43d4efa8fb8.jpeg",
      "lyric": "",
      "audio_url": "https://cdn1.suno.ai/ce98b991-0258-4f05-8245-e43d4efa8fb8.mp3",
      "video_url": "",
      "created_at": "2025-06-17T16:44:13.007Z",
      "model": "chirp-auk",
      "state": "succeeded",
      "style": "",
      "duration": 179.72
    }
  ]
}
```

生成的結果與上文類似，這就完成了對原先生成的歌曲進行翻版生成的過程。

## 替換片段

當生成歌曲之後需要進行替換歌曲片段單獨操作的二次創作時，可以對歌曲的某個片段進行替換操作。

> ⚠️ **注意**：`replace_section` 單獨使用時**只會返回新生成的"替換片段"本身**（即被替換那一段的新音頻，時長大約等於 `replace_section_end - replace_section_start`，並附帶少量上下文），**並不會返回拼接好的整首歌曲**。要得到與原曲拼接後的完整成品，需要在 `replace_section` 成功後，針對返回的片段 ID 再發起一次 [音樂拼接](#音樂拼接) 任務。完整流程見下文。

參數說明如下：

- action：內容為 `replace_section`。
- audio_id：原始歌曲（被替換的源歌曲）的 ID。
- model: 歌曲生成模型。
- lyric: 替換後的完整歌詞（包含被替換段及其上下文，與 `prompt` 中的內容保持一致）。
- prompt：需要替換的那一段新歌詞。
- style：歌曲的風格，選填。
- replace_section_start：被替換片段在原曲中的起始時間（秒）。
- replace_section_end：被替換片段在原曲中的終止時間（秒）。

### 步驟一：發起替換片段任務

比如原先生成後的歌曲 ID 是：18db7ed0-2b8a-41db-91c1-b0781dcca0d4（時長 94.12 秒），希望把第 30 秒到第 60 秒處的副歌替換為新的歌詞，那麼可以設置參數如下：

```json
{
  "action": "replace_section",
  "audio_id": "18db7ed0-2b8a-41db-91c1-b0781dcca0d4",
  "model": "chirp-v5-5",
  "custom": false,
  "instrumental": false,
  "lyric": "[Intro]\n鑼鼓喧天 紅燈高掛\n[Verse 1]\n爆竹聲聲辭舊歲\n春風暖暖入萬家\n紅包壓歲笑開顏\n金蛇起舞賀新春\n[Chorus]\n梅花綻放春意洋溢滿地\n梅花綻放春意洋溢滿地\n梅花綻放春意洋溢滿地\n梅花綻放春意洋溢滿地\n[Verse 2]\n餃子飄香年夜飯\n燈籠搖曳照團圓",
  "prompt": "梅花綻放春意洋溢滿地\n梅花綻放春意洋溢滿地\n梅花綻放春意洋溢滿地\n梅花綻放春意洋溢滿地",
  "replace_section_start": 30.0,
  "replace_section_end": 60.0
}
```

返回的是新生成的替換片段（共 2 個候選），樣例如下：
```json
{
  "success": true,
  "task_id": "dd067075-a295-4160-8375-d5504327d55b",
  "trace_id": "c34f589b-9195-4d0b-af78-9c890e77609c",
  "data": [
    {
      "id": "364f9d8b-ca25-463b-9a5e-d0b7139e2d6a",
      "title": "",
      "image_url": "https://cdn2.suno.ai/image_364f9d8b-ca25-463b-9a5e-d0b7139e2d6a.jpeg",
      "image_large_url": "https://cdn2.suno.ai/image_364f9d8b-ca25-463b-9a5e-d0b7139e2d6a.jpeg",
      "lyric": "[Intro]\n鑼鼓喧天 紅燈高掛\n[Verse 1]\n爆竹聲聲辭舊歲\n春風暖暖入萬家\n紅包壓歲笑開顏\n金蛇起舞賀新春\n[Chorus]\n梅花綻放春意洋溢滿地\n梅花綻放春意洋溢滿地\n梅花綻放春意洋溢滿地\n梅花綻放春意洋溢滿地\n[Verse 2]\n餃子飄香年夜飯\n燈籠搖曳照團圓",
      "audio_url": "https://cdn1.suno.ai/364f9d8b-ca25-463b-9a5e-d0b7139e2d6a.mp3",
      "video_url": "",
      "created_at": "2026-05-06T06:55:00.000Z",
      "model": "chirp-v5-5",
      "state": "succeeded",
      "style": "",
      "duration": 45.16
    },
    {
      "id": "fae966ea-5f7f-4e80-9962-1c57963c7f8a",
      "title": "",
      "audio_url": "https://cdn1.suno.ai/fae966ea-5f7f-4e80-9962-1c57963c7f8a.mp3",
      "model": "chirp-v5-5",
      "state": "succeeded",
      "duration": 33.8
    }
  ]
}
```

可以看到返回的兩條音訊時長（45.16 秒、33.8 秒）遠短於原曲（94.12 秒），它們就是替換片段本身（包含一點點前後上下文用於過渡），並**不是**整首拼接後的歌曲。從兩條候選中挑選一條滿意的即可，下一步就基於這條片段做拼接。

### 步驟二：將替換片段拼接回原曲

針對上面挑選的片段（例如 `364f9d8b-ca25-463b-9a5e-d0b7139e2d6a`），按 [音樂拼接](#音樂拼接) 一節的方法發起 `concat` 任務：

```json
{
  "action": "concat",
  "audio_id": "364f9d8b-ca25-463b-9a5e-d0b7139e2d6a",
  "model": "chirp-v5-5"
}
```

返回的是拼接好的整首完整歌曲，範例如下：

```json
{
  "success": true,
  "task_id": "5dbd4a78-0197-4ef3-9c16-8bddaf4f0c94",
  "trace_id": "580bd1da-2ad3-4d75-be1f-6c14bd4b489d",
  "data": [
    {
      "id": "365a9640-0452-4567-80f0-4f5a2a17ddd5",
      "title": "新年快樂",
      "image_url": "https://cdn2.suno.ai/image_364f9d8b-ca25-463b-9a5e-d0b7139e2d6a.jpeg",
      "lyric": "[Intro]\n鑼鼓喧天 紅燈高掛\n[Verse 1]\n爆竹聲聲辭舊歲\n春風暖暖入萬家\n紅包壓歲笑開顏\n金蛇起舞賀新春\n[Chorus]\n梅花綻放春意洋溢滿地\n梅花綻放春意洋溢滿地\n梅花綻放春意洋溢滿地\n梅花綻放春意洋溢滿地\n[Verse 2]\n餃子飄香年夜飯\n燈籠搖曳照團圓",
      "audio_url": "https://cdn1.suno.ai/365a9640-0452-4567-80f0-4f5a2a17ddd5.mp3",
      "video_url": "",
      "created_at": "2026-05-06T06:56:46.057Z",
      "model": "chirp-v5-5",
      "state": "succeeded",
      "style": "traditional Chinese new year, festive, female vocals, upbeat",
      "duration": 105.28
    }
  ]
}
```

此時 `duration` 已恢復成完整歌曲長度（105.28 秒，約等於原曲長度），`audio_url` 指向的就是替換完成後的整首歌曲。這就完成了「生成 → 替換片段 → 拼接整曲」的二次創作流程。

## 聲曲分離

當生成歌曲之後需要進行伴奏和人聲單獨操作的二次創作時，可以分離純音樂伴奏和清唱人聲。就可以指定如下內容：

- action：內容為 `stems`。
- audio_id：之前生成歌曲的 ID。

比如原先生成後的歌曲 ID 是：ec13e502-d043-4eb2-92ee-e900c6da69d1，那麼可以設定參數如下：

```json
{
  "action": "stems",
  "audio_id": "ec13e502-d043-4eb2-92ee-e900c6da69d1"
}
```

通過以上參數即可得到聲曲分離的結果，結果如下：
```
```json
{
  "success": true,
  "task_id": "4050affc-f8a6-4cba-a86c-bf201eed053d",
  "trace_id": "5107ee58-687d-422f-9195-fa0e82e1fcc8",
  "data": [
    {
      "id": "e3de0928-085a-42c4-b982-3b24738d1989",
      "title": "裝飾天空 - 人聲",
      "image_url": "https://cdn2.suno.ai/image_e3de0928-085a-42c4-b982-3b24738d1989.jpeg",
      "lyric": "[Verse]\n雪花在高高的屋頂上舞動\n孩子們的笑聲充滿天空\n教堂的鐘聲響起\n假日的快樂人群\n[Verse 2]\n糖果手杖和熱可可溫暖\n緊緊包裹在我們自己的風暴中\n掛著夢想和歡樂的長襪\n魔法每年都在增長\n[Chorus]\n用閃爍的星星裝飾天空\n假日的喜悅屬於我們\n唱出愛與光的歌曲\n聖誕節閃耀著如此純淨和明亮\n[Verse 3]\n爐邊的故事來自久遠\n馴鹿在冰冷的光芒中跳躍\n常青樹和亮片的光芒\n聖誕時光是一個美好的夢\n[Bridge]\n心中充滿了朋友和親人\n槲寄生讓愛得以勝利\n我們分享愛與希望的禮物\n聖誕精神無處不在\n[Chorus]\n用閃爍的星星裝飾天空\n假日的喜悅屬於我們\n唱出愛與光的歌曲\n聖誕節閃耀著如此純淨和明亮",
      "audio_url": "https://cdn1.suno.ai/e3de0928-085a-42c4-b982-3b24738d1989.mp3",
      "video_url": "https://cdn1.suno.ai/e3de0928-085a-42c4-b982-3b24738d1989.mp4",
      "created_at": "2025-01-05T07:49:16.881Z",
      "model": "",
      "state": "succeeded",
      "style": "假日, 快樂",
      "duration": 174.16
    },
    {
      "id": "ad5d7c89-709c-4eb4-a5a6-72f9f5e57fdb",
      "title": "裝飾天空 - 器樂",
      "image_url": "https://cdn2.suno.ai/image_ad5d7c89-709c-4eb4-a5a6-72f9f5e57fdb.jpeg",
      "lyric": "",
      "audio_url": "https://cdn1.suno.ai/ad5d7c89-709c-4eb4-a5a6-72f9f5e57fdb.mp3",
      "video_url": "https://cdn1.suno.ai/ad5d7c89-709c-4eb4-a5a6-72f9f5e57fdb.mp4",
      "created_at": "2025-01-05T07:49:16.892Z",
      "model": "",
      "state": "succeeded",
      "style": "假日, 快樂",
      "duration": 174.16
    }
  ]
}
```

生成的結果與上文類似，這就完成了對原先生成的歌曲進行聲曲分離的過程。

## 全軌道聲曲分離

當生成歌曲之後需要進行全軌道聲曲分離操作時，就可以指定如下內容：

- action：內容為 `all_stems`。
- audio_id：之前生成歌曲的 ID。

比如原先生成後的歌曲 ID 是：bdf23a5a-59f5-4103-b452-054a824a7f9f，那麼可以設置參數如下：

```json
{
  "action": "all_stems",
  "audio_id": "bdf23a5a-59f5-4103-b452-054a824a7f9f"
}
```

通過以上參數即可得到全軌道聲曲分離的結果，結果如下：

```json
{
  "success": true,
  "task_id": "f4b16fb9-8478-4857-88c7-b9a1f0bb9518",
  "trace_id": "9c560ebd-4fc6-4bdb-988a-8890160a92fb",
  "data": [
    {
      "id": "f86ca64a-9519-4ea7-a592-52438e001412",
      "title": "安全之弦 (人聲)",
      "image_url": "https://cdn2.suno.ai/image_f86ca64a-9519-4ea7-a592-52438e001412.jpeg",
      "lyric": "",
      "audio_url": "https://cdn1.suno.ai/f86ca64a-9519-4ea7-a592-52438e001412.mp3",
      "video_url": "",
      "created_at": "2025-06-11T02:40:30.770Z",
      "model": "chirp-ahi-stem-12-t1",
      "state": "succeeded",
      "duration": 154.92
    }, {
      "id": "99e649a7-a394-47b9-a915-d7f847285a36",
      "title": "安全之弦 (伴唱)",
      "image_url": "https://cdn2.suno.ai/image_99e649a7-a394-47b9-a915-d7f847285a36.jpeg",
      "lyric": "",
      "audio_url": "https://cdn1.suno.ai/99e649a7-a394-47b9-a915-d7f847285a36.mp3",
      "video_url": "",
      "created_at": "2025-06-11T02:40:30.770Z",
      "model": "chirp-ahi-stem-12-t1",
      "state": "succeeded",
      "duration": 154.92
    }, {
      "id": "6d710bf7-809f-4fdc-bb63-b8cb3a456d42",
      "title": "安全之弦 (鼓)",
      "image_url": "https://cdn2.suno.ai/image_6d710bf7-809f-4fdc-bb63-b8cb3a456d42.jpeg",
      "lyric": "",
      "audio_url": "https://cdn1.suno.ai/6d710bf7-809f-4fdc-bb63-b8cb3a456d42.mp3",
      "video_url": "",
      "created_at": "2025-06-11T02:40:30.770Z",
      "model": "chirp-ahi-stem-12-t1",
      "state": "succeeded",
      "duration": 154.92
    }, {
      "id": "e05f07e3-7d80-4713-8e51-7f176c733543",
      "title": "安全之弦 (貝斯)",
      "image_url": "https://cdn2.suno.ai/image_e05f07e3-7d80-4713-8e51-7f176c733543.jpeg",
      "lyric": "",
      "audio_url": "https://cdn1.suno.ai/e05f07e3-7d80-4713-8e51-7f176c733543.mp3",
      "video_url": "",
      "created_at": "2025-06-11T02:40:30.770Z",
      "model": "chirp-ahi-stem-12-t1",
      "state": "succeeded",
      "duration": 154.92
    }, {
      "id": "93fe7cd8-62fd-4739-b78e-142c7e0b8562",
      "title": "安全之弦 (吉他)",
      "image_url": "https://cdn2.suno.ai/image_93fe7cd8-62fd-4739-b78e-142c7e0b8562.jpeg",
      "lyric": "",
      "audio_url": "https://cdn1.suno.ai/93fe7cd8-62fd-4739-b78e-142c7e0b8562.mp3",
      "video_url": "",
      "created_at": "2025-06-11T02:40:30.770Z",
      "model": "chirp-ahi-stem-12-t1",
      "state": "succeeded",
      "duration": 154.92
    }
  ]
}
```
{
      "id": "8367d71c-fdd3-441c-8ebe-70c33cca821b",
      "title": "安全之弦 (鍵盤)",
      "image_url": "https://cdn2.suno.ai/image_8367d71c-fdd3-441c-8ebe-70c33cca821b.jpeg",
      "lyric": "",
      "audio_url": "https://cdn1.suno.ai/8367d71c-fdd3-441c-8ebe-70c33cca821b.mp3",
      "video_url": "",
      "created_at": "2025-06-11T02:40:30.770Z",
      "model": "chirp-ahi-stem-12-t1",
      "state": "succeeded",
      "duration": 154.92
    }, {
      "id": "28c03590-731c-416e-8fd3-95cdb3d75043",
      "title": "安全之弦 (打擊樂)",
      "image_url": "https://cdn2.suno.ai/image_28c03590-731c-416e-8fd3-95cdb3d75043.jpeg",
      "lyric": "",
      "audio_url": "https://cdn1.suno.ai/28c03590-731c-416e-8fd3-95cdb3d75043.mp3",
      "video_url": "",
      "created_at": "2025-06-11T02:40:30.770Z",
      "model": "chirp-ahi-stem-12-t1",
      "state": "succeeded",
      "duration": 154.92
    }, {
      "id": "3d4c1a28-4e1c-485a-8201-d21bb93aca2f",
      "title": "安全之弦 (弦樂)",
      "image_url": "https://cdn2.suno.ai/image_3d4c1a28-4e1c-485a-8201-d21bb93aca2f.jpeg",
      "lyric": "",
      "audio_url": "https://cdn1.suno.ai/3d4c1a28-4e1c-485a-8201-d21bb93aca2f.mp3",
      "video_url": "",
      "created_at": "2025-06-11T02:40:30.770Z",
      "model": "chirp-ahi-stem-12-t1",
      "state": "succeeded",
      "duration": 154.92
    }, {
      "id": "b9db8ded-01ec-4e37-b8a5-64aab3a814c2",
      "title": "安全之弦 (合成器)",
      "image_url": "https://cdn2.suno.ai/image_b9db8ded-01ec-4e37-b8a5-64aab3a814c2.jpeg",
      "lyric": "",
      "audio_url": "https://cdn1.suno.ai/b9db8ded-01ec-4e37-b8a5-64aab3a814c2.mp3",
      "video_url": "",
      "created_at": "2025-06-11T02:40:30.770Z",
      "model": "chirp-ahi-stem-12-t1",
      "state": "succeeded",
      "duration": 154.92
    }, {
      "id": "10a5248e-32e6-42b9-8da1-678a8a392aef",
      "title": "安全之弦 (特效)",
      "image_url": "https://cdn2.suno.ai/image_10a5248e-32e6-42b9-8da1-678a8a392aef.jpeg",
      "lyric": "",
      "audio_url": "https://cdn1.suno.ai/10a5248e-32e6-42b9-8da1-678a8a392aef.mp3",
      "video_url": "",
      "created_at": "2025-06-11T02:40:30.770Z",
      "model": "chirp-ahi-stem-12-t1",
      "state": "succeeded",
      "duration": 154.92
    }, {
      "id": "2d272128-111f-4901-8f62-5ae1eb43095a",
      "title": "安全之弦 (銅管樂器)",
      "image_url": "https://cdn2.suno.ai/image_2d272128-111f-4901-8f62-5ae1eb43095a.jpeg",
      "lyric": "",
      "audio_url": "https://cdn1.suno.ai/2d272128-111f-4901-8f62-5ae1eb43095a.mp3",
      "video_url": "",
      "created_at": "2025-06-11T02:40:30.770Z",
      "model": "chirp-ahi-stem-12-t1",
      "state": "succeeded",
      "duration": 154.92
    }, {
      "id": "4a7c19a5-f8d4-4e4a-add9-aa0bad9307cc",
      "title": "安全之弦 (木管樂器)",
      "image_url": "https://cdn2.suno.ai/image_4a7c19a5-f8d4-4e4a-add9-aa0bad9307cc.jpeg",
      "lyric": "",
      "audio_url": "https://cdn1.suno.ai/4a7c19a5-f8d4-4e4a-add9-aa0bad9307cc.mp3",
      "video_url": "",
      "created_at": "2025-06-11T02:40:30.770Z",
      "model": "chirp-ahi-stem-12-t1",
      "state": "succeeded",
      "duration": 154.92
    }, {
      "id": "1ca774f9-3e75-48a6-941b-808875eadcd2",
      "title": "安全之弦 (人聲)",
      "image_url": "https://cdn2.suno.ai/image_1ca774f9-3e75-48a6-941b-808875eadcd2.jpeg",
      "lyric": "",
      "audio_url": "https://cdn1.suno.ai/1ca774f9-3e75-48a6-941b-808875eadcd2.mp3",
      "video_url": "",
      "created_at": "2025-06-11T02:40:30.770Z",
      "model": "chirp-ahi-stem-12-t1",
      "state": "succeeded",
      "duration": 154.92
    }, {
      "id": "14c5ffc7-addf-4fee-afd2-4b8b3e7ee470",
      "title": "安全之弦 (和聲)",
      "image_url": "https://cdn2.suno.ai/image_14c5ffc7-addf-4fee-afd2-4b8b3e7ee470.jpeg",
      "lyric": "",
      "audio_url": "https://cdn1.suno.ai/14c5ffc7-addf-4fee-afd2-4b8b3e7ee470.mp3",
      "video_url": "",
      "created_at": "2025-06-11T02:40:30.771Z",
      "model": "chirp-ahi-stem-12-t1",
      "state": "succeeded",
      "duration": 154.92
    }, {
      "id": "9d044557-450d-48ab-90dc-8eaf6f1cdb6c",
      "title": "安全之弦 (鼓)",
      "image_url": "https://cdn2.suno.ai/image_9d044557-450d-48ab-90dc-8eaf6f1cdb6c.jpeg",
      "lyric": "",
      "audio_url": "https://cdn1.suno.ai/9d044557-450d-48ab-90dc-8eaf6f1cdb6c.mp3",
      "video_url": "",
      "created_at": "2025-06-11T02:40:30.771Z",
      "model": "chirp-ahi-stem-12-t1",
      "state": "succeeded",
      "duration": 154.92
    },
{
      "id": "efd052d0-c12f-47b3-8282-1f3ef7610e1f",
      "title": "安全之弦 (Bass)",
      "image_url": "https://cdn2.suno.ai/image_efd052d0-c12f-47b3-8282-1f3ef7610e1f.jpeg",
      "lyric": "",
      "audio_url": "https://cdn1.suno.ai/efd052d0-c12f-47b3-8282-1f3ef7610e1f.mp3",
      "video_url": "",
      "created_at": "2025-06-11T02:40:30.771Z",
      "model": "chirp-ahi-stem-12-t1",
      "state": "succeeded",
      "duration": 154.92
    }, {
      "id": "5775372b-292e-4420-96ef-60e57a60cc1f",
      "title": "安全之弦 (Guitar)",
      "image_url": "https://cdn2.suno.ai/image_5775372b-292e-4420-96ef-60e57a60cc1f.jpeg",
      "lyric": "",
      "audio_url": "https://cdn1.suno.ai/5775372b-292e-4420-96ef-60e57a60cc1f.mp3",
      "video_url": "",
      "created_at": "2025-06-11T02:40:30.771Z",
      "model": "chirp-ahi-stem-12-t1",
      "state": "succeeded",
      "duration": 154.92
    }, {
      "id": "dab3f220-19cd-408e-9b96-30ec18f5b049",
      "title": "安全之弦 (Keyboard)",
      "image_url": "https://cdn2.suno.ai/image_dab3f220-19cd-408e-9b96-30ec18f5b049.jpeg",
      "lyric": "",
      "audio_url": "https://cdn1.suno.ai/dab3f220-19cd-408e-9b96-30ec18f5b049.mp3",
      "video_url": "",
      "created_at": "2025-06-11T02:40:30.771Z",
      "model": "chirp-ahi-stem-12-t1",
      "state": "succeeded",
      "duration": 154.92
    }, {
      "id": "2d0cd6d4-af82-4bb5-86fe-d92bdb367157",
      "title": "安全之弦 (Percussion)",
      "image_url": "https://cdn2.suno.ai/image_2d0cd6d4-af82-4bb5-86fe-d92bdb367157.jpeg",
      "lyric": "",
      "audio_url": "https://cdn1.suno.ai/2d0cd6d4-af82-4bb5-86fe-d92bdb367157.mp3",
      "video_url": "",
      "created_at": "2025-06-11T02:40:30.771Z",
      "model": "chirp-ahi-stem-12-t1",
      "state": "succeeded",
      "duration": 154.92
    }, {
      "id": "f3191a1a-5e8d-4afe-b638-3add222d52cd",
      "title": "安全之弦 (Strings)",
      "image_url": "https://cdn2.suno.ai/image_f3191a1a-5e8d-4afe-b638-3add222d52cd.jpeg",
      "lyric": "",
      "audio_url": "https://cdn1.suno.ai/f3191a1a-5e8d-4afe-b638-3add222d52cd.mp3",
      "video_url": "",
      "created_at": "2025-06-11T02:40:30.771Z",
      "model": "chirp-ahi-stem-12-t1",
      "state": "succeeded",
      "duration": 154.92
    }, {
      "id": "a8834ea5-200b-4206-a812-9780ef336660",
      "title": "安全之弦 (Synth)",
      "image_url": "https://cdn2.suno.ai/image_a8834ea5-200b-4206-a812-9780ef336660.jpeg",
      "lyric": "",
      "audio_url": "https://cdn1.suno.ai/a8834ea5-200b-4206-a812-9780ef336660.mp3",
      "video_url": "",
      "created_at": "2025-06-11T02:40:30.771Z",
      "model": "chirp-ahi-stem-12-t1",
      "state": "succeeded",
      "duration": 154.92
    }, {
      "id": "f50d1a31-ef72-400a-b8ae-0367849d007d",
      "title": "安全之弦 (FX)",
      "image_url": "https://cdn2.suno.ai/image_f50d1a31-ef72-400a-b8ae-0367849d007d.jpeg",
      "lyric": "",
      "audio_url": "https://cdn1.suno.ai/f50d1a31-ef72-400a-b8ae-0367849d007d.mp3",
      "video_url": "",
      "created_at": "2025-06-11T02:40:30.771Z",
      "model": "chirp-ahi-stem-12-t1",
      "state": "succeeded",
      "duration": 154.92
    }, {
      "id": "cb581673-23cc-40d6-9f9b-0f76720f0d18",
      "title": "安全之弦 (Brass)",
      "image_url": "https://cdn2.suno.ai/image_cb581673-23cc-40d6-9f9b-0f76720f0d18.jpeg",
      "lyric": "",
      "audio_url": "https://cdn1.suno.ai/cb581673-23cc-40d6-9f9b-0f76720f0d18.mp3",
      "video_url": "",
      "created_at": "2025-06-11T02:40:30.771Z",
      "model": "chirp-ahi-stem-12-t1",
      "state": "succeeded",
      "duration": 154.92
    }, {
      "id": "d91cfb52-f0a3-4546-bf8a-2ad14c3775a5",
      "title": "安全之弦 (Woodwinds)",
      "image_url": "https://cdn2.suno.ai/image_d91cfb52-f0a3-4546-bf8a-2ad14c3775a5.jpeg",
      "lyric": "",
      "audio_url": "https://cdn1.suno.ai/d91cfb52-f0a3-4546-bf8a-2ad14c3775a5.mp3",
      "video_url": "",
      "created_at": "2025-06-11T02:40:30.771Z",
      "model": "chirp-ahi-stem-12-t1",
      "state": "succeeded",
      "duration": 154.92
    }
  ]
}
```
 

生成的结果与上文类似，这就完成了对原先生成的歌曲进行声曲分离的过程。

## 自定义生成的高级参数

官方允许在自定义模式下使用高级参数 `weirdness`==>`怪异度`、`style_influence`==>`风格影响`、`audio_weight`==>`音频影响`来进行生成，对应如下如的官方示例：

<p><img src="https://cdn.acedata.cloud/1xonxy.png" width="500" className="m-auto" /></p>

其中高级参数的范围都在 0-1 之间，具体的参数如下图所示：

<p><img src="https://cdn.acedata.cloud/7i94ih.png" width="500" className="m-auto" /></p>

填写完毕之后自动生成了代码如下：

<p><img src="https://cdn.acedata.cloud/2dlbo6.png" width="500" className="m-auto" /></p>

对应的 Python 代码：
```python
import requests

url = "https://api.acedata.cloud/suno/audios"

headers = {
    "accept": "application/json",
    "authorization": "Bearer {token}",
    "content-type": "application/json"
}

payload = {
    "action": "generate",
    "model": "chirp-v4-5",
    "lyric": "Hello Hello Hello ",
    "custom": True,
    "weirdness": 0.4,
    "style_influence": 0.4,
    "audio_weight": 0.4
}

response = requests.post(url, json=payload, headers=headers)
print(response.text)
```

點擊運行，可以發現會得到一個結果，如下：

```json
{
  "success": true,
  "task_id": "2f3aa682-e1a7-43a9-9fbb-ed0ce8668a4b",
  "trace_id": "07408194-7deb-4a52-a36d-9a9a13143b5f",
  "data": [
    {
      "id": "c66e2077-7580-43f2-9937-c67a8afcd8bd",
      "title": "",
      "image_url": "https://cdn2.suno.ai/image_c66e2077-7580-43f2-9937-c67a8afcd8bd.jpeg",
      "lyric": "Hello Hello Hello ",
      "audio_url": "https://cdn1.suno.ai/c66e2077-7580-43f2-9937-c67a8afcd8bd.mp3",
      "video_url": "",
      "created_at": "2025-07-10T12:54:35.199Z",
      "model": "chirp-auk",
      "state": "succeeded",
      "style": "",
      "duration": 187.92
    },
    {
      "id": "a922f97b-307c-4c4d-aae3-a47ba8202a10",
      "title": "",
      "image_url": "https://cdn2.suno.ai/image_a922f97b-307c-4c4d-aae3-a47ba8202a10.jpeg",
      "lyric": "Hello Hello Hello ",
      "audio_url": "https://cdn1.suno.ai/a922f97b-307c-4c4d-aae3-a47ba8202a10.mp3",
      "video_url": "",
      "created_at": "2025-07-10T12:54:35.199Z",
      "model": "chirp-auk",
      "state": "succeeded",
      "style": "",
      "duration": 229.64
    }
  ]
}
```

這樣就使用了高級參數進行生成自定義歌曲，結果與上文類似。

## 控制歌曲時長

默認情況下生成的歌曲時長由模型自行決定，通常在 30 秒到 4 分鐘之間。如果需要更長或更短的成品，可以通過 `duration` 參數指定期望時長，單位是秒，取值為整數，通常建議在 10 到 360 之間。

該參數主要面向自定義模式（`custom` 為 `true`）下的歌曲生成，配合較新的模型（如 `chirp-v5-5`）效果最好。不同模型和操作對時長控制的支持程度不同，若當前組合不支持，該參數可能被忽略或返回錯誤提示，可根據返回結果調整。

對應的 Python 代碼：

```python
import requests

url = "https://api.acedata.cloud/suno/audios"

headers = {
    "accept": "application/json",
    "authorization": "Bearer {token}",
    "content-type": "application/json"
}

payload = {
    "action": "generate",
    "model": "chirp-v5-5",
    "custom": True,
    "title": "Under the City Lights",
    "style": "lo-fi piano",
    "lyric": "[Verse]\nSunrise creepin\nGold on the floor\n[Chorus]\nUnder the city lights\n",
    "duration": 330
}

response = requests.post(url, json=payload, headers=headers)
print(response.text)
```

需要注意的是，請求中的 `duration` 是**期望時長**，而響應 `data` 中每首歌曲的 `duration` 字段是該首歌曲的**實際時長**。兩者名稱相同但含義不同，實際時長會圍繞期望值浮動，不保證精確相等。

如果某個模型或操作不支持時長控制，接口可能忽略該參數或返回錯誤提示，建議先用一次請求確認效果再批量使用。

## Add Insterumental 功能

2025 年 8 月份 suno 新出 Add Insterumental 功能，首先需要上傳一首清唱無配音的歌曲, 讓 suno 幫你配樂，首先可以先到 [Suno Upload API](https://platform.acedata.cloud/documents/suno-upload)上傳一首清唱無配樂的歌曲，對應如下如的操作如下圖所示：

<p><img src="https://cdn.acedata.cloud/fxl914.png" width="500" className="m-auto" /></p>

然後需要記錄上傳後的`audio_id`，具體的結果如下圖所示：

<p><img src="https://cdn.acedata.cloud/47t6wj.png" width="500" className="m-auto" /></p>

最後得到了一个`audio_id`：92254cab-3372-4d9e-bce9-cdcfdbc39070，然後我們還需要填寫如下參數：

- action：內容為 `underpainting`。
- underpainting_start：對上傳的歌曲進行添加伴奏的起始時間，默認值是 0。
- underpainting_end：對上傳的歌曲進行添加伴奏的終點時間，必須小於歌曲的總時長。
- audio_id：上傳的清唱無配音的歌曲 ID。
- style：伴奏的風格，最好是不用歌詞 毕竟是配音。

填寫完畢之後自動生成了代碼如下：

<p><img src="https://cdn.acedata.cloud/8x1ic6.png" width="500" className="m-auto" /></p>

對應的 Python 代碼：

```python
import requests

url = "https://api.acedata.cloud/suno/audios"

headers = {
    "accept": "application/json",
    "authorization": "Bearer {token}",
    "content-type": "application/json"
}

payload = {
    "action": "underpainting",
    "model": "chirp-v4-5",
    "style": "Pop rap, uplifting, magnetic male vocals, piano, synth, electric guitar, driving bass, clear structure",
    "audio_id": "92254cab-3372-4d9e-bce9-cdcfdbc39070",
    "underpainting_end": 120
}

response = requests.post(url, json=payload, headers=headers)
print(response.text)
```

點擊運行，可以發現會得到一個結果，如下：
```json
{
  "success": true,
  "task_id": "822d2e14-c535-4d48-a4e5-1b6ab00b04a7",
  "trace_id": "257eac2c-8e4f-44d0-8454-5e215111eefa",
  "data": [
    {
      "id": "2788cd21-bd84-422d-beb5-859c60fbf5b6",
      "title": "",
      "image_url": "https://cdn2.suno.ai/image_2788cd21-bd84-422d-beb5-859c60fbf5b6.jpeg",
      "lyric": "",
      "audio_url": "https://cdn1.suno.ai/2788cd21-bd84-422d-beb5-859c60fbf5b6.mp3",
      "video_url": "",
      "created_at": "2025-08-27T15:25:42.548Z",
      "model": "chirp-v4",
      "state": "succeeded",
      "style": "流行饶舌，振奮人心，磁性的男性聲音，鋼琴，合成器，電吉他，強勁的低音，清晰的結構",
      "duration": 10.16
    },
    {
      "id": "a4bb7220-e971-4cbf-a626-b86c648bcf55",
      "title": "",
      "image_url": "https://cdn2.suno.ai/image_a4bb7220-e971-4cbf-a626-b86c648bcf55.jpeg",
      "lyric": "",
      "audio_url": "https://cdn1.suno.ai/a4bb7220-e971-4cbf-a626-b86c648bcf55.mp3",
      "video_url": "",
      "created_at": "2025-08-27T15:25:42.548Z",
      "model": "chirp-v4",
      "state": "succeeded",
      "style": "流行饶舌，振奮人心，磁性的男性聲音，鋼琴，合成器，電吉他，強勁的低音，清晰的結構",
      "duration": 2.52
    }
  ]
}
```

這樣就完成了對上傳的清唱無配音歌曲進行配樂的操作，結果與上文類似。

## Add Vocals 功能

2025 年 8 月份 suno 新出 Add Vocals 功能，首先需要上傳一首純音樂，讓 suno 填詞、出人聲歌唱，首先可以先到 [Suno Upload API](https://platform.acedata.cloud/documents/suno-upload)上傳一首清唱無配樂的歌曲，對應如下如的操作如下圖所示：

<p><img src="https://cdn.acedata.cloud/fxl914.png" width="500" className="m-auto" /></p>

然後需要記錄上傳後的`audio_id`，具體的結果如下圖所示：

<p><img src="https://cdn.acedata.cloud/47t6wj.png" width="500" className="m-auto" /></p>

最後得到了個`audio_id`：92254cab-3372-4d9e-bce9-cdcfdbc39070，然後我們還需要填寫如下參數：

- action：內容為 `overpainting`。
- overpainting_start：對上傳的歌曲進行添加人聲的起始時間，默認值是 0。
- overpainting_end：對上傳的歌曲進行添加人聲的終點時間，必須小於歌曲的總時長。
- audio_id：上傳的清唱無配音的歌曲 ID。
- custom：該模式下必須使用自定義模式填入歌詞。
- lyric：自定義模式下填寫的歌詞。
- style：伴奏的風格。

填寫完畢之後自動生成了代碼如下：

<p><img src="https://cdn.acedata.cloud/a4pbes.png" width="500" className="m-auto" /></p>

對應的 Python 代碼：

```python
import requests

url = "https://api.acedata.cloud/suno/audios"

headers = {
    "accept": "application/json",
    "authorization": "Bearer {token}",
    "content-type": "application/json"
}

payload = {
    "action": "overpainting",
    "model": "chirp-v4-5",
    "lyric": "Yea your were the best I could get \\nBut I knew that it couldn’t last \\nStayed down since we were friends \\nHad to leave those thoughts in the past \\nMade like 40k just last week \\nOn top of the 20 with my babe\\ndon’t care for what niggas say",
    "custom": True,
    "audio_id": "92254cab-3372-4d9e-bce9-cdcfdbc39070",
    "overpainting_end": 120
}

response = requests.post(url, json=payload, headers=headers)
print(response.text)
```

點擊運行，可以發現會得到一個結果，如下：

```json
{
  "success": true,
  "task_id": "31bb250e-6614-49ec-ac85-631f224daeba",
  "trace_id": "efe8e7f3-9a65-4f13-a9f8-51e8478899df",
  "data": [
    {
      "id": "a597f945-64df-4722-a631-d436450832bd",
      "title": "",
      "image_url": "https://cdn2.suno.ai/image_a597f945-64df-4722-a631-d436450832bd.jpeg",
      "lyric": "Yea your were the best I could get \\nBut I knew that it couldn’t last \\nStayed down since we were friends \\nHad to leave those thoughts in the past \\nMade like 40k just last week \\nOn top of the 20 with my babe\\ndon’t care for what niggas say",
      "audio_url": "https://cdn1.suno.ai/a597f945-64df-4722-a631-d436450832bd.mp3",
      "video_url": "",
      "created_at": "2025-08-27T15:33:51.550Z",
      "model": "chirp-v4",
      "state": "succeeded",
      "style": "",
      "duration": 105.32
    },
    {
      "id": "b41a8b91-3d88-4ebd-a6cf-732764b24954",
      "title": "",
      "image_url": "https://cdn2.suno.ai/image_b41a8b91-3d88-4ebd-a6cf-732764b24954.jpeg",
      "lyric": "Yea your were the best I could get \\nBut I knew that it couldn’t last \\nStayed down since we were friends \\nHad to leave those thoughts in the past \\nMade like 40k just last week \\nOn top of the 20 with my babe\\ndon’t care for what niggas say",
      "audio_url": "https://cdn1.suno.ai/b41a8b91-3d88-4ebd-a6cf-732764b24954.mp3",
      "video_url": "",
      "created_at": "2025-08-27T15:33:51.550Z",
      "model": "chirp-v4",
      "state": "succeeded",
      "style": "",
      "duration": 148.16
    }
  ]
}
```

這樣就完成了對上傳的清唱無配音歌曲進行配人聲的操作，結果與上文類似。

## Remaster 功能

2025 年 12 月份 suno 新出 Remaster 功能，該功能可以重新生成歌曲，不可跨帳號，然後我們還需要填寫如下參數：

- action：內容為 `remaster`。
- audio_id：需要重新生成的歌曲 ID。
- model：僅支持 v4.5+、v5。
- variation_category：僅在 v5 以上版本支持，而且只有 3 個值 high normal subtle。

填寫完畢之後自動生成了代碼如下：

<p><img src="https://cdn.acedata.cloud/7h4zmw.png" width="500" className="m-auto" /></p>

對應的 Python 代碼：
```
```python
import requests

url = "https://api.acedata.cloud/suno/audios"

headers = {
    "accept": "application/json",
    "authorization": "Bearer {token}",
    "content-type": "application/json"
}

payload = {
    "action": "remaster",
    "variation_category": "high",
    "audio_id": "21fd46d4-45c3-4826-bec9-3f3df667902e"
}

response = requests.post(url, json=payload, headers=headers)
print(response.text)
```

點擊運行，可以發現會得到一個結果，如下：

```json
{
  "success": true,
  "task_id": "791ee74c-9363-4352-afe7-babb19b89899",
  "trace_id": "77acda57-c3d4-4e11-a506-4f50e0609916",
  "data": [
    {
      "id": "b0515cdf-9cb5-46cd-b0fe-10a239dc9274",
      "title": "Navidad en costura  (Remastered)",
      "image_url": "https://cdn2.suno.ai/image_b0515cdf-9cb5-46cd-b0fe-10a239dc9274.jpeg",
      "image_large_url": "https://cdn2.suno.ai/image_large_b0515cdf-9cb5-46cd-b0fe-10a239dc9274.jpeg",
      "lyric": "En Teror las clases siguen,\nni en Navidad hay parón;\ncose el grupo entre villancicos\ny un buen trocito de turrón.\nLa Popular abre sus puertas,\ny el taller suena mejor;\nhilo, aguja y canto alegre\nlo pasaremos mejor\nSeguimos en las costuras,\ncon música y diversión;\nlos alumnos comeremos\nGolosinas un montón ",
      "audio_url": "https://cdn1.suno.ai/b0515cdf-9cb5-46cd-b0fe-10a239dc9274.mp3",
      "video_url": "",
      "created_at": "2025-12-04T13:09:59.936Z",
      "model": "chirp-v4",
      "state": "succeeded",
      "style": "Villancico",
      "duration": 32.2
    },
    {
      "id": "06edab94-a4f9-4c0c-abac-a2e8a97c76a8",
      "title": "Navidad en costura  (Remastered)",
      "image_url": "https://cdn2.suno.ai/image_06edab94-a4f9-4c0c-abac-a2e8a97c76a8.jpeg",
      "image_large_url": "https://cdn2.suno.ai/image_large_06edab94-a4f9-4c0c-abac-a2e8a97c76a8.jpeg",
      "lyric": "En Teror las clases siguen,\nni en Navidad hay parón;\ncose el grupo entre villancicos\ny un buen trocito de turrón.\nLa Popular abre sus puertas,\ny el taller suena mejor;\nhilo, aguja y canto alegre\nlo pasaremos mejor\nSeguimos en las costuras,\ncon música y diversión;\nlos alumnos comeremos\nGolosinas un montón ",
      "audio_url": "https://cdn1.suno.ai/06edab94-a4f9-4c0c-abac-a2e8a97c76a8.mp3",
      "video_url": "",
      "created_at": "2025-12-04T13:09:59.936Z",
      "model": "chirp-v4",
      "state": "succeeded",
      "style": "Villancico",
      "duration": 32.2
    }
  ]
}
```

這樣就完成了對已生成歌曲重新生成的操作，結果與上文類似。

## Mashup 混曲生成功能

2025 年 12 月份 suno 新出 Mashup 功能，該功能可以根據兩首參考歌曲生成歌曲，然後我們還需要填寫如下參數：

- action：內容為 `mashup`。
- mashup_audio_ids：兩首參考歌曲的 ID。

填寫完畢之後自動生成了代碼如下：

<p><img src="https://cdn.acedata.cloud/8mo82l.png" width="500" class="m-auto"></p>

對應的 Python 代碼：

```python
import requests

url = "https://api.acedata.cloud/suno/audios"

headers = {
    "accept": "application/json",
    "authorization": "Bearer {token}",
    "content-type": "application/json"
}

payload = {
    "action": "mashup",
    "lyric": "Sambuy come back  \\nBluespawn, greenspawn are making you a spawn  \\n\\nSambuy come  \\nSambuy come back  \\nBluespawn, greenspawn are making you a spawn  \\n\\nSambuy come  \\nSambuy come back  \\nBluespawn, greenspawn are making you a spawn  \\n\\nSambuy come  \\nSambuy come back  \\nSambuy come, Sambuy come back  \\nBluespawn, greenspawn are making you a spawn  \\n\\nSambuy come  \\nSambuy come back  \\nBluespawn, greenspawn are making you a spawn  \\nSambuy come back",
    "model": "chirp-v4-5",
    "custom": True,
    "instrumental": False,
    "mashup_audio_ids": ["9f102969-0024-479e-b38f-c0c5db21383d","6aebb715-701c-433f-b976-a7ff5cf16255"]
}

response = requests.post(url, json=payload, headers=headers)
print(response.text)
```

點擊運行，可以發現會得到一個結果，如下：
```json
{
  "success": true,
  "task_id": "3fe070eb-2ab1-4424-909c-acfe3ca761af",
  "trace_id": "2b220382-7a84-4853-ae43-05cdcad233a9",
  "data": [
    {
      "id": "5ff751dc-0e72-4de9-a54b-2cad50984b47",
      "title": "",
      "image_url": "https://cdn2.suno.ai/image_5ff751dc-0e72-4de9-a54b-2cad50984b47.jpeg",
      "image_large_url": "https://cdn2.suno.ai/image_large_5ff751dc-0e72-4de9-a54b-2cad50984b47.jpeg",
      "lyric": "Sambuy come back  \\nBluespawn, greenspawn are making you a spawn  \\n\\nSambuy come  \\nSambuy come back  \\nBluespawn, greenspawn are making you a spawn  \\n\\nSambuy come  \\nSambuy come back  \\nBluespawn, greenspawn are making you a spawn  \\n\\nSambuy come  \\nSambuy come back  \\nSambuy come, Sambuy come back  \\nBluespawn, greenspawn are making you a spawn  \\n\\nSambuy come  \\nSambuy come back  \\nBluespawn, greenspawn are making you a spawn  \\nSambuy come back",
      "audio_url": "https://cdn1.suno.ai/5ff751dc-0e72-4de9-a54b-2cad50984b47.mp3",
      "video_url": "",
      "created_at": "2026-01-25T15:13:30.181Z",
      "model": "chirp-v4-5",
      "state": "succeeded",
      "style": "",
      "duration": 219.08
    },
    {
      "id": "19c515c4-d7b3-4a17-8ab0-dd1ebd4861b8",
      "title": "",
      "image_url": "https://cdn2.suno.ai/image_19c515c4-d7b3-4a17-8ab0-dd1ebd4861b8.jpeg",
      "image_large_url": "https://cdn2.suno.ai/image_large_19c515c4-d7b3-4a17-8ab0-dd1ebd4861b8.jpeg",
      "lyric": "Sambuy come back  \\nBluespawn, greenspawn are making you a spawn  \\n\\nSambuy come  \\nSambuy come back  \\nBluespawn, greenspawn are making you a spawn  \\n\\nSambuy come  \\nSambuy come back  \\nBluespawn, greenspawn are making you a spawn  \\n\\nSambuy come  \\nSambuy come back  \\nSambuy come, Sambuy come back  \\nBluespawn, greenspawn are making you a spawn  \\n\\nSambuy come  \\nSambuy come back  \\nBluespawn, greenspawn are making you a spawn  \\nSambuy come back",
      "audio_url": "https://cdn1.suno.ai/19c515c4-d7b3-4a17-8ab0-dd1ebd4861b8.mp3",
      "video_url": "",
      "created_at": "2026-01-25T15:13:30.181Z",
      "model": "chirp-v4-5",
      "state": "succeeded",
      "style": "",
      "duration": 163.12
    }
  ]
}
```

這樣就完成了對參考歌曲混曲生成的操作，結果與上文類似。

## Samples 取樣生成歌曲

2025 年 12 月份 suno 新推出 Samples 功能，該功能可以根據兩首參考歌曲生成歌曲，然後我們還需要填寫如下參數：

- action：內容為 `samples`。
- samples_start：取樣開始時間。
- samples_end：取樣結束時間。
- audio_id：需要取樣的參考歌曲的 ID。

填寫完畢之後自動生成了程式碼如下：

<p><img src="https://cdn.acedata.cloud/vkzumz.png" width="500" class="m-auto"></p>

對應的 Python 程式碼：

```python
import requests

url = "https://api.acedata.cloud/suno/audios"

headers = {
    "accept": "application/json",
    "authorization": "Bearer {token}",
    "content-type": "application/json"
}

payload = {
    "action": "samples",
    "model": "chirp-v5",
    "lyric": "[Verse 1]\\nPhone lit up\\nHeadline in my hand\\nFeels made up\\nStill says “you won’t understand”\\nYour name\\nMy name\\nSide by side in the scroll\\nCold black text\\nOn a story I used to hold\\n\\n[Chorus]\\nYou’re breaking news\\nAnd I’m just breaking\\nFront-page truth\\nHeart still shaking\\nEverybody reads\\nWhat we already knew\\nYou’re a story now\\nAnd I’m the one you broke it to\\n\\n[Verse 2]\\nNeighbors talk\\nThrough a half-closed door\\nCoffee cools\\nOn a cracked old floor\\nYour suitcase snaps\\nLike a camera flash\\nOne last quote\\nThen you cut to black\\n\\n[Chorus]\\nYou’re breaking news\\nAnd I’m just breaking\\nFront-page truth\\nHeart still shaking\\nEverybody reads\\nWhat we already knew\\nYou’re a story now\\nAnd I’m the one you broke it to\\n\\n[Bridge]\\nIs there a line\\nWhere we rewind\\nOr just a feed\\nThat leaves us behind\\nTell me\\nWho gets\\nThe final view\\nWhen I stop trending\\nWith you\\n\\n[Chorus]\\nYou’re breaking news\\nAnd I’m just breaking\\nFront-page truth\\nHeart still shaking\\nEverybody reads\\nWhat we already knew\\nYou’re a story now\\nAnd I’m the one you broke it to (yeah)",
    "custom": True,
    "instrumental": False,
    "audio_id": "0fa07665-6b8e-4a8b-8bd3-7e0cfcdada88",
    "samples_end": 102.16,
    "samples_start": 59.88
}

response = requests.post(url, json=payload, headers=headers)
print(response.text)
```

點擊執行，可以發現會得到一個結果，如下：
```json
{
    "success": true,
    "task_id": "12135a45-6384-4683-9bb1-64f19933915a",
    "trace_id": "683e559d-340e-4c72-8fae-e683442ac7e9",
    "data": [
        {
            "id": "9a0b680f-a9ea-4a36-8695-8ea777ab6ee7",
            "title": "風中的口哨",
            "image_url": "https://cdn2.suno.ai/image_9a0b680f-a9ea-4a36-8695-8ea777ab6ee7.jpeg",
            "image_large_url": "https://cdn2.suno.ai/image_large_9a0b680f-a9ea-4a36-8695-8ea777ab6ee7.jpeg",
            "lyric": "[Verse 1]\n電話亮起\n手中握著標題\n感覺像是編造的\n仍然說著「你不會理解」\n你的名字\n我的名字\n並排在滾動中\n冷冷的黑色文字\n在我曾經握著的故事上\n[Chorus]\n你是突發新聞\n而我只是破碎\n頭版真相\n心仍在顫抖\n每個人都在閱讀\n我們已經知道的\n你現在是一個故事\n而我是你告訴的那個人\n[Verse 2]\n鄰居在談話\n透過半關的門\n咖啡冷卻\n在破舊的地板上\n你的行李箱啪的一聲\n像相機的閃光\n最後一句話\n然後你切到黑暗\n[Chorus]\n你是突發新聞\n而我只是破碎\n頭版真相\n心仍在顫抖\n每個人都在閱讀\n我們已經知道的\n你現在是一個故事\n而我是你告訴的那個人\n[Bridge]\n是否有一條線\n讓我們倒帶\n還是只是個餵送\n讓我們被拋在後面\n告訴我\n誰得到\n最後的視角\n當我不再趨勢\n和你在一起\n[Chorus]\n你是突發新聞\n而我只是破碎\n頭版真相\n心仍在顫抖\n每個人都在閱讀\n我們已經知道的\n你現在是一個故事\n而我是你告訴的那個人（是的）",
            "audio_url": "https://cdn1.suno.ai/9a0b680f-a9ea-4a36-8695-8ea777ab6ee7.mp3",
            "video_url": "",
            "created_at": "2026-01-31T14:34:45.043Z",
            "model": "chirp-v5",
            "state": "succeeded",
            "style": "帶有一絲樂觀的原聲，民謠流行，女性聲音",
            "duration": 176.92
        },
        {
            "id": "66473dee-3aaf-43b2-80fd-76568b3abbb1",
            "title": "風中的口哨",
            "image_url": "https://cdn2.suno.ai/image_66473dee-3aaf-43b2-80fd-76568b3abbb1.jpeg",
            "image_large_url": "https://cdn2.suno.ai/image_large_66473dee-3aaf-43b2-80fd-76568b3abbb1.jpeg",
            "lyric": "[Verse 1]\n電話亮起\n手中握著標題\n感覺像是編造的\n仍然說著「你不會理解」\n你的名字\n我的名字\n並排在滾動中\n冷冷的黑色文字\n在我曾經握著的故事上\n[Chorus]\n你是突發新聞\n而我只是破碎\n頭版真相\n心仍在顫抖\n每個人都在閱讀\n我們已經知道的\n你現在是一個故事\n而我是你告訴的那個人\n[Verse 2]\n鄰居在談話\n透過半關的門\n咖啡冷卻\n在破舊的地板上\n你的行李箱啪的一聲\n像相機的閃光\n最後一句話\n然後你切到黑暗\n[Chorus]\n你是突發新聞\n而我只是破碎\n頭版真相\n心仍在顫抖\n每個人都在閱讀\n我們已經知道的\n你現在是一個故事\n而我是你告訴的那個人\n[Bridge]\n是否有一條線\n讓我們倒帶\n還是只是個餵送\n讓我們被拋在後面\n告訴我\n誰得到\n最後的視角\n當我不再趨勢\n和你在一起\n[Chorus]\n你是突發新聞\n而我只是破碎\n頭版真相\n心仍在顫抖\n每個人都在閱讀\n我們已經知道的\n你現在是一個故事\n而我是你告訴的那個人（是的）",
            "audio_url": "https://cdn1.suno.ai/66473dee-3aaf-43b2-80fd-76568b3abbb1.mp3",
            "video_url": "",
            "created_at": "2026-01-31T14:34:45.043Z",
            "model": "chirp-v5",
            "state": "succeeded",
            "style": "帶有一絲樂觀的原聲，民謠流行，女性聲音",
            "duration": 177.48
        }
    ]
}
```

這樣就完成了取樣生成歌曲的操作，結果與上文類似。

## Inspo 靈感創作功能

Suno 新出的 Inspo 靈感創作功能，可以根據 1 到 4 段參考音頻作為靈感來源，生成全新的音樂。與翻版（cover）不同，Inspo 並不會復刻原曲，而是從參考音頻中提取風格靈感，再結合提示詞、風格標籤等生成新作品。使用時需要填寫如下參數：

- action：內容為 `inspo`。
- audio_urls：參考音頻的 URL 列表，需提供 1 到 4 個公開可訪問的音頻地址。
- model：使用的模型，例如 `chirp-v5`。
- prompt：歌詞或創作提示詞（可選）。
- tags：音樂風格標籤，例如 `acoustic, folk, warm`（可選）。
- title：歌曲標題（可選）。
- audio_weight：參考音頻對生成結果的影響權重，取值範圍 0 到 1（可選）。

> 說明：參考音頻需為公開可訪問的音頻文件。若參考音頻與平台曲庫中的已知錄音完全匹配，Suno 可能會因版權校驗而拒絕生成，建議使用自有或由 Suno 生成的音頻作為靈感來源。

對應的 Python 代碼：

```python
import requests

url = "https://api.acedata.cloud/suno/audios"

headers = {
    "accept": "application/json",
    "authorization": "Bearer {token}",
    "content-type": "application/json"
}

payload = {
    "action": "inspo",
    "model": "chirp-v5",
    "audio_urls": [
        "https://cdn1.suno.ai/935dd2f5-ddef-43cf-8e68-5a262425694d.mp3"
    ],
    "prompt": "溫暖的原聲民謠，柔和的聲音",
    "tags": "acoustic, folk, warm",
    "title": "靈感示範",
    "audio_weight": 0.6
}

response = requests.post(url, json=payload, headers=headers)
print(response.text)
```

點擊運行，可以發現會得到一個結果，如下：
```json
{
    "success": true,
    "task_id": "725e6b41-78d0-4adf-856c-05e81098c029",
    "trace_id": "8c2f0b3e-2f6a-4738-8b1d-c58068ca3dab",
    "data": [
        {
            "id": "20ca5628-86e0-4c32-8ae4-9f0c481e45f3",
            "title": "靈感示範",
            "image_url": "https://cdn2.suno.ai/image_20ca5628-86e0-4c32-8ae4-9f0c481e45f3.jpeg",
            "lyric": "",
            "audio_url": "https://cdn1.suno.ai/20ca5628-86e0-4c32-8ae4-9f0c481e45f3.mp3",
            "video_url": "",
            "created_at": "2026-06-18T13:01:53.910Z",
            "model": "chirp-v5",
            "state": "成功",
            "style": "原聲, 民謠, 溫暖",
            "duration": 36.92
        },
        {
            "id": "8744a796-9961-45af-868d-4f3bc1c44257",
            "title": "靈感示範",
            "image_url": "https://cdn2.suno.ai/image_8744a796-9961-45af-868d-4f3bc1c44257.jpeg",
            "lyric": "",
            "audio_url": "https://cdn1.suno.ai/8744a796-9961-45af-868d-4f3bc1c44257.mp3",
            "video_url": "",
            "created_at": "2026-06-18T13:01:53.910Z",
            "model": "chirp-v5",
            "state": "成功",
            "style": "原聲, 民謠, 溫暖",
            "duration": 84.16
        }
    ]
}
```

這樣就完成了靈感創作的操作，返回結果與普通生成歌曲一致。

## 異步回調

由於 Suno 生成音樂的時間相對較長，大約需要 1-2 分鐘，如果 API 長時間無響應，HTTP 請求會一直保持連接，導致額外的系統資源消耗，所以本 API 也提供了異步回調的支持。

整體流程是：客戶端發起請求的時候，額外指定一個 `callback_url` 字段，客戶端發起 API 請求之後，API 會立馬返回一個結果，包含一個 `task_id` 的字段信息，代表當前的任務 ID。當任務完成之後，生成音樂的結果會通過 POST JSON 的形式發送到客戶端指定的 `callback_url`，其中也包括了 `task_id` 字段，這樣任務結果就可以通過 ID 關聯起來了。

下面我們通過示例來了解下具體怎樣操作。

首先，Webhook 回調是一個可以接收 HTTP 請求的服務，開發者應該替換為自己搭建的 HTTP 伺服器的 URL。此處為了方便演示，使用一個公開的 Webhook 樣例網站 https://webhook.site/，打開該網站即可得到一個 Webhook URL，如圖所示：

![](https://cdn.acedata.cloud/fwfqin.png)

將此 URL 複製下來，就可以作為 Webhook 來使用，此處的樣例為 https://webhook.site/03e60575-3d96-4132-b681-b713d78116e2。

接下來，我們可以設置字段 `callback_url` 為上述 Webhook URL，同時填入 `prompt`，如圖所示：

![](https://cdn.acedata.cloud/x8xql1.png)

點擊運行，可以發現會立即得到一個結果，如下：

```
{
  "task_id": "44472ab8-783b-4054-b861-5bf14e462f60"
}
```

稍等片刻，我們可以在 https://webhook.site/03e60575-3d96-4132-b681-b713d78116e2 上觀察到生成歌曲的結果，如圖所示：

![](https://cdn.acedata.cloud/f9kosb.png)

內容如下：

```json
{
  "success": true,
  "task_id": "44472ab8-783b-4054-b861-5bf14e462f60",
  "data": [
    {
      "id": "da4324e5-84b2-484b-b0e9-dd261381c594",
      "title": "冬季低語",
      "image_url": "https://cdn1.suno.ai/image_da4324e5-84b2-484b-b0e9-dd261381c594.png",
      "lyric": "[Verse]\n雪花輕輕從天空飄落\n孩子們在旁邊咯咯笑\n火焰噼啪作響\n舒適而溫暖\n聖誕精神開始蔓延\n[Verse 2]\n閃爍的燈光\n令人驚嘆的景象\n襪子懸掛\n等待被填滿金子\n用愛包裝的禮物\n堆得高高的\n空氣中充滿興奮\n你無法否認\n[Chorus]\n冬季在風中低語\n帶來喜悅和愛\n讓我們慶祝這個季節\n與我們所思念的人",
      "audio_url": "https://cdn1.suno.ai/da4324e5-84b2-484b-b0e9-dd261381c594.mp3",
      "video_url": "https://cdn1.suno.ai/da4324e5-84b2-484b-b0e9-dd261381c594.mp4",
      "created_at": "2024-05-11T07:33:05.430Z",
      "model": "chirp-v3",
      "prompt": "一首聖誕歌曲",
      "style": "流行"
    },
    {
      "id": "b878a87b-a0db-4046-8ccd-ecd2fb3d4372",
      "title": "冬季低語",
      "image_url": "https://cdn1.suno.ai/image_b878a87b-a0db-4046-8ccd-ecd2fb3d4372.png",
      "lyric": "[Verse]\n雪花輕輕從天空飄落\n孩子們在旁邊咯咯笑\n火焰噼啪作響\n舒適而溫暖\n聖誕精神開始蔓延\n[Verse 2]\n閃爍的燈光\n令人驚嘆的景象\n襪子懸掛\n等待被填滿金子\n用愛包裝的禮物\n堆得高高的\n空氣中充滿興奮\n你無法否認\n[Chorus]\n冬季在風中低語\n帶來喜悅和愛\n讓我們慶祝這個季節\n與我們所思念的人",
      "audio_url": "https://cdn1.suno.ai/b878a87b-a0db-4046-8ccd-ecd2fb3d4372.mp3",
      "video_url": "https://cdn1.suno.ai/b878a87b-a0db-4046-8ccd-ecd2fb3d4372.mp4",
      "created_at": "2024-05-11T07:33:05.430Z",
      "model": "chirp-v3",
      "prompt": "一首聖誕歌曲",
      "style": "流行"
    }
  ]
}
```

可以看到結果中有一個 `task_id` 字段，其他的字段都和上文類似，通過該字段即可實現任務的關聯。

當然我們也可以通過流式調用來獲取結果，我們只需要將請求頭裡面的`accept`的值設置為`application/x-ndjson`即可，下面用一個示例輸入作為示範：

<p><img src="https://cdn.acedata.cloud/vgffvk.png" width="500" class="m-auto"></p>

等待過程中我們可以得到以下輸出：

```json
```
{"success":true,"task_id":"1af4b454-ce84-4512-a0a2-de3f8574ecd8","data":[{"id":"1f610752-f426-4fd5-89a8-ba2ad0370881","title":"雪花與槲寄生","lyric":"[主歌]\n燈飾又一次纏繞在樹上\n貓咪打翻了花環\n餅乾也在烤箱裡燒焦\n但我仍然咧嘴笑著\n[前副歌]\n雪花像天空中的閃粉般飄落\n我有種感覺今年會飛逝而過\n[副歌]\n雪花與槲寄生\n無論我走到哪裡\n我知道\n愛就是禮物\n那道光芒\n雪花與槲寄生\n[第二節主歌]\n奶奶唱歌有點走音\n孩子們笑得太大聲\n狗狗偷走了一根火雞腿\n混亂就是聖誕節的人群\n[前副歌]\n但火焰劈啪作響\n溫暖又明亮\n而今晚我的心是一根蠟燭\n[副歌]\n雪花與槲寄生\n無論我走到哪裡\n我知道\n愛就是禮物\n那道光芒\n雪花與槲寄生","audio_url":"","video_url":"","created_at":"2025-12-13T11:29:25.101Z","model":"chirp-v5","state":"pending","prompt":"一首關於聖誕節的歌曲","style":"振奮人心、管弦樂風格，搭配鈴鐺和木吉他"},{"id":"d1a3109d-799b-401e-b032-4b501bcf26f3","title":"雪花與槲寄生","lyric":"[主歌]\n燈飾又一次纏繞在樹上\n貓咪打翻了花環\n餅乾也在烤箱裡燒焦\n但我仍然咧嘴笑著\n[前副歌]\n雪花像天空中的閃粉般飄落\n我有種感覺今年會飛逝而過\n[副歌]\n雪花與槲寄生\n無論我走到哪裡\n我知道\n愛就是禮物\n那道光芒\n雪花與槲寄生\n[第二節主歌]\n奶奶唱歌有點走音\n孩子們笑得太大聲\n狗狗偷走了一根火雞腿\n混亂就是聖誕節的人群\n[前副歌]\n但火焰劈啪作響\n溫暖又明亮\n而今晚我的心是一根蠟燭\n[副歌]\n雪花與槲寄生\n無論我走到哪裡\n我知道\n愛就是禮物\n那道光芒\n雪花與槲寄生","audio_url":"","video_url":"","created_at":"2025-12-13T11:29:25.101Z","model":"chirp-v5","state":"pending","prompt":"一首關於聖誕節的歌曲","style":"振奮人心、管弦樂風格，搭配鈴鐺和木吉他"}]} {"success":true,"task_id":"1af4b454-ce84-4512-a0a2-de3f8574ecd8","data":[{"id":"1f610752-f426-4fd5-89a8-ba2ad0370881","title":"雪花與槲寄生","image_url":"https://cdn2.suno.ai/image_1f610752-f426-4fd5-89a8-ba2ad0370881.jpeg","image_large_url":"https://cdn2.suno.ai/image_large_1f610752-f426-4fd5-89a8-ba2ad0370881.jpeg","lyric":"[主歌]\n燈飾又一次纏繞在樹上\n貓咪打翻了花環\n餅乾也在烤箱裡燒焦\n但我仍然咧嘴笑著\n[前副歌]\n雪花像天空中的閃粉般飄落\n我有種感覺今年會飛逝而過\n[副歌]\n雪花與槲寄生\n無論我走到哪裡\n我知道\n愛就是禮物\n那道光芒\n雪花與槲寄生\n[第二節主歌]\n奶奶唱歌有點走音\n孩子們笑得太大聲\n狗狗偷走了一根火雞腿\n混亂就是聖誕節的人群\n[前副歌]\n但火焰劈啪作響\n溫暖又明亮\n而今晚我的心是一根蠟燭\n[副歌]\n雪花與槲寄生\n無論我走到哪裡\n我知道\n愛就是禮物\n那道光芒\n雪花與槲寄生","audio_url":"https://audiopipe.suno.ai/?item_id=1f610752-f426-4fd5-89a8-ba2ad0370881","video_url":"","created_at":"2025-12-13T11:29:25.101Z","model":"chirp-v5","state":"running","prompt":"一首關於聖誕節的歌曲","style":"振奮人心、管弦樂風格，搭配鈴鐺和木吉他"},{"id":"d1a3109d-799b-401e-b032-4b501bcf26f3","title":"雪花與槲寄生","image_url":"https://cdn2.suno.ai/image_d1a3109d-799b-401e-b032-4b501bcf26f3.jpeg","image_large_url":"https://cdn2.suno.ai/image_large_d1a3109d-799b-401e-b032-4b501bcf26f3.jpeg","lyric":"[主歌]\n燈飾又一次纏繞在樹上\n貓咪打翻了花環\n餅乾也在烤箱裡燒焦\n但我仍然咧嘴笑著\n[前副歌]\n雪花像天空中的閃粉般飄落\n我有種感覺今年會飛逝而過\n[副歌]\n雪花與槲寄生\n無論我走到哪裡\n我知道\n愛就是禮物\n那道光芒\n雪花與槲寄生\n[第二節主歌]\n奶奶唱歌有點走音\n孩子們笑得太大聲\n狗狗偷走了一根火雞腿\n混亂就是聖誕節的人群\n[前副歌]\n但火焰劈啪作響\n溫暖又明亮\n而今晚我的心是一根蠟燭\n[副歌]\n雪花與槲寄生\n無論我走到哪裡\n我知道\n愛就是禮物\n那道光芒\n雪花與槲寄生","audio_url":"https://audiopipe.suno.ai/?item_id=d1a3109d-799b-401e-b032-4b501bcf26f3","video_url":"","created_at":"2025-12-13T11:29:25.101Z","model":"chirp-v5","state":"running","prompt":"一首關於聖誕節的歌曲","style":"振奮人心、管弦樂風格，搭配鈴鐺和木吉他"}]}
{"success":true,"task_id":"1af4b454-ce84-4512-a0a2-de3f8574ecd8","data":[{"id":"1f610752-f426-4fd5-89a8-ba2ad0370881","title":"雪花與槲寄生","image_url":"https://cdn2.suno.ai/image_1f610752-f426-4fd5-89a8-ba2ad0370881.jpeg","image_large_url":"https://cdn2.suno.ai/image_large_1f610752-f426-4fd5-89a8-ba2ad0370881.jpeg","lyric":"[主歌]\n燈飾又纏繞在樹上\n貓咪打翻了花環\n餅乾也在烤箱裡燒焦了\n但我仍然咬著牙微笑\n[前副歌]\n雪花像天空中的閃粉般飄落\n我有種感覺今年會飛快流逝\n[副歌]\n雪花與槲寄生\n無論我走到哪裡\n我知道\n愛就是禮物\n那道光芒\n雪花與槲寄生\n[第二主歌]\n奶奶唱歌有點走音\n孩子們笑得太大聲\n狗狗偷走了一根火雞腿\n混亂成了聖誕節的人群\n[前副歌]\n但火焰劈啪作響\n溫暖又明亮\n而我的心今晚像一根蠟燭\n[副歌]\n雪花與槲寄生\n無論我走到哪裡\n我知道\n愛就是禮物\n那道光芒\n雪花與槲寄生","audio_url":"https://cdn1.suno.ai/1f610752-f426-4fd5-89a8-ba2ad0370881.mp3","video_url":"","created_at":"2025-12-13T11:29:25.101Z","model":"chirp-v5","state":"succeeded","prompt":"一首關於聖誕節的歌曲","style":"振奮人心、帶有鐘聲和木吉他的管弦樂風格","duration":129.92},{"id":"d1a3109d-799b-401e-b032-4b501bcf26f3","title":"雪花與槲寄生","image_url":"https://cdn2.suno.ai/image_d1a3109d-799b-401e-b032-4b501bcf26f3.jpeg","image_large_url":"https://cdn2.suno.ai/image_large_d1a3109d-799b-401e-b032-4b501bcf26f3.jpeg","lyric":"[主歌]\n燈飾又纏繞在樹上\n貓咪打翻了花環\n餅乾也在烤箱裡燒焦了\n但我仍然咬著牙微笑\n[前副歌]\n雪花像天空中的閃粉般飄落\n我有種感覺今年會飛快流逝\n[副歌]\n雪花與槲寄生\n無論我走到哪裡\n我知道\n愛就是禮物\n那道光芒\n雪花與槲寄生\n[第二主歌]\n奶奶唱歌有點走音\n孩子們笑得太大聲\n狗狗偷走了一根火雞腿\n混亂成了聖誕節的人群\n[前副歌]\n但火焰劈啪作響\n溫暖又明亮\n而我的心今晚像一根蠟燭\n[副歌]\n雪花與槲寄生\n無論我走到哪裡\n我知道\n愛就是禮物\n那道光芒\n雪花與槲寄生","audio_url":"https://audiopipe.suno.ai/?item_id=d1a3109d-799b-401e-b032-4b501bcf26f3","video_url":"","created_at":"2025-12-13T11:29:25.101Z","model":"chirp-v5","state":"執行中","prompt":"一首關於聖誕節的歌曲","style":"振奮人心、帶有鐘聲和木吉他的管弦樂風格"}]} {"success":true,"task_id":"1af4b454-ce84-4512-a0a2-de3f8574ecd8","data":[{"id":"1f610752-f426-4fd5-89a8-ba2ad0370881","title":"雪花與槲寄生","image_url":"https://cdn2.suno.ai/image_1f610752-f426-4fd5-89a8-ba2ad0370881.jpeg","image_large_url":"https://cdn2.suno.ai/image_large_1f610752-f426-4fd5-89a8-ba2ad0370881.jpeg","lyric":"[主歌]\n燈飾又纏繞在樹上\n貓咪打翻了花環\n餅乾也在烤箱裡燒焦了\n但我仍然咬著牙微笑\n[前副歌]\n雪花像天空中的閃粉般飄落\n我有種感覺今年會飛快流逝\n[副歌]\n雪花與槲寄生\n無論我走到哪裡\n我知道\n愛就是禮物\n那道光芒\n雪花與槲寄生\n[第二主歌]\n奶奶唱歌有點走音\n孩子們笑得太大聲\n狗狗偷走了一根火雞腿\n混亂成了聖誕節的人群\n[前副歌]\n但火焰劈啪作響\n溫暖又明亮\n而我的心今晚像一根蠟燭\n[副歌]\n雪花與槲寄生\n無論我走到哪裡\n我知道\n愛就是禮物\n那道光芒\n雪花與槲寄生","audio_url":"https://cdn1.suno.ai/1f610752-f426-4fd5-89a8-ba2ad0370881.mp3","video_url":"https://cdn1.suno.ai/1f610752-f426-4fd5-89a8-ba2ad0370881.mp4","created_at":"2025-12-13T11:29:25.101Z","model":"chirp-v5","state":"succeeded","prompt":"一首關於聖誕節的歌曲","style":"振奮人心、帶有鐘聲和木吉他的管弦樂風格","duration":129.92},{"id":"d1a3109d-799b-401e-b032-4b501bcf26f3","title":"雪花與槲寄生","image_url":"https://cdn2.suno.ai/image_d1a3109d-799b-401e-b032-4b501bcf26f3.jpeg","image_large_url":"https://cdn2.suno.ai/image_large_d1a3109d-799b-401e-b032-4b501bcf26f3.jpeg","lyric":"[主歌]\n燈飾又纏繞在樹上\n貓咪打翻了花環\n餅乾也在烤箱裡燒焦了\n但我仍然咬著牙微笑\n[前副歌]\n雪花像天空中的閃粉般飄落\n我有種感覺今年會飛快流逝\n[副歌]\n雪花與槲寄生\n無論我走到哪裡\n我知道\n愛就是禮物\n那道光芒\n雪花與槲寄生\n[第二主歌]\n奶奶唱歌有點走音\n孩子們笑得太大聲\n狗狗偷走了一根火雞腿\n混亂成了聖誕節的人群\n[前副歌]\n但火焰劈啪作響\n溫暖又明亮\n而我的心今晚像一根蠟燭\n[副歌]\n雪花與槲寄生\n無論我走到哪裡\n我知道\n愛就是禮物\n那道光芒\n雪花與槲寄生","audio_url":"https://audiopipe.suno.ai/?item_id=d1a3109d-799b-401e-b032-4b501bcf26f3","video_url":"","created_at":"2025-12-13T11:29:25.101Z","model":"chirp-v5","state":"執行中","prompt":"一首關於聖誕節的歌曲","style":"振奮人心、帶有鐘聲和木吉他的管弦樂風格"}]}
{"success":true,"task_id":"1af4b454-ce84-4512-a0a2-de3f8574ecd8","trace_id":"4440342a-41c4-4140-8bb1-3537a598ca2e","data":[{"id":"1f610752-f426-4fd5-89a8-ba2ad0370881","title":"雪花與槲寄生","image_url":"https://cdn2.suno.ai/image_1f610752-f426-4fd5-89a8-ba2ad0370881.jpeg","image_large_url":"https://cdn2.suno.ai/image_large_1f610752-f426-4fd5-89a8-ba2ad0370881.jpeg","lyric":"[Verse]\n燈飾又纏在樹上\n貓咪把花環弄翻了\n餅乾在烤箱裡燒焦了\n但我仍然咧嘴笑著\n[Prechorus]\n雪花像閃光粉一樣在空中飄落\n我有種預感今年會過得很快\n[Chorus]\n雪花與槲寄生\n無論我走到哪裡\n我知道\n愛是禮物\n那光芒\n雪花與槲寄生\n[Verse 2]\n奶奶唱得有點跑調\n孩子們笑得太大聲\n狗狗偷了一隻火雞腿\n混亂是一群聖誕人潮\n[Prechorus]\n但火爐在噼啪作響\n溫暖而明亮\n而我的心今晚是一支蠟燭\n[Chorus]\n雪花與槲寄生\n無論我走到哪裡\n我知道\n愛是禮物\n那光芒\n雪花與槲寄生","audio_url":"https://cdn1.suno.ai/1f610752-f426-4fd5-89a8-ba2ad0370881.mp3","video_url":"","created_at":"2025-12-13T11:29:25.101Z","model":"chirp-v5","state":"succeeded","prompt":"一首聖誕歌曲","style":"振奮人心，管弦樂搭配鈴聲和木吉他","duration":129.92},{"id":"d1a3109d-799b-401e-b032-4b501bcf26f3","title":"雪花與槲寄生","image_url":"https://cdn2.suno.ai/image_d1a3109d-799b-401e-b032-4b501bcf26f3.jpeg","image_large_url":"https://cdn2.suno.ai/image_large_d1a3109d-799b-401e-b032-4b501bcf26f3.jpeg","lyric":"[Verse]\n燈飾又纏在樹上\n貓咪把花環弄翻了\n餅乾在烤箱裡燒焦了\n但我仍然咧嘴笑著\n[Prechorus]\n雪花像閃光粉一樣在空中飄落\n我有種預感今年會過得很快\n[Chorus]\n雪花與槲寄生\n無論我走到哪裡\n我知道\n愛是禮物\n那光芒\n雪花與槲寄生\n[Verse 2]\n奶奶唱得有點跑調\n孩子們笑得太大聲\n狗狗偷了一隻火雞腿\n混亂是一群聖誕人潮\n[Prechorus]\n但火爐在噼啪作響\n溫暖而明亮\n而我的心今晚是一支蠟燭\n[Chorus]\n雪花與槲寄生\n無論我走到哪裡\n我知道\n愛是禮物\n那光芒\n雪花與槲寄生","audio_url":"https://cdn1.suno.ai/d1a3109d-799b-401e-b032-4b501bcf26f3.mp3","video_url":"","created_at":"2025-12-13T11:29:25.101Z","model":"chirp-v5","state":"succeeded","prompt":"一首聖誕歌曲","style":"振奮人心，管弦樂搭配鈴聲和木吉他","duration":127.16}]}
 

得到的結果跟基本調用類似，上面多個結果就實現了流式調用。

## 錯誤處理

如果發生錯誤，您將得到類似如下的錯誤信息：

```json
{
  "success": false,
  "error": {
    "code": "forbidden",
    "message": "歌曲描述包含藝術家名稱：eminem"
  },
  "trace_id": "9bb7c2f4-3b7b-4965-b50a-f663874b1b6f",
  "task_id": "9bb3a2a6-c438-436d-a9f3-fa466abc077c"
}
```

下面是 HTTP 狀態碼，`error.code`，`error.message` 的列表：

> 說明：不同上游帳號的限額和報錯文案可能存在差異。通常 `chirp-v3-5`/`chirp-v4` 的 `style` 限制更低（200），`chirp-v4-5` 及以上通常支持到 1000；當命中舊上游時，可能出現 `Tags too long.` 或 `style must be less than or equal 120` 等兼容文案。

| 狀態碼 | `error.code`  | `error.message`                                                 |
| ----------- | ------------- | --------------------------------------------------------------- |
| 400         | `bad_request` | `歌曲 ID 不存在或已下線。`                                     |
| 400         | `bad_request` | `提示過長。`                                                  |
| 400         | `bad_request` | `標籤過長。`                                                  |
| 400         | `bad_request` | `上傳的音頻與現有藝術作品匹配。`                              |
| 400         | `bad_request` | `instrumental 必須是布林值`                                   |
| 400         | `bad_request` | `標題過長。`                                                 |
| 400         | `bad_request` | `主題過長。`                                                 |
| 400         | `bad_request` | `style 必須小於或等於 120`                                   |
| 400         | `bad_request` | `custom 必須是布林值`                                        |
| 400         | `bad_request` | `當擴展音頻時需要 audio_id`                                   |
| 400         | `bad_request` | `當擴展音頻時需要 continue_at`                               |
| 400         | `bad_request` | `continue_at 必須是大於 0 的數字`                            |
| 400         | `bad_request` | `當擴展音頻且 instrumental 為假時需要 lyric`                |
| 400         | `bad_request` | `當生成音頻時需要提示`                                       |
| 400         | `bad_request` | `當生成自定義音頻時需要 lyric`                               |
| 403         | `forbidden`   | `提示可能格式錯誤`                                           |
| 403         | `forbidden`   | `提示可能受版權保護`                                         |
| 403         | `forbidden`   | `提示包含不當材料`                                           |
| 403         | `forbidden`   | `歌曲描述被標記為需要審核`                                   |
| 403         | `forbidden`   | `歌曲描述包含藝術家名稱`                                     |
| 403         | `forbidden`   | `標籤包含藝術家名稱`                                        |
| 403         | `forbidden`   | `歌詞包含受版權保護的材料`                                   |
| 403         | `forbidden`   | `歌曲描述包含製作人標籤`                                     |
| 403         | `forbidden`   | `通用的 openAI 錯誤`                                        |
| 403         | `forbidden`   | `提示被標記為需要審核`                                       |
| 500         | `api_error`   | `無法從歌曲描述生成歌詞`                                     |
| 500         | `api_error`   | `作業因未知錯誤失敗`                                         |
| 500         | `api_error`   | `系統中沒有可用的工作者`                                     |
| 500         | `api_error`   | `服務正在維護，生成已暫停`                                   |
| 504         | `timeout`     | `等待音頻生成時超時`                                         |
