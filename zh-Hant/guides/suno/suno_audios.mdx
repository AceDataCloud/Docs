...
- `negative_tags`：自定義模式（`custom` 為 `true`）下希望從生成結果中排除的音樂風格或流派。
- `audio_weight`：上傳的參考音頻占比，範圍 0-1，越大越依賴參考音頻。
- `audio_id`：參考音樂的 ID。
...
- `continue_at`：續寫邊界，單位為秒。例如，213.5 表示從第 3 分 33.5 秒處開始生成後續片段。`lyric` 和 `style` 只引導邊界之後的新內容，不會替換源音頻中邊界之前的歌詞或演唱。
...
## 自定義歌手風格生成功能
如果想使用歌手風格來生成歌曲的話，首先通過上文的基本使用生成一首歌曲，最後得到了需要設置這個歌曲為歌手風格，然後需要進入[Suno Persona API](https://platform.acedata.cloud/documents/suno-persona)根據官方生成的音樂 ID `audio_id` 來生成一個歌手風格的 id 參數 `persona_id`，具體的參數如下圖所示：
...
我們以上面的 `audio_id` 和 `persona_id` 分別為 `97efc9f4-0e8d-4b3e-88df-14568fa1b11f`、`e0d7319e-aa2a-44cb-b00a-916218d7cb0b` 為此次的示例數據。 然後可以將參數 `action` 設置為 `artist_consistency` (如果是新版的歌手風格Persona-v2-vox，`action` 必須設置為 `artist_consistency_vox`)，並且輸入需要繼續生成歌曲的 ID 、 歌手風格 ID，填寫樣例如下：
...
如果想對已經生成的 Suno 歌曲進行繼續生成的話，可以將參數 `action` 設置為 `extend` ，並且輸入需要繼續生成歌曲的 ID，歌曲 ID 的獲取是根據基本使用來獲取，通過上文可知，這時可以看到歌曲的 ID 為：
...
如果想對自己上傳的歌曲進行繼續生成的話，可以將參數 `action` 設置為 `upload_extend` ，並且輸入需要繼續生成自定義上傳的歌曲 ID，歌曲 ID 的獲取是使用 [Suno Upload Generation API](https://platform.acedata.cloud/documents/suno-upload)來獲取，如下圖所示：

<p><img src="https://cdn.acedata.cloud/a0mn5e.png" width="500" className="m-auto" /></p>

接下來必須填寫續寫片段的歌詞，並可以指定風格：

- lyric：只用於引導 `continue_at` 之後新生成片段的歌詞，不會替換源音頻中該時間點之前的歌詞。
- custom：填寫為 `true`，代表自定義生成，該參數默認為 false，代表使用 `prompt` 生成。
- style：續寫片段的歌曲風格，選填。
- continue_at：續寫邊界，單位為秒。例如，213.5 表示從第 3 分 33.5 秒處開始生成後續片段。

> `extend` 用於接著已有歌曲往後創作，不是給整首歌曲換詞。若希望整首歌曲從頭使用新歌詞，請使用 `generate` 重新生成；若希望參考已有歌曲重新演繹，可使用 `cover`。傳入一整首新歌詞時，`continue_at` 之前的原歌詞仍不會被替換。
...
下面保留的是一次真實歷史調用快照，其中 `continue_at` 為 2 秒，因此只有前 2 秒屬於續寫邊界之前的原內容。實際接近歌曲結尾續寫時，應將該值設為期望開始續寫的秒數，並僅在 `lyric` 中提供邊界之後要唱的新段落。
...
可以看出，結果中 `lyric` 會返回此次續寫任務使用的歌詞文本。該字段不是完整成品音頻的逐字轉寫；對於 `extend`，`continue_at` 之前的源音頻仍使用原歌詞，新歌詞只用於引導續寫片段。

## 獲取完整歌曲

目前模型的 `extend` 結果通常已包含 `continue_at` 之前的源音頻和邊界之後的新內容，請先根據返回音頻的實際時長與內容判斷是否已經是完整歌曲。若返回的是獨立續寫片段，或者需要將多次續寫歷史顯式合併為一首歌，再使用拼接功能：
...
> ⚠️ **注意**：`replace_section_result_mode` 默認為 `full_song`：系統會分別拼接 2 個新生成的候選，並返回 2 首完整歌曲。若只需要尚未拼接的替換片段候選，請明確設置為 `candidates`，然後選取候選調用 [音樂拼接](#音樂拼接)。
...
- replace_section_result_mode：返回模式，默認為 `full_song`。`full_song` 分別拼接 2 個候選並返回 2 首完整歌曲；`candidates` 返回 2 個尚未拼接的候選片段。
...
比如原先生成後的歌曲 ID 是：ec13e502-d043-4eb2-92ee-e900c6da69d1，那麼可以設置參數如下：
