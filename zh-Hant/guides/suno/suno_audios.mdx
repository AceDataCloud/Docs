...
該參數用於自定義模式（`custom` 為 `true`）的歌曲生成。需要特別說明的是，`duration` 是一個**傾向性提示，而不是硬性約束**：模型在創作時會參考這個值，但不保證達到，實測中實際時長通常明顯短於期望值，同一次請求返回的兩首歌曲時長也可能相差數倍。因此不要把它當作精確的時長控制來使用，如果業務上需要固定時長，請在拿到成品後自行裁剪或重試。
