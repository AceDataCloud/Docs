...
Suno 現已支持 `chirp-v6`、`chirp-v6-wild` 和 `chirp-v6-mini`。推薦使用 `chirp-v6`；舊模型名稱繼續兼容。
...
- `model`：此次音樂生成任務的模型。v6 系列包含 `chirp-v6`、`chirp-v6-wild` 和 `chirp-v6-mini`；舊模型名稱繼續兼容。
...
- data：是一個列表，包含了生成的歌曲的詳細信息。
  ...
  - image_url：歌曲的封面圖片
  - lyric：歌曲的歌詞
  - audio_url：歌曲的終態音頻地址。平台會優先返回 Ace Data Cloud CDN 地址；若持久化失敗，可能返回原始媒體地址，請及時下載。
...
| 400         | `bad_request` | `audio_id 是擴展音頻時所需`                                   |
| 400         | `bad_request` | `continue_at 是擴展音頻時所需`                               |
| 400         | `bad_request` | `continue_at 必須是大於 0 的數字`                             |
| 400         | `bad_request` | `lyric 是擴展音頻且 instrumental 為 false 時所需`            |
| 400         | `bad_request` | `prompt 是生成音頻時所需`                                    |
| 400         | `bad_request` | `lyric 是生成自定義音頻時所需`                                |
