...
description: "Localization translation 整合指南 - Ace Data Cloud"
---
...
要使用 Localization Translate API，首先到 [Ace Data Cloud 控制台](https://platform.acedata.cloud/console/applications) 獲取您的 API Token，留作備用。
...
> 📘 完整文檔：[Localization Translate API →](https://platform.acedata.cloud/documents/localization-translate)
...
- `input`：需要翻譯的內容。`extension=md` 時傳字符串，`extension=json` 時傳 JSON 對象。
- `locale`：自定義翻譯的語言。
- `extension`：輸入內容格式，可選 `md` 或 `json`。
- `model`：翻譯採用的大模型，默認是 gpt-3.5。

| `extension` | `input` 類型 | 示例                                       |
| ----------- | ---------- | ---------------------------------------- |
| `md`        | string     | `"# Title\n\nParagraph"`                 |
| `json`      | object     | `{"title": "Hello", "button": "Submit"}` |

`extension` 必須與 `input` 類型匹配；字符串不能使用 `json`，JSON 對象不能使用 `md`。
