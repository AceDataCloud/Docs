...
`cache_ttl_seconds`、`async`），外加两个 Extract 专属字段：
...
頂層字段也被回填：`byline = "alice"`、`publishedAt = "..."`。

### 5. Amazon 商品頁（Amazon 無 JSON-LD —— 需要啟用 LLM）
...
`data.structured.llm.data`（類型化 `product`）：

```json
{
  "kind": "product",
  "name": "Apple 2023 MacBook Pro M2 Pro 14 吋",
  "brand": "Apple",
  "price": 1799,
  "currency": "USD",
  "bullets": ["Apple M2 Pro 晶片，10 核 CPU", "..."],
  "specifications": [{ "name": "顯示器大小", "value": "14.2 吋" }, "..."]
}
```
