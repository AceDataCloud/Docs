...
- **استدعاء أدوات وكيل**: تحتوي على مجموعة من الأدوات مثل البحث عبر الإنترنت، واستخراج صفحات الويب، وقراءة الملفات، ويمكن ربطها بخادم MCP المصرح به من قبل المستخدم (مثل Google Drive، Notion، Slack، GitHub، إلخ)، حيث يمكن للنموذج استدعاء الأدوات بشكل مستقل في طلب واحد لإكمال المهام المعقدة.
...
كما هو الحال في v1، أرسل `stateful: true` لتمكين حفظ الجلسة، وستعيد واجهة برمجة التطبيقات `id`؛ في الطلبات اللاحقة، يمكنك إحضار `id` للمتابعة في المحادثة، دون الحاجة للحفاظ على تاريخ الرسائل بنفسك.
...
> `stateful` الافتراضي هو `true`، الإغفال عنه و تمرير `true` صيغتان متساويتان. إذا كنت لا ترغب في أن يحتفظ الخادم بهذه الجولة من المحادثة، يمكنك تعيين `stateful: false` بشكل صريح.

## استجابة متدفقة

v2 يدعم نوعين من التنسيقات المتدفقة، حسب اختيار رأس `accept`:
...
| لا حاجة للتدفق                 | `application/json` (افتراضي) | إرجاع مرة واحدة `{answer, id}`                       |
...
| `tool_result`       | نتيجة تنفيذ الأداة، تتطابق مع السطر السابق `tool_use` عبر `tool_id`، يشير `is_error` إلى ما إذا كان قد فشل.                                       |
...
| `system_message`    | معلومات تنبيه النظام (ليست محتوى المستخدم والمساعد)، تستخدم فقط كتنبيه واجهة المستخدم.                                                                     |
| `compact`           | حدث تم ضغط السياق الداخلي، لا حاجة لمعالجة خاصة.                                                                             |
| `error`             | حدث خطأ في هذه الجولة، يصف `message` محتوى الخطأ.                                                                        |
| `done`              | انتهاء الاستجابة المتدفقة، يحمل `usage` (يتضمن `prompt_tokens` / `completion_tokens` / `total_tokens`) و `terminal_reason`. |
...
إذا كان استدعاؤك يأتي من Webhook للإنذارات، أو CI/CD، أو نظام مراقبة أو مهام خلفية أخرى، يمكنك تعيين `async: true` لجعل الواجهة ترجع على الفور معرف المهمة، وتستمر الخلفية في التنفيذ:
...
التفويض المسبق هو هذان القائمتان فقط: القائمة الفارغة تعني عدم تفويض أي قدرات، ولا حاجة لحقول مفتاح إضافية.
...
بعض الأدوات ستجعل النموذج "يسأل المستخدم"، وفي هذه الحالة سيصدر النموذج حدث `ask_user_question`، وستكون المحادثة متوقفة في حالة `awaiting_user_input`:
...
إذا قرر المستخدم التخلي عن هذا السؤال، يمكنه ببساطة إرسال `question` / `message` جديدة، وستقوم المنصة تلقائيًا بوضع علامة على استدعاء الأداة المعلقة كـ "تخطي المستخدم".
...
بعد الانتقال، يمكنك تفعيل قدرات v2 الجديدة حسب الحاجة (مثل `message` متعددة الوسائط، SSE، استدعاء الأدوات، CRUD `action`)، يمكنك التقدم بالوتيرة التي تناسبك.
...
- `400 bad_request`: نقص في الحقول المطلوبة، عدم تطابق `tool_use_id`، مخطط `messages` غير قانوني، إلخ.
