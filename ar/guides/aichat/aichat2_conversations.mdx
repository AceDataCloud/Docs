...
واجهة برمجة تطبيقات AI Chat v2 (`/aichat2/conversations`) هي الجيل الجديد من واجهات المحادثة، وهي نسخة محدثة بالكامل من [واجهة برمجة تطبيقات AI Chat](https://platform.acedata.cloud/documents/aichat-conversations). لقد توسعت على أساس v1 البسيط، الذي يدعم المحادثات المتعددة، لتشمل:
...
- **استدعاء أدوات على شكل وكيل**: تحتوي على مجموعة من الأدوات للبحث عبر الإنترنت، واستخراج صفحات الويب، وقراءة الملفات، ويمكن ربطها بخوادم MCP المصرح بها من قبل المستخدم (مثل Google Drive، Notion، Slack، GitHub، إلخ)، حيث يمكن للنموذج استدعاء الأدوات بشكل مستقل في طلب واحد لإكمال المهام المعقدة.
...
- **قائمة نماذج مستمرة التحديث**: يتم الاتصال افتراضيًا بـ GPT-5.4، Claude Opus 4.8، Claude Sonnet 4.6، Gemini 3.1 Pro، GLM 5.1، DeepSeek V4، Kimi K3 وغيرها من النماذج المعاصرة.
...
> إذا كنت تستخدم حاليًا `/aichat/conversations`، ستظل الواجهة القديمة متاحة، يمكنك الانتقال بالوتيرة التي تناسبك.
...
**يمكن استخدام رمز API واحد لاستدعاء جميع خدمات المنصة، دون الحاجة لتقديم طلبات منفصلة لكل خدمة.** ستتلقى رصيدًا مجانيًا عند الطلب الأول، يمكنك تجربته مجانًا؛ وعند نفاد الرصيد، يمكنك إعادة شحن الرصيد العام في [وحدة التحكم](https://platform.acedata.cloud/console/coin).

> 📘 الوثائق الكاملة: [واجهة برمجة تطبيقات AI Chat v2 →](https://platform.acedata.cloud/documents/aichat2-conversations)

## الاستخدام الأساسي

أبسط طريقة للاستخدام تتطابق تمامًا مع v1: أرسل `model` + `question`، واحصل على `{answer, id}`.
...
- Google: `gemini-3.1-pro-preview`، `gemini-3.1-pro-preview`، `gemini-3.1-flash-image`، `gemini-3.1-pro-preview`، `gemini-2.5-flash-lite`، إلخ.
- xAI: `grok-4`، إلخ.
...
كما هو الحال في v1، أرسل `stateful: true` لتمكين حفظ الجلسة، ستعيد واجهة برمجة التطبيقات `id`؛ في الطلبات اللاحقة، يمكنك إحضار `id` للمتابعة في المحادثة، دون الحاجة للحفاظ على تاريخ الرسائل بنفسك.
...
> `stateful` الافتراضي هو `true`، الإغفال عنه و تمرير `true` صريح يعادل. إذا كنت لا ترغب في أن يحتفظ الخادم بهذه الجولة من المحادثة، يمكنك تعيين `stateful: false` بشكل صريح.
...
بالنسبة للعملاء الذين يهتمون فقط بالإجابة النهائية، فإن تجميع كل `content` من `text_delta` يعادل ما هو موجود في وضع `application/json` تحت `answer`.
...
يمكنك بعد ذلك استخدام `action: retrieve` + `id` للاستعلام عن نتائج المحادثة؛ يمكنك أيضًا تقديم `callback_url`، بعد الانتهاء من المهمة، ستقوم المنصة بإرسال `{ status, answer, usage, error }` عبر POST إلى عنوان ردك. يجب أن يستخدم `callback_url` `http` / `https`، ولا يمكن كتابة عنوان `localhost` أو عنوان IP خاص بشكل مباشر.
...
2. إذا كنت قد قمت بتمرير أسماء نماذج v1 (مثل `gpt-3.5`، `gpt-4-browsing`، إلخ)، يُنصح بالترقية إلى النماذج المعاصرة عند الانتقال إلى v2 (مثل `gpt-5.4`، `claude-opus-4-8`، `gemini-3.1-pro-preview`، إلخ).
...
بعد الانتقال، يمكنك تفعيل قدرات v2 الجديدة حسب الحاجة (إدخال متعدد الوسائط `message`، SSE، استدعاء الأدوات، CRUD `action`)، واتباع الإيقاع المناسب.
...
- `404 not_found`: عند `action: retrieve / update / delete`، المحادثة التي تتوافق مع `id` غير موجودة.
