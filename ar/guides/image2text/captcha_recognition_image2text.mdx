...
بشكل افتراضي، تكون واجهة برمجة التطبيقات متزامنة وتنتظر: ستظل الطلبات تنتظر حتى تكتمل معالجة نتيجة التعرف قبل أن تعود. إذا كنت تقوم بتدوير عدة أدوات للتعرف (multi-solver rotation) وترغب في "تقديم المهمة والحصول على task_id على الفور، ثم الانتقال إلى جدولة أدوات التعرف الأخرى، والعودة لاحقًا للحصول على النتيجة"، يمكنك تمرير `async: true` في جسم الطلب.
...
شرح الفوترة: في الوضع غير المتزامن، لا يتم احتساب رسوم لإنشاء المهام أو الاستعلام عن "قيد المعالجة"؛ **يتم احتساب الرسوم مرة واحدة فقط عند الحصول على نتيجة التعرف بنجاح** (بأسعار متطابقة مع الوضع المتزامن). لذلك، لن تؤدي إلغاء المهام التي لم تكتمل بعد إلى تكبد أي رسوم. `/captcha/tasks` متاحة لجميع واجهات التحقق من الرموز (token و recognition series) ويمكن استخدام نفس `task_id` للاستعلام.
...
من خلال هذه الوثيقة، لقد تعرفت على كيفية استخدام واجهة برمجة التطبيقات للتعرف على الرموز الرقمية الإنجليزية، والتي يمكن استخدامها للتعرف على الرموز الرقمية الإنجليزية ذات الطول المتغير. أدخل محتوى صورة الرمز، وستخرج نتيجة الرمز. نأمل أن تساعدك هذه الوثيقة في التوصيل واستخدام هذه الواجهة بشكل أفضل. إذا كان لديك أي أسئلة، فلا تتردد في الاتصال بفريق الدعم الفني لدينا.
