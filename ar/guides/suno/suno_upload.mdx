...
تسمح لنا SUNO برفع مرجع الصوت لإعادة الإبداع، توضح هذه الوثيقة طرق توصيل API ذات الصلة.

تحتوي هذه API على معلمة إدخال واحدة فقط، وهي `audio_url`، وهي عنوان CDN يمكن الوصول إليه علنًا، وتدعم لاحقة mp3.
...
مع معرف الأغنية، يمكننا استخدام [Suno Audios Generation API](https://platform.acedata.cloud/documents/suno-audios) لإنشاء أغاني مخصصة، على سبيل المثال، إذا تم تمرير `action` كـ `upload_extend`، وتم تمرير `audio_id` كمعرف الأغنية الذي تم إرجاعه، يمكننا إنشاء أغنية جديدة بناءً على مرجع الصوت.
