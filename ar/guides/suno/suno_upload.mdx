...
عندما لا يمكن معالجة الصوت الذي تملكه أو حصلت على إذن لاستخدامه من خلال الرفع العادي، يمكنك تعيين `mode` كـ `enhanced`. يجب أن يتضمن هذا الوضع أيضًا تقديم `name` يتكون من 1 إلى 100 حرف، واستخدام `audio_url` يمكن الوصول إليه علنًا عبر HTTPS.
...
ستعيد الواجهة على الفور `task_id` و `trace_id`. استخدم [Suno Tasks API](https://platform.acedata.cloud/documents/suno-tasks) للاستعلام عن المهمة، وبعد النجاح، اقرأ معرف الصوت المرفوع من `response.data.audio_id`. يمكنك أيضًا تقديم `callback_url` عبر HTTPS لاستقبال الحالة النهائية.

يمكن استخدام `audio_id` الناتج عن الرفع المعزز في Cover و Samples و Mashup. قد تظل عمليات استخراج المرافقة و MIDI، وفصل المسارات الكاملة، وفصل الصوت/المرافقة تخضع لقيود حساب الصوت؛ العمليات غير المدرجة لا تضمن دعم الاستخدام عبر الحسابات. كلما كان الصوت أقصر، كانت نسبة نجاح المعالجة عادةً أعلى.
