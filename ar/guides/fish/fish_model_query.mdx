...
> لإنشاء نغمة، يرجى الرجوع إلى [Fish Model Create API](https://platform.acedata.cloud/documents/fish-model)، وللبحث عن تفاصيل نغمة واحدة بواسطة `_id` يرجى الرجوع إلى [Fish Model Get API](https://platform.acedata.cloud/documents/fish-model-get).

## عملية التقديم

لاستخدام API، يجب أولاً التقديم للحصول على الخدمة المناسبة من خلال صفحة [Fish Model API](https://platform.acedata.cloud/services/fish)، بعد الدخول إلى الصفحة اضغط على زر "Acquire".
...
القيمة `_id` المعادة يمكن استخدامها كقيمة لحقل `reference_id` في [Fish TTS API](https://platform.acedata.cloud/documents/fish-tts) لاستخدام هذه النغمة المستنسخة في توليد الصوت.
...
توفر واجهة Fish Model Query API قدرة بحث عن النغمات متوافقة تمامًا مع Fish Audio الرسمية، ويمكن استخدامها للحفاظ على مكتبة نغمات استنساخ خاصة بك على هذه المنصة. وبالاشتراك مع [Fish Model Get API](https://platform.acedata.cloud/documents/fish-model-get) يمكنك الحصول على تفاصيل كاملة لنغمة واحدة حسب المعرف.
