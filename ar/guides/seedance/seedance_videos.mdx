---
title: "واجهة برمجة تطبيقات توليد مقاطع فيديو SeeDance - تعليمات الدمج"
...
لاستخدام واجهة برمجة تطبيقات توليد مقاطع فيديو SeeDance، يجب أولاً الذهاب إلى [وحدة التحكم في Ace Data Cloud](https://platform.acedata.cloud/console/applications) للحصول على رمز API الخاص بك، احتفظ به للاستخدام لاحقًا.
...
> 📘 الوثائق الكاملة: [واجهة برمجة تطبيقات توليد مقاطع فيديو SeeDance →](https://platform.acedata.cloud/documents/seedance-videos)
...
يمكننا أن نرى أننا حصلنا على معلومات الفيديو المرضية، كل ما علينا هو الحصول على الفيديو الناتج من SeeDance بناءً على عنوان URL للفيديو في `data`.
...
| `--cf`           | `camerafixed`       | هل الكاميرا ثابتة | `true` / `false`                                            |
| `--wm`           | `watermark`         | هل تضيف علامة مائية | `true` / `false`                                            |
...
payload = {
    "content": [
        ...
        {
            "type": "text",
            "text": "فتاة تحمل ثعلبًا في ذراعيها. تفتح عينيها وتنظر برقة إلى الكاميرا، بينما الثعلب يحتضنها بحب. بينما تسحب الكاميرا ببطء، يتم تهوية شعرها برفق بواسطة الرياح. --ratio adaptive  --dur 5"
        }
    ],
...
- image_url
  - url رابط الصورة
    في نفس الوقت، يجب أن يتضمن `content` أيضًا نوع `text` ككلمات تحفيزية.
...
- **متطلبات مادة الصوت المرجعي (`audio_url`)**: صيغة `wav` / `mp3`؛ **مدة كل مقطع من 2 إلى 15 ثانية**، بحد أقصى 3 مقاطع و**إجمالي المدة لا يتجاوز 15 ثانية**؛ لا يتجاوز كل مقطع 15 ميغابايت. أي تجاوز في المدة سيؤدي إلى فشل معالجة المادة في مرحلة المعالجة.
- **متطلبات مادة الفيديو المرجعي (`video_url`)**: صيغة `mp4` / `mov`؛ **مدة كل مقطع من 2 إلى 15 ثانية**، بحد أقصى 3 مقاطع و**إجمالي المدة لا يتجاوز 15 ثانية**.
