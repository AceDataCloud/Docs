...
مع اتساع استخدام الذكاء الاصطناعي، أصبحت برامج الذكاء الاصطناعي متنوعة وشائعة. لقد أصبح الذكاء الاصطناعي جزءًا متزايدًا من جوانب حياة الناس وعملهم. كما أن الصناعات التي تشملها الذكاء الاصطناعي تتزايد، بدءًا من الكتابة، إلى التعليم الطبي، وصولاً إلى الفيديوهات الآن.
...
لاستخدام واجهة برمجة تطبيقات توليد فيديوهات Luma، يجب أولاً الذهاب إلى [لوحة تحكم Ace Data Cloud](https://platform.acedata.cloud/console/applications) للحصول على رمز API الخاص بك، للاحتفاظ به للاستخدام لاحقًا.
...
> 📘 الوثائق الكاملة: [واجهة برمجة تطبيقات توليد فيديوهات Luma →](https://platform.acedata.cloud/documents/luma-videos)
...
- `aspect_ratio`: نسبة عرض الفيديو إلى ارتفاعه، الافتراضي 16:9.
- `end_image_url`: اختياري، لتحديد الإطار النهائي.
- `enhancement`: اختياري، مفتاح تعزيز الوضوح.
- `loop`: هل يجب توليد فيديو متكرر، الافتراضي false.
- `timeout`: اختياري، عدد الثواني للمهلة.
- `callback_url`: عنوان الاسترجاع غير المتزامن.
- `async`: اختياري، إذا تم تعيينه إلى `true`، ستعيد الواجهة على الفور `task_id`، دون الحاجة لتقديم `callback_url`، ثم يمكنك الاستعلام عن النتائج من خلال واجهة الاستعلام عن المهام المقابلة.

يمكنك النقر على زر "Try" لاختبار واجهة برمجة التطبيقات مباشرة، انتظر 1-2 دقيقة، وستكون النتائج كما يلي:
...
يمكنك أن ترى أننا حصلنا على معلومات حول هذا الفيديو، بما في ذلك معرف الفيديو، رابط الفيديو، غلاف الفيديو، وغيرها من المحتويات.

تفسير الحقول كما يلي:

- success: هل تم التوليد بنجاح، إذا كان ناجحًا سيكون `true`، وإلا سيكون `false`
- task_id: المعرف الفريد لمهمة توليد الفيديو هذه
- video_id: المعرف الفريد للفيديو الناتج عن مهمة توليد الفيديو هذه
- prompt: الكلمات الرئيسية لمهمة توليد الفيديو هذه
- video_url: رابط الفيديو الناتج عن مهمة توليد الفيديو هذه
- video_height: ارتفاع صورة غلاف الفيديو الناتج
- video_width: عرض صورة غلاف الفيديو الناتج
- state: حالة مهمة توليد الفيديو هذه، إذا كانت المهمة مكتملة ستكون `completed`.
...
بعد ذلك، نريد تخصيص توليد الفيديو بناءً على الإطارات والكلمات الرئيسية، يمكننا تحديد المحتويات كما يلي:
...
- end_image_url: تحديد الإطار النهائي للفيديو الناتج.
...
النتيجة النهائية مشابهة لما سبق، حيث يحتوي الفيديو الناتج على الإطار الأول الذي قمنا بتمريره، بالطبع يمكن أيضًا تمرير روابط صور الإطارات الأولى والأخيرة معًا لتوليد الفيديو، كل ما عليك هو إضافة صورة الإطار النهائي على الأساس السابق، ومعلومات صورة الإطار النهائي كما يلي:

![الإطار النهائي](https://cdn.acedata.cloud/0iad3k.png)

مثال على كيفية التعبئة كما يلي:

<p><img src="https://cdn.acedata.cloud/20igwi.png" width="500" className="m-auto" /></p>
أخيرًا تم التوصل إلى النتائج التالية:
...
النتائج مشابهة لما سبق، الفيديو الناتج يحتوي أيضًا على صورة الإطار الأول والإطار الأخير، مما يحقق تخصيص الإطارات الأولى والأخيرة لإنشاء الفيديو.

## وظيفة توسيع الفيديو

إذا كنت ترغب في الاستمرار في إنشاء الفيديو الناتج، يمكنك تعيين المعامل `action` إلى `extend`، وإدخال ID الفيديو أو رابط الفيديو الذي تحتاج إلى الاستمرار في إنشائه، يتم الحصول على ID الفيديو ورابط الفيديو بناءً على الاستخدام الأساسي كما هو موضح في الصورة أدناه:
...
> ملاحظة: هنا في الفيديو، `video_id` و `video_url` هما ID ورابط الفيديو الناتج، إذا كنت لا تعرف كيفية إنشاء الفيديو، يمكنك الرجوع إلى الاستخدام الأساسي المذكور أعلاه لإنشاء الفيديو.
...
- video_id: ID الفيديو الفريد الذي يحتاج إلى توسيع.
- end_image_url: رابط صورة الإطار الأخير التي يمكن تحديدها لتوسيع الفيديو، معامل اختياري.
...
عند الضغط على التشغيل، يمكنك أن ترى أنك ستحصل على نتيجة كما يلي:
...
يمكن أن نرى من النتيجة أنه يمكن أيضًا تحقيق وظيفة توسيع الفيديو بناءً على رابط الفيديو.

أخيرًا، يمكننا أيضًا تحديد صورة إطار أخير للتوسيع في الفيديو الموسع، أدناه معلومات صورة الإطار الأخير:

![الإطار الأخير](https://cdn.acedata.cloud/0iad3k.png)

بعد ذلك، بناءً على ما سبق، أضفنا معلومات صورة الإطار الأخير، كما هو موضح أدناه:

<p><img src="https://cdn.acedata.cloud/9p1vrj.png" width="500" className="m-auto" /></p>

عند الضغط على التشغيل، حصلنا على المعلومات التالية:
...
يمكن أن نرى أنه بناءً على الفيديو الموسع السابق، يمكن أيضًا تحديد صورة الإطار الأخير للتوسيع.

## ردود الفعل غير المتزامنة

نظرًا لأن وقت إنشاء الفيديو بواسطة Luma طويل نسبيًا، حوالي 1-2 دقيقة، إذا لم يكن هناك استجابة من API لفترة طويلة، ستظل طلبات HTTP متصلة، مما يؤدي إلى استهلاك موارد النظام الإضافية، لذا فإن هذه API توفر أيضًا دعم ردود الفعل غير المتزامنة.
