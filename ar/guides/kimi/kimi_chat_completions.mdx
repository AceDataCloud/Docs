...
Kimi هو سلسلة نماذج الذكاء الاصطناعي التي أطلقتها "الجانب المظلم من القمر". النموذج الموصى به حاليًا هو `kimi-k3`، والذي يركز على البرمجة طويلة المدى، والوكيل، والاستدلال المعقد، وأعمال المعرفة، ويمكن استدعاؤه من خلال واجهة برمجة التطبيقات المتوافقة مع OpenAI.
...
لاستخدام واجهة برمجة تطبيقات Kimi Chat Completion، يجب أولاً الذهاب إلى [لوحة تحكم Ace Data Cloud](https://platform.acedata.cloud/console/applications) للحصول على رمز API الخاص بك، للاحتفاظ به كنسخة احتياطية.
...
يمكنك أيضًا ملاحظة أن هناك كود استدعاء مطابق على الجانب الأيمن، يمكنك نسخ الكود وتشغيله مباشرة، أو يمكنك النقر مباشرة على زر "Try" لإجراء اختبار.
...
- `usage`، معلومات إحصائية عن توكنات السؤال والجواب.
...
- [Thinking Effort](https://platform.kimi.ai/docs/guide/use-thinking-effort)：يوضح أن Kimi K3 دائمًا ما يتم تفعيل الاستدلال، والقيمة الوحيدة المدعومة حاليًا لـ `reasoning_effort` هي `max`.
...
تعديل كما هو موضح في الصورة، ولكن يجب أن يحتوي كود الاستدعاء على التغييرات المناسبة لدعم الاستجابة المتدفقة.

<p><img src="https://cdn.acedata.cloud/a3nzpw.png" width="400" className="m-auto" /></p>

بعد تغيير `stream` إلى `true`، ستقوم واجهة برمجة التطبيقات بإرجاع بيانات JSON المقابلة سطرًا بسطر، وعلى مستوى الكود، نحتاج إلى إجراء التعديلات المناسبة للحصول على النتائج سطرًا بسطر.

مثال على كود الاستدعاء بلغة Python:
...
فيما يلي مقتطف من استجابة K3 Max المتدفقة الحقيقية، بما في ذلك الكتل الابتدائية، والاستدلال، والنص، والنهاية، وبيانات الاستخدام:
```json
...
- `choices`، معلومات الإجابة المقدمة من Kimi بناءً على كلمات السؤال.
...
jsonObject.put("messages", new JSONArray().put(new JSONObject().put("role", "user").put("content", "مرحبا")));
...
إذا كنت ترغب في دمج وظيفة المحادثات متعددة الجولات، تحتاج إلى رفع عدة كلمات سؤال في حقل `messages`، مثال على عدة كلمات سؤال موضحة في الصورة أدناه:
...
من خلال رفع عدة كلمات سؤال، يمكنك بسهولة تحقيق المحادثات متعددة الجولات. فيما يلي استجابة K3 Max الحقيقية التي تم الحصول عليها من هذا الطلب (تم حذف الحقول الإضافية غير المستخدمة):
...
يمكنك أن ترى أن المعلومات الموجودة في `choices` تتوافق مع المحتوى الأساسي المستخدم، وهذا يتضمن ردود Kimi على عدة محادثات، مما يتيح لك الإجابة على الأسئلة المقابلة بناءً على محتوى المحادثات المتعددة.

## معالجة الأخطاء

عند استدعاء واجهة برمجة التطبيقات، إذا واجهت أخطاء، ستقوم واجهة برمجة التطبيقات بإرجاع رمز الخطأ والمعلومات ذات الصلة. على سبيل المثال:
...
من خلال هذه الوثيقة، لقد فهمت كيفية استخدام واجهة برمجة تطبيقات Kimi Chat Completion لتحقيق المحادثات العادية، والاستجابات المتدفقة، والمحادثات متعددة الجولات، وكذلك كيفية التحكم في قوة الاستدلال لـ K3 من خلال `reasoning_effort`.
