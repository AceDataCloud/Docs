...
ستقدم هذه الوثيقة شرحًا عن كيفية توصيل بروتوكول التعرف على hCaptcha API، والذي يتيح للمستخدمين عدم الحاجة إلى التعرف على صور رمز التحقق hCaptcha والنقر عليها، بل يكفي تقديم مفتاح الموقع لتحقيق فك التشفير التلقائي في الخلفية وإكمال التحقق.
...
أولاً، يجب أن نفهم طريقة الاستخدام الأساسية، وهي إدخال URL الموقع الذي يحتاج إلى معالجة رمز التحقق hCaptcha، للحصول على النتيجة المعالجة. أولاً، نحتاج إلى تمرير حقل `website_url` ببساطة، وموقعنا التجريبي هو: `https://accounts.hcaptcha.com/demo`، نحتاج إلى الحصول على `website_key` من صفحة `website_url`، يجب أولاً فتح هذه الصفحة، ثم الضغط على F12 للدخول إلى وحدة التحكم، وأخيرًا إجراء بحث عالمي في صفحة العناصر عن `hcaptcha-demo`، وسنحصل على النتيجة التالية:
...
- `accept`: نوع التنسيق الذي ترغب في تلقيه كاستجابة، هنا يتم ملؤه بـ `application/json`، أي بتنسيق JSON.
