...
ستقدم هذه المقالة شرحًا عن بروتوكول التعرف على hCaptcha API، والذي يتيح للمستخدمين عدم الحاجة إلى التعرف على صور رمز التحقق hCaptcha والنقر عليها، بل يكفي تقديم مفتاح الموقع لتحقيق فك التشفير التلقائي في الخلفية وإكمال التحقق.
...
**رمز API واحد يكفي لاستدعاء جميع خدمات المنصة، دون الحاجة لتقديم طلب منفصل لكل خدمة.** سيتم منحك رصيد مجاني عند التقديم لأول مرة، يمكنك تجربته مجانًا؛ وعند نفاد الرصيد، يمكنك إعادة شحن الرصيد العام في [لوحة التحكم](https://platform.acedata.cloud/console/coin).

> 📘 الوثائق الكاملة: [hCaptcha بروتوكول التعرف على API →](https://platform.acedata.cloud/documents/captcha-token-hcaptcha)

## الاستخدام الأساسي

أولاً، يجب أن نفهم طريقة الاستخدام الأساسية، وهي إدخال URL الموقع الذي يحتاج إلى معالجة رمز التحقق hCaptcha، للحصول على النتيجة المعالجة، يجب أولاً تمرير حقل `website_url` ببساطة، موقعنا التجريبي هو: `https://accounts.hcaptcha.com/demo`، نحتاج إلى الحصول على `website_key` من صفحة `website_url`، يجب أولاً فتح هذه الصفحة، ثم الضغط على F12 للدخول إلى وحدة التحكم، وأخيرًا البحث عالميًا في صفحة العناصر عن `hcaptcha-demo`، وسنحصل على النتيجة التالية:
...
- `proxy`: اختياري، استخدم وكيل خاص بك (Bring Your Own Proxy). بعد التعيين، سيقوم الخادم العلوي باستخدام IP الوكيل الذي قدمته لفك تشفير الرمز، للتحكم في جودة IP المخرج (على سبيل المثال، لتجنب حظر IP الوكيل العام من قبل الموقع المستهدف مما يؤدي إلى إرجاع `410 Gone`). التنسيق هو `scheme://[user:pass@]host:port`، حيث أن `scheme` يدعم `http`/`https`/`socks4`/`socks5`، مثل `http://user:pass@1.2.3.4:8080`. إذا لم يتم ملؤه، سيتم استخدام الوكيل الافتراضي للمنصة.
...
- `token`، نتيجة التحقق من مهمة hCaptcha بعد المعالجة.
