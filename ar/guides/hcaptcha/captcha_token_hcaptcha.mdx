---
title: "hCaptcha بروتوكول التعرف على API توصيل الشرح"
description: "hCaptcha verification code recognition service API guide - Ace Data Cloud"
---

ستقدم هذه الوثيقة شرحًا عن بروتوكول التعرف على hCaptcha API، والذي يسمح للمستخدمين بتجاوز التعرف والنقر على صور التحقق من hCaptcha، حيث يمكنهم فقط تقديم مفتاح الموقع لتحقيق فك التشفير التلقائي في الخلفية وإكمال التحقق.

## عملية التقديم

لاستخدام بروتوكول التعرف على hCaptcha API، يجب أولاً الذهاب إلى [لوحة تحكم Ace Data Cloud](https://platform.acedata.cloud/console/applications) للحصول على رمز API الخاص بك، للاحتفاظ به كنسخة احتياطية.
...
**رمز API واحد يكفي لاستدعاء جميع خدمات المنصة، دون الحاجة لتقديم طلب منفصل لكل خدمة.** عند التقديم لأول مرة، ستحصل على رصيد مجاني لتجربته مجانًا؛ وعند نفاد الرصيد، يمكنك إعادة شحن الرصيد العام في [لوحة التحكم](https://platform.acedata.cloud/console/coin).

> 📘 الوثائق الكاملة: [hCaptcha بروتوكول التعرف على API →](https://platform.acedata.cloud/documents/captcha-token-hcaptcha)

## الاستخدام الأساسي

أولاً، يجب أن نفهم طريقة الاستخدام الأساسية، وهي إدخال URL الموقع الذي يحتاج إلى معالجة رمز التحقق hCaptcha، للحصول على النتيجة المعالجة. أولاً، نحتاج إلى تمرير حقل `website_url` ببساطة، وموقعنا التجريبي هو: `https://accounts.hcaptcha.com/demo`، نحتاج إلى الحصول على `website_key` من صفحة `website_url`، يجب أولاً فتح هذه الصفحة، ثم الضغط على F12 للدخول إلى وحدة التحكم، وأخيرًا البحث عالميًا في صفحة العناصر عن `hcaptcha-demo`، وسنحصل على النتيجة التالية:
...
- `rqdata`: اختياري. قد تقدم تحديات hCaptcha Enterprise `data-rqdata` في الصفحة، وعند مواجهة مثل هذه التحديات، يتم ملء قيمتها الأصلية؛ لا حاجة لملء hCaptcha العادية.
- `proxy`: اختياري، احضر وكيلك الخاص (Bring Your Own Proxy). بعد الإعداد، سيقوم النظام بفك تشفير الرمز باستخدام IP الوكيل الذي تقدمه، للتحكم في جودة IP المخرج (على سبيل المثال، لتجنب حظر IP الوكيل العام من قبل الموقع المستهدف وإرجاع `410 Gone`). التنسيق هو `scheme://[user:pass@]host:port`، حيث يدعم `scheme` `http`/`https`/`socks4`/`socks5`، مثل `http://user:pass@1.2.3.4:8080`. إذا لم يتم ملؤه، سيتم استخدام الوكيل الافتراضي للمنصة.
