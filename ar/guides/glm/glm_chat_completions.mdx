...
**يمكن استخدام رمز API واحد لاستدعاء جميع خدمات المنصة، دون الحاجة لتقديم طلب منفصل لكل خدمة.** ستتلقى رصيدًا مجانيًا عند الطلب الأول، يمكنك تجربته مجانًا؛ إذا كان الرصيد غير كافٍ، يمكنك إعادة شحن الرصيد العام في [لوحة التحكم](https://platform.acedata.cloud/console/coin).
...
- `model`: اختر نموذج GLM الذي تريد استدعاءه، النماذج المدعومة حاليًا تشمل:
  ...
  - `glm-5.1`: النموذج الرائد من الجيل السابق، ذو قدرة شاملة قوية.
...
- `temperature`: عشوائية التوليد، بين 0-2، كلما زادت القيمة كانت النتائج أكثر تشتتًا.
...
إذا كنت ترغب في إرجاع الاستجابة بشكل متدفق، قم بتعيين معلمة `stream` في جسم الطلب إلى `true`.
...
يمكنك أن ترى أن هناك العديد من `data` في الاستجابة، كل `data` تحتوي على جزء متزايد. `choices[i].delta.content` هو النص الجديد المضاف في الجزء الحالي، يمكنك تجميع هذه الأجزاء لتشكيل رد كامل. عندما يكون محتوى `data` هو `[DONE]`، فهذا يعني أن الاستجابة المتدفقة قد انتهت. ستلخص آخر جزء يحمل `usage` استخدام الرموز لهذه الطلب.
...
يمكنك أن ترى أن المعلومات الموجودة في `choices` تتماشى مع الاستخدام الأساسي، حيث يقدم النموذج ردًا بناءً على تاريخ المحادثة الكامل، مما يدعم التفاعل السياقي متعدد الجولات.
...
| ------------- | -------------------------- |
| `glm-5.2`     | أحدث طراز، يُوصى به للاستخدام في الاستدلالات المعقدة، المهام المتعلقة بالرمز و الوكلاء |
| `glm-5.1`     | الطراز السابق، مناسب للاستدلالات المعقدة، تحليل الوثائق الطويلة         |
| `glm-4.7`     | استدعاء الأدوات، توليد الرموز، مهام تنسيق الوكلاء وغيرها      |
| `glm-4.6`     | خيار متوازن للحوار العام، إنشاء المحتوى             |
| `glm-3-turbo` | مهام توليد النصوص العامة، السيناريوهات الحساسة للتكلفة          |
