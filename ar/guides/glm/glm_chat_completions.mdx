...
GLM（النموذج اللغوي العام） هو سلسلة من نماذج اللغة الكبيرة من Zhipu AI (智谱 AI / Z.ai) ، والتي تتمتع بقدرات قوية في فهم وتوليد اللغة الصينية والإنجليزية، وتظهر أداءً ممتازًا في المهام مثل المشهد الصيني، وتوليد الشيفرات، والاستدلال، والحوار المتعدد الجولات. تم تحسين النماذج الجديدة مثل GLM-5.3 و GLM-5.2 و GLM-4.7 بشكل كبير في السياقات الطويلة، واستدعاء الأدوات، ومهام الشيفرة، ويمكن استخدامها على نطاق واسع في مشاهد مثل الأسئلة الذكية، وإنشاء المحتوى، والمساعدة في الشيفرة، وروبوتات خدمة العملاء.

تتناول هذه الوثيقة بشكل رئيسي عملية استخدام GLM Chat Completion API، حيث يمكنك من خلاله استدعاء نماذج سلسلة GLM بسهولة عبر واجهة متوافقة مع OpenAI. عند الحاجة لاستخدام `glm-5.3` في Coze (扣子)، يرجى الرجوع إلى [دليل نماذج Coze المخصصة](https://platform.acedata.cloud/documents/coze-integration) واختيار بروتوكول OpenAI Chat Completions.
...
- `model`：اختر نموذج GLM الذي تريد استدعاءه، النماذج المدعومة حاليًا تشمل:
  - `glm-5.3`：أحدث نموذج رائد، يدعم 1M سياق وأقصى 128K إخراج، مناسب للاستدلال المعقد، والشيفرة، ومهام الوكيل. الاستدلال مفعل دائمًا، ويمكنك اختيار `reasoning_effort` كـ `low` أو `high` أو `max`.
...
- `messages`：مصفوفة الكلمات الدلالية، تحتوي كل رسالة على `role` و `content`، حيث يدعم `role` ثلاثة أدوار: `user` و `assistant` و `system`.
...
- `top_p`：معلمة أخذ العينات الأساسية، تتحكم في عتبة الاحتمال التراكمي للرموز المرشحة.
...
من خلال رفع عدة أسئلة، يمكنك بسهولة تنفيذ محادثة متعددة الجولات، ويمكنك الحصول على الرد التالي:
...
payload = {
    "model": "glm-4.7",
    "messages": [
        {"role": "system", "content": "أنت مساعد كتابة محترف باللغة الصينية، يرجى الرد بنبرة احترافية ومختصرة."},
...
يدعم نموذج GLM استدعاء الأدوات المتوافقة مع OpenAI، يمكنك من خلال معلمة `tools` إعلان الوظائف القابلة للاستدعاء، وسيقوم النموذج عند الحاجة بإرجاع معلومات استدعاء الوظائف المهيكلة في `choices[i].message.tool_calls`.
...
- `429 too_many_requests`：تم تفعيل حد التردد، يرجى المحاولة لاحقًا.
...
عند إرجاع `api_error` وكانت الرسالة `الخدمة غير متاحة مؤقتًا، يرجى المحاولة لاحقًا.`، فهذا عادةً ما يعني أن خدمة GLM العلوية غير متاحة مؤقتًا، يُنصح بإعادة المحاولة مع تراجع أسي، أو التبديل إلى نموذج GLM آخر متاح (مثل التبديل مؤقتًا من `glm-5.1` إلى `glm-4.7` أو `glm-4.6`).
