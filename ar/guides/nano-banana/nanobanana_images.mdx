---
title: "واجهة برمجة تطبيقات صور نانو موز - تعليمات الدمج"
description: "Nano Banana Image Generation API guide - Ace Data Cloud"
---

تقدم هذه الوثيقة تعليمات دمج واستخدام واجهة برمجة تطبيقات صور نانو موز. تدعم هذه الواجهة قدرتين: **توليد الصور (generate)** و **تحرير الصور (edit)**.
...
curl -X POST 'https://api.acedata.cloud/nano-banana/images' \
  ...
  -d '{
    ...
    "prompt": "صورة قريبة واقعية لشخص مسن ياباني يصنع الفخار مع تجاعيد عميقة محفورة من الشمس وابتسامة دافئة وعارفة. إنه يقوم بفحص وعاء شاي تم تلميعه حديثًا بعناية. الإعداد هو ورشته الريفية المشرقة تحت الشمس. المشهد مضاء بضوء الساعة الذهبية الناعم الذي يتدفق عبر نافذة، مما يبرز الملمس الدقيق للطين. تم التقاطها بعدسة بورتريه 85 مم، مما ينتج عنه خلفية ناعمة ومشوشة (بوكيه). المزاج العام هادئ وماهر. اتجاه الصورة عمودي.",
    "count": 1
  }'
```
...
payload = {
    ...
    "prompt": (
        ...
        "الشمس. المشهد مضاء بضوء الساعة الذهبية الناعم الذي يتدفق عبر نافذة، "
        "مما يبرز الملمس الدقيق للطين. تم التقاطها بعدسة بورتريه 85 مم، مما "
        "ينتج عنه خلفية ناعمة ومشوشة (بوكيه). المزاج العام هادئ وماهر. اتجاه "
        "الصورة عمودي."
    ),
    "count": 1
}
...
على سبيل المثال، هنا نقدم صورة لشخص وصورة لملابس، لجعل الشخص يرتدي هذه الملابس، يمكننا تمرير روابط الصور مع تعيين `action` إلى `edit`، يمكن أن تكون الروابط URL روابط عامة يمكن الوصول إليها عبر بروتوكول `https` أو `http`، أو يمكن أن تكون صورة مشفرة بتنسيق Base64، مثل `data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAA+gAAAVGCAMAAAA6u2FyAAADAFBMVEXq6uwdHCEeHyMdHS....`
...
- **400 `token_mismatched`**：الطلب غير قانوني أو يوجد خطأ في المعلمات.
