...
- **النموذج (model)** (اختياري):
  ...
  - `nano-banana-2`: يعتمد على معاينة صورة Gemini 3.1 Flash، جودة احترافية + سرعة فلاش
...
curl -X POST 'https://api.acedata.cloud/nano-banana/images' \
  ...
  -d '{
    ...
    "prompt": "صورة قريبة واقعية لشخص مسن ياباني يصنع الفخار مع تجاعيد عميقة محفورة من الشمس وابتسامة دافئة وعارفة. إنه يقوم بفحص وعاء شاي تم تلميعه حديثًا بعناية. الإعداد هو ورشته الريفية المشرقة تحت الشمس. المشهد مضاء بضوء الساعة الذهبية الناعم المتدفق من خلال نافذة، مما يبرز النسيج الدقيق للطين. تم التقاطها بعدسة بورتريه 85 مم، مما ينتج عنه خلفية ناعمة ومشوشة (بوكيه). المزاج العام هادئ وماهر. اتجاه الصورة عمودي.",
    "count": 1
  }'
```
...
payload = {
    ...
    "prompt": (
        ...
        "نافذة، مما يبرز النسيج الدقيق للطين. تم التقاطها بعدسة بورتريه 85 مم، "
...
عندما ترغب في تحرير صورة موجودة، قم بتعيين `action` إلى `edit`، ومرر قائمة روابط الصور التي ترغب في تحريرها عبر `image_urls` (صورة واحدة أو أكثر)، مع تقديم وصف لهدف التحرير عبر `prompt`.
...
- **إمكانية الوصول إلى الصور**：يجب أن تكون `image_urls` روابط مباشرة يمكن الوصول إليها على الإنترنت (HTTP/HTTPS)، يُنصح باستخدام HTTPS
