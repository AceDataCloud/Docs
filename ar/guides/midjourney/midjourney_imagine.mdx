...
Midjourney هي أداة رسم بالذكاء الاصطناعي قوية جدًا، حيث يمكنها توليد صور رائعة في غضون دقيقة أو دقيقتين فقط عند إدخال الكلمات الرئيسية. تميزت Midjourney بقدرتها الفائقة على الرسم، وأصبحت الآن مستخدمة على نطاق واسع في مختلف الصناعات والقطاعات، مما زاد من تأثيرها بشكل ملحوظ.

تتناول هذه الوثيقة بشكل رئيسي عملية استخدام عملية Imagine في Midjourney API، حيث يمكننا من خلالها بسهولة توليد الصور المطلوبة من النص.

## 申请流程

لاستخدام Midjourney Imagine API، يجب أولاً الذهاب إلى [لوحة تحكم Ace Data Cloud](https://platform.acedata.cloud/console/applications) للحصول على رمز API الخاص بك، للاحتفاظ به كنسخة احتياطية.
...
> 📘 الوثيقة الكاملة: [Midjourney Imagine API →](https://platform.acedata.cloud/documents/midjourney-imagine "Midjourney Imagine API")
...
عند استخدام هذه الواجهة لأول مرة، نحتاج على الأقل إلى ملء محتويين، أحدهما هو `authorization`، والذي يمكن اختياره مباشرة من القائمة المنسدلة. المعامل الآخر هو `prompt`، حيث أن `prompt` هو وصف الصورة التي نريد توليدها، يُنصح باستخدام الوصف باللغة الإنجليزية، حيث ستكون الصورة الناتجة أكثر دقة. هنا استخدمنا محتوى المثال `Lamborghini speeds inside a volcano`، والذي يمثل أننا نريد رسم لامبورغيني تتسارع داخل بركان.
...
- `mode`: وضع التوليد، يمكن اختيار `fast`/`relax`/`turbo`، الافتراضي هو fast. V8.1 يدعم فقط `fast` و `relax`.
- `version`: إصدار النموذج. V8.1 يستخدم دقة SD/HD، ولا يدعم معامل `quality`؛ SD يستهلك حوالي 0.8 دقيقة GPU، وHD يستهلك حوالي 1.3 دقيقة GPU.
- `timeout`: وقت المهلة (بالثواني)، ستعود النتيجة مباشرة عند انتهاء المهلة.
- `translation`: هل يتم الترجمة التلقائية لـ prompt غير الإنجليزية.
- `split_images`: هل يتم تقسيم نتائج 2x2 وإرجاع صورة واحدة.
- `action`/`image_id`: يجب تحديدها عند الاستمرار في إجراء عمليات على الصور التاريخية.
- `callback_url`: عنوان رد الاتصال غير المتزامن.
- `async`: اختياري، عند تعيينه إلى `true`، ستعود الواجهة مباشرة بـ `task_id`، دون الحاجة لتقديم `callback_url`، ثم يمكنك الاستعلام عن النتائج من خلال واجهة استعلام المهام المقابلة.

<p><img src="https://cdn.acedata.cloud/zv3db5.png" width="500" className="m-auto" /></p>

بعد الاستدعاء، نجد أن النتيجة العائدة هي كما يلي:
...
- `task_id`: معرف المهمة التي تم توليد هذه الصورة من أجلها، يستخدم لتحديد هوية مهمة توليد الصورة هذه بشكل فريد.
- `image_id`: المعرف الفريد للصورة، والذي يجب تمريره عند الحاجة إلى إجراء تغييرات على الصورة في المرة القادمة.
- `image_url`: عنوان URL للصورة المصغرة، يمكن فتحه مباشرة لرؤية التأثير الناتج.
- `image_width`: عرض الصورة المصغرة بالبكسل.
- `image_height`: ارتفاع الصورة المصغرة بالبكسل.
- `raw_image_url`: عنوان URL للصورة الأصلية، ومحتواها مشابه لمحتوى الصورة المصغرة، ولكنها أكثر وضوحًا، وقد تكون سرعة تحميلها أبطأ قليلاً.
- `raw_image_width`: عرض الصورة الأصلية بالبكسل.
- `raw_image_height`: ارتفاع الصورة الأصلية بالبكسل.
- `actions`: قائمة العمليات الإضافية التي يمكن إجراؤها على الصورة الناتجة. هنا تم إدراج 8 عمليات، حيث تمثل `upscale` التكبير، و`variation` التغيير. لذا، فإن `upscale1` تعني إجراء عملية تكبير على الصورة الأولى في الزاوية اليسرى العليا، و`variation3` تعني إجراء تغيير بناءً على الصورة الثالثة في الزاوية اليسرى السفلى.

عند فتح `image_url` أو `raw_image_url`، يمكنك اكتشاف ما هو موضح في الصورة.

![](https://cdn.acedata.cloud/qr2iyj.png)

يمكنك رؤية أنه تم توليد صورة معاينة 2x2. حتى الآن، تم الانتهاء من أول استدعاء API.

## 图像放大与变换

الآن سنحاول إجراء عمليات إضافية على الصورة التي تم توليدها حاليًا، على سبيل المثال، إذا كنا نعتقد أن الصورة الثانية في الزاوية اليمنى العليا جيدة، ولكننا نريد إجراء بعض التعديلات، فيمكننا ملء `action` كـ `variation2`، مع تمرير `image_id`:
...
يمكننا أن نرى أنه بالنسبة للصورة الموجودة في الزاوية اليمنى العليا، حصلنا مرة أخرى على أربع صور مشابهة.

في هذه الحالة، يمكننا اختيار واحدة منها لإجراء عملية تكبير دقيقة، على سبيل المثال، إذا اخترنا الصورة الرابعة، يمكننا تمرير `action` كـ `upscale4`، ومن ثم تمرير معرف الصورة الحالي مرة أخرى باستخدام `image_id`.
...
## إعادة كتابة الصورة (صورة أساسية)

تدعم هذه الواجهة أيضًا إعادة كتابة الصور، والمعروفة باسم الصورة الأساسية، حيث يمكننا إدخال رابط صورة بالإضافة إلى وصف النص الذي نريد إعادة كتابته، وستقوم هذه الواجهة بإرجاع الصورة المعدلة.
...
على سبيل المثال، لدينا هنا صورة لغروب الشمس على الطريق، مع بعض الأشجار والمباني بجانب الطريق، كما هو موضح في الصورة:

![](https://cdn.acedata.cloud/mq335u.png)

الآن نريد تعديلها لتصبح بجانب شاطئ البحر، مع وجود سيارة متوقفة على جانب الطريق. يمكننا بناء `prompt` كما يلي:

```bash
https://cdn.acedata.cloud/v014oc.png an illustration of a car parked on the beach --iw 2
```

يمكننا أن نرى أن بداية `prompt` لدينا هي رابط صورة يبدأ بـ HTTPS، ثم نضيف مسافة، تليها محتوى النص الخاص بـ `prompt`. هنا استخدمنا أيضًا بعض المعلمات المتقدمة الإضافية، مثل `—iw 2` لضبط وزن الصورة.
...
يمكننا أن نرى أنه، مع الحفاظ على أسلوب الصورة الأصلية وتكوينها، أصبح المشهد بأكمله بجانب الشاطئ، بينما ظهرت سيارة على الطريق، وهذا هو Prompt with Image.

## دمج الصور

تدعم هذه الواجهة البرمجية أيضًا دمج الصور، يمكننا تمرير عدة صور لتحقيق تأثيرات دمج مختلفة.
...
يمكننا أن نلاحظ، مثل Image with Prompt، هنا وضعنا عدة روابط صور في بداية prompt، ويفصل بينها مسافة، ثم نضيف النص prompt، ونمرر المحتوى أعلاه ككل إلى معلمة `prompt`، والنتيجة كما يلي:
...
تدعم هذه الواجهة البرمجية أيضًا وظيفة الرسم الجزئي للصور، ولكنها تدعم فقط الصور التي تم إنشاؤها في المحتوى السابق، يمكننا تمرير معرف فريد لصورة تم إنشاؤها، ومعلمة سلوك إعادة الرسم `action`، بالإضافة إلى قناع mask للمنطقة التي تحتاج إلى إعادة الرسم، لتحقيق إعادة الرسم في منطقة القناع تلك.
...
class MainWindow(QDialog):
    ...
    def init_ui(self):
        ...
        save_button = QPushButton("   بدء إعادة الرسم الجزئي   ")
...
من خلال الكود أعلاه يمكن الحصول على صورة القناع، وفي هذه العملية يجب أن نلاحظ أن صورة القناع يجب أن تكون بنفس أبعاد الصورة الأصلية، ثم المنطقة البيضاء في صورة القناع هي المنطقة التي تحتاج إلى إعادة الرسم، أدناه عرض مقارنة بين صورة القناع التي تحتاج إلى إعادة الرسم وصورة القط الأصلية:
...
- `prompt`: وصف لإعادة الرسم الجزئي للصورة (معامل اختياري).
...
لذا وفقًا للقواعد أعلاه، نحتاج إلى تعيين المعاملات بشكل صحيح، المعامل `prompt` هو معامل غير إلزامي، هنا لتسهيل المقارنة، تم تعيين منطقة القناع `prompt` إلى `A cute cat`، إعداد المعاملات المحددة كما هو موضح في الصورة أدناه:

<p><img src="https://cdn.acedata.cloud/p0hwrf.png" width="500" class="m-auto"></p>

### مثال على الكود

يمكن ملاحظة أنه تم توليد تلقائيًا على الجانب الأيمن من الصفحة كودات بلغات مختلفة، كما هو موضح في الصورة:
...
يمكن ملاحظة أن هذه العملية قد أجريت على منطقة القناع للصورة، والنتيجة التي تم إرجاعها تتوافق مع ما تم ذكره أعلاه، والنتيجة كما هو موضح في الصورة أدناه:

![](https://cdn.acedata.cloud/8vslhq.png)

يمكننا أن نرى أننا نجحنا في تنفيذ إعادة الرسم الجزئي للمنطقة المخصصة للصورة المولدة.
...
تقوم Midjourney رسميًا بتوليد الصور مع تقدم، في البداية تكون الصورة ضبابية، ثم بعد عدة تكرارات، تصبح الصورة تدريجياً أكثر وضوحًا، وفي النهاية نحصل على الصورة الكاملة.

لذا، يمكن تقسيم عملية توليد صورة واحدة تقريبًا إلى مراحل "إرسال الأمر" -> "بدء توليد الصورة (تكرارات متعددة تصبح أكثر وضوحًا)" -> "انتهاء توليد الصورة".
