...
ستقدم هذه الوثيقة شرحًا حول واجهة برمجة التطبيقات للتعرف على بروتوكول Recaptcha2، والتي تتيح للمستخدمين التحقق من صحة رموز Recaptcha2 دون الحاجة إلى التعرف عليها أو النقر على صور التحقق، بل يكفي تقديم مفتاح الموقع لتحقيق فك التشفير التلقائي في الخلفية وإكمال التحقق.
...
أولاً، يجب أن نفهم طريقة الاستخدام الأساسية، وهي إدخال عنوان URL للموقع الذي يحتاج إلى معالجة رمز التحقق، للحصول على النتيجة المعالجة. أولاً، نحتاج إلى تمرير حقل `website_url` ببساطة، وموقعنا التجريبي هو: `https://www.google.com/recaptcha/api2/demo`، نحتاج إلى الحصول على `website_key` من صفحة `website_url`، يجب أولاً فتح هذه الصفحة، ثم الضغط على F12 للدخول إلى وحدة التحكم، وأخيرًا البحث عالميًا في صفحة العناصر عن `recaptcha-demo`، وسنحصل على النتيجة التالية:
...
- `token`، نتيجة التحقق من مهمة Recaptcha2.

يمكننا أن نرى أننا حصلنا على نتيجة التحقق من Recaptcha2، ثم يمكننا استخدامها في POST أو محاكاة تقديمها إلى الموقع المستهدف، وهي صالحة للاستخدام مرة واحدة لمدة 120 ثانية، يُنصح باستخدامها خلال 60 ثانية. بعد ذلك، سنقدم جزءًا من كود Python لتقديم الـ token المعالج إلى الموقع المستهدف لتجاوز Recaptcha2.
