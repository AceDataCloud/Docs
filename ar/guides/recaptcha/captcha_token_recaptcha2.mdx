...
- `proxy`: اختياري، استخدم وكيل خاص بك (Bring Your Own Proxy). بعد الإعداد، سيقوم الخادم العلوي باستخدام عنوان IP الخاص بالوكيل الذي تقدمه لفك تشفير رمز التحقق، للتحكم في جودة عنوان IP الخارجي (على سبيل المثال، لتجنب حظر عنوان IP الخاص بالوكيل العام من قبل الموقع المستهدف وإرجاع `410 Gone`). التنسيق هو `scheme://[user:pass@]host:port`، حيث يدعم `scheme` `http`/`https`/`socks4`/`socks5`، مثل `http://user:pass@1.2.3.4:8080`. إذا لم يتم ملؤه، سيتم استخدام الوكيل الافتراضي للمنصة.
...
- `token`، نتيجة التحقق بعد معالجة مهمة Recaptcha2.
- `started_at`، `finished_at`: وقت بدء معالجة الطلب وإنتاج النتيجة، طابع زمني Unix (ثوانٍ، عائم).
- `elapsed`: إجمالي الوقت المستغرق في المعالجة (ثوانٍ).

يمكننا أن نرى أننا حصلنا على نتيجة التحقق من Recaptcha2، ثم يمكننا استخدامها في POST أو محاكاة تقديمها إلى الموقع المستهدف، للاستخدام لمرة واحدة، فترة صلاحيتها 120 ثانية، يُنصح باستخدامها خلال 60 ثانية، بعد ذلك سنقدم جزءًا من Python لإرسال الـ token المعالج إلى الموقع المستهدف لتجاوز Recaptcha2.

أولاً، نحتاج إلى معرفة كيفية إرسال الموقع لطلب POST، حتى نتمكن من تمرير الـ token الذي تم إنشاؤه، نحتاج أولاً إلى فتح وحدة التحكم F12، ثم نقوم بالتحقق يدويًا، وأخيرًا يمكننا أن نرى أن الموقع أرسل طلب POST، كل ما علينا هو مراجعة بناء هذا الطلب، العملية المحددة كما يلي:

- أولاً، تحقق يدويًا، كما هو موضح في الصورة أدناه:

<p><img src="https://cdn.acedata.cloud/gwiflr.png" width="500" className="m-auto" /></p>

- ثم انقر على submit، راقب تغييرات الشبكة في وحدة التحكم، كما هو موضح في الصورة أدناه:
...
بشكل افتراضي، تكون واجهة برمجة التطبيقات متزامنة وتنتظر: ستنتظر الطلب حتى يتم معالجة الرمز قبل أن تعود. إذا كنت تقوم بتدوير حل متعدد (multi-solver rotation) وترغب في "تقديم المهمة والحصول على task_id على الفور، ثم جدولة حل آخر، والعودة لاحقًا للحصول على النتيجة"، يمكنك تمرير `async: true` في جسم الطلب.
...
بعد ذلك، استخدم `task_id` هذا للاستعلام عن `POST /captcha/tasks` (يوصى كل 3~5 ثوانٍ) للحصول على النتيجة:
...
من خلال هذه الوثيقة، لقد فهمت كيفية استخدام واجهة برمجة التطبيقات الخاصة بـ Recaptcha2 للتعرف على البروتوكول بحيث لا يحتاج المستخدم إلى التعرف على أو النقر على صور تحقق Recaptcha2، بل يكفي تقديم مفتاح الموقع لتحقيق فك التشفير التلقائي في الخلفية وإكمال التحقق. نأمل أن تساعدك هذه الوثيقة في التكامل واستخدام هذه الواجهة بشكل أفضل. إذا كان لديك أي استفسارات، فلا تتردد في الاتصال بفريق الدعم الفني لدينا.
