...
ستقدم هذه الوثيقة شرحًا حول واجهة برمجة التطبيقات للتعرف على بروتوكول Recaptcha2، والتي تتيح للمستخدمين التحقق من رموز Recaptcha2 دون الحاجة إلى التعرف عليها أو النقر على صور التحقق، بل يكفي تقديم مفتاح الموقع لتحقيق فك التشفير التلقائي في الخلفية وإكمال التحقق.

## عملية التقديم

لاستخدام واجهة برمجة التطبيقات للتعرف على بروتوكول Recaptcha2، يجب أولاً الذهاب إلى [وحدة التحكم في Ace Data Cloud](https://platform.acedata.cloud/console/applications) للحصول على رمز API الخاص بك، احتفظ به للاستخدام لاحقًا.
...
حيث أن `data-sitekey` المقابل له هو سلسلة الأحرف التي تمثل قيمة `website_key`، وفيما يلي النتائج المحددة للمعلمات:
...
- `proxy`: اختياري، استخدم وكيل خاص بك (Bring Your Own Proxy). بعد الإعداد، سيقوم الخادم العلوي باستخدام عنوان IP الخاص بالوكيل الذي تقدمه لفك تشفير رمز التحقق، للتحكم في جودة عنوان IP الخارجي (على سبيل المثال، لتجنب حظر عنوان IP الخاص بالوكيل العام من قبل الموقع المستهدف وإرجاع `410 Gone`). التنسيق هو `scheme://[user:pass@]host:port`، حيث يدعم `scheme` كل من `http`/`https`/`socks4`/`socks5`، مثل `http://user:pass@1.2.3.4:8080`. إذا لم يتم ملؤه، سيتم استخدام الوكيل الافتراضي للمنصة.
...
يمكننا أن نرى أننا حصلنا على نتيجة التحقق من Recaptcha2، ثم يمكننا استخدامها في POST أو محاكاة تقديمها إلى الموقع المستهدف، للاستخدام مرة واحدة، صلاحيتها 120 ثانية، يُنصح باستخدامها خلال 60 ثانية، بعد ذلك سنقدم جزءًا من Python لإرسال التوكن المعالج إلى الموقع المستهدف لتجاوز Recaptcha2.

أولاً، نحتاج إلى معرفة كيفية إرسال الموقع لطلب POST، حتى نتمكن من تمرير التوكن الذي تم إنشاؤه، نحتاج أولاً إلى فتح وحدة التحكم F12، ثم نقوم بالتحقق يدويًا، وأخيرًا يمكننا رؤية أن الموقع أرسل طلب POST، كل ما علينا هو مراجعة بناء هذا الطلب، العملية المحددة كما يلي:
