...
ستقدم هذه المقالة شرحًا حول واجهة برمجة التطبيقات للتعرف على بروتوكول Recaptcha3، والتي تتيح للمستخدمين التحقق من صحة Recaptcha3 دون الحاجة إلى التعرف على الصور أو النقر عليها، بل من خلال تقديم مفتاح الموقع فقط لتحقيق فك التشفير التلقائي في الخلفية وإكمال التحقق.
...
افتح f12، ثم ابحث في صفحة العناصر عن `.execute(`، في منطقة الإطار الأحمر يمكننا رؤية معلمة `action`، بينما تتبعها سلسلة من الأحرف، وهذه أيضًا هي المحتويات المطلوبة في النص أدناه، كما هو موضح في الصورة أدناه.

<p><img src="https://cdn.acedata.cloud/imwlto.png" width="500" className="m-auto" /></p>

بعد ذلك، تحتاج أيضًا إلى إدخال عنوان URL للموقع الذي يحتاج إلى معالجة CAPTCHA، للحصول على النتيجة المعالجة، يجب أولاً تمرير حقل `website_url` ببساطة، وأخيرًا تحتاج إلى إدخال معلمة `website_key`، والتي يمكن الحصول عليها من النص أعلاه، وهي أيضًا سلسلة من الأحرف بعد execute. يمكننا بعد ذلك ملء المحتويات المقابلة في الواجهة، كما هو موضح في الصورة أدناه:
...
- `page_action`: يجب الحصول عليها من كود الموقع الذي يحتوي على CAPTCHA.
- `website_url`: عنوان URL للموقع الذي يحتاج إلى معالجة CAPTCHA.
- `website_key`: معرف مفتاح الموقع في Recaptcha3.

بعد الاختيار، يمكن ملاحظة أنه تم إنشاء كود مطابق على الجانب الأيمن، كما هو موضح في الصورة أدناه:
...
يمكننا أن نرى أننا حصلنا على نتيجة معالجة Recaptcha3، ثم يمكننا استخدامها في POST أو GET لمحاكاة الإرسال إلى الموقع المستهدف، للاستخدام مرة واحدة، صلاحيتها 120 ثانية، يُنصح باستخدامها خلال 60 ثانية. فيما يلي سنقدم بإيجاز طريقة لتقديم الـ token الناتج إلى الموقع المستهدف:
