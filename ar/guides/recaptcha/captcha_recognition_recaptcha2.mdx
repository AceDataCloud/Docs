...
بشكل افتراضي، تكون واجهة برمجة التطبيقات متزامنة وتنتظر: ستنتظر الطلب حتى يتم معالجة نتيجة التعرف. إذا كنت تقوم بتدوير عدة حلّات (multi-solver rotation) وترغب في "تقديم المهمة والحصول على task_id على الفور، ثم جدولة حلّات أخرى، والعودة لاحقًا لقراءة النتائج"، يمكنك تمرير `async: true` في جسم الطلب.
...
من خلال هذه الوثيقة، لقد فهمت كيفية استخدام واجهة برمجة التطبيقات للتعرف على صور Recaptcha2 لجعل المستخدم يدخل المحتوى المعترف به وصورة كود Recaptcha2، وأخيرًا إعادة إحداثيات الصورة الصغيرة التي تحتاج إلى النقر عليها، لإكمال التحقق. نأمل أن تساعدك هذه الوثيقة في الربط واستخدام هذه الواجهة بشكل أفضل. إذا كان لديك أي استفسارات، فلا تتردد في الاتصال بفريق الدعم الفني لدينا.
