...
> سيتم الاحتفاظ بالمهمة فقط إذا كان الطلب الأصلي للصور يحتوي على `callback_url`. لن يتم تخزين الطلبات التي تم استدعاؤها بطريقة متزامنة (غير استدعائية).
...
يجب تمرير `id` أو `trace_id` على الأقل. في معظم الحالات، يمكنك استخدام `id` الموجود في استجابة الطلب مباشرة، و`trace_id` يُستخدم فقط إذا كنت ترغب في ربطه بمعرف عملك الخاص.
...
- `type`: نوع واجهة المصدر. المهام المكتوبة بواسطة سلسلة `gpt-image` (مثل `gpt-image-2`) تكون من نوع `images`؛ بينما تستخدم القنوات القديمة لـ OpenAI (مثل `gpt-image-1`، nano-banana) `images_generations` / `images_edits`، وبعض واجهات الدردشة تكون من نوع `chat_completions_image`.
- `request`: جسم الطلب الكامل للطلب الأصلي.
- `response`: جسم الاستجابة النهائية التي تم إرجاعها من واجهة المصدر عند اكتمال الاستدعاء.
...
| `type`           | string   | تصفية حسب نوع المصدر (القيم: `images`، `images_generations`، `images_edits`) |
| `offset`         | int      | نقطة البداية للتقسيم، الافتراضي `0`                          |
| `limit`          | int      | عدد العناصر في الصفحة الواحدة، الافتراضي `12`                 |
| `created_at_min` | float    | الطابع الزمني الابتدائي (بالثواني Unix)                       |
| `created_at_max` | float    | الطابع الزمني النهائي (بالثواني Unix)                         |
...
```
## مثال من النهاية إلى النهاية: تقديم واستطلاع

تخدم واجهة برمجة التطبيقات للمهام بشكل رئيسي العمليات غير المتزامنة في وضع الاستدعاء. في وضع الاستدعاء، ستقوم واجهة تقديم الطلبات **بإرجاع `task_id`** (أي معرف المهمة) على الفور، وبعد ذلك يمكنك فقط استخدام هذا `task_id` لاستطلاع واجهة المهام، دون الحاجة إلى إنشاء `trace_id` بنفسك.
...
# 1. تقديم مهمة إنشاء صورة (وضع الاستدعاء: فقط أضف callback_url وسيتم إرجاع task_id على الفور)
...
# 2. استخدم مباشرة task_id من استجابة التقديم لاستطلاع واجهة المهام حتى تكتمل المهمة
while True:
    task = requests.post(
        f"{API}/openai/tasks",
        headers=HEADERS,
...
- واجهة المهام نفسها **لا تتقاضى رسومًا**، يمكنك الاستطلاع بحرية. فقط الطلبات الأصلية لإنشاء/تحرير الصور ستتحمل الرسوم.
