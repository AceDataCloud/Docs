...
تستخدم واجهة برمجة تطبيقات OpenAI Tasks للاستعلام عن نتائج المهام التي تم تقديمها سابقًا إلى واجهة OpenAI للصور بنمط **الاستدعاء**. عندما لا يمكنك الانتظار لاستجابة HTTP المتزامنة، أو ترغب في الاستعلام عن المهمة لاحقًا، يرجى استخدام هذه الواجهة.
...
يجب تمرير `id` أو `trace_id` على الأقل. في معظم الحالات، يمكنك استخدام `id` الموجود في استجابة التقديم مباشرة، و`trace_id` يُستخدم فقط إذا كنت ترغب في ربطه بمعرف عملك الخاص.
...
- `type`: نوع المهمة. المهام المكتوبة في سلسلة `gpt-image` (مثل `gpt-image-2`) تكون من نوع `images`؛ بينما `gpt-image-1`، nano-banana وغيرها تستخدم `images_generations` / `images_edits`، وبعض واجهات الدردشة تكون من نوع `chat_completions_image`.
- `request`: جسم الطلب الكامل للطلب الأصلي.
- `response`: جسم الاستجابة النهائية التي تم إرجاعها عند اكتمال الاستدعاء.
- `created_at` / `finished_at`: طوابع زمنية Unix (بالثواني). `started_at` هو سلسلة ISO 8601.
- `elapsed`: الوقت المستغرق (بالثواني، كعدد عشري).
  ...
  "items": [
    {
      ...
      "type": "images",
      "request": { "model": "gpt-image-2", "prompt": "قطة" },
...
تعمل واجهة برمجة التطبيقات للمهام بشكل رئيسي في وضع الاستدعاء غير المتزامن. في وضع الاستدعاء، ستقوم واجهة تقديم الطلبات **بإرجاع `task_id`** (أي معرف المهمة) على الفور، وبعد ذلك يمكنك فقط استخدام هذا `task_id` لاستطلاع واجهة المهام، دون الحاجة إلى إنشاء `trace_id` بنفسك.
...
# 1. تقديم مهمة توليد صورة (وضع الاستدعاء: أضف callback_url وستحصل على task_id على الفور)
...
- سيتم كتابة سجل المهمة فقط عندما يحتوي الطلب الأصلي على `callback_url`؛ الاستدعاءات المتزامنة لن تنتج عنها مهام قابلة للاستعلام.
