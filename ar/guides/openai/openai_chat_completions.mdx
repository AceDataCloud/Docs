...
لاستخدام OpenAI Chat Completion API، يجب أولاً الذهاب إلى [لوحة تحكم Ace Data Cloud](https://platform.acedata.cloud/console/applications) للحصول على رمز API الخاص بك، للاحتفاظ به كنسخة احتياطية.
...
> 📘 الوثائق الكاملة: [OpenAI Chat Completion API →](https://platform.acedata.cloud/documents/openai-chat-completions)
...
عند استخدام هذه الواجهة لأول مرة، نحتاج على الأقل إلى ملء ثلاثة محتويات، أحدها هو `authorization`، والذي يمكن اختياره مباشرة من القائمة المنسدلة. المعامل الآخر هو `model`، حيث يمثل `model` نوع النموذج الذي نختار استخدامه من موقع OpenAI ChatGPT الرسمي، وهنا لدينا 20 نوعًا من النماذج، يمكن الاطلاع على التفاصيل في النماذج التي نقدمها. المعامل الأخير هو `messages`، حيث أن `messages` هو مصفوفة كلمات الاستفسار التي ندخلها، وهي مصفوفة تمثل إمكانية تحميل عدة كلمات استفسار في نفس الوقت، حيث تحتوي كل كلمة استفسار على `role` و `content`، حيث يمثل `role` دور المستفسر، وقد قدمنا ثلاث هويات، وهي `user` و `assistant` و `system`. أما `content` فهو المحتوى المحدد لاستفسارنا.
...
- `model`، النموذج الذي تم اختياره من OpenAI ChatGPT الرسمي.
- `choices`، معلومات الرد التي قدمها ChatGPT على كلمات الاستفسار.
- `usage`، معلومات إحصائية حول الرموز المستخدمة في هذه المحادثة.

حيث أن `choices` تحتوي على معلومات رد ChatGPT، ويمكن ملاحظة أن `choices` تحتوي على الرد من ChatGPT، كما هو موضح في الصورة.
...
تعديل كما هو موضح في الصورة، ولكن يجب أن يكون هناك تغييرات مطابقة في كود الاستدعاء لدعم الاستجابة المتدفقة.
...
data: {"choices": [{"delta": {"content": " يمكنني", "role": "assistant"}, "index": 0}], "created": 1721007348, "id": "chatcmpl-YzczYjVhNjhjMzMwNDQ5MDkyNGYzOGZjZGE1ZGQ5OGU", "model": "gpt-4", "object": "chat.completion.chunk", "recipient": "all"}

data: {"choices": [{"delta": {"content": " مساعدتك", "role": "assistant"}, "index": 0}], "created": 1721007348, "id": "chatcmpl-YzczYjVhNjhjMzMwNDQ5MDkyNGYzOGZjZGE1ZGQ5OGU", "model": "gpt-4", "object": "chat.completion.chunk", "recipient": "all"}
...
## الاتصال بـ OpenAI-Python

تتوافق واجهة برمجة تطبيقات OpenAI Chat Completion مع واجهة OpenAI الرسمية، ويمكن استخدام SDK الرسمي [OpenAI-Python](https://github.com/openai/openai-python) للاتصال، وسنقدم هنا لمحة سريعة عن كيفية الاستخدام.
...
افترض أننا أنشأنا ملف كود مثال باسم `index.py`، والمحتوى كما يلي:
...
بالإضافة إلى gpt-4o، هناك نموذج آخر بتكلفة أقل يسمى gpt-4o-mini. gpt-4o-mini هو الجيل الأحدث من نماذج اللغة الكبيرة التي طورتها OpenAI، حيث إنه سريع الاستجابة وأيضًا أرخص، ويدعم أيضًا النماذج متعددة الوسائط. يمكن الرجوع إلى محتوى استخدام نموذج gpt-4o أعلاه لاستخدام وظيفة الرؤية.
...
يمكن رؤية أن الصورة المرجعية هي صورة حقيقية، يمكننا جعلها تتغير إلى أسلوب معين، مثل تحويلها إلى صورة بأسلوب أنمي، مثال الطلب المحدد:
...
حيث أن `choices` في `message.content` هو نتيجة الحوار الكاملة التي تم إنشاؤها، والصورة مضمنة بتنسيق Markdown (رابط الصورة هو عنوان مؤقت، يرجى تنزيلها وحفظها في الوقت المناسب). يمكن رؤية أن الصورة المولدة هي بالفعل بأسلوب أنمي، كما هو موضح في الصورة أدناه:

<p><img src="https://cdn.acedata.cloud/qmr391.jpg" width="400" class="m-auto"></p>

### إنشاء صورة نصية بحتة

يمكننا من خلال عبارة توجيهية أن نجعلها تولد صورة، وتعود إلينا بنتيجة حوارية، دعنا نأخذ `إنشاء صورة لمدينة مستقبلية عند غروب الشمس` كمثال، إليك المثال المحدد:
...
يمكن رؤية أن النتيجة تتوافق مع العبارة التوجيهية، كما هو موضح أدناه:
...
دعنا نأخذ `إنشاء صورة لشاب يحمل فنجان قهوة، ويبدو أنه على وشك الشرب` كمثال، إليك المثال المحدد:
```json
{
  "model": "gpt-4o-image",
  "messages": [
    {
      ...
      "content": [
        {
          "type": "text",
          "text": "إنشاء صورة لشاب يحمل فنجان قهوة، ويبدو أنه على وشك الشرب"
        },
        {
          "type": "image_url",
          "image_url": {
...
يمكنك أن ترى أن النتيجة المولدة هي بالفعل مزيج من الصورتين، وفيما يلي النتيجة المحددة:
...
من خلال هذه الوثيقة، لقد فهمت كيفية استخدام واجهة برمجة تطبيقات OpenAI Chat Completion لتحقيق وظائف المحادثة الخاصة بـ OpenAI ChatGPT بسهولة. نأمل أن تساعدك هذه الوثيقة في التوصيل والاستخدام الأفضل لهذه الواجهة. إذا كان لديك أي استفسارات، فلا تتردد في الاتصال بفريق الدعم الفني لدينا.
