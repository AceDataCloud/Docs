...
> 传 `size: "auto"`（默认）时，模型会**读取提示词本身来决定画幅**：写「超宽电影感横幅 21:9」会得到 `3024x1296`，写「竖版海报 9:16」会得到 `1440x2560`，没有画幅暗示的提示词则回到 `2048x2048`。所以当你心里有构图但不想算具体像素时，`auto` 通常比硬填一个尺寸更合适。省略 `size` 字段的效果与 `auto` 相同。
...
payload = {
    "model": "gpt-image-2",
    "prompt": "صورة سينمائية لامرأة شابة واقفة في متجر صغير في الليل، مضاءة بواسطة لافتات نيون وردية وزرقاء ناعمة من خلال النافذة. تم تصويرها على فيلم 35 مم، عمق ميدان ضحل، حبيبات خفيفة، مزاج حزين.",
    "size": "1024x1536"
}
...
```json
{
  ...
  "data": [
    {
      "revised_prompt": "صورة سينمائية لامرأة شابة واقفة في متجر صغير في الليل، مضاءة بواسطة لافتات نيون وردية وزرقاء ناعمة من خلال النافذة. تم تصويرها على فيلم 35 مم، عمق ميدان ضحل، حبيبات خفيفة، مزاج حزين.",
...
`gpt-image-2` يظهر استقرارًا في الطباعة ورسم الخطوط، مما يجعله مثاليًا لإنشاء الملصقات، القوائم، بطاقات التهنئة وغيرها من التصاميم التي تحتوي على نصوص.
...
| `nano-banana-pro`    | 0.35            | الرائد في السلسلة، أفضل في التركيب والتفاصيل والنصوص             |
...
> - سيتم تعيين `size` وفقًا للجدول أدناه إلى `aspect_ratio` داخلي، الأبعاد غير المدرجة ستتحول إلى `1:1`:
>   - `1024x1024` / `512x512` / `256x256` → `1:1`
>   - `1792x1024` → `16:9`
>   - `1024x1792` → `9:16`
> - لا تدعم المعلمات `quality`، `style`، `response_format`، `background`، `output_format`، وما إلى ذلك؛ إذا تم إدخالها، سيتم تجاهلها. `n > 1` مدعوم (1–10)، سيعود ويحسب عدد الصور المقابلة.
> - هيكل العودة يتبع تنسيق OpenAI (`data[].url`)، لكن `created` ثابت عند `0`، ولن يتم إرجاع `b64_json`، و`revised_prompt` دائمًا يساوي `prompt` الأصلي.
...
```json
{
  ...
  "data": [
    {
      "url": "https://platform.cdn.acedata.cloud/nano-banana/your_image.png"
    }
  ]
}
```
...
```json
{
  ...
  "data": [
    {
      "revised_prompt": "صورة رائعة تعرض عجل بحري صغير، وُلد بلون بني، مع عيون ساحرة واسعة. إنه مستلقٍ بشكل رائع على ظهره، يتجدف في مياه البحر الهادئة. يبدو فروه الكثيف والناعم مبللاً ولامعًا، مما يعكس جوهر موطنه. الكائن الصغير يلعب بفضول مع صدفة بحرية بأقدامه الصغيرة، ويبدو بريئًا وساحرًا تمامًا في بيئته الطبيعية.",
...
```json
{
  ...
  "data": [
    {
      "revised_prompt": "عجل بحري صغير لطيف مستلقٍ بشكل مرح على ظهره في الماء، مع فروه يبدو لامعًا وناعمًا. إحدى قدميه الصغيرة تمتد بفضول، ولديه تعبير عن الفرح والدفء على وجهه وهو ينظر إلى السماء. جسمه محاط بف bubbles من دوارته المرحة في الماء. نسيم لطيف يلعب بفروه مما يجعله يبدو أكثر سحرًا. المشهد يعكس هدوء وسحر الحياة البحرية.",
...
```json
{
  ...
  "data": [
    {
      "revised_prompt": "صورة ساحرة لأوتير بحر صغير. يُرى الأوتير وهو يستريح بهدوء على ظهره وسط الأمواج الزرقاء اللطيفة. فراء الأوتير الصغير مزيج جذاب من درجات البني الرمادي الناعمة، يلمع برفق في ضوء الشمس الخافت. يلمس كفوفه الصغيرة، مرفوعة قليلاً نحو السماء كما لو كان يلعب مع شيء غير مرئي. عيونه الدائرية المعبرة واسعة بدافع الفضول، تتلألأ بالحياة والبراءة. استخدم أسلوبًا واقعيًا لاستحضار موطن الأوتير الطبيعي ومظهره الخارجي الرقيق بشكل جذاب.",
  ...
  "data": [
    {
      ...
      "revised_prompt": "صورة ساحرة لأوتير بحر صغير. الأوتير يطفو برفق على بحر أزرق هادئ، يستمتع بأشعة الشمس الذهبية الدافئة المتدفقة من سماء صافية فوقه. فراء الأوتير بني غامق غني، ويبدو ناعمًا ورقيقًا بشكل لا يصدق. عيون الأوتير مشرقة ومعبرة، مليئة بالفضول والفرح الطفولي. لديه آذان صغيرة منتصبة وأنف يشبه الزر مما يضيف إلى جاذبيته العامة. في البحر من حوله، يمكن رؤية قطرات الماء المتلألئة، التي تضيء بأشعة الشمس، المنظر بالتأكيد رائع."
    }
  ]
}
```
...
نظرًا لأن وقت توليد الصور بواسطة واجهة برمجة تطبيقات OpenAI Images Generations قد يكون طويلًا نسبيًا، إذا لم تستجب واجهة برمجة التطبيقات لفترة طويلة، ستظل طلبات HTTP متصلة، مما يؤدي إلى استهلاك موارد النظام الإضافية، لذا توفر هذه الواجهة أيضًا دعمًا للردود غير المتزامنة.
...
```json
{
  ...
  "data": {
    ...
    "data": [
      {
        "revised_prompt": "صورة رائعة تعرض أوتير بحر صغير...",
...
يمكننا أن نرى أن النتيجة تحتوي على حقل `task_id`، وحقل `data` يحتوي على نفس نتيجة توليد الصورة كما في الاستدعاء المتزامن، من خلال حقل `task_id` يمكن ربط المهمة.
