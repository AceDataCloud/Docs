...
OpenAI خدمة تعديل الصور، يمكن إدخال أي عدد من الصور والتعليمات، وإخراج الصور المعدلة. حاليًا، تدعم الواجهة كل من `dall-e-2`، `gpt-image-1`، وأحدث **`gpt-image-2`**، بالإضافة إلى نماذج سلسلة **`nano-banana` / `nano-banana-2-lite` / `nano-banana-2` / `nano-banana-pro`** المتصلة من خلال نفس الواجهة.
...
curl -X POST "https://api.acedata.cloud/openai/images/edits" \
  ...
  -d '{
    ...
    "prompt": "قم بتحويل هذه المعلومات الرسومية إلى وضع الظلام: خلفية بحرية داكنة، نص كريمي فاتح، بطاقات وحدات رمادية داكنة مستديرة مع ظلال ناعمة. احتفظ بجميع التخطيطات والهياكل وترتيب الوحدات كما هي - فقط عكس نظام الألوان.",
    "size": "1024x1536"
  }'
```
...
```json
{
  ...
  "data": [
    {
      "revised_prompt": "قم بتحويل هذه المعلومات الرسومية إلى وضع الظلام: خلفية بحرية داكنة، نص كريمي فاتح، بطاقات وحدات رمادية داكنة مستديرة مع ظلال ناعمة. احتفظ بجميع التخطيطات والهياكل وترتيب الوحدات كما هي - فقط عكس نظام الألوان.",
...
> يتصل Nano Banana عبر طبقة التكيف ببروتوكول OpenAI، ويدعم فقط المعلمات التالية: `model`، `prompt`، `image`.
...
> - يتبع هيكل الإرجاع تنسيق OpenAI (`data[].url`)، لكن `created` ثابت عند `0`، ولن يتم إرجاع `b64_json`، و`revised_prompt` دائمًا يساوي `prompt` الأصلي.
...
آلية ردود `callback_url` غير المتزامنة فعالة أيضًا لـ nano-banana، وتدفق الاستدعاء متطابق تمامًا مع النماذج الأخرى، انظر القسم التالي [ردود غير متزامنة](#ردود-غير-متزامنة).
...
بهذا نكون قد أكملنا عملية تحرير الصورة، حاليًا تدعم واجهة Edits ثلاثة نماذج: `dall-e-2`، `gpt-image-1` و `gpt-image-2`، حيث أن `gpt-image-2` هو النموذج الموصى به حاليًا، انظر القسم السابق [نموذج GPT-Image-2](#نموذج-gpt-image-2).

## ردود غير متزامنة

نظرًا لأن وقت تحرير الصور عبر OpenAI Images Edits API قد يكون طويلًا نسبيًا، إذا لم يكن هناك استجابة من API لفترة طويلة، ستظل طلبات HTTP متصلة، مما يؤدي إلى استهلاك موارد النظام الإضافية، لذا توفر هذه الواجهة أيضًا دعمًا للردود غير المتزامنة.
