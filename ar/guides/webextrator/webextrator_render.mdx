...
Render هو واجهة WebExtrator الأساسية. إذا كنت بحاجة إلى نتائج **مهيكلة** (نص المقال، أسعار المنتجات، مكونات الوصفات ...)، يرجى استخدام
[`/webextrator/extract`](development_webextrator_extract) — حيث يتم تشغيل مجموعة كاملة من خطوط استخراج الأنواع على نفس أساس العرض.
...
**يمكن استخدام رمز API واحد لاستدعاء جميع خدمات المنصة، دون الحاجة لتقديم طلب منفصل لكل خدمة.** عند التقديم لأول مرة، ستحصل على رصيد مجاني لتجربته؛ عند نفاد الرصيد، يمكنك إعادة شحن الرصيد العام في [لوحة التحكم](https://platform.acedata.cloud/console/coin).
...
| `delay`             | number   |  ❌  | `0`                          | **عدد الثواني الإضافية** للانتظار بعد تفعيل `wait_until` (لإعادة عرض SPA).         |
| `wait_for_selector` | string   |  ❌  | —                            | الانتظار لظهور هذا المحدد CSS، أكثر استقرارًا من `networkidle`.                  |
...
| `cookies`           | array    |  ❌  | —                            | ملفات تعريف الارتباط المدخلة قبل التنقل، الهيكل كما هو موضح أدناه.              |
...
| `async`             | boolean  |  ❌  | `false`                      | تعيينها إلى `true` لإرجاع `task_id` على الفور، واسترجاع النتائج عبر `callback_url` أو واجهة برمجة تطبيقات المهام. |
...
تقوم المنصة بإرسال `POST` بنفس envelope **تمامًا** مثل وضع التزامن إلى `callback_url`،
`Content-Type: application/json`. تعتبر أي استجابة `2xx` بمثابة تأكيد؛ بينما سيتم إعادة المحاولة مع تراجع أسي لمدة حوالي 5 دقائق في حالة `5xx`.
...
يرجع على الفور `{ "success": true, "task_id": "...", "trace_id": "...", "started_at": 1777717800.123 }`؛
...
- **تجاهل مفتاح التخزين المؤقت لـ `async`.** طلبات URL المتزامنة وغير المتزامنة لنفس العنوان تضرب نفس إدخال التخزين المؤقت،
