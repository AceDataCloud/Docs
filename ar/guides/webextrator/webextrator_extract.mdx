...
2. **استخراج LLM المصنف** — يتم تفعيله فقط عندما لا يتم العثور على schema.org. اختيار Schema حسب نوع الصفحة، تحقق صارم باستخدام Zod.
3. **Readability + Markdown كخطة بديلة** — تعمل دائمًا، تكمل الحقول العليا التي لم يتم ملؤها في الطبقتين السابقتين.

طلبات URL المتكررة سيتم التقاطها بواسطة ذاكرة التخزين المؤقت Redis، &lt;1 ms للعودة.

## عملية التقديم

لاستخدام صفحة خدمة WebExtrator، يجب أولاً الذهاب إلى [وحدة التحكم Ace Data Cloud](https://platform.acedata.cloud/console/applications) للحصول على رمز API الخاص بك، احتفظ به للاستخدام لاحقًا.
...
**رمز API واحد يكفي لاستدعاء جميع خدمات المنصة، دون الحاجة لتقديم طلب منفصل لكل خدمة.** عند التقديم لأول مرة، ستحصل على رصيد مجاني لتجربته؛ عند نفاد الرصيد، يمكنك إعادة شحن الرصيد العام في [وحدة التحكم](https://platform.acedata.cloud/console/coin).
...
| `description`   | string?  | الأولوية: `<meta name="description" />` → `og:description` → استخراج schema.org / LLM → اقتباس من الفقرة الأولى.   |
...
| `links`         | string[] | ما يصل إلى 100 رابط خارجي، تم تصفيتها من المقاطع / `javascript:` / `mailto:` / `tel:`。                               |
...
| `cacheStoredAt` | number?  | الطابع الزمني بالمللي ثانية Unix عند كتابة إدخال الذاكرة المؤقتة لأول مرة.                                                                |
...
```json
{
  ...
  "points": 173,
  "commentCount": 42,
  "body": "مرحبًا HN، قمنا ببناء بديل مستضاف ذاتيًا لواجهة برمجة تطبيقات Diffbot Analyze ..."
}
```
...
console.log(recipe.name, recipe.cookTime, recipe.ingredients.length, 'نوع من المكونات');
```

## نصائح ومشاكل

- **إذا كان بالإمكان تمرير `expected_type`، فافعل ذلك.** نصيحة مجانية، تخطى الحكم الاستدلالي، مفيد بشكل خاص للصفحات التي لا تتواجد في القائمة المدمجة.
...
- **عند التصحيح، تحقق أولاً من `rawSignals.hasJsonLd`.** إذا كانت `true` ولكن `structured.schemaOrg.primary` كانت `null`، فهذا يعني أن الصفحة استخدمت نوعًا لم يتم تغطيته بواسطة المترجم الخاص بنا - قدم مشكلة وسنضيفه.
- **`structured.llmError` هو معلوماتي.** لا يزال الطلب ناجحًا، ولا يزال يتم إرجاع النتائج الاستدلالية. تحقق من `llmError.error` لتحديد السبب (انتهاء الوقت، فشل تحليل JSON، فشل تحقق Zod).
...
- **تُحتسب أيضًا الضربات على التخزين المؤقت.** التخزين المؤقت مخصص لتقليل التأخير وحماية مجموعة المتصفح، وليس لتوفير المال.
