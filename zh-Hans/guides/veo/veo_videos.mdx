...
首先先了解下基本的使用方式，就是输入提示词 `prompt`、 生成行为 `action`、首尾帧参考图片数组 `image_urls` 以及模型 `model`，便可获得处理后的结果，首先需要简单地传递一个 `action` 字段，它的值为 `text2video`，它主要包含三种行为：文生视频（`text2video`）、图生视频（`image2video`）、获取1080p视频（`get1080p`），然后我们还需要输入模型 `model`，目前主要有 `veo31-fast`、`veo3`、`veo31`、`veo2-fast`、`veo3-fast` 和 `veo31-fast-ingredients` 模型，具体的内容如下：
...
- `model`：生成视频的模型，主要有 `veo31-fast`、`veo3`、`veo31`、`veo2-fast`、`veo3-fast` 和 `veo31-fast-ingredients` 模型。
...
- `model`：生成视频的模型，主要有`veo31-fast`、`veo3`、`veo31`、`veo2-fast`、`veo3-fast` 和 `veo31-fast-ingredients`。
