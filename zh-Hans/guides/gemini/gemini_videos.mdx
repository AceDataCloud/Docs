...
支持直接对已有视频进行编辑或参考：在 `video_urls` 中传入一个参考视频链接（最多 1 个），并**同时**在 `image_urls` 中提供至少一张参考图（上游硬性要求）。可通过 `prompt` 描述想要的编辑效果（改风格、换场景、增删元素等）：
...
> 注意：参考视频最多 1 个；且提供 `video_urls` 时必须提供至少一张 `image_urls`（否则会返回参数错误）。
