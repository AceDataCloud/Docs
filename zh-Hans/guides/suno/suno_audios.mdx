...
- `duration`：期望的歌曲时长，单位秒，需为整数，通常建议在 10 到 360 之间。该参数主要用于自定义模式（`custom` 为 `true`）的歌曲生成，部分模型或操作可能不支持，此时该值会被忽略或返回错误。这是目标时长，实际成品时长以响应中的 `duration` 字段为准，可能有小幅偏差。
...
默认情况下生成的歌曲时长由模型自行决定，通常在 30 秒到 4 分钟之间。如果需要更长或更短的成品，可以通过 `duration` 参数指定期望时长，单位是秒，取值为整数，通常建议在 10 到 360 之间。

该参数主要面向自定义模式（`custom` 为 `true`）下的歌曲生成，配合较新的模型（如 `chirp-v5-5`）效果最好。不同模型和操作对时长控制的支持程度不同，若当前组合不支持，该参数可能被忽略或返回错误提示，可根据返回结果调整。
...
需要注意的是，请求中的 `duration` 是**期望时长**，而响应 `data` 中每首歌曲的 `duration` 字段是该首歌曲的**实际时长**。两者名称相同但含义不同：`duration` 是一个倾向性提示而非硬性约束，模型会参考它但不保证达到，实测中实际时长通常短于期望值，同一次请求返回的两首歌曲时长也可能相差较大。歌词长度、风格和模型都会影响最终成品，若需要较长的成品，建议同时提供更完整的歌词。

如果某个模型或操作不支持时长控制，接口可能忽略该参数或返回错误提示，建议先用一次请求确认效果再批量使用。
