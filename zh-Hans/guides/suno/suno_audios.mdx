...
- `duration`：期望的歌曲时长，单位秒，取值 10-360 的整数。仅在 `action` 为 `generate`、`custom` 为 `true` 且 `model` 为 `chirp-v5-5` 时生效，其余组合会返回 400。这是目标时长，实际成品时长以响应中的 `duration` 字段为准，可能有小幅偏差。
...
需要注意的是，请求中的 `duration` 是**期望时长**，而响应 `data` 中每首歌曲的 `duration` 字段是该首歌曲的**实际时长**。两者名称相同但含义不同，实际时长会围绕期望值浮动，不保证精确相等。
