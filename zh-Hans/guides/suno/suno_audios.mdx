...
- `variety`：生成结果的多样性强度，可选 `off`、`normal`、`high`、`extra`、`max`；不需要控制多样性时可省略。
...
{
  "action": "add_stem",
  "audio_id": "1981d50e-fcc1-43ca-8588-b8ae700e1d38",
  "stem_type": "bass",
  "model": "chirp-v6",
  "prompt": "Add warm bass guitar",
  "title": "Add bass audit",
...
双通道实测均返回两个约 10 秒的候选音频。按公共字段归一化后，以下是双通道实测数据按公共字段投影后的终态 JSON；生产响应在媒体持久化完成后包含 `task_id`、`trace_id` 和 `audio_url`：
...
```json
{
  ...
  "data": [
    {
      "id": "4560586e-c670-4dfb-bbaa-e29e50762514",
      "title": "Add bass audit",
      "duration": 10.0,
      "state": "succeeded",
      "model": "chirp-v6"
    ...
    {
      "id": "c9118413-32a6-4146-a2c6-5207833527d1",
      "title": "Add bass audit",
      "duration": 10.0,
      "state": "succeeded",
      "model": "chirp-v6"
...
双通道实测均从一条约 10 秒的音频中返回两组 Bass 候选。按公共字段归一化后，`data` 保留扁平列表，`stem_sets` 提供目标音轨和互补音轨之间的稳定对应关系：
...
```json
{
  ...
  "data": [
    {
      "id": "63398395-7a5f-4443-979f-a94a9f53549b",
      "title": "Capability audit (Bass)",
      "duration": 10.0,
      "state": "succeeded",
      "stem_set": 1
    },
    {
      "id": "794396eb-6335-4487-a27c-af186158f295",
      "title": "Capability audit (Without Bass)",
      "duration": 10.0,
      "state": "succeeded",
      "stem_set": 1
    },
    {
      "id": "00568c79-4603-4e35-9955-f1fd7089eb71",
      "title": "Capability audit (Bass)",
      "duration": 10.0,
      "state": "succeeded",
      "stem_set": 2
    ...
    {
      "id": "e486f90b-66ef-425b-9d3c-4493469d28a6",
      "title": "Capability audit (Without Bass)",
      "duration": 10.0,
      "state": "succeeded",
      "stem_set": 2
  ...
  "stem_sets": [
    ...
    },
    {
      ...
      "remainder": {
        "id": "e486f90b-66fe-425b-9d3c-4493469d28a6",
        "state": "succeeded",
        "stem_set": 2
      }
    }
...
- 可选：`model`、`bpm`、`key`、`audio_format`。
- `sound_type`：可选 `one-shot` 或 `loop`。
- 计费：0.112 Credits。
...
双通道实测均成功生成音效。其中一次调用返回两个结果，时长分别为 16.0 秒和 17.96 秒；以下是双通道实测数据按公共字段投影后的终态 JSON；生产响应在媒体持久化完成后包含 `task_id`、`trace_id` 和 `audio_url`：
...
```json
{
  ...
  "data": [
    {
      "id": "49277ed4-7252-4ea7-8dc7-cf5e876ff730",
      "title": "Single soft analog synth pluck, no reverb",
      "duration": 16.0,
      "state": "succeeded"
    },
    {
      "id": "9a85d035-5294-4964-81f0-162f9eec7112",
      "title": "Single soft analog synth pluck, no reverb",
      "duration": 17.96,
      "state": "succeeded"
    }
...
- 必填：`action`、`audio_id`、`speed_multiplier`、`keep_pitch`、`title`。
- `speed_multiplier`：范围 0.25–4。
- `keep_pitch`：是否在变速时保持原音高。
- 计费：0.28 Credits。
...
双通道实测均完成 1.1 倍速处理。其中一次 9.6 秒输入得到约 8.73 秒结果；以下是双通道实测数据按公共字段投影后的终态 JSON；生产响应在媒体持久化完成后包含 `task_id`、`trace_id` 和 `audio_url`：
...
```json
{
  ...
  "data": [
    {
      "id": "8f8a269e-a055-4d61-8c19-37fa21721fb0",
      "title": "Faster Version",
      "duration": 8.73,
      "state": "succeeded",
      "model": "chirp-v6"
