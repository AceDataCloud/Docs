...
要使用提取 ADSL 的 IP 和对 ADSL Proxy 添加 IP 白名单，首先可以到 [ADSL Proxy Extract API](https://platform.acedata.cloud/documents/adsl-extract) 和 [ADSL Proxy Whitelist API](https://platform.acedata.cloud/documents/adsl-whitelist) 页面点击「Acquire」按钮，获取请求所需要的凭证：
...
接下来就可以在界面 [ADSL Proxy Extract API](https://platform.acedata.cloud/documents/adsl-extract) 上填写对应的内容，接下来演示如何提取 ADSL 的 IP。
