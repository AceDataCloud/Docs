...
- **输出模式**：默认同步等待生成完成并返回完整 `task`；传 `async: true` 或 `callback_url` 时改为异步
- **结果查询**：异步模式或历史任务可通过 [MiniMax H3 任务查询 API](/documents/minimax-tasks-integration) 获取状态和成片
...
默认同步模式下，调用 `/minimax/videos` 后保持连接；生成完成时直接从响应的 `task.content.url` 获取视频。

需要立即释放连接时，传 `async: true` 或 `callback_url`：
...
| `async` | boolean | 否 | `false` | `false` 时同步返回完整结果；`true` 时立即返回任务标识，随后通过任务查询接口获取结果 |
...
传入 `callback_url` 会自动启用异步模式：创建接口立即返回 `task_id` 和 `trace_id`，任务完成后向该地址 POST 最终结果，结构与任务查询响应一致。
