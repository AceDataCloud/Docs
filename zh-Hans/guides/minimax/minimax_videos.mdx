...
接口不接受旧版或兼容字段，例如 `prompt`、`image_urls`、`audio_urls`、`messages`、`first_frame_image` 和 `async`。收到这类参数错误时，请删除旧字段并迁移到 `content`；例如把 `"prompt": "一只猫挥手"` 改为 `"content": [{"type": "text", "text": "一只猫挥手"}]`。不要同时发送新旧两种格式。
