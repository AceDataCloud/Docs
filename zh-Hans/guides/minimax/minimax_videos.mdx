...
- **输出模式**：固定异步，成功提交后立即返回 `task_id` 与 `trace_id`
...
调用 `/minimax/videos` 后会立即返回任务标识，长任务在后台执行；无需传 `async`。
...
| `async` | boolean | 否 | `true` | 兼容字段；接口始终异步并立即返回任务标识 |
| `callback_url` | string | 否 | - | 接收最终任务结果的公网回调 URL |
