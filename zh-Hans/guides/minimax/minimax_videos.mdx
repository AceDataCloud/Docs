...
`POST https://api.acedata.cloud/minimax/videos` 使用 MiniMax H3 的 V2 多模态 `content` 结构创建视频任务。接口固定为异步模式：创建成功后立即返回 `task_id`，再通过 [MiniMax H3 任务查询 API](/documents/minimax-tasks-integration) 获取状态和成片。

MiniMax H3 可以统一理解文本、图片、视频和音频。你不需要传 `action` 来选择生成模式，接口会根据 `content` 中的素材类型和 `role` 自动判断用途。
...
1. 打开 [控制台的应用管理](/console/applications)，创建或选择一个应用。
2. 获取该应用的 API Token。一个 Token 可以调用平台上的所有已开通服务，无需为 MiniMax 单独申请。
3. 将 Token 放入请求头，格式为 `Authorization: Bearer YOUR_API_KEY`。

如果尚未登录，控制台会先引导你完成注册或登录。首次申请通常包含体验额度；通用余额可在 [余额页面](/console/coin) 查看和充值。
...
1. 调用 `/minimax/videos` 创建任务，保存响应中的 `task_id`。
2. 每隔约 10 秒调用 `/minimax/tasks` 查询一次，避免过于频繁地轮询。
...
| `callback_url` | string | 否 | - | 接收任务状态变化的公网可访问回调 URL |
...
接口不接受旧版或兼容字段，例如 `prompt`、`image_urls`、`audio_urls`、`messages`、`first_frame_image` 和 `async`。提示词和所有素材都必须放在 `content` 中。
...
传入 `callback_url` 后，回调服务需要处理两个阶段：

1. **地址验证**：平台先发送带 `challenge` 的 POST 请求。你的服务必须在 3 秒内原样返回该 `challenge`。
2. **状态通知**：验证通过后，任务状态变化时会向该地址 POST 数据，结构与任务查询响应一致。

回调中的状态包括 `queued`、`running`、`succeeded`、`failed` 和 `cancelled`。即使使用回调，也建议保存 `task_id`，以便主动查询或补偿漏掉的通知。
...
创建接口只代表任务已成功进入队列，不代表视频已经生成。只有任务最终为 `succeeded` 时才会计费；查询任务本身免费，不会重复扣费。
