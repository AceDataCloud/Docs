...
`POST https://api.acedata.cloud/minimax/tasks` 用于查询、批量列出或删除 [MiniMax H3 视频生成 API](/documents/minimax-videos-integration) 创建的异步任务。

创建视频后必须保存 `task_id`。推荐每隔约 10 秒查询一次，直到任务进入终态；任务查询免费，不会产生重复计费。

## 获取 API Token

1. 打开 [控制台的应用管理](/console/applications)，创建或选择一个应用并获取 API Token。
2. 在请求头中传入 `Authorization: Bearer YOUR_API_KEY`。
3. 查询任务时应使用创建该任务的同一个 Token。
