...
要使用 Localization Translate API，首先到 [Ace Data Cloud 控制台](https://platform.acedata.cloud/console/applications) 获取您的 API Token，留作备用。
...
> 📘 完整文档：[Localization Translate API →](https://platform.acedata.cloud/documents/localization-translate)
...
- `input`：需要翻译的内容。`extension=md` 时传字符串，`extension=json` 时传 JSON 对象。
- `locale`：自定义翻译的语言。
- `extension`：输入内容格式，可选 `md` 或 `json`。
- `model`：翻译采用的大模型，默认是 gpt-3.5。

| `extension` | `input` 类型 | 示例 |
| --- | --- | --- |
| `md` | string | `"# Title\n\nParagraph"` |
| `json` | object | `{"title": "Hello", "button": "Submit"}` |

`extension` 必须与 `input` 类型匹配；字符串不能使用 `json`，JSON 对象不能使用 `md`。
