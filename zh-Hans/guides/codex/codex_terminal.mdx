...
> 本文使用 `env_key = "ACEDATACLOUD_API_KEY"`，Token 只从环境变量读取。不要同时启用 `requires_openai_auth` 或保留 `~/.codex/auth.json` 中的旧登录凭据，否则 Codex 可能读取错误或过期的 Token 并返回 401。
...
| `gpt-6-astra` | 最新旗舰，最复杂的端到端推理与编程任务首选 |
| `gpt-5.6-sol` | 高要求复杂推理与编程任务 |
...
- `untrusted`：Agent 仅允许读取和分析，更适合不熟悉的项目。
