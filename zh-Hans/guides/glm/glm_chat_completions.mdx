...
本文档主要介绍 GLM Chat Completion API 的使用流程，利用它您可以通过统一的 OpenAI 兼容接口轻松调用 GLM 系列模型。需要在 Coze（扣子）中使用 `glm-5.2` 时，请参考 [Coze 自定义模型教程](https://platform.acedata.cloud/documents/coze-integration)，并选择 OpenAI Chat Completions 协议。
...
- `model`：选择要调用的 GLM 模型，目前支持的模型包括：
  - `glm-5.2`：最新旗舰模型，适合复杂推理、代码与 Agent 任务。
  - `glm-5.1`：上一代旗舰模型，综合能力强。
...
| 模型           | 适用场景                                       |
| -------------- | ---------------------------------------------- |
| `glm-5.2`      | 最新旗舰，推荐用于复杂推理、代码与 Agent 任务  |
| `glm-5.1`      | 上一代旗舰，适合复杂推理、长文档分析           |
