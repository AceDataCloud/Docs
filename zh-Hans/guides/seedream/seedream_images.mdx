...
- `prompt`：提示词。
- `model`：生成模型，默认 `doubao-seedream-5-0-260128`（SeeDream 5.0，最新）。支持 `doubao-seedream-5-0-260128`、`doubao-seedream-4-5-251128`、`doubao-seedream-4-0-250828`、`doubao-seedream-3-0-t2i-250415`、`doubao-seededit-3-0-i2i-250628`。**`model` 必须传入完整模型串（如 `doubao-seedream-5-0-260128`），传 `doubao-seedream-5.0-lite` 这类简写会返回 400。**
- `image`: 输入的图片信息，支持 URL 或 Base64 编码。其中，`doubao-seedream-5-0-260128`、`doubao-seedream-4-5-251128`、`doubao-seedream-4-0-250828` 支持单图或多图输入，`doubao-seededit-3-0-i2i-250628` 仅支持单图输入，`doubao-seedream-3-0-t2i-250415` 不支持该参数。
- `size`: 指定生成图像的尺寸信息，支持以下两种方式，不可混用。方式 1 | 指定生成图像的分辨率，并在 prompt 中用自然语言描述图片宽高比。**各模型支持的预设不同**：`doubao-seedream-5-0-260128` 支持 `2K`/`3K`/`4K`；`doubao-seedream-4-5-251128` 仅支持 `2K`/`4K`；`doubao-seedream-4-0-250828` 支持 `1K`/`2K`/`4K`；`doubao-seedream-3-0-t2i-250415` 与 `doubao-seededit-3-0-i2i-250628` **不支持预设**，仅接受方式 2。方式 2 | 指定生成图像的宽高像素值：默认 `2048x2048`，总像素与宽高比取值范围随模型不同（例如 5.0 / 4.5 总像素下限 3,686,400，4.0 下限 921,600，3.0-t2i / seededit-3.0-i2i 范围 [512x512, 2048x2048]）。
...
- `output_format`: 指定生成图像的文件格式，支持 `jpeg`（默认）和 `png`。仅 `doubao-seedream-5-0-260128` 支持。
