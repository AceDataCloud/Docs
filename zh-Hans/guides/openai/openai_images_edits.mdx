...
OpenAI 图片编辑服务，可以传入任意多张图片和指令，输出修改之后的图片。目前接口同时支持 `dall-e-2`、`gpt-image-1`、最新的 **`gpt-image-2`**，以及通过同一接口接入的 **`nano-banana` / `nano-banana-2-lite` / `nano-banana-2` / `nano-banana-pro`** 系列模型。
...
- **`gpt-image-2:official`**：官方中转线路。支持 `n > 1`（一次返回多张）与真实 2K / 4K，**按每张图片计费，单价为默认 `gpt-image-2` 的 2 倍**。当前仅由 openai-hk 渠道提供，线路不可用时直接返回错误，不会降级到逆向线路。
- **`gpt-image-2:reverse`**：与默认 `gpt-image-2` 完全等价（逆向线路），价格不变。
...
> **关于 `n` 参数**
>
> `gpt-image-2` 编辑接口目前**不支持 `n > 1`**：该参数会被静默忽略，无论传 `n=1` 还是 `n=10`，单次请求都只会返回 1 张图，并且只按 1 张计费。如果你需要一次拿到多张候选编辑结果，请**自行并发发起多次请求**。该限制同样适用于 `gpt-image-1` / `gpt-image-1.5`，以及 `nano-banana` / `nano-banana-2-lite` / `nano-banana-2` / `nano-banana-pro` 系列。`dall-e-2` 是目前唯一原生支持 `n > 1` 的编辑模型。
