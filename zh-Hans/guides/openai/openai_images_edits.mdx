...
尺寸规则与生成接口一致：宽高须为 16 的倍数，长边不超过 3840，总像素不超过 8,294,400。省略 `size` 或使用 `auto` 时，模型会结合提示词和第一张参考图选择画幅。

默认 `gpt-image-2` 与 `:reverse` 按成功张数计费；`gpt-image-2:official` 根据文字输入、参考图输入和图片输出的实际 Token 结算，最终以使用记录为准。
...
完整字段和实时枚举以 [OpenAI Images Edits API](https://platform.acedata.cloud/documents/openai-images-edits) 页面为准。纯文本生成图片请查看 [GPT Image 2 图像生成](https://platform.acedata.cloud/documents/openai-images-generations)。
