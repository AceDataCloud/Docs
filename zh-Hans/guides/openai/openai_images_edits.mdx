...
| 字段 | 说明 |
| --- | --- |
| `model` | `gpt-image-2`、`gpt-image-2.5-flare`（更快）、`gpt-image-2.5-sunburst`（更高保真与控制）、`gpt-image-2:official` 或 `gpt-image-2:reverse` |
...
`gpt-image-2`、`gpt-image-2.5-flare`、`gpt-image-2.5-sunburst` 与 `:reverse` 按成功张数计费；`gpt-image-2:official` 根据文字输入、参考图输入和图片输出的实际 Token 结算，最终以使用记录为准。
