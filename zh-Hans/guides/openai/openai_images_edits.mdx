...
`mask` 目前以 multipart 文件字段提供。需要通过 URL 传入原图时，也请将请求改为 multipart：`image` 可以继续作为 URL 文本字段，`mask` 使用 `@mask.png` 上传。蒙版会约束编辑区域，但生成模型仍可能对边缘进行自然融合；需要严格边界时，应使用清晰的 Alpha 边缘并在提示词中重复说明哪些内容必须保持不变。
