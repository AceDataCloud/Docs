---
title: "Nano Banana Images API 对接说明"
...
- **模型（model）**（可选）：
  ...
  - `nano-banana:official`、`nano-banana-2:official`、`nano-banana-pro:official`：对应模型的官方渠道版本，画质与稳定性更佳，计费不同
...
- **可选**：`model`（默认 `nano-banana`，可选 `nano-banana-2`、`nano-banana-pro`，或对应的 `:official` 官方渠道版本）、`aspect_ratio`（宽高比，如 `1:1`、`16:9`）、`resolution`（分辨率，如 `1K`、`2K`、`4K`）、`callback_url`（用于异步回调）
