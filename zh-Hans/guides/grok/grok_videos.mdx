...
- `grok-imagine-video-1.5-fast`（默认）：支持文生视频（仅传 `prompt`）和图生视频（传 `image_url`），时长最长 30 秒，价格更低。
- `grok-imagine-video-1.5`：仅支持图生视频，**必须**传入 `image_url`，时长最长 15 秒。
...
- `prompt`：描述想要生成视频内容的文本提示词。使用 `grok-imagine-video-1.5-fast` 做文生视频时**必填**；传入 `image_url` 时可选。
- `model`：生成视频的模型，可选 `grok-imagine-video-1.5-fast`（默认）或 `grok-imagine-video-1.5`。
- `image_url`：图生视频的输入图片链接。当 `model` 为 `grok-imagine-video-1.5` 时**必填**。
...
- `duration`：生成视频的时长（秒）。`grok-imagine-video-1.5-fast` 取值范围 1–30，`grok-imagine-video-1.5` 取值范围 1–15，默认 6。推荐使用 6 秒或 10 秒，这两个标准时长相对稳定。
...
如果想基于一张输入图片生成视频，可以传入 `image_url`。使用 `grok-imagine-video-1.5` 时必须提供该字段：
...
- `grok-imagine-video-1.5-fast`：按时长分档计费，与分辨率无关——`1–10` 秒、`11–20` 秒、`21–30` 秒分别对应不同档位价格。
- `grok-imagine-video-1.5`：按「输出秒数」计费，总价 = 单价 × `duration`，`1080p` 比 `480p`/`720p` 更贵。
...
- `400`：请求参数有误，例如文生视频缺少 `prompt`，或 `grok-imagine-video-1.5` 缺少 `image_url`，或 `duration` 超出范围（`grok-imagine-video-1.5-fast` 为 1–30，`grok-imagine-video-1.5` 为 1–15）。
