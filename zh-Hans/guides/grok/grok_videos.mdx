...
- `grok-imagine-video`（默认）：支持文生视频（仅传 `prompt`）和图生视频（传 `image_url`），时长最长 30 秒，价格更低。
- `grok-imagine-video-1.5-preview`：仅支持图生视频，**必须**传入 `image_url`，时长最长 15 秒。
...
- `duration`：生成视频的时长（秒）。`grok-imagine-video` 取值范围 1–30，`grok-imagine-video-1.5-preview` 取值范围 1–15，默认 8。
...
- `grok-imagine-video`：按时长分档计费，与分辨率无关——`1–10` 秒、`11–20` 秒、`21–30` 秒分别对应不同档位价格。
- `grok-imagine-video-1.5-preview`：按「输出秒数」计费，总价 = 单价 × `duration`，`1080p` 比 `480p`/`720p` 更贵。
...
- `400`：请求参数有误，例如文生视频缺少 `prompt`，或 `grok-imagine-video-1.5-preview` 缺少 `image_url`，或 `duration` 超出范围（`grok-imagine-video` 为 1–30，`grok-imagine-video-1.5-preview` 为 1–15）。
