...
在 Chat Completions 格式中，`usage.prompt_tokens` 已包含缓存命中的输入 token。计费时会根据 `prompt_tokens_details.cached_tokens` 和 `cached_creation_tokens` 对相应部分应用缓存读取与写入价格，不会把缓存 token 重复计算为普通输入。
