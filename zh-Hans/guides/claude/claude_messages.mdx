...
- `usage`：token 使用统计。`input_tokens` 是未缓存输入；`cache_creation_input_tokens` 和 `cache_read_input_tokens` 分别是缓存写入与读取；`output_tokens` 是输出 token 数。Fable 5.1 的官方缓存读取基价为 $0.25/百万 Token，5 分钟与 1 小时缓存写入基价分别为 $12.50 和 $20/百万 Token；平台实际价格按套餐折扣换算。非流式响应还可能包含 Ace Data Cloud 记录的 `cost`。
