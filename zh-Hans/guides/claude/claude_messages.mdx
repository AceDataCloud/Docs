...
- `model`：选择使用的 Claude 模型。`claude-opus-5-5` 仅通过 Messages API 提供，支持 100 万 Token 上下文、最大输出 128K Token，并始终启用自适应思考。最新旗舰为 `claude-fable-5-1`（100 万 Token 上下文、最大输出 128K Token）；原 `claude-fable-5` 仍兼容保留。
