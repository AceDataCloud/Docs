...
要使用 hCaptcha 协议识别 API，首先到 [Ace Data Cloud 控制台](https://platform.acedata.cloud/console/applications) 获取您的 API Token，留作备用。
...
> 📘 完整文档：[hCaptcha 协议识别 API →](https://platform.acedata.cloud/documents/captcha-token-hcaptcha)
...
- `rqdata`：可选。hCaptcha Enterprise 挑战可能在页面中提供 `data-rqdata`，遇到此类挑战时填写其原始值；普通 hCaptcha 无需填写。
- `proxy`：可选，自带代理（Bring Your Own Proxy）。设置后系统会通过你提供的代理 IP 去解验证码，用于控制出口 IP 质量（例如避免因公共代理 IP 被目标站点拦截而返回 `410 Gone`）。格式为 `scheme://[user:pass@]host:port`，`scheme` 支持 `http`/`https`/`socks4`/`socks5`，例如 `http://user:pass@1.2.3.4:8080`。不填则使用平台默认代理。
