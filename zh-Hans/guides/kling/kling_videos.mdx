...
- `element_list`：主体参考列表，只适用模型 `kling-video-o1`，用于引用主体库中的元素 ID，用法见下文「Omni 全能参考」。
- `image_list`：参考图片列表（多图参考），只适用模型 `kling-video-o1`，用法见下文「Omni 全能参考」。
- `video_list`：参考视频列表（支持视频编辑），只适用模型 `kling-video-o1`，用法见下文「Omni 全能参考」。
- `prompt`：提示词。
...
使用时需在 `prompt` 中以 `&lt;&lt;<image_1>>>`、`&lt;&lt;<image_2>>>` 引用。数量限制：不存在参考视频时，参考图片 + 多图主体之和 ≤ 7；存在参考视频时 ≤ 4。仅传首 / 尾帧时也可直接用 `start_image_url` / `end_image_url`。
...
`element_list` 用于引用「主体库」中已上传的元素，数组元素字段为 `element_id`（主体库中的元素 ID，整数）。使用时同样需在 `prompt` 中以 `&lt;&lt;<element_1>>>` 引用。参考主体数量需与参考图片数量合并计算：存在参考视频时两者之和 ≤ 4，不存在参考视频时 ≤ 7。
