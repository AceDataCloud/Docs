...
- `proxy`：可选，自带代理（Bring Your Own Proxy）。设置后服务会通过你提供的代理 IP 去解验证码，用于控制出口 IP 质量（例如避免因公共代理 IP 被目标站点拦截而返回 `410 Gone`）。格式为 `scheme://[user:pass@]host:port`，`scheme` 支持 `http`/`https`/`socks4`/`socks5`，例如 `http://user:pass@1.2.3.4:8080`。不填则使用平台默认代理。
